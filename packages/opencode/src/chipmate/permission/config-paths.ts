...
import { ChipMatePaths } from "@/chipmate/paths"
...
export namespace ConfigProtection {
   ...
   * Matches .chipmate/ and legacy .chipmate/ at any depth within the project.
   */
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["chipmate.json", "chipmate.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".chipmate"
      // Match at root (e.g. ".chipmate/foo") or nested (e.g. "packages/sub/.chipmate/foo")
  ...
  function configs(): string[] {
    ...
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "chipmate") : ""]),
    ).filter(Boolean)
  }
  ...
  function skillRoot(pattern: string): string | undefined {
    ...
    const roots = [...configs(), ...ChipMatePaths.globalDirs()]
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/chipmate") ||
        key.includes("/config/chipmate/") ||
        key.endsWith("/.config/chipmate") ||
        key.includes("/.config/chipmate/"),
    )
  }
