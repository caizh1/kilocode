---
title: "Browser Use"
description: "Using ChipMate to interact with web browsers"
---

# Browser Use

ChipMate provides browser automation capabilities that let you interact with websites directly from your coding workflow. This feature supports testing web applications, automating browser tasks, and capturing screenshots without leaving your editor.
...
Browser automation is built into the extension and requires no manual setup. Enable it from **Settings → Web Tools → Browser Automation** and ChipMate handles the rest automatically.

{% /tab %}
{% tab label="CLI" %}

ChipMate uses [Playwright](https://playwright.dev/) for browser automation. Add it to your `chipmate.jsonc` configuration:
...
1. Ask ChipMate to visit a website
2. ChipMate launches the browser and shows you a screenshot
3. Request additional actions (clicking, typing, scrolling)
4. ChipMate closes the browser when finished

For example:

- `Open the browser and view our site.`
- `Can you check if my website at https://chipmate.ai is displaying correctly?`
...
ChipMate launches a browser automatically when asked and returns screenshots after each action so you can see what's happening. It can navigate to URLs, click elements, fill in forms, scroll, hover, select from dropdowns, and drag and drop — all driven by natural language instructions in chat.
...
Browser automation is configured in your `chipmate.jsonc` file. No additional settings are required — Playwright manages the browser lifecycle automatically.
