...
internal class AgentCreateDialog(private val names: Collection<String>) : DialogWrapper(true), AgentCreateDialogHandle {
    ...
    private val mode = ComboBox(arrayOf(ChipMateCliParser.MODE_PRIMARY, ChipMateCliParser.MODE_SUBAGENT, ChipMateCliParser.MODE_ALL)).apply {
        selectedItem = ChipMateCliParser.MODE_PRIMARY
    }
    ...
    init {
        title = ChipMateBundle.message("settings.agentBehavior.agents.create.title")
        init()
        initValidation()
    }
    ...
    override fun result(): AgentCreateDto = AgentCreateDto(
        ...
        mode = mode.selectedItem?.toString() ?: ChipMateCliParser.MODE_PRIMARY,
    ...
    override fun createCenterPanel(): JComponent {
        ...
        SettingsRows().apply {
            row(SettingsRow(
                ChipMateBundle.message("settings.agentBehavior.agents.create.name"),
                description = ChipMateBundle.message("settings.agentBehavior.agents.create.name.description"),
                value = id,
            ))
            row(SettingsRow(
                ChipMateBundle.message("settings.agentBehavior.agents.create.mode"),
                description = ChipMateBundle.message("settings.agentBehavior.agents.create.mode.description"),
                value = mode,
            ))
            row(SettingsRow(
                ChipMateBundle.message("settings.agentBehavior.agents.create.scope"),
                description = ChipMateBundle.message("settings.agentBehavior.agents.create.scope.description"),
                value = scope,
            ))
            row(SettingsStackedRow(
                ChipMateBundle.message("settings.agentBehavior.agents.create.description"),
                description = ChipMateBundle.message("settings.agentBehavior.agents.create.description.description"),
                value = box(description),
            ))
            row(SettingsStackedRow(
                ChipMateBundle.message("settings.agentBehavior.agents.create.prompt"),
                description = ChipMateBundle.message("settings.agentBehavior.agents.create.prompt.description"),
                value = box(prompt),
            ))
            panel.next(this)
        }
    ...
    override fun getDimensionServiceKey(): String = "ChipMate.AgentCreateDialog"

    override fun doValidateAll(): List<ValidationInfo> = validateAgentCreate(result(), names).map { err ->
        ValidationInfo(ChipMateBundle.message(err.key), component(err.field))
    }
    ...
    private class PromptField : EditorTextField(
        ...
        init {
            ...
            setPlaceholder(ChipMateBundle.message("settings.agentBehavior.agents.create.prompt.placeholder"))
...
private fun projectLabel() = ChipMateBundle.message("settings.agentBehavior.agents.create.scope.project")

private fun globalLabel() = ChipMateBundle.message("settings.agentBehavior.agents.create.scope.global")
