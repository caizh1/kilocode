...
ChipMate uses tools to interact with your code and environment. These specialized helpers perform specific actions like reading files, making edits, running commands, or searching your codebase. Tools provide automation for common development tasks without requiring manual execution.

## Tool Workflow

Describe what you want to accomplish in natural language, and ChipMate will:
...
**ChipMate:** (Proposes the `write` tool)
...
Every tool use is subject to a permission check. The default action for any tool with no matching rule in your config is **`ask`** — meaning ChipMate will pause and prompt you before executing it.
...
| `websearch` | Searches the web (ChipMate/OpenRouter users) | Web |
