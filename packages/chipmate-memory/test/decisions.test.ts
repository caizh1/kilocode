...
describe("memory marker metadata", () => {
  ...
  test("decodes legacy metadata that only carries sources", () => {
    expect(
      MemoryMarkerMeta.fromParts([
        { type: "text", metadata: { chipmateMemory: { type: "startup", tokens: 3, sources: ["project.md"] } } },
      ]),
    ).toEqual({
      type: "startup",
      tokens: 3,
  ...
  test("persists recall snippets only in verbose mode", () => {
    ...
    expect(MemoryMarkerMeta.metadata(marker).chipmateMemory).not.toHaveProperty("items")
    expect(MemoryMarkerMeta.metadata(marker, true).chipmateMemory).toHaveProperty("items", ["private memory"])
  })
