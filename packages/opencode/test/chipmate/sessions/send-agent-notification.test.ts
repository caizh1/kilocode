...
beforeEach(() => {
  originalNotificationTimeout = process.env.CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS
  process.env.CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS = "50"
})

afterEach(() => {
  if (originalNotificationTimeout === undefined) {
    delete process.env.CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS
  } else {
    process.env.CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS = originalNotificationTimeout
  }
})
...
it.instance("dedicated immediate POST hits ingest path with an agent_notification item", () => {
  const originalKey = process.env.CHIPMATE_API_KEY
  const originalIngest = process.env.CHIPMATE_SESSION_INGEST_URL
  ...
  process.env.CHIPMATE_API_KEY = "test-token"
  process.env.CHIPMATE_SESSION_INGEST_URL = "https://ingest.chipmatesessions.ai"
  reset("test-token")

  return Effect.gen(function* () {
    const sessions = yield* ChipMateSessions.Service
    yield* Effect.promise(() => ChipMateSessions.bootstrap("session-1"))
    ...
    Effect.ensuring(
      Effect.gen(function* () {
        ...
        if (originalKey === undefined) delete process.env.CHIPMATE_API_KEY
        else process.env.CHIPMATE_API_KEY = originalKey
        if (originalIngest === undefined) delete process.env.CHIPMATE_SESSION_INGEST_URL
        else process.env.CHIPMATE_SESSION_INGEST_URL = originalIngest
        reset("test-token")
        request.mockRestore()
      }),
    ),
...
it.instance("2xx ingest response returns ok:true", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* ChipMateSessions.Service
    yield* Effect.promise(() => ChipMateSessions.bootstrap("session-2"))
...
it.instance("non-2xx ingest response returns ok:false with no retry", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* ChipMateSessions.Service
    yield* Effect.promise(() => ChipMateSessions.bootstrap("session-3"))
...
it.instance("coalescing: sendAgentNotification awaits in-flight bootstrap for a single /api/session POST", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* ChipMateSessions.Service
    const createPromise = ChipMateSessions.create("session-6")
...
it.instance("coalescing: bootstrapInflight stores the real promise and concurrent create() calls share one POST", () => {
  ...
  return Effect.gen(function* () {
    ...
    expect(ChipMateSessions._getBootstrapInflight("session-7")).toBeUndefined()

    // Fire two concurrent create() calls for the same session.
    const create1 = ChipMateSessions.create("session-7")
    const create2 = ChipMateSessions.create("session-7")
    ...
    const inflight = ChipMateSessions._getBootstrapInflight("session-7")
