...
2. Call `create_word_document` once with a lightweight structural skeleton: title, document metadata, summary and `{{TOC}}`, the canonical chapter order, unique H2/H3 insertion anchors, and every successfully rendered image block in its final semantic location. Exclude long prose, long lists, large tables, and code bodies from this initial call.
3. Fill the skeleton in canonical order with serial `apply_word_document_edits` calls. One main chapter or one important-submodule design unit is the maximum batch size, not a minimum; split a larger unit across unique H2/H3 anchors. Insert non-image paragraphs, lists, tables, and code after each unique anchor exactly once so repeated `insert_after_heading` calls cannot reverse content order.
...
5. After all chapter batches and late image repairs are complete, call `materialize_word_fields`, inspect the final outline and image count, and only then run `render_word_document` and page visual review.
...
The default Word format is Codex `standard_business_brief`: US Letter portrait, one-inch margins, Calibri 11 pt body text, the standard blue H1/H2/H3 ladder, real numbering, fixed-DXA tables, quiet running header, and a right-aligned page number. For a full detailed design, place a `{{TOC}}` placeholder after the title/summary and use the existing `materialize_word_fields` tool only after all chapter and late-image mutations so the table of contents is derived from the final real heading structure.
...
Before calling `create_word_document`, confirm that every successful render in the diagram map has a corresponding image block in the lightweight skeleton and that every later content batch has a unique heading anchor. Do not require the initial call to carry the document's long-form prose, tables, lists, or code. After all serial chapter edits, field operations, and late image insertions, call `inspect_word_document` and require `imageCount` to equal the number of successfully rendered figures planned for insertion. If the counts differ, reuse the existing PNGs with `insert_mermaid_into_word` for the missing figures only, then inspect again; do not regenerate unrelated content.
...
After any Word repair, re-run `inspect_word_document` against the authoritative path and compare both structure and figures with the plan. The expected outline and its order, every target-module and important-submodule design-unit heading, later cross-module chapters, captions, image relationships, and image count must still be present. An image-count match does not compensate for a missing chapter, a collapsed image relationship, a repeated wrong figure, or a reordered design unit.
