...
import { createWordDocument, inspectWordDocument, renderWordDocument } from "../../src/kilocode/documents/word"
import { provideTmpdirInstance } from "../fixture/fixture"

const mermaid = process.env["KILO_MERMAID_RENDER_ENDPOINT"]
const word = process.env["KILO_WORD_RENDER_ENDPOINT"]
const enabled = process.env["KILO_RENDER_INTEGRATION"] === "1" && Boolean(mermaid && word)
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.promise(async () => {
            ...
            const output = process.env["KILO_RENDER_ACCEPTANCE_DIR"]
            if (!output) return
