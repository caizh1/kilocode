...
import { RenderPlantUmlDiagramTool } from "../../src/kilocode/tool/plantuml-diagram"
...
async function withEndpoint<T>(endpoint: string | undefined, fn: () => Promise<T>) {
  const previous = process.env["KILO_PLANTUML_RENDER_ENDPOINT"]
  if (endpoint === undefined) delete process.env["KILO_PLANTUML_RENDER_ENDPOINT"]
  if (endpoint !== undefined) process.env["KILO_PLANTUML_RENDER_ENDPOINT"] = endpoint
  try {
    return await fn()
  } finally {
    if (previous === undefined) delete process.env["KILO_PLANTUML_RENDER_ENDPOINT"]
    if (previous !== undefined) process.env["KILO_PLANTUML_RENDER_ENDPOINT"] = previous
  }
}
...
describe("render_plantuml_diagram", () => {
  ...
  test.serial("rejects missing endpoint, unverified metadata, invalid PNG, and dimension mismatches", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(missing.metadata.error).toContain("KILO_PLANTUML_RENDER_ENDPOINT")
  ...
  test("requires an explicit destination choice and keeps the tool scoped to Ask, Code, and Plan", async () => {
    ...
    expect(KiloToolRegistry.available(def, agent("ask"))).toBe(true)
    expect(KiloToolRegistry.available(def, agent("code"))).toBe(true)
    expect(KiloToolRegistry.available(def, agent("plan"))).toBe(true)
    expect(KiloToolRegistry.available(def, agent("debug"))).toBe(false)
    expect(KiloToolRegistry.available(def, agent("explore"))).toBe(false)
    expect(KiloToolRegistry.available(def, agent("compaction"))).toBe(false)
  })
})
