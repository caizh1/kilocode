...
import { Badge } from "@chipmate/chipmate-web-ui/badge"
import { Button } from "@chipmate/chipmate-web-ui/button"
import { Card } from "@chipmate/chipmate-web-ui/card"
import { Icon } from "@chipmate/chipmate-web-ui/icon"
import { ResizeHandle } from "@chipmate/chipmate-web-ui/resize-handle"
import { Spinner } from "@chipmate/chipmate-web-ui/spinner"
import { File } from "@chipmate/chipmate-web-ui/file"
import { FileComponentProvider } from "@chipmate/chipmate-web-ui/context/file"
import { SessionReview, type SessionReviewDiffStyle } from "@chipmate/chipmate-web-ui/session-review"
...
export function ProjectConsoleRoute() {
  ...
  const [selected, setSelected] = createSignal(window.localStorage.getItem(`chipmate.console.${params.project}.dir`) ?? "")
  const [active, setActive] = createSignal(window.localStorage.getItem(`chipmate.console.${params.project}.pty`) ?? "")
  ...
  function labelKey(dir: string) {
    return `chipmate.console.${project()}.worktree.${encodeURIComponent(dir)}.label`
  }
  ...
  function remember(dir: string, pty?: string) {
    window.localStorage.setItem(`chipmate.console.${project()}.dir`, dir)
    if (pty) {
      window.localStorage.setItem(`chipmate.console.${project()}.pty`, pty)
      return
    }
    window.localStorage.removeItem(`chipmate.console.${project()}.pty`)
  }
  ...
  function submitEditor() {
    ...
    run("Creating worktree", async () => {
      ...
      window.localStorage.setItem(`chipmate.console.${project()}.dir`, next.directory)
    })
  }
  ...
  function addSession(item = current()) {
    ...
    const label = `ChipMate ${terminalsFor(item.dir).length + 1}`
      ...
      <main class="project-console-main">
        ...
        <Show when={!terminal() && !snap.loading && !snap.error && !failure()}>
          <div class="project-terminal-empty">
            ...
            <span>Use + next to a worktree to start ChipMate CLI.</span>
          </div>
        </Show>
      </main>
