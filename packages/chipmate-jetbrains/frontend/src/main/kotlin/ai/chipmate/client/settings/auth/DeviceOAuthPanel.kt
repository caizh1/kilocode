package ai.chipmate.client.settings.auth
...
internal class DeviceOAuthPanel(
    ...
    val qrLabel = JBLabel().apply {
        ...
        name = "$prefix.qr"
        accessibleContext.accessibleName = ChipMateBundle.message("profile.login.qr")
        accessibleContext.accessibleDescription = copy.qrDescription
    }

    private val openBtn = JButton(ChipMateBundle.message("profile.login.openBrowser"))
    private val cancelBtn = JButton(ChipMateBundle.message("profile.login.cancel")).also { it.addActionListener { cancel() } }
    private val copyUrlBtn = HoverIcon().apply {
        icon = AllIcons.Actions.Copy
        toolTipText = ChipMateBundle.message("profile.login.copyUrl")
    }
    private val codePanel = RoundedContentPanel(UiStyle.Gap.sm(), UiStyle.Gap.md()).apply {
        ...
        addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(e: MouseEvent) {
                val c = code ?: return
                copyToClipboard(c, ChipMateBundle.message("profile.login.codeCopied"), this@DeviceOAuthPanel)
            }
        })
    }
    ...
    private val codeHint = JBLabel(ChipMateBundle.message("profile.login.clickToCopy")).apply {
        foreground = UiStyle.Colors.weak()
        horizontalAlignment = SwingConstants.CENTER
    }
    private val waitIcon = AsyncProcessIcon("ChipMateOAuth")
    ...
    private fun build() {
        ...
        add(stepLabel(ChipMateBundle.message("profile.login.step.one"), ChipMateBundle.message("profile.login.step.url")), gbc(row++, UiStyle.Gap.md()))
        add(urlRow(), gbc(row++, UiStyle.Gap.sm()))
        add(qrLabel, gbc(row++, UiStyle.Gap.md()).centered())
        val s2 = stepLabel(ChipMateBundle.message("profile.login.step.two"), ChipMateBundle.message("profile.login.step.code"))
        step2 = s2
    ...
    fun update(info: DeviceOAuthInfo) {
        ...
        if (info.url != last) {
            ...
            copyUrlBtn.addActionListener { copyToClipboard(info.url, ChipMateBundle.message("profile.login.urlCopied"), copyUrlBtn) }
            qrLabel.icon = try {
    ...
    private fun syncTime() {
        ...
        val min = remain / 60
        val sec = remain % 60
        waitLabel.text = ChipMateBundle.message("profile.login.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
    }
