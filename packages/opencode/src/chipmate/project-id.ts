...
/**
 * Read project ID from .chipmate/config.json.
...
async function getProjectIdFromConfig(directory: string): Promise<string | undefined> {
  // Check each product-profile config directory in priority order.
...
/**
 * Resolve project ID with priority: product config -> git origin URL.
 * @returns Normalized project ID or undefined
 */
...
export namespace ChipMateProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@chipmate/ChipMateProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("ChipMateProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(ChipMateProjectID.Service, ChipMateProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getChipMateProjectId(): Promise<string | undefined> {
