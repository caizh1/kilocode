...
import { Daemon } from "../../src/chipmate/daemon/daemon"
import { DaemonClient } from "../../src/chipmate/daemon/client"
import { tmpdir } from "../fixture/fixture"

const original = {
  state: process.env.CHIPMATE_TEST_DAEMON_STATE_DIR,
  log: process.env.CHIPMATE_TEST_DAEMON_LOG_DIR,
  disabled: process.env.CHIPMATE_NO_DAEMON,
}

afterEach(async () => {
  if (process.env.CHIPMATE_TEST_DAEMON_STATE_DIR !== original.state) await Daemon.stop().catch(() => undefined)
  restore()
})

function restore() {
  if (original.state === undefined) delete process.env.CHIPMATE_TEST_DAEMON_STATE_DIR
  else process.env.CHIPMATE_TEST_DAEMON_STATE_DIR = original.state
  if (original.log === undefined) delete process.env.CHIPMATE_TEST_DAEMON_LOG_DIR
  else process.env.CHIPMATE_TEST_DAEMON_LOG_DIR = original.log
  if (original.disabled === undefined) delete process.env.CHIPMATE_NO_DAEMON
  else process.env.CHIPMATE_NO_DAEMON = original.disabled
}

function dirs(root: string) {
  process.env.CHIPMATE_TEST_DAEMON_STATE_DIR = path.join(root, "state")
  process.env.CHIPMATE_TEST_DAEMON_LOG_DIR = path.join(root, "log")
  return {
    XDG_DATA_HOME: path.join(root, "xdg-data"),
    XDG_CONFIG_HOME: path.join(root, "xdg-config"),
    XDG_STATE_HOME: path.join(root, "xdg-state"),
    XDG_CACHE_HOME: path.join(root, "xdg-cache"),
    CHIPMATE_TEST_DAEMON_EPHEMERAL_PORT: "1",
  }
}
...
function cli(args: string[], env: NodeJS.ProcessEnv = {}) {
  return Bun.spawn([process.execPath, "--conditions=browser", path.join(process.cwd(), "src/index.ts"), ...args], {
    ...
    env: {
      ...process.env,
      ...env,
      CHIPMATE_CONFIG_CONTENT: '{"experimental":{"openTelemetry":false}}',
...
describe("daemon manager", () => {
  ...
  test("does not forward bundled bun entrypoints to the daemon child", () => {
    const proc = {
      argv: ["/tmp/chipmate", "/$bunfs/root/src/index.js", "daemon", "start"],
      execArgv: ["--user-agent=chipmate/test", "--use-system-ca", "--"],
      execPath: "/tmp/chipmate",
    }
    expect(Daemon.command(undefined, proc)).toStrictEqual(["/tmp/chipmate"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/chipmate.exe", "B:/~BUN/root/src/index.js", "daemon", "start"],
        execPath: "C:/tmp/chipmate.exe",
      }),
    ).toStrictEqual(["C:/tmp/chipmate.exe"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/chipmate.exe", "b:\\~BUN\\root\\src\\index.js", "daemon", "start"],
  ...
  test("forwards source entrypoints to the daemon child", () => {
    expect(
      Daemon.command(undefined, {
        argv: ["/tmp/bun", "/tmp/chipmate/src/index.ts", "daemon", "start"],
        execArgv: ["--conditions=browser"],
        execPath: "/tmp/bun",
      }),
    ).toStrictEqual(["/tmp/bun", "--conditions=browser", "/tmp/chipmate/src/index.ts"])
  })
  ...
  test("does not reuse legacy fixed-password daemons", () => {
    ...
    const state: Daemon.State = {
      ...
      username: "chipmate",
      password: "chipmate",
      token: Buffer.from("chipmate:chipmate").toString("base64"),
      version: "test",
  ...
  test("starts, reuses, authenticates, and stops a daemon", async () => {
    ...
    expect(started.state?.password).not.toBe("chipmate")
    expect(started.state?.token).not.toBe(Buffer.from("chipmate:chipmate").toString("base64"))
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const source = path.join(process.cwd(), "src/chipmate/daemon/daemon.ts")
      const script = `
  ...
  test("supports console stop as a daemon stop alias", async () => {
    ...
    expect(code).toBe(0)
    expect(stdout).toContain("chipmate daemon stopped")
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const stdout = capture(proc.stdout, "Press Ctrl+C to stop the ChipMate daemon.")
      const stderr = new Response(proc.stderr).text()
      ...
      try {
        ...
        expect(await deadline(proc.exited, 10_000)).toBe(0)
        expect((await Daemon.status()).running).toBe(false)
        expect(await stdout.text).toContain("chipmate daemon started")
        await stderr
      } finally {
        if (proc.exitCode === null) proc.kill("SIGKILL")
        await proc.exited
  ...
  test("daemon client honors the escape hatch", async () => {
    await using tmp = await tmpdir()
    const started = await Daemon.start(opts(tmp.path))
    process.env.CHIPMATE_NO_DAEMON = "1"
