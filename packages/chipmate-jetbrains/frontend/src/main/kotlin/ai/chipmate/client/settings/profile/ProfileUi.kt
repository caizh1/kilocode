...
internal const val DASHBOARD_URL = "https://app.chipmate.ai/profile"
internal const val TOP_UP_URL = "https://app.chipmate.ai/credits"
internal const val PASS_URL = "https://chipmate.ai/pricing/chipmate-pass"
...
internal class ProfileUi(
    ...
    private val account = LoggedInProfileUi(
        ...
        pass = {
            telemetry("ChipMate Pass Opened", mapOf("surface" to "settings"))
            browse(PASS_URL)
        },
        logout = ::logout,
     ...
     * account content) when [ChipMateAppStateDto.progress]`.profile` is [ProfileStatusDto.PENDING],
     ...
     */
    @RequiresEdt
    fun update(state: ChipMateAppStateDto) {
        checkEdt()
     ...
     */
    @RequiresEdt
    fun update(profile: ProfileDto?, status: ChipMateAppStatusDto) {
        checkEdt()
        this.status = status
    ...
    private fun targetCard(): Card {
        ...
        val transientLoad = s == ChipMateAppStatusDto.DOWNLOADING || s == ChipMateAppStatusDto.CONNECTING || s == ChipMateAppStatusDto.LOADING || s == ChipMateAppStatusDto.MIGRATION_REQUIRED
        if (transientLoad && shown == Card.LOGGED_IN) return Card.LOGGED_IN
        return when {
            s == ChipMateAppStatusDto.DISCONNECTED || transientLoad -> Card.LOGGED_OUT
            s == ChipMateAppStatusDto.ERROR -> Card.LOGGED_OUT
     ...
     * Called from [ai.chipmate.client.settings.profile.UserProfileConfigurable.disposeUIResources].
     */
    @RequiresEdt
    fun dispose() {
        attempt++
...
internal fun compactLoginError(e: Exception): String {
    val msg = e.message?.trim() ?: return ChipMateBundle.message("profile.login.failed")
    val lower = msg.lowercase()
    if (HTML_MARKERS.any { lower.contains(it) }) {
        val status = HTTP_STATUS_RE.find(msg)?.groupValues?.getOrNull(1)
        return if (status != null) "${ChipMateBundle.message("profile.login.failed")} ($status)"
        else ChipMateBundle.message("profile.login.failed")
    }
    val norm = msg.replace(Regex("\\s+"), " ")
    val summary = norm.take(180)
    return if (summary.isNotBlank()) summary else ChipMateBundle.message("profile.login.failed")
}
