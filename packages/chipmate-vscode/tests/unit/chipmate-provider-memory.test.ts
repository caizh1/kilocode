...
import { ChipMateProviderMemory } from "../../src/chipmate-provider/memory"

function subject(client: ChipMateClient | undefined) {
  const posts: unknown[] = []
  const memory = new ChipMateProviderMemory({
    client: () => client,
    session: () => undefined,
    dir: () => "/repo",
...
describe("ChipMateProviderMemory", () => {
  it("shows stored memory and explains empty projects", async () => {
    ...
    try {
      ...
      expect(picker.mock.calls[0]?.[0]).toContainEqual(
        expect.objectContaining({ label: "Storage", detail: "/repo/.chipmate/memory" }),
      )
      expect(picker.mock.calls[0]?.[0]).toContainEqual(
  ...
  it("routes inspect operations to the memory folder", async () => {
    ...
    try {
      ...
      expect(reveal).toHaveBeenCalledWith("revealFileInOS", expect.objectContaining({ fsPath: "/repo/.chipmate/memory" }))
  ...
  it("handles clients without memory endpoints gracefully", async () => {
    const item = subject({} as ChipMateClient)
  ...
  it("evicts older cached memory payloads", async () => {
    let client: ChipMateClient | undefined
    const posts: unknown[] = []
    const item = new ChipMateProviderMemory({
      client: () => client,
      session: () => undefined,
    ...
    expect(posts[1]).toMatchObject({
      type: "memoryLoaded",
      sessionID: "ses_8",
      status: { root: "/repo/ses_8/.chipmate/memory" },
    })
  })
    ...
    const item = subject({
      memory: {
        configure: async (input: unknown) => {
          ...
          return { data: { root: "/repo/.chipmate/memory", state: state.state } }
        },
        purge: async (input: unknown) => {
          calls.push(["purge", input])
          return { data: { root: "/repo/.chipmate/memory", purged: true } }
        },
        status: async () => ({ data: state }),
        show: async () => ({ data: view }),
      },
