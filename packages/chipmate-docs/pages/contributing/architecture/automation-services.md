...
description: "Architecture of ChipMate Cloud automation services that dispatch scoped work"
---
...
This page describes cloud automation boundaries present in `ChipMate-Org/cloud`. Static source shows supported code paths, Worker bindings, and deployable surfaces. It does not prove live production enablement or rollout policy.
{% /callout %}
...
|---|---|---|---|---|
| ChipMate Bot | GitHub, Slack, or Linear command ingress | Web control plane bot libraries | Cloud Agent for requested repository work | Command response and coding-session result |
...
| Auto Fix | `chipmate-auto-fix` label or dispatch rule | `auto-fix-infra` Durable Object per fix ticket | Cloud Agent branch and pull-request work | Pull request and lifecycle status callback |
...
| Webhook Agent Ingest | HTTP webhook or scheduled alarm | `webhook-agent-ingest` queue and `TriggerDO` | Cloud Agent or ChipMate Chat destination | Destination delivery and queue retry behavior |

## ChipMate Bot ingress and source-control targets

ChipMate Bot command ingress and repository target support are separate concerns.

| Concern | Current static-source statement |
|---|---|
| Command ingress | GitHub, Slack, and Linear are current ChipMate Bot command surfaces |
...
| GitLab command ingress | GitLab repository support does not imply note or comment-triggered ChipMate Bot commands |

Do not document GitLab issue notes or merge-request comments as current ChipMate Bot trigger surfaces unless source adds explicit ingress handling.
...
| Output | High-confidence classification applies labels such as `chipmate-auto-fix` for downstream Auto Fix |
...
| Destination | `chipmateclaw_chat` | Posts to user-scoped ChipMate Chat destination through ChipMate Chat service binding |
...
flowchart LR
  ...
  agent["Cloud Agent"]
  chat["ChipMate Chat destination"]
...
|---|---|
| ChipMate Bot | `apps/web/src/lib/bot/`{% linebreak /%}`apps/web/src/lib/bots/`{% linebreak /%}`apps/web/src/lib/slack-bot/` |
