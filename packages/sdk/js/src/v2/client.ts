...
import { ChipMateClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as ChipMateClientConfig, ChipMateClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ...
    ["x-chipmate-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-chipmate-directory")
  next.headers.delete("x-chipmate-workspace")
  return next
}

export function createChipMateClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-chipmate-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-chipmate-workspace": config.experimental_workspaceID,
    }
  }
  ...
  return new ChipMateClient({ client })
}
