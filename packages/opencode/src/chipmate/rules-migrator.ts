...
export namespace RulesMigrator {
  // Only support .chipmaterules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".chipmaterules"
  const home = () => process.env.CHIPMATE_TEST_HOME || process.env.HOME || process.env.USERPROFILE || os.homedir()

  // Directory-based rules
  const CHIPMATE_RULES_DIRS = [".chipmate/rules"]
  const globalRulesDirs = () => [path.join(home(), ".chipmate", "rules")]
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directories (~/.chipmate/rules/*.md and ~/.chipmate/rules/*.md)
    ...
    // 2. Project .chipmate/rules/ and .chipmate/rules/ directories
    const seen = new Set<string>()
    for (const rulesRel of CHIPMATE_RULES_DIRS) {
    ...
    // 3. Legacy .chipmaterules file (only chipmate, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directories (.chipmate/rules-{mode}/*.md and .chipmate/rules-{mode}/*.md)
      const modeSeen = new Set<string>()
      for (const prefix of [".chipmate"]) {
      ...
      // Legacy mode-specific file (.chipmaterules-{mode})
      const legacyModeFile = path.join(projectDir, `.chipmaterules-${mode}`)
  ...
  export async function migrate(options: {
    ...
    for (const rule of rules) {
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .chipmate/rules/ directory.`,
        )
      }
    }
