package ai.chipmate.client.session.model
...
class SessionModelTest : BasePlatformTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(ChipMateAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(ChipMateWorkspaceStatusDto.PENDING, model.workspace.status)
    ...
    fun `test isReady requires app and workspace readiness`() {
        model.app = ChipMateAppStateDto(ChipMateAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = ChipMateWorkspaceStateDto(ChipMateWorkspaceStatusDto.READY)
