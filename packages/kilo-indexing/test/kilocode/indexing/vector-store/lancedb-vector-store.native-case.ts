...
describe("LanceDB 原生批量 generation finalize 隔离用例", () => {
  ...
  test("清空候选代后保留完整向量身份并支持中断恢复和跨进程重开", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "kilo-lancedb-rebuild-resume-"))
    roots.push(root)
    const workspace = path.join(root, "工作区")
  ...
  test("正常进程退出后保留候选代并由下一进程安全接管", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "kilo-lancedb-worker-handoff-"))
    roots.push(root)
    const workspace = path.join(root, "工作区")
  ...
  test("旧版已激活索引只读保留并重建到紧凑路径", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "kilo-lancedb-legacy-active-migration-"))
    roots.push(root)
