...
/** Byte-budget capping, freshness fingerprinting, and the index envelope (the ```chipmate-memory-v1 block). */
export namespace MemoryBudget {
  ...
  function wrap(input: { root: string; limits: MemorySchema.Limits; lines: string[] }) {
    ...
    return [
      "```chipmate-memory-v1 context_not_instruction",
      "scope: project",
  ...
  export function cap(input: string, max: number): Result {
    ...
    if (lines[0]?.startsWith("```chipmate-memory-v1") && close > 0) {
      ...
      const note =
        "note: index truncated; call chipmate_memory_recall mode=typed|digest|search query=<topic> to search omitted memory"
  ...
  export function stale(input: string) {
    return !input.trimStart().startsWith("```chipmate-memory-v1")
  }
