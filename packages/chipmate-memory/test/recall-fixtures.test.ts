...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "chipmate-memory-recall-"))
  return {
    dir,
...
describe("memory recall lexical fixtures", () => {
  ...
  test("expected hit: path and tool query finds environment memory", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        ...
        text: "Run bun test ./test/chipmate/memory from packages/opencode.",
      })
  ...
  test("expected miss: ubiquitous-term overlap does not leak unrelated memory", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({ root: t.root, key: "the_docs", text: "The docs live in packages/chipmate-docs." })
  ...
  test("expected miss: corpus-derived filtering drops non-English function words", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({ root: t.root, key: "docs_es", text: "La ruta de docs vive en packages/chipmate-docs." })
