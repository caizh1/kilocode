...
internal abstract class SettingsListPanel(
    ...
    protected val view = SettingsListView(ChipMateBundle.message("settings.agentBehavior.empty"), cfg) { key, id ->
        onCell(key, id)
    }
    ...
    protected open fun emptyText(): String = ChipMateBundle.message("settings.agentBehavior.empty")

    protected open fun loadingText(): String = ChipMateBundle.message("settings.agentBehavior.loading")
    ...
    private fun toolbar(): JComponent {
        ...
        if (showRefresh()) {
            ...
            actions += SettingsToolbarAction(
                ChipMateBundle.message("settings.agentBehavior.refresh"),
                ChipMateBundle.message("settings.agentBehavior.refresh.description"),
    ...
    protected suspend fun waitForReady() {
        val flow = service<ChipMateAppService>().state
        val saw = withTimeoutOrNull(RELOAD_START_TIMEOUT_MS) {
            flow.first { it.status != ChipMateAppStatusDto.READY }
        } != null
        if (!saw) return
    ...
    private companion object {
        val LOG = ChipMateLog.create(SettingsListPanel::class.java)
