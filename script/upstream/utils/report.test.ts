...
test("recommends keep ours for ChipMate directories", () => {
  expect(getRecommendation("packages/chipmate-vscode/.prettierignore", [], []).recommendation).toBe("keep-ours")
  expect(getRecommendation("packages/chipmate-i18n/tsconfig.json", [], []).recommendation).toBe("keep-ours")
})
