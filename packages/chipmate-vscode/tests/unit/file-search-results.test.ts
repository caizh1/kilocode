...
import { mergeFileSearchResults } from "../../src/chipmate-provider/file-search-results"
...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/chipmate-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/chipmate-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/chipmate-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
