/**
 * ChipMate Gateway specific routes
 * Handles profile fetching and organization management for ChipMate Gateway provider
 *
 * This factory function accepts OpenCode dependencies to create ChipMate-specific routes
 */

import { fetchChipMateNotifications, ChipMateNotificationSchema } from "../api/notifications.js"
import { fetchChipMateImageModels } from "../api/models.js"
import { fetchOrganizationModes, clearModesCache } from "../api/modes.js"
import { CHIPMATE_API_BASE, HEADER_FEATURE, HEADER_ORGANIZATIONID } from "../api/constants.js"
...
interface ChipMateRoutesDeps extends ImportDeps {
  Hono: new () => Hono
...
/**
 * Create ChipMate Gateway routes with OpenCode dependencies injected
 *
 * @example
 * ```typescript
 * import { createChipMateRoutes } from "@chipmate/chipmate-gateway"
 ...
 *
 * export const ChipMateRoutes = createChipMateRoutes({
 *   Hono,
 *   describeRoute,
 *   validator,
 *   resolver,
 ...
 * ```
 */
export function createChipMateRoutes(deps: ChipMateRoutesDeps) {
  const {
    Hono,
    describeRoute,
    validator,
  ...
  const ChipMatePassState = z.object({
  ...
  const ProfileWithBalance = z.object({
    ...
    chipmatePass: ChipMatePassState.nullable(),
  ...
  const getProxyAuth = async () => {
    const auth = await Auth.get("chipmate")
  ...
  return new Hono()
    .get(
      "/profile",
      describeRoute({
        summary: "Get ChipMate Gateway profile",
        description: "Fetch user profile and organizations from ChipMate Gateway",
        operationId: "chipmate.profile",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          return c.json({ error: "Not authenticated with ChipMate Gateway" }, 401)
        }
      },
    )
    .post(
      "/organization",
      describeRoute({
        summary: "Update ChipMate Gateway organization",
        description: "Switch to a different ChipMate Gateway organization",
        operationId: "chipmate.organization.set",
        responses: {
          200: {
        ...
        try {
          return c.json(
            await setOrganization(
              {
                auth: Auth,
                clear: () => ModelCache.clear("chipmate"),
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy a Fill-in-the-Middle completion request to the ChipMate Gateway",
        operationId: "chipmate.fim",
        responses: {
          200: {
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy an audio transcription request to the ChipMate Gateway",
        operationId: "chipmate.audio.transcriptions",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        if (!proxy.auth) return c.json({ error: "Not authenticated with ChipMate Gateway" }, 401)
        ...
        const headers = {
          ...
          ...buildChipMateHeaders(undefined, { chipmateOrganizationId: proxy.organizationId }),
          [HEADER_FEATURE]: "vscode-extension",
        }

        const response = await fetch(`${CHIPMATE_API_BASE}/api/gateway/v1/audio/transcriptions`, {
          method: "POST",
          headers,
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "List image-capable models from the ChipMate Gateway OpenRouter passthrough",
        operationId: "chipmate.models.images",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const result = await fetchChipMateImageModels({
            chipmateToken: proxy.token,
            chipmateOrganizationId: proxy.organizationId,
          })
          if (result.error) {
    ...
    .post(
      ...
      describeRoute({
        ...
        description:
          "Proxy an image generation request (chat-completions with modalities) to the ChipMate Gateway OpenRouter passthrough",
        operationId: "chipmate.image.generations",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        const response = await fetch(`${CHIPMATE_API_BASE}/api/openrouter/chat/completions`, {
          method: "POST",
          headers,
          signal: c.req.raw.signal,
    ...
    .get(
      "/notifications",
      describeRoute({
        summary: "Get ChipMate notifications",
        description: "Fetch notifications from ChipMate Gateway for CLI display",
        operationId: "chipmate.notifications",
        responses: {
          200: {
            description: "Notifications list",
            content: {
              "application/json": {
                schema: resolver(z.array(ChipMateNotificationSchema)),
              },
            },
          },
          ...errors(400, 401),
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch full session data from the ChipMate cloud for preview",
        operationId: "chipmate.cloud.session.get",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          const auth = await Auth.get("chipmate")
          if (!auth) return c.json({ error: "Not authenticated with ChipMate Gateway" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          ...
          console.error("[ChipMate Gateway] cloud/session/get: unhandled error", err?.message ?? err)
    ...
    .post(
      ...
      describeRoute({
        ...
        operationId: "chipmate.cloud.session.import",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const auth = await Auth.get("chipmate")
          if (!auth) return c.json({ error: "Not authenticated with ChipMate" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          if (!token) return c.json({ error: "No valid token found" }, 401)
          ...
          console.error("[ChipMate Gateway] cloud/session/import: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
    .get(
      "/claw/status",
      describeRoute({
        summary: "Get ChipMateClaw instance status",
        description: "Fetch the user's ChipMateClaw instance status via the ChipMateClaw worker",
        operationId: "chipmate.claw.status",
        responses: {
          200: {
              ...
              "application/json": {
                schema: resolver(
                  z.object({
                    ...
                    // cloud `services/chipmateclaw/src/index.ts` and the
                    // `PlatformStatusResponse` type in
                    // cloud/apps/web/src/lib/chipmateclaw/types.ts. Keeping them in
      ...
      async (c: any) => {
        try {
          ...
          if (err instanceof GatewayError) {
            return c.json({ error: `ChipMateClaw request failed: ${err.status} ${err.message}` }, err.status as any)
          }
          console.error("[ChipMate Gateway] claw/status: error", err?.message ?? err)
          return c.json({ error: "Failed to reach ChipMateClaw" }, 502)
        }
      },
    )
    .get(
      ...
      describeRoute({
        summary: "Get ChipMateClaw chat credentials",
        description:
          "Returns the bearer token and endpoint URLs the client uses to talk to the ChipMate Chat worker " +
          "and the Event Service. The bearer is the user's existing long-lived ChipMate JWT — chipmate-chat and " +
          "event-service both verify it directly with NEXTAUTH_SECRET, so no separate token mint is needed.",
        operationId: "chipmate.claw.chatCredentials",
        responses: {
          200: {
            description: "ChipMate Chat credentials or null",
            content: {
              "application/json": {
                schema: resolver(
                  z
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch cloud CLI sessions from ChipMate API",
        operationId: "chipmate.cloudSessions",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          console.error("[ChipMate Gateway] cloud-sessions: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
