...
import { prepareCommand as prepareSandbox } from "@chipmate/sandbox" // chipmate_change
import { tap as tapStdio, tapped } from "./chipmate/stdio-tap" // chipmate_change - Bun drops buffered stdio on close
...
import { settle } from "./chipmate/exit-code" // chipmate_change - settle signal termination as 128 + signum
...
export const make = Effect.gen(function* () {
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stdout"), // chipmate_change - read the spawn-time tap
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stderr"), // chipmate_change - read the spawn-time tap
  ...
  const spawn = (
    ...
    direct: boolean, // chipmate_change - avoid shadowing settle
  ) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      const signal = Deferred.makeUnsafe<readonly [code: number | null, signal: NodeJS.Signals | null]>()
      const proc = launch(command.command, command.args, opts)
      tapStdio(proc) // chipmate_change - must run in the same tick as spawn
      let end = false
      ...
      proc.on("exit", (...args) => {
        exit = args
        if (direct) Deferred.doneUnsafe(signal, Exit.succeed(args)) // chipmate_change - bounded grep must not await inherited pipes
      })
      proc.on("close", (...args) => {
        if (end) return
        end = true
  ...
  // chipmate_change start - inspect descendants owned by commands that settle on direct exit
  ...
  const spawnCommand: (
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          const validation = SpawnValidation.take(command) // chipmate_change - retain target validation through preparation
          const direct = SpawnExit.take(command) // chipmate_change - opt selected commands into direct-exit settlement
          const dir = yield* cwd(command.options)
          // chipmate_change start - prepare agent-scoped commands through the selected sandbox backend
          ...
          // chipmate_change start - close target-swap races at the raw spawn boundary
          if (validation)
          ...
          const [proc, signal] = yield* Effect.acquireRelease(
            // chipmate_change start - spawn the prepared command and options
            spawn(
              target,
              {
                cwd: dir,
              ...
              direct, // chipmate_change
            ),
            Effect.fnUntraced(function* ([proc, signal]) {
              const done = yield* Deferred.isDone(signal)
              const kill = timeout(proc, command, target.options) // chipmate_change
              if (done) {
                const [code] = yield* Deferred.await(signal)
                if (process.platform === "win32") return yield* Effect.void
                // chipmate_change start - clean up only descendants owned by direct-settling commands
              ...
              // chipmate_change start - preserve kill options from the prepared command
          ...
          return makeHandle({
            ...
            exitCode: Effect.flatMap(Deferred.await(signal), settle), // chipmate_change - signal termination settles as 128 + signum
