...
// path when its layer is built, so CHIPMATE_TEST_HOME must be set before the runtime
...
// CHIPMATE_DISABLE_CLAUDE_CODE flag the test env sets); project .chipmate skills are untrusted.
...
function writeProjectSkill(dir: string, name: string, body: string) {
  return Effect.promise(() =>
    Bun.write(
      path.join(dir, ".chipmate", "skill", name, "SKILL.md"),
      `---\nname: ${name}\ndescription: ${name} test skill.\n---\n\n${body}\n`,
    ),
  )
}
...
describe("skill shell injection", () => {
  ...
  unix("does not ask or run anything for a skill with only an inline code example", () =>
    Effect.gen(function* () {
      // This is the real-world trigger: chipmate-config.md documents the placeholder syntax
