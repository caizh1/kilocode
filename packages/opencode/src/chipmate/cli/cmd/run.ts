import { createChipMateClient, type ChipMateClient } from "@chipmate/sdk/v2"
import { UI } from "@/cli/ui"
import { DaemonClient } from "@/chipmate/daemon/client"
import { isBuiltinCommand, type BuiltinCommand } from "@/chipmate/session/builtin-commands"
import { Provider } from "@/provider/provider"
import { Filesystem } from "@/util/filesystem"

export namespace ChipMateRun {
  export async function resolveBuiltin(sdk: ChipMateClient, command?: string, directory?: string) {
...
export namespace ChipMateRunDaemon {
  export type Input = {
    directory?: string
    execute: (client: ChipMateClient) => Promise<void>
  }
  ...
  export async function attach(input: Input) {
    ...
    const client = createChipMateClient({ baseUrl: daemon.url, directory: dir, headers: daemon.headers })
