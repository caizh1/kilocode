...
describe("agent terminal package contributions", () => {
  ...
  test("adds Agent Console while preserving the internal-offline toolbar gates", async () => {
    ...
    const toolbar = [
      ...
      ["chipmate.v2.sidebarTitle.chipmateClawOpen", "ChipMateClaw", "$(comment-discussion)"],
    ...
    const gated = new Set(["chipmate.v2.sidebarTitle.chipmateClawOpen", "chipmate.v2.sidebarTitle.profileButtonClicked"])
    expect(
      title
