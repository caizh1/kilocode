---
title: "API Reference"
description: "Complete API reference for the ChipMate AI Gateway, including chat completions, FIM completions, and model listing endpoints."
---

# API Reference

The ChipMate AI Gateway provides an OpenAI-compatible API. All endpoints use the base URL:
...
```bash
curl -X POST "https://api.chipmate.ai/api/fim/completions" \
  -H "Authorization: Bearer $CHIPMATE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
