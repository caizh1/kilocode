...
import { SandboxStore } from "../../../src/chipmate/sandbox/store"
...
async function config(dir: string, value: unknown) {
  await Bun.write(path.join(dir, "chipmate.json"), JSON.stringify(value, null, 2))
}
...
describe("config overlay routes", () => {
  test("writes a missing project target atomically", async () => {
    ...
    const target = await ChipMateConfigOverlay.target({ scope: "project", directory: project.path })

    const result = await ChipMateConfigWriter.write({
      scope: "project",
  ...
  test("ignores a nested unset path when the project target is missing", async () => {
    ...
    expect(await Bun.file(path.join(project.path, ".chipmate", "chipmate.jsonc")).exists()).toBe(false)
  })

  test("removes an existing nested unset path", async () => {
    await using project = await tmpdir()
    const file = path.join(project.path, ".chipmate", "chipmate.jsonc")
  ...
  test("returns exact raw target data and a stable missing-file revision", async () => {
    await using project = await tmpdir()
    const first = await ChipMateConfigOverlay.target({ scope: "project", directory: project.path })
    const second = await ChipMateConfigOverlay.target({ scope: "project", directory: project.path })
    ...
    const saved = await ChipMateConfigOverlay.target({ scope: "project", directory: project.path })
  ...
  test("rejects a comment-only external edit with a typed revision conflict", async () => {
    ...
    const response = await Server.Default().app.request("/config/overlay", {
      method: "PATCH",
      headers: { "content-type": "application/json", "x-chipmate-directory": project.path },
      body: JSON.stringify({
        scope: "project",
        expected: {
  ...
  test("rejects a newly created higher-priority target", async () => {
    await using project = await tmpdir()
    const before = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))
    await Filesystem.write(path.join(project.path, "chipmate.json"), "{}")
  ...
  test("rejects a project config target that escapes through a symlink", async () => {
    ...
    await Filesystem.write(path.join(outside.path, "chipmate.jsonc"), "{}")
    await symlink(outside.path, path.join(project.path, ".chipmate"), "dir")
    const before = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))
    ...
    expect(await Bun.file(path.join(outside.path, "chipmate.jsonc")).text()).not.toContain('"model"')
  })

  test("does not expose partial content when an atomic replacement fails", async () => {
    await using project = await tmpdir()
    const file = path.join(project.path, "chipmate.jsonc")
    await Filesystem.write(file, '{\n  "model": "test/before"\n}\n')
    const target = await ChipMateConfigOverlay.target({ scope: "project", directory: project.path })

    await expect(
      ChipMateConfigWriter.write({
        scope: "project",
        directory: project.path,
        expected: target,
  ...
  test("rechecks missing target parents before replacement", async () => {
    ...
    expect(await Bun.file(path.join(outside.path, "chipmate.jsonc")).exists()).toBe(false)
  })
  ...
  test("ignores unsafe patch paths", () => {
    const patched = ChipMateConfigOverlay.patch({
      scope: "project",
      unset: [
        ["__proto__", "polluted"],
  ...
  test("loads .chipmate and ignores .opencode in project overlays", async () => {
    await using project = await tmpdir()
    const entries = [
      {
        root: ".opencode",
      ...
      },
      {
        root: ".chipmate",
        source: "chipmate",
        value: { username: "chipmate", model: "test/chipmate" },
      },
    ] as const

    for (const item of entries) {
      const dir = path.join(project.path, item.root)
      await Filesystem.write(path.join(dir, "chipmate.json"), JSON.stringify(item.value))
      await Filesystem.write(
    ...
    const body = await ChipMateConfigOverlay.resolve({
      directory: project.path,
      scope: "project",
      effective: {},
      global: {},
    ...
    expect(body.project.username).toBe("chipmate")
    expect(body.project.model).toBe("test/chipmate")
    ...
    expect(body.targets.project.path).toBe(path.join(project.path, ".chipmate", "chipmate.json"))
  })
  ...
  test.serial("tolerates unsafe project config instead of failing the overlay", async () => {
    ...
    await Filesystem.write(
      path.join(project.path, ".chipmate", "chipmate.json"),
  ...
  test.serial("writes project indexing overrides to .chipmate/chipmate.jsonc", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir()
    ...
    expect(await Bun.file(path.join(project.path, ".chipmate", "chipmate.json")).exists()).toBe(false)
  ...
  test.serial("removes local scalar override and falls back to global", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir({ config: { model: "chipmate/project-model", username: "alice" } })
    await setGlobal(global.path, { model: "chipmate/global-model" })
  ...
  test.serial("writes project mcp overrides without copying inherited servers", async () => {
    ...
    const saved = (await Bun.file(path.join(project.path, ".chipmate", "chipmate.jsonc")).json()) as {
  ...
  test.serial("writes partial global workflow overrides when both JSON and JSONC exist", async () => {
    ...
    await using project = await tmpdir()
    ;(Global.Path as { config: string }).config = global.path
    await Filesystem.write(path.join(global.path, "chipmate.json"), JSON.stringify({ username: "legacy" }))
    await Filesystem.write(path.join(global.path, "chipmate.jsonc"), "{\n  // Keep JSONC as the active target.\n}\n")
    ...
    const saved = ConfigParse.jsonc(await Bun.file(path.join(global.path, "chipmate.jsonc")).text(), "chipmate.jsonc")
    expect(saved).toMatchObject({
      command: { review: { model: "anthropic/claude-sonnet-4-6", variant: "high" } },
    })
    expect(await Bun.file(path.join(global.path, "chipmate.json")).json()).toMatchObject({ username: "legacy" })
  })
  ...
  test.serial("merges workflow overrides with a command body in the lower-precedence global file", async () => {
    ...
    ;(Global.Path as { config: string }).config = global.path
    await Filesystem.write(
      path.join(global.path, "chipmate.json"),
  ...
  terminal("preserves active terminals after updating global console preferences", async () => {
    ...
    await using project = await tmpdir()
    ;(Global.Path as { config: string }).config = global.path
    const headers = { "x-chipmate-directory": project.path }
