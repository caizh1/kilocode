...
description: "Architecture overview for ChipMate Cloud services and hosted runtimes"
---

# Cloud Platform Architecture

ChipMate Cloud is hosted platform layer for authentication, model routing, billing, product configuration, automation, and scoped execution services. Cloud implementation lives in open-source [`ChipMate-Org/cloud`](https://github.com/ChipMate-Org/cloud) repository.

{% callout type="info" title="Static source scope" %}
This page describes Worker surfaces, bindings, routes, and code paths present in `ChipMate-Org/cloud`. Static source shows deployable architecture, not live production enablement, rollout percentages, retention configuration, or vendor settings. Validate live environment before making production or compliance claims. Use [ChipMate Cloud Security Architecture](/docs/contributing/architecture/cloud-security) for trust boundaries and data flows.
{% /callout %}
...
| Shared cloud services | Model routing, asynchronous orchestration, real-time delivery, persistence adapters, and operational services | ChipMate Gateway, Workers, queues, Durable Objects, R2, KV, Hyperdrive |
| Scoped execution | Runs code or owner-scoped runtime workloads | Cloud Agent, App Builder preview sandbox, deployment builder sandbox, ChipMateClaw runtime, Gas Town container |
| External providers | Services outside ChipMate Cloud trust boundary | Model providers, source-control providers, messaging providers, telemetry providers |
...
flowchart LR
  ...
  claw["ChipMateClaw"]
  chat["ChipMate Chat / Event Service / Notifications"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| ChipMateClaw | `chipmateclaw`{% linebreak /%}`chipmateclaw-billing`{% linebreak /%}`gmail-push`{% linebreak /%}`chipmateclaw-inbound-email` | Owner-scoped assistant runtime coordination, billing, and external ingress |
| Real-time chat | `chipmate-chat`{% linebreak /%}`event-service`{% linebreak /%}`notifications` | Conversation state, WebSocket delivery, and mobile push |
| Multi-agent orchestration | `gastown`{% linebreak /%}`wasteland` | Town execution and collaborative commons |
| Evaluation and operations | `o11y`{% linebreak /%}`chipmate-ops`{% linebreak /%}`model-eval-ingest` | Metrics, alerts, operations, and model-evaluation ingestion |
| Attribution | `ai-attribution` | AI-edit attribution events |

## ChipMate Gateway

Gateway consists of cloud API routes plus `packages/chipmate-gateway/` client integration in `ChipMate-Org/chipmate`. It handles account-aware and anonymous-free model access. See [Cloud Security](/docs/contributing/architecture/cloud-security#model-request-gateway) for request branches and endpoint families.
...
Auto Model clients send stable `chipmate-auto/*` tier IDs. Gateway resolves tiers server-side before provider routing so mappings can change without client releases. See [Models and Providers](/docs/gateway/models-and-providers#auto-models) for current tier behavior.
...
|---|---|
| ChipMate Bot | Launches Cloud Agent for requested repository work |
...
| Webhook Agent Ingest | Delivers configured prompt to Cloud Agent or ChipMate Chat destination |
...
`services/webhook-agent-ingest/` is configured-trigger boundary. It accepts HTTP webhooks and scheduled alarms, then dispatches selected Cloud Agent or ChipMate Chat destination. [Automation Services](/docs/contributing/architecture/automation-services#webhook-agent-ingest) owns activation, authentication, queue, and alarm details.
...
sequenceDiagram
  ...
  participant Chat as ChipMate Chat conversation-state DOs
...
ChipMate Chat stores conversation state in Durable Objects and fans events out through Event Service. Notifications checks Event Service presence context before selected pushes and processes Expo receipts asynchronously. See [Cloud Security](/docs/contributing/architecture/cloud-security#chat-events-and-notifications) for ticket and push-delivery trust boundaries.

## ChipMateClaw

ChipMateClaw is owner-scoped hosted OpenClaw runtime coordination. Durable Objects track instance lifecycle, routing, configuration, and reconciliation. Runtime provider support includes Fly, docker-local development, and Northflank paths; source support does not prove active provider rollout. See [Cloud Security](/docs/contributing/architecture/cloud-security#chipmateclaw-ingress) for ingress controls.

| Ingress path | Auth or validation | Entry boundary | Async handoff | Target |
|---|---|---|---|---|
| Browser request | JWT auth | ChipMateClaw proxy | None | Owner-scoped runtime |
| One-time access code | Redeemed code and auth cookie | Access gateway | None | Owner-scoped OpenClaw UI |
| Controller machine check-in | Machine API key and derived gateway token | ChipMateClaw controller route | None | Owner-scoped runtime controller |
| ChipMate Chat RPC | Service binding | ChipMateClaw binding | None | Owner-scoped runtime |
| Cloudflare Email Routing | Alias lookup and bounded parse | `chipmateclaw-inbound-email` | Queue | ChipMateClaw platform service |
| Gmail Pub/Sub push | Google OIDC validation | `gmail-push` | Queue | Owner-scoped runtime controller |

ChipMateClaw resolves owner or instance scope before runtime delivery. Table compares ingress boundaries; it does not describe global shared destinations.
...
flowchart TB
  subgraph worker ["Cloudflare Worker"]
    direction LR
    auth["JWT auth<br/>tied to ChipMate user"]
    instanceDO["Per-instance Durable Object"]
    dbConnection["ChipMate database connection"]
  end

  db["ChipMate database<br/>Instances<br/>Short-lived access codes<br/>Image catalog<br/>Billing and user preferences"]
  ...
  subgraph flyInstance ["Fly instance: owner-scoped runtime"]
    direction TB
    subgraph container ["ChipMateClaw container"]
      direction TB
      controller["ChipMateClaw controller<br/>Supervises OpenClaw gateway<br/>Exposes control endpoints<br/>Proxies HTTP and WebSocket traffic"]
...
Paths below are relative to [`ChipMate-Org/cloud`](https://github.com/ChipMate-Org/cloud).
...
| ChipMateClaw | `services/chipmateclaw/`{% linebreak /%}`services/chipmateclaw-billing/`{% linebreak /%}`services/gmail-push/`{% linebreak /%}`services/chipmateclaw-inbound-email/` |
| Chat, events, and notifications | `services/chipmate-chat/`{% linebreak /%}`services/event-service/`{% linebreak /%}`services/notifications/` |
| Multi-agent orchestration | `services/gastown/`{% linebreak /%}`services/wasteland/` |
| Observability and operations | `services/o11y/`{% linebreak /%}`services/chipmate-ops/`{% linebreak /%}`services/model-eval-ingest/` |
