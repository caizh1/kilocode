...
import { MemoryConfig } from "@chipmate/chipmate-memory/effect/config"
import { MemoryError } from "@chipmate/chipmate-memory/effect/errors"
import type { MemoryPorts } from "@chipmate/chipmate-memory/effect/ports"
import { MemoryRedact } from "@chipmate/chipmate-memory/redact"
import { MemoryShared } from "@chipmate/chipmate-memory/shared"
...
function recalledMemory(turn: Turn) {
  return [turn.user, ...turn.assistants].flatMap((item) => item.parts).some((part) => {
    if (part.type === "tool") {
      return (
        part.tool === "chipmate_memory_recall" &&
    ...
    const marker = (part.metadata as { chipmateMemory?: { type?: string; count?: number } } | undefined)?.chipmateMemory
