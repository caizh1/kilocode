import type { AgentRequirementResult } from "@chipmate/sdk/v2"
...
export async function check(input: { client: Client; agent: string; directory: string }): Promise<Check> {
  const response = await input.client.chipmate.agentRequirements(
...
export function format(input: ErrorData | RequirementError) {
  ...
  if (info.vscode_extensions.length) {
    lines.push(
      ...
      "This agent requires VS Code extensions and is not supported in this CLI environment. Use the ChipMate VS Code extension instead.",
    )
  }
