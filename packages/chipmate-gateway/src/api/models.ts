...
import { CHIPMATE_API_BASE, CHIPMATE_OPENROUTER_BASE, MODELS_FETCH_TIMEOUT_MS, PROMPTS, AI_SDK_PROVIDERS } from "./constants.js"

export type ChipMateModelsResult = {
...
/**
 * Fetch models from ChipMate API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Typed result with models and optional error info
 */
export async function fetchChipMateModels(options?: {
  chipmateToken?: string
  chipmateOrganizationId?: string
  baseURL?: string
}): Promise<ChipMateModelsResult> {
  const raw = await fetchRawChipMateModels(options)
  ...
  for (const model of raw.data) {
    // Skip models that explicitly don't support tools — ChipMate requires tool calling
...
export type ChipMateImageModel = {
  id: string
  name: string
  description?: string
}

export type ChipMateImageModelsResult = {
  models: ChipMateImageModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

export type ChipMateTranscriptionModel = {
  id: string
  name: string
}

export type ChipMateTranscriptionModelsResult = {
  models: ChipMateTranscriptionModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

/**
 * Fetch image-capable models from ChipMate API (OpenRouter-compatible endpoint).
 * Uses the same raw fetch as {@link fetchChipMateModels} but keeps only models
 * whose `output_modalities` include `"image"`.
 */
export async function fetchChipMateImageModels(options?: {
  chipmateToken?: string
  chipmateOrganizationId?: string
  baseURL?: string
}): Promise<ChipMateImageModelsResult> {
  const raw = await fetchRawChipMateModels(options)
  if (raw.error) return { models: [], error: raw.error }

  const models: ChipMateImageModel[] = []
...
export async function fetchChipMateTranscriptionModels(options?: {
  chipmateToken?: string
  chipmateOrganizationId?: string
  baseURL?: string
}): Promise<ChipMateTranscriptionModelsResult> {
  const token = options?.chipmateToken
  const organizationId = options?.chipmateOrganizationId
  const url = new URL("transcription-models", resolveChipMateGatewayBaseUrl({ baseURL: options?.baseURL, token }))
  const response = await fetch(url, {
    headers: {
      ...getDefaultHeaders(),
      ...buildChipMateHeaders(undefined, { chipmateOrganizationId: organizationId }),
...
/**
 * Shared raw fetch + validate used by both {@link fetchChipMateModels} and {@link fetchChipMateImageModels}.
 */
async function fetchRawChipMateModels(options?: {
  chipmateToken?: string
  chipmateOrganizationId?: string
  baseURL?: string
}): Promise<
  { data: OpenRouterModel[]; error?: undefined } | { data?: undefined; error: NonNullable<ChipMateModelsResult["error"]> }
> {
  const token = options?.chipmateToken
  const organizationId = options?.chipmateOrganizationId

  // Construct base URL
  const defaultBaseURL = organizationId ? `${CHIPMATE_API_BASE}/api/organizations/${organizationId}` : CHIPMATE_OPENROUTER_BASE

  const baseURL = options?.baseURL ?? defaultBaseURL

  // Transform URL with token if available
  const finalBaseURL = token ? getChipMateUrlFromToken(baseURL, token) : baseURL
