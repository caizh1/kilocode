...
export namespace ConsoleAssets {
  ...
  async function dir() {
    const override = process.env.CHIPMATE_CONSOLE_ASSET_DIR
  ...
  function source() {
    return path.resolve(import.meta.dirname, "../../../../chipmate-console")
  }
  ...
  async function build(app: string, out: string) {
    ...
    const run = runBuild(app, out).catch((err) => {
      log.warn("failed to build ChipMate Console assets", { err })
      return undefined
    })
    builds.set(app, run)
    return await run
  ...
  async function runBuild(app: string, out: string) {
    log.info("building ChipMate Console assets", { app })
    const proc = Bun.spawn([process.execPath, "run", "build"], {
      cwd: app,
      env: { ...process.env, CHIPMATE_CONSOLE_BASE: base },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
    ...
    if (code !== 0) throw new Error(`ChipMate Console build failed with exit code ${code}: ${stderr || stdout}`)
    if (!(await ready(out, true))) throw new Error("ChipMate Console build did not produce /console assets")
    return out
  }
