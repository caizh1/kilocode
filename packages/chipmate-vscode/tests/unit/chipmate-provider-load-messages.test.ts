...
const { ChipMateProvider, unwrapSyncEvent } = await import("../../src/ChipMateProvider")
...
function makeProvider(client: ReturnType<typeof createClient>) {
  ...
  const provider = new ChipMateProvider({} as never, connection as never)
...
describe("ChipMateProvider.handleAbort", () => {
...
describe("ChipMateProvider sandbox status", () => {
...
describe("ChipMateProvider sandbox toggle", () => {
  ...
  it("resolves a blank worktree default against the routed directory", async () => {
    ...
    await internal.fetchAndSendSandboxDefault("/repo/.chipmate/worktrees/wt-1")

    expect(client.configReads).toEqual([{ directory: "/repo/.chipmate/worktrees/wt-1" }])
    expect(client.sandboxSupport).toEqual([{ directory: "/repo/.chipmate/worktrees/wt-1" }])
  })
  ...
  it("waits for a blank toggle before creating the first prompt session", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({ metadata: { "chipmate.sandbox": { enabled: true, version: 0 } } }),
    ])
    expect(client.prompted).toHaveLength(1)
  })
  ...
  it("snapshots the remembered default before sending the first prompt", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({
        directory: "/repo",
        metadata: { "chipmate.sandbox": { enabled: true, version: 0 } },
      }),
    ])
    expect(client.sandboxed).toHaveLength(0)
    expect(client.prompted).toHaveLength(1)
...
describe("ChipMateProvider sidebar tabs", () => {
...
describe("ChipMateProvider revert ordering", () => {
...
describe("ChipMateProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("ChipMateProvider.handleDeleteSession / background processes", () => {
...
describe("ChipMateProvider.handleLoadMessages / slim payload", () => {
...
describe("ChipMateProvider.loadMessages / sub-agent viewer", () => {
...
describe("ChipMateProvider.handleLoadMessages / prepend into deleted session", () => {
