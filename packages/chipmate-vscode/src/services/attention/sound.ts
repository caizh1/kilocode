...
import type { TuiAttentionSoundName } from "@chipmate/plugin/tui"
...
async function run(commands: Array<{ cmd: string; args: string[]; env?: NodeJS.ProcessEnv }>) {
  for (const command of commands) {
    const ok = await exec(command.cmd, command.args, command.env ? { env: command.env } : {}).then(
      () => true,
      (error) => {
        console.debug("[ChipMate New] notification sound command failed", { cmd: command.cmd, error })
        return false
      },
    )
    if (ok) return true
    ...
    return [
      {
        ...
        args: [
          ...
          "-NonInteractive",
          "-Command",
          "$sound = [System.Media.SoundPlayer]::new($env:CHIPMATE_SOUND_PATH); $sound.PlaySync(); $sound.Dispose()",
        ],
        env: { ...process.env, CHIPMATE_SOUND_PATH: file },
      },
    ]
  }
  return []
...
async function perform(name: TuiAttentionSoundName, selected: AttentionSoundID, dir: string) {
  ...
  if (!fs.existsSync(file)) {
    console.warn("[ChipMate New] notification sound is missing", { file })
    return false
  }
  const ok = await run(fileCommands(file))
  if (ok) console.debug("[ChipMate New] notification sound played", { name, selected })
  return ok
}
