/**
 * HTTP client for the chipmate-chat Cloudflare Worker.
 *
 * Minimal inline port of `@chipmate/chipmate-chat/client` (cloud monorepo) tailored
 ...
 * the chipmate-chat worker is the source of truth and validates at its edge.
 */
...
export type ChipMateChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class ChipMateChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`ChipMateClaw request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "ChipMateChatApiError"
  }
}
...
export class ChipMateChatClient {
  private readonly baseUrl: string
  private readonly getToken: () => Promise<string>
  private readonly onUnauthorized: (() => void) | undefined
  private readonly sendQueues: SendQueue = new Map()

  constructor(config: ChipMateChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new ChipMateChatApiError(res.status, body)
    }
