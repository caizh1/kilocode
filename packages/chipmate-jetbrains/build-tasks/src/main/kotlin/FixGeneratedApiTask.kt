...
abstract class FixGeneratedApiTask : DefaultTask() {
    ...
    private fun fixEmptyWrappers(root: File) {
        val models = File(root, "ai/chipmate/jetbrains/api/model")
    ...
    private fun fixJsonElementQueryParams(root: File) {
        val api = File(root, "ai/chipmate/jetbrains/api/client/DefaultApi.kt")
        if (!api.isFile) return

        val models = File(root, "ai/chipmate/jetbrains/api/model")
        val missing = mutableSetOf<String>()
        val text = Regex("""import ai\.chipmate\.jetbrains\.api\.model\.(\w+Parameter)\n""")
    ...
    private fun fixMissingModels(root: File) {
        ...
        val imports = Regex("""import ai\.chipmate\.jetbrains\.api\.model\.(\w+)\n""")
        root.walkTopDown().filter { it.extension == "kt" }.forEach { file ->
            val missing = mutableSetOf<String>()
