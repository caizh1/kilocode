/**
 * Contract tests for ChipMate-specific fields on QuestionOption / QuestionInfo.
 *
 * packages/opencode/src/question/index.ts is a shared upstream file.
 * Two ChipMate additions to the Option schema have been silently dropped by
...
describe("QuestionOption schema — ChipMate-specific field contract", () => {
  ...
  // Static source checks — guard the chipmate_change markers so a conflict
  // resolution that drops the fields is caught immediately.
  test("source declares mode as an optional field inside a chipmate_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/chipmate_change start[^\n]*localization and mode selection hints/)
    expect(src).toMatch(/mode:\s*Schema\.optional\(Schema\.String\)/)
    expect(src).toMatch(/chipmate_change end/)
  })

  test("source declares labelKey and descriptionKey inside a chipmate_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/chipmate_change start[^\n]*localization and mode selection hints/)
