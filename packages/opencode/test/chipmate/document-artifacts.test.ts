...
import {
  ...
  listArtifacts,
  resolveOpenArtifact,
} from "../../src/chipmate/documents/artifacts"
...
describe("chipmate document artifacts", () => {
  test("declares and lists artifact manifests under .chipmate/artifacts", async () => {
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.promise(() => within(async () => {
            ...
            expect(artifact.artifactDir).toStartWith(".chipmate/artifacts/")
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.promise(() => within(async () => {
            ...
            expect(diagnostics.path).toStartWith(".chipmate/artifacts/artifact-diagnostics-")
        ...
        () =>
          Effect.promise(() => within(async () => {
            await expect(declareArtifact({ kind: "word-document", artifactDir: "../outside" })).rejects.toThrow(
              "artifactDir must be inside .chipmate/artifacts",
            )
            await expect(
              declareArtifact({
                kind: "word-document",
