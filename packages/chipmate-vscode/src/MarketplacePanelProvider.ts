...
import type { GlobalEvent, SessionStatus } from "@chipmate/sdk/v2/client"
import { buildWebviewHtml, getWebviewFontSize } from "./utils"
import { watchFontSizeConfig } from "./chipmate-provider/font-size"
import { mapSSEEventToWebviewMessage } from "./chipmate-provider-utils"
...
export class MarketplacePanelProvider implements vscode.Disposable {
  ...
  private async sync(reconcile: boolean): Promise<void> {
    ...
    try {
      ...
      console.warn("[ChipMate New] Marketplace session status sync failed:", err)
    }
  }
  ...
  private async handleBatchMessage(msg: MarketplaceMessage): Promise<boolean> {
    ...
    console.info("[ChipMate New] Marketplace batch upload requested.")
  ...
  private async handleSkillMessage(msg: MarketplaceMessage): Promise<boolean> {
    switch (msg.type) {
      ...
      case "uploadMarketplaceSkill":
        console.info("[ChipMate New] Marketplace upload skill requested.")
        await this.uploadSkillMessage(msg)
        return true
      case "starMarketplaceSkill":
        console.info("[ChipMate New] Marketplace star skill requested.")
        if (msg.mpSkillId) await this.starMarketplaceSkill(msg.mpSkillId)
        return true
      case "unpublishMarketplaceSkill":
        console.info("[ChipMate New] Marketplace unpublish skill requested.")
  ...
  private async fetchData(): Promise<void> {
    ...
    try {
      ...
      console.warn("[ChipMate New] Marketplace data fetch failed:", err)
      this.post({
        type: "marketplaceData",
        marketplaceItems: [],
  ...
  private async fetchSkillDetail(id: string): Promise<void> {
    try {
      ...
      const error = marketplaceDataErrorMessage(err)
      console.warn("[ChipMate New] Marketplace Skill detail fetch failed:", err)
  ...
  private async detachManagedSkill(id: string, scope: "global" | "project") {
    ...
    if (apiKey) {
      await this.marketplace
        ...
        .catch((err) => console.warn("[ChipMate New] Local Skill override sync failed:", err))
    }
  ...
  private async uploadMarketplaceSkills(ids: string[]): Promise<void> {
    ...
    const installed = await this.installedSkillsForUpload().catch((err: unknown) => {
      console.warn("[ChipMate New] Marketplace failed to list installed Skills for batch upload:", err)
      return undefined
    })
    if (!installed) {
  ...
  private async installedSkillForUpload(id: string): Promise<UploadableSkill | undefined> {
    try {
      ...
      console.warn("[ChipMate New] Marketplace failed to list installed skills for upload:", err)
  ...
  private async syncManaged(
    ...
    try {
      ...
      console.warn("[ChipMate New] Marketplace installation state sync failed:", err)
  ...
  private async restoreManaged(): Promise<void> {
    ...
    for (const item of await this.registry.list(origin)) {
      ...
      try {
        ...
        console.warn("[ChipMate New] Marketplace installation metadata recovery failed:", err)
      }
    }
  }
  ...
  private post(msg: unknown): void {
    ...
    void this.panel.webview.postMessage(msg).then(undefined, (err) => {
      console.warn("[ChipMate New] Marketplace panel postMessage failed:", err)
    })
  }
