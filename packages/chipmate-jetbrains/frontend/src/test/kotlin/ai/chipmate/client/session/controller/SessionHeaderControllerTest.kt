...
class SessionHeaderControllerTest : SessionControllerTestBase() {
    ...
    fun `test session updated refreshes model metadata`() {
        ...
        assertTrue(modelEvents.any { it is ai.chipmate.client.session.model.SessionModelEvent.SessionUpdated })
    }
    ...
    fun `test compact calls RPC with selected model when eligible`() {
        ...
        assertEquals("chipmate", call.third.providerID)
    ...
    fun `test compact is blocked without model`() {
        appRpc.state.value = ai.chipmate.rpc.dto.ChipMateAppStateDto(ai.chipmate.rpc.dto.ChipMateAppStatusDto.READY, config = ai.chipmate.rpc.dto.ConfigDto(model = null))
