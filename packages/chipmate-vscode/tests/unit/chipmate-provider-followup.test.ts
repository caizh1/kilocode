...
import type { Event, Session } from "@chipmate/sdk/v2/client"
...
describe("ChipMateProvider follow-up sessions", () => {
  it("ignores subagents before adopting pending follow-up sessions", async () => {
    const service = connection()
    const provider = new ChipMateProvider({} as never, service as never)
  ...
  it("calls onFollowupAdopted listeners with session and directory", async () => {
    ...
    internal.pendingFollowup = { dir: "/repo/.chipmate/worktrees/feat", time: Date.now() }
    service.emit(created({ id: "ses-wt", directory: "/repo/.chipmate/worktrees/feat" }))
    await Promise.resolve()

    expect(adopted).toEqual([{ id: "ses-wt", dir: "/repo/.chipmate/worktrees/feat" }])
  })
})
