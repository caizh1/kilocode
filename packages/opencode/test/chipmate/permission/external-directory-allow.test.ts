...
describe("external_directory allow config protection", () => {
  ...
  it.live("persists approval for one exact global skill directory", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const pattern = glob(path.join(ChipMatePaths.globalDirs()[0], "skills", "axiom-sre", "*"))
          const input = {
          ...
          const sibling = glob(path.join(ChipMatePaths.globalDirs()[0], "skills", "other", "*"))
  ...
  it.live("limits selected approval rules to the exact global skill directory", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const pattern = glob(path.join(ChipMatePaths.globalDirs()[0], "skills", "selected-skill", "*"))
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const pattern = glob(path.join(ChipMatePaths.globalDirs()[0], "skills", name, "*"))
          const input = {
            permission: "external_directory",
            patterns: [pattern],
  ...
  it.live("does not drain a global skill from an exact project rule", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const target = glob(
            path.join(ChipMatePaths.globalDirs()[0], "skills", String(PermissionV1.ID.ascending()), "*"),
          )
          const approved = glob(
            path.join(ChipMatePaths.globalDirs()[0], "skills", String(PermissionV1.ID.ascending()), "*"),
          )
