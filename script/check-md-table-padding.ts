 ...
 *
 * Enforcement scope (ChipMate-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "chipmate" or starting with "chipmate-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("chipmate") || p.startsWith("chipmate-"))) return false
