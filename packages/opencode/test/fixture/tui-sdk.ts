...
export function createFetch(override?: FetchHandler) {
  ...
  const fetch = (async (input: RequestInfo | URL) => {
    ...
    switch (url.pathname) {
      case "/agent":
      case "/command":
      // case "/experimental/workspace": // chipmate_change
      case "/experimental/workspace/status":
      case "/formatter":
      case "/lsp":
      case "/network": // chipmate_change
      case "/background-process": // chipmate_change
        return json([])
      case "/config":
      case "/experimental/resource":
      case "/global/config": // chipmate_change
      case "/mcp":
      case "/provider/auth":
      case "/session/status":
        return json({})
      ...
      case "/project/current":
        return json({ id: "proj_test", worktree: "/tmp/project-root" }) // chipmate_change
      case "/provider":
