...
describe("FileWatcher", () => {
  test("bounds watcher startup so a missing ready event cannot hang indexing", async () => {
    const old = process.env.CHIPMATE_INDEXING_WATCHER_READY_TIMEOUT_MS
    ...
    process.env.CHIPMATE_INDEXING_WATCHER_READY_TIMEOUT_MS = "0"
    ...
    try {
      ...
      if (old === undefined) delete process.env.CHIPMATE_INDEXING_WATCHER_READY_TIMEOUT_MS
      else process.env.CHIPMATE_INDEXING_WATCHER_READY_TIMEOUT_MS = old
    }
  })
  ...
  test("processFile skips files matched by .chipmateignore during incremental updates", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "file-watcher-test-"))
    const cacheDir = path.join(root, ".cache")
    const file = path.join(root, "secret.ts")

    await mkdir(cacheDir, { recursive: true })
    await writeFile(path.join(root, ".chipmateignore"), "secret.ts\n")
    ...
    expect(result.reason).toBe("File is ignored by .gitignore or .chipmateignore")
  })

  test("processFile skips plugin state directories under the workspace root", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "file-watcher-test-"))
    const cacheDir = path.join(root, ".cache")
    const file = path.join(root, ".chipmate", "worktrees", "feature", "main.c")
  ...
  test("processFile indexes an explicitly opened worktree root", async () => {
    ...
    const root = path.join(base, ".chipmate", "worktrees", "feature")
