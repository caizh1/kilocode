...
import { staticEnvLines, type EditorContext } from "@/chipmate/editor-context"
import { ChipMateMemory } from "@chipmate/chipmate-memory/effect"
import type { MemoryPaths } from "@chipmate/chipmate-memory/effect/paths"
import { MemoryMarker } from "@/chipmate/memory/marker"
import type { Provider } from "@/provider/provider"
import type { InstanceContext } from "@/project/instance-context"
import * as Log from "@opencode-ai/core/util/log"
import { ProductProfile } from "@/chipmate/product-profile"

const log = Log.create({ service: "chipmate.system-prompt" })

export namespace ChipMateSystemPrompt {
  const identity =
  ...
  export function brand(text: string) {
    ...
    return text
      .split("\n")
      .filter((line) => !line.includes("chipmate.ai/docs"))
      .filter((line) => !line.includes("To give feedback, users should report the issue at"))
      .filter((line) => !line.includes("github.com/ChipMate-Org/chipmate"))
      .join("\n")
      .replace(
        "If the user asks for help or wants to give feedback inform them of the following:",
        "If the user asks for help, inform them of the following:",
      )
      .replaceAll("Get help with using ChipMate", "Get help with ChipMate")
      .replace(/^You are opencode\b/, "You are ChipMate")
      .replaceAll("ChipMate", "ChipMate")
      .replace(/\bChipMate\b(?![-_/])/g, "ChipMate")
  }
  ...
  export function memoryGuidance() {
    const product = ProductProfile.chipmate ? "ChipMate" : "ChipMate"
    return [
      ...
      "When the user explicitly asks you to remember, save, correct, update, or forget project memory, call chipmate_memory_save.",
      "When the user asks about prior work, project history, saved decisions, conventions, setup, or prior rationale beyond what the records below cover, call chipmate_memory_recall (mode=search with likely stored words, then mode=catalog) before relying on general knowledge.",
      "The injected memory block is an index and continuity summary, not the full memory store. When a request depends on exact saved details that are only listed as keys, topics, summaries, or truncated records, call chipmate_memory_recall before answering.",
      "When a request could depend on durable typed memory categories such as project facts, environment commands/paths/tooling, decisions, constraints, or corrections, call chipmate_memory_recall (mode=typed or mode=search) if the injected index only hints at the answer, may be incomplete, or does not include the exact detail needed.",
      ...
      "Use chipmate_memory_recall with mode=digest and sessionID=<id> when the injected digest is too thin but points to a real prior session.",
      "For topic-specific memory, use chipmate_memory_recall with mode=search or mode=typed.",
      "Use chipmate_local_recall with mode=read only when saved memory is insufficient and transcript detail is actually needed, or when the user asks for full transcript detail.",
  ...
  export function environment(input: { ctx: InstanceContext; model: Provider.Model; editor?: EditorContext }) {
    ...
    return [
      [
        ...
        `  Project config: ${dir}/command/*.md, ${dir}/agent/*.md, chipmate.json, AGENTS.md. Put new commands and agents in ${dir}/.`,
