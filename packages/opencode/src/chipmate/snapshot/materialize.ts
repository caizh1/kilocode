...
export namespace ChipMateSnapshotMaterialize {
  ...
  export const ref = (gitdir: string) => `refs/chipmate/materialize/${Hash.fast(path.resolve(gitdir))}`
  const snapshotRef = (hash: string, time = Date.now()) => `refs/chipmate/snapshots/${time}/${hash}`
  ...
  export const prune = Effect.fnUntraced(function* (input: Input, before: number) {
    ...
    const refs = result.text
      ...
      .map((item) => item.trim())
      .filter((item) => {
        const match = item.match(/^refs\/chipmate\/snapshots\/(\d+)\/[0-9a-f]+$/)
