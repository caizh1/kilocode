...
describe("Skill cache refresh", () => {
  it.live("refreshes project Skill discovery and state after filesystem changes", () =>
    Effect.gen(function* () {
      ...
      const first = path.join(dir, ".kilo", "skills", "first", "SKILL.md")
      const second = path.join(dir, ".kilo", "skills", "second", "SKILL.md")
  ...
  it.live("refreshes global Skill caches for every loaded directory", () =>
    Effect.gen(function* () {
      ...
      const file = path.join(home, ".kilo", "skills", "shared", "SKILL.md")
      yield* Effect.promise(() =>
  ...
  it.live("keeps a running session alive while Skill caches refresh", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const file = path.join(dir, ".kilo", "skills", "live", "SKILL.md")
