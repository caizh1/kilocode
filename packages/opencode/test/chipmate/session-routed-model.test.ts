...
import type { Part, StepFinishPart } from "@chipmate/sdk/v2"
import { RoutedModelMeta } from "../../src/chipmate/cli/cmd/tui/routes/session/routed-model-meta"
import { ChipMateRoutedModel } from "../../src/chipmate/session/routed-model"
...
describe("session routed model", () => {
  ...
  test("preserves finish-step response model in provider metadata", async () => {
    const events = await adapt([
      unchecked({
        ...
        usage: {},
        providerMetadata: { openrouter: { routed: true }, chipmate: { existing: true } },
      }),
    ])
    ...
    expect(event.providerMetadata).toEqual({
      ...
      chipmate: { existing: true, routedModelID: "openai/gpt-5.5-20260423" },
    })
  })
  ...
  test("shortens date-suffixed routed model ids for display", () => {
    expect(ChipMateRoutedModel.display("moonshotai/kimi-k2.7-code-20260612")).toBe("moonshotai/kimi-k2.7-code")
    expect(ChipMateRoutedModel.display("openai/gpt-5.5-2026-04-23")).toBe("openai/gpt-5.5")
    expect(ChipMateRoutedModel.display("openai/gpt-5.5")).toBe("openai/gpt-5.5")
  })

  test("formats routed model names for compact display", () => {
    expect(ChipMateRoutedModel.displayName("Qwen: Qwen3.7 Plus (20% off)")).toBe("Qwen 3.7 Plus")
    expect(ChipMateRoutedModel.displayName("anthropic.claude-opus-4-5-20251101-v1:0")).toBe(
      "anthropic.claude-opus-4-5-20251101-v1:0",
    )
    expect(ChipMateRoutedModel.displayName("moonshotai/kimi-k2.7-code")).toBe("kimi-k2.7-code")
    expect(ChipMateRoutedModel.displayName("o3")).toBe("o3")
  })

  test("shows compact labels only for ChipMate auto selections", () => {
    ...
    const routed = RoutedModelMeta.info(undefined, parts, false, {
      providerID: "chipmate",
      modelID: "chipmate-auto/efficient",
    })
    ...
    const same = RoutedModelMeta.info(
      undefined,
      [reason, finish({ providerID: "chipmate", modelID: "chipmate-auto/efficient" })],
      false,
      {
        providerID: "chipmate",
        modelID: "chipmate-auto/efficient",
  ...
  test("reads routed model only for selected ChipMate auto models", () => {
    const meta = { chipmate: { routedModelID: "openai/gpt-5.5-20260423" } }

    expect(
      ChipMateRoutedModel.readAuto(meta, {
        providerID: ProviderV2.ID.chipmate,
        modelID: "chipmate-auto/efficient",
      }),
    ).toEqual({
      providerID: ProviderV2.ID.chipmate,
