...
test("compaction export derives root session from ChipMateSession", async () => {
  const root = join(import.meta.dir, "../../..")
  const text = await Bun.file(join(root, "src/session/compaction.ts")).text()
  const block = text.slice(
    text.indexOf("// chipmate_change start - export self-contained compaction capture"),
    text.indexOf("yield* prune({ sessionID"),
  )
  expect(block).toContain("ChipMateSession.resolveRoot(input.sessionID)")
})

test("llm export does not retain raw stream parts", async () => {
  const root = join(import.meta.dir, "../../..")
  const llm = await Bun.file(join(root, "src/session/llm.ts")).text()
  const events = await Bun.file(join(root, "src/chipmate/session-export/events.ts")).text()
...
test("workspace git subprocesses are hidden on Windows", async () => {
  const root = join(import.meta.dir, "../../..")
  const text = await Bun.file(join(root, "src/chipmate/session-export/workspace-provider.ts")).text()
