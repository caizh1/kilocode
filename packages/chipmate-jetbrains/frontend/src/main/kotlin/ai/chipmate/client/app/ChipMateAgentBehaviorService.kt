...
class ChipMateAgentBehaviorService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: ChipMateAgentBehaviorRpcApi?,
) {
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = ChipMateLog.create(ChipMateAgentBehaviorService::class.java)
    }

    private suspend fun <T> call(block: suspend ChipMateAgentBehaviorRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(ChipMateAgentBehaviorRpcApi.getInstance()) }
    }
