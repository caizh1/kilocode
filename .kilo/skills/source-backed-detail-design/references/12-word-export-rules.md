...
New detailed-design documents use the Codex `standard_business_brief` baseline: US Letter portrait, one-inch margins, Calibri 11 pt body, the standard blue H1/H2/H3 hierarchy, real numbering, fixed-DXA tables, quiet running header, and right-aligned page number. Put `{{TOC}}` after the title/summary for a full detailed design and call `materialize_word_fields` only after all chapter and late-image mutations so the table of contents comes from the final real headings.
