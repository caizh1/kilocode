...
- [ ] Review 重点是确认迁移能力没有破坏 Kilo 原有 QA、代码理解、Document RAG、autocomplete、terminal、tool registry、VS Code extension 激活和打包能力。
- [x] 本计划默认目标仓库为 `/Users/archer/Work/kilocode`。
- [x] 本计划默认源能力仓库为 `/Users/archer/Work/opencode`。

## Summary

- [x] 将 ChipMate 的非 QA 特色能力迁移到 `/Users/archer/Work/kilocode`。
- [x] 保留 Kilo 原生 QA、代码理解、Document RAG、skill discovery、autocomplete 能力。
...
- [x] 不替换 Kilo 原生 agent、QA、terminal、autocomplete、Document RAG 流水线。
- [x] 所有生成文件进入统一 artifact 目录，并返回 manifest。
- [ ] 每个里程碑完成后必须 review 是否影响 Kilo 原有能力。
...
- [ ] M11: 做最终迁移 review，确认没有明显破坏 Kilo 原有能力的问题。

## Key Decisions

- [x] QA 不迁移，继续使用 Kilo 原生 `codebase_analysis`、`semantic_search`、`document_search`。
- [x] ChipMate planner/question routing 不迁移。
- [x] ChipMate CodeGraph/RAG 管理 UI 不迁移。
- [x] Kilo 已有 Document RAG，不迁移 ChipMate Document RAG。
- [x] Kilo 已有 Qwen Coder autocomplete `qwen-direct` 路径，不迁移 ChipMate completion。
- [x] Skill discovery 使用 Kilo 原生能力，不迁移 ChipMate skill discovery。
- [x] Word 通用生成、增删改、渲染、合并、diff 完整迁移。
- [x] Word contract 边界已收紧：仅保留通用工具 schema、参数校验和安全规则；不把 ChipMate 文档 contract 或 repair/gating 流程迁入 Kilo QA。
...
- [x] Artifact 默认目录使用 `.kilo/artifacts`。
- [x] 删除类 Word edit 默认 dry-run。
- [ ] 每个阶段完成后都必须 review 是否影响 Kilo 原有 QA、terminal、tool registry、VS Code extension 激活、settings、packaging。

## Target Architecture

- [x] 在 `/Users/archer/Work/kilocode/packages/opencode/src/kilocode/documents/` 新增文档能力内核。
- [x] 在 `/Users/archer/Work/kilocode/packages/opencode/src/kilocode/tool/` 新增 artifact、Word、Mermaid 工具。
- [x] 在 Kilo tool registry 中以旁路工具方式注册新增工具。
- [x] 在 `/Users/archer/Work/kilocode/packages/kilo-vscode/src/services/` 新增 VS Code artifact UI 和 Agent Terminal 适配。
- [x] 在 Kilo 原生 skill 目录迁入 `documents` skill；该 skill 只提供通用 Word/Mermaid/artifact 指南。
- [x] 在 Kilo 原生 skill 目录迁入 `source-backed-detail-design` skill；该 skill 是可选指南，不是 ChipMate contract/gating runtime。
- [x] 保持 Kilo 原生 agent/QA 主链路不变。
- [x] 保持 Kilo 原生 `document_search` 只负责读已有文档，新 Word tools 负责生成/编辑产物。
...
- [x] Artifact 默认目录为 `.kilo/artifacts/<timestamp>-<task-slug>/`。
...
- [x] 新增 `kilo.documents.openArtifact` command。
- [x] 新增 `kilo.documents.openArtifactFolder` command。
- [x] 新增 `kilo.documents.exportDiagnostics` command。
- [x] 新增 `kilo.agentTerminal.open` command。
- [x] 新增 `kilo.documents.artifacts.root` setting，默认 `.kilo/artifacts`。
- [x] 新增 `kilo.documents.wordRender.remoteEndpoint` setting，默认空。
- [x] 新增 `kilo.documents.wordRender.autoVerify` setting，默认 `false`。
- [x] 明确 Word render runtime 仍通过显式 tool 参数、`KILO_WORD_RENDER_ENDPOINT`，或本机已存在的 `soffice`/`pdftoppm` 环境驱动，不改 Kilo 原生 session 配置链，也不把 renderer 二进制打包进 VSIX。
- [x] 新增 `kilo.documents.tools.enabled` setting，默认 `true`。
- [x] `kilo.documents.tools.enabled` 至少保护 VS Code artifact 命令入口；普通 QA、Document RAG、autocomplete、原生 terminal 不受该开关影响。
- [x] 新增 `kilo.agentTerminal.enabled` setting，默认 `false`。

## M0: 迁移文档和边界

- [x] 新增 `/Users/archer/Work/kilocode/docs/no-qa-regression-boundary.md`，明确 QA 不迁移。
- [x] 新增 `/Users/archer/Work/kilocode/docs/document-tools-contract.md`，定义 artifact、Word、Mermaid 通用 tool API 边界；该文件不代表迁移 ChipMate runtime contract。
- [x] 新增 `/Users/archer/Work/kilocode/docs/source-backed-detail-design-skill-contract.md`，定义详细设计 skill 使用 Kilo 原生证据工具的边界；该文件已收紧为 skill boundary，不迁移 ChipMate skill contract。
- [x] 明确普通代码问答、调用链分析、宏/寄存器/MMIO 分析继续优先走 Kilo 原生代码理解工具。
- [x] 明确只有用户要求 Word、`.docx`、图、详细设计文档、artifact、Agent Terminal 时才触发新能力。
- [x] Review M0: 检查文档中没有写成“替换 Kilo QA”或“迁移 ChipMate planner”。
- [x] Review M0: 检查计划没有要求修改 Kilo 原有 QA prompt 的核心行为。
...
- [x] 将 artifact tools 注册到 Kilo tool registry。
- [x] artifact 根目录默认 `.kilo/artifacts`。
...
- [x] 增加 tool registry 回归测试覆盖 artifact tools 追加且不替换原有 Kilo QA tools。
...
- [x] Review M1: 检查 Kilo 原有 tool registry 工具仍可正常注册。
...
- [x] 扩展 tool registry 回归测试覆盖 Word tools 追加且不替换原有 Kilo QA tools。
- [x] 测试创建简单 Word。
- [x] 测试创建含表格 Word。
- [x] 测试创建含图片 Word。
...
- [x] Review M2: 检查没有覆盖 Kilo `document_search` 的读取职责。
- [x] Review M2: 检查新增依赖不会破坏 Kilo build/package。
...
- [x] 静态 Review M3: Word edit/tool 操作失败会返回普通 tool result，不替换或中断 Kilo 原生 QA 路由；真实 session 行为仍留到 M10 smoke。
- [x] Review M3: 检查普通文件 edit 工具仍按 Kilo 原行为工作。
...
- [x] Review M4: 检查新增 Word 能力仍然不影响 Kilo document_search。
...
- [x] 迁移 `source-backed-detail-design` skill 到 Kilo 原生 skill 目录。
- [x] 迁移或合并 `chip-design-doc` skill 中的关键文档结构。
- [x] 将 skill 内 ChipMate 工具名替换为 Kilo 新工具名。
- [x] 更新 `source-backed-detail-design` skill：详细设计流程改为 Kilo 原生 agent 可选指南，不强制 ChipMate work-package contract、Word contract 或 quality-gate blocking 机制。
- [x] Skill 明确先用 Kilo 原生 `codebase_analysis`、`semantic_search`、`document_search` 搜证据。
...
- [x] Review M7: 检查 skill 不绕过 Kilo 原生代码理解工具。
- [x] Review M7: 检查普通问答不会被强制套详细设计流程。
- [x] Review M7: 检查详细设计生成失败不会影响 Kilo QA 会话。
...
- [x] Review M8: 检查 UI 不影响 Kilo 原有 chat rendering。
- [x] Review M8: 检查 artifact 卡片不会出现在普通 QA 中。
- [x] Review M8: 检查按钮和命令不会覆盖 Kilo 原有 command id。

## M9: Agent Terminal

- [x] 新增 `kilo.agentTerminal.enabled` 设置，初期默认关闭。
- [x] 新增 `kilo.agentTerminal.open` command。
- [x] 新增 VS Code terminal profile。
- [x] 迁移 Agent Terminal runtime 中可复用部分。
- [x] 迁移 VS Code terminal profile 适配。
- [x] 迁移项目上下文摘要能力。
- [x] 复用 Kilo 原生命令/agent 能力。
- [x] 不替换 Kilo shell tool。
- [x] 支持自然语言命令规划。
- [x] 支持危险命令确认。
- [x] 支持命令失败后的修复建议。
- [x] 支持项目上下文摘要。
...
- [x] 不改 Kilo 原生 terminal/session manager 行为，除非为新增 profile 做最小适配。
- [x] 测试打开 Agent Terminal。
- [x] 测试自然语言安全命令。
- [x] 测试危险命令确认。
- [x] 测试命令失败修复建议。
- [x] Review M9: 检查 Agent Terminal 默认关闭。
- [x] Review M9: 检查没有替换 Kilo 原有 terminal/session manager。
...
- [x] 增加 Kilo 原生 `codebase_analysis` 仍可用的回归测试。
- [x] 增加 Kilo 原生 `semantic_search` 仍可用的回归测试。
- [x] 增加 Kilo 原生 `document_search` 仍可用的回归测试。
- [x] 增加 Kilo autocomplete/Qwen direct 不受影响的 smoke 检查。
...
- [x] 新增 Kilo no-regression runtime review 模板，用于 M11 逐项记录原生 QA、Document RAG、autocomplete、terminal、tool registry、VS Code 激活和打包能力是否被破坏。
- [x] 记录版本、artifact、验证命令、已知限制。
- [x] Review M10: 检查所有新增能力能按需触发。
- [x] Review M10: 检查所有非目标能力没有被误迁移。
- [x] Review M10: 检查 Kilo 原有 QA、terminal、autocomplete、Document RAG 没有明显退化；当前结论为 `PASS_WITH_LIMITS`，focused no-regression refresh `docs/chipmate-feature-migration-validation-runs/20260708-045849-m10-kilo-no-regression-review/summary.md` 通过，但 installed S1-S3/S14-S16 和 M11 final no-regression 仍未完成。
...
- [x] Internal offline Linux x64 VSIX generated: `packages/kilo-vscode/out/kilo-vscode-linux-x64-baseline.vsix`, version `0.0.38`, `143.95 MiB`.
- [x] Internal offline Windows x64 VSIX generated: `packages/kilo-vscode/out/kilo-vscode-win32-x64-baseline.vsix`, version `0.0.38`, `159.93 MiB`.
...
- [x] Added and ran targeted Word test coverage for complex block replacement, ambiguous locator rejection, and render artifact manifest updates: `cd packages/opencode && bun test test/kilocode/word-documents.test.ts --timeout 60000`, 7/7 pass.
- [x] Refreshed Word focused test after local renderer fallback: `docs/chipmate-feature-migration-validation-runs/20260708-225900-word-render-local-fallback-focused-test/summary.md`; `bun test test/kilocode/word-documents.test.ts --timeout 60000` passes 7/7 and now covers endpoint warning, remote renderer, and fake local `soffice`/`pdftoppm` fallback wiring.
- [x] Added and ran targeted VS Code artifact/Agent Terminal service coverage: `cd packages/kilo-vscode && bun test tests/unit/document-artifact-card-service.test.ts tests/unit/document-artifact-card-view.test.ts tests/unit/agent-terminal-runtime.test.ts tests/unit/agent-terminal-service.test.ts tests/unit/agent-terminal-package-contribution.test.ts --timeout 60000`, 12/12 pass.
- [x] Added and reran source-backed detail-design skill boundary coverage after removing ChipMate Word/document contract scope and old contract-helper resources: `cd packages/opencode && bun test test/kilocode/source-backed-detail-design-skill.test.ts --timeout 60000`, 5/5 pass.
- [x] Added and ran VS Code extension-host smoke for activation, command registry, and settings visibility: `cd packages/kilo-vscode && bun run compile-tests && bunx vscode-test --fail-zero --timeout 30000`, 2/2 pass; summary in `docs/chipmate-feature-migration-validation-runs/20260708-084152-extension-host-smoke/summary.md`.
- [x] Re-ran VS Code extension lint after extension-host smoke changes: `cd packages/kilo-vscode && bun run lint`, PASS.
- [x] Hardened Poppler Windows packaging with `POPPLER_WINDOWS_ARCHIVE`, `KILO_POPPLER_CACHE_DIR`, retry, and fetch timeout support; `cd packages/kilo-vscode && bun test tests/unit/poppler-helper.test.ts --timeout 60000`, 2/2 pass.
- [x] Rebuilt current internal-offline Linux and Windows VSIX after test-tooling and Poppler helper changes: `cd packages/kilo-vscode && bun run package:internal-offline`, PASS; current Linux size `150944476` bytes / `143.95 MiB`, current Windows size `167696538` bytes / `159.93 MiB`.
...
- [x] Attempted CLI native C QA smoke on QEMU UFS: `docs/chipmate-feature-migration-validation-runs/20260708-092652-cli-native-c-qa-smoke/summary.md`; Kilo started indexing and reused codegraph/postings, but the run is not accepted as S1 pass because the configured DeepSeek credential returned `401 invalid api key`.
...
- [x] Added runtime unblock runbook for finishing M10/M11 after provider/template/internal workspace are available: `docs/chipmate-feature-migration-runtime-unblock-runbook.md`; enhanced `docs/chipmate-feature-migration-runtime-smoke.sh` to inject `KILO_SMOKE_TEMPLATE_DOCX` into S9; verification `docs/chipmate-feature-migration-validation-runs/20260708-095817-runtime-unblock-runbook-verify/summary.md` confirms script syntax and help output.
- [x] Ran runtime smoke preflight against the current Kilo auth state: `docs/chipmate-feature-migration-validation-runs/20260708-093856-runtime-smoke-preflight-current-auth/summary.md`; current status remains `BLOCKED_AUTH`, so installed/chat acceptance prompts still cannot be accepted.
...
- [x] Added offline handoff companion files beside the target VSIX artifacts: `packages/kilo-vscode/out/SHA256SUMS-chipmate-0.0.38-offline.txt` and `packages/kilo-vscode/out/OFFLINE_RELEASE_NOTES-chipmate-0.0.38.md`; validation summary `docs/chipmate-feature-migration-validation-runs/20260708-094445-offline-handoff-files/summary.md`; these record target filenames, version, size, SHA256, install command, and remaining runtime sign-off caveats.
...
- [x] Added offline release index companion files: `packages/kilo-vscode/out/OFFLINE_RELEASE_INDEX-chipmate-0.0.38.json` and `packages/kilo-vscode/out/OFFLINE_RELEASE_INDEX-chipmate-0.0.38.md`; validation summary `docs/chipmate-feature-migration-validation-runs/20260708-094727-offline-release-index/summary.md`; these list target artifacts, checksums, verification references, and remaining sign-off caveats.
...
- [x] Created clean non-self-referential offline handoff bundle: `packages/kilo-vscode/out/chipmate-0.0.38-offline-handoff.tar.gz` plus `packages/kilo-vscode/out/chipmate-0.0.38-offline-handoff.tar.gz.sha256`; AppleDouble cleanup summary `docs/chipmate-feature-migration-validation-runs/20260708-101239-offline-handoff-bundle-clean-rebuild/summary.md`; final bundle-internal docs rebuild summary `docs/chipmate-feature-migration-validation-runs/20260708-102040-offline-handoff-bundle-final-docs-rebuild/summary.md`; external bundle index files `packages/kilo-vscode/out/OFFLINE_BUNDLE_INDEX-chipmate-0.0.38.json` and `packages/kilo-vscode/out/OFFLINE_BUNDLE_INDEX-chipmate-0.0.38.md`; bundle SHA256 `159de0e74c619e430d02a19e614fc0ebbaa58710fb831b2bed01e070bf10f752`; size `316392364` bytes / `301.74 MiB`.
...
- [x] Reran focused built-in skill and source-backed boundary tests: `bun test test/kilocode/builtin-skills.test.ts test/kilocode/source-backed-detail-design-skill.test.ts --timeout 60000`, 9/9 pass.
...
- [x] Refreshed clean offline handoff bundle: `packages/kilo-vscode/out/chipmate-0.0.38-offline-handoff.tar.gz`, SHA256 `97a70108ae648a1998168d679c3c65638ea7756926860fad113cd06c31d2dba5`, size `317784738` bytes; package-integrity evidence `docs/chipmate-feature-migration-validation-runs/20260708-114900-offline-handoff-verify-after-repackage/summary.md`.
...
- [x] Windows target package runner now downgrades its summary to `PASS_WITH_LIMITS` and writes `BLOCKED_ENV` intake evidence when Python or the intake verifier is unavailable; refreshed kit/delivery evidence `docs/chipmate-feature-migration-validation-runs/20260708-045533-windows-runner-intake-boundary-refresh/summary.md`. This preserves the package-only boundary and does not claim Windows runtime S1-S16 or Kilo native QA no-regression completion.
- [x] M10 Kilo no-obvious-regression focused review refreshed: `docs/chipmate-feature-migration-validation-runs/20260708-045849-m10-kilo-no-regression-review/summary.md`; native tool boundary 32/32 pass, VS Code contribution boundary 10/10 pass. This checks the review action only and does not close installed runtime S1-S16 or M11.
...
- [x] Review permissions，确认写文件、删除、覆盖、命令执行走 Kilo 权限体系。
...
- [x] Review source-backed detail-design skill，确认使用 Kilo 原生证据工具。
...
- [x] 建立最终 review 模板，要求最终结论明确列出已迁移能力、未迁移能力、验证过的 Kilo 原有能力和已知风险。
- [x] Review known issues，确认未完成项写入文档，不伪装成已完成。
- [ ] 最终结论必须明确写出已迁移能力、未迁移能力、验证过的 Kilo 原有能力、已知风险。

## Regression Review Matrix

- [x] 检查普通代码 QA 是否仍优先使用 Kilo 原生 `codebase_analysis`。
- [x] 检查开放式代码搜索是否仍可使用 Kilo 原生 `semantic_search`。
- [x] 检查已有文档问答是否仍可使用 Kilo 原生 `document_search`。
- [x] 检查 Kilo autocomplete/Qwen direct 设置和 provider 注册未受影响。
- [x] 检查 Kilo 原有 terminal/session manager 未受 Agent Terminal 影响。
- [x] 静态检查 Kilo VS Code extension 激活路径没有被替换；新增模块以追加注册方式接入。
- [x] 检查新增 document tools 不会默认抢占普通 QA。
- [x] 检查新增 settings 不改变 Kilo 既有默认 provider/model 行为。
- [x] 检查新增 artifact 文件不会污染 repo tracked source：`.gitignore` 已忽略 `.kilo/artifacts/` 和 `.kilocode/artifacts/`。
...
- [ ] 详细设计: “为该嵌入式 C 模块生成源码驱动详细设计文档”，预期 Kilo 搜证据，生成 Markdown、图、Word。
...
- [ ] 打包验收: 安装 VSIX 后普通 Kilo QA、Word、Mermaid、artifact、Agent Terminal smoke 均通过。
...
- [x] 不迁移 Kilo 已有 Document RAG。
- [x] 不迁移 Kilo 已有 Qwen Coder autocomplete。
- [x] 不迁移完整 draw.io 链路。
- [x] 不实现完整 mail-merge 占位符引擎。
- [x] 不迁移 AI 注释生成 runtime。
- [x] 不迁移 C 编码规范专项报告。
- [x] 不让 Agent Terminal 替换 Kilo 原生命令工具。
- [x] 不把 renderer 大依赖打进 VSIX。

## Assumptions

- [x] 计划文件默认保存到 `/Users/archer/Work/kilocode/docs/chipmate-feature-migration-plan.md`。
- [x] Artifact 根目录默认 `.kilo/artifacts`。
...
- [x] Skill discovery 使用 Kilo 原生。
- [x] 详细设计证据工具使用 Kilo 原生 `codebase_analysis`、`semantic_search`、`document_search`。
...
- [x] Added internal embedded-C source-backed detail-design evidence intake helper: `docs/chipmate-feature-migration-internal-embedded-c-intake.py` validates returned evidence shape without running Kilo QA, VS Code, source-backed skill execution, Word generation, diagram rendering, recipe repair, missing-diagram auto-repair, or document-contract planning.
