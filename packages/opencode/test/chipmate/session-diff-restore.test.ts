...
import { appendSessionDiffs, mergeSessionDiffs } from "../../src/chipmate/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "../../src/chipmate/session-portability/session-diff-restore"

const dirs: string[] = []

function tmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "chipmate-session-diff-"))
  dirs.push(dir)
  return dir
}
