import { ChipMateTitleIcon } from "./title-icon"
...
export namespace ChipMateTerminalTitle {
  ...
  } satisfies Record<ChipMateTitleIcon.Value, Record<Indicator, string>>

  export function format(input: { base: string; title?: string; indicator: Indicator; icon?: ChipMateTitleIcon.Value }) {
    const text = input.title ? `${input.base} | ${truncate(input.title)}` : input.base
    const prefix = icons[input.icon ?? ChipMateTitleIcon.Default][input.indicator]
