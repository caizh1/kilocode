/**
 * Question handlers — extracted from ChipMateProvider.
 *
...
import type { ChipMateClient, QuestionRequest } from "@chipmate/sdk/v2/client"
  ...
  try {
    for (;;) {
      ...
      for (const dir of dirs) {
        ...
        if (error) {
          failed.add(dir)
          console.error(`[ChipMate New] ChipMateProvider: Failed to fetch pending questions for ${dir}:`, error)
          continue
        }
        scanned.add(dir)
        if (!data) continue
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to fetch pending questions:", error)
  }
}
  ...
  try {
    ...
    if (route?.kind === "retry" && route.dir !== dir) {
      try {
        ...
        console.error("[ChipMate New] ChipMateProvider: Failed to reply to recovered question:", retry)
        ctx.postMessage({ type: "questionError", requestID })
        return false
      }
    }
    console.error("[ChipMate New] ChipMateProvider: Failed to reply to question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
  ...
  try {
    ...
    if (route?.kind === "retry" && route.dir !== dir) {
      try {
        ...
        console.error("[ChipMate New] ChipMateProvider: Failed to reject recovered question:", retry)
        ctx.postMessage({ type: "questionError", requestID })
        return false
      }
    }
    console.error("[ChipMate New] ChipMateProvider: Failed to reject question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
