---
title: "Using LM Studio with ChipMate | Local LLMs"
description: "Run local LLMs in ChipMate by connecting to LM Studio's OpenAI-compatible server. Setup guide for VS Code and the CLI."
sidebar_label: LM Studio
---

# Using LM Studio With ChipMate

ChipMate supports running models locally using LM Studio. LM Studio provides a user-friendly interface for downloading, configuring, and running local language models. It also includes a built-in local inference server that emulates the OpenAI API, making it easy to integrate with ChipMate.
...
2.  **Download a Model:** Use the LM Studio interface to search for and download a model. Some recommended models include:
    ...
    - Any other model that is supported by ChipMate, or for which you can set the context window.
...
If the model you loaded in LM Studio doesn't appear in the ChipMate model picker, you can register it as a custom model in your config file:
...
- **Local Server:** The LM Studio local server must be running for ChipMate to connect to it.
