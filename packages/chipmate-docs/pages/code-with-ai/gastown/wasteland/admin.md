...
4. Authorize DoltHub — OAuth through ChipMate Integrations is the default. The advanced API token option is also supported, but it must have write access to the upstream repo.
...
### ChipMate Cloud hosted option

<!-- TODO: verify — confirm whether ChipMate Cloud offers a hosted wasteland creation flow or if self-serve is CLI/dashboard only -->

ChipMate Cloud provides the managed infrastructure (Cloudflare containers, encryption, billing) but the upstream DoltHub database is always user-owned. There is no fully-hosted wasteland where ChipMate manages the DoltHub repo on your behalf — you always create and own the upstream. The wasteland container runs on ChipMate's infrastructure, but data lives in your DoltHub account.
...
{% browserFrame url="app.chipmate.ai/wasteland/my-org/wl-internal/wanted/post" caption="Posting a wanted item — title, description, priority, and type fields" %}
...
<!-- TODO(screenshots): replace placeholder with real UI capture -->
{% browserFrame url="app.chipmate.ai/wasteland/my-org/wl-internal/review" caption="The review inbox — pending submissions grouped by type" %}
...
1. Click **Add member**
2. Enter the user's ChipMate user ID
...
The **Test admin access** button checks whether ChipMate can write to a scratch branch on the upstream. If it works, you see a green "Admin access verified" badge. If it fails, DoltHub OAuth may be disconnected, the API token may be invalid, the wrong org may be connected, or write access may be missing.

Use this after reconnecting DoltHub OAuth, updating an API token, or toggling the admin checkbox. It confirms ChipMate can push before you run admin actions.
