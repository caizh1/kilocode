 ...
 * Uses the SDK's chipmate.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  console.info(`[FIM] request provider=${info.providerID} model=${info.requestModel} url=/chipmate/fim`)

  const { stream } = await client.chipmate.fim(
    {
      prefix,
      suffix,
 ...
 */
export function hasValidCredentials(connectionService: ChipMateConnectionService): boolean {
