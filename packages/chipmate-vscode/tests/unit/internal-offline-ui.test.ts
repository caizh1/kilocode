...
describe("internal offline webview gateway UI", () => {
  ...
  it("hides ChipMateClaw and Profile entry points from internal builds", () => {
    ...
    for (const command of ["chipmate.v2.chipmateClawOpen", "chipmate.v2.profileButtonClicked"]) {
    ...
    expect(prompt).toContain('if (isInternalOfflineBuild()) hidden.add("chipmateclaw")')
  })
})
