...
class ChipMateCliDownloaderTest {
    @TempDir
    ...
    fun `downloads extracts and caches pinned cli`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertEquals(File(File(dir, "1.2.3"), ChipMateCliPlatform.current()).absolutePath, cli.parentFile.parentFile.absolutePath)
            assertEquals("#!/bin/sh\n", cli.readText())
            assertTrue(File(cli.parentFile, "chipmate-sandbox-mutation-worker.js").isFile)
            assertEquals("/release/v1.2.3/chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(CliDownload(0, "1.2.3", ChipMateCliPlatform.current()), seen.first())
            assertTrue(seen.any { it.percent == 100 && it.version == "1.2.3" && it.platform == ChipMateCliPlatform.current() })
            assertTrue(
                log.messages.any {
                    it.startsWith("INFO: ChipMate CLI 1.2.3 for ${ChipMateCliPlatform.current()} is not cached; downloading new release into ") &&
                        it.contains("/.tmp/")
                }
            )
            assertTrue(log.messages.any { it.contains("ChipMate CLI path diagnostics:") && it.contains("cacheRoot=${dir.absolutePath}") })
            assertTrue(log.messages.any { it.contains("ChipMate CLI cache target:") && it.contains("exe=${cli.absolutePath}") })
            assertTrue(log.messages.any { it.contains("ChipMate CLI cache lock path:") && it.contains(File(dir, ".lock").canonicalPath) })

            val cachedProgress = mutableListOf<CliDownload>()
            val cached = ChipMateCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            assertContains(log.messages, "INFO: ChipMate CLI 1.2.3 (${ChipMateCliPlatform.current()}) already cached at ${cli.absolutePath}; skipping download and extraction")

            File(cli.parentFile.parentFile, ".complete").writeText("ok\n")
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val stale = ChipMateCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val forced = ChipMateCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
    ...
    fun `falls back to github metadata when bundled checksum is malformed`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val cli = ChipMateCliDownloader(
                ...
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
                digests = mapOf(ChipMateCliPlatform.current() to "not-a-digest"),
            ).resolve("1.2.3")

            assertTrue(cli.isFile)
            assertContains(
                log.messages,
                "WARN: Ignoring malformed bundled ChipMate CLI checksum for ${ChipMateCliPlatform.current()}: not-a-digest"
            )
            assertEquals("/api/v1.2.3", server.takeRequest().path)
            assertEquals("/release/v1.2.3/chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(2, server.requestCount)
    ...
    fun `prunes stale versions and removes the extracted archive`() = runBlocking {
        MockWebServer().use { server ->
            val stale = File(File(dir, "0.0.1"), ChipMateCliPlatform.current())
            ...
            val archived = File(cli.parentFile.parentFile, "chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}")
    ...
    fun `rejects cli archive with mismatched digest`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertFalse(File(File(File(dir, "1.2.3"), ChipMateCliPlatform.current()), ".complete").exists())
        }
    }
        ...
        MockWebServer().use { server ->
            ...
            val ex = assertFailsWith<IllegalStateException> {
                ChipMateCliDownloader(
                    ...
                    baseUrl = server.url("/release").toString(),
                    api = server.url("/api").toString(),
                    digests = mapOf(ChipMateCliPlatform.current() to "sha256:${sha256("different".toByteArray())}"),
                ).resolve("1.2.3")
            }
    ...
    fun `fails clearly and logs when the release has no matching asset`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val name = "chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}"
    ...
    fun `fails clearly and logs when the asset has no digest`() = runBlocking {
        MockWebServer().use { server ->
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
                    """{"assets":[{"name":"chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}"}]}"""
                )
            )
            val log = TestLog()
            val ex = assertFailsWith<IllegalStateException> {
    ...
    fun `cache lock times out clearly when already held in this process`() = runBlocking {
        ...
        RandomAccessFile(file, "rw").channel.use { channel ->
            channel.lock().use {
                ...
                assertContains(ex.message.orEmpty(), "Timed out waiting for ChipMate CLI cache lock")
                assertContains(ex.message.orEmpty(), file.canonicalPath)
                assertTrue(log.messages.any { it.contains("Waiting for ChipMate CLI cache lock") && it.contains(file.canonicalPath) })
                assertTrue(log.messages.any { it.contains("Timed out waiting for ChipMate CLI cache lock") && it.contains(file.canonicalPath) })
            }
        }
    }
    ...
    private fun archive(script: String = "#!/bin/sh\n"): ByteArray {
        val files = mapOf(
            "bin/${ChipMateCliPlatform.exe()}" to script.toByteArray(),
            "bin/chipmate-sandbox-mutation-worker.js" to "worker\n".toByteArray(),
        )
        if (ChipMateCliPlatform.archive() == "zip") return zip(files)
        return tar(files)
    }

    private fun metadata(bytes: ByteArray) = metadata("sha256:${sha256(bytes)}")

    private fun digests(bytes: ByteArray) = mapOf(ChipMateCliPlatform.current() to "sha256:${sha256(bytes)}")

    private fun metadata(digest: String) = MockResponse().setResponseCode(200).setBody(
        """{"assets":[{"name":"chipmate-${ChipMateCliPlatform.current()}.${ChipMateCliPlatform.archive()}","digest":"$digest"}]}"""
    )
