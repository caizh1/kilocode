...
class ChipMatePromptCompletionProvider(
    private val workspace: Workspace,
    private val service: ChipMateWorkspaceService,
    ...
    private fun slash(prefix: String, result: CompletionResultSet) {
        ...
        result.withPrefixMatcher(PlainPrefixMatcher.ALWAYS_TRUE)
            .addElement(info(prefix, ChipMateBundle.message("prompt.completion.noMatches")))
    }
    ...
    private fun mention(prefix: String, result: CompletionResultSet) {
        ...
        if (search.indexing) {
            val msg = ChipMateBundle.message("prompt.mention.indexing")
        ...
        if (known.isEmpty() && search.files.isEmpty()) {
            val msg = ChipMateBundle.message("prompt.completion.noMatches")
            out.addElement(info(prefix, msg))
        }
    }
