...
import { assertNetwork, assertWrite, enabled as sandboxed } from "@chipmate/sandbox"
import { Bus } from "@/bus"
import { GlobalBus } from "@/bus/global"
import { Config } from "@/config/config"
import * as Network from "@/chipmate/sandbox/network"
import * as SandboxPolicy from "@/chipmate/sandbox/policy"
import { SandboxStore } from "@/chipmate/sandbox/store"
...
test("refreshes the session snapshot after a backend restart", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-sandbox-restart-"))
  ...
  const env = {
    ...process.env,
    CHIPMATE_TEST_HOME: path.join(root, "home"),
...
posix("canonicalizes a symlinked policy state root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-sandbox-state-link-"))
  const target = path.join(root, "real-state")
  const link = path.join(root, "state")
  await fs.mkdir(target)
  await fs.symlink(target, link)
  const script = 'import { SandboxStore } from "@/chipmate/sandbox/store"; console.log(SandboxStore.root)'
  ...
  try {
    ...
    expect(result.stdout.toString().trim().split("\n").at(-1)).toBe(
      path.join(await fs.realpath(target), "chipmate-sandbox-policy"),
    )
  } finally {
    await fs.rm(root, { recursive: true, force: true })
  }
...
linux("reports configured network namespace availability", async () => {
  const root = await fs.mkdtemp(path.join(process.env.TMPDIR ?? "/tmp", "chipmate-sandbox-status-"))
  ...
  try {
    const result = Bun.spawnSync([process.execPath, "-e", script], {
      cwd: import.meta.dir,
      env: { ...process.env, CHIPMATE_BWRAP_PATH: helper, CHIPMATE_SERVER_PASSWORD: "sandbox-test" },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
...
it.instance("does not let project config weaken an initialized policy", () =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const password = Flag.CHIPMATE_SERVER_PASSWORD
      Flag.CHIPMATE_SERVER_PASSWORD = "sandbox-test"
      return password
    }),
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const file = path.join(test.directory, "chipmate.json")
    ...
    (password) => Effect.sync(() => (Flag.CHIPMATE_SERVER_PASSWORD = password)),
  ),
)
