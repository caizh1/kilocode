...
import {
  ...
  clearModesCache,
  fetchBalance,
  fetchChipMateNotifications,
  fetchChipMatePassState,
  fetchOrganizationModes,
  fetchProfile,
} from "@chipmate/chipmate-gateway"
import { DIRECT_FIM_ENV, requestMistralFim, resolveFimTarget } from "@chipmate/chipmate-gateway/fim"
import { DIRECT_EDIT_ENV, extractFencedBody, resolveEditTarget } from "@chipmate/chipmate-gateway/edit"
import { buildMercuryEditPrompt } from "@chipmate/chipmate-gateway/edit-prompt"
import { buildChipMateHeaders } from "@chipmate/chipmate-gateway"
...
import { AudioTranscriptionsBody, ClawStatus, CloudSessionImportError, EditBody, FimBody, QwenFimBody } from "../groups/chipmate-gateway"
...
const log = Log.create({ service: "chipmate-gateway" })
...
export const chipmateGatewayHandlers = HttpApiBuilder.group(InstanceHttpApi, "gateway", (handlers) =>
  Effect.gen(function* () {
    ...
    const profile = Effect.fn("ChipMateGatewayHttpApi.profile")(function* () {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))

      const currentOrgId = info.accountId ?? null
      const [profile, balance, chipmatePass] = yield* Effect.tryPromise({
        try: () =>
          Promise.all([
    ...
    const authStatus = Effect.fn("ChipMateGatewayHttpApi.authStatus")(function* () {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const proxyAuth = Effect.fn("ChipMateGatewayHttpApi.proxyAuth")(function* () {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return {
        auth: info,
        token: getToken(info),
    ...
    const modes = Effect.fn("ChipMateGatewayHttpApi.modes")(function* () {
    ...
    const fim = Effect.fn("ChipMateGatewayHttpApi.fim")(function* (ctx: { payload: typeof FimBody.Type }) {
      const target = resolveFimTarget(ctx.payload.provider, ctx.payload.model)
      const info = target.provider === "chipmate" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "chipmate") return info?.token
      ...
      if (target.provider === "chipmate" && !info?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
          ...
          const run = async (url: string): Promise<Response> => {
            ...
            return fetch(url, {
              ...
              headers: {
                ...
                ...(target.provider === "chipmate"
                  ? buildChipMateHeaders(undefined, { chipmateOrganizationId: info?.organizationId })
                  : {}),
                ...(target.provider === "chipmate" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
              },
              signal,
              body: JSON.stringify({
                model: target.model,
    ...
    const qwenFim = Effect.fn("ChipMateGatewayHttpApi.qwenFim")(function* (ctx: { payload: typeof QwenFimBody.Type }) {
    ...
    const edit = Effect.fn("ChipMateGatewayHttpApi.edit")(function* (ctx: { payload: typeof EditBody.Type }) {
      const target = resolveEditTarget(ctx.payload.provider, ctx.payload.model)
      if (target.provider === "chipmate" && !target.url) {
        return yield* Effect.fail(new HttpApiError.BadRequest({}))
      }
      const proxy = target.provider === "chipmate" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "chipmate") return proxy?.token
        const item = yield* auth.get(target.provider).pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
        if (item?.type === "api") return item.key
        return DIRECT_EDIT_ENV[target.provider].map((key) => process.env[key]).find(Boolean)
      })
      if (target.provider === "chipmate" && !proxy?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const audioTranscriptions = Effect.fn("ChipMateGatewayHttpApi.audioTranscriptions")(function* (ctx: {
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          fetch(`${CHIPMATE_API_BASE}/api/gateway/v1/audio/transcriptions`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const notifications = Effect.fn("ChipMateGatewayHttpApi.notifications")(function* () {
      // Locally-detected notice about leftover opencode config; appended so it reuses each client's dismissal path.
      const notice = ChipMateConfig.opencodeConfigNotification({
        directory: Instance.directory,
        worktree: Instance.worktree,
        scanProject: !Flag.CHIPMATE_DISABLE_PROJECT_CONFIG,
      })
    ...
    const organization = Effect.fn("ChipMateGatewayHttpApi.organization")(function* (ctx) {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      yield* cache.clear("chipmate")
      clearModesCache()
      yield* store.disposeAll().pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return true
    })

    const clawStatus = Effect.fn("ChipMateGatewayHttpApi.clawStatus")(function* () {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.ServiceUnavailable({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
        ...
        Effect.match({
          onFailure: (err) => {
            if (err instanceof GatewayError)
              return jsonError(`ChipMateClaw request failed: ${err.status} ${err.message}`, err.status)
            logError("claw/status", err)
            return jsonError("Failed to reach ChipMateClaw", 502)
          },
    ...
    const clawChatCredentials = Effect.fn("ChipMateGatewayHttpApi.clawChatCredentials")(function* () {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessions = Effect.fn("ChipMateGatewayHttpApi.cloudSessions")(function* (ctx) {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSession = Effect.fn("ChipMateGatewayHttpApi.cloudSession")(function* (ctx) {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessionImport = Effect.fn("ChipMateGatewayHttpApi.cloudSessionImport")(function* (ctx) {
      const info = yield* auth.get("chipmate").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const imageModels = Effect.fn("ChipMateGatewayHttpApi.imageModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      const result = yield* Effect.tryPromise({
        try: () =>
          fetchChipMateImageModels({
            chipmateToken: info.token,
            chipmateOrganizationId: info.organizationId,
          }),
        catch: () => new HttpApiError.BadRequest({}),
      })
    ...
    const transcriptionModels = Effect.fn("ChipMateGatewayHttpApi.transcriptionModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
