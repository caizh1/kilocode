...
function makeStubChipMateDir({ mode, callLog, stderrText = "event stream disconnected" }) {
  const dir = mktemp("docs-sync-chipmate-")
  const chipmatePath = path.join(dir, "chipmate")
...
if (mode === "write-edit-summary") {
  ...
  // emitting stderr so selftest can assert runChipMate persisted it unconditionally.
  process.stderr.write(stderrText + "\\n");
...
if (mode === "triage-embed-env-secret") {
  ...
  const secret = process.env.CHIPMATE_API_KEY || "missing-secret";
...
process.exit(1);
`
  writeExecutable(chipmatePath, script)
  return dir
}
...
function runNodeScript(scriptPath, { cwd, env = {}, chipmateDir, args = [] }) {
  const pathEnv = [chipmateDir, process.env.PATH].filter(Boolean).join(path.delimiter)
...
function samplePr(n, { merged_at, repo = "ChipMate-Org/cloud" } = {}) {
  return {
    repo,
    number: n,
    title: `feat: sample ${n}`,
...
// ---------------------------------------------------------------------------
// Case 2 — Defect B: edit.mjs with stub chipmate (exit 0 + stderr)
// ---------------------------------------------------------------------------
function case2_defectB() {
  ...
  const chipmateDir = makeStubChipMateDir({ mode: "stderr-exit0", stderrText })
...
// ---------------------------------------------------------------------------
// Case 2b — AC4a: every docs-sync chipmate run argv carries --auto
// ---------------------------------------------------------------------------
...
/** Label → chipmate-stderr filename rule (must match lib.mjs runChipMate). */
function chipmateStderrLogName(label) {
  return `chipmate-stderr-${String(label).replace(/[^A-Za-z0-9._-]/g, "-")}.log`
}

function case2b_autoFlag() {
  console.log("case 2b: AC4a (--auto on every docs-sync chipmate run)")
  ...
  // (ii) Fix verify failures step: join the run: | block and require --auto on chipmate run
  {
    ...
    // Join continuation backslashes then collapse whitespace for the chipmate run line
    const joined = lines
      .map((l) => l.replace(/^\s+/, ""))
      .join("\n")
      .replace(/\\\n/g, " ")
      .replace(/\s+/g, " ")
    assert.match(joined, /chipmate run\b/, `expected chipmate run in Fix verify block:\n${joined}`)
    const chipmateCmd = joined.match(/chipmate run\b[^|]*/)?.[0] ?? ""
    assert.ok(
      /\s--auto\b/.test(chipmateCmd) || /chipmate run\s+--auto\b/.test(chipmateCmd),
      `Fix verify chipmate run must contain --auto; got: ${chipmateCmd}`,
    )

    // The step runs under `set -o pipefail` + the default `bash -e`, so an
    // unguarded chipmate pipeline aborts the block before verify2.log is written
    // once the CLI exits nonzero on a mid-stream error. The rebuild must decide
    // this step's outcome, not the agent's exit code.
    // Window is the end of the chipmate pipeline → the rebuild, so a comment
    // elsewhere in the block cannot satisfy the guard assertion.
    const teeIdx = joined.indexOf("tee -a docs-sync-out/edit-log.txt")
    assert.ok(teeIdx >= 0, `expected the chipmate pipeline to tee edit-log.txt:\n${joined}`)
    const chipmatePipeline = joined.slice(teeIdx, joined.indexOf("bun run", teeIdx))
    assert.match(
      chipmatePipeline,
      /\|\|\s*(echo|true)\b/,
      `Fix verify chipmate pipeline must be guarded (|| echo/true) so bash -e cannot skip the rebuild; got: ${chipmatePipeline}`,
    )
  ...
  // (iii) authoritative: real stub invocations with callLog — every argv has --auto
  {
    ...
    const cwd = setupEditCwd(worthy, triage)
    const callLog = path.join(cwd, "chipmate-calls.log")
    const stderrText = "event stream disconnected DIAG-AUTO"
    const chipmateDir = makeStubChipMateDir({ mode: "stderr-exit0", stderrText, callLog })
    ...
    for (const line of lines) {
      ...
      assert.ok(
        ...
        `every chipmate argv must include --auto; got ${JSON.stringify(argv)}`,
      )
    }
  }
}
...
function case2c_stderrLogAlways() {
  console.log("case 2c: unconditional chipmate-stderr-*.log")
  ...
  // Failure path: stub exits 0 without summary (same mode as case 2)
  {
    ...
    const logName = chipmateStderrLogName("edit batch 0 attempt 1")
    const logPath = path.join(cwd, "docs-sync-out", logName)
    assert.equal(logName, "chipmate-stderr-edit-batch-0-attempt-1.log")
  ...
  // Success path: stub writes summary (today's path that discarded stderr)
  {
    ...
    const chipmateDir = makeStubChipMateDir({ mode: "write-edit-summary", stderrText })
    const result = runNodeScript(EDIT_SCRIPT, {
      cwd,
      chipmateDir,
      env: {
...
// ---------------------------------------------------------------------------
// Case 2d — redact secret env values from captured chipmate stderr (artifact-safe)
// ---------------------------------------------------------------------------
function case2d_redactEnvSecrets() {
  console.log("case 2d: redact env secrets from chipmate stderr capture")
...
// ---------------------------------------------------------------------------
// Case 2f — redact secret values from captured chipmate stdout (triage-raw artifact)
// ---------------------------------------------------------------------------
function case2f_redactStdout() {
  console.log("case 2f: redact env secrets from chipmate stdout (triage-raw)")

  const digest = [samplePr(501), samplePr(502)]
  const cwd = setupTriageCwd(digest)
  const secret = "selftest-stdout-secret-99999"
  const chipmateDir = makeStubChipMateDir({ mode: "triage-embed-env-secret" })
...
function case2g_redactStream() {
  ...
  const result = spawnSync(process.execPath, [filterPath], {
    env: { ...process.env, CHIPMATE_API_KEY: secret },
    input,
    encoding: "utf8",
    timeout: 10_000,
  })
...
function case2h_pendingCauseIsReadable() {
  ...
  const cwd = setupEditCwd(worthy, triage)
  // Verbatim shape of a real chipmate TUI stderr line (see PR #12521's pending table).
  const ESC = "\u001b"
  const stderrText = `${ESC}[0m→ ${ESC}[0mRead packages/chipmate-docs/AGENTS.md${ESC}[2K${ESC}[1G done`
  const chipmateDir = makeStubChipMateDir({ mode: "stderr-exit0", stderrText })
  ...
  for (const e of summary) {
    ...
    assert.match(e.reason, /Read packages\/chipmate-docs\/AGENTS\.md/)
  }

  // The raw artifact log keeps the escapes — it is the debugging record.
  const rawLog = fs.readFileSync(path.join(cwd, "docs-sync-out", "chipmate-stderr-edit-batch-0-attempt-1.log"), "utf8")
...
function case3_watermark() {
  ...
  // all covered → processed-through === now
  {
    ...
    const summary = [
      { pr: 10, url: prA.url, action: "updated packages/chipmate-docs/pages/x.md", reason: "" },
  ...
  // triage pending:true backfill rows land in uncovered (consumption)
  {
    const triage = [
      {
        pr: 99,
        url: "https://github.com/ChipMate-Org/cloud/pull/99",
        docs_worthy: false,
        pending: true,
  ...
  {
    const uncovered = [{ url: "https://github.com/ChipMate-Org/cloud/pull/50", reason: "missing" }]
...
function case4_routing() {
  ...
  const summary = [
    { pr: 1, url: "https://github.com/ChipMate-Org/cloud/pull/1", action: "updated pages/a.md", reason: "" },
    { pr: 2, url: "https://github.com/ChipMate-Org/cloud/pull/2", action: "skipped", reason: "already documented" },
    { pr: 3, url: "https://github.com/ChipMate-Org/cloud/pull/3", action: "pending", reason: "edit batch 1: exit 0" },
  ]
  const triage = [
    {
      pr: 4,
      url: "https://github.com/ChipMate-Org/cloud/pull/4",
      docs_worthy: false,
      pending: false,
      reason: "chore only",
    },
    {
      pr: 5,
      url: "https://github.com/ChipMate-Org/cloud/pull/5",
      docs_worthy: false,
      pending: true,
  ...
  // clean() prevents marker forgery in agent-generated row strings
  {
    const forgedRows = routeRows({
      summary: [
        {
          pr: 9,
          url: "https://github.com/ChipMate-Org/cloud/pull/9",
          action: "skipped",
  ...
  const legacyRows = [
    "| [ChipMate-Org/cloud#1](https://github.com/ChipMate-Org/cloud/pull/1) | edit pass failed or timed out for this PR |",
    "| [ChipMate-Org/cloud#2](https://github.com/ChipMate-Org/cloud/pull/2) | triage failed to classify this PR |",
    "| [ChipMate-Org/cloud#3](https://github.com/ChipMate-Org/cloud/pull/3) | not classified by triage |",
    "| [ChipMate-Org/cloud#4](https://github.com/ChipMate-Org/cloud/pull/4) | already covered by existing docs |",
  ]
...
function case5_recollection() {
  ...
  const uncovered = [{ url: "https://github.com/ChipMate-Org/cloud/pull/11", reason: "pending" }]
...
function case6_budgets() {
  ...
  // --- edit budget ---
  {
    ...
    const cwd = setupEditCwd(worthy, triage)
    const callLog = path.join(cwd, "chipmate-calls.log")
    const chipmateDir = makeStubChipMateDir({ mode: "record", callLog })
    ...
    assert.equal(callCount, 0, `chipmate must not be invoked for deferred edit batches; got ${callCount}`)
  ...
  // --- triage budget ---
  {
    ...
    const calls = fs.existsSync(callLog) ? fs.readFileSync(callLog, "utf8").trim() : ""
    const callCount = calls ? calls.split("\n").filter(Boolean).length : 0
    assert.equal(callCount, 0, `chipmate must not be invoked for deferred triage chunks; got ${callCount}`)
...
function case8_triage() {
  ...
  // 8a Run A: SINCE_OVERRIDE=true + everything pending → warning present
  {
    ...
    const cwd = setupTriageCwd(digest)
    const chipmateDir = makeStubChipMateDir({ mode: "stderr-exit0", stderrText: "stream end before idle" })
    const summaryFile = path.join(cwd, "step-summary.md")
    fs.writeFileSync(summaryFile, "")
  ...
  // 8a Run B: SINCE_OVERRIDE unset → warning absent
  {
    ...
    const cwd = setupTriageCwd(digest)
    const chipmateDir = makeStubChipMateDir({ mode: "stderr-exit0", stderrText: "stream end" })
    const summaryFile = path.join(cwd, "step-summary.md")
    fs.writeFileSync(summaryFile, "")
  ...
  // 8a Run C: SINCE_OVERRIDE=true with MIXED stub (worthy > 0) → warning ABSENT
  {
    ...
    const cwd = setupTriageCwd(digest)
    const chipmateDir = makeStubChipMateDir({ mode: "mixed-triage" })
    const summaryFile = path.join(cwd, "step-summary.md")
    fs.writeFileSync(summaryFile, "")
  ...
  // 8b — partial classification → missing URLs pending:true + computeUncovered
  {
    ...
    const cwd = setupTriageCwd(digest)
    const chipmateDir = makeStubChipMateDir({ mode: "partial-triage" })
    const summaryFile = path.join(cwd, "step-summary.md")
    fs.writeFileSync(summaryFile, "")
...
function case9_reverts() {
  ...
  // --- parseRevertTargets ---
  const defaultRepo = "ChipMate-Org/chipmate"
...
`
  {
    ...
    assert.deepEqual(
      ...
      [
        {
          repo: "ChipMate-Org/chipmate",
          number: 12249,
          url: "https://github.com/ChipMate-Org/chipmate/pull/12249",
        },
        {
          repo: "ChipMate-Org/chipmate",
          number: 12481,
          url: "https://github.com/ChipMate-Org/chipmate/pull/12481",
        },
      ],
    )
  }
  ...
  {
    const targets = parseRevertTargets("Reverts ChipMate-Org/cloud#42.", defaultRepo)
    assert.equal(targets.length, 1)
    assert.equal(targets[0].repo, "ChipMate-Org/cloud")
    assert.equal(targets[0].number, 42)
    assert.equal(targets[0].url, "https://github.com/ChipMate-Org/cloud/pull/42")
  }
  ...
  const fUrl = "https://github.com/ChipMate-Org/chipmate/pull/100"
  const r1Url = "https://github.com/ChipMate-Org/chipmate/pull/200"
  const r2Url = "https://github.com/ChipMate-Org/chipmate/pull/300"
  const f2Url = "https://github.com/ChipMate-Org/chipmate/pull/101"
    ...
    const annotations = computeRevertAnnotations([
      {
        url: r1Url,
        merged_at: mergedAt,
        targets: [{ repo: "ChipMate-Org/chipmate", number: 100, url: fUrl }],
      },
    ])
    ...
    const annotations = computeRevertAnnotations([
      {
        ...
        merged_at: mergedAt,
        targets: [
          { repo: "ChipMate-Org/chipmate", number: 100, url: fUrl },
          { repo: "ChipMate-Org/chipmate", number: 101, url: f2Url },
        ],
      },
    ])
...

  {
    ...
    const annotations = computeRevertAnnotations([
      ...
      },
      {
        url: r2Url,
        merged_at: mergedAt2,
        targets: [{ repo: "ChipMate-Org/chipmate", number: 200, url: r1Url }],
      },
    ])
  ...
  // --- applyRevertAnnotations ---
  {
    const digest = [
      ...
      { url: "https://github.com/ChipMate-Org/chipmate/pull/999", title: "untouched" },
    ]
...

  {
    ...
    const canonical = "https://github.com/ChipMate-Org/chipmate/pull/12249"
    const lowerTarget = "https://github.com/chipmate-org/chipmate/pull/12249"
    const reverter = "https://github.com/ChipMate-Org/chipmate/pull/12497"
    const digest = [{ url: canonical, title: "feat stream" }]
    const applied = applyRevertAnnotations(digest, [
      {
        url: reverter,
        merged_at: mergedAt,
        targets: [{ repo: "chipmate-org/chipmate", number: 12249, url: lowerTarget }],
      },
    ])
  ...
  // --- unannotatedRevertSignals ---
  {
    ...
    const sUrl = "https://github.com/ChipMate-Org/chipmate/pull/500"
    const gUrl = "https://github.com/ChipMate-Org/chipmate/pull/102"
    const signals = [
      {
        url: sUrl,
        merged_at: mergedAt,
        targets: [
          { repo: "ChipMate-Org/chipmate", number: 100, url: fUrl },
          { repo: "ChipMate-Org/chipmate", number: 102, url: gUrl },
        ],
      },
    ]
...

  {
    ...
    const r3Url = "https://github.com/ChipMate-Org/chipmate/pull/301"
    const signals = [
      {
        url: r1Url,
        merged_at: mergedAt,
      ...
      },
      {
        ...
        targets: [{ repo: "ChipMate-Org/chipmate", number: 201, url: r2Url }],
      },
    ]
    const result = unannotatedRevertSignals(signals, [])
    assert.deepEqual(result, {
...

  {
    ...
    const mUrl = "https://github.com/ChipMate-Org/chipmate/pull/800"
    const aUrl = "https://github.com/ChipMate-Org/chipmate/pull/801"
    const bUrl = "https://github.com/ChipMate-Org/chipmate/pull/802"
    const signals = [
      {
        url: r1Url,
        merged_at: mergedAt,
      ...
      },
      {
        ...
        merged_at: mergedAt2,
        targets: [
          { repo: "ChipMate-Org/chipmate", number: 200, url: r1Url },
          { repo: "ChipMate-Org/chipmate", number: 801, url: aUrl },
          { repo: "ChipMate-Org/chipmate", number: 802, url: bUrl },
        ],
      },
    ]
...

  {
    ...
    const emptyUrl = "https://github.com/ChipMate-Org/chipmate/pull/400"
...

  {
    ...
    const signalUrl = "https://github.com/ChipMate-Org/chipmate/pull/600"
    const targetMixed = "https://github.com/ChipMate-Org/chipmate/pull/700"
    const targetLower = "https://github.com/chipmate-org/chipmate/pull/700"
    const signals = [
      {
        url: signalUrl,
        merged_at: mergedAt,
        targets: [{ repo: "ChipMate-Org/chipmate", number: 700, url: targetMixed }],
      },
    ]
...

  {
    ...
    const s12497 = "https://github.com/ChipMate-Org/chipmate/pull/12497"
    const t12249 = "https://github.com/ChipMate-Org/chipmate/pull/12249"
    const t12481 = "https://github.com/ChipMate-Org/chipmate/pull/12481"
    const signals = [
      {
        url: s12497,
        merged_at: mergedAt,
        targets: [
          { repo: "ChipMate-Org/chipmate", number: 12249, url: t12249 },
          { repo: "ChipMate-Org/chipmate", number: 12481, url: t12481 },
        ],
      },
    ]
    const result = unannotatedRevertSignals(signals, [
  ...
  // --- prompt-text guards ---
  {
    ...
    assert.ok(
      triagePrompt.includes("reverted by https://github.com/ChipMate-Org/chipmate/pull/12497"),
...
function case10_learnings() {
  ...
  const chipmateconnectBotEmail = "240665456+chipmateconnect[bot]@users.noreply.github.com"
  ...
  // 10a — three commit classes
  {
    ...
    // 1. chipmateconnect[bot] commit that touches packages/chipmate-docs/pages/x.md
    gitIn(dir, ["config", "user.email", chipmateconnectBotEmail])
    fs.mkdirSync(path.join(dir, "packages", "chipmate-docs", "pages"), { recursive: true })
    fs.writeFileSync(path.join(dir, "packages", "chipmate-docs", "pages", "x.md"), "# x\n")
    gitIn(dir, ["add", "packages/chipmate-docs/pages/x.md"])
    gitIn(dir, ["commit", "-m", "docs: add x page"])
    const chipmateconnectSha = gitIn(dir, ["rev-parse", "HEAD"])
    // 2. github-actions[bot] commit
    gitIn(dir, ["config", "user.email", githubBotEmail])
    fs.writeFileSync(path.join(dir, "packages", "chipmate-docs", "pages", "y.md"), "# y\n")
    gitIn(dir, ["add", "packages/chipmate-docs/pages/y.md"])
    ...
    const callLog = path.join(dir, "chipmate-calls.log")
    const chipmateDir = makeStubChipMateDir({ mode: "extraction-delta", callLog })
    ...
    // Assert: input file written, exactly one correction — chipmateconnect only.
    ...
    assert.equal(input.corrections.length, 1, "exactly one correction (chipmateconnect commit)")
    assert.equal(
      input.corrections[0].source,
      `commit:${chipmateconnectSha.slice(0, 7)}`,
      "correction must be chipmateconnect commit only",
    )
  }
  ...
  // timestamps use different timezone offsets (Z vs +05:00).
  {
    ...
    fs.writeFileSync(path.join(dir, "packages", "chipmate-docs", "pages", "x.md"), "# x\n")
    gitIn(dir, ["add", "packages/chipmate-docs/pages/x.md"])
    gitIn(dir, ["commit", "-m", "first edit x", "--author", `chipmateconnect[bot] <${chipmateconnectBotEmail}>`], {
    ...
    fs.writeFileSync(path.join(dir, "packages", "chipmate-docs", "pages", "x.md"), "# x\n## edit\n")
    gitIn(dir, ["add", "packages/chipmate-docs/pages/x.md"])
    gitIn(dir, ["commit", "-m", "second edit x", "--author", `chipmateconnect[bot] <${chipmateconnectBotEmail}>`], {
    ...
    const learningsPath = path.join(dir, "packages", "chipmate-docs", "LEARNINGS.md")
    fs.mkdirSync(path.dirname(learningsPath), { recursive: true })
    fs.writeFileSync(learningsPath, renderLearnings(existing))
    gitIn(dir, ["add", "packages/chipmate-docs/LEARNINGS.md"])
    gitIn(dir, ["commit", "-m", "seed learnings", "--author", `chipmateconnect[bot] <${chipmateconnectBotEmail}>`])
      ...
      comments: [
        {
          ...
          path: "packages/chipmate-docs/pages/x.md",
    ...
    const chipmateDir = makeStubChipMateDir({ mode: "extraction-delta", callLog: path.join(cwd, "chipmate-calls.log") })
    ...
    assert.equal(commitA.comment.path, "packages/chipmate-docs/pages/x.md")
  ...
  // 10b — watermark suppression (no model call when marker covers all)
  {
    ...
    fs.writeFileSync(path.join(dir, "packages", "chipmate-docs", "pages", "x.md"), "# x\n")
    gitIn(dir, ["add", "packages/chipmate-docs/pages/x.md"])
    gitIn(dir, ["commit", "-m", "docs update", "--author", `chipmateconnect[bot] <${chipmateconnectBotEmail}>`])
    ...
    const calls = fs.existsSync(callLog) ? fs.readFileSync(callLog, "utf8").trim() : ""
    assert.equal(calls, "", `chipmate must not be invoked when marker covers all; got ${calls}`)
    ...
    const lp = path.join(cwd, "packages", "chipmate-docs", "LEARNINGS.md")
  ...
  // 10c — rerun idempotency
  {
    ...
    // First run
    {
      ...
      firstLEARNINGS = fs.readFileSync(path.join(dir, "packages", "chipmate-docs", "LEARNINGS.md"), "utf8")
    }
    ...
    // Second run with marker covering first run's result
    {
      ...
      const callLog2 = path.join(cwd, "chipmate-calls-run2.log")
      const chipmateDir2 = makeStubChipMateDir({ mode: "extraction-delta", callLog: callLog2 })
      writeExtractionDelta(cwd, { add, remove: [] })
      const result = runNodeScript(LEARN_SCRIPT, {
        cwd,
        chipmateDir: chipmateDir2,
        env: {
          TRIAGE_MODEL: "test/model",
          DOCS_SYNC_FIXTURE: fixturePath,
          LEARNINGS_BUDGET_MINUTES: "1",
      ...
      assert.equal(calls2, "", "second run must not invoke chipmate")
      ...
      const afterApply = fs.readFileSync(path.join(dir, "packages", "chipmate-docs", "LEARNINGS.md"), "utf8")
  ...
  // 10e — prompt injection (triage/edit argv carry tagged rules)
  {
    ...
    // its cwd so readLearningsBlock picks it up.
    {
      ...
      const triageChipMateDir = makeStubChipMateDir({ mode: "record", callLog: triageCallLog })
      runNodeScript(TRIAGE_SCRIPT, {
        cwd: triageCwd,
        chipmateDir: triageChipMateDir,
    ...
    // Run edit.mjs against a recording stub.
    {
      const triageEntry = {
        pr: 1,
        url: "https://github.com/ChipMate-Org/cloud/pull/1",
        docs_worthy: true,
        reason: "needs docs",
        target_sections: [],
        priority: "medium",
      ...
      const editChipMateDir = makeStubChipMateDir({ mode: "record", callLog: editCallLog })
      runNodeScript(EDIT_SCRIPT, {
        cwd: editCwd,
        chipmateDir: editChipMateDir,
  ...
  // 10g — general-rule check (validateDelta rejections)
  {
    ...
    const entryWithPage = {
      id: "bad-page",
      rule: "Edit packages/chipmate-docs/pages/x.md",
      scope: "both",
      source: "commit:aaaaaaa",
      date: "2026-08-03",
    }
  ...
  // 10h — comment trust (isTrustedComment)
  {
    ...
    assert.equal(isTrustedComment({ author_association: "CONTRIBUTOR", user: { login: "chipmate-code-bot[bot]" } }), false)
  ...
  // 10i — draft gate (nonContentFiles)
  {
    ...
    const files = ["packages/chipmate-docs/LEARNINGS.md", "packages/chipmate-docs/pages/a.md"]
    ...
    const withConfig = ["packages/chipmate-docs/next.config.js"]
    const flagged = nonContentFiles(withConfig)
    assert.equal(flagged.length, 1, "next.config.js must still trigger the gate")
    assert.equal(flagged[0], "packages/chipmate-docs/next.config.js")
  }
  ...
  // 10j — no --auto on extraction call
  {
    ...
    // Find the extraction-mode runChipMate args array
    const argsStart = src.indexOf("runChipMate({")
    assert.ok(argsStart >= 0, "runChipMate call must exist in learn.mjs")
    const argsBlock = src.slice(argsStart, src.indexOf("})", argsStart) + 2)
    assert.ok(!argsBlock.includes("--auto"), "extraction runChipMate must not include --auto")
    assert.ok(argsBlock.includes("-f"), "extraction runChipMate must include -f")
  }
...
<!-- docs-sync:learnings:end -->`
  ...
  // git show origin/<branch>:packages/chipmate-docs/LEARNINGS.md fails, and
  // git show origin/main:packages/chipmate-docs/LEARNINGS.md returns the main's entries.
  {
    ...
    const lp = path.join(dir, "packages", "chipmate-docs", "LEARNINGS.md")
    ...
    try {
      gitIn(dir, ["show", "origin/docs/auto-sync:packages/chipmate-docs/LEARNINGS.md"])
    } catch {
      branchFailed = true
    }
    ...
    const mainContent = gitIn(dir, ["show", "origin/main:packages/chipmate-docs/LEARNINGS.md"])
  ...
  // 10m — empty delta advances marker with no file change
  {
    ...
    const existing = []
    fs.writeFileSync(path.join(cwd, "docs-sync-out", "learnings.json"), JSON.stringify(existing, null, 2))
    const learningsPath = path.join(cwd, "packages", "chipmate-docs", "LEARNINGS.md")
    fs.mkdirSync(path.dirname(learningsPath), { recursive: true })
    fs.writeFileSync(learningsPath, renderLearnings(existing))
