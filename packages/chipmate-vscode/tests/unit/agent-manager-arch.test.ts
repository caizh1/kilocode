...
const CHIPMATE_PROVIDER_FILE = path.join(ROOT, "src/ChipMateProvider.ts")
const CSS_FILES = [
...
describe("Agent Manager CSS Prefix", () => {
  it("all class selectors should use am- prefix", () => {
    ...
    // - `chipmate-diff-theme` is the shared Pierre diff theme utility defined
    //   in webview-ui/src/styles/diff.css and reused across webviews.
    // - `css` is matched from `@import "./diff.css"` file extension, not a
    //   class selector.
    const host = new Set(["vscode-high-contrast", "vscode-high-contrast-light", "chipmate-diff-theme", "css"])
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow chipmate-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, chipmate-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Worktree Actions", () => {
  ...
  it("registers distinct dialog and quick-create worktree shortcuts", () => {
    ...
    const dialog = manifest.contributes.keybindings.find(
      (item) => item.command === "chipmate.v2.agentManager.newWorktree",
    )
    const quick = manifest.contributes.keybindings.find(
      (item) => item.command === "chipmate.v2.agentManager.quickWorktree",
    )
  ...
  it("forwards the quick-worktree command to immediate creation", () => {
    ...
    const start = source.indexOf('vscode.commands.registerCommand("chipmate.v2.agentManager.quickWorktree"')
    const end = source.indexOf('vscode.commands.registerCommand("chipmate.v2.agentManager.openWorktree"', start)
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the ChipMate server never sends a
...
// ---------------------------------------------------------------------------
// ChipMateProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("ChipMateProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(CHIPMATE_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/chipmate-provider-utils.ts"), "utf-8")
   ...
   * chipmate-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in chipmate-provider-utils").toBeGreaterThan(-1)
  ...
  it("connected state handler flushes deferred session refresh", () => {
    ...
    const snippet = provider.slice(connectedIdx, connectedIdx + 1600)
...
//
// Regression: PR #7473 moved ChipMateNotifications into MessageList. It calls
...
const AGENT_MANAGER_APP_FILE = path.join(ROOT, "webview-ui/agent-manager/AgentManagerProviders.tsx")
