...
1. **`bun run watch`** is already running for the chipmate extension.
2. In the chipmate VSCode window, press **F5** → opens the **Extension Development Host**.
3. In the Dev Host: `File → Open Folder…` → `packages/chipmate-vscode/docs/nes-examples/` inside this repo.
4. Open Settings (`Cmd+,`), confirm:
   - `chipmate-code.new.autocomplete.enableAutoTrigger` → ✓ (default true)
   - `chipmate-code.new.autocomplete.model` → **Mercury Next Edit (Inception)** ← *NOT* "Mercury Edit 2", which is the classic FIM option
   - `chipmate-code.new.autocomplete.nextEdit.apiKey` → your `sk_...` key
