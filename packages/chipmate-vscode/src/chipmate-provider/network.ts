 ...
 * user to press Enter; `chipmate run` auto-retries with backoff. This module implements auto-reply
 ...
 */
export function handleNetworkEvent(type: string, props: Props, client: ChipMateClient | null, dir: GetDir) {
  ...
  if (type === "session.network.restored" && props.requestID) {
    ...
    if (!entry) return
    console.log("[ChipMate New] network: auto-replying to restore", props.requestID)
