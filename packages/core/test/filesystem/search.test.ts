  ...
  it.live("globs files as an array", () =>
    withTmp((cwd) =>
      Effect.gen(function* () {
        ...
        expect(result.items.map((item) => item.path)).toEqual([RelativePath.make("src/match.ts")]) // chipmate_change
        expect(result.truncated).toBe(false) // chipmate_change
      }),
    ),
  )
  ...
  it.live("greps files with include filtering", () =>
    withTmp((cwd) =>
      Effect.gen(function* () {
        ...
        expect(result.items).toHaveLength(1) // chipmate_change
        expect(result.items[0]?.entry.path).toBe(RelativePath.make("src/match.ts")) // chipmate_change
        expect(result.items[0]?.submatches[0]?.text).toBe("needle") // chipmate_change
      }),
    ),
  )
})
