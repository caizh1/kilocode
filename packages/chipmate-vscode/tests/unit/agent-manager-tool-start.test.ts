...
function result(path: string): CreateWorktreeResult {
  return { path, branch: "chipmate/test", parentBranch: "main", startPointSource: "fallback" } as CreateWorktreeResult
}
...
function deps(overrides: Partial<ToolDeps> = {}): ToolDeps {
  ...
  return {
    ...
    createWorktree: mock(async () => ({ worktree: { id: "wt-1" }, result: result("/repo/.chipmate/worktrees/wt-1") })),
    ...
    sessionMetadata: mock(async () => ({ "chipmate.sandbox": { enabled: true, version: 0 } })),
...
describe("agent manager tool start", () => {
  ...
  it("starts worktree sessions through existing hooks", async () => {
    ...
    expect(c.createSessionInWorktree).toHaveBeenCalledWith("/repo/.chipmate/worktrees/wt-1", "chipmate/test", "wt-1", {
      sandboxInheritanceToken: "si-token",
    })
    expect(c.registerWorktreeSession).toHaveBeenCalledWith("s-wt", "/repo/.chipmate/worktrees/wt-1")
    expect(c.notifyReady).toHaveBeenCalled()
    expect(client.session.promptAsync).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionID: "s-wt",
        directory: "/repo/.chipmate/worktrees/wt-1",
        model: { providerID: "test", modelID: "reasoning/model" },
        variant: "low",
      }),
      { throwOnError: true },
  ...
  it("deduplicates concurrent delivery of the same request", async () => {
    ...
    const c = deps({
      ...
      createWorktree: mock(async () => {
        ...
        return { worktree: { id: "wt-1" }, result: result("/repo/.chipmate/worktrees/wt-1") }
      }),
    })
    const req: ToolRequest = {
