...
import { ChipMateSessionTuiSync } from "../../src/chipmate/session/tui-sync"
...
function syncVariant(input: { current: string | undefined; message: Message; parts?: readonly { type: string }[] }) {
  if (!ChipMateSessionTuiSync.model({ role: input.message.role, parts: input.parts })) return input.current
  return input.message.model?.variant ?? "default"
}

describe("ChipMateSessionTuiSync.model", () => {
  test("syncs normal user messages", () => {
    expect(ChipMateSessionTuiSync.model({ role: "user", parts: [{ type: "text" }] })).toBe(true)
  })

  test("skips compaction marker user messages", () => {
    expect(ChipMateSessionTuiSync.model({ role: "user", parts: [{ type: "compaction" }] })).toBe(false)
  })

  test("skips messages before parts load", () => {
    expect(ChipMateSessionTuiSync.model({ role: "user" })).toBe(false)
  })
  ...
  test("skips messages checked with stored parts", () => {
    ...
    expect(ChipMateSessionTuiSync.model({ role: msg.role, parts })).toBe(false)
  })

  test("skips non-user messages", () => {
    expect(ChipMateSessionTuiSync.model({ role: "assistant", parts: [{ type: "text" }] })).toBe(false)
  })
