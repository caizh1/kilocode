...
const files = Number(process.env.CHIPMATE_INDEXING_STRESS_FILES ?? 1260)
const target = Number(process.env.CHIPMATE_INDEXING_STRESS_LINES ?? 3_100_000)
const symbols = Number(process.env.CHIPMATE_INDEXING_STRESS_SYMBOLS_PER_FILE ?? 32)
const stress = process.env.CHIPMATE_INDEXING_STRESS === "1" ? test : test.skip
