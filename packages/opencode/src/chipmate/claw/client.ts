...
/**
 * ChipMateClaw ChipMate Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates ChipMate Chat events for
...
import { ChipMateChatClient } from "./chipmate-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new ChipMateChatClient({
    baseUrl: input.envelope.chipmateChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/chipmateclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/chipmateclaw/${input.sandboxId}/${id}`
