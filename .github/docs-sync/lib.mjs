    ...
    try {
      res = await fetch(`${API}${path}`, {
        ...
        headers: {
          ...
          "user-agent": "chipmate-docs-sync-bot",
        },
...
/**
 * Backoff schedule between chipmate-run attempts. Production waits 60s then 300s
...
/**
 * Blocking sleep used between chipmate-run retries. Prefer this over async sleep
...
// CSI sequences (colour, cursor moves, erases). chipmate renders its TUI to stderr,
...
// docs-sync-out/chipmate-stderr-*.log stays raw — that is the debugging record.
 ...
 * env holds long-lived secrets (CHIPMATE_API_KEY), so exact values of secret-looking env vars
...
export function redactEnvSecrets(text) {
  let out = String(text ?? "")
  // Also match CREDENTIAL/PASSWORD/ORG_ID/_PAT (e.g. CHIPMATE_ORG_ID, GH_PAT) beyond KEY|TOKEN|SECRET.
...
/**
 * Run `chipmate` via spawnSync so stderr is always recoverable — including when
 ...
 * docs-sync-out/chipmate-stderr-<sanitized-label>.log (unconditional — success and
 * failure). The console return value still uses the short tailText.
 */
export function runChipMate({ args, timeoutMs, streamStdout = false, label = "chipmate" }) {
  const result = spawnSync("chipmate", args, {
    encoding: "utf8",
  ...
  try {
    ...
    fs.writeFileSync(`docs-sync-out/chipmate-stderr-${safe}.log`, body)
  } catch (err) {
    console.warn(`${label}: failed to write chipmate-stderr log: ${err.message}`)
  }
