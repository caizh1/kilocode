export const INDEXING_PLUGIN_NAMES = ["chipmate-indexing", "@chipmate/chipmate-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.chipmate/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@chipmate" && parts[i + 1] === "chipmate-indexing")
  if (scoped >= 0) return "@chipmate/chipmate-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "chipmate-indexing")
  if (workspace >= 0) return "@chipmate/chipmate-indexing"
