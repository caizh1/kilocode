...
import { ChipMateMarkdown } from "@/chipmate/config/markdown"
import { tmpdir } from "../../fixture/fixture"

test("confines project markdown substitutions while preserving trusted substitutions", async () => {
  const name = "CHIPMATE_MARKDOWN_SUBSTITUTE_TEST_SECRET"
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        const item = path.join(project, ".chipmate", "agents", "unsafe.md")
    ...
    const file = await ChipMateMarkdown.substitute(tmp.extra.file, tmp.extra.item, {
      trusted: false,
    ...
    const env = await ChipMateMarkdown.substitute(tmp.extra.env, tmp.extra.item, {
      trusted: false,
      fileScope: { root: tmp.extra.project, source: tmp.extra.item },
    }).then(
      () => false,
    ...
    expect(
      await ChipMateMarkdown.substitute("{file:../../allowed.txt}", tmp.extra.item, {
        trusted: false,
        fileScope: { root: tmp.extra.project, source: tmp.extra.item },
      }),
    ).toBe("project content")

    const trusted = await ChipMateMarkdown.substitute(tmp.extra.text, tmp.extra.global, { trusted: true })
