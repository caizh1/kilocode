...
import { isInterrupted } from "@/chipmate/effect/cause"
...
// Keep Core V2 tools on the same effective ChipMate config used by stable tools. Core's standalone
// scanner cannot see account/managed config or CHIPMATE_CONFIG_CONTENT, so replace its provisional
// references after both config systems finish booting.
export const sync = Effect.fn("ChipMateReference.sync")(function* (input: {
