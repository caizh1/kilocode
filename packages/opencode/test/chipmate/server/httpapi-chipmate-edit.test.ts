...
const env = {
  CHIPMATE_AUTH_CONTENT: process.env.CHIPMATE_AUTH_CONTENT,
...
function app() {
  const handler = HttpRouter.toWebHandler(
    // chipmate_change - keep the filewatcher-disable flag visible (see httpapi-instance-route-auth.test.ts)
...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(ChipMateGatewayPaths.edit, {
    method: "POST",
...
describe("HttpApi ChipMate next edit", () => {
  test("requires ChipMate Gateway authentication for the ChipMate-backed model", async () => {
    process.env.CHIPMATE_AUTH_CONTENT = "{}"
  ...
  test("proxies ChipMate-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
