...
import { backendSupport, run, type Profile } from "@chipmate/sandbox"
import { CurrentProxyFactory, startProxy, type ProxyFactory } from "@chipmate/sandbox"
...
async function fixture() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-linux-sandbox-"))
...
linux("applies the profile environment without inheriting denied values", async () => {
  ...
  const policy: Profile = {
    ...base,
    environment: { deny: ["CHIPMATE_SANDBOX_DENIED"], set: { CHIPMATE_SANDBOX_SET: "expected" } },
  }
  const script = [
    'if (process.env.CHIPMATE_SANDBOX_SET !== "expected") process.exit(2)',
    "if (process.env.CHIPMATE_SANDBOX_DENIED !== undefined) process.exit(3)",
  ].join("\n")
          ...
          spawner
            .spawn(
              ChildProcess.make(process.execPath, ["-e", script], {
                cwd: root.project,
                env: { CHIPMATE_SANDBOX_DENIED: "ambient" },
                extendEnv: true,
              }),
            )
            .pipe(Effect.flatMap((handle) => handle.exitCode)),
...
linux("rejects a Bubblewrap helper inside a writable root", async () => {
  const root = await fixture()
  const source = process.env.CHIPMATE_BWRAP_PATH ?? "/usr/bin/bwrap"
  ...
  const script = [
    ...
    'import { ChildProcess, ChildProcessSpawner } from "effect/unstable/process"',
    'import { backendSupport, run } from "@chipmate/sandbox"',
    'import { CrossSpawnSpawner } from "@opencode-ai/core/cross-spawn-spawner"',
  ...
  try {
    const result = spawnSync(process.execPath, ["-e", script], {
      cwd: import.meta.dir,
      env: { ...process.env, CHIPMATE_BWRAP_PATH: link },
      encoding: "utf8",
    })
...
linux("reports network namespace support separately and fails deny mode closed", async () => {
  ...
  try {
    const result = spawnSync(process.execPath, ["-e", script], {
      cwd: import.meta.dir,
      env: { ...process.env, CHIPMATE_BWRAP_PATH: helper },
      encoding: "utf8",
    })
    expect(result.status, result.stderr).toBe(0)
  } finally {
...
linux("fails closed when Bubblewrap is unavailable", () => {
  ...
  const result = spawnSync(process.execPath, ["-e", script], {
    cwd: import.meta.dir,
    env: { ...process.env, CHIPMATE_BWRAP_PATH: "/missing/chipmate-bwrap" },
    encoding: "utf8",
  })
  expect(result.status, result.stderr).toBe(0)
})
