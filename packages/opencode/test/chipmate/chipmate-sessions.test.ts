...
import { clearInFlightCache } from "../../src/chipmate-sessions/inflight-cache"
import { ChipMateSessions } from "../../src/chipmate-sessions/chipmate-sessions"
import { provide } from "../../src/chipmate/instance"
import { RemoteWS } from "../../src/chipmate-sessions/remote-ws"
...
import { RemoteProtocol } from "../../src/chipmate-sessions/remote-protocol"
...
function reset(...tokens: string[]) {
  clearInFlightCache("chipmate-sessions:token")
  clearInFlightCache("chipmate-sessions:client")
  for (const token of tokens) clearInFlightCache(`chipmate-sessions:token-valid:${token}`)
}
...
it.instance("bootstraps session ingest from CHIPMATE_API_KEY without stored auth", () => {
  const original = process.env.CHIPMATE_API_KEY
  const calls: string[] = []
  ...
  process.env.CHIPMATE_API_KEY = "env-token"
  reset("env-token")

  return Effect.promise(() => ChipMateSessions.bootstrap("session-env")).pipe(
    Effect.andThen(() => Effect.sync(() => expect(calls).toEqual(["Bearer env-token", "Bearer env-token"]))),
    Effect.ensuring(
      Effect.sync(() => {
        if (original === undefined) delete process.env.CHIPMATE_API_KEY
        else process.env.CHIPMATE_API_KEY = original
        reset("env-token")
        request.mockRestore()
      }),
    ),
...
it.instance("prefers stored auth over CHIPMATE_API_KEY for session ingest", () => {
  const original = process.env.CHIPMATE_API_KEY
  const calls: string[] = []
  const fetch: typeof globalThis.fetch = Object.assign(
    async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  return Effect.gen(function* () {
    ...
    yield* auth.set("chipmate", { type: "api", key: "stored-token" })
    yield* Effect.promise(() => ChipMateSessions.bootstrap("session-auth"))
    expect(calls).toEqual(["Bearer stored-token", "Bearer stored-token"])
  }).pipe(
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("chipmate").pipe(Effect.orDie)
        if (original === undefined) delete process.env.CHIPMATE_API_KEY
        else process.env.CHIPMATE_API_KEY = original
        reset("env-token", "stored-token")
        request.mockRestore()
...
it.instance("does not duplicate created-session subscribers when init is repeated", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* ChipMateSessions.Service
    yield* auth.set("chipmate", { type: "api", key: "test-token" })
    yield* sessions.init()
    yield* sessions.init()
...
// chipmate_change start - K1 W1 / DEF-1: instance advertisement + per-session platform.
//
// `enableRemote` is idempotent/coalescing and is called from `/remote`, the
// explicit `chipmate remote` command, and bootstrap auto-enable (`CHIPMATE_REMOTE=1` /
...
describe("ChipMateSessions.setInstanceAdvertisement (K1 W1 / DEF-1)", () => {
  ...
  beforeEach(() => {
    heartbeatCalls = 0
    outOfBand = undefined
    process.env["CHIPMATE_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["CHIPMATE_SESSION_INGEST_URL"]
    process.env["CHIPMATE_API_KEY"] = "tok"
    reset("tok")
    ChipMateSessions.resetInstanceAdvertisementForTests()
    ...
    clearInFlightCache("chipmate-sessions:token")
    clearInFlightCache("chipmate-sessions:token-valid:tok")

    // chipmate_change - only mock the specific endpoint authValid() calls
    // (${CHIPMATE_API_BASE}/api/user). A blanket mock that returned 200 for
  ...
  afterEach(async () => {
    ...
    pub.mockRestore()
    mock.restore()
    delete process.env["CHIPMATE_DISABLE_SESSION_INGEST"]
    delete process.env["CHIPMATE_SESSION_INGEST_URL"]
    delete process.env["CHIPMATE_PLATFORM"]
    delete process.env["CHIPMATE_API_KEY"]
    reset("tok")
  })

  // Reads the `getSessions` closure that chipmate-sessions.ts passed to
  ...
  test("explicit set after enable replaces the payload (chipmate remote race)", async () => {
    await using tmp = await tmpdir({ git: true })
    await provide({
      directory: tmp.path,
      fn: async () => {
        ...
        ChipMateSessions.setInstanceAdvertisement({
          name: "mbp-igor",
          projectName: "cloud",
          version: "1.2.3",
        })
  ...
  test("setter triggers an out-of-band heartbeat when a connection is already established", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        ChipMateSessions.setInstanceAdvertisement({ name: "h", projectName: "p" })
        await outOfBand
  ...
  test("setter replaces payload and fires one out-of-band heartbeat per call", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        await ChipMateSessions.enableRemote()
        ChipMateSessions.setInstanceAdvertisement({ name: "first", projectName: "p" })
        await outOfBand
        const before = heartbeatCalls
        ChipMateSessions.setInstanceAdvertisement({ name: "second", projectName: "p" })
        await outOfBand
  ...
  test("explicit set before enableRemote is preserved (no re-set on enable)", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        ChipMateSessions.setInstanceAdvertisement({ name: "pre-set", projectName: "proj", version: "9.9.9" })
        await ChipMateSessions.enableRemote()
        const payload = await capturedGetSessions()()
  ...
  test("per-session platform resolution matches meta() order — env var fallback", async () => {
    ...
    //   ChipMateSession.resolvePlatform(id) || process.env["CHIPMATE_PLATFORM"] || "cli"
    // For an id with no override, the env var (when set) wins over the default.
    process.env["CHIPMATE_PLATFORM"] = "vscode"
    await using tmp = await tmpdir({ git: true })
    await provide({
      directory: tmp.path,
      fn: async () => {
        ...
        const expectedPlatform = process.env["CHIPMATE_PLATFORM"] || "cli"
...
// chipmate_change start - K1 W1: real integration between SessionStatus,
...
// real ChipMateSessions seams and proves that a non-idle status is cleared
// deterministically, which is exactly what lets the fence resolve and the
// exit_cli handler ACK.
describe("ChipMateSessions.detachRemoteSession heartbeat fence (K1 W1)", () => {
  let heartbeatCalls = 0
  let outOfBand: Promise<void> | undefined
    ...
    test(`clears ${label} SessionStatus so the detach heartbeat fence resolves`, async () => {
      ...
      await provide({
        ...
        fn: async () => {
          ...
          await ChipMateSessions.attachRemoteSession(id)
          ...
          await ChipMateSessions.detachRemoteSession(id)
...
describe("ChipMateSessions heartbeat attention status (DEF-3)", () => {
  beforeEach(() => {
    process.env["CHIPMATE_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["CHIPMATE_SESSION_INGEST_URL"]
    process.env["CHIPMATE_API_KEY"] = "tok"
  ...
  test("idle/busy/retry unchanged when no attention is pending", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        await ChipMateSessions.attachRemoteSession(idleId)
        await ChipMateSessions.attachRemoteSession(busyId)
        await ChipMateSessions.attachRemoteSession(retryId)
