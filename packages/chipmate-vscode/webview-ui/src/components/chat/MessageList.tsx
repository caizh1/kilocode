...
import { useData } from "@chipmate/chipmate-ui/context/data"
...
export const MessageList: Component<MessageListProps> = (props) => {
  ...
  // back to chipmate-ui's default hideDetails renderer, which never shows a
  ...
  // Extracts only the text chipmate-ui's tool renderers actually put on screen —
  ...
  //
  // read/glob/grep/list are the one confirmed exception: chipmate-ui always
  ...
  // see packages/opencode/src/chipmate/todo-view.ts). Recursively collecting
  ...
  // registers for "task", overriding chipmate-ui's default) exactly: title is
  ...
  function taskText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    // chipmate-ui's default hideDetails task card, which never shows result
  ...
  function questionText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    questions.forEach((q, i) => {
      ...
      // values. Once completed, chipmate-ui's question renderer instead shows
    ...
    <div class="message-list-container">
      ...
      <Show when={isEmpty()}>
        <div class="welcome-header">
          ...
          <ChipMateNotifications sessionID={props.sessionID} />
        </div>
      </Show>
      <div
        ref={setScrollRef}
        ...
        <div ref={autoScroll.contentRef} class={isEmpty() ? "message-list-content-empty" : "message-list-content"}>
          ...
          <Show when={isEmpty() && props.readonly}>
            <div class="message-list-empty">
              <p class="chipmate-about-text">{language.t("session.messages.initializing")}</p>
            </div>
          </Show>
          <Show when={isEmpty() && !props.readonly}>
            {props.emptyState ? (
