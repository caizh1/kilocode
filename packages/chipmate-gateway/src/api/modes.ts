import { z } from "zod"
import { CHIPMATE_API_BASE, MODELS_FETCH_TIMEOUT_MS } from "./constants.js"
...
/**
 * Fetch custom modes for an organization from the ChipMate Cloud API.
 *
...
export async function fetchOrganizationModes(token: string, organizationId: string): Promise<OrganizationMode[]> {
  ...
  try {
    const url = `${CHIPMATE_API_BASE}/api/organizations/${encodeURIComponent(organizationId)}/modes`
    ...
    console.warn("[ChipMate Gateway] Error fetching organization modes:", err)
    return []
  }
}
