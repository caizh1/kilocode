...
description: "Learn about the AI models available through the ChipMate AI Gateway, including model IDs and how to use them."
---

# Models & Providers

The ChipMate AI Gateway provides access to hundreds of AI models through a single unified API. You can switch between models by changing the model ID string -- no code changes required.
...
const result = streamText({
  model: chipmate.chat("anthropic/claude-sonnet-4.6"),
  prompt: "Hello!",
})
```
...
```
GET https://api.chipmate.ai/api/gateway/models
```
...
Auto virtual models select an underlying model using tier-specific routing. Frontier uses the `x-chipmate-mode` request header. Efficient classifies task difficulty in session context and falls back to the API interface for its baseline model, Free uses deterministic affinity across available candidates, and Small uses account balance.

{% callout type="info" title="Underlying models can change" %}
The mappings below reflect the current routing. The underlying models behind each `chipmate-auto/*` tier are updated server-side as better options become available or as providers change pricing and availability — the tier IDs themselves remain stable.
{% /callout %}

### `chipmate-auto/frontier`
...
curl -X POST "https://api.chipmate.ai/api/gateway/chat/completions" \
  ...
  -H "x-chipmate-mode: plan" \
  -H "Content-Type: application/json" \
  -d '{"model": "chipmate-auto/efficient", "messages": [{"role": "user", "content": "Design a database schema"}]}'
```
