...
// chipmate_change start - normalize the runtime SyncEvent wire envelope to the legacy consumer shape
...
export function useEvent() {
  ...
  function subscribe(handler: (event: Event, metadata: EventMetadata) => void) {
    return sdk.event.on("event", (event) => {
      ...
      if (event.directory !== "global" && event.project !== project.project()) return // chipmate_change
