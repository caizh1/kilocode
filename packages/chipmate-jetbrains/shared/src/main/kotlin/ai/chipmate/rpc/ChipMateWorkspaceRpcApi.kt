 ...
 * or worktree). Each call routes to a [ChipMateBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface ChipMateWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): ChipMateWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<ChipMateWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<ChipMateWorkspaceStateDto>
    ...
    /** Fuzzy file/folder search via ChipMate Core. */
