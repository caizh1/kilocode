...
import { ChipMatePtyTermination } from "../../src/chipmate/pty/termination"

function fake(pid = 123) {
  const calls: Array<string | undefined> = []
  const proc: ChipMatePtyTermination.Process = {
    pid,
    onExit: () => ({ dispose() {} }),
...
function runtime(
  ...
  const sleeps: number[] = []
  const value: ChipMatePtyTermination.Runtime = {
    platform,
...
describe("pty process-tree termination", () => {
  test("uses hidden taskkill for Windows process trees", async () => {
    ...
    await ChipMatePtyTermination.terminate(item.proc, input.value)
