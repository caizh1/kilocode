...
class AutoApproveSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val state = ChipMateAppStateDto(ChipMateAppStatusDto.READY, config = ConfigDto())
    ...
    fun `test page is not editable before app is ready`() {
        rpc.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.LOADING)
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.LOADING)
        edt { ui = AutoApproveSettingsUi(uiScope, app, workspaces, picker) }
        flushUntil { text(requireUi()).contains("External Directory") }
