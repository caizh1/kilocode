...
import ai.chipmate.client.actions.ChipMateActionPlaces
...
import ai.chipmate.client.session.controller.SessionControllerListener
...
class ConnectionPanel(
    ...
    companion object {
        internal const val CLI_GROUP_ID = "ChipMate.CliGroup"
    ...
    private val retry = ActionLink(ChipMateBundle.message("session.connection.retry")) {
        showRecoveryPopup()
    }.apply {
        isVisible = false
    ...
    private fun showConnecting() {
        ...
        label.text = ChipMateBundle.message("session.connection.connecting")
        detail = null
        expanded = false
    ...
    private fun showDownloading(percent: Int, version: String?, platform: String?) {
        ...
        label.text = if (version != null && platform != null) {
            ChipMateBundle.message("session.connection.downloading.version", version, platform, pct)
        } else {
            ChipMateBundle.message("session.connection.downloading", pct)
        }
        detail = null
        expanded = false
        toggle.isVisible = false
    ...
    private fun showRecoveryPopup() {
        JBPopupFactory.getInstance()
            .createActionGroupPopup(
                ...
                true,
                ChipMateActionPlaces.connectionRetryPopup(),
            )
    ...
    private fun recoveryGroup(): ActionGroup {
        ...
        group.add(object : DumbAwareAction(ChipMateBundle.message("session.connection.retry")) {
            override fun actionPerformed(e: AnActionEvent) {
                controller.retryConnection()
            }
        })
        group.addSeparator()
        ActionManager.getInstance().getAction("ChipMate.Restart")?.let { group.add(it) }
        ActionManager.getInstance().getAction("ChipMate.Reinstall")?.let { group.add(it) }
        return group
    }
