...
export function isInternalOffline(): boolean {
  return process.env.CHIPMATE_INTERNAL_OFFLINE === "1" || process.env.CHIPMATE_INTERNAL_OFFLINE === "1"
}

export function internalOfflineIndexingOpenAICompatibleBaseUrl(): string | undefined {
  const value = process.env.CHIPMATE_INTERNAL_INDEXING_OPENAI_COMPATIBLE_BASE_URL?.trim()
