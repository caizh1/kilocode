...
import ai.chipmate.client.settings.auth.QrCode
...
class QrCodeTest {
    ...
    fun `generates qr image with requested size`() {
        val img = QrCode.image("https://app.chipmate.ai/device-auth?code=TEST", 64)
    ...
    fun `different inputs produce different images`() {
        val a = QrCode.image("https://auth.chipmate.ai/device?code=AAA", 64)
        val b = QrCode.image("https://auth.chipmate.ai/device?code=ZZZ", 64)
        var differs = false
    ...
    fun `icon wraps image with correct dimensions`() {
        val icon = QrCode.icon("https://auth.chipmate.ai/device", 64)
