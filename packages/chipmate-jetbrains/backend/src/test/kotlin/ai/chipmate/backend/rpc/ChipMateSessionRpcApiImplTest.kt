...
class ChipMateSessionRpcApiImplTest {
    ...
    fun `events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = ChipMateSessionRpcApiImpl(log = log, source = flowOf(ChatEventDto.TurnOpen("ses_test")))
    ...
    fun `events logs cancelled completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = ChipMateSessionRpcApiImpl(log = log, source = flow { kotlinx.coroutines.awaitCancellation() })
    ...
    fun `events logs failed completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = ChipMateSessionRpcApiImpl(log = log, source = flow { throw IllegalStateException("stream failed") })
    ...
    fun `diffSides rebuilds full before by reverse-applying the patch to the working tree`() = runBlocking(Dispatchers.Default) {
        val dir = createTempDirectory("chipmate-diff")
        try {
            ...
            val diff = ChipMateSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto(file, 1, 1, patch, "modified"), null)
    ...
    fun `diffSides returns null when the working tree drifted from the patch`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            assertNull(ChipMateSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto(file, 1, 1, patch, "modified"), null))
        } finally {
            delete(dir)
        }
    }
    ...
    fun `diffSides returns null for added files and missing patches`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            assertNull(ChipMateSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto("new.kt", 1, 0, added, "added"), null))
            assertNull(ChipMateSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto("new.kt", 1, 0, null, "added"), null))
        } finally {
            delete(dir)
        }
    }
    ...
    fun `diffSides prefers authoritative CLI content over local reconstruction`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            val api = ChipMateSessionRpcApiImpl(app(mock))
    ...
    private suspend fun app(mock: MockCliServer): ChipMateBackendAppService {
        val app = ChipMateBackendAppService.create(scope, FakeCliServer(mock), TestLog()).also { apps.add(it) }
        app.connect()
        ...
        assertIs<ChipMateAppState.Ready>(state, "App startup failed")
        return app
    }
