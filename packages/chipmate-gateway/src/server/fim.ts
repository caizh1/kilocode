    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "chipmate"
          ? buildChipMateHeaders(undefined, { chipmateOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "chipmate" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "chipmate" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "chipmate" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with ChipMate Gateway" }, 401)
    }

    if (target.provider === "chipmate" && !token) {
