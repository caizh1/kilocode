...
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "../../../src/chipmate-sessions/rename-adoptions"
...
const ChipMateSessions = (await import("../../../src/chipmate-sessions/chipmate-sessions")).ChipMateSessions

// Session must be provideMerged so yield* Session.Service and the
// ChipMateSessions Updated handler share one store (otherwise get() misses creates).
...
function layer(overrides: Partial<Config.Interface> = {}) {
  return AppNodeBuilder.build(
    LayerNode.group([ChipMateSessions.node, Session.node, SessionProjector.node]),
...
function reset(...tokens: string[]) {
  ...
  clearInFlightCache("chipmate-sessions:org")
...
const ENV_KEYS = [
  "CHIPMATE_API_KEY",
  "CHIPMATE_SESSION_INGEST_URL",
  "CHIPMATE_ORG_ID",
  "CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS",
] as const
...
beforeEach(() => {
  delete process.env.CHIPMATE_ORG_ID
  clearRenameMarks()
})
...
function metaItems(requests: Req[]) {
  ...
  for (const r of requests) {
    ...
    for (const item of data) {
      if (item?.type === "chipmate_meta") items.push(item.data)
    }
  }
  return items
}
...
const drainIngest = Effect.gen(function* () {
  ...
  yield* Effect.promise(() => ChipMateSessions.drainIngestForShutdown())
})

it.instance("meta org precedence: session metadata > CHIPMATE_ORG_ID > auth accountId", () => {
  const requests: Req[] = []
  installFetch(mockFetch(requests))
  patchEnv({
    CHIPMATE_API_KEY: "test-token",
    CHIPMATE_SESSION_INGEST_URL: "https://ingest.chipmatesessions.ai",
    CHIPMATE_ORG_ID: ORG_ENV,
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      yield* Effect.promise(() => ChipMateSessions.bootstrap(withMeta.id))
      requests.length = 0
      ...
      yield* Effect.promise(() => ChipMateSessions.bootstrap(plain.id))
      requests.length = 0
      ...
      yield* Effect.promise(() => ChipMateSessions.bootstrap(authOnly.id))
      requests.length = 0
      emitUpdated(instance.directory, authOnly.id, authOnly.title)
      yield* drainIngest
      expect(metaItems(requests).some((m) => m.orgId === ORG_AUTH)).toBe(true)
    }).pipe(Effect.ensuring(auth.remove("chipmate").pipe(Effect.orDie)))
...
it.instance("meta falls through invalid metadata orgId to env", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => ChipMateSessions.bootstrap(bad.id))
    requests.length = 0
...
it.instance("meta falls back to env when session row has no resolvable org", () => {
  // Precedence only: real Updated → chipmate_meta with a live session that has no
...
it.instance("meta falls back to env when Session.Service.get fails", () => {
  ...
  // global runtime; unknown id → get rejects → .catch(() => null) → CHIPMATE_ORG_ID.
  ...
  patchEnv({ CHIPMATE_ORG_ID: ORG_ENV })
  clearInFlightCache("chipmate-sessions:org")

  return Effect.gen(function* () {
    const result = yield* Effect.promise(() => ChipMateSessions._metaForTests("ses_missing_for_meta_get_fail"))
...
it.instance("title broadcast: auto-title posts generated true; custom posts generated false; adoption skips", () => {
  ...
  patchEnv({
    CHIPMATE_API_KEY: "test-token",
    CHIPMATE_SESSION_INGEST_URL: "https://ingest.chipmatesessions.ai",
    CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS: "5000",
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    const id = created.id
    yield* Effect.promise(() => ChipMateSessions.bootstrap(id))
...
it.instance("reportSessionTitle goes through readiness and tolerates POST failure", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service
    const created = yield* sessions.create({})
    yield* Effect.promise(() => ChipMateSessions.bootstrap(created.id))

    const failed = yield* chipmate.reportSessionTitle(created.id, "X", { generated: false })
    expect(failed).toEqual({ ok: false, reason: "http_500" })
    expect(requests.some((r) => r.path.endsWith("/title"))).toBe(true)

    titleStatus = 200
    const ok = yield* chipmate.reportSessionTitle(created.id, "Y", { generated: true })
    expect(ok).toEqual({ ok: true })
  }).pipe(Effect.provide(layer()))
})

it.instance("reportSessionTitle reports not_connected when unauthenticated", () => {
  patchEnv({ CHIPMATE_API_KEY: undefined })
  delete process.env.CHIPMATE_API_KEY
  reset()

  return Effect.gen(function* () {
    const chipmate = yield* ChipMateSessions.Service
    const result = yield* chipmate.reportSessionTitle("ses_x", "T", { generated: false })
...
it.instance(
  ...
  () => {
    ...
    const customLayer = ChipMateSessions.layer.pipe(
