  ...
  const profiles = {
    darwin: {
      ...
      groups: {
        filesystem: [
          "filesystem/*.test.ts",
          "chipmate/{external-directory-boundary,read-directory}.test.ts",
          "util/filesystem.test.ts",
        ],
        pty: ["server/httpapi-pty.test.ts", "server/httpapi-v2-pty.test.ts"],
        runtime: [
          "cli/serve/*.test.ts",
          "chipmate/background-process.test.ts",
          "chipmate/cli/install-artifact.test.ts",
          "chipmate/cli/tui/thread.test.ts",
          "chipmate/core-watcher.test.ts",
          "chipmate/interactive-terminal.test.ts",
          "tool/shell.test.ts",
          "util/{process,which}.test.ts",
        ],
        sandbox: ["chipmate/sandbox/*.test.ts"],
      },
    },
  } as const
