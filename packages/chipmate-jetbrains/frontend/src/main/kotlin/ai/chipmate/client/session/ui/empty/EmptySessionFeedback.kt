package ai.chipmate.client.session.ui.empty
...
internal class EmptySessionFeedback(
    ...
    companion object {
        ...
        fun content(open: (String) -> Unit): JComponent {
            val logo = JBLabel(IconLoader.getIcon("/icons/chipmate-content.svg", EmptySessionPanel::class.java)).apply {
            ...
            val actions = Stack.vertical(gap = UiStyle.Gap.sm())
                .next(ActionButton(ChipMateBundle.message("feedback.dialog.github"), AllIcons.Vcs.Vendors.Github) {
                    open(GITHUB_ISSUES_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
                .next(ActionButton(ChipMateBundle.message("feedback.dialog.discord"), DISCORD_ICON) {
                    open(DISCORD_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
                .next(ActionButton(ChipMateBundle.message("feedback.dialog.support"), AllIcons.Actions.Help) {
                    open(SUPPORT_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
        ...
        private fun messageHtml() = XmlStringUtil.wrapInHtml(
            "<div style='text-align:center'>${XmlStringUtil.escapeString(ChipMateBundle.message("feedback.dialog.message"))}</div>"
        )

        private fun buttonHtml() = XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(ChipMateBundle.message("feedback.button"))
        )

        private const val CHIPMATE_URL = "https://chipmate.ai"
        private const val GITHUB_ISSUES_URL = "https://github.com/ChipMate-Org/chipmate/issues/new/choose"
        private const val DISCORD_URL = "https://chipmate.ai/discord"
        private const val SUPPORT_URL = "https://chipmate.ai/support"
