...
import { createChipMateClient, type ChipMateClient } from "@chipmate/sdk/v2/client"
...
async function parallel(items: string[], fn: (item: string) => Promise<void>): Promise<void> {
  ...
  for (const [index, error] of failures.slice(1)) {
    console.warn(`[ChipMate New] ConnectionService: Additional prompt drain failed for ${items[index]}:`, error)
  }
  throw failures[0]![1]
}
...
/**
 * Shared connection service that owns the single ServerManager, ChipMateClient (SDK), and SdkSSEAdapter.
 * Multiple ChipMateProvider instances subscribe to it for SSE events and state changes.
 */
export class ChipMateConnectionService {
  readonly sandboxPreference: SandboxPreference
  private readonly serverManager: ServerManager
  private client: ChipMateClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<ChipMateClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed ChipMateProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed ChipMateProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed ChipMateProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed ChipMateProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed ChipMateProvider instances.
   */
  ...
  /**
   * Subscribe to model-selector expand/collapse changes broadcast from any ChipMateProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a model-selector expand/collapse change to all subscribed ChipMateProvider instances.
   */
   ...
   * ChipMateProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted ChipMateProvider.
   *
  ...
  private sendViewed(): void {
    ...
    void this.client.session
      ...
      .catch((err) => console.warn("[ChipMate New] ConnectionService: viewed flush failed:", err))
      .finally(() => {
  ...
  dispose(): void {
    ...
    if (this.client?.session?.viewed) {
      void this.client.session
        ...
        .catch((err) => console.warn("[ChipMate New] ConnectionService: final viewed flush failed:", err))
    }
    this.attached.clear()
    this.visible.clear()
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[ChipMate New] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`chipmate:${password}`).toString("base64")}` },
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`chipmate:${server.password}`).toString("base64")}`
