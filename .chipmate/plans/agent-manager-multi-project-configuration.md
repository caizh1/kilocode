...
ChipMate has four configuration stores:

| Store | Example | Owner |
|---|---|---|
| VS Code preferences | VS Code `settings.json` | This VS Code user/installation |
| ChipMate user config | `~/.config/chipmate/chipmate.json` | User defaults across projects and ChipMate clients |
| ChipMate project config | `<repo>/.chipmate/chipmate.jsonc` | Repository behavior and overrides |
...
Several controls are VS Code preferences and bypass ChipMate config entirely, including autocomplete UI, browser automation, notifications, max auto-approve cost, commit-message output language, and indexing button visibility.

## Confirmed blocking failure

The current protocol does not bind a draft to the config target it was loaded from:

1. `ChipMateProvider.fetchAndSendConfig()` resolves a mutable current directory and sends unqualified `configLoaded` state.
...
5. `ChipMateProvider.handleUpdateConfig()` resolves the current directory again at save time and may write A's draft to B.
...
| Auto Approve | User ChipMate config; max cost remains VS Code preference |
...
Project: backend
Target: /projects/backend/.chipmate/chipmate.jsonc
```
...
1. Add revisioned target descriptors and compare-and-swap writes to the ChipMate config overlay API.
