...
//
// Unit tests for ChipMateSnapshotTrack.wrap — the slow-repo guard that sits
...
import { ChipMateSnapshotTrack } from "../../src/chipmate/snapshot/track"
...
const makeHooks = (
  answer: ChipMateSnapshotTrack.Answer | Promise<ChipMateSnapshotTrack.Answer>,
): { hooks: ChipMateSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  const hooks: ChipMateSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("ChipMateSnapshotTrack.protect", () => {
  it.effect("returns at the availability deadline without waiting for cancellation", () =>
    Effect.gen(function* () {
      const state = ChipMateSnapshotTrack.makeState()
      ...
      const fiber = yield* ChipMateSnapshotTrack.protect({
        inner,
        state,
        fallback,
        operation: "patch",
  ...
  it.effect("bypasses later operations after a deadline opens the directory circuit", () =>
    Effect.gen(function* () {
      ...
      const started = yield* Deferred.make<void>()
      let calls = 0
      const first = yield* ChipMateSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
      ...
      const second = yield* ChipMateSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
          return "unexpected"
        }),
  ...
  test("keeps circuit state isolated by directory", () => {
    const states = ChipMateSnapshotTrack.makeStates()
...
describe("ChipMateSnapshotTrack.wrap", () => {
  ...
  test("concurrent timeout does not override an active continue choice", async () => {
    const state = ChipMateSnapshotTrack.makeState()
    const answer = Promise.withResolvers<ChipMateSnapshotTrack.Answer>()
    const asked = Promise.withResolvers<void>()
    const { hooks, calls } = makeHooks(answer.promise)
    const firstHooks: ChipMateSnapshotTrack.Hooks = {
      ...hooks,
      async ask(input) {
        asked.resolve()
...
describe("ChipMateSnapshotTrack progress indicator", () => {
  test("classifies persisted progress as transient", () => {
    const part = ChipMateSnapshotTrack.progressPart({
      sessionID: SESSION,
      messageID: MESSAGE,
    ...
    expect(ChipMatePartLifecycle.transient(part)).toBe(true)
  })
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      ChipMateSnapshotTrack.formatProgress(ChipMateSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = ChipMateSnapshotTrack.formatProgress(ChipMateSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("ChipMateSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with CHIPMATE_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(ChipMateSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(ChipMateSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(ChipMateSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
