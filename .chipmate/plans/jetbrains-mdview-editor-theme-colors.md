...
- From `packages/chipmate-jetbrains/`, run `./gradlew frontend:test --tests '*MdView*'`.
- From `packages/chipmate-jetbrains/`, run `./gradlew frontend:test --tests '*PlanExitViewTest*'` if the focused test is not covered by the MdView filter.
- From `packages/chipmate-jetbrains/`, run `bun run typecheck`.
