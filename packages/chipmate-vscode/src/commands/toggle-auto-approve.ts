...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission
            ...
            .catch((err) => {
              console.error("[ChipMate New] toggleAutoApprove: failed to drain pending:", err)
            })
        }
      } catch (err) {
        console.error("[ChipMate New] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const approve = async (event: Asked, directory?: string) => {
    ...
    return client.permission
      ...
      .then(
        () => true,
        (err) => {
          console.error("[ChipMate New] toggleAutoApprove: failed to auto-reply:", err)
          return false
        },
      )
  }
  ...
  context.subscriptions.push(vscode.commands.registerCommand("chipmate.v2.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: ChipMateConnectionService): ChipMateClient | undefined {
  try {
