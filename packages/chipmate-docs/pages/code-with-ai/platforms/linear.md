---
title: "Linear"
description: "Using ChipMate in Linear"
---

# ChipMate for Linear

**ChipMate for Linear** connects ChipMate to your Linear workspace, so you can trigger implementations, debug issues, and investigate bugs directly from your project management tool. Mention `@chipmate` on any issue and the bot gets to work.
...
{% image src="/docs/img/connect/linear/linear-fix-issue.png" alt="Asking @chipmate to fix an issue in Linear" width="800" /%}
...
{% image src="/docs/img/connect/linear/linear-multi-repo.png" alt="Asking @chipmate to apply changes across multiple repositories from Linear" width="800" /%}
...
```
@chipmate what could be the cause of this issue?
```

The bot examines the issue context and searches the connected codebase to surface likely causes.

{% image src="/docs/img/connect/linear/linear-understand-issue.png" alt="Asking @chipmate to analyze the cause of a Linear issue" width="800" /%}

---

## How It Works

1. **Mention `@chipmate`** in a comment on a Linear issue
2. **ChipMate reads the issue context** — title, description, labels, and comment thread
3. **A Cloud Agent spins up** to process the request (you'll see a thinking animation in Linear while this happens)
4. **ChipMate responds** with an answer or opens a pull request with the implementation

When ChipMate creates a pull request, it links back to the Linear issue so everything stays connected.
...
- Your GitHub or GitLab integration configured via the Integrations tab at [app.chipmate.ai](https://app.chipmate.ai)
...
3. Authorize ChipMate to access your Linear workspace

Once connected, `@chipmate` is available as a mention in any issue across your Linear workspace.
...
Confirm that the Linear integration is set up in the Integrations tab at [app.chipmate.ai](https://app.chipmate.ai) and that the bot has access to your workspace.
