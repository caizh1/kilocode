...
internal data class AgentEditDraft(
    ...
    val mode: String = ChipMateCliParser.MODE_PRIMARY,
    val defaultMode: String = ChipMateCliParser.MODE_PRIMARY,
...
internal fun agentsDraft(config: ConfigDto?, details: List<AgentDetailDto>): AgentsDraft {
    ...
    val agent = config?.defaultAgent?.takeIf { name ->
        val item = items[name]
        item != null && ChipMateCliParser.defaultAgentCandidate(item.mode, item.hidden) && !item.disable
    }
...
internal fun updateAgent(draft: AgentsDraft, agent: AgentEditDraft): AgentsDraft {
    ...
    val clear = !restricted && (agent.hidden || agent.disable || ChipMateCliParser.isSubagent(agent.mode))
...
internal fun patch(from: AgentsDraft, to: AgentsDraft): ConfigPatchDto? {
    ...
    if (from.defaultAgent != to.defaultAgent) values[ChipMateCliParser.CONFIG_DEFAULT_AGENT] = to.defaultAgent
    ...
    val imported = draft.imported.map { (name, patch) ->
        ...
        AgentDisplayRow(
            AgentEditDraft(
                ...
                model = cfg?.model,
                variant = cfg?.variant,
                mode = cfg?.mode ?: ChipMateCliParser.MODE_PRIMARY,
                defaultMode = cfg?.mode ?: ChipMateCliParser.MODE_PRIMARY,
