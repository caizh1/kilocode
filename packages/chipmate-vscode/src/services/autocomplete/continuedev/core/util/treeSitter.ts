...
async function loadLanguageForFileExt(fileExtension: string): Promise<Language> {
  ...
  // In production (compiled): __dirname = /path/to/chipmate/src/dist or dist/
  // In development: __dirname = /path/to/chipmate/src/services/autocomplete/continuedev/core/util
  const candidatePaths: string[] = [
    // Production: WASM files are in the same directory as the compiled code
    path.join(__dirname, filename),
    // Bundled extension: __dirname is packages/chipmate-vscode/dist.
