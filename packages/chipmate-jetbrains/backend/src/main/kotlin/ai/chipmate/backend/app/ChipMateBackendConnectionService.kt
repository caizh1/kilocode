 ...
 *
 * Concurrency is handled by the owning [ChipMateBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [ChipMateBackendAppService].
 */
class ChipMateConnectionService(
  ...
  private val appLoadTimeoutMs: Long,
    ...
    constructor(
      ...
      server: CliServer,
      onReconnect: () -> Unit,
    ) : this(cs, server, onReconnect, ChipMateLog.create(ChipMateConnectionService::class.java), 30_000L)
     ...
     *
     * Called under [ChipMateBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [ChipMateBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = ChipMateBackendHttpClients.api(password)
        val lc = ChipMateBackendHttpClients.appLoad(password, appLoadTimeoutMs)
        val hc = ChipMateBackendHttpClients.health(password)
        apiClient = ac
        appLoadClient = lc
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            ...
            synchronized(lock) {
                ...
                lastEvent.set(System.currentTimeMillis())
                val kind = type ?: ChipMateCliDataParser.extractEventType(data)
     ...
     * goes through [ChipMateBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        appLoadApi = null
        apiClient?.let { ChipMateBackendHttpClients.shutdown(it) }
        apiClient = null
        appLoadClient?.let { ChipMateBackendHttpClients.shutdown(it) }
        appLoadClient = null
        healthClient?.let { ChipMateBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("ChipMateConnectionService disposed")
    }
}
