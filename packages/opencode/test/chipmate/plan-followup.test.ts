...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/chipmate/plan-followup"
import { Instance } from "../../src/chipmate/instance"
import * as ChipMateInstance from "../../src/chipmate/instance"
...
async function withInstance(fn: () => Promise<void>) {
  ...
  const provide = spyOn(ChipMateInstance, "provide").mockImplementation((input) =>
...
describe("plan follow-up", () => {
  ...
  test("ask - hides custom answer row on VS Code where the main prompt input handles typed replies", () =>
    withInstance(async () => {
      ...
      try {
        ...
        process.env.CHIPMATE_CLIENT = prev
      }
    }))
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { ChipMateSessionPromptQueue } = await import("../../src/chipmate/session/prompt-queue")
      ...
      ChipMateSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = ChipMateSessionPromptQueue.scope(seeded.sessionID, all)
