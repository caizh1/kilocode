...
class ContextSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun tearDown() {
        try {
            ...
            uiScope.cancel()
            appScope.cancel()
            ChipMatePluginSettings.unsetAutoEditorContext()
        } finally {
            super.tearDown()
        }
    }
    ...
    fun `test editor context toggle marks modified and applies without a config patch`() {
        val panel = requireUi()
        assertTrue(ChipMatePluginSettings.getAutoEditorContext())
        ...
        assertFalse(ChipMatePluginSettings.getAutoEditorContext())
