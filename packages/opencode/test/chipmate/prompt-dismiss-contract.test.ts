/**
 * Contract test for prompt.ts ChipMate-specific invariants.
 *
 * prompt.ts is a shared upstream file. The ChipMate-specific "new prompt unblocks
 * pending suggestions/questions then enqueues without cancelling the in-flight
 * stream" behaviour lives inside a chipmate_change block. An upstream merge
...
describe("prompt.ts ChipMate-specific invariants", () => {
  test("imports Suggestion from chipmate/suggestion", () => {
    const content = fs.readFileSync(PROMPT_FILE, "utf-8")
    expect(content).toMatch(/import\s*\{[^}]*Suggestion[^}]*\}\s*from\s*["']@\/chipmate\/suggestion["']/)
  })
  ...
  test("enqueue reserves the follow-up before dismissing blockers, without cancelling the in-flight fiber", () => {
    ...
    const block = content.match(
      /chipmate_change start[^\n]*register the queued follow-up[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?ChipMateSessionPromptQueue\.enqueue\([\s\S]*?dismiss/,
    )
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("ChipMateSessionPromptQueue.hasFollowup(sessionID)")
  })
})
