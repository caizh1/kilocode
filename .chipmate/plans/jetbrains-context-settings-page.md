...
Implement the Tier 1 Context settings from `docs/jetbrains-vscode-settings-parity.md` in the JetBrains plugin. This is a pure `chipmate.json` settings UI: no CLI feature work, no SDK regen, and no session-rendering changes.

## Goal

Add a new JetBrains settings page under `Settings -> Tools -> ChipMate -> Context` for:
...
- JetBrains settings guidance: `packages/chipmate-jetbrains/AGENTS.md`, especially `Settings UI`.
- Existing settings pages are registered in `packages/chipmate-jetbrains/frontend/src/main/resources/chipmate.jetbrains.frontend.xml`.
- Existing page pattern to mirror:
  - `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/models/ModelsConfigurable.kt`
  - `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/models/ModelsSettingsUi.kt`
  - `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/models/ModelsSettingsState.kt`
- Existing global config write path is sufficient once DTO/parser support is added:
  - Frontend: `ChipMateAppService.updateConfigAsync(...)`
  - RPC: `ChipMateAppRpcApi.updateConfig(patch: ConfigPatchDto)`
  - Backend: `ChipMateBackendAppService.updateConfig(...)`
...
- Keep all UI strings in `ChipMateBundle.properties`. Let other locale bundles fall back unless the repo's resource-bundle checks require duplicated English keys.

## Part A - Shared DTOs

File: `packages/chipmate-jetbrains/shared/src/main/kotlin/ai/chipmate/rpc/dto/ChipMateAppStateDto.kt`
...
File: `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/cli/ChipMateCliDataParser.kt`
...
Add package: `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/context/`
...
- `ID = "ai.chipmate.jetbrains.settings.context"`.
- `getDisplayName()` returns `ChipMateBundle.message("settings.context.displayName")`.
...
- Extend `BaseSettingsUi<ContextSettingsContent, ContextDraft, ConfigPatchDto, ChipMateAppStateDto, Unit>`.
...
File: `packages/chipmate-jetbrains/frontend/src/main/resources/chipmate.jetbrains.frontend.xml`

Add a child configurable:

```xml
<applicationConfigurable
        parentId="ai.chipmate.jetbrains.settings"
        id="ai.chipmate.jetbrains.settings.context"
        groupWeight="3"
        instance="ai.chipmate.client.settings.context.ContextConfigurable"
        bundle="messages.ChipMateBundle"
...
File: `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/ChipMateSettingsConfigurable.kt`
...
`ChipMateSettingsSelection.kt` probably needs no code changes because child IDs already share the root prefix.

## Part F - Strings

File: `packages/chipmate-jetbrains/frontend/src/main/resources/messages/ChipMateBundle.properties`
...
settings.context.compaction.description=Control when ChipMate summarizes long sessions to reduce context usage.
...
settings.context.watcher.description=Glob patterns ChipMate should ignore when watching repository file changes.
...
Add: `packages/chipmate-jetbrains/frontend/src/test/kotlin/ai/chipmate/client/settings/context/ContextSettingsStateTest.kt`
...
Add: `packages/chipmate-jetbrains/frontend/src/test/kotlin/ai/chipmate/client/settings/context/ContextSettingsUiTest.kt`
...
- File: `packages/chipmate-jetbrains/frontend/src/test/kotlin/ai/chipmate/client/testing/FakeAppRpcApi.kt`
...
Update: `packages/chipmate-jetbrains/backend/src/test/kotlin/ai/chipmate/backend/cli/ChipMateCliDataParserTest.kt`
...
Update: `packages/chipmate-jetbrains/backend/src/test/kotlin/ai/chipmate/backend/app/ChipMateBackendAppServiceTest.kt`
...
Update: `packages/chipmate-jetbrains/frontend/src/test/kotlin/ai/chipmate/client/settings/ChipMateSettingsConfigurableTest.kt`

Add:

- `ContextConfigurable.ID == "ai.chipmate.jetbrains.settings.context"`.
- Root page includes a Context link.
- Link order matches XML order.

## Validation

Run from `packages/chipmate-jetbrains/`:
...
./gradlew :backend:test --tests '*ChipMateCliDataParserTest'
./gradlew :backend:test --tests '*ChipMateBackendAppServiceTest'
```
...
1. Run `./gradlew runIde` from `packages/chipmate-jetbrains/`.
2. Open `Settings -> Tools -> ChipMate -> Context`.
...
7. Inspect the global ChipMate config file through the existing `Open: global ...` action if needed.
...
- Changeset: when implementing this user-facing JetBrains settings feature, add a patch changeset for `chipmate-code`/JetBrains according to repo release guidance.
