...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const flags = {
      source: Flag.CHIPMATE_MODELS_URL,
    ...
    yield* Effect.acquireUseRelease(
      Effect.promise(async () => {
        Flag.CHIPMATE_MODELS_URL = source
      ...
      () =>
        Effect.promise(async () => {
          Flag.CHIPMATE_MODELS_URL = flags.source
          Flag.CHIPMATE_MODELS_PATH = flags.path
          Flag.CHIPMATE_DISABLE_MODELS_FETCH = flags.disabled
  ...
  { config: { disabled_providers: ["chipmate", "apertis"] } },
)
