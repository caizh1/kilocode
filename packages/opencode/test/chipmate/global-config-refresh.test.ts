...
async function update(target: ReturnType<typeof app>, provider: "chipmate" | "openrouter") {
...
async function provider(target: ReturnType<typeof app>, directory: string) {
  const response = await target.request("/config", { headers: { "x-chipmate-directory": directory } })
  return (await response.json()).indexing?.provider as string | undefined
}

async function config(dir: string, value: object) {
  await Bun.write(path.join(dir, "chipmate.json"), JSON.stringify(value))
}
...
describe("global config refresh", () => {
  test("update reloads existing instance before responding", async () => {
    ...
    try {
      const pending = update(target, "chipmate")
      await started.promise
      const early = await Promise.race([pending.then(() => true), Bun.sleep(10).then(() => false)])
      expect(early).toBe(false)
      release.resolve()
      expect((await pending).status).toBe(200)
      expect(await provider(target, workspace.path)).toBe("chipmate")
    } finally {
      release.resolve()
      unregister()
    }
  ...
  test("update ignores disposal notification failures", async () => {
    ...
    try {
      expect((await update(target, "chipmate")).status).toBe(200)
    } finally {
