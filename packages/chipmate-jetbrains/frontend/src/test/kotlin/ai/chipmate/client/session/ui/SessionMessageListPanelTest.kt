...
import ai.chipmate.client.session.views.PlanExitView
...
import ai.chipmate.client.session.views.question.QuestionResultView
...
import ai.chipmate.client.session.views.MessageToolbar
import ai.chipmate.client.session.views.MessageView
import ai.chipmate.client.session.views.PromptAttachmentView
import ai.chipmate.client.session.views.TextView
import ai.chipmate.client.session.views.TurnView
import ai.chipmate.client.session.views.base.PartView
import ai.chipmate.client.session.views.tool.TaskToolView
import ai.chipmate.client.session.views.tool.ToolView
import ai.chipmate.client.session.views.todo.TodoWriteView
import ai.chipmate.client.ui.DiffStatBadge
...
import ai.chipmate.rpc.dto.MessageSummaryDto
...
import ai.chipmate.rpc.dto.TodoDto
...
class SessionMessageListPanelTest : BasePlatformTestCase() {
    ...
    fun `test queued turn shows badge and remove action`() {
        ...
        assertFalse(components(u1).filterIsInstance<JBLabel>().any { it.text == ChipMateBundle.message("session.queued") })
        assertTrue(components(u2).filterIsInstance<JBLabel>().any { it.text == ChipMateBundle.message("session.queued") })

        val remove = components(u2).filterIsInstance<HoverIcon>().single()
        assertEquals(ChipMateBundle.message("session.queued.remove"), remove.toolTipText)
    ...
    fun `test user prompt toolbar omits rollback when revert handler is absent`() {
        ...
        assertFalse(components(toolbar).filterIsInstance<JButton>().any { it.toolTipText == ChipMateBundle.message("revert.message.rollback") })
    }
    ...
    fun `test user prompt toolbar shows rollback when revert handler is present`() {
        ...
        val rollback = components(toolbar)
            ...
            .first { it.toolTipText == ChipMateBundle.message("revert.message.rollback") }
        rollback.doClick()
    ...
    fun `test text markdown link uses panel url opener`() {
        ...
        model.upsertMessage(msg("a1", "assistant"))
        model.updateContent("a1", part("p1", "a1", "text", text = "[docs](https://chipmate.ai/docs)"))

        val view = item.findMessage("a1")!!.part("p1") as TextView
        view.md.simulateLink("https://chipmate.ai/docs")

        assertEquals(listOf("https://chipmate.ai/docs"), urls)
    }
    ...
    fun `test rollback banner uses session dialog card with standard actions`() {
        ...
        assertEquals(
            listOf(ChipMateBundle.message("revert.banner.redo"), ChipMateBundle.message("revert.banner.redo.all")),
            buttons.map { it.text },
        )
        assertEquals(listOf(ChipMateBundle.message("revert.banner.redo")), buttons.filter { it.isVisible }.map { it.text })
        ...
        val hint = components(banner)
            ...
            .first { it.text == ChipMateBundle.message("revert.banner.hint") }
    ...
    fun `test rollback banner opens rolled back diff`() {
        ...
        val button = components(banner).filterIsInstance<HoverIcon>()
            .first { it.toolTipText == ChipMateBundle.message("session.part.tool.openDiff") }
        assertTrue(button.isVisible)
        assertTrue(button.isEnabled)
        button.doClick()

        assertEquals(listOf(diff), opened.single())
        assertEquals(ChipMateBundle.message("revert.banner.openDiff.title"), titles.single())
    ...
    fun `test rollback banner shows redo all only for multiple reverted messages`() {
        ...
        assertTrue(components(banner).filterIsInstance<JButton>().first { it.text == ChipMateBundle.message("revert.banner.redo.all") }.isVisible)

        model.setRevert(SessionRevertDto("u2"))
        banner.update()

        assertFalse(components(banner).filterIsInstance<JButton>().first { it.text == ChipMateBundle.message("revert.banner.redo.all") }.isVisible)
    }
    ...
    fun `test rollback banner buttons invoke actions`() {
        ...
        components(banner).filterIsInstance<JButton>().first { it.text == ChipMateBundle.message("revert.banner.redo") }.doClick()
        components(banner).filterIsInstance<JButton>().first { it.text == ChipMateBundle.message("revert.banner.redo.all") }.doClick()
    ...
    fun `test rollback state disables banner actions and shows inline progress`() {
        ...
        val buttons = components(banner).filterIsInstance<JButton>().filter { it.text.isNotEmpty() }
        assertTrue(buttons.filter { it.text == ChipMateBundle.message("revert.banner.redo") }.all { !it.isEnabled })
        assertTrue(buttons.filter { it.text == ChipMateBundle.message("revert.banner.redo.all") }.all { !it.isEnabled })
    ...
    fun `test rollback banner explains snapshotless history only revert`() {
        ...
        val notice = components(banner).filterIsInstance<JBLabel>()
            .first { it.text == ChipMateBundle.message("revert.banner.filesNotRestored") }
    ...
    fun `test completed plan update replaces tool view and keeps open file action`() {
        ...
        model.updateContent(
            ...
            toolPart(
                ...
                metadata = mapOf("plan" to ".chipmate/plans/x.md"),
            ),
        )

        val view = mv.part("tp1") as PlanExitView
        view.simulateLink(".chipmate/plans/x.md")

        assertEquals(listOf(".chipmate/plans/x.md"), opened)
    }
    ...
    private fun disableValidateRoots() {
        val key = "chipmate.session.validateRoots"
