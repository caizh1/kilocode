...
describe("ChipMate reference compatibility", () => {
  it.live("materializes and refreshes configured Git references", () =>
    withRemote((fixture) => {
      ...
      return Effect.gen(function* () {
        ...
        process.env.CHIPMATE_REPO_CLONE_GITHUB_BASE_URL = base.endsWith("/") ? base : `${base}/`
        yield* Effect.addFinalizer(() =>
          Effect.sync(() => {
            if (previous === undefined) delete process.env.CHIPMATE_REPO_CLONE_GITHUB_BASE_URL
            else process.env.CHIPMATE_REPO_CLONE_GITHUB_BASE_URL = previous
          }),
        )
