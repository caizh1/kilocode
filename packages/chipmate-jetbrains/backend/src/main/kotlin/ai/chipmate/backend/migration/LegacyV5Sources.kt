...
class LegacyV5Sources(
    ...
    private val root = File(home, ".chipmate")
    private val storage = File(root, "globalStorage")
    private val scoped = File(storage, "chipmate.chipmate-code")

    fun secretsJson(): String? = File(root, "secrets.json").text("secrets")
    fun globalStateXml(): String? = File(config, "options/chipmate-extension-storage.xml").text("globalStateXml")
