...
import { SemanticSearchTool } from "../../src/chipmate/tool/semantic-search"
...
function stub(state: ChipMateIndexing.Status["state"] = "Complete") {
  const pipeline = {
    state,
    message: state,
    processedFiles: 1,
  ...
  const current = spyOn(ChipMateIndexing, "current").mockResolvedValue({
    state,
    message: state,
    processedFiles: 1,
    totalFiles: 1,
  ...
  test("asks permission and forwards normalized relative path to indexing search", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const search = spyOn(ChipMateIndexing, "search").mockResolvedValue([])
        const restore = stub()
