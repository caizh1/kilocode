...
// The "ChipMate" Pierre/Shiki theme used by every diff review surface (Code / Diff /
// File / SessionReview) and by markdown code highlighting. Pierre resolves the
// theme by name when a worker pool initializes (resolveThemes(["ChipMate"])) or when
// getSharedHighlighter() attaches it; if the name was never registered it throws
// "resolveTheme: No valid loader for ChipMate".
//
...
// diff component imports the worker factory, which calls ensureChipMateDiffTheme()
...
// without markdown (e.g. chipmate-console) to fire a racy `void import(...)` purely to
...
// the registration through ensureChipMateDiffTheme() instead.

export const CHIPMATE_DIFF_THEME = "ChipMate"

const registrations = (() => {
  const key = Symbol.for("chipmate.ui.pierre.chipmate-diff-theme")
...
// guard keyed by the public registration function. Duplicate ChipMate modules sharing
// one Pierre instance stay no-ops, while separately bundled Pierre instances still
// receive their own registration.
export function ensureChipMateDiffTheme(): void {
  if (registrations.has(registerCustomTheme)) return

  registerCustomTheme(CHIPMATE_DIFF_THEME, () => {
