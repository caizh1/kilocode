...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = ChipMateAppService(appScope, rpc)
        workspaces = ChipMateWorkspaceService(appScope, workspaceRpc)
        val state = ChipMateAppStateDto(
            ChipMateAppStatusDto.READY,
            config = ConfigDto(model = "chipmate/old"),
    ...
    fun `test matching app state before save callback keeps pending target`() {
        ...
        rpc.state.value = state("chipmate/new")
    ...
    fun `test matching app state after save callback preserves dirty draft`() {
        ...
        edt {
            assertSelected(panel, "chipmate/old")
            assertTrue(panel.modified())
        }
    }
    ...
    fun `test stale app state during pending save is ignored`() {
        ...
        rpc.state.value = state("chipmate/old")
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "ChipMate" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to ChipMate") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to ChipMate"))
    ...
    private fun state(model: String): ChipMateAppStateDto = ChipMateAppStateDto(
        ChipMateAppStatusDto.READY,
    ...
    private fun select(panel: ModelsSettingsUi, id: String) {
        ...
        picker.onSelect(ModelPicker.Item(id, id.replaceFirstChar { it.titlecase() }, "chipmate", "ChipMate"))
    }
