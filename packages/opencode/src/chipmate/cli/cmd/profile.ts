...
import type { ChipMateBalance, ChipMateProfile } from "@chipmate/chipmate-gateway"
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<ChipMateProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<ChipMateBalance | null>
...
export const ProfileCommand = cmd({
  command: "profile",
  describe: "show ChipMate account profile",
  builder: (yargs: Argv) =>
    yargs.option("json", {
...
export async function handle(args: Args) {
  ...
  const auth = await get("chipmate")
  const error = args.error ?? UI.error
  const exit = args.exit ?? ((code: number) => (process.exitCode = code))

  if (!auth || auth.type !== "oauth") {
    error("Not authenticated with ChipMate Gateway")
    exit(1)
    return
  }

  const { fetchBalance, fetchProfile } = await import("@chipmate/chipmate-gateway")
