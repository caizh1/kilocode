...
import { environmentDetails } from "../../src/chipmate/editor-context"
import { ChipMateSystemPrompt } from "../../src/chipmate/system-prompt"
...
import PROMPT_GPT55 from "../../src/session/prompt/chipmate-gpt-5.5.txt"
...
describe("environmentDetails", () => {
  test("includes cwd and worktree in dynamic context", () => {
    const result = environmentDetails({
      directory: "/repo/.chipmate/worktrees/feature",
      worktree: "/repo/.chipmate/worktrees/feature",
      activeFile: "src/app.ts",
    })

    expect(result).toContain("Working directory: /repo/.chipmate/worktrees/feature")
    expect(result).toContain("Workspace root folder: /repo/.chipmate/worktrees/feature")
...
describe("ChipMate product prompts", () => {
  ...
  const code = [
    ...
    'import { ChipMateSessionPrompt } from "./src/chipmate/session/prompt.ts"',
    'import { reviewCommand } from "./src/chipmate/review/command.ts"',
    'import { ProviderTest } from "./test/fake/provider.ts"',
    'import SOUL from "./src/chipmate/soul.txt"',
    'import PLAN from "./src/chipmate/session/native-plan-prompt.txt"',
    ...
    'import GPT55 from "./src/session/prompt/chipmate-gpt-5.5.txt"',
    ...
    'import ULTRA from "./src/chipmate/agent/ultra.txt"',
    "const raw = [ANTHROPIC, BEAST, CODEX, DEFAULT, GEMINI, GPT, GPT55, KIMI, LING, TRINITY]",
    "const prompts = raw.map(ChipMateSystemPrompt.brand)",
    "const plan = ChipMateSystemPrompt.brand(PLAN)",
    "const review = reviewCommand().template",
    "const memory = ChipMateSystemPrompt.memoryGuidance()",
    'const custom = "Custom agent keeps ChipMate as user-authored text"',
    'const technical = "Keep CHIPMATE_CONFIG, chipmate.jsonc, chipmate_change, chipmate_memory_save, and https://github.com/ChipMate-Org/example"',
    "const model = ProviderTest.model()",
    ...
    'const user = { id: "msg_user-chipmate", sessionID: "session-chipmate", role: "user", time: { created: 0 }, agent: "build", model: { providerID: "openai", modelID: model.id }, system: "User-owned system text mentions ChipMate" }',
    'const base = { user, sessionID: "session-chipmate", model, agent, system: ["Project instructions describe ChipMate CLI and technical chipmate.jsonc context"], messages: [{ role: "user", content: "hello" }], tools: {}, provider, plugin, flags: { outputTokenMax: undefined, client: "vscode" }, isWorkflow: false }',
    ...
    "const ultraOrder = ChipMateSessionPrompt.system({ ...order, agent: ultraAgent })",
    'const codeOrder = ChipMateSessionPrompt.system({ ...order, agent: { ...agent, name: "code", native: true } })',
    ...
    "console.log(JSON.stringify({ soul: SystemPrompt.soul(), soulRaw: SOUL.trim(), instructions: SystemPrompt.instructions(), prompts, plan, review, memory, custom: ChipMateSystemPrompt.provider({ custom, defaults: [DEFAULT] })[0], technical: ChipMateSystemPrompt.brand(technical), standard: standard.system[0], standardMessage: standard.messages[0], oauth: String(oauth.params.options.instructions), oauthRoles: oauth.messages.map((message) => message.role), customRequest: customRequest.system[0], base: ChipMateSystemPrompt.provider({ defaults: SystemPrompt.provider(model) })[0], ultraRaw: ULTRA, ultra: ultra.system[0], ultraOauth: String(ultraOauth.params.options.instructions), ultraCustom: ultraCustom.system[0], ultraOrder, codeOrder, agents: agents.map((request) => request.system[0]), modes: modes.map((request) => request.system[0]) }))",
  ].join(";")
  ...
  test("brands every built-in model-facing entry only for ChipMate", async () => {
    const result = await run(
      {
        ...
        CHIPMATE_STORAGE_ROOT: "/tmp/chipmate-system-prompt-test",
        CHIPMATE_VSCODE_GLOBAL_STORAGE: "/tmp/chipmate-system-prompt-test",
      },
      code,
    )
    ...
    for (const prompt of prompts) {
      ...
      expect(prompt).not.toMatch(/^You are (?:ChipMate|ChipMate|opencode)\b/m)
      expect(prompt).not.toMatch(/\bChipMate(?: Code)?\b/)
      expect(prompt).not.toContain("https://chipmate.ai/docs")
      expect(prompt).not.toContain("github.com/ChipMate-Org/chipmate")
    }
    ...
    expect(output.custom).toBe("Custom agent keeps ChipMate as user-authored text")
    expect(output.technical).toBe(
      "Keep CHIPMATE_CONFIG, chipmate.jsonc, chipmate_change, chipmate_memory_save, and https://github.com/ChipMate-Org/example",
    )
    ...
    for (const prompt of [output.standard, output.oauth]) {
      expect(prompt).toContain("Project instructions describe ChipMate CLI and technical chipmate.jsonc context")
      expect(prompt).toContain("User-owned system text mentions ChipMate")
      expect(prompt).not.toMatch(/^You are (?:ChipMate|ChipMate|opencode)\b/m)
    }
    expect(output.customRequest).toContain("Custom agent keeps ChipMate as user-authored text")
    ...
    expect(output.ultraCustom).toContain("Custom agent keeps ChipMate as user-authored text")
    ...
    expect(output.ultraRaw).toContain("frozen Code answer")
    expect(output.ultraRaw).toContain("one fresh, read-only Ask synthesizer")
    expect(output.ultraRaw).toContain("does not use Council voting")
    expect(output.ultraRaw).toContain("cannot see each other")
    expect(output.ultraRaw).toContain("delivers the synthesizer answer exactly")
    ...
    expect(output.ultraRaw).not.toContain("ultra_council_adjudicate")
    expect(output.ultraRaw).not.toContain("ultra_submit_arbitration")
    expect(output.ultraRaw).toContain("quality-based retries")
  ...
  test("keeps native ChipMate prompts byte-for-byte compatible", async () => {
    const result = await run(
      {
        CHIPMATE_PRODUCT_PROFILE: "",
        CHIPMATE_STORAGE_ROOT: "",
    ...
    expect(output.plan).toMatch(/You are ChipMate\b/)
    expect(output.review).toMatch(/^You are ChipMate\b/)
    expect(output.memory).toMatch(/^The following ChipMate memory blocks\b/)
