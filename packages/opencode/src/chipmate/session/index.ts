...
import { Bus } from "@/bus"
import { Instance, type InstanceContext } from "@/chipmate/instance"
...
import { ENV_FEATURE } from "@chipmate/chipmate-gateway"
import { existsSync } from "fs"
import path from "path"
import { ProductProfile } from "../product-profile"
import { iife } from "@/util/iife"
import { ChipMateSessionEvent, type ChipMateSessionCloseReason } from "./event"

export namespace ChipMateSession {
  const log = Log.create({ service: "session.chipmate" })

  // ---------------------------------------------------------------------------
  // Events
  // ---------------------------------------------------------------------------

  export const Event = ChipMateSessionEvent
  export type CloseReason = ChipMateSessionCloseReason
  ...
  // Emitted by ChipMateSessionPromptQueue on every transition that changes the set
  ...
  // context (e.g. ChipMateSessionPromptQueue transitions, which fire from inside
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? process.env["CHIPMATE_PLATFORM"]
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (ChipMate / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for ChipMate)
   ...
   *
   * ChipMate does not charge end users a per-request fee, so for the ChipMate provider the
  ...
  export function providerCost(input: {
    ...
    const isChipMate = (input.provider?.id ?? input.providerID) === "chipmate"
    ...
    if (orUsage) {
      ...
      // ChipMate doesn't charge a fee on top of the upstream inference cost, so for ChipMate
      ...
      const cost = isChipMate && upstream !== undefined ? upstream : regular
      if (cost !== undefined) return cost
    }

    // 2. Anthropic Messages or OpenAI Responses via OpenRouter. The ChipMate Gateway wrapper
    //    restores the verbatim usage payload under the AI SDK's raw usage escape hatch.
    //    ChipMate doesn't charge end users a per-request fee, so only upstream cost is relevant.
    ...
    //    gateway fee that ChipMate would pass through, but ChipMate doesn't charge end users a
  ...
  export function shareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      ...
      return ChipMateSessions.share(id)
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { ChipMateSessions } = await import("@/chipmate-sessions/chipmate-sessions")
      await ChipMateSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { ChipMateSessions } = await import("@/chipmate-sessions/chipmate-sessions")
    await ChipMateSessions.remove(id).catch(() => {})
  }
...
export { chipmateSessionFork } from "./fork-command"
