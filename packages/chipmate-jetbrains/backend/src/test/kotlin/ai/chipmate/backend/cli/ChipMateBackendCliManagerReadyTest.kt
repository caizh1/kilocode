...
class ChipMateBackendCliManagerReadyTest {

    @Test
    fun `ready line returns port`() = runBlocking {
        val state = awaitReady(
            stdout = ByteArrayInputStream("chipmate server listening on http://127.0.0.1:12345\n".toByteArray()),
    ...
    fun `init after dispose is rejected without spawning`() = runBlocking {
        val manager = ChipMateBackendCliManager(log = TestLog())
        manager.dispose()
        var resolved = false
    ...
    fun `ipv6 bind form remains a known non match`() = runBlocking {
        ...
        val state = awaitReady(
            stdout = ByteArrayInputStream("chipmate server listening on http://[::1]:12345\n".toByteArray()),
            stderr = StringBuilder(),
            pwd = "pwd123",
            timeoutMs = TIMEOUT_MS,
            alive = { false },
