---
title: "Workflows"
description: "Create automated workflows with ChipMate"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for ChipMate to execute.

{% image src="/docs/img/screenshot-tests/chipmate-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in ChipMate" width="420" caption="Workflows tab in ChipMate" /%}

## Creating Workflows

Workflows are Markdown files stored as **slash commands** in `.chipmate/commands/`:

- **Global commands**: `~/.config/chipmate/commands/` (available in all projects)
- **Project commands**: `[project]/.chipmate/commands/` (project-specific)

If `.chipmate/commands/` is a symlink to a directory outside the project, allow that exact source in your global `~/.config/chipmate/chipmate.jsonc`:
...
For example, a file at `.chipmate/commands/submit-pr.md` is invoked with `/submit-pr`.
...
The new extension automatically migrates legacy workflows from `.chipmate/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.
...
Create a file called `submit-pr.md` in your `.chipmate/commands` directory:
