...
export async function startSpeechCapture(input: Input): Promise<boolean> {
  ...
  try {
    ...
    const file = path.join(os.tmpdir(), `chipmate-stt-${process.pid}-${Date.now()}.wav`)
...
export async function stopSpeechCapture(requestId: string): Promise<Audio> {
  ...
  const size = await stat(state.file)
    .then((info) => info.size)
    .catch((err: unknown) => {
      console.warn("[ChipMate New] Failed to stat speech recording", err)
      return 0
    })
...
async function findFFmpeg(): Promise<string> {
  const paths = [
    process.env.CHIPMATE_FFMPEG_PATH,
  ...
  for (const bin of paths) {
    ...
    try {
      ...
      console.warn(`[ChipMate New] FFmpeg candidate failed: ${bin}`, err)
    }
  }

  throw new Error("Speech input needs the bundled FFmpeg helper, but it was not found. Rebuild or reinstall ChipMate.")
}
...
async function inputArgSets(bin: string): Promise<Args[]> {
  ...
  if (process.platform === "linux") {
    const device = process.env.CHIPMATE_FFMPEG_AUDIO_DEVICE
    if (device)
      return [
...
async function windowsInputArgSets(bin: string): Promise<Args[]> {
  const configured = process.env.CHIPMATE_FFMPEG_AUDIO_DEVICE
...
async function removeFile(file: string): Promise<void> {
  await unlink(file).catch((err: unknown) => {
    console.warn("[ChipMate New] Failed to remove speech recording", err)
  })
}
