...
class RevertBanner(
    ...
    private val diff = toolbarButton(
        ToolbarButtonAction(SessionViewIcons.openDiff, ChipMateBundle.message("session.part.tool.openDiff"), ::openDiffViewer),
    ).apply {
        isEnabled = false
        isVisible = false
    }
    ...
    private val hint = JBLabel(ChipMateBundle.message("revert.banner.hint")).apply {
        font = JBFont.small()
    }

    private val notice = JBLabel(ChipMateBundle.message("revert.banner.filesNotRestored")).apply {
        font = JBFont.small()
    }
    ...
    init {
        ...
        card.setActions(listOf(
            BaseQuestionView.Action("redo", ChipMateBundle.message("revert.banner.redo"), primary = false) { redoAction() },
            BaseQuestionView.Action("all", ChipMateBundle.message("revert.banner.redo.all"), primary = false) { redoAllAction() },
        ))
    ...
    fun update() {
        ...
        title.text = ChipMateBundle.message(if (total == 1) "revert.banner.count.one" else "revert.banner.count.other", total)
     ...
     */
    private fun resolveDiffs(revert: ai.chipmate.rpc.dto.SessionRevertDto): List<DiffFileDto> =
    ...
    private fun openDiffViewer() {
        ...
        openDiff(diffs, ChipMateBundle.message("revert.banner.openDiff.title"), "revert:${sessionId ?: "pending"}:${revert.messageID}")
    }
