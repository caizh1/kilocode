package ai.chipmate.client.settings.base
...
class ChipMateReadyConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = ChipMateAppService(scope, rpc)
    ...
    fun `test disconnected shows unavailable and does not create ready component`() {
        ...
        edt {
            val text = text(root)
            assertTrue(text, text.contains(ChipMateBundle.message("settings.cli.unavailable.title")))
            assertTrue(text, text.contains(ChipMateBundle.message("settings.cli.unavailable.message")))
    ...
    fun `test ready transition creates ready component once`() {
        ...
        rpc.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY)
        flushUntil { edt { cfg?.created == 1 } }
        rpc.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.DISCONNECTED)
        flush()

        edt {
            assertEquals(1, cfg?.created)
            assertTrue(text(root).contains("Ready content"))
            assertFalse(text(root).contains(ChipMateBundle.message("settings.cli.unavailable.title")))
        }
    }
