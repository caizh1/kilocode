...
import {
  ChipMateQwenInlineCompletionProvider,
  QWEN_DOCUMENT_SELECTOR,
  isQwenSupportedDocument,
} from "../../src/services/qwen-autocomplete/ChipMateQwenInlineCompletionProvider"
...
describe("qwen autocomplete registration", () => {
  it("follows the official ChipMate coexistence gate at runtime", async () => {
  ...
  it("registers only the qwen provider when qwen-direct is enabled", async () => {
    ...
    expect(calls[0]!.provider).toBeInstanceOf(ChipMateQwenInlineCompletionProvider)
    reg.dispose()
  })
...
describe("ChipMateQwenInlineCompletionProvider", () => {
