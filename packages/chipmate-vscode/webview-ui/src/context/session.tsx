...
import { CHIPMATE_PROVIDER_ID, parseModelString } from "../../../src/shared/provider-model"
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  const [pendingChipMateModel, setPendingChipMateModel] = createSignal<{
    modelID?: string
    agent?: string
    after: number
  } | null>(null)
  ...
  function selectChipMateModel(modelID?: string, agent?: string) {
    if (!modelID && !agent) return
    setPendingChipMateModel({ ...(modelID && { modelID }), ...(agent && { agent }), after: catalog() })
    if (modelID) vscode.postMessage({ type: "requestProviders" })
  }

  const unsubChipMateModel = vscode.onMessage((message: ExtensionMessage) => {
    if (message.type === "providersLoaded") {
      setCatalog((value) => value + 1)
      return
    }
    if (message.type === "selectChipMateModel") selectChipMateModel(message.modelID, message.agent)
  })
  onCleanup(unsubChipMateModel)

  createEffect(() => {
    const pending = pendingChipMateModel()
    if (!pending || agents().length === 0 || (pending.modelID && catalog() <= pending.after)) return
    setPendingChipMateModel(null)
    if (pending.modelID && !provider.providers()[CHIPMATE_PROVIDER_ID]?.models[pending.modelID]) {
      console.warn("[ChipMate New] Ignoring unavailable ChipMate catalog model:", pending.modelID)
      return
    }
    if (pending.agent && !agentNames().has(pending.agent)) {
      console.warn("[ChipMate New] Ignoring unavailable ChipMate agent:", pending.agent)
      return
    }
    if (pending.agent) selectAgent(pending.agent)
    if (pending.modelID) selectModel(CHIPMATE_PROVIDER_ID, pending.modelID)
  })
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed": {
        ...
        console.error("[ChipMate New] Cloud session import failed:", message.error)
        break
      }
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[ChipMate New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!providerID || !modelID) {
      console.warn("[ChipMate New] Cannot send message: no model selected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!providerID || !modelID) {
      console.warn("[ChipMate New] Cannot send command: no model selected")
      return
    }
  ...
  function abort() {
    ...
    if (!scope) {
      console.warn("[ChipMate New] Cannot abort: no current or pending session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[ChipMate New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    ...
    if (id.startsWith("cloud:")) {
      console.warn("[ChipMate New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
    agentDrafts.prune(draftSessionID())
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot rename session: not connected")
      return
    }
  ...
  function exportSessionTranscript(id: string) {
    if (!server.isConnected()) {
      console.warn("[ChipMate New] Cannot export session transcript: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[ChipMate New] Cannot export cloud session transcript")
      return
    }
