...
import { slashDisplay } from "@/chipmate/cli/cmd/command-display" // chipmate_change
import { createSessionPart, sessionMentionText } from "../../chipmate/session-mentions" // chipmate_change
import { DialogSessionMention } from "../../chipmate/dialog-session-mention" // chipmate_change
import { useDialog } from "../../ui/dialog" // chipmate_change
import { Locale } from "../../util/locale"
import type { PromptInfo } from "../../prompt/history"
import { useFrecency } from "../../prompt/frecency"
import { useBindings, useCommandSlashes, useOpencodeModeStack } from "../../keymap"
import { displayCharAt, mentionTriggerIndex } from "../../prompt/display"
import type { FileSystemEntry } from "@chipmate/sdk/v2"
...
export type AutocompleteRef = {
  ...
  // chipmate_change start - validate cursor moves and close overlays without mutating draft text
  onCursorChange: () => void
  dismiss: () => void
  // chipmate_change end
  visible: false | "@" | "/"
...
export function Autocomplete(props: {
  ...
  // chipmate_change start - "Past chats" opens a searchable session picker dialog
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      // chipmate_change start - preserve suffixes like :skill when inserting selected slash commands
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    const nonFileOptions: AutocompleteOption[] =
      store.visible === "@" ? [...referenceAliasesValue, ...agentsValue, ...mcpResources(), pastChatsOption] : [...commandsValue] // chipmate_change - add past chats option
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      // chipmate_change start - close stale suggestions while allowing normal cursor movement
      {
        key: "right",
        fallthrough: true,
        cmd: () => {
  ...
  // chipmate_change start - keep slash text intact when overlays hide the prompt,
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      // chipmate_change start - dismiss stale popup after cursor leaves active filter region
      onCursorChange() {
        if (!store.visible) return
        ...
        >
          {(option, index) => (
            <box
              ...
              onMouseUp={() => select()}
              gap={1} // chipmate_change - keep descriptions separated from labels in flex layout
            >
              ...
              <Show when={option().description}>
                <text fg={index === store.selected ? selectedForeground(theme) : theme.textMuted} wrapMode="none">
                  {option().description?.trimStart()}{/* chipmate_change */}
                </text>
              </Show>
            </box>
          )}
