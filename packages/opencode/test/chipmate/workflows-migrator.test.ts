...
import { WorkflowsMigrator } from "../../src/chipmate/workflows-migrator"
...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    test("discovers project workflows", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".chipmate", "workflows")
    ...
    test("discovers workflows from legacy .chipmate/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".chipmate", "workflows")
    ...
    test("discovers global workflows from ~/.chipmate/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".chipmate", "workflows", "global.md"), "# Global\n\nGlobal workflow")
      ...
      expect(
        workflows.some((w) => w.source === "global" && w.path.includes(path.join(".chipmate", "workflows", "global.md"))),
      ).toBe(true)
    })
    ...
    test("skips environment substitutions in project workflows", async () => {
      const name = "CHIPMATE_WORKFLOW_PROJECT_SECRET"
      const prior = process.env[name]
      process.env[name] = "environment secret"
      try {
        await using tmp = await tmpdir({
          init: async (dir) => {
            await Bun.write(path.join(dir, ".chipmate", "workflows", "workflow.md"), `{env:${name}}`)
            await Bun.write(path.join(dir, ".chipmate", "workflows", "safe.md"), "safe workflow")
          },
        })
    ...
    test("preserves file and environment substitutions in trusted global workflows", async () => {
      const name = "CHIPMATE_WORKFLOW_GLOBAL_SECRET"
      const prior = process.env[name]
      process.env[name] = "environment secret"
      try {
        await using tmp = await tmpdir({
          init: async (dir) => {
            ...
            await Bun.write(
              path.join(dir, ".chipmate", "workflows", "trusted.md"),
              [`{file:../../secret.txt}`, `{env:${name}}`].join("\n"),
            )
            await Bun.write(path.join(dir, "project", "README.md"), "project")
            await Bun.write(path.join(dir, "project", ".chipmate", "workflows", "trusted.md"), `{env:${name}}`)
          },
        })
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.ChipMateWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".chipmate", "workflows")
