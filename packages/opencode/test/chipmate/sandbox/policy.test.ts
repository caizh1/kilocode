...
import { assertWrite, run as runSandbox } from "@chipmate/sandbox"
import { Effect, Exit } from "effect"
import { profile } from "@/chipmate/sandbox/policy"
import { SandboxPreference } from "@/chipmate/sandbox/preference"
...
function expected(...directories: string[]) {
  return new Set([...directories, ...chipmate])
}
...
describe("sandbox policy", () => {
  ...
  test("keeps ChipMate state writable without exposing sandbox policy state", async () => {
    await using tmp = await fixture()
    const dirs = tmp.extra
