...
import { TestCli } from "../../script/chipmate/test-cli"
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-test-cli-cwd-"))
        ...
        const output = await (async () => {
          ...
          let text = ""
          while (!text.includes("chipmate server listening on")) {
        ...
        expect(output, `stdout:\n${output}\nstderr:\n${err}`).toContain("chipmate server listening on")
        expect(code, `stdout:\n${output}\nstderr:\n${err}`).not.toBe(1)
      } finally {
