...
describe("ChipMate logger compatibility", () => {
  ...
  test("does not truncate the dev log twice during one run", async () => {
    const previous = Global.Path.log
    const run = process.env.CHIPMATE_RUN_ID
    const initialized = process.env.CHIPMATE_LOG_INITIALIZED_RUN_ID
    await using tmp = await tmpdir()
    Global.Path.log = tmp.path
    process.env.CHIPMATE_RUN_ID = "run-1"
    delete process.env.CHIPMATE_LOG_INITIALIZED_RUN_ID
    ...
    try {
      ...
      await Log.init({ print: true })
      Global.Path.log = previous
      if (run === undefined) delete process.env.CHIPMATE_RUN_ID
      else process.env.CHIPMATE_RUN_ID = run
      if (initialized === undefined) delete process.env.CHIPMATE_LOG_INITIALIZED_RUN_ID
      else process.env.CHIPMATE_LOG_INITIALIZED_RUN_ID = initialized
    }
  })
})
