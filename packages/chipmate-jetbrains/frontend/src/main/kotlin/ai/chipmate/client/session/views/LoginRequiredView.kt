 ...
 * enters [ai.chipmate.client.session.model.SessionState.LoginRequired].
 *
 * Mirrors the anchored placement of [PermissionView] and [question.QuestionView]:
 * it stays as a stable child inside [ai.chipmate.client.session.ui.SessionMessageListPanel]
...
class LoginRequiredView(
    ...
    init {
        ...
        card.setHeader(ChipMateBundle.message("session.login.required.title"))
        card.setActions(listOf(
            BaseQuestionView.Action(ID_DISMISS, ChipMateBundle.message("session.login.required.dismiss"), primary = false) { dismiss() },
            BaseQuestionView.Action(ID_OPEN, ChipMateBundle.message("session.login.required.button"), primary = true) { openProfile() },
        ))
    ...
    internal fun openProfileButton() = button(ChipMateBundle.message("session.login.required.button"))
    internal fun dismissButton() = button(ChipMateBundle.message("session.login.required.dismiss"))
