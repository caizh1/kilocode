...
import { NotifyUserTool } from "@/chipmate/tool/notify-user"
...
function runNotifyTool(params: { readonly message: string }, sessions: ChipMateSessions.Interface) {
  const layer = Layer.mergeAll(
    Layer.succeed(ChipMateSessions.Service, ChipMateSessions.Service.of(sessions)),
...
describe("notify_user tool", () => {
  ...
  test("registers with id and description", async () => {
    const layer = Layer.mergeAll(
      Layer.succeed(ChipMateSessions.Service, ChipMateSessions.Service.of({
        init: () => Effect.void,
  ...
  test("trims message before sending", async () => {
    ...
    const sessions = ChipMateSessions.Service.of({
      init: () => Effect.void,
  ...
  test("returns failure text when not connected", async () => {
    ...
    expect(result.output).toContain("not connected to ChipMate cloud")
  })
