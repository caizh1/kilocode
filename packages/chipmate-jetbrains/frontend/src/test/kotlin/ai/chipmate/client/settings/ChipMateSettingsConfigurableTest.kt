...
import ai.chipmate.client.settings.rules.RulesConfigurable
...
class ChipMateSettingsConfigurableTest : BasePlatformTestCase() {

    fun `test id matches xml registration`() {
        val cfg = ChipMateSettingsConfigurable()
        assertEquals("ai.chipmate.jetbrains.settings", cfg.id)
    }

    fun `test child profile id matches xml registration`() {
        // Verify the constants used in XML registrations are stable
        assertEquals("ai.chipmate.jetbrains.settings.profile", UserProfileConfigurable.ID)
    }

    fun `test child models id matches xml registration`() {
        assertEquals("ai.chipmate.jetbrains.settings.models", ModelsConfigurable.ID)
    }

    fun `test child context id matches xml registration`() {
        assertEquals("ai.chipmate.jetbrains.settings.context", ContextConfigurable.ID)
    }

    fun `test child provider and behavior ids match xml registration`() {
        assertEquals("ai.chipmate.jetbrains.settings.providers", ProvidersConfigurable.ID)
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior", AgentBehaviorConfigurable.ID)
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior.rules", RulesConfigurable.ID)
    }
    ...
    fun `test root implements SearchableConfigurable but not Parent`() {
        ...
        val interfaces = ChipMateSettingsConfigurable::class.java.interfaces
        assertFalse(
            "ChipMateSettingsConfigurable must not implement SearchableConfigurable.Parent",
    ...
    fun `test open invokes select with child found by id`() {
        ...
        assertEquals("ai.chipmate.jetbrains.settings.profile", UserProfileConfigurable.ID)
        assertEquals("ai.chipmate.jetbrains.settings.profile", profile.id)
    }

    fun `test isModified always false`() {
        assertFalse(ChipMateSettingsConfigurable().isModified)
    }
