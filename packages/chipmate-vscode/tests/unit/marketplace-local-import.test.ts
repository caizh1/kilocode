...
describe("local Skill import transactions", () => {
  ...
  it("reports managed conflicts, rolls back a failed item, and continues the batch", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-local-import-batch-"))
    try {
  ...
  it("cancellation lets the active item finish and skips subsequent items", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-local-import-cancel-"))
    try {
      const source = path.join(root, "source")
      const project = path.join(root, "project")
...
async function fixture(run: (value: { root: string; source: string; project: string; home: string }) => Promise<void>) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-local-import-"))
