...
test("config writes include the selected directory", async () => {
  ...
  const query = { url: "http://chipmate:secret@127.0.0.1:4097", dir: "/tmp/project", scope: "project" as const }
...
test("viewed snapshots post the presence payload against the selected directory", async () => {
  const calls = setup()
  const client = await import("./client")
  const query = { url: "http://chipmate:secret@127.0.0.1:4097", dir: "/tmp/project" }
