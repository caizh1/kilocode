...
const auth = Effect.acquireRelease(
  Effect.sync(() => {
    const value = process.env.CHIPMATE_AUTH_CONTENT
    delete process.env.CHIPMATE_AUTH_CONTENT
    return value
  }),
  (value) =>
    Effect.sync(() => {
      if (value === undefined) delete process.env.CHIPMATE_AUTH_CONTENT
      else process.env.CHIPMATE_AUTH_CONTENT = value
    }),
)
            ...
            Effect.gen(function* () {
              ...
              const result = yield* Effect.gen(function* () {
                ...
                return {
                  ...
                  list: yield* credentials.list(Integration.ID.make("chipmate")),
                }
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        auth.pipe(
          Effect.flatMap(() =>
            Effect.gen(function* () {
              const integration = Integration.ID.make("chipmate")
              ...
              yield* Effect.gen(function* () {
                ...
                yield* db.insert(DataMigrationTable).values({
                  name: "credential.chipmate-account-json-v2",
              ...
              const repaired = yield* Effect.gen(function* () {
                ...
                return yield* db
                  ...
                  .where(eq(DataMigrationTable.name, "credential.chipmate-account-json-v3"))
                  .get()
              }).pipe(Effect.provide(database))
