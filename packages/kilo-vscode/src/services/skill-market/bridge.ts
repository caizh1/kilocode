...
export class SkillMarketBridge {
  ...
  private event(event: SSEPayload, directory?: string) {
    if (event.type === "kilocode.skill_market.requested") {
      if (!directory) return
      this.start((event as EventKilocodeSkillMarketRequested).properties, directory)
      return
    }
    if (event.type !== "kilocode.skill_market.cancelled") return
    const id = (event as EventKilocodeSkillMarketCancelled).properties.requestID
  ...
  private async reply(requestID: string, directory: string, result: SkillMarketResult) {
    const response = await this.connection.getClient().kilocode.skillMarket.reply({ requestID, directory, result })
    if (response.error) console.error(`[Kilo New] SkillMarketBridge: reply ${requestID} failed:`, response.error)
  }

  private async reject(requestID: string, directory: string, error: SkillMarketFailure) {
    const response = await this.connection.getClient().kilocode.skillMarket.reject({ requestID, directory, error })
    if (response.error) console.error(`[Kilo New] SkillMarketBridge: rejection ${requestID} failed:`, response.error)
  }

  private async recover(revision: number) {
    const client = this.connection.getClient()
    for (const directory of this.connection.getKnownDirectories()) {
      const response = await client.kilocode.skillMarket.list({ directory }).catch(() => undefined)
...
export function createSkillMarketBridge(connection: KiloConnectionService, context: ExtensionContext) {
