/**
 * ChipMate-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@chipmate/sdk/v2"
import { ChipMateAutoMethod } from "@/chipmate/components/dialog-chipmate-auto-method"
export { selectProvider } from "@/chipmate/anaconda-desktop/tui/setup"
...
/**
 * If the provider is ChipMate Gateway, renders the custom `ChipMateAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "chipmate") return undefined
  return () => (
    <ChipMateAutoMethod
 ...
 * provider is ChipMate Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  ...
  if (providerID !== "chipmate") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
        ChipMate Gateway gives you access to all the best coding models at the cheapest prices with a single API key.
      </text>
      <text fg={theme.text}>
        Go to <span style={{ fg: theme.primary }}>https://chipmate.ai/gateway</span> to get a key
      </text>
    </box>
  )
}
