...
import { createAutoApproveBridge } from "../../src/chipmate-provider/auto-approve"
import type { Event, ChipMateClient } from "@chipmate/sdk/v2/client"
...
function config(initial: boolean, info: Record<string, unknown> = {}) {
  ...
  return {
    ...
    emit(key = "chipmate.v2.autoApprove.enabled") {
...
function connection(client: ChipMateClient | null, dirs = new Map<string, string>()) {
  const svc = {
    getClient: () => {
...
describe("registerToggleAutoApprove", () => {
  ...
  it("uses the SSE directory for worktree permissions before session mappings are available", async () => {
    ...
    await ctrl.approve(asked("perm_worktree", "ses_worktree"), "/workspace/.chipmate/worktrees/feature")
    await ctrl.approve(asked("perm_child", "ses_child"), "/workspace/.chipmate/worktrees/feature")

    expect(replies).toEqual([
      { requestID: "perm_worktree", directory: "/workspace/.chipmate/worktrees/feature", reply: "once" },
      { requestID: "perm_child", directory: "/workspace/.chipmate/worktrees/feature", reply: "once" },
    ])
  })
  ...
  it("uses the shared permission directory before falling back to session mappings", async () => {
    ...
    const conn = connection(
      ...
      new Map([["perm_shared", "/workspace/.chipmate/worktrees/shared"]]),
    )
    const ctrl = registerToggleAutoApprove(
      context(),
      conn.svc,
    ...
    expect(replies).toEqual([
      { requestID: "perm_shared", directory: "/workspace/.chipmate/worktrees/shared", reply: "once" },
    ])
  })
