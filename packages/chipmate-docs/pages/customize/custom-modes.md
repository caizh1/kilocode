---
title: "Custom Modes"
description: "Create and configure custom modes in ChipMate"
---

# Custom Modes

ChipMate allows you to create **custom modes** (also called **agents**) to tailor ChipMate's behavior to specific tasks or workflows. Custom modes can be **global** (available across all projects), **project-specific** (defined within a single project), or **organization-managed** (provided by your ChipMate organization).

{% callout type="info" %}
The current VS Code extension (built on the ChipMate CLI) uses **agent Markdown files** to define custom modes. The legacy extension used `custom_modes.yaml` / `.chipmatemodes`. See the tabs below for the relevant approach.
{% /callout %}
...
If your ChipMate organization provides custom modes, ChipMate adds them to your local experience as organization-sourced agents/custom modes. They appear alongside built-in and personal agents so members can select them directly where ChipMate shows user-selectable agents or modes.
...
### 1. Ask ChipMate! (Recommended)

Ask ChipMate to create an agent for you:

```
Create a new agent called "docs-writer" that can only read files and edit Markdown files.
```

ChipMate will generate the agent definition and write it to `.chipmate/agent/` in your project.
...
```
.chipmate/agents/my-agent.md
.chipmate/agent/my-agent.md
.chipmate/agents/my-agent.md
```
...
```
~/.config/chipmate/agent/my-agent.md
```
...
**Example agent file** (`.chipmate/agents/docs-writer.md`):
...
### 4. Config File (`chipmate.jsonc`)

Define agents under the `agent` key in your project's `chipmate.jsonc`:
...
2. Global config (`~/.config/chipmate/chipmate.jsonc`)
3. Project config (`chipmate.jsonc` at project root)
4. `.chipmate/` / legacy `.chipmate/` directory configs and agent `.md` files
5. Environment variable overrides (`CHIPMATE_CONFIG_CONTENT`)
...
```jsonc
// chipmate.jsonc — override the built-in "code" agent
{
  "agent": {
    "code": {
      "model": "openai/gpt-4o",
...
Or as a `.md` file (`.chipmate/agents/code.md`):
...
If you have existing `.chipmatemodes` or `custom_modes.yaml` files from the VSCode extension, the extension automatically migrates them on startup. The migration converts:
...
| OS | Path |
|---|---|
| macOS | `~/Library/Application Support/Code/User/globalStorage/chipmate.chipmate-code/settings/custom_modes.yaml` |
| Linux | `~/.config/Code/User/globalStorage/chipmate.chipmate-code/settings/custom_modes.yaml` |
| Windows | `%APPDATA%\Code\User\globalStorage\chipmate.chipmate-code\settings\custom_modes.yaml` |

Project-level `.chipmatemodes` and workspace-scoped files are handled by the CLI backend that the extension delegates to — see the [CLI tab](#cli) for the full load-order table. After the extension migrates on startup, the legacy file is no longer consulted; remove new modes through the extension UI instead of editing `custom_modes.yaml` directly.
...
**Legacy `custom_modes.yaml` is not loaded from `~/.config/chipmate/`.** If you're migrating from the previous VS Code extension, global custom modes are read from `~/.chipmate/cli/global/settings/custom_modes.yaml` (not from the CLI's XDG config directory). The recommended approach is to convert legacy modes to agent `.md` files and place them in `~/.config/chipmate/agent/` instead — see [Markdown files](#3-markdown-files-with-yaml-frontmatter) and [Migration](#migration-from-vscode-extension-modes) below.
{% /callout %}
...
### 2. Using `chipmate agent create`

The CLI provides an interactive command:

```bash
chipmate agent create
```
...
The TUI also **remembers the last model you picked for each agent** across sessions. A config-pinned `model` acts as the default when no manual pick exists. To reset a pick and let the config take over, use the model picker (`Ctrl+X m`) and select a different model, or remove the saved pick from `~/.local/state/chipmate/model.json`.
...
If you have existing `.chipmatemodes` or `custom_modes.yaml` files from the VSCode extension, the CLI automatically migrates them on startup. The migration converts:
...
| 2 | `~/.chipmate/cli/global/settings/custom_modes.yaml` | YAML | Global |
| 3 | `~/.chipmatemodes` | YAML | Global |
| 4 | `<project>/.chipmatemodes` | YAML | Project (wins on conflict) |

{% callout type="info" %}
`~/.config/chipmate/` is the XDG config directory for the new agent format — legacy `custom_modes.yaml` placed there will **not** be loaded. Use `~/.config/chipmate/agent/*.md` or `~/.config/chipmate/chipmate.jsonc` for new agent definitions instead.
{% /callout %}
...
{% callout type="tip" %}
Ask ChipMate to create or validate permission glob patterns when you need more complex file restrictions.
{% /callout %}
...
### Basic Documentation Writer (`.chipmate/agents/docs-writer.md`)
...
### Test Engineer (`.chipmate/agents/test-engineer.md`)
...
### Security Reviewer (`.chipmate/agents/security-review.md`)
...
### Config File Example (`chipmate.jsonc`)
...
- **Agent not appearing:** Ensure the `.md` file is in a recognized directory (`.chipmate/agents/`, `.chipmate/agent/`, `.chipmate/agents/`). Check that the `mode` property is `primary` or `all` if you expect it in the agent picker.
...
- **Legacy modes are auto-migrated:** If you have `.chipmatemodes` files, they'll be converted on startup — no manual migration needed
...
- **Keep prompts focused:** The markdown body is your system prompt — write it as if briefing a colleague
- **Use `mode: subagent`** for helper agents that shouldn't be directly selectable by users
- **Test with `chipmate agent create`** to see how the CLI generates agent definitions, then customize from there
- **Legacy modes are auto-migrated:** If you have `.chipmatemodes` files, they'll be converted on startup — no manual migration needed
...
Ready to explore more? Check out the [Show and Tell](https://github.com/ChipMate-Org/chipmate/discussions/categories/show-and-tell) to discover and share custom modes and agents created by the community!
