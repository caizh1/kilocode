...
import { Capture } from "@/chipmate/session-export/capture"
import { resetEligibility } from "@/chipmate/session-export/eligibility"
...
describe("Capture", () => {
  ...
  test("free org requests do not start session export", async () => {
    const cap = new Capture({ worker, agentVersion: "v0", nowMs: () => 100, syncSeq: () => 7 })
    cap.beforeRequest({
      input: { model: { api: { npm: "@chipmate/chipmate-gateway" }, isFree: true }, org: { type: "org", id: "org_1" } },
      requestMeta: meta("s1"),
      assembled: { system: [], messages: [], tools: {}, permissions: [], params: {} },
    })
    cap.afterRequest({
  ...
  test("title agent requests do not start session export", () => {
    const cap = new Capture({ worker, agentVersion: "v0", nowMs: () => 100, syncSeq: () => 7 })
    cap.beforeRequest({
      input: { model: { api: { npm: "@chipmate/chipmate-gateway" }, isFree: true }, org: { type: "personal" } },
  ...
  test("eligible input posts llm_request_started with full envelope", () => {
    ...
    cap.beforeRequest({
      input: {
        model: { api: { npm: "@chipmate/chipmate-gateway" }, isFree: true, providerID: "chipmate", id: "free-1" },
        org: { type: "personal" },
      },
      requestMeta: meta("s1"),
    ...
    expect(msg.envelope.model.providerId).toBe("chipmate")
  ...
  test("dispatch projects non-cloneable tool functions out of envelopes", () => {
    ...
    cap.beforeRequest({
      input: {
        model: { api: { npm: "@chipmate/chipmate-gateway" }, isFree: true, providerId: "chipmate", modelId: "free-1" },
        org: { type: "personal" },
      },
      requestMeta: meta("s1"),
      assembled: {
  ...
  return [
    {
      info: {
        ...
        time: { created: 0 },
        agent: "build",
        model: { providerID: ProviderV2.ID.make("chipmate"), modelID: ModelV2.ID.make("free-1") },
      },
      parts: [],
    },
  ]
