...
} from "../../src/chipmate/documents/word"
...
describe("chipmate Word documents", () => {
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.promise(async () => {
            ...
            expect(created.artifactDir).toMatch(/^\.chipmate(?:code)?\/artifacts\//)
      ...
      provideTmpdirInstance(
        (dir) =>
          Effect.promise(async () => {
            ...
            const previousEndpoint = process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]
            const previousSoffice = process.env["CHIPMATE_WORD_RENDER_SOFFICE"]
            const previousPdftoppm = process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"]
            const previousPdftotext = process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"]
            const previousPdfinfo = process.env["CHIPMATE_WORD_RENDER_PDFINFO"]
            try {
              process.env["PATH"] = ""
              delete process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]
              delete process.env["CHIPMATE_WORD_RENDER_SOFFICE"]
              delete process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"]
              delete process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"]
              delete process.env["CHIPMATE_WORD_RENDER_PDFINFO"]
              ...
              if (previousSoffice === undefined) delete process.env["CHIPMATE_WORD_RENDER_SOFFICE"]
              else process.env["CHIPMATE_WORD_RENDER_SOFFICE"] = previousSoffice
              if (previousPdftoppm === undefined) delete process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"]
              else process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"] = previousPdftoppm
              if (previousPdftotext === undefined) delete process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"]
              else process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"] = previousPdftotext
              if (previousPdfinfo === undefined) delete process.env["CHIPMATE_WORD_RENDER_PDFINFO"]
              else process.env["CHIPMATE_WORD_RENDER_PDFINFO"] = previousPdfinfo
            }
            ...
            try {
              process.env["CHIPMATE_WORD_RENDER_SOFFICE"] = fakeSoffice
              process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"] = fakePdftoppm
              process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"] = fakePdftotext
              process.env["CHIPMATE_WORD_RENDER_PDFINFO"] = fakePdfinfo
              ...
              if (previousEndpoint === undefined) delete process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]
              else process.env["CHIPMATE_WORD_RENDER_ENDPOINT"] = previousEndpoint
              if (previousSoffice === undefined) delete process.env["CHIPMATE_WORD_RENDER_SOFFICE"]
              else process.env["CHIPMATE_WORD_RENDER_SOFFICE"] = previousSoffice
              if (previousPdftoppm === undefined) delete process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"]
              else process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"] = previousPdftoppm
            ...
            const tocEntry = '<w:p><w:pPr><w:pStyle w:val="TOC1"/></w:pPr><w:hyperlink w:anchor="_ChipMateToc1" w:history="1"><w:r><w:t xml:space="preserve">Render</w:t></w:r></w:hyperlink><w:r><w:tab/></w:r><w:fldSimple w:instr=" PAGEREF _ChipMateToc1 \\h "><w:r><w:t>1</w:t></w:r></w:fldSimple></w:p>'
