...
class ChipMateBackendHttpClientsTest {
    ...
    fun `api client sends correct basic auth header`() {
        ...
        val client = ChipMateBackendHttpClients.api(pwd)
        try {
            ...
            val expected = "Basic ${Base64.getEncoder().encodeToString("chipmate:$pwd".toByteArray())}"
            assertEquals(expected, recorded.getHeader("Authorization"))
        } finally {
            ChipMateBackendHttpClients.shutdown(client)
            server.shutdown()
        }
    }
    ...
    fun `api client has no call or read timeout`() {
        val client = ChipMateBackendHttpClients.api("test")
        try {
    ...
    fun `health client has short timeout`() {
        val client = ChipMateBackendHttpClients.health("test")
        try {
    ...
    fun `health client sends correct basic auth header`() {
        ...
        val client = ChipMateBackendHttpClients.health(pwd)
        try {
            val request = okhttp3.Request.Builder()
    ...
    fun `cli download client keeps release download timeouts`() {
        val client = ChipMateBackendHttpClients.cliDownload()
        try {
    ...
    fun `model fetch client has bounded 15 second timeouts`() {
        val client = ChipMateBackendHttpClients.modelFetch()
        try {
    ...
    fun `bounded client applies per request timeout and preserves auth`() {
        ...
        val client = ChipMateBackendHttpClients.api(pwd)
        val bounded = ChipMateBackendHttpClients.bounded(client, 7)
        try {
