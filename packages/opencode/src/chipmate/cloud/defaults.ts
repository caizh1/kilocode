...
export namespace CloudDefaults {
  ...
  export type ModelStateInfo = ChipMateModelState.State

  export interface ModelStateInterface {
    readonly get: () => Effect.Effect<ModelStateInfo>
  }

  export class ModelState extends Context.Service<ModelState, ModelStateInterface>()("@chipmate/CloudModelState") {}

  export const modelStateLayer = Layer.succeed(
    ModelState,
    ModelState.of({ get: () => Effect.promise(() => ChipMateModelState.get()) }),
  )
  ...
  function normalize(value: string) {
    return value.startsWith("chipmate/") ? value.slice("chipmate/".length) : value
  }
  ...
  export const resolve = Effect.fn("CloudDefaults.resolve")(function* (input: Input = {}) {
    ...
    return yield* Effect.fail(
      new ResolutionError({
        kind: "model",
        message: "The ChipMate model catalog has no available default model",
      }),
    )
  })
}
