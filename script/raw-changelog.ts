...
const repo = process.env.GH_REPO ?? "ChipMate-Org/chipmate"
...
const order = ["Core", "TUI", "SDK", "Extensions"] as const // chipmate_change
const sections = {
  core: "Core",
  tui: "TUI",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "sdk", "plugin", "extensions/vscode", "github"] // chipmate_change
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/sdk packages/plugin packages/extensions github`.text() // chipmate_change
