...
import ai.chipmate.client.session.model.AgentItem
import ai.chipmate.client.session.model.ModelLimitItem
import ai.chipmate.client.session.model.ModelItem
import ai.chipmate.client.session.model.SessionModel
import ai.chipmate.client.session.model.SessionModelEvent
import ai.chipmate.client.session.model.SessionState
import ai.chipmate.client.session.model.Permission
import ai.chipmate.client.session.model.PermissionFileDiff
import ai.chipmate.client.session.model.PermissionMeta
import ai.chipmate.client.session.model.PermissionRuleCandidate
import ai.chipmate.client.session.model.PermissionRuleDecision
import ai.chipmate.client.session.model.PermissionRequestState
import ai.chipmate.client.session.model.Question
import ai.chipmate.client.session.model.QuestionItem
import ai.chipmate.client.session.model.QuestionOption
import ai.chipmate.client.session.model.Reasoning
import ai.chipmate.client.session.model.ToolCallRef
import ai.chipmate.client.session.model.Text
import ai.chipmate.client.plugin.ChipMatePluginSettings
import ai.chipmate.client.session.SessionRef
import ai.chipmate.client.telemetry.Telemetry
import ai.chipmate.client.util.UiTimer
...
import ai.chipmate.rpc.dto.PermissionRequestDto
import ai.chipmate.rpc.dto.PermissionRuleDecisionDto
...
import ai.chipmate.rpc.dto.QuestionRequestDto
...
import ai.chipmate.log.ChatLogSummary
...
class SessionController(
  parent: Disposable,
  ref: SessionRef? = null,
  private val sessions: ChipMateSessionService,
  private val workspace: Workspace,
  private val app: ChipMateAppService,
  ...
  private val log: ChipMateLog = LOG,
) : Disposable {
    ...
    companion object {
        private val LOG = ChipMateLog.create(SessionController::class.java)
    ...
    val autoApprove: Boolean get() = ChipMatePluginSettings.getAutoApprove()
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    val msg = e.message ?: ChipMateBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun setAutoApprove(value: Boolean) {
        assertEdt()
        ChipMatePluginSettings.setAutoApprove(value)
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ChipMateBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun revert(message: String, part: String? = null) {
        ...
        val op = beginReverting(
            ChipMateBundle.message("session.status.rollingback"),
    ...
    fun unrevert() {
        ...
        if (revertOp != null) return
        val op = beginReverting(
            ChipMateBundle.message("session.status.redoing"),
    ...
    fun cancelRevert() {
        ...
        if (state is SessionState.Reverting) {
            model.setState(state.copy(text = ChipMateBundle.message("session.status.operation.finishing")))
        }
        revertJob?.cancel()
    }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != ChipMateAppStatusDto.READY || model.app.status == ChipMateAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == ChipMateWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
            ...
            try {
                ...
                edt {
                    updatePermission(
                        ...
                        e.message ?: ChipMateBundle.message("session.permission.error"),
                    )
                }
            }
        }
    ...
    private fun approve(id: String, restore: () -> Permission) {
        ...
        updateModel { model.setState(SessionState.Busy(ChipMateBundle.message("session.status.considering"))) }
        cs.launch {
            try {
                ...
                edt {
                    ...
                    show(restore().copy(
                        ...
                        message = e.message ?: ChipMateBundle.message("session.permission.error"),
                    ))
                }
            }
        }
            ...
            try {
                ...
                runEdt {
                    ...
                    if (current is SessionState.AwaitingPermission &&
                        ...
                        model.setState(SessionState.Busy(ChipMateBundle.message("session.status.considering")))
                    }
                }
            } catch (e: Exception) {
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == ChipMateAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != ChipMateWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == CHIPMATE_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == ChipMateWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ChipMateBundle.message("history.error.local")))
                    }
                    showSession()
                    loaded(false)
                }
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: ChipMateBundle.message("history.error.cloud")))
                    }
                    showSession()
                    loaded(false)
                }
     ...
     * [ai.chipmate.client.session.ui.RevertBanner] falls back to `model.diff` when the CLI does not
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(ChipMateBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                ...
                tool = null
                if (revertOp != null) {
                    revertDeferred = SessionState.Busy(ChipMateBundle.message("session.status.considering"))
                    return
                }
                model.setState(SessionState.Busy(ChipMateBundle.message("session.status.considering")))
            }
    ...
    private fun error(event: ChatEventDto.Error, reveal: Boolean) {
        ...
        if (isPaidModelAuthRequired(event.error)) {
            ...
            model.setState(SessionState.LoginRequired(ChipMateBundle.message("session.login.required.description")))
            return
        }
        val msg = event.error?.message ?: event.error?.type ?: ChipMateBundle.message("session.error.unknown")
    ...
    private fun afterResolve(idle: Boolean = false): SessionState {
        return pending.values.firstOrNull()?.let { SessionState.AwaitingPermission(it) }
            ?: if (idle) SessionState.Idle else SessionState.Busy(ChipMateBundle.message("session.status.considering"))
    }
        ...
        val state = when (dto.type) {
            ...
            "busy" -> {
                ...
                if (current is SessionState.Idle || current is SessionState.Error)
                    SessionState.Busy(ChipMateBundle.message("session.status.considering"))
    ...
    private fun onRevertTimeout(op: RevertOp) {
        ...
        revertJob?.cancel()
        failReverting(op, RuntimeException(ChipMateBundle.message("session.error.revert.timeout")))
    }
    ...
    private fun failReverting(op: RevertOp, e: Exception) {
        ...
        revertDeferred = null
        model.setState(SessionState.Error(e.message ?: ChipMateBundle.message("session.error.unknown")))
    }
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != ChipMateAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> ChipMateBundle.message("session.status.thinking")
        "text" -> ChipMateBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> ChipMateBundle.message("session.status.delegating")
            "todowrite", "todoread" -> ChipMateBundle.message("session.status.planning")
            "read" -> ChipMateBundle.message("session.status.gathering")
            "glob", "grep", "list" -> ChipMateBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> ChipMateBundle.message("session.status.searching.web")
            "edit", "write" -> ChipMateBundle.message("session.status.editing")
            "bash" -> ChipMateBundle.message("session.status.commands")
            else -> ChipMateBundle.message("session.status.considering")
        }
        else -> ChipMateBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == ChipMateAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                ChipMateBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (app.status == ChipMateAppStatusDto.DOWNLOADING) {
        ...
        if (workspace.status == ChipMateWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                ChipMateBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (app.status == ChipMateAppStatusDto.READY && workspace.status == ChipMateWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == ChipMateAppStatusDto.READY && workspace.status == ChipMateWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = ChipMateBundle.message("session.connection.warning.config")
...
private const val CHIPMATE_PROVIDER = "chipmate"
private const val CHIPMATE_AUTO_MODEL = "chipmate-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(CHIPMATE_PROVIDER, CHIPMATE_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != CHIPMATE_PROVIDER && item.providerID !in providers.connected) return null
