/**
 * Authentication handlers — extracted from ChipMateProvider.
 *
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[ChipMate New] ChipMateProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "chipmate", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[ChipMate New] ChipMateProvider: 🔐 Got auth URL:", auth.url)
    ...
    await ctx.client.provider.oauth.callback({ providerID: "chipmate", method: 0, directory: dir }, { throwOnError: true })

    // Check if this attempt was cancelled
    if (attempt !== getAttempt()) return

    console.log("[ChipMate New] ChipMateProvider: 🔐 Login successful")

    await ctx.disposeGlobal()

    // Step 3: Fetch profile and push to webview
    const { data: profile } = await ctx.client.chipmate.profile(undefined, { throwOnError: true })
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[ChipMate New] ChipMateProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "chipmate" }, { throwOnError: true })
    console.log("[ChipMate New] ChipMateProvider: 🚪 Logged out successfully")
    ...
    console.error("[ChipMate New] ChipMateProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[ChipMate New] ChipMateProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.chipmate.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[ChipMate New] ChipMateProvider: Failed to switch organization:", error)
    // Re-fetch current profile to reset webview state — best-effort
    try {
      const result = await ctx.client.chipmate.profile()
      ctx.postMessage({ type: "profileData", data: result.data ?? null })
    } catch (profileError) {
      console.error("[ChipMate New] ChipMateProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to refresh agents after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[ChipMate New] ChipMateProvider: 🔄 Refreshing profile...")
  const result = await ctx.client.chipmate.profile().catch(() => ({ data: null }))
  ctx.postMessage({ type: "profileData", data: result.data ?? null })
}
