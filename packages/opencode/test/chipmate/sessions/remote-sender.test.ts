...
import { RemoteCommand } from "../../../src/chipmate-sessions/remote-command"
import { RemoteModelCatalog } from "../../../src/chipmate-sessions/remote-model-catalog"
import { RemoteSender } from "../../../src/chipmate-sessions/remote-sender"
import type { RemoteWS } from "../../../src/chipmate-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/chipmate-sessions/remote-protocol"
...
describe("RemoteSender", () => {
  ...
  test("send_message keeps chipmate-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_chipmate",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "chipmate/gpt-5-mini",
      },
    })
    ...
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("chipmate"), modelID: ModelV2.ID.make("gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
  ...
  test("send_message does not special-case chipmate-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    expect(sent[0]).toEqual({ type: "response", id: "req_model_chipmate", result: {} })
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("chipmate"), modelID: ModelV2.ID.make("chipmate/gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
  ...
  test("subscribe does not replay state for sessions with no pending questions or permissions", async () => {
    ...
    spyOn(ChipMateSessionPromptQueue, "snapshot").mockReturnValue([])
  ...
  test("subscribe always replays the current queue snapshot, including empty", async () => {
    ...
    expect(ChipMateSessionPromptQueue.snapshot).toHaveBeenCalledWith(SessionID.make("ses_target"))
  })
  ...
  test("subscribe replays a non-empty queue snapshot for the subscribed session", async () => {
    ...
    spyOn(Suggestion, "list").mockResolvedValue([])
    spyOn(ChipMateSessionPromptQueue, "snapshot").mockReturnValue(["msg_a", "msg_b"] as any)
...
// chipmate_change start - remote slash command discovery and execution
describe("RemoteSender slash commands", () => {
  ...
  test("exit_cli rejects invalid, missing, unresolved, and unavailable sessions before ACK", async () => {
    ...
    // chipmate_change - K1 W1: the new exit_cli handler requires hasSession
  ...
  test("exit_cli ACKs before invoking the worker callback in a microtask", async () => {
    ...
    try {
      const sender = RemoteSender.create({
        ...
        // chipmate_change - K1 W1: owns the target so the new detach path
  ...
  test("concurrent exit_cli requests ACK while idempotent local Exit cleans up once", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // chipmate_change - K1 W1: owns both targets; zero remaining after
  ...
  test("exit_cli logs only the error class when graceful exit fails after ACK", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // chipmate_change - K1 W1: owns the target; zero remaining; the
  ...
  test("create_session in-process attaches the new session via the attach seam", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // responsible for the on-boot attach via the CHIPMATE_REMOTE_ATTACH_SESSION
      // init branch in chipmate-sessions.ts.
      attachSession: async () => undefined,
    })
  ...
  test("exit_cli keeps the headless host alive when zero sessions remain and no callback is registered (chipmate remote)", async () => {
    const { conn, sent } = fakeConn()
    const order: string[] = []
    const sender = RemoteSender.create({
      conn,
  ...
  test("create_session forwards agent, model (with variant), and orgId metadata", async () => {
    ...
    sender.handle({
      ...
      data: {
        protocolVersion: 1,
        agent: "build",
        model: { providerID: "chipmate", modelID: "chipmate-auto/efficient", variant: "high" },
        orgId: org,
      },
    })
    await response.promise
    ...
    expect(createCalls).toEqual([
      {
        agent: "build",
        model: {
          id: ModelV2.ID.make("chipmate-auto/efficient"),
          providerID: ProviderV2.ID.make("chipmate"),
          variant: "high",
        },
        metadata: { orgId: org },
  ...
  test("create_session accepts each optional field alone", async () => {
    ...
    for (const [id, data] of [
      ...
      ["req_model", { protocolVersion: 1, model: { providerID: "chipmate", modelID: "m1" } }],
    ...
    expect(createCalls).toEqual([
      { agent: "plan" },
      { model: { id: ModelV2.ID.make("m1"), providerID: ProviderV2.ID.make("chipmate") } },
      { metadata: { orgId: org } },
    ])
  })
  ...
  test("system session.renamed applies setTitle and marks adoption", async () => {
    ...
    const { clear, consumeRenameAdoption } = await import("../../../src/chipmate-sessions/rename-adoptions")
    clear(sid)
  ...
  test("system session.renamed clears adoption mark when setTitle fails", async () => {
    ...
    const { clear, consumeRenameAdoption, markRenameAdopted } = await import(
      "../../../src/chipmate-sessions/rename-adoptions"
    )
    clear(sid)
