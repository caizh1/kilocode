...
import * as ChipMateGrep from "./chipmate/ripgrep-grep" // chipmate_change
import * as SpawnExit from "./chipmate/spawn-exit" // chipmate_change
import * as SpawnValidation from "./chipmate/spawn-validation" // chipmate_change
...
const RawMatch = Schema.Struct({
  type: Schema.Literals(["match", "context"]), // chipmate_change - retain requested context records
  data: Schema.Struct({
...
type RawMatchData = (typeof RawMatch.Type)["data"] & { readonly context: boolean } // chipmate_change
...
export interface GlobInput {
  ...
  readonly follow?: boolean
  readonly signal?: AbortSignal
  readonly validate?: Effect.Effect<void, unknown> // chipmate_change - bind approved searches at spawn
}

export interface GrepInput extends ChipMateGrep.Options {
  // chipmate_change
  readonly cwd: string
  readonly pattern: string
...
export interface Interface {
  ...
  readonly glob: (input: GlobInput) => Effect.Effect<SearchResult<Entry>, Error> // chipmate_change
  readonly grep: (input: GrepInput) => Effect.Effect<SearchResult<ChipMateGrep.GrepMatch>, Error | InvalidPatternError> // chipmate_change
}

// chipmate_change start - retain truncation state through model-facing tools
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = <A>(input: {
      ...
      readonly stop?: (item: A) => boolean // chipmate_change - stop bounded searches at the overflow match
      readonly validate?: Effect.Effect<void, unknown> // chipmate_change - spawn-bound target validation
    }) => {
      const program = Effect.scoped(
        Effect.gen(function* () {
          const filepath = yield* binary.filepath
          // chipmate_change start - validate approved targets after all spawn preparation
          const command = ChildProcess.make(filepath, input.args, {
            cwd: input.cwd,
            extendEnv: true,
            stdin: "ignore",
            forceKillAfter: input.stop ? Duration.seconds(1) : undefined, // chipmate_change - bound grep interruption
          })
          const validated = input.validate ? SpawnValidation.attach(command, input.validate) : command
          const spawned = input.stop ? SpawnExit.attach(validated) : validated // chipmate_change
          ...
          let observed = 0
          let stopped = false // chipmate_change
          const take = input.stop // chipmate_change start
            ? Stream.takeUntil<A>((row) => {
                stopped = input.stop?.(row) ?? false
                return stopped
              })
            : Stream.take(input.limit + 1) // chipmate_change end
          const rows = yield* Stream.decodeText(handle.stdout).pipe(
            ...
            take, // chipmate_change
            Stream.runCollect,
            Effect.map((chunk) => [...chunk]),
          )
          if (stopped) return { items: rows, truncated: true, partial: false } // chipmate_change
          const truncated = input.stop ? false : rows.length > input.limit // chipmate_change - custom stop owns truncation
      ...
      return abortable.pipe(
        // chipmate_change start - surface the underlying reason instead of a bare wrapper message
    ...
    return Service.of({
      glob: (input) =>
        run<string>({
          ...
          limit: input.limit,
          signal: input.signal,
          validate: input.validate, // chipmate_change - preserve spawn-bound target validation
          args: [
            "--no-config",
            "--files",
            ...(input.hidden ? ["--hidden"] : []),
          ...
          // chipmate_change start - retain spawn metadata after mapping paths
          Effect.map((result) => ({
            ...result,
      ...
      grep: (input) =>
        run<RawMatchData>({
          ...input,
          stop: ChipMateGrep.stop(input.limit), // chipmate_change
          args: [
            "--no-config",
            "--json",
            "--hidden",
            "--no-messages",
            ...ChipMateGrep.flags(input), // chipmate_change
            ...
            (Buffer.byteLength(line, "utf8") > MAX_RECORD_BYTES
              ...
              Effect.flatMap((json) => {
                if (
                  ...
                  typeof json !== "object" ||
                  !("type" in json) ||
                  (json.type !== "match" && json.type !== "context") // chipmate_change
                )
                ...
                return Schema.decodeUnknownEffect(RawMatch)(json).pipe(
                  Effect.map((match) => ({
                    ...
                    context: match.type === "context", // chipmate_change
                  })),
          ...
          // chipmate_change start - retain spawn metadata after mapping matches
          Effect.map((result) => ({
            ...result,
            items: ChipMateGrep.select(input, result.items).map((match) => {
              ...
              return ChipMateGrep.decorate(item, match.context, match.lines.text.length > 2_000)
            }),
          })),
          // chipmate_change end
        ),
