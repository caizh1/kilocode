...
export async function migrate(
  ...
  try {
    ...
    const project = await client.chipmate.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.chipmate.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.chipmate.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.chipmate.sessionImport.part(part, { throwOnError: true })
    }
