...
for ($index = 1; $index -le $Runs; $index++) {
  ...
  $env:CHIPMATE_DISABLE_CHANNEL_DB = "true"
  $env:CHIPMATE_PRODUCT_PROFILE = "chipmate-v2"
  $env:CHIPMATE_STORAGE_ROOT = $storage
  $env:CHIPMATE_VSCODE_GLOBAL_STORAGE = $storage
  $env:CHIPMATE_SERVER_PASSWORD = "windows-qa"
  $env:CHIPMATE_PARENT_PID = [string]$PID
  $env:CHIPMATE_CLIENT = "vscode"
  $env:CHIPMATE_APP_NAME = "chipmate"
  $env:CHIPMATE_DISABLE_MODELS_FETCH = "1"
  $env:CHIPMATE_TELEMETRY_LEVEL = "off"
