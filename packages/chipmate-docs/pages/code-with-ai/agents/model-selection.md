...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models ChipMate users are actually having success with right now.

## Check the Live Models List

**[👉 See what's working today at chipmate.ai/models](https://chipmate.ai/models)**
...
- For non-interactive use, pass `--model` flag to `chipmate run` (e.g., `chipmate run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `chipmate.jsonc`, or configure per-agent models in the `agent` section.
...
You don't need a paid API key to use ChipMate productively. For the lowest cost on paid work, [Auto Efficient](/docs/code-with-ai/agents/auto-model#tiers) (`chipmate-auto/efficient`) routes each request to the cheapest model proven accurate enough for that task. The fastest way to start for free is [Auto Model Free](/docs/code-with-ai/agents/auto-model) (`chipmate-auto/free`), which routes to the best available free models automatically. See [Using ChipMate for Free](/docs/getting-started/using-chipmate-for-free) for the full zero-cost setup.

If you prefer to pick models yourself, type `free` in the model picker to filter by free models, or browse the full list at [chipmate.ai/models](https://chipmate.ai/models).

{% callout type="info" %}
Free model availability changes as providers adjust promotional periods. Check [chipmate.ai/models](https://chipmate.ai/models) for the live list.
{% /callout %}
...
- **Via config file** — edit `chipmate.jsonc`:
...
```
vscode://chipmate.chipmate-code/chipmate/switch?model=<modelID>
vscode://chipmate.chipmate-code/chipmate/switch?agent=<agentName>
vscode://chipmate.chipmate-code/chipmate/switch?model=<modelID>&agent=<agentName>
```

Replace `<modelID>` with a ChipMate Gateway model ID such as `chipmate-auto/free`. Replace `<agentName>` with a visible primary agent ID such as `code` or `plan`, rather than its display name.

### Example: Auto Free

To open ChipMate and switch to the [Auto Free](/docs/code-with-ai/agents/auto-model) tier (`chipmate-auto/free`), use:

```
vscode://chipmate.chipmate-code/chipmate/switch?model=chipmate-auto%2Ffree
```

To switch only to Plan and use its normal model selection, specify the agent without a model:

```
vscode://chipmate.chipmate-code/chipmate/switch?agent=plan
```

To select both at the same time, include both parameters:

```
vscode://chipmate.chipmate-code/chipmate/switch?model=chipmate-auto%2Ffree&agent=plan
```
...
- **VS Code open**: the ChipMate sidebar is focused and the linked selection is applied to the active session immediately.
- **VS Code closed**: VS Code launches, then applies the selection once the extension is ready.
- When `model` is provided, it must identify a model in the current ChipMate Gateway catalog. Invalid or unavailable models cause the deep link to be ignored.
...
```html
<a href="vscode://chipmate.chipmate-code/chipmate/switch?model=chipmate-auto%2Ffree&amp;agent=plan">
  Open ChipMate with Auto Free in Plan
</a>
```
...
The AI model space moves fast. Bookmark [chipmate.ai/models](https://chipmate.ai/models) and check back when you're evaluating options. What's best today might not be best next month — and that's actually exciting.
