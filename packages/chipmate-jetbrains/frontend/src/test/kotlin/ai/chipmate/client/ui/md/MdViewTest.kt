...
class MdViewTest : BasePlatformTestCase() {
    ...
    fun `test set renders prose file refs as underlined brown links`() {
        ...
        assertTrue(sheet.contains("a.chipmate-file-ref, code a.chipmate-file-ref { color: $color; font-family:"))
    ...
    fun `test inline code file refs keep code color and become file links`() {
        ...
        assertTrue(html.contains("<code style=\"color: $color\"><a class=\"chipmate-file-ref\" href=\"packages/opencode/src/session/prompt.ts\">packages/opencode/src/session/prompt.ts</a></code>"))
        assertFalse(html.contains("background:"))
    }

    fun `test file refs keep line suffix and trailing punctuation outside link`() {
        view.set("See chipmate/session/prompt.ts:302 and native-plan-prompt.txt:37-38.")
        val html = view.html()
    ...
    fun `test code font override appears in override sheet`() {
        ...
        assertTrue(sheet.contains("a.chipmate-file-ref, code a.chipmate-file-ref { color:"))
