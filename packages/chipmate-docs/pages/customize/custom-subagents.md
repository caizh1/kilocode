...
ChipMate includes two built-in subagents:
...
Add agents to the `agent` section of your `chipmate.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/chipmate/agents/`
- **Project-specific**: `.chipmate/agents/`

The **filename** (without `.md`) becomes the agent name.

If `.chipmate/agents/` is a symlink to a directory outside the project, allow that exact source in your global `~/.config/chipmate/chipmate.jsonc`:
...
2. **Global config** (`~/.config/chipmate/config.json`)
3. **Project config** (`chipmate.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/chipmate/agents/*.md`)
5. **Project agent markdown files** (`.chipmate/agents/*.md`)
