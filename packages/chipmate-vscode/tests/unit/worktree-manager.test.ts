...
async function createTempRepo(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-"))
  tempDirs.push(dir)
...
async function createTempRepoWithOrigin(): Promise<{ bare: string; clone: string }> {
  ...
  const seed = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-seed-"))
  tempDirs.push(seed)
  ...
  const bare = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-bare-"))
  tempDirs.push(bare)
  await simpleGit().clone(seed, bare, ["--bare"])

  const clone = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-clone-"))
  tempDirs.push(clone)
...
describe("WorktreeStateManager.updateWorktreeLabel", () => {
  it("persists label on a worktree", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-label-"))
    tempDirs.push(dir)
...
describe("WorktreeManager.createWorktree", () => {
  ...
  it("throws when workspace is not a git repo", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-wt-nogit-"))
    tempDirs.push(dir)
  ...
  it(
    ...
    async () => {
      ...
      while (Date.now() < deadline) {
        ...
        if (!entries.some((e) => e.startsWith(".chipmate-delete-"))) break
        await new Promise((r) => setTimeout(r, 100))
      }

      // No .chipmate-delete-* temp dirs should remain
      const entries = await fs.readdir(worktreesDir)
      const orphans = entries.filter((e) => e.startsWith(".chipmate-delete-"))
...
describe("WorktreeManager.discoverWorktrees orphan cleanup", () => {
  it("cleans up .chipmate-delete-* dirs left by interrupted deletions", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)
    ...
    const orphan = path.join(root, ".chipmate-v2", "worktrees", ".chipmate-delete-fake-uuid")
...
describe("WorktreeManager.ensureGitExclude", () => {
  ...
  it("does not add official ChipMate Agent Manager paths", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)
    ...
    const content = await fs.readFile(path.join(root, ".git", "info", "exclude"), "utf-8")
    expect(content).not.toContain(".chipmate/worktrees/")
    expect(content).not.toContain(".chipmate/worktrees/")
  })
...
describe("WorktreeManager.removeWorktree safety", () => {
  it("refuses to remove paths outside the worktrees directory", async () => {
    ...
    // Create a directory outside .chipmate/worktrees/
