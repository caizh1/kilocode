...
const configured = process.env.CHIPMATE_BWRAP_PATH

afterEach(() => {
  if (configured === undefined) {
    delete process.env.CHIPMATE_BWRAP_PATH
    return
  }
  process.env.CHIPMATE_BWRAP_PATH = configured
})

describe("local Bubblewrap helper", () => {
  it("copies the configured helper to a cache outside the extension", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-vscode-bwrap-"))
    try {
      const source = path.join(root, "source", "bwrap")
      const extension = path.join(root, "workspace", "packages", "chipmate-vscode")
      const cache = path.join(root, "cache")
      await fs.mkdir(path.dirname(source), { recursive: true })
      await fs.writeFile(source, "bubblewrap")
      process.env.CHIPMATE_BWRAP_PATH = source
      ...
      expect(resolveLocalBwrapEnv(extension, true, "linux-x64", cache)).toEqual({ CHIPMATE_BWRAP_PATH: dest })
  ...
  it("does not stage Bubblewrap for unsupported operating systems", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-vscode-bwrap-"))
    try {
      process.env.CHIPMATE_BWRAP_PATH = path.join(root, "missing")
