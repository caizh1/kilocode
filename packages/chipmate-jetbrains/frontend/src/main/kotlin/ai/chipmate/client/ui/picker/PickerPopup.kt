package ai.chipmate.client.ui.picker
...
internal class PickerPopup<T>(
    ...
    private val emptyText: String = ChipMateBundle.message("model.picker.no.matches"),
) {
    ...
    private val field = if (search) SearchTextField(false).apply {
        textEditor.emptyText.text = ChipMateBundle.message("model.picker.search")
    } else null
    ...
    private fun syncExpand() {
        ...
        expand.toolTipText = if (expanded) {
            ChipMateBundle.message("model.picker.details.minimize")
        } else {
            ChipMateBundle.message("model.picker.details.maximize")
        }
