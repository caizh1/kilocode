...
export namespace BackgroundProcess {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@chipmate/BackgroundProcess.State",
  ) {}
  ...
  async function claim(shared: Shared) {
    ...
    const pending = Flock.acquire(shared.key, {
      ...
      .catch((err) => {
        log.debug("persistent process scope is managed by another ChipMate process", { err, scope: shared.key })
        return false
      })
      .finally(() => {
  ...
  function infer() {
    return Flag.CHIPMATE_CLIENT === "cli" && process.env.CHIPMATE_BACKGROUND_PROCESS_PORTS === "true"
  }
  ...
  function env(id?: ID, token?: string) {
    const result: NodeJS.ProcessEnv = {
      ...userEnv(process.env),
      TERM: "dumb",
      ...(id ? { CHIPMATE_BACKGROUND_PROCESS_ID: id } : {}),
      ...(token ? { CHIPMATE_BACKGROUND_PROCESS_TOKEN: token } : {}),
    }
    delete result.CHIPMATE_SERVER_PASSWORD
    delete result.CHIPMATE_SERVER_USERNAME
    delete result.CHIPMATE_BACKGROUND_PROCESS_PORTS
    return result
  }
  ...
  async function linux(active: Active): Promise<Probe> {
    ...
    if (leader && pgrp(leader) === pid) {
      ...
      if (data?.toString("utf8").split("\0").includes(`CHIPMATE_BACKGROUND_PROCESS_TOKEN=${token}`)) return "owned"
    }
  ...
  async function launch(state: State, input: StartInput, id = ID.ascending()) {
    ...
    if (logpath) {
      ...
      if (!(await claim(state.shared)))
        throw new Error("Persistent processes for this project are managed by another ChipMate process")
  ...
  ChipMateShutdown.register(() => runtime.dispose())
