...
import { MaxCostNudge, type MaxCostChoice } from "@opencode-ai/core/chipmate/cost/max-cost-nudge"
import { type ChipMateConnectionService, ServerStartupError } from "./services/cli-backend"
...
import { saveImage } from "./chipmate-provider/save-image"
import { handleEditorAction } from "./chipmate-provider/editor-actions"
import { exportTranscript } from "./chipmate-provider/export-transcript"
import {
  TelemetryProxy,
...
import { createMarketplaceRemover, removeMcp } from "./chipmate-provider/remove-config-item"
import { AgentRequirementsController } from "./chipmate-provider/agent-requirements-controller"
...
import { slimInfo, slimPart, slimParts } from "./chipmate-provider/slim-metadata"
import { handleSidebarWorktreeMessage } from "./chipmate-provider/sidebar-worktree"
import { parseMessageFiles, type MessageFile } from "./chipmate-provider/message-files"
import { renameSession } from "./chipmate-provider/rename-session"
...
import { interceptMessage } from "./chipmate-provider/git-changes-request"
import { matchFollowup, recordFollowup, type Followup } from "./chipmate-provider/followup-session"
import { clearCommandsCache, loadCommands } from "./chipmate-provider/commands"
import { fetchMessagePage, MESSAGE_PAGE_LIMIT } from "./chipmate-provider/message-page"
import {
  dismissNotification,
...
} from "./chipmate-provider/notifications"
import { childID } from "./chipmate-provider/task-session"
import { VisibleTaskStreams } from "./chipmate-provider/visible-task-streams"
import { handleNetworkEvent, clearNetworkWaits } from "./chipmate-provider/network"
import { SessionAbort } from "./chipmate-provider/abort"
import {
...
import { routeEarlyMessage } from "./chipmate-provider/early-message"
import * as ModelState from "./chipmate-provider/model-state"
import { handleForkSession } from "./chipmate-provider/fork-session"
import { openConfig } from "./chipmate-provider/open-config"
...
} from "./chipmate-provider/work-style"
import * as McpOAuth from "./chipmate-provider/mcp-oauth"
...
import {
  ...
  type AuthContext,
} from "./chipmate-provider/handlers/auth"
import {
  handleRequestCloudSessions,
  handleRequestCloudSessionData,
  handleImportAndSend,
  type CloudSessionContext,
} from "./chipmate-provider/handlers/cloud-session"
import {
  handlePermissionResponse,
  fetchAndSendPendingPermissions,
  type PermissionContext,
} from "./chipmate-provider/handlers/permission-handler"
import {
  handleQuestionReply,
  handleQuestionReject,
  fetchAndSendPendingQuestions,
} from "./chipmate-provider/handlers/question"
import { fetchAndSendPendingSuggestions } from "./chipmate-provider/handlers/suggestion"
import { nativeTitle } from "./chipmate-provider/native-tab-title"
import { isInternalOfflineBuild } from "./shared/internal-offline"
import { handleInternalOfflineAuth } from "./chipmate-provider/internal-offline-auth"
...
import type { Agent } from "@chipmate/sdk/v2/client"
import { configFeatures } from "./features"
import { createAutoApproveBridge } from "./chipmate-provider/auto-approve"
import type { ChipMateProviderOptions } from "./chipmate-provider/options"
import { fetchChipMateEmbeddingModelCatalog } from "@chipmate/chipmate-gateway"
import { fetchImageModels } from "./image-generation/models"
import { stopSessionProcesses } from "./chipmate-provider/background-process"
...
import {
  ...
  watchIndexingConfig,
} from "./chipmate-provider/indexing-settings"
import { buildChatSettingsMessage, validChatSetting, watchChatConfig } from "./chipmate-provider/chat-settings"
import { buildThroughputSettingMessage, watchThroughputConfig } from "./chipmate-provider/throughput-settings"
...
function sandboxClient(client: ChipMateClient | null) {
...
export class ChipMateProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  ...
  private readonly indexing = {
    ...
    request: 0,
    accepted: 0,
    // Explicit Agent Manager routing must win over inferred memory/SSE directories.
    authoritative: new Set<string>(),
    local: new Set<string>(),
    dead: false,
  }
  /** Cached chipmateEmbeddingModelsLoaded payload so requestChipMateEmbeddingModels is resilient offline. */
  private cachedChipMateEmbeddingModelsMessage: unknown = null
  ...
  private pendingChipMateModel: { modelID?: string; agent?: string } | null = null
  ...
  private readonly memory = new ChipMateProviderMemory({
  ...
  private readonly marketplaceRemove: ReturnType<typeof createMarketplaceRemover>
  ...
  constructor(
    ...
    private readonly opts: ChipMateProviderOptions = {},
  ) {
  ...
  private checkpoint(sid: string, run: () => Promise<void>): void {
    ...
    void pending.then(cleanup, (error) => {
      console.error("[ChipMate New] checkpoint mutation failed:", error)
      cleanup()
    })
  }
  ...
  /**
   * Convenience getter that returns the shared SDK ChipMateClient or null if not yet connected.
   * Preserves the existing null-check pattern used throughout handler methods.
   */
  private get client(): ChipMateClient | null {
    try {
  ...
  private openMarketplacePanel(directory: unknown): void {
    if (typeof directory === "string" && directory) {
      vscode.commands.executeCommand("chipmate.v2.marketplaceButtonClicked", directory)
      return
    }
    vscode.commands.executeCommand("chipmate.v2.marketplaceButtonClicked", this.projectDirectory)
  }

  // Strip metadata unused by the webview to keep session switches fast.
  // Logic in chipmate-provider/slim-metadata.ts.
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[ChipMate New] ChipMateProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[ChipMate New] ChipMateProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    // Profile returns 401 when user isn't logged into ChipMate Gateway — that's expected.
    ...
    if (this.connectionState === "connected" && this.client) {
      ...
      if (!internal) {
        console.log("[ChipMate New] ChipMateProvider: 👤 syncWebviewState fetching profile...")
        const profileResult = await retry(() => this.client!.chipmate.profile())
        const profileData = profileResult.data ?? null
        console.log("[ChipMate New] ChipMateProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
        this.postMessage({
          type: "profileData",
          data: profileData,
        })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "chipmate.v2.sidebarVisible", visible)
  }

  /** Resolve a WebviewPanel for displaying ChipMate in an editor tab. */
  ...
  public setSessionDirectory(sessionId: string, directory: string): void {
    ...
    this.indexing.authoritative.add(sessionId)
  ...
  public markSessionLocal(sessionId: string): void {
    const dir = this.getIndexingDirectory()
    this.aborts.preserve(sessionId, this.sessionStatusMap.get(sessionId), this.getWorkspaceDirectory(sessionId))
    this.sessionDirectories.delete(sessionId)
    this.indexing.authoritative.add(sessionId)
    this.indexing.local.add(sessionId)
    this.requirements.clear()
    this.updateIndexingTarget(dir)
    if (this.connectionState === "connected") void this.fetchAndSendSandboxStatus(sessionId)
  }

  public forgetSessionDirectory(sessionId: string): void {
    const dir = this.getIndexingDirectory()
    this.sessionDirectories.delete(sessionId)
  ...
  public async getSessionInfo(sessionId: string): Promise<Session | undefined> {
    ...
    return retry(() => client.session.get({ sessionID: sessionId, directory }, { throwOnError: true }))
      ...
      .catch((error: unknown) => {
        console.warn("[ChipMate New] ChipMateProvider: Failed to resolve managed session:", error)
        return undefined
      })
  }
  ...
  public selectChipMateModel(modelID?: string, agent?: string): void {
    if (!modelID && !agent) return
    this.pendingChipMateModel = { ...(modelID && { modelID }), ...(agent && { agent }) }
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("chipmate.v2.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("chipmate.v2.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("chipmate.v2.showChanges", { sessionId, turnId }),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[ChipMate New] ChipMateProvider: ✅ webviewReady received")
          if (this.extensionContext) {
            void recordUpdateWebviewReady(this.extensionContext, {
              ...
              .then((recorded) => {
                if (recorded) console.log("[ChipMate New] 更新激活 Webview 回执已记录。")
              })
        ...
        case "syncSession":
          this.handleSyncSession(message.sessionID, message.parentSessionID).catch((e) =>
            console.error("[ChipMate New] handleSyncSession failed:", e),
          )
          break
        case "loadSessions":
          this.handleLoadSessions().catch((e) => console.error("[ChipMate New] handleLoadSessions failed:", e))
          break
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("chipmate.v2.settingsButtonClicked", message.tab)
          break
        case "openChipMateClaw":
          vscode.commands.executeCommand("chipmate.v2.chipmateClawOpen")
          break
        ...
        case "retryConnection":
          console.log("[ChipMate New] ChipMateProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[ChipMate New] ChipMateProvider: ❌ Retry connection failed:", e),
          )
          break
        case "reload":
          this.handleReload().catch((e) => console.error("[ChipMate New] ChipMateProvider: Reload failed:", e))
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand("chipmate.v2.openSubAgentViewer", message.sessionID, message.title)
          break
        case "saveImage":
          return saveImage(this.getWorkspaceDirectory(this.currentSession?.id), message)
        case "requestProviders":
          this.fetchAndSendProviders().catch((e) => console.error("[ChipMate New] fetchAndSendProviders failed:", e))
          break
        ...
        case "fetchCustomProviderModels":
          this.handleFetchCustomProviderModels(message).catch((e) =>
            console.error("[ChipMate New] fetchCustomProviderModels failed:", e),
          )
          break
        case "compact":
          ...
          break
        case "requestAgents":
          this.fetchAndSendAgents().catch((e) => console.error("[ChipMate New] fetchAndSendAgents failed:", e))
          break
        case "setDocumentAgentScope":
          await this.handleSetDocumentAgentScope(message.sessionID, message.scope)
          break
        case "requestSkills":
          this.fetchAndSendSkills().catch((e) => console.error("[ChipMate New] fetchAndSendSkills failed:", e))
          break
        ...
        case "requestMcpStatus":
          this.fetchAndSendMcpStatus().catch((e) => console.error("[ChipMate New] fetchAndSendMcpStatus failed:", e))
          break
        case "connectMcp": {
          ...
          if (c1) {
            ...
            ).catch((e) => console.error("[ChipMate New] connectMcpServer failed:", e))
          }
          break
        }
        case "disconnectMcp": {
          ...
          if (c2) {
            void McpOAuth.disconnectMcpServer(c2, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[ChipMate New] disconnectMcpServer failed:", e))
          }
          break
        }
        case "authenticateMcp": {
          ...
          if (c) {
            void McpOAuth.authenticateMcpServer(c, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[ChipMate New] authenticateMcpServer failed:", e))
          }
          break
        }
        ...
        case "requestRemoteStatus":
          this.remoteService
            ...
            .catch((err) => console.error("[ChipMate New] remote message failed:", err))
          break
        case "deleteSession":
        ...
        case "requestNotifications":
          this.fetchAndSendNotifications().catch((e) =>
            console.error("[ChipMate New] fetchAndSendNotifications failed:", e),
          )
          break
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[ChipMate New] ChipMateProvider: Failed to enhance prompt:", err)
   ...
   * Subscribes to the shared ChipMateConnectionService.
   */
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (payload, directory) => {
          ...
          if (event.type === "chipmate-sessions.remote-status-changed") return true
          ...
          // ChipMateProvider instance. The Settings panel is a separate provider with no tracked
        ...
        if (state === "connected") {
          ...
          try {
            ...
            if (sdkClient && !internal) {
              const profileResult = await sdkClient.chipmate.profile()
            ...
            console.error("[ChipMate New] ChipMateProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other ChipMateProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other ChipMateProvider instances
      ...
      // Subscribe to profile change broadcast from other ChipMateProvider instances
      ...
      // Subscribe to favorites change broadcast from other ChipMateProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // Subscribe to model-selector expand/collapse broadcast from other ChipMateProvider instances
      ...
      console.log("[ChipMate New] ChipMateProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[ChipMate New] ChipMateProvider: ❌ Failed to initialize connection:", error)
      this.connectionState = "error"
      this.postMessage({
        type: "connectionState",
        state: "error",
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private fetchAndSendSessionModelUsage(sessionID: string, requestID: string): Promise<void> {
    ...
    return this.connectionService
      ...
      .then((client) => client.chipmate.sessionModelUsage({ sessionID, directory }, { throwOnError: true }))
      .then((response) => {
      ...
      .catch((error: unknown) => {
        console.warn("[ChipMate New] ChipMateProvider: Failed to load session model usage:", error)
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[ChipMate New] ChipMateProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    if (!this.indexing.authoritative.has(sessionID) && parentSessionID) {
      const dir = this.sessionDirectories.get(parentSessionID)
      if (dir) {
        const current = this.getIndexingDirectory()
        this.indexing.local.delete(sessionID)
        this.sessionDirectories.set(sessionID, dir)
        this.updateIndexingTarget(current)
      }
    }
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[ChipMate New] ChipMateProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleTerminalContext(requestId: string): Promise<void> {
    try {
      ...
      console.error("[ChipMate New] Failed to capture terminal context:", error)
      this.postMessage({
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleDeleteMessage(sessionID: string, messageID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to delete message:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to rename session:", error)
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("chipmate.v2.model")
          const message = {
          ...
          console.error("[ChipMate New] ChipMateProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    if (!this.client) return
    try {
      const result = await this.client.chipmate.removeAgent({ name, directory: this.getWorkspaceDirectory() })
      if (result.error) {
        console.error("[ChipMate New] removeAgent returned error:", result.error)
      }
    } catch (err) {
      console.error("[ChipMate New] Failed to remove agent:", err)
    }
    this.cachedAgentsMessage = null
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[ChipMate New] ChipMateProvider: Failed to remove MCP server:", name)
    }
  }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch global config:", error)
    }
  }
    ...
    switch (message.type) {
      ...
      case "selectDocumentRagFolder":
        void this.selectDocumentRagFolder(message.scope ?? "project").catch((err) =>
          console.error("[ChipMate New] selectDocumentRagFolder failed:", err),
        )
        return true
      case "rebuildDocumentRag":
        void this.rebuildDocumentRag().catch((err) => console.error("[ChipMate New] rebuildDocumentRag failed:", err))
        return true
      ...
      case "requestChatSettings":
        ...
        return true
      case "requestChipMateEmbeddingModels":
        void this.fetchAndSendChipMateEmbeddingModels().catch((err) =>
          console.error("[ChipMate New] fetchAndSendChipMateEmbeddingModels failed:", err),
        )
        return true
      case "requestImageModels":
        void this.fetchAndSendImageModels().catch((err) =>
          console.error("[ChipMate New] fetchAndSendImageModels failed:", err),
        )
        return true
      case "openSettingsTab":
        if (message.tab === "indexing") {
          await vscode.commands.executeCommand("chipmate.v2.openIndexingSettings")
        }
        return true
      default:
        return false
  ...
  private fetchAndSendIndexingStatus(): Promise<void> {
    ...
    const dir = this.getIndexingDirectory()
    if (!dir) {
      this.postIndexingStandby("Open a file in the target workspace to view indexing status.")
      return Promise.resolve()
    }

    const client = this.client
    if (!client) {
      if (this.cachedIndexingStatusMessage && this.indexing.dir && sameDirectory(this.indexing.dir, dir)) {
    ...
    const revision = this.indexing.revision
    const target = this.indexing.target
    const request = ++this.indexing.request
    const generation = this.connectionGeneration
    return (async () => {
      void MemoryDebug.append({ event: "indexing.status.request", data: { workspace: MemoryDebug.hash(dir) } })
      const auth = Buffer.from(`chipmate:${config.password}`).toString("base64")
      const res = await fetch(`${config.baseUrl}/indexing/status`, {
        headers: {
          Authorization: `Basic ${auth}`,
          "x-chipmate-directory": encodeURIComponent(dir),
        },
      })
      ...
      if (
        ...
        request <= this.indexing.accepted
      )
        return

      this.indexing.accepted = request
      this.storeIndexingStatus(dir, status)
      void MemoryDebug.append({
      ...
      if (
        ...
        return
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch indexing status:", error)
    })
  }

  private acceptIndexingStatus(dir: string, status: IndexingStatus): void {
    if (this.indexing.dead || !this.isCurrentIndexingDirectory(dir)) return
    this.indexing.revision += 1
    this.storeIndexingStatus(dir, status)
  }

  private storeIndexingStatus(dir: string, status: IndexingStatus): void {
    if (this.indexing.dead || !this.isCurrentIndexingDirectory(dir)) return
    const message = { type: "indexingStatusLoaded", status }
    this.indexing.dir = dir
    this.cachedIndexingStatusMessage = message
    this.recordIndexingStatus(status)
    this.postMessage(message)
  }

  private postIndexingStandby(message: string): void {
    ...
    this.postMessage({ type: "indexingStatusLoaded", status })
  }

  private isCurrentIndexingDirectory(dir: string): boolean {
    const current = this.getIndexingDirectory()
    return Boolean(current && sameDirectory(current, dir))
  }

  private updateIndexingTarget(previous: string | undefined): void {
    if (this.indexing.dead) return
    const current = this.getIndexingDirectory()
    const same = previous ? Boolean(current && sameDirectory(previous, current)) : current === undefined
    if (same) return

    this.indexing.revision += 1
    this.indexing.target += 1
    this.indexing.dir = undefined
    this.cachedIndexingStatusMessage = null
  ...
  private async selectDocumentRagFolder(scope: "global" | "project" = "project"): Promise<void> {
    ...
    if (target !== this.indexing.target || !this.isCurrentIndexingDirectory(root)) {
      void vscode.window.showWarningMessage("The project changed while selecting Document RAG folders. Try again.")
      return
    }
  ...
  private async rebuildDocumentRag(): Promise<void> {
    ...
    const dir = this.getIndexingDirectory()
    if (!dir) {
      this.postMessage({ type: "error", message: "Open a workspace folder before rebuilding Document RAG" })
      return
    }
    ...
    try {
      ...
      if (
        ...
        return
      console.error("[ChipMate New] ChipMateProvider: Failed to rebuild document index:", error)
  ...
  private async fetchAndSendChipMateEmbeddingModels(): Promise<void> {
    const catalog = await fetchChipMateEmbeddingModelCatalog()
    const message = { type: "chipmateEmbeddingModelsLoaded", catalog }
    this.cachedChipMateEmbeddingModelsMessage = message
    this.postMessage(message)
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[ChipMate New] ChipMateProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[ChipMate New] ChipMateProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[ChipMate New] ChipMateProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[ChipMate New] ChipMateProvider: config warnings fetched", { from, count: list.length })
      ...
      console.warn("[ChipMate New] ChipMateProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  private async fetchAndSendNotifications(): Promise<void> {
    if (isInternalOfflineBuild()) {
      const message: NotificationsMessage = { type: "notificationsLoaded", notifications: [], dismissedIds: [] }
      this.cachedNotificationsMessage = message
      this.postMessage(message)
      return
    }
  ...
  // Cloud session methods extracted to chipmate-provider/handlers/cloud-session.ts
  ...
  private sendNotificationSettings(): void {
    const attention = vscode.workspace.getConfiguration("chipmate.v2.attention")
    this.postMessage({
  ...
  private async runToggleSandbox(
    ...
    try {
      ...
      const remembered = await this.connectionService.sandboxPreference
        ...
        .then(() => true)
        .catch((error) => {
          console.error("[ChipMate New] Failed to persist sandbox default:", error)
          return false
        })
      this.postMessage({
        type: "sandboxStatus",
  ...
  private maxCostSetting(): number {
    return this.setMaxCost(vscode.workspace.getConfiguration("chipmate.v2").get<number>("maxCost", 0))
  }

  private commitMessageLanguageSetting(): string {
    return vscode.workspace.getConfiguration("chipmate.v2").get<string>("languageCommitMessage", "sync")
  }
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "ChipMateProvider: Message request", () =>
        this.withRetry(
          () =>
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      await this.requirements.assertAgentRequirements(agent, dir)
      await this.checkpoints.get(sid)
      await runWithMessageConfirmation(this.confirmations, messageID, "ChipMateProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[ChipMate New] ChipMateProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[ChipMate New] ChipMateProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[ChipMate New] ChipMateProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[ChipMate New] ChipMateProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  // Permission + question handlers extracted to chipmate-provider/handlers/permission.ts and question.ts
  ...
  // Cloud session handlers extracted to chipmate-provider/handlers/cloud-session.ts
  ...
  // Auth handlers extracted to chipmate-provider/handlers/auth.ts
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    try {
      const profileResult = await this.client!.chipmate.profile()
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to refresh profile after org switch:", error)
    }
    try {
      await this.fetchAndSendProviders()
    } catch (error) {
      console.error("[ChipMate New] ChipMateProvider: Failed to refresh providers after org switch:", error)
    }
  }

  /**
   * Handle a generic setting update from the webview.
   * The key uses dot notation relative to `chipmate.v2` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown, requestId?: string): Promise<void> {
    try {
      ...
      if (section === "indexing" && !validIndexingSetting(leaf, value)) {
        throw new Error(`Invalid indexing setting: ${leaf}`)
      }
  ...
  /**
   * Reset all "chipmate.v2.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "chipmate.v2." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "chipmate.v2.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all ChipMate extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "chipmate.v2."
    const ext = vscode.extensions.getExtension("chipmate.chipmate")
    ...
    await this.extensionContext?.globalState.update("chipmate.v2.dismissedNotificationIds", undefined)
    await this.extensionContext?.globalState.update("chipmate.v2.agentMigrationBannerDismissed", undefined)
    await this.extensionContext?.globalState.update("chipmate.v2.marketplace.dismissedSuggestions", undefined)
    ...
    vscode.window.showInformationMessage("ChipMate settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("chipmate.v2.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("chipmate.v2").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private async handleReload(): Promise<void> {
    if (!this.client) {
      console.warn("[ChipMate New] handleReload: no client connection")
      return
    }
    ...
    try {
      ...
      console.error("[ChipMate New] handleReload: reload endpoint failed:", err)
  ...
  private handleEvent(event: ProviderEvent, directory?: string): void {
    ...
    // so this runs on every ChipMateProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[ChipMate New] ChipMateProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (event.type === "indexing.status") {
      const dir = this.getIndexingDirectory()
      if (!directory || !dir || !sameDirectory(directory, dir)) return
    }
    ...
    if (next.type === "indexingStatusLoaded") {
      ...
      this.acceptIndexingStatus(directory, next.status)
      return
    }
    this.streams.flush(sessionID)
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[ChipMate New] ChipMateProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[ChipMate New] ChipMateProvider: ❌ postMessage failed", error)
    })
  }

  private flushPendingChipMateModel(): void {
    if (!this.webview || !this.isWebviewReady || !this.client || !this.pendingChipMateModel) return

    const pending = this.pendingChipMateModel
    this.pendingChipMateModel = null
    this.postMessage({ type: "selectChipMateModel", ...pending })
  }
  ...
  public async toggleMemory(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to toggle memory:", error)
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[ChipMate New] ChipMateProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
   ...
   * given directory, filtered through .chipmate-v2ignore.
   */
  ...
  private async gatherEditorContext(dir?: string): Promise<EditorContext> {
    ...
    // Visible files (capped to avoid bloating context, filtered through .chipmate-v2ignore)
    const visibleFiles = [
      ...new Set(
        [
    ...
    // Active file (also filtered through .chipmate-v2ignore)
  ...
  private getRootDirectory(): string {
    ...
    return this.extensionContext?.globalStorageUri.fsPath ?? process.cwd()
  }

  private trackDirectory(sessionId: string, dir: string) {
    if (this.indexing.authoritative.has(sessionId)) return
    const current = this.getIndexingDirectory()
    const known = this.currentSession?.id === sessionId ? this.currentSession.directory || dir : dir
    this.indexing.local.delete(sessionId)
    if (sameDirectory(known, this.getRootDirectory())) {
      this.sessionDirectories.delete(sessionId)
      this.updateIndexingTarget(current)
      return
    }
    this.sessionDirectories.set(sessionId, known)
    this.updateIndexingTarget(current)
  }
  ...
  private getIndexingDirectory(): string | undefined {
    if (this.projectDirectory !== undefined) return this.projectDirectory ?? undefined
    const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath
    if (!root) return undefined
    const session = this.currentSession
    if (!session) return root
    return (
      this.sessionDirectories.get(session.id) ??
      (this.indexing.local.has(session.id) ? root : session.directory) ??
      root
    )
  }
  ...
  dispose(): void {
    ...
    this.indexing.target += 1
    this.indexing.dir = undefined
    this.indexing.authoritative.clear()
    this.indexing.local.clear()
    this.cachedIndexingStatusMessage = null
