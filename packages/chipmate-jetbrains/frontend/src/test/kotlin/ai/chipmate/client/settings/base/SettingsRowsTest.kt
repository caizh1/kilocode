...
class SettingsRowsTest : BasePlatformTestCase() {
    ...
    fun `test top banner renders login action`() {
        ...
        assertTrue(text(top).contains("Sign in to ChipMate"))
    ...
    fun `test settings panel keeps banner in scroll content and progress in overlay`() {
        ...
        assertTrue(text(panel.content).contains("Sign in to ChipMate"))
        assertFalse(text(panel.overlay).contains("Sign in to ChipMate"))
