...
import { ChipMateEmbedder } from "./embedders/chipmate"
...
function product(): string | undefined {
  return process.env.CHIPMATE_PRODUCT_PROFILE === "chipmate-v2" ? "chipmate-v2" : undefined
}
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(runtime?: EmbeddingRuntimeProfile): IEmbedder {
    ...
    if (provider === "chipmate") {
      if (!config.chipmateOptions?.apiKey) throw new Error("ChipMate API key is required for embedding.")
      if (!config.modelId) throw new Error("ChipMate embedding model is required.")
      return new ChipMateEmbedder({
        apiKey: config.chipmateOptions.apiKey,
        baseUrl: config.chipmateOptions.baseUrl,
        organizationId: config.chipmateOptions.organizationId,
