...
import { existsSync, writeFileSync } from "fs" // chipmate_change
import fs from "fs/promises"
import * as Global from "../global"
import { Schema } from "effect"
import { Glob } from "./glob"
import { createStream } from "rotating-file-stream" // chipmate_change
import { CHIPMATE_RUN_ID } from "./opencode-process" // chipmate_change
...
const keep = 10
const initializedRunID = "CHIPMATE_LOG_INITIALIZED_RUN_ID"
...
let write = stderr
let stream: ReturnType<typeof createStream> | undefined // chipmate_change

export async function init(options: Options) {
  if (options.level) level = options.level
  void cleanup(Global.Path.log)
  // chipmate_change start - initialize one rotating stream and truncate dev.log once per ChipMate run
  if (stream) {
    const active = stream
    stream = undefined
