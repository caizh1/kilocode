...
class ChipMatePromptCompletionProviderTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeWorkspaceRpcApi
    private lateinit var provider: ChipMatePromptCompletionProvider

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        rpc = FakeWorkspaceRpcApi()
        val workspaces = ChipMateWorkspaceService(scope, rpc)
        provider = ChipMatePromptCompletionProvider(
    ...
    fun `test accepting mention mid token replaces glued suffix`() {
        ...
        assertEquals(
            ChipMatePromptCompletionProvider.MentionHit(0, 22, "backend/deploy-dev.sh", true),
    ...
    fun `test slash completion ranks prefix matches before contains matches`() {
        rpc.state.value = ChipMateWorkspaceStateDto(
            ChipMateWorkspaceStatusDto.READY,
            commands = listOf(
    ...
    fun `test highlights known slash command at start`() {
        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(0, 4, ChipMatePromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights client slash command aliases at start`() {
        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(0, 6, ChipMatePromptCompletionProvider.HighlightKind.COMMAND)),
            provider.highlights("/clear"),
        )
    }

    fun `test highlights server slash command at start`() {
        rpc.state.value = ChipMateWorkspaceStateDto(ChipMateWorkspaceStatusDto.READY, commands = listOf(CommandDto("deploy")))

        waitFor { provider.highlights("/deploy prod").isNotEmpty() }

        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(0, 7, ChipMatePromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights special mentions without tracked paths`() {
        assertEquals(
            listOf(
                ChipMatePromptCompletionProvider.Highlight(4, 16, ChipMatePromptCompletionProvider.HighlightKind.MENTION),
            ),
    ...
    fun `test serverCommand does not route client aliases`() {
        rpc.state.value = ChipMateWorkspaceStateDto(ChipMateWorkspaceStatusDto.READY, commands = listOf(CommandDto("clear")))
    ...
    fun `test highlights tracked mentions longest first`() {
        ...
        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(4, 14, ChipMatePromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test highlights unresolved mention after validation`() {
        ...
        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(4, 16, ChipMatePromptCompletionProvider.HighlightKind.INVALID)),
    ...
    fun `test highlights existing hand typed mention after validation`() {
        ...
        assertEquals(
            listOf(ChipMatePromptCompletionProvider.Highlight(4, 13, ChipMatePromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test mentionAt resolves tracked file mention`() {
        ...
        assertEquals(
            ChipMatePromptCompletionProvider.MentionHit(4, 13, "src/a.ts", true),
    ...
    fun `test mentionAt marks unresolved file mention after validation`() {
        ...
        assertEquals(
            ChipMatePromptCompletionProvider.MentionHit(4, 15, "missing.ts", false),
    ...
    private fun noMatches() = ChipMateBundle.message("prompt.completion.noMatches")
}
