...
class McpEditDialogTest : BasePlatformTestCase() {
    ...
    fun `test environment can add and remove rows`() {
        ...
        val result = edt {
            ...
            fields[1].text = "NEXT"
            fields[2].text = "value"
            descendants(root).filterIsInstance<JButton>().single { it.text == ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.add") }.doClick()
    ...
    private fun title(field: String) = ChipMateBundle.message("settings.agentBehavior.mcp.edit.$field")
