...
internal val DIFF_FILE_KEY: Key<String> = Key.create("chipmate.diff.file")
...
internal class DiffEditorView(
    ...
    private val banner = EditorNotificationPanel(EditorNotificationPanel.Status.Warning).apply {
        text(ChipMateBundle.message("diff.editor.outdated"))
        createActionLabel(ChipMateBundle.message("diff.editor.refresh")) { refresh() }
        isVisible = false
    }
    ...
    private val openFileAction = object : DumbAwareAction(
        ChipMateBundle.message("diff.editor.openFile"),
        ChipMateBundle.message("diff.editor.openFile"),
        ...
        override fun actionPerformed(e: AnActionEvent) {
            ...
            scope.launch { service<ChipMateWorkspaceService>().openFile(path) }
        }
    }
    ...
    private fun installMenu() {
        val group = DefaultActionGroup(
            ...
            TreeAction(ChipMateBundle.message("diff.editor.refresh"), AllIcons.Actions.Refresh, ::refresh),
        )
    ...
    private fun labels(): Pair<String, String> {
        if (params["source"] == "branch") {
            return ChipMateBundle.message("diff.editor.side.base") to ChipMateBundle.message("diff.editor.side.current")
        }
        return ChipMateBundle.message("diff.editor.side.original") to ChipMateBundle.message("diff.editor.side.modified")
    }
...
internal fun emptyChangesComponent(): JComponent = JPanel(BorderLayout()).apply {
    add(com.intellij.ui.components.JBLabel(ChipMateBundle.message("diff.editor.empty")), BorderLayout.CENTER)
}
...
internal fun treeToolbarGroup(tree: Tree, refresh: () -> Unit) = DefaultActionGroup(
    TreeAction(ChipMateBundle.message("diff.editor.refresh"), AllIcons.Actions.Refresh, refresh),
    Separator.getInstance(),
    TreeAction(ChipMateBundle.message("diff.editor.tree.expandAll"), AllIcons.Actions.Expandall) { expandAll(tree) },
    TreeAction(ChipMateBundle.message("diff.editor.tree.collapseAll"), AllIcons.Actions.Collapseall) { collapseAll(tree) },
)

private fun fileCount(count: Int): String = ChipMateBundle.message(
