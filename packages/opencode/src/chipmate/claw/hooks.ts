...
/**
 * ChipMateClaw SolidJS reactive helpers — ChipMate Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: ChipMate Chat connection + active-conversation state
 *
...
/**
 * Poll the ChipMateClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.chipmate.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.chipmate.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.chipmate.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.chipmateChatUrl) missing.push("chipmateChatUrl")
    ...
    try {
      log.info("connecting chipmate-chat")
      chat = await connect({
        envelope,
