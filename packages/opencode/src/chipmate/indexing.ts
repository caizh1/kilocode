...
import { toIndexingConfigInput, type IndexingConfig } from "@chipmate/chipmate-indexing/config"
...
} from "@chipmate/chipmate-indexing/status"
...
import { LanceDBRuntime } from "./lancedb" // chipmate_change
import { indexingWithChipMateDefault, resolveChipMateIndexingAuth, type ChipMateIndexingAuth } from "./indexing-auth" // chipmate_change
import { applyInternalIndexingDefaults } from "./internal-offline" // chipmate_change
import { MemoryDebug } from "./memory-debug"
import { memoryExit } from "./indexing-memory"
import { worktreeRoots } from "./primary-worktree"

const log = Log.create({ service: "chipmate-indexing" })
...
function disabledByEnvironment(reason: string) {
  ...
  return disabledIndexingStatus(`Codebase indexing is disabled by CHIPMATE_DISABLE_CODEBASE_INDEXING (reason: ${detail}).`)
}

function disableReason(): string | undefined {
  const value = process.env["CHIPMATE_DISABLE_CODEBASE_INDEXING"]?.trim()
  if (!value || ["0", "false", "off", "no"].includes(value.toLowerCase())) return undefined
  return value
}

function emptyWorkspace(dir: string): boolean {
  const value = process.env["CHIPMATE_VSCODE_EMPTY_WORKSPACE_DIR"]?.trim()
...
async function inputFromConfig(cfg: Config.Info): Promise<ReturnType<typeof toIndexingConfigInput>> {
  const auth = await chipmateAuth(cfg)
  const globalConfig = await AppRuntime.runPromise(Config.Service.use((svc) => svc.getGlobal()))
  const global = globalConfig.indexing
  const merged = indexingWithChipMateDefault(resolveConfig(cfg.indexing, global), auth)
  const raw = toIndexingConfigInput({
    ...merged,
    enabled: merged?.enabled === true,
  })
  return model(enrichChipMate(raw, auth), auth)
}
const baselineDirectory = Effect.fn("ChipMateIndexing.baselineDirectory")(function* (dir: string) {
...
async function chipmateAuth(cfg: Config.Info): Promise<ChipMateIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("chipmate"))
  return resolveChipMateIndexingAuth({ config: cfg, auth: info })
}

function enrichChipMate(input: ReturnType<typeof toIndexingConfigInput>, auth: ChipMateIndexingAuth) {
  if (input.embedderProvider !== "chipmate") return input

  return {
    ...input,
    chipmateApiKey: input.chipmateApiKey ?? auth.apiKey,
    chipmateBaseUrl: input.chipmateBaseUrl ?? auth.baseUrl,
    chipmateOrganizationId: input.chipmateOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: ChipMateIndexingAuth) {
  if (input.embedderProvider !== "chipmate" || !input.enabled) return input

  const catalog = await fetchChipMateEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported ChipMate embedding model configuration", { model: input.modelId })
    }
...
export namespace ChipMateIndexing {
  ...
  export async function models() {
    try {
      ...
      const fallback = await fetchChipMateEmbeddingModelCatalog()
      return fallback.models.length > 0 ? fallback : catalog
    } catch (err) {
      log.warn("falling back to public ChipMate embedding model catalog", { err })
      return fetchChipMateEmbeddingModelCatalog()
    }
  }
