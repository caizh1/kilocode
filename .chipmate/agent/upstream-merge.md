...
**Do not load the `chipmate-merge-minimizer` skill.** That skill is for
authoring new ChipMate changes against shared upstream files; during an upstream
merge it gives the wrong guidance (it nudges toward extracting ChipMate logic out
...
- does a `chipmate_change` marker in HEAD encode a bug fix, a feature, or a
  defensive check?
...
When HEAD includes a non-obvious ChipMate-specific wrapper (e.g. a helper in
`packages/opencode/src/chipmate/`), find out why it exists before deciding to
keep or bypass it:

```bash
git log --all --oneline -S "<symbol>" -- packages/opencode/src/chipmate/
git log --all --oneline -- <chipmate-file>
```
...
- one-sentence rationale (what ChipMate behaviour is preserved, what upstream
...
3. State which ChipMate behaviour must survive and why (reference PR numbers /
   `chipmate_change` comments when possible).
...
- `.worktrees/opencode-merge/chipmate-main` — ChipMate base snapshot
- `.worktrees/opencode-merge/auto-merge` — automated merge snapshot (original
  conflict reference)

Apply in order:

- prefer upstream code and architecture whenever compatible with ChipMate behaviour
- preserve ChipMate-specific behaviour marked with `chipmate_change`
- keep `chipmate_change` markers around ChipMate-specific code in shared opencode
  files
- when upstream refactors a region that HEAD had annotated with
  `chipmate_change`, **check whether the marker encodes a bug fix or a feature
  ...
  `Effect.gen` block.
- when a `take-theirs` drops a line that was the target of a ChipMate pre-filter,
  the upstream line may be actively wrong for ChipMate — e.g. an inner `continue`
  filter whose condition collides with an outer filter ChipMate added. Re-read the
  surrounding 20 lines before committing to `take-theirs`.
- if ChipMate-specific code must be refactored to fit new upstream architecture,
  explain the refactor in the final summary
- if upstream moved the relevant logic to another file, port the ChipMate behaviour
  there and list both paths in the final summary. Verify the new file already
  carries the ChipMate-renamed symbols (e.g. `x-chipmate-directory`) by diffing against
  pristine upstream.
- if upstream extracts shared policy into a helper, move ChipMate-specific additions
...
- if upstream deleted a file, analyse whether the ChipMate behaviour should be
...
**commenting it out with `chipmate_change` markers** over deletion when the
surrounding structure (an `if`, a loop) still makes sense. That keeps the
intent visible to the next merger. Example:

```ts
} else if (input?.scope !== "project" && !Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES) {
...
shows up in `git diff --diff-filter=U`; the new sibling may need a ChipMate feature
...
- `chipmate_change` comments in the *auto-merge* snapshot that didn't end up in
...
- **Orphaned imports and references.** A rename upstream may leave a ChipMate
...
- **Partial auto-merges.** Upstream may have refactored a region ChipMate
  ...
  path, the file compiles upstream but breaks on ChipMate.
...
- other CI guards that touched files imply (knip for `chipmate-vscode/`,
  `check-chipmate-change`, source-links, visual regression,
  `script/check-forbidden-strings.ts`)
- if you encounter a hardcoded upstream URL, repo path, or attribution string
  during conflict resolution that obviously shouldn't ship in ChipMate (e.g. another
...
shared code, but the change cascades into ChipMate-only files (chipmate tests,
chipmate-specific source, plugins) that still reference the old symbol. Those
...
### 9.6. Handle upstream-added tests that diverge from ChipMate

Upstream sometimes adds tests that encode design contracts ChipMate intentionally
breaks. These auto-merge cleanly and then fail. Three resolution patterns:

- **Rewrite the test** when the test is a contract assertion and ChipMate has a
  different but equally valid contract. Invert or adjust the assertion with a
  `chipmate_change` marker explaining the divergence.
- **Skip the test** when the test relies on patterns that ChipMate has replaced
  ...
  ChipMate's extensions break). Mark with `chipmate_change` and a rationale
  explaining what would need to change for the test to run.
- **Delete the test** when it covers functionality ChipMate deliberately removed
  (fallback paths, deprecated endpoints, products ChipMate doesn't ship). Note
...
`packages/extensions/zed/extension.toml` (version field + 5 ChipMate-Org download
...
(e.g. `1.14.30`), whose release tag does not exist on ChipMate's pipeline, so the
...
(excluding `node_modules`, hidden dirs, and `packages/chipmate-jetbrains/` which
tracks its own cadence), plus the Zed extension toml. It is idempotent — rerun
it any time to rebase the version back onto ChipMate main (useful during
...
- **Non-trivial merge decisions**: a short section per file (or group of
  ...
  Focus on *what ChipMate behaviour survived* and *what upstream features were
  adopted*. Link to ChipMate PRs when a `chipmate_change` encodes a specific fix.
...
- Upstream can reintroduce bugs a ChipMate `chipmate_change` had already fixed —
  during big refactors check every `chipmate_change` the refactor touched.
- "Take-theirs" on an inner conditional is often wrong when ChipMate added an outer
...
- Stricter DOM lib types (upstream TS upgrade) can surface latent casting
  issues around `WebSocket.send`, `Headers`, etc. — prefer narrowing the ChipMate
...
- ChipMate code may rely on ambient context (async-local storage, globally-set
  flags, process env) being populated at a lifecycle moment that upstream
  refactors away. If ChipMate behaviour reads ambient state during init, forked
...
- Tests that intercept via process-global or module-global spies can become
  ...
  the spy.
- When ChipMate extends a shared data shape with extra optional fields, different
  ...
  every ChipMate addition — audit the encoding assumption before adding fields.
...
- Upstream-added tests can encode a design contract ChipMate deliberately breaks.
  The test passes upstream because upstream doesn't share ChipMate's requirement.
  Decide between refactoring ChipMate to match the upstream contract or rewriting
  the test to assert ChipMate's divergent contract — with a `chipmate_change`
