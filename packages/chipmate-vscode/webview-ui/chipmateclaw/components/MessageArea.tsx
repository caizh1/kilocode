// ChipMateClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/chipmate-chat/components/MessageArea.tsx
...
export function MessageArea() {
  ...
  //
  // Uses @chipmate/chipmate-ui's createAutoScroll (the same one MessageList.tsx
  ...
  const sendDisabledReason = createMemo(() => {
    ...
    const st = botDisplay().state
    if (st === "unknown") return t("chipmateClaw.chat.waitingBotStatus")
    return t("chipmateClaw.chat.botOffline")
  })

  // Render the typing banner with friendly names. Bot members come in as
  // `bot:chipmateclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "ChipMateClaw" label) and render any human
  // collaborators by their raw memberId.
  const typingNames = createMemo(() => {
    const activeId = claw.activeConversationId()
    if (!activeId) return []
    const assistant = claw.assistantName() ?? t("chipmateClaw.message.bot")
  ...
  return (
    <Show
      ...
      fallback={
        <div class="chipmateclaw-empty-area">
          <p>{t("chipmateClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="chipmateclaw-area">
        {/* Messages */}
        <div class="chipmateclaw-area-messages-wrap">
          <div
            class="chipmateclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
          ...
          >
            <div ref={auto.contentRef}>
              <Show when={claw.messages().length === 0}>
                <div class="chipmateclaw-empty">
                  {claw.assistantName()
                    ? t("chipmateClaw.chat.emptyWithBot").replace("{bot}", claw.assistantName()!)
                    : t("chipmateClaw.chat.empty")}
                </div>
              </Show>
        ...
        <Show when={typingNames().length > 0}>
          <div class="chipmateclaw-typing">
            <Spinner />
            <span>
              {typingNames().length === 1
                ? t("chipmateClaw.typing.one").replace("{name}", typingNames()[0])
                : t("chipmateClaw.typing.many").replace("{count}", String(typingNames().length))}
            </span>
          </div>
        </Show>
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="chipmateclaw-reply-preview">
              <span class="chipmateclaw-reply-preview-label">{t("chipmateClaw.message.replyTo")}</span>
              <span class="chipmateclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
                class="chipmateclaw-iconbtn-sm"
                onClick={() => setReplyingTo(null)}
                aria-label={t("chipmateClaw.message.cancelReply")}
              >
                ×
              </button>
            </div>
        ...
        <div class="chipmateclaw-input-wrap">
          <textarea
            ref={input}
            class="chipmateclaw-input"
            placeholder={sendDisabledReason() ?? t("chipmateClaw.chat.placeholder")}
            disabled={!canSend()}
            value={text()}
            onInput={onInput}
            onKeyDown={onKeyDown}
            rows={1}
            aria-label={t("chipmateClaw.chat.placeholder")}
          />
          <Button variant="primary" disabled={!canSend() || !text().trim()} onClick={submit}>
            {t("chipmateClaw.chat.send")}
          </Button>
        </div>
      </div>
    </Show>
