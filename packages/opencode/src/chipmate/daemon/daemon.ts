...
export namespace Daemon {
  const username = "chipmate"
  const lock = "chipmate-daemon"
  ...
  function root() {
    return process.env.CHIPMATE_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.CHIPMATE_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
  ...
  export function matches(state: State, input: Options, explicit: readonly NetworkOption[]) {
    if (state.password === "chipmate") return false
  ...
  async function port(input: Options) {
    ...
    if (input.env?.CHIPMATE_TEST_DAEMON_EPHEMERAL_PORT) return 0
  ...
  async function line(out: string) {
    ...
    const match = text.match(/chipmate server listening on http:\/\/([^:\s]+):(\d+)/)
