...
const CHIPMATEIGNORE = ".chipmate-v2ignore"
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    const chipmateignorePath = path.join(this.workspacePath, CHIPMATEIGNORE)
    if (fs.existsSync(chipmateignorePath)) {
      const chipmateignoreContent = fs.readFileSync(chipmateignorePath, "utf-8")
      if (chipmateignoreContent.trim()) {
        this.ignoreInstance.add(chipmateignoreContent)
        this.ignoreInstance.add(CHIPMATEIGNORE)
        return
      }
    }
