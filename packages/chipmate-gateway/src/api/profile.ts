...
import type { ChipMateProfile, Organization, ChipMateBalance } from "../types.js"
import { CHIPMATE_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from ChipMate API
 */
export async function fetchProfile(token: string): Promise<ChipMateProfile> {
  const response = await fetch(`${CHIPMATE_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat ChipMateProfile structure
  return {
 ...
 */
export const getChipMateProfile = fetchProfile
 ...
 */
export function defaultOrganizationId(profile: ChipMateProfile): string | undefined {
...
/**
 * Fetch user balance from ChipMate API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<ChipMateBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-chipmate-organizationid"] = organizationId
    }

    const response = await fetch(`${CHIPMATE_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getChipMateBalance = fetchBalance
...
export async function fetchDefaultModel(token?: string, organizationId?: string): Promise<string> {
  ...
  const url = `${CHIPMATE_API_BASE}${path}`
 ...
 * Alias for compatibility with existing code
 */
export const getChipMateDefaultModel = fetchDefaultModel
