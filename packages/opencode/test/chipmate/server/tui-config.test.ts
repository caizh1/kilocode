...
describe("TUI config routes", () => {
  test("gets effective project TUI config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const cfg = path.join(dir, ".chipmate")
  ...
  test("loads legacy .chipmate TUI config and ignores .opencode", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, ".chipmate", "tui.json"), JSON.stringify({ theme: "nord" }))
      },
    })
  ...
  test("patches project TUI config", async () => {
    ...
    const saved = await Bun.file(path.join(tmp.path, ".chipmate", "tui.json")).json()
