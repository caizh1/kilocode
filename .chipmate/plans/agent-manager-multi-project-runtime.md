...
- Setting — `chipmate-code.new.agentManager.multiProject` (boolean, default false, application scope).
...
- Known boundary: `ChipMateProvider.projectID` and session refresh remain single-project; cross-project SSE filtering by session→directory owner is a follow-up (see "Verified findings" below).
...
- Global extension storage contains only the catalog of additional projects and project-list UI metadata. Each repository remains authoritative for its own `.chipmate/agent-manager.json`.
- A collapsed, never-opened additional project causes no `.chipmate/agent-manager.json` read, Git mutation, setup/run initialization, terminal creation, or poller subscription.
...
- `AgentManagerProvider` currently owns the panel and all repository-bound resources in one 1,941-line singleton. Its root is always `Host.workspacePath()` and its state/managers are lazily cached against that root (`packages/chipmate-vscode/src/agent-manager/AgentManagerProvider.ts:56-212`, `1654-1695`). The architecture test caps this file at 2,000 lines and explicitly forbids raising the cap (`packages/chipmate-vscode/tests/unit/agent-manager-arch.test.ts:793-854`).
- Opening the panel immediately initializes the current repository, performs `ensureGitExclude`, loads `.chipmate/agent-manager.json`, discovers/restores worktrees, registers sessions, starts state-dependent behavior, and recovers prompts (`AgentManagerProvider.ts:267-360`). This whole sequence cannot run for a collapsed catalog entry.
- `WorktreeStateManager` is correctly repository-scoped by constructor root and writes only `<root>/.chipmate/agent-manager.json` (`WorktreeStateManager.ts:100-124`, `632-656`, `797-847`). Its save queue and `flush()` are suitable for a context shutdown barrier, but it has no cancellation/dispose primitive (`755-795`).
- The embedded Agent Manager `ChipMateProvider` is one provider attached to the one panel/webview (`agent-manager/vscode-host.ts:67-169`). It already routes many SDK calls through a session-to-directory map, but local sessions are allowed to fall back to `workspaceFolders[0]` (`ChipMateProvider.ts:4343-4378`; `chipmate-provider-utils.ts:319-345`). That fallback becomes a cross-project write risk.
- `ChipMateProvider` has singleton project state (`projectID`, `currentSession`, project-scoped caches, one active project directory) and current session refresh treats all extra directories as worktrees of one canonical project (`ChipMateProvider.ts:344-359`, `1984-2029`; `chipmate-provider-utils.ts:230-295`). It is not multi-project-correct without refactoring.
...
- Existing ChipMate experimental config is CLI/project config, not an application feature-flag service (`features.ts`; `webview-ui/src/types/messages/config.ts`). A multi-project bootstrap flag cannot be project-scoped because choosing which project config to read is the behavior being gated.
...
Persist one unsynced global-state value under `chipmate.agentManager.projects.v1`:
...
- Removing is allowed only for non-pinned projects. It removes only the catalog entry after the context is idle/disposed; it never deletes `.chipmate`, worktrees, sessions, branches, or shares.
...
- the one panel and embedded `ChipMateProvider`/`SessionProvider`;
...
It does not own a webview, `ChipMateProvider`, VS Code terminal command registrations, global connection subscriptions, or a global orchestration subscription.
...
### 3. One embedded ChipMateProvider, made project-aware

Use one embedded `ChipMateProvider`. Multiple providers are not safe or useful here:
...
- Replace singleton Agent Manager use of `ChipMateProvider.projectID` with the route table/directory ownership check. Keep existing project filtering for non-Agent-Manager provider instances.
...
Contribute an application-scoped VS Code setting, default false, for example `chipmate-code.new.agentManager.multiProject`, with `scope: "application"`. Read it in the host/coordinator; do not put it in CLI `experimental` config.
...
2. Extend `Host` in `src/agent-manager/host.ts`; implement global-state storage, canonical Git/URI resolution, pinned project resolution, and the application flag in `src/agent-manager/vscode-host.ts`. Preserve URIs in `openFolder`. Add the setting contribution in `packages/chipmate-vscode/package.json`.
...
6. Modify `ChipMateProvider`, `chipmate-provider/options.ts`, and the extracted helpers in `chipmate-provider-utils.ts` so Agent Manager Local sessions also have explicit routes, unknown strict routes fail closed, child routes inherit project identity, and project activation invalidates project-scoped cache work by generation. Keep non-Agent-Manager provider behavior unchanged.
...
Add or update these focused tests under `packages/chipmate-vscode/tests/unit/`:
...
- Extend `chipmate-provider-session-refresh.test.ts`: lists per project instead of merging project roots; outputs project stamps; failed worktree listing preserves only that project's sessions; activation generation drops stale config/session results.
- Extend `chipmate-provider-worktree-context.test.ts`: strict unknown session/worktree has no workspace fallback; Local routes explicitly resolve to their own project root; a secondary Local session cannot resolve to pinned root.
...
Run from `packages/chipmate-vscode/` after each slice:

```sh
bun test tests/unit/agent-manager-project-registry.test.ts tests/unit/agent-manager-project-router.test.ts tests/unit/chipmate-provider-session-refresh.test.ts tests/unit/chipmate-provider-worktree-context.test.ts
bun run typecheck
bun run lint
bun run test:unit
bun run knip
...
- The largest correctness risk is a hidden workspace-root fallback in `ChipMateProvider` or a helper. Treat every Agent Manager method that accepts `sessionID`, `worktreeId`, `draftID`, `requestID`, or directory as part of the routing audit. Tests should fail if strict mode calls `workspaceFolders[0]` for an identified operation.
- Project-scoped ChipMateProvider caches can show stale settings after a rapid project switch. Activation generation checks are required before posting and before replacing caches; merely cancelling transcript load is insufficient.
...
- Rollback is safe: turn off the application flag. The pinned path and existing `.chipmate/agent-manager.json` format remain intact, while the unused global catalog is preserved for a later re-enable.
