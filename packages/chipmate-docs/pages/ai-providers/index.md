---
title: "AI Providers"
description: "Configure and connect different AI model providers to ChipMate"
---

# AI Providers

ChipMate supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **ChipMate's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
- **[Atomic Chat](/docs/ai-providers/atomic-chat)** - Local models with TurboQuant inference and auto-discovery in ChipMate
...
|---|---|
| Ease of use | [ChipMate (built-in)](/docs/ai-providers/chipmate) |
...
You can prevent specific providers from loading using `disabled_providers` in your `chipmate.json` (or `chipmate.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `chipmate`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to ChipMate?** Start with the [ChipMate provider](/docs/ai-providers/chipmate) - no setup required
