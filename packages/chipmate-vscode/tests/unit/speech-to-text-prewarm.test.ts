...
const SCRIPT = `
  ...
  const [config, setConfig] = createSignal({ disabled_providers: ["chipmate"] })
  ...
  if (sent.length !== 0) fail("prewarmed without ChipMate access")
  setAuth({ chipmate: "api" })
  if (sent.length !== 0) fail("prewarmed while ChipMate was disabled")
  setConfig({})
  if (sent.length !== 1 || sent[0]?.type !== "speechToTextPrewarm") {
    fail("did not prewarm after ChipMate access became available")
  }
  setAuth({ chipmate: "oauth" })
...
describe("speech-to-text prewarm", () => {
  it("starts only after ChipMate speech access becomes available", () => {
    const attempts = 3
