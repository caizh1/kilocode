---
title: "Using xAI Grok with ChipMate"
description: "Connect xAI's Grok models to ChipMate. Use a SuperGrok or X Premium subscription via OAuth or a paid API key. Guide to setup in VS Code and the CLI."
sidebar_label: xAI (Grok)
---

# Using xAI (Grok) With ChipMate
...
ChipMate supports two ways to connect xAI:
...
- **OAuth login — no API keys:** Sign in through your browser and ChipMate handles token management automatically.
- **Automatic token refresh:** ChipMate refreshes your access token in the background so long-running sessions stay authenticated.

{% callout type="note" %}
SuperGrok and X Premium subscription access works with ChipMate's core functionality (VS Code extension and CLI). For cloud features such as Cloud Agents or ChipMateClaw, use the [ChipMate Gateway](/docs/gateway) — the Gateway supports xAI via [BYOK](/docs/getting-started/byok) with an API key (OAuth and subscription-based access are not supported through the Gateway).
{% /callout %}
...
```bash
chipmate auth login --provider xai
```

ChipMate offers three methods at the prompt:
...
- **Sign out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `chipmate auth logout` and choose xAI.
...
- **Token rotation:** xAI rotates refresh tokens on each use. ChipMate persists the latest tokens automatically. If you run ChipMate from multiple processes simultaneously, the first refresh can invalidate the other process's token — re-run `chipmate auth login --provider xai` to restore the session.
...
3. **Create a Key:** Click to create a new API key. Give your key a descriptive name (e.g., "ChipMate").
...
The extension stores this in your `chipmate.json` config file. You can also edit the config file directly — see the **CLI** tab for the file format.

{% /tab %}
{% tab label="CLI" %}

Set the API key as an environment variable or configure it in your `chipmate.json` config file:
...
**Config file** (`~/.config/chipmate/chipmate.json` or `./chipmate.json`):
