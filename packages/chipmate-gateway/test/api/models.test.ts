// Verifies fetchChipMateModels typed result and 401 fallback behaviour.

import { test, expect } from "bun:test"
import { fetchChipMateModels, fetchChipMateTranscriptionModels } from "../../src/api/models.js"
...
test("returns empty models and error when both auth and public requests return 401", async () => {
  ...
  const result = await fetchChipMateModels({ chipmateToken: "bad-token" })
...
test("falls back to public endpoint on 401 and returns models", async () => {
  ...
  const result = await fetchChipMateModels({
    chipmateToken: "expired-token",
...
test("preserves Auto Efficient routing metadata as a dedicated model field", async () => {
  ...
  expect(result.error).toBeUndefined()
  expect(result.models["chipmate-auto/efficient"].autoRouting).toEqual({
    models: ["google/gemini-2.5-flash", "anthropic/claude-sonnet-4.6"],
  })
})
...
test("fetches and filters the transcription catalog", async () => {
  ...
  const result = await fetchChipMateTranscriptionModels({ chipmateToken: "token" })
...
test("keeps organization catalog errors from silently falling back to personal models", async () => {
  ...
  const result = await fetchChipMateTranscriptionModels({ chipmateToken: "token", chipmateOrganizationId: "org-1" })
  ...
  expect(calls[0]).toContain("/api/gateway/transcription-models")
  expect(headers[0]?.get("X-CHIPMATE-ORGANIZATIONID")).toBe("org-1")
})
