...
const path = "packages/chipmate-jetbrains/CHANGELOG.md"
...
if (values.help) {
  console.log(
    `Usage: bun .chipmate/skills/release-jetbrains/script/update-changelog.ts --version <version> --file <section.md>`,
  )
  process.exit(0)
}
...
async function temp(body: object) {
  const file = `/tmp/chipmate-jetbrains-changelog-${Date.now()}.json`
