...
`codebase_analysis`, the old autocomplete runtime, `/kilo/fim`, `/kilo/edit`,
...
| qwen-direct current behavior | `packages/kilo-vscode/src/services/qwen-autocomplete/**` |
...
`kilo.autocomplete.qwen.maxTokens`, and `maxPromptTokens` is the Phase 2C prompt
...
- Forbidden imports and systems remain absent:
  ...
  clipboard/static/recently visited context, Kilo RAG, `semantic_search`,
  `codebase_analysis`, Chat, Agent, CodeGraph, old runtime, `/kilo/fim`,
  `/kilo/edit`, `/v1/chat/completions`.

## Phase 2E-4 Validation Plan

Run from `packages/kilo-vscode` after this doc-only audit:
