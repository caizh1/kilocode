...
describe("DocumentAgentScope", () => {
  ...
  test("session allows cannot widen the document agent boundary", () => {
    const ruleset = KiloSessionPrompt.guardPermissions({
  ...
  test("scope control is invisible to existing agents", () => {
    ...
    expect(KiloToolRegistry.available(tool, agent("document"))).toBe(true)
    expect(KiloToolRegistry.available(tool, agent("ask"))).toBe(false)
    expect(KiloToolRegistry.available(tool, agent("code"))).toBe(false)
    expect(KiloToolRegistry.available(tool, agent("ultra"))).toBe(false)
  })
  ...
  test("scope metadata does not change existing QA-facing agent permissions", () => {
    ...
    for (const name of ["ask", "code", "ultra"]) {
      const base = KiloSessionPrompt.guardPermissions({
        agent: { name, permission: agentRules },
        session: { permission: sessionRules, metadata: {} },
      })
      const scoped = KiloSessionPrompt.guardPermissions({
        agent: { name, permission: agentRules },
        session: {
