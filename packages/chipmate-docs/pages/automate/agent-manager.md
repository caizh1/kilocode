...
The Agent Manager is a control panel for running and orchestrating multiple ChipMate agents, with support for parallel worktree-isolated sessions.

The Agent Manager is a **full-panel editor tab** built directly into the extension. It uses the extension's embedded runtime, so no separate ChipMate CLI installation or CLI authentication setup is required. It supports:
...
- Command Palette: "ChipMate: Open Agent Manager"
...
Managed worktrees are created under `.chipmate/worktrees/` in your project. ChipMate also stores Agent Manager UI state in `.chipmate/agent-manager.json`.
...
2. Enter a branch name (or let ChipMate generate one)
...
3. ChipMate creates one worktree + session per version and runs them in parallel
...
ChipMate can start Agent Manager sessions from chat with the `agent_manager` tool. It is available by default only in the VS Code extension because Agent Manager is an extension feature.
...
The dropdown choice is remembered per panel and becomes the default for new panels. You can also set the default directly with the `chipmate-code.new.agentManager.terminalButtonDestination` setting (`vscode` or `agentManager`). The `Cmd+/` (macOS) / `Ctrl+/` (Windows/Linux) shortcut follows the same destination.
...
Create a script file in `.chipmate/` using the appropriate filename for your platform:
...
ChipMate runs the script automatically whenever a new worktree is created. It uses `sh` for POSIX scripts, PowerShell for `.ps1`, and `cmd.exe` for `.cmd` / `.bat`, so executable permissions are not required.
...
# Copy a nested env file that ChipMate does not auto-copy.
...
Before the setup script runs, ChipMate automatically copies root-level `.env` files from the main repo into the new worktree.
...
- Files that already exist in the worktree, because ChipMate never overwrites them

Use `.chipmate/setup-script` for anything outside the automatic copy rules, including nested env files, ignored local config, local certificates, local database files, generated config directories, or tool-specific files required to run the project.
...
For example, on macOS / Linux create `.chipmate/run-script`:
...
Agent Manager state is persisted in `.chipmate/agent-manager.json`. It stores worktrees, sections, session tabs, ordering, collapsed state, diff preferences, and cached PR metadata. Git branches and worktree directories remain on disk separately.

Closing a managed worktree removes it from Agent Manager, deletes its `.chipmate/worktrees/` directory, and deletes the local branch. Closing an imported external worktree removes the Agent Manager entry but leaves the external directory and branch untouched.
