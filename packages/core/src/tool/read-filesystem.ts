...
// chipmate_change start - bind approved reads to filesystem identity
...
export const inspect = Effect.fn("ReadTool.inspect")(function* (fs: FSUtil.Interface, input: string) {
  ...
  // chipmate_change start - retain the approved identity for descriptor verification
...
// chipmate_change - reject targets replaced after permission approval
...
export const read = Effect.fn("ReadTool.read")(function* (
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      // chipmate_change start - open the approved path and verify the descriptor identity
...
// chipmate_change - verify approved directory identity before listing
