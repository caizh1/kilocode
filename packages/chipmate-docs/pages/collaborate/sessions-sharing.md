...
description: "Share and collaborate on ChipMate sessions"
---

# Sessions & Sharing

A session is your platform-agnostic interaction with ChipMate. It remembers your repository, your task, and the conversation so you can pause and resume work without losing context. Local sessions stay on the machine where ChipMate runs. Account-backed cloud sessions are private to your account by default. You can optionally share a session link with others who can read or fork it.
...
This information lets ChipMate show your recent sessions and continue right from the same context the next time you open it.
...
3. Interact with ChipMate via any of our interfaces- the CLI, the Cloud Agent, or the Extensions in your favorite IDE.
...
ChipMate stores local session history and metadata in an SQLite database on the machine where ChipMate runs. Cloud sessions and shared-session copies use ChipMate's cloud services and are separate from the local database. If you share a session, ChipMate keeps a public copy for the share link while the original session remains private.
