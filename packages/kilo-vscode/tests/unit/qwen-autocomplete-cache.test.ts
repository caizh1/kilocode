...
describe("KiloQwenInlineCompletionProvider cache integration", () => {
  ...
  it("does not cache guard-blocked, qwen-error, empty, or stale results", async () => {
    ...
    const blockedProvider = new KiloQwenInlineCompletionProvider({
      read: () => cfg,
      guard: () => true,
  ...
  it("does not reference legacy or chat endpoints from qwen cache code", () => {
    ...
    const source = [
      ...
      readFileSync(`${root}/KiloQwenInlineCompletionProvider.ts`, "utf8"),
    ].join("\n")
...
function complete(
  provider: KiloQwenInlineCompletionProvider,
