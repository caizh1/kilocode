...
} from "../../src/chipmate-provider/handlers/permission-handler"
...
describe("recoveryDirs", () => {
  ...
  it("returns workspace root plus each unique worktree directory", () => {
    const dirs = new Map([
      ["s1", "/workspace/.chipmate/worktrees/alpha"],
      ["s2", "/workspace/.chipmate/worktrees/beta"],
      ["s3", "/workspace/.chipmate/worktrees/alpha"],
    ])
    expect(recoveryDirs("/workspace", dirs)).toEqual([
      "/workspace",
      "/workspace/.chipmate/worktrees/alpha",
      "/workspace/.chipmate/worktrees/beta",
    ])
  })

  it("includes extra worktree directories", () => {
    const dirs = new Map([["s1", "/workspace/.chipmate/worktrees/alpha"]])
    expect(recoveryDirs("/workspace", dirs, ["/workspace/.chipmate/worktrees/beta", "/workspace"])).toEqual([
      "/workspace",
      "/workspace/.chipmate/worktrees/alpha",
      "/workspace/.chipmate/worktrees/beta",
    ])
...
describe("handlePermissionResponse", () => {
  it("uses the recorded SSE directory instead of a stale session fallback", async () => {
    ...
    permDirs.set("p1", "/workspace/.chipmate/worktrees/feature")

    await handlePermissionResponse(fake, "p1", "s1", "once", [], [])

    expect(replies).toEqual([{ requestID: "p1", reply: "once", directory: "/workspace/.chipmate/worktrees/feature" }])
  })
  ...
  it("saves selected rules and replies in the recorded SSE directory", async () => {
    ...
    expect(saves).toEqual([
      {
        requestID: "p1",
        directory: "/workspace/.chipmate/worktrees/feature",
        approvedAlways: ["bun *"],
        deniedAlways: ["rm *"],
      },
    ])
    expect(replies).toEqual([{ requestID: "p1", reply: "reject", directory: "/workspace/.chipmate/worktrees/feature" }])
  })
...
describe("fetchAndSendPendingPermissions", () => {
  ...
  it("queries workspace root plus each unique worktree directory", async () => {
    ...
    expect(queries).toContain("/workspace/.chipmate/worktrees/alpha")
    expect(queries).toContain("/workspace/.chipmate/worktrees/beta")
  ...
  it("queries extra Agent Manager worktree directories", async () => {
    const { fake, queries, permDirs } = ctx({
      tracked: ["s1"],
      extra: ["/workspace/.chipmate/worktrees/late"],
      permsPerDir: { "/workspace/.chipmate/worktrees/late": [pending("p1", "s1")] },
    })
    await fetchAndSendPendingPermissions(fake)
    expect(queries).toEqual(["/workspace", "/workspace/.chipmate/worktrees/late"])
    expect(permDirs.get("p1")).toBe("/workspace/.chipmate/worktrees/late")
  })

  it("preserves cached routes for directories that fail to list", async () => {
    const dirs = new Map([["s1", "/workspace/.chipmate/worktrees/failing"]])
    const error = new Error("temporary failure")
    const { fake, permDirs } = ctx({
      tracked: ["s1"],
      dirs,
      errors: { list: { "/workspace/.chipmate/worktrees/failing": error } },
    })
    const spy = spyOn(console, "error").mockImplementation(() => {})
    permDirs.set("workspace-stale", "/workspace")
    permDirs.set("worktree-pending", "/workspace/.chipmate/worktrees/failing")

    await fetchAndSendPendingPermissions(fake)
    spy.mockRestore()

    expect(permDirs.has("workspace-stale")).toBe(false)
    expect(permDirs.get("worktree-pending")).toBe("/workspace/.chipmate/worktrees/failing")
  })

  it("deduplicates directories", async () => {
    const dirs = new Map([
      ["s1", "/workspace/.chipmate/worktrees/alpha"],
      ["s2", "/workspace/.chipmate/worktrees/alpha"],
    ])
    const { fake, queries } = ctx({ tracked: ["s1", "s2"], dirs })
    await fetchAndSendPendingPermissions(fake)
    expect(queries.filter((d) => d === "/workspace/.chipmate/worktrees/alpha")).toHaveLength(1)
  })
