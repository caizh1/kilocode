...
class ChipMateWorkspaceServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeWorkspaceRpcApi
    private lateinit var service: ChipMateWorkspaceService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeWorkspaceRpcApi()
        service = ChipMateWorkspaceService(scope, rpc)
    }
    ...
    fun `test openPath opens first file match`() = runBlocking {
        rpc.fileMatches = listOf(
            WorkspaceFileDto("/test/.chipmate/plans/a.md", "a.md"),
            WorkspaceFileDto("/other/.chipmate/plans/a.md", "a.md"),
        )

        val ok = withContext(Dispatchers.Default) {
            service.openPath("/test", ".chipmate/plans/a.md")
        }

        assertTrue(ok)
        assertEquals(listOf("/test" to ".chipmate/plans/a.md"), rpc.fileCalls)
        assertEquals(listOf("/test/.chipmate/plans/a.md"), rpc.opened)
    }
    ...
    fun `test openPath returns false when no match exists`() = runBlocking {
        val ok = withContext(Dispatchers.Default) {
            service.openPath("/test", ".chipmate/plans/missing.md")
        }

        assertFalse(ok)
        assertEquals(listOf("/test" to ".chipmate/plans/missing.md"), rpc.fileCalls)
        assertTrue(rpc.opened.isEmpty())
    }

    fun `test openPath returns false when backend open fails`() = runBlocking {
        rpc.fileMatches = listOf(WorkspaceFileDto("/test/.chipmate/plans/a.md", "a.md"))
