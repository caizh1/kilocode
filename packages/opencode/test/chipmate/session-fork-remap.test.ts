...
const dbfile = path.join(os.tmpdir(), `chipmate-fork-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("Session.fork task children", () => {
  ...
  test(
    ...
    async () => {
      const flag = Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES
      ...
      try {
        ...
        Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES = flag
      }
    },
    { timeout: 30000 },
  )
