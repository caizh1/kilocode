...
import { memoryRow } from "@/chipmate/cli/cmd/tui/component/memory-status"
import { runMemoryCommand } from "@/chipmate/cli/cmd/tui/memory-command"
import { MemoryTuiEvents } from "@/chipmate/cli/cmd/tui/memory-events"
import { MemoryTuiState } from "@/chipmate/cli/cmd/tui/memory-state"
...
describe("memory TUI command parser", () => {
  ...
  test("status opens overview dialog", async () => {
    const shown: string[] = []
    const opened: string[] = []
    const client = { memory: {} } as unknown as ChipMateClient
  ...
  test("inspect reveals the memory folder", async () => {
    ...
    const client = {
      memory: {
        status: async () => ({ data: { root: "/tmp/chipmate-memory", state: { enabled: true } } }),
      },
    } as unknown as ChipMateClient
    ...
    expect(opened).toEqual(["/tmp/chipmate-memory"])
    expect(shown).toEqual(["Memory folder: /tmp/chipmate-memory"])
  })
  ...
  test("on and off call enable and disable endpoints", async () => {
    ...
    const client = {
      memory: {
        enable: async () => {
          calls.push("enable")
          return { data: { root: "/tmp/chipmate-data/memory/repo-abc123", index: { tokens: 42 } } }
        },
        disable: async () => {
          calls.push("disable")
