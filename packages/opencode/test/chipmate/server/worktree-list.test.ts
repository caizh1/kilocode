...
const state = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES
    Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.CHIPMATE_EXPERIMENTAL_WORKSPACES = original
        await resetDatabase()
      }),
    )
  }),
...
describe("ChipMate Console worktree listing", () => {
  run(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const directory = path.join(test.directory, ".chipmate", "worktrees", "console-list")
