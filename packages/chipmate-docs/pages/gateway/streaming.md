---
title: "Streaming"
description: "Learn how to implement real-time streaming responses with the ChipMate AI Gateway using Server-Sent Events (SSE)."
---

# Streaming

The ChipMate AI Gateway supports streaming responses from all models using Server-Sent Events (SSE). Streaming allows your application to display tokens as they're generated, providing a more responsive user experience.
...
const response = await fetch("https://api.chipmate.ai/api/gateway/chat/completions", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.CHIPMATE_API_KEY}`,
