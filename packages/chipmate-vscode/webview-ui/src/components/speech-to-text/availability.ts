...
export function hasSpeechToTextAccess(cfg: Cfg, auth: Readonly<Record<string, AuthState>>): boolean {
  ...
  const enabled = !cfg.enabled_providers || cfg.enabled_providers.includes(CHIPMATE_PROVIDER_ID)
  const type = auth[CHIPMATE_PROVIDER_ID]
  return enabled && !cfg.disabled_providers?.includes(CHIPMATE_PROVIDER_ID) && (type === "api" || type === "oauth")
}
