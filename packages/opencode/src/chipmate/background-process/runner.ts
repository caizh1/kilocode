import { ChipMatePtySelfCommand } from "@/chipmate/pty/self-command"
...
export namespace BackgroundProcessRunner {
  ...
  export function command(input: Input) {
    const self = ChipMatePtySelfCommand.command()
  ...
  export async function maybe(argv = process.argv) {
    ...
    process.env.CHIPMATE_BACKGROUND_PROCESS_TOKEN = token
