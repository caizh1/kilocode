---
title: "ChipMate CLI: Run the AI Coding Agent from Your Terminal"
description: "Using ChipMate from the command line"
platform: new
---

{% callout type="warning" title="Version Notice" %}
This documentation applies only to ChipMate version 1.0 and later. Users running versions below 1.0 should upgrade before proceeding.
{% /callout %}

# ChipMate CLI: AI Coding Agent in Your Terminal

Orchestrate agents from your terminal. Plan, debug, and code fast with keyboard-first navigation on the command line.

The ChipMate CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (ChipMate CLI 1.0):** [ChipMate-Org/chipmate](https://github.com/ChipMate-Org/chipmate) · [Report an issue](https://github.com/ChipMate-Org/chipmate/issues)
...
Change directory to where you want to work and run chipmate:
...
After installation, run `chipmate` and use the `/connect` command to add your first provider credentials. This is the interactive way to configure API keys for model providers.

## Update

Upgrade the ChipMate CLI:

`chipmate upgrade`

Or use npm:

`npm update -g @chipmate/cli`

## What you can do with ChipMate CLI
...
- **Switch between hundreds of LLMs without constraints.** Other CLI tools only work with one model or curate opinionated lists. With ChipMate, you can switch models without booting up another tool.
...
The `chipmate console` command and its browser interface are deprecated and will be removed in a future release. Use the CLI TUI, VS Code extension, or JetBrains plugin to run sessions. Use the CLI slash commands, other extensions, or edit ChipMate configuration files directly to manage settings.
{% /callout %}
...
#### ChipMate Gateway Commands (when connected)

| Command | Aliases | Description |
|---|---|---|
| `/profile` | `/me`, `/whoami` | View your ChipMate Gateway profile |
| `/teams` | `/team`, `/org`, `/orgs` | Switch between ChipMate Gateway teams |
...
- Config files in **`~/.config/chipmate/`**: use **`chipmate.jsonc`** for provider, model, permission, and **MCP** settings. Restart the CLI after editing. See [Using MCP in ChipMate](/docs/automate/mcp/using-in-chipmate-code) for MCP config format.
- **`tui.jsonc`** for terminal UI settings such as notifications, sounds, themes, and keybindings
- `chipmate auth` for credential management
...
- Edit `~/.config/chipmate/tui.jsonc` (or `tui.json`) for global settings.
- Edit `.chipmate/tui.json` (or `tui.jsonc`) for project settings.
...
Supported sound names are `default`, `question`, `permission`, `error`, `done`, and `subagent_done`. Relative paths are resolved from the directory containing the TUI configuration file. If an override cannot be loaded, ChipMate falls back to the active sound pack and then the built-in `opencode.default` pack.
...
ChipMate uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where ChipMate was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many bash commands).
...
The ChipMate CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @chipmate/cli` (ChipMate CLI 1.0) is built from [ChipMate-Org/chipmate](https://github.com/ChipMate-Org/chipmate). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).

### Config File Location (ChipMate CLI 1.0)

| Scope | Path |
|---|---|
| **Global** | `~/.config/chipmate/chipmate.json[c]` or legacy `opencode.json[c]` (Windows config dir may vary) |
| **Project** | `./chipmate.json[c]`, legacy `./opencode.json[c]`, or config inside `./.chipmate/` (legacy `./.chipmate/` is also read) |

Project-level configuration takes precedence over global settings.

{% callout type="warning" %}
**Migrating from opencode?** ChipMate no longer falls back to opencode configuration stored in `.opencode` directories (such as `~/.config/opencode` or a project `./.opencode/`). To keep using it, move your global config into `~/.config/chipmate/` and any project config into `./.chipmate/`.
{% /callout %}
...
Some terminals don't send modifier keys with Enter by default. Windows Terminal requires a one-time configuration to forward `Shift+Enter` as an escape sequence that ChipMate can read.
...
Save the file and restart Windows Terminal or open a new tab. `Shift+Enter` will now insert a newline in the ChipMate prompt instead of submitting the message.

### OpenTelemetry Export

ChipMate telemetry is enabled by default and can be disabled with `experimental.openTelemetry = false`:
...
`{env:VAR}` (and `{file:...}`) references are resolved **only** in trusted config: your global config (`~/.config/chipmate`), a config passed via `CHIPMATE_CONFIG` / `CHIPMATE_CONFIG_CONTENT`, or organization/MDM-managed config. A project-level `chipmate.json` / `opencode.json` committed to a repository **cannot** use `{env:VAR}` — the reference is ignored and a warning is logged. This prevents a malicious repository from exfiltrating your secrets to an attacker-controlled `baseURL` simply by being opened. `{file:...}` still works in project config, but only for files that resolve inside the project root — references that leave it (absolute paths outside the root, `../` traversal, and symlink escapes) are rejected.
{% /callout %}
...
Interactive mode is the default mode when running ChipMate without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode ChipMate will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
ChipMate only saves the pattern you select. Approving a specific command does not approve redirected variants or broader command patterns unless that broader option is shown and selected.

## Autonomous Mode (Non-Interactive)

Autonomous mode allows ChipMate to run in automated environments like CI/CD pipelines without requiring user interaction.

```bash
# Run in autonomous mode with a message
chipmate run --auto "Implement feature X"
```
...
- name: Run ChipMate
  run: |
    chipmate run "Implement the new feature" --auto
```
...
Remote Connections let you access your local CLI sessions from the Cloud Agents web interface. Requires [ChipMate Gateway](/docs/gateway) connection.
...
Requires connection to ChipMate Gateway. The `/remote` command appears only when authenticated.

**Enable by default:**

Add to `~/.config/chipmate/config.json`:
...
Once enabled, start a CLI session and open [Cloud Agents](https://app.chipmate.ai/cloud). Your local session appears in the dashboard. See [Cloud Agent Remote Connections](/docs/code-with-ai/platforms/cloud-agent#remote-connections) for details.

### Requirements

- Connection to ChipMate Gateway
- Same ChipMate account on CLI and Cloud Agent
- CLI must remain running with internet connection

{% callout type="warning" title="Security Warning" %}
Anyone with access to your ChipMate account can send messages to your computer when remote mode is enabled.
{% /callout %}
...
- `CHIPMATE_PROVIDER`: Override the active provider ID
- For `chipmate` provider: `CHIPMATE_<FIELD_NAME>` (e.g., `CHIPMATE_MODEL` → `chipmateModel`)
- For other providers: `CHIPMATE_<FIELD_NAME>` (e.g., `CHIPMATE_API_KEY` → `apiKey`)

## Using the CLI in an Organization

If you belong to a ChipMate organization (Team or Enterprise), you can route CLI requests through that organization. The process differs slightly between interactive and non-interactive usage.
...
### Non-Interactive Usage (`chipmate run`)

There is no `--org` or `--team` flag on `chipmate run`. Instead, the organization is determined from the following sources, in order of priority (highest first):

1. **`CHIPMATE_ORG_ID` environment variable** — Best for non-interactive and CI environments.
