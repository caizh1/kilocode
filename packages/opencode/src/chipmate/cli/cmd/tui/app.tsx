/**
 * ChipMate-specific TUI app customizations.
 *
...
import { isChipMateError, showChipMateErrorToast } from "@/chipmate/chipmate-errors"
import { registerChipMateCommands } from "@/chipmate/chipmate-commands"
import { initializeTUIDependencies } from "@chipmate/chipmate-gateway/tui"
import { DialogProcessList } from "@/chipmate/cli/cmd/tui/component/dialog-process-list"
import { useIndexingWarnings } from "@/chipmate/cli/cmd/tui/indexing-warning"
import { ChipMateTerminalTitle } from "./terminal-title"
import type { ChipMateTitleIcon } from "./title-icon"
import { Session as SessionApi } from "@/session/session"

// Re-export so upstream can render the route without importing directly
export { ChipMateClawView } from "@/chipmate/claw/view"
export { ChipMateTerminalTitle } from "./terminal-title"

// Hot reload TUI-local settings (keybinds/theme/ui) when changed from the ChipMate Console.
// Called from the App body (below SDKProvider and the TuiConfig provider).
export { useTuiConfigHotReload } from "@/chipmate/cli/cmd/tui/context/tui-config-hot-reload"
export { ChipMateTuiConfig } from "@/chipmate/cli/cmd/tui/context/tui-config"
...
export const APP_TITLE = "ChipMate CLI"

/** Public docs URL shown in the command palette. */
export const DOCS_URL = "https://chipmate.ai/docs"

/** Human-readable product name used in user-facing messages. */
export const APP_NAME = "ChipMate"
...
export function useSessionEffects(deps: {
  ...
  const pty = process.env.CHIPMATE_PTY_ID
...
export function getTerminalTitle(input: {
  ...
  icon?: ChipMateTitleIcon.Value
}): ChipMateTerminalTitle.Result | undefined {
  if (input.route.data.type === "home") {
    return {
      title: ChipMateTerminalTitle.format({ base: input.base, indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
  ...
  if (input.route.data.type === "session") {
    const state = ChipMateTerminalTitle.session({
      base: input.base,
    ...
    return {
      ...state,
      title: ChipMateTerminalTitle.format({ base: input.base, title, indicator: state.indicator, icon: input.icon }),
    }
  }

  if (input.route.data.type === "plugin") {
    return {
      title: ChipMateTerminalTitle.format({
        base: input.base,
  ...
  if (input.route.data.type === "chipmateclaw") {
    return {
      title: ChipMateTerminalTitle.format({ base: input.base, title: "ChipMateClaw", indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
...
/**
 * Intercepts ChipMate-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && isChipMateError(error as any)) {
    showChipMateErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into chipmate-gateway
 * - Registers ChipMate Gateway commands (profile, teams, chipmateclaw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for chipmate-gateway
  ...
  // Register ChipMate Gateway commands (profile, teams, chipmateclaw, remote, etc.)
  registerChipMateCommands(useSDK)
  ...
  useBindings(() => ({
    commands: [
      ...
      },
      {
        ...
        // chipmate_change - the saved rule is server side, so it also stops VS Code, JetBrains and
