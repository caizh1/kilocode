...
class EmptySessionPanel(
    ...
    init {
        ...
        val logo = JBLabel(
            IconLoader.getIcon("/icons/chipmate-content.svg", EmptySessionPanel::class.java),
        ).apply {
            horizontalAlignment = JBLabel.CENTER
        }
    ...
    internal fun feedbackText() = ChipMateBundle.message("feedback.button")
    ...
    internal fun explanationText() = ChipMateBundle.message("session.empty.welcome")
    ...
    internal open class ShowHistoryButton(
        text: String = ChipMateBundle.message("session.showHistory"),
    ...
    private fun welcomeHtml() = XmlStringUtil.wrapInHtml(
        "<div style='text-align:center'>${XmlStringUtil.escapeString(ChipMateBundle.message("session.empty.welcome"))}</div>"
    )
