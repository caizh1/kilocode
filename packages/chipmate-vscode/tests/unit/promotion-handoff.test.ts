...
describe("promotion handoff", () => {
  it("describes the new worktree location", () => {
    const text = handoffText({ directory: "/repo/.chipmate/worktrees/feature", branch: "feature/test" })

    expect(text).toContain("This session was moved to a git worktree.")
    expect(text).toContain("Use this as the current working directory: /repo/.chipmate/worktrees/feature")
    ...
    expect(promptAsync).toHaveBeenCalledWith(
      {
        ...
        parts: [
          {
            type: "text",
            text: handoffText({ directory: "/repo/.chipmate/worktrees/feature", branch: "feature/test" }),
            synthetic: true,
          },
        ],
      },
