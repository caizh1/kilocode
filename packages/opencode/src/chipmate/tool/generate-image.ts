...
import { CHIPMATE_OPENROUTER_BASE } from "@chipmate/chipmate-gateway"
import DESCRIPTION from "./generate-image.txt"

const log = Log.create({ service: "tool.generate_image" })

const CHIPMATE_OPENROUTER_URL = `${CHIPMATE_OPENROUTER_BASE}/chat/completions`
...
function buildRequest(resolved: ResolvedProvider, prompt: string, model: string, inputImage?: string): ResolvedRequest {
  ...
  if (resolved.organizationId) headers["X-CHIPMATE-ORGANIZATIONID"] = resolved.organizationId
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const auth = yield* authSvc.get("chipmate")
          ...
          if (!resolved) {
            return {
              ...
              output:
                "No image generation provider available. Log in to ChipMate or set OPENROUTER_API_KEY, then try again.",
