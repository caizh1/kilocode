---
title: "Using MCP in ChipMate"
description: "How to use MCP servers in ChipMate"
---

# Using MCP in ChipMate

Model Context Protocol (MCP) extends ChipMate's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with ChipMate.
...
MCP server configurations are stored inside the main ChipMate config file. There are two levels:

1. **Global Configuration**: `~/.config/chipmate/chipmate.jsonc` — applies to all projects.
2. **Project-level Configuration**: `chipmate.jsonc` in your project root, or `.chipmate/chipmate.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the ChipMate settings UI:
...
If the UI cannot add a server, edit a ChipMate config file directly and add the server under the top-level `mcp` key. For project-specific servers, edit `./chipmate.json` or `./chipmate.jsonc` if your project already has one; otherwise use `./.chipmate/chipmate.json` or `./.chipmate/chipmate.jsonc` for a cleaner setup. For servers you want in every workspace, use `~/.config/chipmate/chipmate.json` or `~/.config/chipmate/chipmate.jsonc`.

### Config Format

MCP servers are configured under the `mcp` key in `chipmate.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, ChipMate will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.

{% /tab %}
{% tab label="CLI" %}

The CLI accepts several config filenames. The recommended file is `chipmate.json`:

| Scope | Recommended Path | Also supported |
|---|---|---|
| **Global** | `~/.config/chipmate/chipmate.json` | `chipmate.jsonc`, `opencode.json`, `opencode.jsonc`, `config.json` |
| **Project** | `./chipmate.json` or `./.chipmate/chipmate.json` | `chipmate.jsonc`, `opencode.jsonc`, `opencode.json` |
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. ChipMate tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| Command | Description |
|---|---|
| `chipmate mcp list` | List all configured MCP servers |
| `chipmate mcp add` | Add an MCP server |
| `chipmate mcp auth` | Authenticate with an MCP server |
| `chipmate mcp logout` | Log out from an MCP server |
| `chipmate mcp debug` | Debug an MCP server connection |
...
Run `chipmate mcp list` to verify the server status.
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `chipmate.jsonc`:
...
ChipMate does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **ChipMate Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [ChipMate Marketplace](https://github.com/ChipMate-Org/chipmate-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask ChipMate:** You can ask ChipMate to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend ChipMate with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, ChipMate will automatically detect available tools and resources. To use them:

1. Type your request in the ChipMate chat interface
2. ChipMate will identify when an MCP tool can help with your task
...
When a connected MCP server provides instructions, ChipMate adds them to the model context so the agent can follow the server's usage guidance. ChipMate omits those instructions when every tool from that server is denied.

Resource-capable servers also make the `list_mcp_resources`, `list_mcp_resource_templates`, and `read_mcp_resource` tools available to the agent. Resource templates describe parameterized URIs; the agent fills in a template, then reads the resulting resource URI. Resource listing and reads use ChipMate's normal read approval flow.
...
- **Server Not Responding:** Check if the server process is running. Use `chipmate mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `chipmate.jsonc` config or via `{env:VARIABLE_NAME}` references.
