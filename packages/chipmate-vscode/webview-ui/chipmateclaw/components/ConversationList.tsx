// ChipMateClaw conversation sidebar — mirrors the web UI in
// cloud/apps/web/src/app/(app)/claw/chipmate-chat/components/ConversationList.tsx
...
export function ConversationList() {
  ...
  const groups = createMemo(() =>
    groupConversations(claw.conversations(), {
      today: t("chipmateClaw.conversations.groupToday"),
      yesterday: t("chipmateClaw.conversations.groupYesterday"),
      week: t("chipmateClaw.conversations.groupThisWeek"),
      older: t("chipmateClaw.conversations.groupOlder"),
    }),
  )
  ...
  return (
    <div class="chipmateclaw-convlist">
      <div class="chipmateclaw-convlist-header">
        <span class="chipmateclaw-convlist-title">{t("chipmateClaw.conversations.title")}</span>
        <button
          type="button"
          class="chipmateclaw-iconbtn"
          onClick={() => claw.createConversation()}
          aria-label={t("chipmateClaw.conversations.new")}
          title={t("chipmateClaw.conversations.new")}
        >
          +
        </button>
      </div>
      <div class="chipmateclaw-convlist-scroll" ref={scrollEl}>
        <Show
          when={claw.conversations().length > 0}
          fallback={<div class="chipmateclaw-convlist-empty">{t("chipmateClaw.conversations.empty")}</div>}
        >
          <For each={groups()}>
            {(group) => (
              <div class="chipmateclaw-convlist-group">
                <div class="chipmateclaw-convlist-grouplabel">{group.label}</div>
...
function ConversationItem(props: { conversation: ConversationListItem }) {
  ...
  return (
    <div
      class={`chipmateclaw-convitem ${isActive() ? "chipmateclaw-convitem-active" : ""}`}
      onClick={() => {
    ...
    >
      <Show
        ...
        fallback={
          <input
            ref={inputEl}
            class="chipmateclaw-convitem-renameinput"
            value={renameText()}
      ...
      >
        <span class="chipmateclaw-convitem-title">
          <Show when={isUnread()}>
            <span class="chipmateclaw-convitem-unread" aria-hidden="true" />
          </Show>
          {props.conversation.title ?? t("chipmateClaw.conversations.untitled")}
        </span>
      </Show>
      <div class="chipmateclaw-convitem-actions" onClick={(e) => e.stopPropagation()}>
        <button
          type="button"
          class="chipmateclaw-iconbtn-sm"
          onClick={startRename}
          title={t("chipmateClaw.conversations.rename")}
          aria-label={t("chipmateClaw.conversations.rename")}
        >
          ✎
        </button>
        <button
          ...
          title={t("chipmateClaw.conversations.leave")}
          aria-label={t("chipmateClaw.conversations.leave")}
        >
          ×
        </button>
      </div>
