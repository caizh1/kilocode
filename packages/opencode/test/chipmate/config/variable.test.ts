...
import { ConfigVariableGuard } from "@/chipmate/config/variable"
...
test("rejects untrusted file references that escape the scope root", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-root-"))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-outside-"))
...
test("allows untrusted file references that stay inside the scope root", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-inside-")))
  ...
  try {
    expect(
      await ConfigVariable.substitute({
        ...
        dir: root,
        text: "{file:value}",
        fileScope: { root, source: path.join(root, "chipmate.json") },
      }),
    ).toBe("allowed")
  } finally {
    await fs.rm(root, { recursive: true, force: true })
...
test("allows untrusted absolute file references that resolve inside the scope root", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-abs-inside-")))
  const file = path.join(root, "value")
  await fs.writeFile(file, "allowed")
  try {
...
test("rejects server credential environment substitutions", async () => {
  await expect(
    ConfigVariable.substitute({
      ...trusted,
      text: "password={env:CHIPMATE_SERVER_PASSWORD}",
      env: { CHIPMATE_SERVER_PASSWORD: "secret" },
    }),
  ).rejects.toBeInstanceOf(InvalidError)
})
...
test("reads ordinary file substitutions on every platform when trusted", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-file-"))
...
test.skipIf(process.platform !== "linux")("does not substitute an environment file through a symlink", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-"))
...
test("scope-blocked file reference still rejects under missing:empty", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-empty-root-")))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-empty-out-"))
  const file = path.join(outside, "secret")
  await fs.writeFile(file, "top-secret")
  try {
    await expect(
...
test("missing (non-blocked) file reference is emptied under missing:empty", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-config-variable-missing-")))
  try {
...
test("guard read rejects an out-of-scope file with a BlockedError", async () => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-guard-root-")))
  const outside = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-guard-out-"))
  const file = path.join(outside, "secret")
  await fs.writeFile(file, "top-secret")
  try {
    const err = await ConfigVariableGuard.read(file, { root, source: "chipmate.json", token: "{file:...}" }).then(
      () => undefined,
      (e) => e,
    )
