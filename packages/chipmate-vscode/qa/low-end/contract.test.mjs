...
test("keeps the agreed proxy controls and absolute budgets executable", () => {
  ...
  assert.equal(budgets.settingsScrollP95Ms, 20)
  assert.equal(budgets.settingsScrollMaxMs, 50)
  assert.equal(budgets.settingsScrollOver32Percent, 1)
  ...
  assert.match(runner, /if \(arg === "--settings-only"\) result\.settingsOnly = true/)
  assert.match(runner, /for \(let cycle = 0; cycle < 5; cycle\+\+\)/)
  assert.match(runner, /}, 250\)/)
