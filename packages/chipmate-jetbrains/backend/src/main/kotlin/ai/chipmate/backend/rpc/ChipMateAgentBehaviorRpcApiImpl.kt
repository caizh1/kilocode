...
import ai.chipmate.backend.cli.ChipMateClaudeCompatSettings
...
import ai.chipmate.rpc.dto.AgentCreateDto
import ai.chipmate.rpc.dto.AgentDetailDto
import ai.chipmate.jetbrains.api.model.AgentBuilderSaveRequest
import ai.chipmate.rpc.dto.CommandFileDto
import ai.chipmate.rpc.dto.ConfigPatchDto
import ai.chipmate.rpc.dto.McpConfigDto
import ai.chipmate.rpc.dto.McpServerConfigDto
...
class ChipMateAgentBehaviorRpcApiImpl(private val backend: ChipMateBackendAppService? = null) : ChipMateAgentBehaviorRpcApi {
    companion object {
        private val LOG = ChipMateLog.create(ChipMateAgentBehaviorRpcApiImpl::class.java)
    ...
    private val app: ChipMateBackendAppService get() = backend ?: service()
    ...
    override suspend fun agents(directory: String): List<AgentDetailDto> {
        ...
        val removable = ChipMateCliDataParser.parseAgentRemovable(request(directory, "/agent", null))
    ...
    override suspend fun skills(directory: String): List<SkillDto> {
        val items = ChipMateCliDataParser.parseAgentBehaviorSkills(request(directory, "/skill", null))
    ...
    override suspend fun removeSkill(directory: String, location: String): Boolean =
        post(directory, "/chipmate/skill/remove", JsonObject(mapOf("location" to JsonPrimitive(location))))
    ...
    override suspend fun removeAgent(directory: String, name: String): Boolean =
        post(directory, "/chipmate/agent/remove", JsonObject(mapOf("name" to JsonPrimitive(name))))
    ...
    override suspend fun commands(directory: String) = ChipMateCliDataParser.parseAgentBehaviorCommands(request(directory, "/command", null))

    override suspend fun commandFiles(directory: String): List<CommandFileDto> =
        ChipMateCliDataParser.parseAgentBehaviorCommandFiles(request(directory, "/chipmate/command/files", null))

    override suspend fun removeCommand(directory: String, location: String): Boolean =
        post(directory, "/chipmate/command/remove", JsonObject(mapOf("location" to JsonPrimitive(location))))
    ...
    override suspend fun mcpStatus(directory: String) = ChipMateCliDataParser.parseMcpStatus(request(directory, "/mcp", null)).also { items ->
    ...
    override suspend fun mcpConfig(directory: String): Map<String, McpServerConfigDto> {
        ...
        val workspace = if (directory.isBlank()) emptyMap() else try {
            ChipMateCliDataParser.parseConfig(request(directory, "/config", null)).mcp
        } catch (e: Exception) {
    ...
    override suspend fun saveMcp(directory: String, name: String, scope: String, config: McpConfigDto?): Boolean {
        ...
        if (scope == "workspace") {
            patchConfig("/config?directory=${encode(directory)}", ChipMateCliDataParser.buildConfigPatch(patch))
        } else {
    ...
    override suspend fun claudeCodeCompat(): Boolean = ChipMateClaudeCompatSettings.get()

    override suspend fun setClaudeCodeCompat(value: Boolean): Boolean {
        ChipMateClaudeCompatSettings.set(value)
        app.restart()
        return value
    }
    ...
    private suspend fun commandRoots(directory: String): Set<Path> = buildSet {
        ...
        val config = paths?.let(ChipMateCliDataParser::parsePathConfig)
        if (config != null) addConfigCommandRoots(this, config)
        val home = paths?.let(ChipMateCliDataParser::parsePathHome)
    ...
    private fun addHomeCommandRoots(roots: MutableSet<Path>, home: String) {
        ...
        for (cfg in listOf(".chipmate")) addConfigCommandRoots(roots, base.resolve(cfg).toString())
    }
    ...
    private fun urlCached(path: Path): Boolean {
        return cacheRoots().any { root -> path.startsWith(root.resolve("chipmate").resolve("skills").normalize()) }
    }
