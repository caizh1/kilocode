...
Phase 2D ports the source-confirmed `AutocompleteLruCache` prefix/completion matching semantics into a qwen-owned in-memory adapter. It still does not implement SQLite persistence, `GeneratorReuseManager`, `CompletionStreamer`, streaming, snippets/context retrieval, document.version/model/config hash keys, backward deletion reuse, or old runtime activation.
...
| `streamFilters.ts` / `filterQwenCompletion` | Apply Continue-like filter subset after full response. | `CompletionStreamer`, `StreamTransformPipeline`, `lineStream.ts`; commit `eaa23c5a`. | C | No | Partial | Converts async stream filters to post-response string filtering; cannot call upstream `fullStop`. | qwen-direct endpoint is non-streaming. | High | Replace with Continue parity | Streaming phase |
| `streamFilters.ts` / `stopAtStopTokensText` | Trim text at configured stop tokens. | Continue `completionOptions.stop` and stream transforms. | B | No | Partial | Done after model response; upstream can stop generation earlier. | Non-streaming qwen transport. | Medium | Adapter acceptable, document tests | 2D-0 |
| `streamFilters.ts` / `stopAtStartOfText` | Stop if completion starts echoing suffix. | Continue stream transform behavior around suffix overlap. | C | No | Partial | Local string algorithm approximates streaming character transform. | Non-streaming qwen transport. | High | Replace with Continue parity | Streaming phase |
| `streamFilters.ts` / `stopAtLines`, `validatePatternInLine`, `stopAtLinesExact` | Stop at known stop lines and line below cursor. | Local `continuedev/core/autocomplete/filtering/streamTransforms/lineStream.ts`; commit `eaa23c5a`. | C | No | Partial | Same broad checks but array/string based; no `fullStop`. | Non-streaming qwen transport. | Medium | Adapter acceptable, document tests | 2D-0 |
| `streamFilters.ts` / `stopAtRepeatingLines` | Remove excessive repeated lines. | `lineStream.ts` / `stopAtRepeatingLines`; commit `eaa23c5a`. | C | No | Partial | Same threshold shape, post-response only. | Non-streaming qwen transport. | Medium | Adapter acceptable, document tests | 2D-0 |
| `streamFilters.ts` / `avoidEmptyComments`, `avoidPathLine`, `skipPrefixes`, `noDoubleNewLine`, `stopAtSimilarLine` | Filter common bad display output. | `lineStream.ts` functions; commit `eaa23c5a`. | C | No | Partial | qwen hardcodes C/C++ comment marker and applies after full text. | qwen-direct C/C++ baseline. | Medium | Adapter acceptable, document tests | 2D-0 |
...
| `multiline.ts` / `shouldCompleteMultilineQwen` | Decide whether qwen may show multiline text. | Upstream `classification/shouldCompleteMultiline.ts` and `AutocompleteLanguageInfo`; commit `eaa23c5a`. | C | No | Partial | Simplified to always/never/auto, selected means true, C/C++ line comment false, otherwise true. | Phase 1 C/C++ qwen baseline. | High | Replace with Continue parity | Later multiline phase |
| `guard.ts` / `shouldGuardQwenDocument` | Deny non-file/outside workspace/ignored files. | Continue `isSecurityConcern` and old runtime `FileIgnoreController`; commit `eaa23c5a`. | C | No | Partial | Uses `.kilocodeignore`/`.gitignore` shim and workspace containment; not full Continue security concern parity. | Kilo file access safety without full Continue runtime. | High | Replace with Continue parity | Later safety phase |
...
| `prefilter.ts` / `QWEN_DOCUMENT_SELECTOR` | Limit qwen provider to file-backed C/C++/headers. | Upstream `shouldPrefilter`, language config, and onlyMyCode/disabled-language options. | C | No | Partial | Hardcoded C/C++ selector and local non-code exclusions; no full config parity. | Company qwen C/C++ endpoint target. | Medium | Audit further | 2D-0 |
| `prefilter.ts` / `isQwenSupportedDocument`, `shouldPrefilterQwenDocument` | Reject unsupported/empty docs before request. | Upstream `shouldPrefilter`; commit `eaa23c5a`. | C | No | Partial | qwen-specific scheme/ext/language rules; no full Continue prefilter configuration. | Narrow qwen-direct scope. | Medium | Audit further | 2D-0 |
| `config.ts` / `readQwenAutocompleteConfig` | Read qwen autocomplete settings. | Continue `DEFAULT_AUTOCOMPLETE_OPTS`, config handler, LLM autocomplete options; commit `eaa23c5a`. | B | No | Partial | Kilo settings namespace, direct endpoint/model/apiKey, diagnostics settings. Phase 2C maps `maxPromptTokens=1024`, `prefixPercentage=0.3`, `maxSuffixPercentage=0.2`, `modelTimeout=150`, while `maxTokens` remains output-token request budget. Phase 2D adds `cache.enabled` and clamped `cache.maxEntries`; no TTL. Phase 2E-4 adds explicit `contextLength=0` and `recentlyEdited.injectIntoPrompt=false`. | VSIX and qwen-direct setup; qwen-direct cannot infer Continue `ILLM.contextLength`, so snippet injection stays off until user sets it. | Low | Adapter acceptable, document tests | 2C/2D/2E-4 |
...
Function/module ledger counts after Phase 2D: A = 8, B = 25, C = 12, D = 0 in the table above. Missing Continue autocomplete capability gaps are listed separately below as D-class gaps.

Overall audit counts after Phase 2D including missing autocomplete capabilities: A = 8, B = 25, C = 12, D = 18. The out-of-scope Chat/QA/Agent/Edit row is not counted as a D autocomplete gap.
...
| Non-streaming-compatible stream filter subset | qwen endpoint is currently `/v1/completions` with `stream: false`. | Continue filters are async stream transforms and can call `fullStop` to stop generation. | High latency and quality risk because bad generations are truncated only after completion. | Replace with Continue parity when streaming is available | Streaming phase |
| Lightweight ignore/security guard | Needed safety before enabling qwen requests. | Continue has `isSecurityConcern`, config-aware prefiltering, and old runtime file ignore controller behavior. | High security/explainability risk if local allow/deny differs. | Replace with Continue parity | Later safety phase |
...
| Simplified multiline classifier | Phase 1 limited qwen to C/C++ and basic comment suppression. | Continue classifier uses richer language metadata and context. | High display-stability risk, especially around multiline acceptance/rejection. | Replace with Continue parity | Later multiline phase |
...
| Full stream filters | Yes | Yes, later | High | Streaming phase | Yes | Current subset cannot call `fullStop`. |
...
| Recently opened files | Yes | Yes | Medium | 2E | No | Continue snippet source. |
...
| Full ignore/security concern parity | Yes | Yes | High | Later safety phase | No | `isSecurityConcern` plus file ignore behavior should be reconciled. |
