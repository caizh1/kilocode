...
function directory(headers?: HeadersInit): string {
  const value = new Headers(headers).get("x-chipmate-directory")
...
describe("ChipMateProvider indexing refresh", () => {
  it("keeps indexing idle when no workspace folder is open", async () => {
    ...
    try {
      ...
      const provider = new ChipMateProvider(
        {} as never,
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    const worktree = "/repo/中文 workspace/.chipmate/worktrees/feature"
    ...
    try {
      ...
      const auth = Buffer.from("chipmate:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-chipmate-directory")).toBe(encodeURIComponent(worktree))
    } finally {
      globalThis.fetch = original
    }
  })
  ...
  it("uses the panel project instead of the current session worktree", async () => {
    ...
    try {
      const conn = createIndexingConnection()
      const provider = new ChipMateProvider({} as never, conn.service as never, undefined, {
      ...
      expect(new Headers(calls[0]?.headers).get("x-chipmate-directory")).toBe(encodeURIComponent("/repo-b"))
    } finally {
      globalThis.fetch = original
    }
  })
  ...
  it("keeps two project panels isolated on a shared event dispatcher", () => {
    const conn = createIndexingConnection()
    const a = new ChipMateProvider({} as never, conn.service as never, undefined, { projectDirectory: "/repo-a" })
    const b = new ChipMateProvider({} as never, conn.service as never, undefined, { projectDirectory: "/repo-b" })
  ...
  it("restores a persisted local target after provider recreation", async () => {
    ...
    try {
      ...
      const first = new ChipMateProvider({} as never, old.service as never)
      ...
      const second = new ChipMateProvider({} as never, next.service as never)
  ...
  it("rebuilds documents and accepts the returned status for the panel project", async () => {
    ...
    try {
      ...
      expect(new Headers(calls[0]?.init?.headers).get("x-chipmate-directory")).toBe(encodeURIComponent("/repo-b"))
