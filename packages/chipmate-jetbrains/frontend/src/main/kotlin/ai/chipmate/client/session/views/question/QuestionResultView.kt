...
class QuestionResultView(tool: Tool, private val selection: SessionSelection? = null) : PartView() {
    ...
    fun bodyText(): String = result.questions.mapIndexed { i, q ->
        ...
        listOf(q, joined.ifBlank { ChipMateBundle.message("session.question.review.notAnswered") }).joinToString("\n")
    }.joinToString("\n")
    ...
    private fun syncLabels() {
        title.text = ChipMateBundle.message("session.question.result.title")
        val count = result.answers.count { it.isNotEmpty() }
        sub.text = ChipMateBundle.message("session.question.result.answered", count)
