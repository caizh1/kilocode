...
Popular choices:
- **ChipMate Auto Frontier** — highest quality models, best results (recommended)
- **ChipMate Auto Efficient** — cheapest model proven accurate enough for each task, with capability matched to difficulty (minimum for Gas Town)
...
Adding a GitHub PAT ensures that all commits, branches, and PRs created by your agents appear as **you** in git history. Without it, activity shows up under the ChipMate GitHub App bot account.
{% /callout %}
...
- PRs show as created by the ChipMate bot
- Some `gh` CLI operations may not work

### GitHub App Installation

The [ChipMate GitHub App](https://github.com/apps/chipmate-code) provides base-level repository access. It's installed per-organization or per-repository and gives agents read/write access to code, PRs, and issues.
