...
describe("DirectoryScanner", () => {
  ...
  test("skips files matched by .chipmateignore during full scans", async () => {
    const root = await mkdtemp(join(tmpdir(), "scanner-test-"))
    const cacheDir = await mkdtemp(join(tmpdir(), "scanner-cache-"))
    const blocked = join(root, "blocked.ts")
    const open = join(root, "open.ts")

    await Bun.write(join(root, ".chipmateignore"), "blocked.ts\n")
  ...
  test("skips plugin state directories during full scans", async () => {
    ...
    const nested = join(root, ".chipmate", "worktrees", "feature", "copy.c")

    await mkdir(join(root, ".chipmate", "worktrees", "feature"), { recursive: true })
  ...
  test("skips files matched by nested .chipmateignore during full scans", async () => {
    const root = await mkdtemp(join(tmpdir(), "scanner-test-"))
    const cacheDir = await mkdtemp(join(tmpdir(), "scanner-cache-"))
    try {
      ...
      await Bun.write(join(dir, ".chipmateignore"), "blocked.ts\n")
      await Bun.write(blocked, "export const blocked = 1\n")
      await Bun.write(open, "export const open = 1\n")
