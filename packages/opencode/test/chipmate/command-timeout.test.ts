...
import { CommandTimeout } from "@/chipmate/command-timeout"
...
const max = process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS
const msg = process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS_MESSAGE
...
afterEach(() => {
  if (max === undefined) delete process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS
  else process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS = max
  if (msg === undefined) delete process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS_MESSAGE
  else process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS_MESSAGE = msg
})
...
describe("CommandTimeout", () => {
  ...
  test("resolves hosted timeout policy", () => {
    for (const value of [undefined, "0", "-1", "abc"]) {
      if (value === undefined) delete process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS
      else process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS = value
      expect(CommandTimeout.env()).toBeUndefined()
    }

    process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS = "250"
    process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS_MESSAGE = " You're running in a sandbox. "
  ...
  test("does not expose the backend password to command substitutions", async () => {
    delete process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS
    const secret = process.env.CHIPMATE_SERVER_PASSWORD
    process.env.CHIPMATE_SERVER_PASSWORD = "must-not-leak"
    try {
      const exe = JSON.stringify(process.execPath)
      const script = JSON.stringify("process.stdout.write(process.env.CHIPMATE_SERVER_PASSWORD || 'clean')")
      ...
      if (secret === undefined) delete process.env.CHIPMATE_SERVER_PASSWORD
      else process.env.CHIPMATE_SERVER_PASSWORD = secret
    }
  })
  ...
  it.effect("caps output draining at the exact environment deadline", () =>
    Effect.gen(function* () {
      process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS = "25"
  ...
  it.effect("captures command substitutions concurrently without stderr", () =>
    Effect.gen(function* () {
      process.env.CHIPMATE_COMMAND_TIMEOUT_MAX_MS = "500"
