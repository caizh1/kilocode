...
import { Reference } from "../reference" // chipmate_change
...
export const Input = Schema.Struct({
  ...
  limit: FileSystem.SearchLimit.pipe(Schema.optional).annotate({ // chipmate_change
    description: "Maximum matches to return",
  }),
})

// chipmate_change start - retain bounded-search status in tool results and model output
...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const global = yield* Global.Service // chipmate_change
    const ripgrep = yield* Ripgrep.Service
    const location = yield* Location.Service
    const references = yield* Reference.Service // chipmate_change
      ...
      .register({
        [name]: Tool.make({
          ...
          toModelOutput: ({ output }) => [
            {
              type: "text",
              // chipmate_change start - model paths remain absolute while the typed result retains metadata
              text: toModelOutput({
                ...output,
            ...
            Effect.gen(function* () {
              yield* permission.assert({
                ...
                metadata: {
                  root: ".",
                  path: input.path,
                  reference: input.reference, // chipmate_change
              ...
              // chipmate_change start - enforce the active Location despite RelativePath being a nominal brand
              ...
              return yield* ripgrep
                ...
                .pipe(
                  // chipmate_change start - preserve search status after canonical path mapping
                  Effect.map(
                    (result) =>
                      new Result({
                        ...result,
...
export const node = makeLocationNode({
  name: "tool/grep",
  layer,
  deps: [ToolRegistry.node, FSUtil.node, Ripgrep.node, Location.node, PermissionV2.node, Global.node, Reference.node], // chipmate_change - search targets and references
})
