package ai.chipmate.client.session.ui.header

import ai.chipmate.client.plugin.ChipMateBundle
import ai.chipmate.client.session.model.SessionHeaderSnapshot
...
import ai.chipmate.client.session.views.todo.TodoListPanel
...
class SessionHeaderPanel(
    ...
    companion object {
        ...
        internal const val EXPANDED_KEY = "chipmate.session.header.expanded"
    }
    ...
    private val compact = HoverIcon().apply {
        icon = COMPRESS_ICON
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = ChipMateBundle.message("session.header.compact.description")
        accessibleContext.accessibleName = ChipMateBundle.message("session.header.compact")
    ...
    private val expand = JBLabel().apply {
        ...
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = ChipMateBundle.message("session.header.expand")
        accessibleContext.accessibleName = ChipMateBundle.message("session.header.expand")
        addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(event: MouseEvent) {
                toggle()
            }
    ...
    private val tokenTitle = JBLabel(ChipMateBundle.message("session.header.tokens"))
    ...
    private val todoRow = JPanel(FlowLayout(FlowLayout.LEFT, UiStyle.Gap.sm(), 0)).apply {
        ...
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = ChipMateBundle.message("session.header.todos.toggle")
        accessibleContext.accessibleName = ChipMateBundle.message("session.header.todos.toggle")
        add(todoArrow)
        add(todos)
    }
    ...
    private fun setTokens(value: TokensDto?) {
        ...
        tokenTitle.text = ChipMateBundle.message("session.header.tokens")
        tokens.toolTipText = ChipMateBundle.message("session.header.tokens.description")
        tokenTitle.isVisible = total > 0
        set(input, if (sent > 0) num(sent) else null)
        set(output, if (received > 0) num(received) else null)
        set(cacheWrite, if (write > 0) ChipMateBundle.message("session.header.cache.write", num(write)) else null)
        set(cacheRead, if (read > 0) ChipMateBundle.message("session.header.cache.read", num(read)) else null)
    ...
    private fun setCost(value: String?) {
        ...
        cost.text = costValue
        cost.icon = null
        val tip = costValue.takeIf { it.isNotBlank() }?.let { ChipMateBundle.message("session.header.cost.tooltip", it) }
    ...
    private fun setExpand(expanded: Boolean) {
        ...
        expand.toolTipText = ChipMateBundle.message(key)
        expand.accessibleContext.accessibleName = ChipMateBundle.message(key)
    }
