 ...
 *
 * Uses a full ChipMateProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, ChipMateProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("chipmate.v2.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      ...
      try {
        ...
        void client.session
          ...
          .catch((err: unknown) => {
            console.error("[ChipMate New] SubAgentViewerProvider: Failed to load session metadata:", err)
          })
      } catch (err) {
        console.error("[ChipMate New] SubAgentViewerProvider: Failed to load session metadata:", err)
      }
    ...
    panel.onDidDispose(() => {
      console.log("[ChipMate New] Sub-agent viewer panel disposed:", sessionID)
