...
import ai.chipmate.client.ui.PickerButton
import ai.chipmate.client.ui.picker.PickerPopup
import ai.chipmate.client.ui.picker.popupBackground
...
class ModelPicker : PickerButton() {
    ...
    var emptyText: String = ChipMateBundle.message("settings.models.notSet")
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = ChipMateBundle.message("model.picker.tooltip")
    ...
    private fun refresh() {
        ...
        toolTipText = if (item?.let(ModelText::collectsData) == true) ModelText.dataCollectedTooltip() else ChipMateBundle.message("model.picker.tooltip")
        isEnabled = true
...
internal const val MODEL_PICKER_EXPANDED_KEY = "chipmate.model.picker.expanded"
...
internal object ModelText {
    private val small = setOf("chipmate-auto/small", "auto-small")
    ...
    fun buttonLabel(item: ModelPicker.Item): String {
        ...
        if (item.provider == "chipmate") return part
        val provider = item.providerName.trim()
        if (provider.isEmpty()) return part
        return "$provider / $part"
    }

    fun small(item: ModelPicker.Item): Boolean = item.provider == "chipmate" && item.id in small

    fun providerSort(id: String): Int = if (id == "chipmate") 0 else 1

    fun dataCollected(): String = ChipMateBundle.message("model.picker.dataCollected")

    fun dataCollectedTooltip(): String = XmlStringUtil.wrapInHtmlLines(
        ChipMateBundle.message("model.picker.tooltip"),
        ChipMateBundle.message("model.picker.dataCollected.current"),
    )

    fun freeLabel(): String = ChipMateBundle.message("model.picker.free")
