...
//
// ChipMate-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with chipmate_change).

import { createChipMate, type ChipMateProvider, AI_SDK_PROVIDERS, PROMPTS } from "@chipmate/chipmate-gateway"
import { DEFAULT_HEADERS } from "@/chipmate/const"
...
export const CHIPMATE_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@chipmate/chipmate-gateway": async () => createChipMate as unknown as (options: any) => BundledSDK,
}
...
export const CHIPMATE_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns chipmate-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "chipmate" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges chipmate-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function patchChipMateProviderPrivacy(provider: { options?: Record<string, any> } | undefined, config: any) {
  if (!provider || config.hide_prompt_training_models !== true) return
  provider.options = { ...provider.options, dataCollection: "deny" }
}

export function chipmateCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    chipmate: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if (config.provider?.["chipmate"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.CHIPMATE_ORG_ID) {
        options.chipmateOrganizationId = env.CHIPMATE_ORG_ID
      }
      ...
      return {
        ...
        options,
        async getModel(sdk: ChipMateProvider, modelID: string) {
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "chipmate",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line chipmate_change markers
  }
}
...
export function chipmateSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("chipmate")) return ["chipmate-auto/small"]
  return undefined
}
