...
import { renderPlantUml } from "../../src/chipmate-provider/render-plantuml"
...
describe("ChipMate Server configuration", () => {
  ...
  it("does not read a legacy marketplace endpoint", () => {
    set(state, "chipmate.marketplace", "baseUrl", "globalValue", "http://legacy.test:6001/marketplace")
    expect(resolveChipmateServer()).toEqual({ baseUrl: CHIPMATE_SERVER_DEFAULT, source: "default" })
  })
  ...
  it("derives Marketplace and renderer runtime endpoints from the unified origin", () => {
    ...
    delete process.env.CHIPMATE_REVIEW_RULES_ENDPOINT
    ...
    try {
      ...
      else process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT = mermaid
      if (plantuml === undefined) delete process.env.CHIPMATE_PLANTUML_RENDER_ENDPOINT
      else process.env.CHIPMATE_PLANTUML_RENDER_ENDPOINT = plantuml
      if (review === undefined) delete process.env.CHIPMATE_REVIEW_RULES_ENDPOINT
      else process.env.CHIPMATE_REVIEW_RULES_ENDPOINT = review
    }
  })
  ...
  it("keeps explicit renderer environment overrides", () => {
    ...
    process.env.CHIPMATE_REVIEW_RULES_ENDPOINT = "https://override.test:7443/rules"
    set(state, "chipmate.v2.chipmateServer", "baseUrl", "globalValue", "https://runtime.test:7443")
    try {
      expect(renderEnv()).toEqual({})
    } finally {
  ...
  it("reports injected, inherited, and absent Mermaid endpoint states without exposing credentials", () => {
    expect(
      mermaidEndpoint({
        CHIPMATE_MERMAID_RENDER_ENDPOINT: "https://user:secret@render.test:7443/render/mermaid?token=secret#hash",
      }),
  ...
  it("reports injected, inherited, and absent PlantUML endpoint states without exposing credentials", () => {
    expect(
      plantumlEndpoint({
        CHIPMATE_PLANTUML_RENDER_ENDPOINT: "https://user:secret@render.test:7443/render/plantuml?token=secret#hash",
      }),
  ...
  it("ignores legacy per-service settings", () => {
    set(state, "chipmate.marketplace", "baseUrl", "globalValue", "http://market.test:6001/marketplace")
    set(
      state,
