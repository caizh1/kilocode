...
import { ChipMateSnapshotMaterialize } from "../../src/chipmate/snapshot/materialize"
import { ChipMateSnapshotSeed } from "../../src/chipmate/snapshot/seed"
...
test(
  ...
  async () => {
    ...
    expect(
      (
        await $`git --git-dir=${common} rev-parse --verify --quiet ${ChipMateSnapshotMaterialize.ref(fast.gitdir)}`
          .nothrow()
          .text()
      ).trim(),
    ).toBe("")
...
test(
  ...
  async () => {
    ...
    const sourceRef = ChipMateSnapshotMaterialize.ref(result.gitdir)
    ...
    const expired = `refs/chipmate/snapshots/1/${result.value!}`
    ...
    expect((await $`git --git-dir=${result.gitdir} for-each-ref refs/chipmate/snapshots`.text()).trim()).toContain(
      result.value!,
    )

    const locked = `refs/chipmate/snapshots/2/${result.value!}`
...
test("interrupted seed removes borrowed state after source gc", async () => {
  ...
  const original = await fs.readFile(index)
  const ref = ChipMateSnapshotMaterialize.ref(gitdir)
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* ChipMateSnapshotSeed.seed({
        dir: source.path,
