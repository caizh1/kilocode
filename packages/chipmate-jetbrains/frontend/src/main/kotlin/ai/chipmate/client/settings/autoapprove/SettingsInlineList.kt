...
import ai.chipmate.client.settings.base.SettingsInlineListPanel
...
internal fun levelChoiceLabel(choice: LevelChoice): String = when (choice) {
    is LevelChoice.Default -> ChipMateBundle.message("settings.autoApprove.default", levelLabel(choice.level))
...
internal class SettingsInlineList(
    ...
    internal var editInput: (String) -> String? = { key ->
        Messages.showInputDialog(
            this,
            placeholder,
            ChipMateBundle.message("settings.autoApprove.edit"),
            null,
            key,
            null,
        )
    ...
    override fun toolbarActions(): List<AnAction> = buildList {
        ...
        if (add != null && onAdd != null) {
            add(SettingsToolbarAction(
                ChipMateBundle.message("settings.autoApprove.add"),
                add,
        ...
        if (onRemove != null) {
            add(SettingsToolbarAction(
                ChipMateBundle.message("settings.autoApprove.delete"),
                ChipMateBundle.message("settings.autoApprove.delete.description"),
    ...
    private data class PermissionItem(val row: PermissionListRow) : SettingsListItem {
        ...
        override val cells: List<SettingsListCell> = buildList {
            if (row.editable) add(SettingsListCell(
                id = EDIT_CELL,
                label = ChipMateBundle.message("settings.autoApprove.edit"),
            ))
            add(SettingsListCell(
                id = LEVEL_CELL,
                label = if (row.inherited) ChipMateBundle.message(
