...
const auth = Buffer.from(`chipmate:${password}`).toString("base64")
...
const child = spawn(args.cli, ["serve", "--hostname", "127.0.0.1", "--port", String(port)], {
  ...
  env: {
    ...
    CHIPMATE_CONFIG_CONTENT: '{"enabled_providers":["anthropic"]}',
...
try {
  ...
  const response = await fetch(`http://127.0.0.1:${port}/pty`, {
    ...
    headers: {
      ...
      "x-chipmate-directory": args.root,
    },
    body: JSON.stringify({
      ...
      env: { CHIPMATE_AGENT_CONSOLE: "1", CHIPMATE_AGENT_CONSOLE_TOKEN: token },
    }),
  })
