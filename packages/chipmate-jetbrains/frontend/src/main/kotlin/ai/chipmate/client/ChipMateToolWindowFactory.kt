...
import ai.chipmate.client.session.SessionSidePanelManager
...
/**
 * Creates the ChipMate tool window and delegates session content management.
 *
 ...
 * completes.
 */
class ChipMateToolWindowFactory : ToolWindowFactory, DumbAware {
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        project.service<ChipMateToolWindowSetupService>().create(toolWindow)
    }
}

private val LOG = ChipMateLog.create(ChipMateToolWindowFactory::class.java)

@Service(Service.Level.PROJECT)
internal class ChipMateToolWindowSetupService(
    ...
    fun create(toolWindow: ToolWindow) {
        ...
        try {
            val workspaces = service<ChipMateWorkspaceService>()
            ...
            LOG.error("Failed to create ChipMate tool window content", e)
        }
    }
    ...
    private fun setup(
        ...
        try {
            ...
            val actions = listOfNotNull(
                ActionManager.getInstance().getAction("ChipMate.NewSession"),
                ActionManager.getInstance().getAction("ChipMate.History"),
                ActionManager.getInstance().getAction("ChipMate.Settings"),
            )
            toolWindow.setTitleActions(actions)
        } catch (e: Exception) {
            Telemetry.send("Tool Window Setup Failed", mapOf("stage" to "setup", "errorClass" to e::class.java.name))
            LOG.error("Failed to set up ChipMate tool window content", e)
        }
    }
}
