...
import {
  ...
  parseStore,
  type Metadata,
} from "../../../src/chipmate/anaconda-desktop/domain"
import {
  candidates,
  command,
  directory,
  ...
  supported,
  type Info,
} from "../../../src/chipmate/anaconda-desktop/platform"
...
describe("Anaconda Desktop platform adapters", () => {
  test("resolves official cross-platform user-data locations", () => {
    expect(directory({ platform: "darwin", arch: "arm64", home: "/Users/chipmate", env: {} })).toBe(
      "/Users/chipmate/Library/Application Support/anaconda-desktop",
    )
    expect(
      directory({ platform: "win32", arch: "x64", home: "C:\\Users\\chipmate", env: { APPDATA: "D:\\Roaming" } }),
    ).toBe(path.win32.join("D:\\Roaming", "anaconda-desktop"))
    expect(directory(linux({ XDG_DATA_HOME: "/data" }))).toBe("/data/anaconda-desktop")
    expect(directory(linux())).toBe("/home/chipmate/.local/share/anaconda-desktop")
  })
  ...
  test("builds hidden Windows and Wayland-safe Linux launch commands", () => {
    const windows: Info = {
      platform: "win32",
      arch: "x64",
      home: "C:\\Users\\chipmate",
      env: { LOCALAPPDATA: "C:\\Users\\chipmate\\AppData\\Local" },
    }
  ...
  test("passes only required desktop environment variables to the launched app", async () => {
    ...
    const output = await Process.run(
      [
        process.execPath,
        "-e",
        "process.stdout.write(JSON.stringify({ path: process.env.PATH, secret: process.env.CHIPMATE_SERVER_PASSWORD }))",
      ],
      { env },
    )
