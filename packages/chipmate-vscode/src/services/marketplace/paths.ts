...
export class MarketplacePaths {
  ...
  /** Project-scope config file: <workspace>/.chipmate-v2/chipmate.json */
  configPath(scope: "project" | "global", workspace?: string): string {
    if (scope === "project") return path.join(workspace!, ".chipmate-v2", "chipmate.json")
    return path.join(this.global, "chipmate.json")
  }
