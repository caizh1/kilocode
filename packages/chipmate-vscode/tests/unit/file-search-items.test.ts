...
import { mergeFileSearchItems } from "../../src/chipmate-provider/file-search-items"
...
describe("mergeFileSearchItems", () => {
  ...
  it("normalizes Windows separators for matching and output", () => {
    const result = mergeFileSearchItems({
      query: "chipmate-vscode",
      files: ["packages\\chipmate-vscode\\src\\ChipMateProvider.ts"],
      folders: ["packages\\chipmate-vscode\\"],
    })
    expect(result).toEqual([
      { path: "packages/chipmate-vscode/", type: "folder" },
      { path: "packages/chipmate-vscode/src/ChipMateProvider.ts", type: "file" },
    ])
  })
  ...
  it("keeps active and open file results before prefix folder matches", () => {
    const result = mergeFileSearchItems({
      query: "e",
      files: ["packages/chipmate-vscode/src/extension.ts"],
      folders: ["packages/extensions/", "packages/example/", "packages/core/src/effect/"],
      open: new Set(["packages/chipmate-vscode/src/extension.ts"]),
    })
    expect(result).toEqual([
      { path: "packages/chipmate-vscode/src/extension.ts", type: "opened-file" },
