...
class TurnLifecycleTest : SessionControllerTestBase() {
    ...
    fun `test TurnClose error reason does not clobber Error state`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [error] [Timed out]
            """,
            m,
        )
    }
    ...
    fun `test Error fires StateChanged to Error`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [error] [Bad Request]
            """,
            m,
        )
    }
    ...
    fun `test Error with null message falls back to type`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [error] [timeout]
            """,
            m,
        )
    }
    ...
    fun `test paid model auth error enters login required state`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [login-required] [Go to User Profile settings to sign in, then continue this session.]
            """,
            m,
        )
    }
    ...
    fun `test login clears paid model gate`() {
        ...
        appRpc.state.value = ChipMateAppStateDto(
            ...
            config = ConfigDto(model = "chipmate/gpt-5"),
    ...
    fun `test login resumes paid model prompt`() {
        ...
        assertEquals("chipmate/openai", prompt.providerID)
    ...
    fun `test rollback click enters reverting state before rpc resolves`() {
        ...
        assertEquals(ChipMateBundle.message("session.status.rollingback"), (state as SessionState.Reverting).text)
    ...
    fun `test reverting state renders in dump`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [reverting] [rolling back]
            """,
            m,
        )
    ...
    fun `test redo enters reverting with redoing status`() {
        ...
        val state = m.model.state
        assertTrue("expected Reverting, was $state", state is SessionState.Reverting)
        assertEquals(ChipMateBundle.message("session.status.redoing"), (state as SessionState.Reverting).text)
    ...
    fun `test rollback watchdog cancels timed out rollback and releases lock`() {
        ...
        val state = m.model.state
        assertTrue("expected Error, was $state", state is SessionState.Error)
        assertEquals(ChipMateBundle.message("session.error.revert.timeout"), (state as SessionState.Error).message)
