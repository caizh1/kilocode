...
describe("agent builder routes", () => {
  test("previews and saves project agent markdown", async () => {
    ...
    expect(output.path).toBe(path.join(tmp.path, ".chipmate", "agent", "reviewer.md"))
  ...
  test("saves without a duplicated body id", async () => {
    ...
    expect(output.path).toBe(path.join(tmp.path, ".chipmate", "agent", "canonical.md"))
  ...
  for (const value of [false, true]) {
    ...
    test.serial(`${value ? "httpapi" : "legacy"} rejects invalid route ids`, async () => {
      ...
      expect(saved.status).toBe(400)
      expect(await Bun.file(path.join(tmp.path, ".chipmate", "agent", "bad:id.md")).exists()).toBe(false)
    })
  }
  ...
  // (ChipMateAgent.cacheKey), not file-based `.md` agents, so without it a new agent would not
