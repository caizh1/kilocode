...
import { appendTerminalOutput } from "@/chipmate/interactive-terminal/output"
...
export namespace InteractiveTerminal {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@chipmate/InteractiveTerminal.State",
  ) {}
  ...
  function gate(shell: string, command: string) {
    ...
    return `stty -echo; IFS= read -r __chipmate_gate; stty echo; ${command}`
  }
  ...
  function environment(input: NodeJS.ProcessEnv) {
    ...
    env.CHIPMATE_TERMINAL = "1"
    env.CHIPMATE_INTERACTIVE_TERMINAL = "1"
