...
const settlementTimeout = Duration.seconds(30) // chipmate_change - bound retained OAuth attempt secrets

type AttemptTime = { created: number; expires: number }
type PendingAttempt = {
  status: "pending"
  completing: boolean
  settling: boolean // chipmate_change - cancellation and expiry cannot overtake credential persistence
...
export const locationLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // chipmate_change start - persist before exposing completion and make settlement atomic with cancellation
                  ...
                  .create({
                    ...
                    value:
                      exit.value.type === "oauth"
                        ? { ...exit.value, methodID: pending.methodID } // chipmate_change - OAuth is a struct, not a class
                        : exit.value,
                  })
                  .pipe(
    ...
    const scrub = Effect.fnUntraced(function* () {
      ...
      const expired = yield* SynchronizedRef.modify(attempts, (current) => {
        ...
        for (const [id, attempt] of current) {
          if (attempt.status === "pending" && !attempt.settling && attempt.time.expires <= now) { // chipmate_change
        ...
        oauth: Effect.fn("Integration.connection.oauth")(function* (input) {
          ...
          yield* SynchronizedRef.update(attempts, (current) =>
            new Map(current).set(id, {
              ...
              settling: false, // chipmate_change
              authorization,
              integrationID: input.integrationID,
          ...
          if (authorization.mode === "auto") {
            // chipmate_change start - settle persistence atomically with cancellation
      ...
      attempt: {
        ...
        complete: Effect.fn("Integration.attempt.complete")(function* (input) {
          const attempt = yield* SynchronizedRef.modify(attempts, (current) => {
            ...
            return [match, new Map(current).set(input.attemptID, { ...match, completing: true, settling: true })] // chipmate_change
          })
          ...
          // chipmate_change start - an interrupted or timed-out callback still settles and releases its attempt.
          return yield* Effect.uninterruptibleMask((restore) =>
            Effect.gen(function* () {
        ...
        cancel: Effect.fn("Integration.attempt.cancel")(function* (attemptID) {
          const attempt = yield* SynchronizedRef.modify(attempts, (current) => {
            ...
            if (!match || match.status !== "pending" || match.settling) return [undefined, current] // chipmate_change
            const next = new Map(current)
