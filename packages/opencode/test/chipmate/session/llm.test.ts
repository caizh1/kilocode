...
describe("chipmate.session.llm.timeout", () => {
  test("uses prepared options before the provider fallback", () => {
    const result = ChipMateLLM.timeout({
  ...
  test("omits the timeout when it is not configured", () => {
    expect(ChipMateLLM.timeout({ options: {} })).toEqual({})
  })
})

describe("chipmate.session.llm.text", () => {
  ...
  test("fails on stream interruption", async () => {
    const text = ChipMateLLM.text(Stream.fail(new DOMException("Aborted", "AbortError")))
