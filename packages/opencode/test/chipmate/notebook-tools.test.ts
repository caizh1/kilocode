...
import { NotebookEditTool, NotebookExecuteTool, NotebookReadTool } from "@/chipmate/tool/notebook-host"
...
test("uses dedicated VS Code notebook permission defaults only when enabled", () => {
  ...
  try {
    ...
    const disabled = ChipMateAgent.prepare({}).defaultsPatch
    expect(disabled.some((rule) => rule.permission.startsWith("notebook_"))).toBe(false)

    const rules = ChipMateAgent.prepare({ experimental: { native_notebook_tools: true } }).defaultsPatch
