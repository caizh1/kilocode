...
description: "Architecture of the ChipMate CLI runtime, daemon, server, persistence, SDK, and indexing"
---

# CLI Runtime Architecture

The CLI (`packages/opencode/`) is ChipMate's local agent engine. It owns agent execution, tools, sessions, provider integration, configuration, local persistence, directory routing, and HTTP surfaces used by editor clients and ChipMate Console.
...
| Term | Meaning |
|---|---|
| ChipMate CLI runtime | Local agent engine in `packages/opencode/` |
| `chipmate serve` server | Local HTTP and SSE process used by editor clients and ChipMate Console; selected browser-oriented paths also use WebSocket |
| Local daemon | Detached reusable `chipmate serve` server managed by `chipmate daemon` commands |
| Directory context | Normalized local filesystem directory used to select local runtime state |
| Local runtime instance | Directory-keyed runtime context inside one ChipMate CLI process |
...
| Process-shared state | Runtime service state shared by every directory context in one ChipMate CLI process |
| Modes | Configurable agent presets for tools, prompts, restrictions, and behavior |
| MCP | Protocol for extending agent tools |

One `chipmate serve` process can host several local runtime instances. Directory-keyed state stays isolated. Process-shared service state does not.

## Command entry points

{% callout type="warning" title="ChipMate Console is deprecated" %}
The `chipmate console` command and the browser interface described on this page are deprecated and will be removed in an upcoming release.
{% /callout %}

| Entry point | Command or caller | Runtime model |
|---|---|---|
| Interactive TUI | `chipmate` | Attaches to local daemon when available; otherwise starts Bun worker and sends SDK-shaped requests over RPC |
| Headless run | `chipmate run` | Uses daemon attach when available, then embedded server fetch fallback |
| Attached run | `chipmate run --attach <url>` | Targets explicit running `chipmate serve` server |
| Explicit API server | `chipmate serve` | Starts HTTP + SSE server for external local clients |
| Local daemon | `chipmate daemon start` | Starts detached `chipmate serve` child for reuse |
| Browser console | `chipmate console` | Starts or reuses local daemon and opens daemon-served `/console` UI |
| Editor-spawned server | VS Code or JetBrains client | Starts bundled `chipmate serve --port 0` child owned by editor client, not local daemon manager |

```mermaid
flowchart LR
  run["chipmate run"]
  tui["chipmate TUI"]
  daemon["Detached daemon: chipmate serve"]
  worker["Bun worker"]
  rpc["RPC-backed fetch and global events"]
  embedded["Embedded Server.Default().app.fetch"]
  serve["Explicit chipmate serve"]
  editors["VS Code or JetBrains"]
  editorServer["Editor-owned chipmate serve --port 0"]
  runtime["ChipMate CLI runtime"]
...
Each running editor host starts one editor-owned `chipmate serve` server. That server can handle coding sessions for workspace root and additional worktree directories at same time. It does not start separate server process for each directory.

```mermaid
flowchart LR
  views["Editor views<br/>Workspace root and worktrees"]
  server["One editor-owned<br/>chipmate serve server"]
  store["InstanceStore"]
...
|---|---|---|
| Local `chipmate serve` access | HTTP, SSE, and selected WebSocket access to local server | ChipMate CLI server and spawning local client |
| Outbound provider authentication | Model provider, ChipMate Gateway, catalog, and indexing access | ChipMate CLI provider router and auth stores |
| Remote MCP OAuth | Browser authorization and credentials for remote MCP server | ChipMate CLI MCP runtime |

### Local `chipmate serve` access

Server Basic Auth is optional. It becomes required when `CHIPMATE_SERVER_PASSWORD` is non-empty. Default username is `chipmate`; `CHIPMATE_SERVER_USERNAME` can override it.
...
| PTY shell child | Removes `CHIPMATE_SERVER_PASSWORD` and `CHIPMATE_SERVER_USERNAME` from spawned user-shell environment |
...
Provider auth records use `api`, `oauth`, or `wellknown` variants in `${Global.Path.data}/auth.json`, written with mode `0600`. `CHIPMATE_AUTH_CONTENT` can supply process-local auth JSON. Separate v2 multi-account auth store also exists for account-oriented flows.

| Path | Behavior |
|---|---|
| Direct providers | Use provider-specific keys, OAuth records, environment values, and configured endpoints |
| ChipMate Gateway | Resolves ChipMate model access and model catalog through gateway client |
| Anonymous ChipMate | If no ChipMate key exists, provider loader sets API key value `anonymous`; gateway model catalog can fall back to public unauthenticated endpoint |
| Organization catalog | ChipMate model fetch includes organization ID when resolved from config, auth, or environment |
...
| Indexing auth | Resolves indexing-specific ChipMate config first, then provider config, auth record, provider options, and `CHIPMATE_API_KEY` / `CHIPMATE_ORG_ID` environment values |
...
2. `x-chipmate-directory` request header.
3. Server process cwd.

Local routing workspace selection is separate. Session workspace, `workspace` query parameter, and `CHIPMATE_WORKSPACE_ID` can select workspace context. Configured `CHIPMATE_WORKSPACE_ID` keeps requests local to current workspace runtime. Other selected workspaces resolve through workspace-routing adapter to local directory or remote target.
...
| Tool registry | Loads built-in, ChipMate-specific, MCP, and readiness-gated semantic search tools |
...
| Provider router | Resolves direct providers, ChipMate Gateway, custom endpoints, and credentials |
| HTTP server | Publishes REST, WebSocket, and SSE surfaces |

## Daemon lifecycle

`chipmate daemon start|status|stop|restart` manage a detached local `chipmate serve` child, with bare `chipmate daemon` equivalent to `chipmate daemon start`. `chipmate console` calls the same start path, so it reuses a healthy daemon instead of spawning a second process, while `chipmate console stop` aliases `chipmate daemon stop`.
...
| Child process | Detached `chipmate serve --hostname <host> --port <port>` process |
...
| Opt-out | `CHIPMATE_NO_DAEMON` disables automatic attach by clients; explicit daemon commands still manage daemon |

Daemon credentials differ from editor-spawned server credentials. Current daemon source stores username `chipmate`, password `chipmate`, and base64 Basic token in `daemon.json`. File permissions protect this local credential record. Editor clients generate random passwords per spawned server.
...
| Area | Behavior |
|---|---|
| Default database | `${Global.Path.data}/chipmate.db` |
| Override | `CHIPMATE_DB`; relative paths resolve under data directory; `:memory:` is accepted |
...
Snapshot implementation state is directory-keyed through `InstanceState`. One `Snapshot.Service` also owns process-shared slow-snapshot guard state outside directory cache. This distinction matters when multiple Agent Manager worktrees use same `chipmate serve` process.
...
3. ChipMate-specific API groups and handlers live under `packages/opencode/src/chipmate/server/httpapi/` and enter shared API through narrow injection seams.
4. `packages/sdk/js/script/build.ts` generates TypeScript v2 client from CLI OpenAPI.
5. `packages/sdk/js/src/v2/client.ts` adds `createChipMateClient()` wrapper for directory and workspace routing, Electron and Node fetch compatibility, and clearer empty-response errors.
...
| Order | Source |
|---|---|
| 1 | Legacy ChipMate migrations |
| 2 | Organization modes |
| 3 | Auth-record `.well-known/opencode` remote config |
| 4 | Global config files |
| 5 | Explicit `CHIPMATE_CONFIG` file |
| 6 | Project `chipmate.json[c]` and `opencode.json[c]` files plus discovered config directories |
| 7 | `CHIPMATE_CONFIG_DIR` directory |
| 8 | `CHIPMATE_CONFIG_CONTENT` |
| 9 | Active ChipMate Cloud organization config |
...
Global config files load from `${Global.Path.config}`. Project updates prefer existing config files found in ancestor `.chipmate` or legacy `.chipmate` directories, then existing project root config files, then create `.chipmate/chipmate.json`. Global indexing settings can carry provider and storage defaults, but global `indexing.enabled` is stripped so project enablement remains local in effective instance config.
...
**Deprecated.** The ChipMate Console browser interface and its `chipmate console` launcher will be removed in a future release.

`chipmate console` starts or reuses daemon, opens `/console`, and prints Console launch URL. Browser launch URL embeds daemon Basic credentials so initial request authenticates.

| Area | Behavior |
|---|---|
| Frontend | Solid/Vite app in `packages/chipmate-console/` |
...
| SDK | Console calls generated JavaScript SDK through `createChipMateClient()` |
...
`packages/chipmate-indexing/` owns indexing engine. CLI bridge injects indexing plugin by default unless default plugins are disabled, then starts indexing asynchronously per normalized directory during instance bootstrap.

| Area | Behavior |
|---|---|
| Bootstrap | `ChipMateBootstrap` forks indexing initialization so instance startup is not blocked |
...
| Worktrees | Agent Manager `.chipmate/worktrees/` and legacy `.chipmate/worktrees/` paths return disabled status |
| Empty VS Code window | Extension sets `CHIPMATE_DISABLE_CODEBASE_INDEXING=vscode-no-workspace`; bridge reports disabled status |
| Embeddings | Supports ChipMate, OpenAI, Ollama, OpenAI-compatible, Gemini, Mistral, Vercel AI Gateway, Bedrock, OpenRouter, and Voyage configuration |
| Vector stores | Supports Qdrant and LanceDB |

## Source map

Paths below are relative to [`ChipMate-Org/chipmate`](https://github.com/ChipMate-Org/chipmate).

| Concern | Source paths |
|---|---|
| CLI entry points | `packages/opencode/src/cli/cmd/` |
| Daemon | `packages/opencode/src/chipmate/daemon/` |
...
| Snapshots | `packages/opencode/src/snapshot/index.ts`{% linebreak /%}`packages/opencode/src/chipmate/snapshot/track.ts` |
| SDK | `packages/sdk/js/`{% linebreak /%}`script/generate.ts` |
| Console | `packages/chipmate-console/`{% linebreak /%}`packages/opencode/src/chipmate/console/` |
| Indexing | `packages/chipmate-indexing/`{% linebreak /%}`packages/opencode/src/chipmate/indexing.ts` |
