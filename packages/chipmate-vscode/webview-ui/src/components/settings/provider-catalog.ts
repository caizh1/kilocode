...
import {
  ...
  createChipMateFallbackProvider,
  providerOrderIndex,
...
export function isPopularProvider(provider: Provider | string, internal = isInternalOfflineBuild()) {
...
export function popularProviderIndex(provider: Provider | string, internal = isInternalOfflineBuild()) {
  if (internal) return FALLBACK_PROVIDER_IDS.length
  const id = typeof provider === "string" ? provider : provider.id
...
export function providerIcon(provider: Provider | string): IconName {
  ...
  if (icon) return icon
  if (providerID === CHIPMATE_PROVIDER_ID) return validIcon("chipmate") ?? "synthetic"
  const fallback = validIcon(providerID)
  if (fallback) return fallback
  return "synthetic"
}

export function chipmateFallbackProvider(): Provider {
  return createChipMateFallbackProvider()
}

export function providerNoteKey(provider: Provider | string, internal = isInternalOfflineBuild()) {
  if (internal) return undefined
  if (typeof provider !== "string" && provider.metadata?.noteKey) return provider.metadata.noteKey
  if (provider === CHIPMATE_PROVIDER_ID) return "settings.providers.note.chipmate"
  return undefined
}
