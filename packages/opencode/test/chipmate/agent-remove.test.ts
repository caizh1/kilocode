...
import { remove } from "../../src/chipmate/agent"
import type { Info as AgentInfo } from "../../src/agent/agent"
import { tmpdir } from "../fixture/fixture"

describe("ChipMate agent remove", () => {
  test("removes config-backed imported agents", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".chipmate")
    const file = path.join(dir, "chipmate.jsonc")
  ...
  "agent": {
    ...
    expect(cfg.agent.code.model).toBe("chipmate/gpt-5")
  })
})
