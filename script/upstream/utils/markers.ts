#!/usr/bin/env bun
/**
 * Shared chipmate_change marker helpers used by both the marker fixer and the
...
export const standalone = [
  /^\s*\/\/\s*chipmate_change\b.*$/,
  /^\s*#\s*chipmate_change\b.*$/,
  /^\s*\{?\s*\/\*\s*chipmate_change\b.*\*\/\}?\s*$/,
]
export const start = /\bchipmate_change\s+start\b/
export const end = /\bchipmate_change\s+end\b/
export const freshmark = /\bchipmate_change\s*-\s*new\s*file\b/
...
function strip(file: string, line: string): { line: string | null; mark?: string } {
  ...
  if (style(file) === "hash") return comment(line, [/^#\s*chipmate_change\b/])
  return comment(line, [/^\{\/\*\s*chipmate_change\b/, /^\/\*\s*chipmate_change\b/, /^\/\/\s*chipmate_change\b/])
}
...
function block(mode: Style, pad: string) {
  if (mode === "hash") return { start: `${pad}# chipmate_change start`, end: `${pad}# chipmate_change end` }
  if (mode === "jsx") return { start: `${pad}{/* chipmate_change start */}`, end: `${pad}{/* chipmate_change end */}` }
  if (mode === "block") return { start: `${pad}/* chipmate_change start */`, end: `${pad}/* chipmate_change end */` }
  return { start: `${pad}// chipmate_change start`, end: `${pad}// chipmate_change end` }
}

function note(mode: Style) {
  if (mode === "hash") return " # chipmate_change"
  if (mode === "jsx") return " {/* chipmate_change */}"
  if (mode === "block") return " /* chipmate_change */"
  return " // chipmate_change"
}
...
export function fresh(file: string, clean: Clean) {
  ...
  const line = clean.marks.file ?? (mode === "hash" ? "# chipmate_change - new file" : "// chipmate_change - new file")
...
export async function changed(base: Text, head: Text, opts?: { ignoreWhitespace?: boolean }): Promise<Diff> {
  const dir = await mkdtemp(path.join(tmpdir(), "chipmate-markers-"))
