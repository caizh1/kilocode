 ...
 *  - if `.chipmateignore` exists and is non-empty, use only its patterns (plus the
 *    `.chipmateignore` file itself);
 ...
 * `/`), and the `*`, `**`, `?`, and `[..]` globs.
 */
internal class ChipMateIgnore private constructor(private val rules: List<Rule>) {
    ...
    companion object {
        val EMPTY = ChipMateIgnore(emptyList())

        const val CHIPMATE = ".chipmateignore"
         ...
         */
        fun load(root: VirtualFile?): ChipMateIgnore {
            if (root == null) return EMPTY
            val chipmate = read(root, CHIPMATE)
            if (!chipmate.isNullOrBlank()) return ChipMateIgnore(compile(chipmate) + compile(CHIPMATE))
            val rules = mutableListOf<Rule>()
            read(root, GIT)?.takeIf { it.isNotBlank() }?.let { rules += compile(it) }
            rules += SENSITIVE.mapNotNull { rule(it) }
            return ChipMateIgnore(rules)
        }

        /** Test seam: build a matcher directly from ignore-file text. */
        fun of(text: String): ChipMateIgnore = ChipMateIgnore(compile(text))
