...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping ChipMate find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`semantic_search`](/docs/automate/tools/semantic-search) tool** to ChipMate for intelligent code discovery
...
- **Enhanced AI Understanding**: ChipMate can better comprehend and work with your codebase
...
1. Open ChipMate **Settings** → **Indexing**, or click the indexing indicator at the bottom of the prompt input panel.
...
You can also edit the `indexing` section in `chipmate.jsonc` directly:
...
- **LanceDB** (default). Embedded and file-based, with no server to run. Stores data under your ChipMate data directory by default.
...
Open a ChipMate TUI session and run:
...
All changes are written to your `chipmate.jsonc` config and take effect immediately.
...
- `lancedb` uses `{ directory? }` and is the default. It is embedded and file-based, with no server to run. ChipMate uses its data directory when `directory` is omitted.
...
Set `indexing.fileExtensions` to a non-empty array to index only the listed file extensions. Values are case-insensitive and may be written with or without a leading dot. When this setting is omitted, ChipMate uses its built-in language list. Configured text formats without a Tree-sitter parser use line-based fallback chunking.
...
- Files matching `.gitignore` and [`.chipmateignore`](/docs/customize/context/chipmateignore) patterns
...
- **API Keys**: Stored in your `chipmate.jsonc` config. Treat that file as a secret in shared environments.
...
- Check that `indexing.enabled` is `true` in your `chipmate.jsonc`
...
Once indexed, ChipMate can use the [`semantic_search`](/docs/automate/tools/semantic-search) tool to find relevant code:
...
The tool provides ChipMate with:
...
- **Secure storage**: API keys are stored in your local `chipmate.jsonc` configuration
- **Fully local option**: Use **Ollama + LanceDB** for completely local processing
- **Access control**: Respects existing file permissions and `.chipmateignore`
