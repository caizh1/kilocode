...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/chipmate-vscode/bin/chipmate`.
 *
 * Strategy:
 * 1) If `bin/chipmate` already exists -> ok.
 ...
 * 4) Copy the resulting binary into `packages/chipmate-vscode/bin/chipmate` and chmod +x.
 *
 * This script is intended to be run from `packages/chipmate-vscode` as part of build/package.
 */

const chipmateVscodeDir = join(import.meta.dir, "..")
const packagesDir = join(chipmateVscodeDir, "..")
const repoDir = join(packagesDir, "..")
const opencodeDir = join(packagesDir, "opencode")
const coreDir = join(packagesDir, "core")
const gatewayDir = join(packagesDir, "chipmate-gateway")
const indexingDir = join(packagesDir, "chipmate-indexing")
const sandboxDir = join(packagesDir, "chipmate-sandbox")

const targetBinDir = join(chipmateVscodeDir, "bin")
const binName = process.platform === "win32" ? "chipmate.exe" : "chipmate"
...
async function findChipMateBinaryInOpencodeDist(): Promise<string | null> {
  ...
  const preferred = join(distDir, `@chipmate`, tag, "bin", binName)
  try {
    statSync(preferred)
    if (!isRunnableOnCurrentPlatform(preferred)) return null
    if (!hasTreeSitterResources(preferred) || !hasChipMateSandboxWorker(preferred)) return null
  ...
  // Fallback: find any dist/**/bin/chipmate or chipmate.exe
  const queue = [distDir]
  while (queue.length) {
    const dir = queue.pop()
    if (!dir) continue
    ...
    for (const e of entries) {
      ...
      if (e.isFile() && e.name === binName && basename(dirname(p)) === "bin") {
        ...
        if (!hasTreeSitterResources(p) || !hasChipMateSandboxWorker(p)) continue
...
async function ensureBuiltBinary(): Promise<string> {
  const found = await findChipMateBinaryInOpencodeDist()
  if (found) return found

  log(
    `No prebuilt binary found under ${relative(chipmateVscodeDir, join(opencodeDir, "dist"))} - attempting build via bun.`,
  )
  ...
  const built = await findChipMateBinaryInOpencodeDist()
  if (!built) {
    throw new Error(
      `CLI build completed but no binary was found in ${join(opencodeDir, "dist")} (expected dist/**/bin/chipmate).`,
    )
  }
  return built
}

async function bundleChipMateSandboxWorker() {
  const result = await Bun.build({
    entrypoints: [join(sandboxDir, "src", "chipmate-sandbox-mutation-worker.ts")],
    target: "bun",
    format: "esm",
    minify: true,
  })
  if (!result.success || result.outputs.length !== 1) throw new Error("Could not bundle ChipMate sandbox mutation worker")
  await Bun.write(chipmateSandboxWorkerForBinary(targetBinPath), result.outputs[0])
}
...
async function ensureLocalHelpers() {
  ...
  if (process.env.CHIPMATE_SKIP_BUNDLED_BWRAP === "1") return
...
async function writeSourceWrapper() {
  ...
  await Bun.write(
    ...
    [
      ...
      "set -euo pipefail",
      `cd ${JSON.stringify(opencodeDir)}`,
      `exec ${JSON.stringify(bun)} --conditions=browser src/chipmate/indexing-process.ts "$@"`,
      "",
    ].join("\n"),
  )
  ...
  await bundleChipMateSandboxWorker()
  await ensureLocalHelpers()

  const hash = await cliSourceHash()
  if (hash) await Bun.write(versionFile, hash + "\n")
  log(
    `Compiled CLI build failed; wrote source wrapper at ${relative(chipmateVscodeDir, targetBinPath)} for local development.`,
  )
}
...
async function main() {
  ...
  const ready =
    ...
    hasChipMateSandboxWorker(targetBinPath)
  ...
  if (ready && !rebuild) {
    ...
    log(
      `CLI binary already present at ${relative(chipmateVscodeDir, targetBinPath)} (${Math.round(st.size / 1024 / 1024)}MB). Use --force to rebuild.`,
    )
    await ensureLocalHelpers()
    return
  }
  ...
  if (exists && rebuild) {
    ...
    rmSync(chipmateSandboxWorkerForBinary(targetBinPath), { force: true })
  ...
  await copyChipMateSandboxWorker(sourceBinPath, targetBinPath)
  chmodSync(targetBinPath, 0o755)
  await ensureLocalHelpers()
  ...
  log(`Copied CLI binary from ${relative(packagesDir, sourceBinPath)} -> ${relative(chipmateVscodeDir, targetBinPath)}`)
}
...
function removeDist() {
  ...
  log(`Removed ${relative(chipmateVscodeDir, distDir)} to force rebuild.`)
}
