...
export namespace RemoteCommand {
  ...
  export const Response = z
    .object({
      ...
      // chipmate_change - K1 W1: `canExitSession` is an INDEPENDENT producer
      // contract advertised by every CLI (interactive TUI and headless
      // `chipmate remote` alike) so the mobile client can rely on the
  ...
  // The only entry from BUILTIN_COMMANDS (chipmate/session/builtin-commands) exposed
  ...
  export function build(items: ReadonlyArray<CommandInfo>, exitAvailable = false): Response {
    ...
    // chipmate_change - K1 W1: always advertise `canExitSession: true`. This
    ...
    // command entry on RemoteExit.get()). A headless `chipmate remote` host has
  ...
  export function create(services: Services): Interface {
    return {
      ...
      execute: async (input) => {
        // chipmate_change - enforce membership in the supplied bounded
