/**
 * ChipMateClaw chat panel
 *
 * Renders a scrollable message list, a slash-command autocomplete popup,
 * and a textarea for chatting with the ChipMateClaw bot via ChipMate Chat.
...
export function ClawChat(props: {
  ...
  // `bot:chipmateclaw:{sandboxId}` — the bot is the only non-self member in 1:1
  // chats, so we resolve bot ids to the user-configured `botName` (which
  // falls back to "ChipMateClaw" upstream) and render any human collaborators
