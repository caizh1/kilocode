...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.chipmate/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.chipmate/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.chipmate/rules/rules.md` | Not available |

### Organization Features

| Feature | Legacy | New |
|---|---|---|
| Share modes banner | `ChipMateShareModesBanner` component | Not available |
