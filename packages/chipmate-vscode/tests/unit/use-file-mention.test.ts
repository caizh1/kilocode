  ...
  it("keeps previous file results visible while the next search is pending", async () => {
    ...
    for (const handler of handlers) {
      handler({
        ...
        dir: "/repo",
        paths: ["packages/chipmate-vscode/src/extension.ts"],
        items: [{ path: "packages/chipmate-vscode/src/extension.ts", type: "opened-file" }],
      })
    }

    expect(mention.mentionResults()).toEqual([
      { type: "opened-file", value: "packages/chipmate-vscode/src/extension.ts" },
      FILE_PICKER_RESULT,
    ])
