...
describe("ChipMate v2 coexistence boundary", () => {
  ...
  it("does not expose legacy migration UI or legacy marketplace paths", () => {
    ...
    const about = fs.readFileSync(path.join(root, "webview-ui/src/components/settings/AboutChipMateTab.tsx"), "utf8")
    ...
    expect(install).not.toContain('target === "project" ? ".chipmate"')
  })
  ...
  it("drops inherited state/config paths before spawning the CLI", () => {
    const env = resolveManagedServerEnv(
      {
        CHIPMATE_CONFIG: "/official/chipmate.jsonc",
        CHIPMATE_CONFIG_CONTENT: "{}",
        CHIPMATE_CONFIG_DIR: "/official/config",
        CHIPMATE_DB: "/official/chipmate.db",
        CHIPMATE_TUI_CONFIG: "/official/tui.json",
        CHIPMATE_ZED_DB: "/official/zed.db",
        PATH: "/usr/bin",
      },
