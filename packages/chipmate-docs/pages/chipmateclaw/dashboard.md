---
title: "ChipMateClaw Dashboard Reference"
description: "Managing your ChipMateClaw instance from the dashboard"
---

# ChipMateClaw Dashboard

This page covers everything you can do from the ChipMateClaw dashboard. For getting started, see [ChipMateClaw Overview](/docs/chipmateclaw/overview).

{% image src="/docs/img/chipmateclaw/dashboard.png" alt="Connect account screen" width="800" caption="The ChipMateClaw Dashboard" /%}

## Personal and Organization Instances

The dashboard controls are the same for personal and organization-scoped ChipMateClaw instances. Organization instances are selected from the organization context and are listed separately from your **Personal** instance. Availability depends on your organization membership and ChipMateClaw entitlement.
...
For access to the full catalog of 335+ models, use the `/model` and `/models` commands in the [Control UI Chat](/docs/chipmateclaw/control-ui#changing-models).

### Channels

ChipMate Chat is always available as ChipMateClaw's first-party channel and does not need a token. You can also connect Telegram, Discord, and Slack by entering bot tokens in the Settings tab. See [Connecting Chat Platforms](/docs/chipmateclaw/chat-platforms) for setup instructions.
...
See [Version Pinning](/docs/chipmateclaw/control-ui/version-pinning) for details.
...
> 💡 **Tip**
> If your instance is in a crash loop and you can't access the Control UI, try **Restore Config** from the ChipMateClaw dashboard first before redeploying.
...
When your instance is running you can access the [OpenClaw Control UI](/docs/chipmateclaw/control-ui) — a browser-based dashboard for managing your agent, channels, sessions, exec approvals, and more:

1. Click **Open** to launch the OpenClaw web interface in a new tab

See the [Control UI reference](/docs/chipmateclaw/control-ui) for a full overview of its capabilities.

{% callout type="warning" %}
Do not use the **Update** feature in the OpenClaw Control UI to update ChipMateClaw. Use **Redeploy** from the ChipMateClaw Dashboard instead. Updating via the Control UI will not apply the correct ChipMateClaw platform image and may break your instance.
{% /callout %}
...
You need to **approve** each request before the user or device can interact with your agent. See [Pairing Requests](/docs/chipmateclaw/chat-platforms#pairing-requests) for details.

## Changelog

The dashboard shows recent ChipMateClaw platform updates. Each entry is tagged as a **feature** or **bugfix**, and some include a deploy hint:
...
- [OpenClaw Control UI](/docs/chipmateclaw/control-ui)
