 ...
 *
 * ChipMate's VS Code webviews use the `chipmate.v2.fontSize` setting, not
 * VS Code editor font-size or raw pixel declarations. This keeps the ChipMate UI
 * independently scalable across sidebar, settings, Agent Manager, ChipMateClaw,
 * diff viewers, code blocks, and shared chipmate-ui controls.
 */
...
describe("webview font-size architecture", () => {
  it("does not use raw font-size pixels or VS Code editor font-size variables in runtime webview UI", () => {
    ...
    expect(
      violations,
      `Use the ChipMate webview font-size tokens instead of raw pixels or VS Code editor font-size variables.\n` +
        `Preferred tokens: var(--font-size-base), var(--font-size-small), or var(--chipmate-font-size-N).\n\n` +
  ...
  it("uses scalable line heights in polished tool previews", () => {
    const files = [
      path.join(REPO, "packages/chipmate-ui/src/components/basic-tool.css"),
      path.join(REPO, "packages/chipmate-ui/src/components/message-part.css"),
    ]
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(util, "buildWebviewHtml must define scaled ChipMate font tokens").toContain("--chipmate-font-size-")

    const missing = WATCHED_PROVIDERS.filter((file) => !fs.readFileSync(file, "utf-8").includes("watchFontSizeConfig"))
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when chipmate.v2.fontSize changes.\n` +
