...
import { ConsoleAssets } from "../../src/chipmate/console/assets"

const original = process.env.CHIPMATE_CONSOLE_ASSET_DIR

afterEach(() => {
  if (original === undefined) delete process.env.CHIPMATE_CONSOLE_ASSET_DIR
  else process.env.CHIPMATE_CONSOLE_ASSET_DIR = original
})
...
async function assets(dir: string) {
  ...
  await Bun.write(path.join(dir, "assets", "app.js"), "console.log('chipmate')")
}

describe("ChipMate Console UI routes", () => {
  test("serves the console index for /console and SPA routes", async () => {
    await using tmp = await tmpdir()
    process.env.CHIPMATE_CONSOLE_ASSET_DIR = tmp.path
  ...
  test("serves console assets without falling back on missing files", async () => {
    ...
    expect(await Bun.file(asset.file).text()).toContain("chipmate")
