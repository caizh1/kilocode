 ...
 * then copies it into packages/chipmate-vscode/bin/chipmate.
 *
...
const targetBinPath = join(targetBinDir, "chipmate")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@chipmate/cli-${process.platform}-${process.arch}`, "bin", "chipmate")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/chipmate`)
  } catch (err) {
...
// Watch for changes
log(`Watching ${relative(chipmateVscodeDir, opencodeSrcDir)}/ for changes...`)
