...
export const Action = Schema.NullOr(Schema.Literals(["ask", "allow", "deny"])) // chipmate_change - nullable allows null as a delete sentinel
...
const InputObject = Schema.StructWithRest(
  Schema.Struct({
    ...
    markdown_source: Schema.optional(Rule), // chipmate_change - explicitly authorize external agent/command sources
    ...
    agent_manager: Schema.optional(Rule), // chipmate_change
...
const normalizeInput = (input: Schema.Schema.Type<typeof InputSchema>): Schema.Schema.Type<typeof InputObject> =>
  input === null || typeof input === "string" ? { "*": input } : input // chipmate_change
