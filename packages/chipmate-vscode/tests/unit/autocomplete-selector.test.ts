...
describe("autocomplete document selector", () => {
  ...
  it("uses the matching FIM model for notebook fallback", () => {
    expect(notebookModel("chipmate", "inception/mercury-next-edit").id).toBe("chipmate/inception/mercury-edit-2")
    expect(notebookModel("inception", "mercury-next-edit").id).toBe("inception/mercury-edit-2")
    expect(notebookModel("chipmate", "mistralai/codestral-2508").id).toBe("chipmate/mistralai/codestral-2508")
  })
})
