...
|---|---|---|---|
| Document artifact manager | Yes | T1 artifact tests plus M1 checklist | S4 direct runtime smoke `docs/chipmate-feature-migration-validation-runs/20260708-233500-runtime-s3-s4-direct-tool-smoke/summary.md` is `NEEDS_REVIEW`, guardrails passed, `declare_artifact` was called, and `.kilo/artifacts/runtime-smoke-s4/artifact.json` exists. Installed target runtime/M11 review still required. |
...
- Source-backed detail-design skill as a Kilo-native skill using Kilo evidence tools.
...
- Kilo Document RAG replacement.
- Kilo qwen-direct autocomplete replacement.
- Full draw.io chain.
...
### Kilo native capabilities verified as preserved
...
| Qwen direct autocomplete | C3 and installed smoke S16 | PARTIAL | Installed manifest still contains `kilo.autocomplete.qwen.*`; full C3 and S16 runtime evidence pending. |
...
|---|---|---|
| Artifact manifest | `.kilo/artifacts/.../artifact.json` | PARTIAL; P8 verifies installed artifact open/export diagnostics in a temporary workspace, and T10 covers repeat diagnostics export with root-level diagnostics JSON; chat generation/list prompt remains pending. |
...
| VSIX package | Filename, version, size, SHA256, handoff files | Current rebuild after source-backed contract-boundary cleanup: `packages/kilo-vscode/out/kilo-vscode-linux-x64-baseline.vsix`, `151641659` bytes / `144.62 MiB`, SHA256 `95218162b0d0a09c6425c80a10e9745569c1b021edd9d666a5f43278d31458ad`; `packages/kilo-vscode/out/kilo-vscode-win32-x64-baseline.vsix`, `168397172` bytes / `160.60 MiB`, SHA256 `3c6b9943ecf8259379c6f75b1ff321bb83677c092aac3974584524fcd90e489d`; version `0.0.38`; handoff files `packages/kilo-vscode/out/SHA256SUMS-chipmate-0.0.38-offline.txt`, `packages/kilo-vscode/out/OFFLINE_RELEASE_NOTES-chipmate-0.0.38.md`, `packages/kilo-vscode/out/OFFLINE_RELEASE_INDEX-chipmate-0.0.38.json`, `packages/kilo-vscode/out/OFFLINE_RELEASE_INDEX-chipmate-0.0.38.md`, `packages/kilo-vscode/out/OFFLINE_BUNDLE_INDEX-chipmate-0.0.38.json`, and `packages/kilo-vscode/out/OFFLINE_BUNDLE_INDEX-chipmate-0.0.38.md`; clean bundle `packages/kilo-vscode/out/chipmate-0.0.38-offline-handoff.tar.gz`, SHA256 `97a70108ae648a1998168d679c3c65638ea7756926860fad113cd06c31d2dba5`, size `317784738`; checksum and exact tar content verification PASS in `docs/chipmate-feature-migration-validation-runs/20260708-114900-offline-handoff-verify-after-repackage/summary.md`. |
...
| Kilo Document RAG replacement | Plan non-goal and existing `document_search` automated coverage | PARTIAL; S3 pending |
| Kilo qwen-direct autocomplete replacement | Installed manifest and T6 extension-host settings smoke expose existing `kilo.autocomplete.qwen.*` settings | PARTIAL; S16 pending |
...
- Evidence: installed VSIX contribution spot-check plus T9/P8 extension-host settings/command smoke show `kilo.autocomplete.qwen.*` settings, qwen diagnostics command, and qwen log command remain.
- Result: PARTIAL.
...
- If installed smoke S1-S3 is not complete, final status cannot claim Kilo QA or Document RAG preservation.
...
| Check | Status | Evidence | Interpretation |
|---|---:|---|---|
| Latest S3 Document RAG readiness rerun | ERROR_NEEDS_REVIEW | `docs/chipmate-feature-migration-validation-runs/20260709-120000-document-rag-readiness-rerun/document-rag-readiness-summary.md`; `docs/chipmate-feature-migration-validation-runs/20260709-120000-document-rag-readiness-rerun/document-rag-provider-diagnostics.md` | Document RAG still does not expose/use `document_search` because the openai-compatible embedder returns HTTP `503`. Kilo native QA is not replaced, but Document RAG runtime preservation remains unproven. |
