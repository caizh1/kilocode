...
//
// ChipMate uses Npm.Service (arborist) for dependency installation and may write
// a .gitignore inside the .chipmate config dir. Users may have pnpm or yarn as
// their system package manager, which can produce lockfiles in the .chipmate/
...
test(".gitignore in .chipmate config dir includes pnpm and yarn lockfile patterns", async () => {
  await using tmp = await tmpdir()
  const dir = path.join(tmp.path, "a")
  const chipmate = path.join(dir, ".chipmate")
  await fs.mkdir(chipmate, { recursive: true })
  ...
  const ignore = await Filesystem.readText(path.join(chipmate, ".gitignore"))
