...
import { filterPromptTrainingModels, nonEmptyProviders } from "../../../src/chipmate/provider/model-filter"

function model(id: string, training?: boolean): Provider.Model {
  return {
    id: ModelV2.ID.make(id),
    providerID: ProviderV2.ID.chipmate,
    api: { id: "chipmate", url: "https://api.chipmate.ai", npm: "@chipmate/chipmate-gateway" },
    name: id,
    capabilities: {
      temperature: true,
      reasoning: false,
...
describe("prompt-training model filter", () => {
  test("hides only explicitly marked ChipMate Gateway models", () => {
    const providers = {
      chipmate: provider("chipmate", {
    ...
    expect(Object.keys(result.chipmate.models)).toEqual(["private", "unknown"])
    expect(Object.keys(result.other.models)).toEqual(["training"])
    expect(Object.keys(providers.chipmate.models)).toEqual(["training", "private", "unknown"])
  })

  test("preserves the catalog when disabled", () => {
    const providers = { chipmate: provider("chipmate", { training: model("training", true) }) }
