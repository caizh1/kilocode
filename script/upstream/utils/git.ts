 ...
 * Paths unchanged upstream keep their prior ChipMate content, including ChipMate-only
...
function compatTag(message: string): string | null {
  const prefix = "refactor: chipmate compat for "
...
export async function findLatestCompatCommit(base: string, target: string): Promise<CompatBase | null> {
  const grep = "^refactor: chipmate compat for "
 ...
 * Used to clean build artifacts from ChipMate-specific directories after checking
...
/**
 * Check if the "ours" version of a conflicted file contains chipmate_change markers.
 ...
 */
export async function oursHasChipMateChanges(file: string): Promise<boolean> {
  const result = await $`git show :2:${file}`.quiet().nothrow()
  if (result.exitCode !== 0) return false
  return result.stdout.toString().includes("chipmate_change")
}
...
export async function trainRerere(grep: string): Promise<number> {
  ...
  const dir = join(tmpdir(), `chipmate-rerere-train-${randomUUID()}`)
