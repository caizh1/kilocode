...
import { loadChipMateProfile, logoutChipMate, setChipMateOrganization, type ChipMateProfileData, type ProjectQuery } from "../../client"
...
type Org = NonNullable<ChipMateProfileData["profile"]["organizations"]>[number]
type State = { kind: "connected"; data: ChipMateProfileData } | { kind: "disconnected" }

async function load(input: ProjectQuery): Promise<State> {
  if (wasDisconnected()) return { kind: "disconnected" }
  try {
    const data = await loadChipMateProfile(input)
...
export function ProfileRoute() {
  ...
  const login = () => page(params(), "/chipmate/login")
  ...
  function choose(id: string | null) {
    ...
    setSaving(next)
    setError("")
    void setChipMateOrganization(current, id)
        ...
        <div class="profile-page">
          <Show when={!disconnected()}>
            <header class="profile-header">
              <div>
                <p class="eyebrow">ChipMate Account</p>
                <h1>Your Profile</h1>
                <p>Manage your ChipMate identity, account context, credits, and billing shortcuts.</p>
              </div>
          ...
          <Show when={disconnected()}>
            <Card class="profile-connect-card">
              ...
              <div class="profile-connect-copy">
                <p class="eyebrow">ChipMate Account</p>
                <h1>Connect your ChipMate account</h1>
                <p>Sign in to view your credits, organizations, and account details in ChipMate Console.</p>
              </div>
              <A
                class="profile-primary-link"
                data-component="button"
          ...
          <Show when={!server.query() && !server.discoverable()}>
            <Card class="profile-banner" variant="warning">
              <strong>ChipMate server not found</strong>
              <span>Start ChipMate Console from a running ChipMate server or pass a server URL with ?server=.</span>
            </Card>
          </Show>
                  ...
                  <div class="profile-org-list">
                    ...
                    >
                      ...
                      <span class="profile-org-body">
                        ...
                        <span>Your personal ChipMate credits and settings</span>
                      </span>
