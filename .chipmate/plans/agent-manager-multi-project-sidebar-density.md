...
Worktree: `/Users/marius/Documents/git/chipmate/.chipmate/worktrees/mewing-profit`
All paths below are relative to `packages/chipmate-vscode/`.

Rules:

- Do exactly these edits. Do not refactor anything else.
- Solid.js, not React: `class=`, not `className=`.
- Do **not** add `chipmate_change` markers. This package is ChipMate-owned and CI fails if you do.
- Do **not** add new i18n strings. None are needed.
- Out of scope: the per-project `SESSIONS` list in the tree (tracked in
  https://github.com/ChipMate-Org/chipmate/issues/12928). Do not touch `UnassignedSessionsSection.tsx`.
...
```bash
cd packages/chipmate-vscode && bun run typecheck && bun run lint
```
...
feasible, but it does not fit this repo set: of the local projects, ~17 are `ChipMate-Org` remotes and
would share one identical ChipMate logo, and ~20 have no git remote at all and would show nothing.
...
Enable `chipmate-code.new.experimental.multiProject` in ChipMate Settings → Experimental, add a second
