...
export const CHIPMATE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CHIPMATE_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CHIPMATE_MODE_KEY, CHIPMATE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CHIPMATE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CHIPMATE_MODE_KEY, stack.at(-1)?.mode ?? CHIPMATE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CHIPMATE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CHIPMATE_MODE_KEY, undefined)
...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("ChipMate mode stack is not registered for this keymap") // chipmate_change
  return value
}
