...
class McpSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test loads configured mcp servers with runtime status`() {
        ...
        edt {
            ...
            assertEquals(ChipMateBundle.message("settings.agentBehavior.mcp.connect"), github.cells.single { it.id == "connect" }.label)
            ...
            assertEquals(ChipMateBundle.message("settings.agentBehavior.mcp.disconnect"), filesystem.cells.single { it.id == "disconnect" }.label)
    ...
    fun `test toolbar add hint is visible`() {
        ...
        edt {
            assertTrue(text(panel).contains(ChipMateBundle.message("settings.agentBehavior.mcp.addHint")))
            true
        }
    }
    ...
    fun `test failed mcp action shows settings error`() {
        ...
        val message = ChipMateBundle.message("settings.agentBehavior.mcp.action.failed")
    ...
    private fun install() {
        ...
        val ready = ChipMateAppStateDto(ChipMateAppStatusDto.READY, config = ConfigDto())
