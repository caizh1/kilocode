...
export const themes: Theme[] = [
  ...
  { id: "chipmate", swatches: ["#0c0a09", "#fafaf9", "#f9f76f", "#a6a09b", "#f9f76f", "#44403b"] },
  { id: "chipmate-v1", swatches: ["#1e1e1e", "#cccccc", "#faf74f", "#007acc", "#007fd4", "#3c3c3c"] },
...
export function useTuiUiSettings() {
  const ctx = useConfig()
  const current = createMemo(() => ctx.data()?.tui.theme ?? "chipmate")
  ...
  function item(id: string) {
    return themes.find((row) => row.id === id) ?? custom(id || "chipmate")
  }
