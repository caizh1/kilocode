package ai.chipmate.client.session.views.question
...
class QuestionView(
    ...
    private val back = HoverIcon().apply {
        ...
        toolTipText = ChipMateBundle.message("session.question.back")
    ...
    private val fwd = HoverIcon().apply {
        ...
        icon = ico
        disabledIcon = IconLoader.getDisabledIcon(ico)
        toolTipText = ChipMateBundle.message("session.question.next")
    ...
    private fun syncPage() {
        ...
        body.removeAll()
        if (review(q)) {
            card.setHeader(ChipMateBundle.message("session.question.review.title"))
            addReview(q)
        } else {
            val item = q.items[idx]
            val hint = ChipMateBundle.message(
    ...
    private fun syncHeader(q: Question) {
        ...
        summary.text = ChipMateBundle.message("session.question.summary", shown, total)
    ...
    private fun syncFooter(q: Question) {
        ...
        actions.add(BaseQuestionView.Action(ID_DISMISS, ChipMateBundle.message("session.question.dismiss"), primary = false) { doReject() })

        if (review(q)) {
            actions.add(BaseQuestionView.Action(ID_BACK, ChipMateBundle.message("session.question.back"), primary = false) { goBack() })
            actions.add(BaseQuestionView.Action(ID_MAIN, ChipMateBundle.message("session.question.submit"), primary = true) { doReply() })
        } else {
            val label = when {
                direct(q) -> ChipMateBundle.message("session.question.submit")
                lastItem(q) -> ChipMateBundle.message("session.question.review")
                else -> ChipMateBundle.message("session.question.next")
            }
    ...
    private fun reviewRow(item: QuestionItem, i: Int): JPanel {
        ...
        val answer = text(
            joined.ifBlank { ChipMateBundle.message("session.question.review.notAnswered") },
            UiStyle.Colors.fg(),
            true,
        )
    ...
    private fun customRow(item: QuestionItem, set: MutableSet<String>): JPanel {
        ...
        val label = text(ChipMateBundle.message("session.question.custom.label"), UiStyle.Colors.fg(), true)
    ...
    private fun buildCustomEditor(): SessionEditorTextField {
        ...
        ed.setPlaceholder(ChipMateBundle.message("session.question.custom.placeholder"))
