...
class ChipMateProcessJobTest {

    /**
     * The kill-on-close job is Windows-only. Everywhere else (including CI) [ChipMateProcessJob.assign]
    ...
    fun `assign is a no-op on non-Windows platforms`() {
        ...
        assertNull(ChipMateProcessJob.assign(4321L, TestLog()))
    }
    ...
    fun `closing the job terminates a disposable child on Windows`() {
        ...
        try {
            val job = assertNotNull(ChipMateProcessJob.assign(child.pid(), TestLog()), "job assigned on Windows")
