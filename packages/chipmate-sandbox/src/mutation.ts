...
declare const CHIPMATE_SANDBOX_MUTATION_WORKER_PATH: string

function worker() {
  if (typeof CHIPMATE_SANDBOX_MUTATION_WORKER_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./chipmate-sandbox-mutation-worker.ts", import.meta.url)), environment: {} }
  }
  const path = CHIPMATE_SANDBOX_MUTATION_WORKER_PATH.startsWith(".")
    ? fileURLToPath(new URL(CHIPMATE_SANDBOX_MUTATION_WORKER_PATH, import.meta.url))
    : resolve(dirname(process.execPath), CHIPMATE_SANDBOX_MUTATION_WORKER_PATH)
...
const CurrentRunner = Context.Reference<Runner>("@chipmate/sandbox/CurrentMutationRunner", {
  defaultValue: () => mutate,
})
