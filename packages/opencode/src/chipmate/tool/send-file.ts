 ...
 * path is impossible by design — the tool is gated on `ChipMateSessions.remoteStatus()
...
export const SendFileTool = Tool.define<typeof Params, {}, FSUtil.Service, "send_file">(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          if (!ChipMateSessions.remoteStatus().connected) {
            return fail(
              "Cannot send files: this session is not connected to ChipMate cloud. Delivery needs an active link.",
            )
          }
          ...
          // chipmate_change start — authorize missing and directory paths with the same
          // security sequence as read.ts before any file inspection via ChipMateReadObject.
          ...
          if (info.type === "Directory") {
            ...
            const explicit =
              typeof ctx.extra?.["referenceRoot"] === "string"
                ? yield* ChipMateReference.path(fs, ctx.extra["referenceRoot"], target).pipe(
                    Effect.option,
          ...
          // 1. Resolve via ChipMateReadObject.file (same authorization sequence as read.ts)
          const file = yield* ChipMateReadObject.file(requested)

          // 2. Authorization — same pattern as read.ts
          const explicit =
            typeof ctx.extra?.["referenceRoot"] === "string"
              ? yield* ChipMateReference.path(fs, ctx.extra["referenceRoot"], file.target).pipe(
                  Effect.option,
                  Effect.map((result) => result._tag === "Some" && result.value),
                )
              : false
          ...
          return yield* ChipMateReadObject.use(file, (bound) =>
            Effect.gen(function* () {
              ...
              return {
                ...
                output: `File ${basename} (${bytes.byteLength} bytes, ${mime}) delivered to the user's ChipMate app. Older app builds ignore non-image file attachments — make sure the user has an up-to-date app to see the delivery.`,
                metadata: {},
                attachments: [
                  {
                    type: "file" as const,
