...
- Confirm `ai.chipmate.jetbrains` is the final permanent Marketplace plugin ID before the first upload.
...
- Confirm the plugin is owned by the correct ChipMate vendor or organization.
...
- Confirm `CHIPMATE_MAINTAINER_APP_ID` and `CHIPMATE_MAINTAINER_APP_SECRET` are available to create release PRs and immediate release tags.
...
- Review and edit `packages/chipmate-jetbrains/CHANGELOG.md` in the generated release PR.
...
- Confirm the follow-up `publish-jetbrains-bundled` workflow completes and attaches `chipmate-code-x.y.z-rc.n-bundled.zip` to the prerelease.
...
- Watch the `publish-jetbrains` workflow.
- Confirm the follow-up `publish-jetbrains-bundled` workflow completes, attaches `chipmate-code-x.y.z-bundled.zip`, and updates `https://chipmate-org.github.io/chipmate/jetbrains/updatePlugins.xml`.
