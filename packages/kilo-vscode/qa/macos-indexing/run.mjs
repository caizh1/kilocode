...
import { utils, write } from "../../../kilo-indexing/node_modules/xlsx/xlsx.mjs"

const prefix = "@kilo-indexing:"
...
const keyenv = argv["api-key-env"] ?? "KILO_EMBEDDING_API_KEY"
...
const indexer = join(bin, "kilo-indexer")
...
function driver(workspace, id) {
  const item = new Driver({
    ...
    env: {
      ...
      KILO_RIPGREP_PATH: rg,
      KILO_TREE_SITTER_WASM_DIR: join(bin, "tree-sitter"),
...
class Driver {
  constructor(options) {
    ...
    this.child.once("exit", (code, signal) => {
      ...
      this.fail(new Error(`kilo-indexer 意外退出：code=${code} signal=${signal}`))
    })
  }
