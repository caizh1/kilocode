...
internal object MdCommon {
    ...
    private val single = setOf("readme.md", "package.json", "tsconfig.json", "jsconfig.json", "chipmate.json", "chipmate.jsonc")
    ...
    fun rules(opts: MdStyle): String {
        ...
        rules.append("a.chipmate-file-ref, code a.chipmate-file-ref { color: ${hex(SessionUiStyle.View.Markdown.string())}; font-family: '${css(opts.codeFont)}', monospace; text-decoration: underline } ")
    ...
    private fun paths(text: String): String {
        ...
        return ref.replace(text) { match ->
            ...
            val href = path.replace(" ", "%20")
                .replace("(", "%28")
                .replace(")", "%29")
            "<a class=\"chipmate-file-ref\" href=\"${attr(href)}\">$path</a>"
        }
    }
