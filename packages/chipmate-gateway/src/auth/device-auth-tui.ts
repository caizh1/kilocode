...
import { getChipMateProfile, getChipMateDefaultModel, defaultOrganizationId } from "../api/profile.js"
import { CHIPMATE_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
import type { AuthOuathResult } from "@chipmate/plugin"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${CHIPMATE_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${CHIPMATE_API_BASE}/api/device-auth/codes/${code}`)
...
export async function authenticateWithDeviceAuthTUI(inputs?: Record<string, string>): Promise<AuthOuathResult> {
  ...
  return {
    ...
    async callback() {
      ...
      const profile = await getChipMateProfile(token).catch(() => undefined)
      const organizationId = profile ? defaultOrganizationId(profile) : undefined

      // Fetch default model
      await getChipMateDefaultModel(token, organizationId)
