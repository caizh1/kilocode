...
import type { AgentManagerRequest, Session } from "@chipmate/sdk/v2/client"
...
describe("AgentManagerOrchestrationBridge", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(root, ".chipmate"), { recursive: true })
    fs.mkdirSync(dir)
  ...
  function harness() {
    ...
    const request = (value: AgentManagerRequest, directory = root) =>
      handlers.event?.(
        { id: `event-${value.id}`, type: "chipmate.agent_manager.requested", properties: value } as SSEPayload,
        directory,
      )
  ...
  it("recovers pending requests for the root and managed worktree directories", async () => {
    ...
    expect(test.client.chipmate.agentManager.list).toHaveBeenCalledWith({ directory: root })
    expect(test.client.chipmate.agentManager.list).toHaveBeenCalledWith({ directory: dir })
    expect(test.promptAsync).toHaveBeenCalledTimes(1)
    expect(test.replies[0]).toEqual({
      requestID: "amr_prompt",
      directory: dir,
