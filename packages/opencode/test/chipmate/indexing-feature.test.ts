...
} from "../../src/chipmate/indexing-feature"
...
describe("indexing plugin helpers", () => {
  test("detects plugin-enabled configs", () => {
    ...
    expect(indexingEnabled({ plugin: ["@chipmate/chipmate-indexing@1.0.0"] })).toBe(true)
  })
  ...
  test("does not add duplicate indexing plugin", () => {
    const list = ensureIndexingPlugin(["@chipmate/chipmate-indexing@1.0.0"], INDEXING_PLUGIN)
    expect(list).toEqual(["@chipmate/chipmate-indexing@1.0.0"])
  })
