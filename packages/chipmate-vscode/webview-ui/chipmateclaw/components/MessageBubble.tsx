// Individual chat message bubble — renders ChipMate Chat ContentBlock[] content
// with reactions, action approvals, edit/delete/reply controls.
//
// Layout mirrors the web client (cloud/apps/web/src/app/(app)/claw/chipmate-chat/
...
export function MessageBubble(props: MessageBubbleProps) {
  ...
  const copyText = async () => {
    ...
    try {
      ...
      showToast({ title: t("chipmateClaw.message.copied"), variant: "success", duration: 2000 })
    } catch {
      showToast({ title: t("chipmateClaw.message.copyFailed"), variant: "error", duration: 3000 })
    }
  }

  return (
    <div class={`chipmateclaw-msg chipmateclaw-msg-${variant()}`}>
      <div class="chipmateclaw-msg-column">
        {/* Author label — above the bubble for bot messages only */}
        <Show when={showAuthor()}>
          <span class="chipmateclaw-msg-author">{props.assistantName ?? t("chipmateClaw.message.bot")}</span>
        </Show>

        {/* Reply preview — above the bubble, aligned with the author label */}
        <Show when={props.replyToMessage}>
          {(reply) => (
            <div class="chipmateclaw-msg-reply">
              <span class="chipmateclaw-msg-reply-arrow" aria-hidden="true">
                ↩
              </span>
              <Show
                when={!reply().deleted}
                fallback={<span class="chipmateclaw-msg-reply-deleted">{t("chipmateClaw.message.replyDeleted")}</span>}
              >
                <span class="chipmateclaw-msg-reply-text">
                  {(() => {
        ...
        <div class="chipmateclaw-msg-bubble-wrap">
          <Show when={!props.message.deleted && !isEditing() && !isDeleting() && !isOptimistic()}>
            <div class="chipmateclaw-msg-toolbar">
              <button
                type="button"
                class="chipmateclaw-iconbtn-sm"
                onClick={() => setShowReactionPick((v) => !v)}
                title={t("chipmateClaw.message.react")}
                aria-label={t("chipmateClaw.message.react")}
              >
              ...
              <Show when={!empty()}>
                <button
                  ...
                  class="chipmateclaw-iconbtn-sm"
                  onClick={copyText}
                  title={t("chipmateClaw.message.copy")}
                  aria-label={t("chipmateClaw.message.copy")}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <Show when={!props.message.deliveryFailed}>
                <button
                  ...
                  title={t("chipmateClaw.message.reply")}
                  aria-label={t("chipmateClaw.message.reply")}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <Show when={props.isOwn && !props.message.deliveryFailed}>
                <button
                  ...
                  class="chipmateclaw-iconbtn-sm"
                  onClick={startEdit}
                  title={t("chipmateClaw.message.edit")}
                  aria-label={t("chipmateClaw.message.edit")}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <Show when={props.isOwn}>
                <button
                  type="button"
                  class="chipmateclaw-iconbtn-sm chipmateclaw-iconbtn-danger"
                  onClick={() => props.onRequestDelete(props.message.id)}
                  title={t("chipmateClaw.message.delete")}
                  aria-label={t("chipmateClaw.message.delete")}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <Show when={showReactionPick()}>
            <div class="chipmateclaw-msg-reactionpick">
              <For each={["👍", "❤️", "😂", "🎉", "🚀", "👀"]}>
                {(emoji) => (
                  <button
                    type="button"
                    class="chipmateclaw-msg-reactionpick-btn"
                    onClick={() => {
          ...
          <div class="chipmateclaw-msg-bubble">
            <Show
              when={!props.message.deleted}
              fallback={<span class="chipmateclaw-msg-deleted">{t("chipmateClaw.message.deleted")}</span>}
            >
              <Show when={isEditing()}>
                <div class="chipmateclaw-msg-edit">
                  <textarea
                    class="chipmateclaw-msg-edit-input"
                    value={editText()}
                    ...
                    autofocus
                  />
                  <div class="chipmateclaw-msg-edit-actions">
                    <button
                      type="button"
                      class="chipmateclaw-iconbtn-sm"
                      onClick={saveEdit}
                      title={t("chipmateClaw.message.save")}
                    >
                      ✓
                    </button>
                    <button
                      ...
                      class="chipmateclaw-iconbtn-sm"
                      onClick={cancelEdit}
                      title={t("chipmateClaw.message.cancel")}
                    >
                      ×
                    </button>
                  </div>
              ...
              <Show when={isDeleting()}>
                <div class="chipmateclaw-msg-confirm-delete">
                  <span>{t("chipmateClaw.message.confirmDelete")}</span>
                  <button
                    type="button"
                    class="chipmateclaw-iconbtn-sm"
                    onClick={() => props.onConfirmDelete(props.message.id)}
                    title={t("chipmateClaw.message.confirmDelete")}
                  >
                    ✓
                  </button>
                  <button
              ...
              <Show when={!isEditing() && !isDeleting()}>
                <div class="chipmateclaw-msg-body">
                  <Show
                    when={!empty()}
                    fallback={<span class="chipmateclaw-msg-thinking">{t("chipmateClaw.message.thinking")}</span>}
                  >
                    <Show when={isBot()} fallback={<span class="chipmateclaw-msg-text">{textContent()}</span>}>
                ...
                {(block) => {
                  ...
                  return (
                    <Show
                      when={!block.resolved}
                      fallback={
                        <div class="chipmateclaw-msg-actions-resolved">
                          <span class="chipmateclaw-msg-actions-resolved-icon">
                    ...
                    >
                      <div class="chipmateclaw-msg-actions">
                        <For each={block.actions}>
                          {(action) => (
                            <button
                              type="button"
                              class={`chipmateclaw-msg-action chipmateclaw-msg-action-${action.style}`}
            ...
            <Show when={!isEditing() && !isDeleting()}>
              <div class="chipmateclaw-msg-footer">
                <Show when={props.message.deliveryFailed}>
                  <span class="chipmateclaw-msg-failed">{t("chipmateClaw.message.notDelivered")}</span>
                </Show>
                <Show when={props.message.clientUpdatedAt && !props.message.deleted}>
                  <span>{t("chipmateClaw.message.edited")}</span>
                </Show>
          ...
          <Show when={!props.message.deleted && props.message.reactions.length > 0}>
            <div class="chipmateclaw-msg-reactions">
              <For each={props.message.reactions}>
                {(r) => (
                  <button
                    type="button"
                    class="chipmateclaw-msg-reaction-pill"
                    onClick={() => props.onRemoveReaction(props.message.id, r.emoji)}
                    title={t("chipmateClaw.message.removeReaction")}
                  >
                    <span>{r.emoji}</span>
                    <span>{r.count}</span>
                  </button>
