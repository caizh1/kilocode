...
- If shared upstream-owned files are involved, use the repository’s `chipmate_change` marker guidance.

## Verification
Run the most specific applicable check for the touched area, such as:
- CLI: `bun test ./path/to/test.ts` from `packages/opencode/`
- VS Code extension: `bun run test:unit` from `packages/chipmate-vscode/`
