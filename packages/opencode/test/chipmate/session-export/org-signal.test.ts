...
import { getActiveOrg, resetOrgSource, setOrgSource } from "@/chipmate/session-export/eligibility"
...
describe("getActiveOrg", () => {
  ...
  afterEach(() => {
    if (env === undefined) delete process.env.CHIPMATE_ORG_ID
    else process.env.CHIPMATE_ORG_ID = env
    resetOrgSource()
  })
  ...
  test("returns env value when CHIPMATE_ORG_ID is set", async () => {
    setOrgSource(async () => ({ type: "org", id: "org_auth" }))
    process.env.CHIPMATE_ORG_ID = "org_envvar"
