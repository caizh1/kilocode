---
title: "Skills"
description: "Extend ChipMate capabilities with skills"
---

# Skills

ChipMate implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when ChipMate initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.chipmate` directory within your Home directory:

- Mac and Linux: `~/.chipmate/skills/`
- Windows: `\Users\<yourUser>\.chipmate\skills\`
...
Located in `.chipmate/skills/` within your project:
...
For interoperability with other tools, ChipMate also loads skills from:
...
You can configure extra skill locations and remote skill URLs in your `chipmate.jsonc` config (project or global):
...
When you change a remote skill's contents or file list, also change its `version`. On the next skill rediscovery (`/reload` or a new session), ChipMate downloads the complete new version before atomically replacing the cached directory. If any download fails, ChipMate keeps the previous cached version.
...
When multiple skills share the same name, project-level skills (`.chipmate/skills/`) take precedence over global skills (`~/.chipmate/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
- In the **CLI**: Skills are loaded when you start a new session or run `chipmate run`
...
In ChipMate, the `name` field **must match** the parent directory name:
...
- **Trusted skills only** — commands execute only in skills from trusted locations: global skills (such as `~/.chipmate/skills/`, `~/.agents/skills/`, and `~/.claude/skills/`), skills built into ChipMate, and absolute skill paths declared in global config. Project skills (`.chipmate/skills/` in a repository) and skills fetched from remote URLs never execute commands; their placeholders are replaced with a marker noting the skill is untrusted.
...
- **Kill switch** — set the `CHIPMATE_DISABLE_SKILL_SHELL` environment variable to disable embedded command execution entirely.
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.chipmate/skills/api-design
   ```
...
- **[ChipMate Marketplace repository](https://github.com/ChipMate-Org/chipmate-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.chipmate/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `chipmate.jsonc`.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [ChipMate Marketplace](https://github.com/ChipMate-Org/chipmate-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [ChipMate Marketplace repository](https://github.com/ChipMate-Org/chipmate-marketplace) can be manually downloaded into your `.chipmate/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/ChipMate-Org/chipmate-marketplace](https://github.com/ChipMate-Org/chipmate-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/ChipMate-Org/chipmate-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to ChipMate, see the [Contributing Guide](/docs/contributing).
