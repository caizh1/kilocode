...
import { DataProvider, type OpenDiffFn, type OpenFileFn } from "@chipmate/chipmate-ui/context/data"
...
const dicts: Record<StoryLocale, Record<string, string>> = {
  en: { ...appEn, ...amEn, ...uiEn, ...chipmateEn },
  zh: { ...appZh, ...amZh, ...uiZh, ...chipmateZh },
  zht: { ...appZht, ...amZht, ...uiZht, ...chipmateZht },
}
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded ChipMate Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent<{
  chipmateAuth?: boolean
  providers?: Record<string, Provider>
  connected?: string[]
}> = (props) => {
  const providers = () => props.providers ?? (MOCK_PROVIDERS as Record<string, Provider>)
  const connected = () => props.connected ?? Object.keys(providers())
  const models = createMemo(() => flattenModels(providers()))
  ...
  const value = {
    ...
    authMethods: () => ({}),
    authStates: () => (props.chipmateAuth ? { chipmate: "oauth" } : {}) as Record<string, ProviderAuthState>,
    isModelValid: () => true,
  }
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    getSessionModel: () => ({ providerID: "chipmate", modelID: "anthropic/claude-sonnet-4-6" }),
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: ChipMateNotification[]
