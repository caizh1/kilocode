...
import ai.chipmate.client.session.model.StepFinish
import ai.chipmate.client.session.model.Tool
import ai.chipmate.client.session.model.ToolExecState
import ai.chipmate.client.session.model.ToolKind
import ai.chipmate.client.session.ui.style.SessionEditorStyle
import ai.chipmate.client.session.ui.style.SessionUiStyle
import ai.chipmate.client.session.controller.SessionControllerTestBase
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.chipmate.rpc.dto.ChipMateAppStateDto(ai.chipmate.rpc.dto.ChipMateAppStatusDto.READY)
    ...
    fun `test branch changes badge invokes callback when clicked`() {
        ...
        assertEquals(ChipMateBundle.message("diff.editor.branch.tooltip"), badge.toolTipText)
        assertEquals(ChipMateBundle.message("diff.editor.branch.tooltip"), badge.accessibleContext.accessibleName)
    ...
    fun `test timeline colors honor semantic named color keys`() {
        val old = UIManager.getColor("ChipMate.Session.Timeline.Read")
        val color = Color(12, 34, 56)

        try {
            UIManager.put("ChipMate.Session.Timeline.Read", color)

            assertEquals(color.rgb, SessionUiStyle.Timeline.READ.rgb)
        } finally {
            UIManager.put("ChipMate.Session.Timeline.Read", old)
        }
    }
    ...
    private fun promptedHeader(): ai.chipmate.client.session.controller.SessionController {
        appRpc.state.value = ai.chipmate.rpc.dto.ChipMateAppStateDto(
            ai.chipmate.rpc.dto.ChipMateAppStatusDto.READY,
            config = ai.chipmate.rpc.dto.ConfigDto(model = "chipmate/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.chipmate.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
