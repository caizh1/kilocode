/**
 * ChipMateClaw VS Code extension message types.
 *
 * Defines the postMessage protocol between the extension host (Node.js)
 * and the ChipMateClaw webview (SolidJS). The extension host owns all network
 * connections (ChipMate Chat HTTP + event-service WebSocket) and relays data
 * to the webview.
 *
 * SYNC: Shared types are mirrored in webview-ui/chipmateclaw/lib/types.ts —
 * keep both in sync.
 */

// ── Instance status (ChipMateClaw worker) ───────────────────────────────
...
export type ClawStatus = {
  ...
  // restore (cloud: `services/chipmateclaw/src/index.ts`).
  status:
    | "provisioned"
    | "starting"
    | "restarting"
...
// ── ChipMate Chat token envelope (gateway response) ─────────────────────
...
// ── ChipMate Chat content blocks ────────────────────────────────────────
// Mirrors `@chipmate/chipmate-chat` schemas. See cloud/packages/chipmate-chat/src/schemas.ts.
...
// ── Typed ChipMate Chat events (server → client) ───────────────────────
// Event names mirror `@chipmate/chipmate-chat/events`.
...
export type ChipMateChatEventMap = {
...
export type ChipMateChatEventName = keyof ChipMateChatEventMap
...
export type ChipMateClawInMessage =
  | { type: "chipmateclaw.ready" }
  | { type: "chipmateclaw.openExternal"; url: string }
  | { type: "chipmateclaw.selectConversation"; conversationId: string }
  | { type: "chipmateclaw.createConversation"; title?: string }
  | { type: "chipmateclaw.renameConversation"; conversationId: string; title: string }
  | { type: "chipmateclaw.leaveConversation"; conversationId: string }
  | { type: "chipmateclaw.loadMoreConversations" }
  | {
      type: "chipmateclaw.sendMessage"
      conversationId: string
      content: ContentBlock[]
      inReplyToMessageId?: string
    }
  | { type: "chipmateclaw.editMessage"; conversationId: string; messageId: string; content: ContentBlock[] }
  | { type: "chipmateclaw.deleteMessage"; conversationId: string; messageId: string }
  | { type: "chipmateclaw.loadMoreMessages"; conversationId: string; before: string }
  | { type: "chipmateclaw.addReaction"; conversationId: string; messageId: string; emoji: string }
  | { type: "chipmateclaw.removeReaction"; conversationId: string; messageId: string; emoji: string }
  | {
      type: "chipmateclaw.executeAction"
      conversationId: string
      messageId: string
      groupId: string
      value: ExecApprovalDecision
    }
  | { type: "chipmateclaw.sendTyping"; conversationId: string }
  | { type: "chipmateclaw.sendTypingStop"; conversationId: string }
  | { type: "chipmateclaw.markRead"; conversationId: string }

// ── Messages: Extension Host → Webview ──────────────────────────────

export type ChipMateClawOutMessage =
  | { type: "chipmateclaw.state"; state: ChipMateClawState }
  | { type: "chipmateclaw.status"; data: ClawStatus | null }
  | { type: "chipmateclaw.locale"; locale: string }
  | { type: "chipmateclaw.error"; error: string }
  | { type: "chipmateclaw.conversations"; conversations: ConversationListItem[]; hasMore: boolean; replace: boolean }
  | { type: "chipmateclaw.activeConversation"; conversationId: string | null }
  | { type: "chipmateclaw.messages"; conversationId: string; messages: Message[]; hasMore: boolean; replace: boolean }
  | { type: "chipmateclaw.messageOptimistic"; conversationId: string; message: Message }
  | { type: "chipmateclaw.messageReplaced"; conversationId: string; pendingId: string; message: Message }
  | { type: "chipmateclaw.messageRemoved"; conversationId: string; messageId: string }
  | { type: "chipmateclaw.botStatus"; status: BotStatusRecord | null }
  | { type: "chipmateclaw.conversationStatus"; status: ConversationStatusRecord | null }
  | { type: "chipmateclaw.typing"; conversationId: string; memberId: string }
  | { type: "chipmateclaw.typingStop"; conversationId: string; memberId: string }
