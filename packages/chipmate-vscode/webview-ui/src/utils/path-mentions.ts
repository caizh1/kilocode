...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[ChipMate New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const CHIPMATE_FILE_PATH_MIME = "application/x-chipmate-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-chipmate-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const chipmate = dt.getData(CHIPMATE_FILE_PATH_MIME)
  if (chipmate) {
    const paths = chipmate.split(/\r?\n/).filter((line) => line.trim() !== "")
