    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<ChipMateSession.CloseReason>()
          const unsub = Bus.subscribe(ChipMateSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(ChipMateSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("ChipMateSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = ChipMateSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: ChipMateSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = ChipMateSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: ChipMateSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
