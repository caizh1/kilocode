...
ChipMate uses a fully automated CI pipeline triggered via GitHub Actions `workflow_dispatch`. A single workflow handles version bumping, building all artifacts, publishing to every distribution channel, and updating package registries.

## How to Trigger a Release

1. Go to the [`publish` workflow](https://github.com/ChipMate-Org/chipmate/actions/workflows/publish.yml) in GitHub Actions.
...
- Runs `packages/opencode/script/build.ts` to compile the ChipMate CLI binary.
...
- Uploads the `dist/` directory as a workflow artifact (`chipmate-cli`) for subsequent jobs.

### 3. Build VS Code Extension (`build-vscode`)

- Downloads the CLI artifacts from the previous job.
- Runs `packages/chipmate-vscode/script/build.ts` to build VSIX packages for all target platforms:
...
- Uploads the VSIX files as a workflow artifact (`chipmate-vscode`).
...
- Publishes platform-specific binary packages to **npm** (e.g. `@chipmate/cli-linux-x64`, `@chipmate/cli-darwin-arm64`, etc.).
- Publishes the main `@chipmate/cli` package to **npm** with optional dependencies on the binary packages.
- Builds and pushes a multi-arch **Docker image** (`ghcr.io/chipmate-org/chipmate`) to GitHub Container Registry (linux/amd64 + linux/arm64).

#### SDK (`@chipmate/sdk`)

- Builds and publishes the TypeScript SDK to **npm**.

#### Plugin (`@chipmate/plugin`)
...
- **AUR (Arch Linux)**: Clones `chipmate-bin` from the AUR, updates the `PKGBUILD` with new version and SHA256 checksums, and pushes.
- **Homebrew**: Clones `ChipMate-Org/homebrew-tap`, updates the `chipmate.rb` formula with new version, download URLs, and SHA256 checksums, and pushes.

## Prerequisites and Permissions

### Repository Access

- The workflow only runs in the `ChipMate-Org/chipmate` repository (guarded by `if: github.repository == 'ChipMate-Org/chipmate'`).
...
| Secret | Purpose |
|---|---|
| `CHIPMATE_API_KEY` | ChipMate API key used during version computation |
| `CHIPMATE_ORG_ID` | ChipMate organization ID |
| `CHIPMATE_MAINTAINER_APP_ID` | GitHub App ID for the chipmate-maintainer bot (used for git commits) |
| `CHIPMATE_MAINTAINER_APP_SECRET` | GitHub App secret for the chipmate-maintainer bot |
