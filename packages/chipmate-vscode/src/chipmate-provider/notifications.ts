...
export async function fetchAndSendNotifications(ctx: NotificationsContext): Promise<void> {
  ...
  try {
    const { data: all } = await retry(() => ctx.client!.chipmate.notifications(undefined, { throwOnError: true }))
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to fetch notifications:", error)
  }
}
