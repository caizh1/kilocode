...
import {
  ...
  type ProviderInfo,
} from "../../src/chipmate-provider-utils"
...
describe("sessionToWebview", () => {
  ...
  it("reports the explicit read-only code scope", () => {
    const result = sessionToWebview(makeSession({ metadata: { "chipmate.documentAgent.scope": "documents_and_code" } }))
...
describe("getErrorMessage", () => {
  ...
  it("reads the first Zod issue message from ConfigInvalidError", () => {
    const err = {
      ...
      data: {
        path: "/Users/me/.config/chipmate/chipmate.json",
        issues: [
...
describe("getConfigErrorDetails", () => {
  ...
  it("formats a single-issue ConfigInvalidError", () => {
    const err = {
      data: {
        path: "/home/me/.config/chipmate/chipmate.json",
        issues: [{ code: "unrecognized_keys", keys: ["indexing"], path: [], message: 'Unrecognized key: "indexing"' }],
      },
    }
    expect(getConfigErrorDetails(err)).toBe('File: /home/me/.config/chipmate/chipmate.json\n\n✖ Unrecognized key: "indexing"')
  })
