...
class SessionUiUpdateTest : BasePlatformTestCase() {
    ...
    fun `test read tool renders as ReadToolView`() {
        ...
        assertTrue(tv is ai.chipmate.client.session.views.tool.ReadToolView)
    }
    ...
    fun `test glob tool renders as GlobToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1")
        assertTrue(tv is ai.chipmate.client.session.views.tool.GlobToolView)
    }
    ...
    fun `test grep tool renders as SearchToolView`() {
        ...
        val tv = panel.findMessage("a1")!!.part("t1")
        assertTrue(tv is ai.chipmate.client.session.views.tool.SearchToolView)
    }
    ...
    fun `test compaction part appears in panel`() {
        ...
        assertTrue(mv.part("cp1") is ai.chipmate.client.session.views.CompactionView)
    }
    ...
    fun `test unknown part type falls back to GenericView`() {
        ...
        val gv = mv.part("g1")
        assertNotNull(gv)
        assertTrue(gv is ai.chipmate.client.session.views.base.GenericView)
        assertTrue((gv as ai.chipmate.client.session.views.base.GenericView).labelText().contains("snapshot"))
    ...
    fun `test assistant file part renders as attachment view`() {
        ...
        assertFalse(buttons(view).any { it.accessibleContext.accessibleName == ChipMateBundle.message("prompt.attachment.remove", "a.png") })
    }
