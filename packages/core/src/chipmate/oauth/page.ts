export * as ChipMateOauthCallbackPage from "./page"

import { OauthCallbackPage, type CallbackPageOptions } from "../../oauth/page"

const CHIPMATE_MARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none" aria-label="ChipMate" role="img">
...
function brand(page: string) {
  return page.replace(/<svg class="wordmark"[\s\S]*?<\/svg>/, CHIPMATE_MARK).replaceAll("OpenCode", "ChipMate")
}
