...
class ChipMateInlineDiffStoreTest : BasePlatformTestCase() {
    private lateinit var coroutines: TestCoroutines
    private lateinit var workspace: FakeWorkspaceRpcApi
    private lateinit var service: ChipMateDiffEditorService

    override fun setUp() {
        super.setUp()
        coroutines = TestCoroutines()
        workspace = FakeWorkspaceRpcApi()
        service = ChipMateDiffEditorService(project, coroutines.scope)
        project.replaceService(ChipMateInlineDiffStore::class.java, ChipMateInlineDiffStore(), testRootDisposable)
        ApplicationManager.getApplication()
            .replaceService(ChipMateWorkspaceService::class.java, ChipMateWorkspaceService(coroutines.scope, workspace), testRootDisposable)
    }
