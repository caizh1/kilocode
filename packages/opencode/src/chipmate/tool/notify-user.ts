...
const FAILURE_TEXT =
  "Push notifications are unavailable: this session is not connected to ChipMate cloud. " +
  "Sign in with `chipmate auth login` and ensure the session is sharing before retrying."

const SUCCESS_TEXT =
  "Notification sent to the user's ChipMate app. Delivery may be suppressed by the user's " +
...
export const NotifyUserTool = Tool.define<typeof Params, Meta, ChipMateSessions.Service, "notify_user">(
  "notify_user",
  Effect.gen(function* () {
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          ...
          if (!ChipMateSessions.remoteStatus().enabled) {
            return {
              title: title(false),
              output: FAILURE_TEXT,
