...
import { target } from "../../src/chipmate/skill-remove"
...
describe("skill removal target", () => {
  ...
  test("rejects locations that are not in the active skill registry", () => {
    const location = path.join(path.parse(process.cwd()).root, "__chipmate_synthetic__", "SKILL.md")
  ...
  test("rejects registered locations that are not manifests", () => {
    const location = path.join(path.parse(process.cwd()).root, "__chipmate_synthetic__", "skill")
  ...
  test("returns only the registered skill manifest", () => {
    const location = path.join(path.parse(process.cwd()).root, "__chipmate_synthetic__", "skill", "SKILL.md")
