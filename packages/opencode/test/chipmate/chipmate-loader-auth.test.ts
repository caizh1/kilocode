...
// Tests that unauthenticated ChipMate models are assembled with paid models and autoloaded anonymously.
...
import { chipmateCustomLoaders, patchChipMateProviderPrivacy } from "../../src/chipmate/provider/provider"
...
it.live("assembles paid ChipMate models without auth", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(
      Effect.provide(layer()),
      provideInstance(process.cwd()),
    )
    const chipmate = Provider.fromModelsDevProvider(providers.chipmate)

    expect(chipmate.models["paid-model"]).toMatchObject({
      id: "paid-model",
      providerID: "chipmate",
      cost: { input: 1, output: 2 },
      isFree: false,
...
it.live("does not infer free status from zero catalog prices", () =>
  Effect.gen(function* () {
    ...
    expect(chipmate.models["free-model"].isFree).toBeUndefined()
  }),
)
...
it.effect("enables a paid catalog when config apiKey is present", () =>
  Effect.gen(function* () {
    const result = yield* load({ config: { provider: { chipmate: { options: { apiKey: "test-key" } } } } })
    expect(result.autoload).toBe(true)
...
it.effect("keeps data collection denied after configured options are applied", () =>
  Effect.sync(() => {
    const provider = { options: { dataCollection: "allow", baseURL: "https://api.chipmate.ai" } }
    patchChipMateProviderPrivacy(provider, { hide_prompt_training_models: true })
    expect(provider.options).toEqual({ dataCollection: "deny", baseURL: "https://api.chipmate.ai" })
  }),
)
