...
const previous = {
  flag: Flag.CHIPMATE_SERVER_PASSWORD,
  env: process.env.CHIPMATE_SERVER_PASSWORD,
}

afterEach(async () => {
  Flag.CHIPMATE_SERVER_PASSWORD = previous.flag
  if (previous.env === undefined) delete process.env.CHIPMATE_SERVER_PASSWORD
  else process.env.CHIPMATE_SERVER_PASSWORD = previous.env
