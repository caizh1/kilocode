---
title: "Benchmarking"
description: "Current evaluation evidence and roadmap for benchmarking ChipMate"
---
...
1. How do models compare when used by same ChipMate agent?
2. How do agents or ChipMate versions compare when used with same model?
...
|---|---|---|
| Harbor-facing smoke eval | Current workflow | `.github/workflows/smoke-test.yml` checks out private `ChipMate-Org/chipmate-bench`, installs dependencies, and runs two smoke tasks through repository scripts |
...
| Private `chipmate-bench` internals | Not verified here | Private repository scripts, adapter behavior, and supported local commands are outside inspected docs scope |
...
|---|---|---|
| Contributor-facing Harbor adapter | Unverified roadmap | Run ChipMate CLI autonomously in controlled evaluation environments |
...
| Standard agent comparison workflow | Planned | Compare agents or ChipMate versions on same tasks |
...
|---|---|---|
| [Harbor](https://harborframework.com) | Evaluation harness and datasets | Confirm supported ChipMate adapter and invocation contract |
...
| [Opik](https://www.comet.com/docs/opik) | Trace ingestion and analysis | Confirm Harbor integration setup and ChipMate adapter support |
...
|---|---|---|---|
| Model comparison | ChipMate agent and task set | Model | Completion, cost, and wall-clock time |
| Agent comparison | Model and task set | Agent or ChipMate version | Completion, cost, and wall-clock time |
| Trace analysis | Evaluation task | Run trajectory | Tool choices, errors, and repeated steps |

## Command verification requirement

Do not document `opik harbor run -a chipmate`, `chipmate --auto`, or `chipmate run --auto` as ready-to-run interfaces until adapter and autonomous CLI invocation are verified in relevant repository. Private `chipmate-bench` workflow commands are implementation evidence, not public usage guarantees.
