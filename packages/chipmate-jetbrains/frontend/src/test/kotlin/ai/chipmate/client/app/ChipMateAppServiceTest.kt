...
/**
 * Service-level tests for [ChipMateAppService] profile/login/logout/org operations.
 *
 * Uses [FakeAppRpcApi] to avoid RPC/backend involvement.
 */
@Suppress("UnstableApiUsage")
class ChipMateAppServiceTest : BasePlatformTestCase() {
    ...
    fun `test refreshProfile returns null and leaves existing state on exception`() = runBlocking(Dispatchers.Default) {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = existing)
    ...
    fun `test logout clears profile when rpc returns true`() = runBlocking(Dispatchers.Default) {
        val prof = profile()
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = prof)
    ...
    fun `test setOrganization updates profile for personal null selection`() = runBlocking(Dispatchers.Default) {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = org)
