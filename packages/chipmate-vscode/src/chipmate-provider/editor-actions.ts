...
function previewImage(dir: vscode.Uri | undefined, dataUrl: string, filename: string): void {
        ...
        return Promise.all(
          stale.map((name) =>
            Promise.resolve(vscode.workspace.fs.delete(vscode.Uri.joinPath(root, name), { recursive: true })).then(
              undefined,
              (err: unknown) => {
                console.warn("[ChipMate New] ChipMateProvider: Failed to delete stale preview:", err)
              },
            ),
          ),
        )
  ...
  void vscode.workspace.fs
    ...
    .then(() => clean())
    .then(open, (err) => console.error("[ChipMate New] ChipMateProvider: Failed to preview image:", err))
}
  ...
  if (message.type === "validateFiles") {
    ...
    if (id && paths && opts.post) {
      ...
      validateFiles(opts.dir(), paths).then(
        ...
        (err) => console.error("[ChipMate New] ChipMateProvider: validateFiles failed:", err),
      )
    }
    return true
  }
...
function openContent(content: string, language?: string): void {
  vscode.workspace.openTextDocument({ content, language: language || "log" }).then(
    ...
    (err) => console.error("[ChipMate New] ChipMateProvider: Failed to open content:", err),
  )
}
    ...
    (matches) => {
      ...
      if (matches.length > 1) {
        ...
        vscode.window.showQuickPick(items, { placeHolder: `Multiple matches for "${name}"` }).then(
          ...
          (err) => console.error("[ChipMate New] ChipMateProvider: showQuickPick failed:", err),
        )
        return
      }
      vscode.window.showWarningMessage(`File not found: ${filePath}`)
    },
    (err: unknown) => console.error("[ChipMate New] ChipMateProvider: findFiles failed:", err),
  )
}
