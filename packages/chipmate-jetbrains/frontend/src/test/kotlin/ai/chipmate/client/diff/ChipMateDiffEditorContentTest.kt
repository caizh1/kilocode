...
class ChipMateDiffEditorContentTest : BasePlatformTestCase() {
    ...
    fun `test tree toolbar installs actions in order`() {
        ...
        try {
            ...
            assertEquals(ChipMateBundle.message("diff.editor.refresh"), actions[0].templatePresentation.text)
            assertTrue(actions[1] is Separator)
            assertEquals(ChipMateBundle.message("diff.editor.tree.expandAll"), actions[2].templatePresentation.text)
            assertEquals(ChipMateBundle.message("diff.editor.tree.collapseAll"), actions[3].templatePresentation.text)
    ...
    fun `test diff request shows placeholder for blank added patch`() {
        ...
        assertEquals(ChipMateBundle.message("diff.editor.patch.unavailable"), contents[1])
    }
    ...
    fun `test inline params require directory and token`() {
        assertTrue(ChipMateDiffEditorKind.isValid(diffParams("inline", "/repo", null, "Session Changes", token = "turn:ses_1:u1")))
        assertFalse(ChipMateDiffEditorKind.isValid(mapOf("source" to "inline", "directory" to "/repo", "title" to "Session Changes")))
        assertFalse(ChipMateDiffEditorKind.isValid(mapOf("source" to "inline", "token" to "turn:ses_1:u1", "title" to "Session Changes")))
    }

    fun `test inline editor title uses params title`() {
        assertEquals("Session Changes", ChipMateDiffEditorKind.title(diffParams("inline", "/repo", null, "Session Changes", token = "token")))
    }
