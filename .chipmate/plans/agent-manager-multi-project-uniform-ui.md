...
- make the one embedded `ChipMateProvider` project/session-route aware.
...
- which project/worktree directory supplies runtime-effective ChipMate configuration in the one embedded `ChipMateProvider`.
...
The coordinator validates the project and target, ensures the context is ready, activates the project-specific ChipMate route, then emits one project-qualified activation result. The sidebar body stays mounted; only shared detail state changes.
...
- `ChipMateProvider.fetchAndSendConfig()` resolves a mutable current directory and emits one unqualified `configLoaded` value (`packages/chipmate-vscode/src/ChipMateProvider.ts:2513`).
- `ConfigProvider` owns one global/project/effective draft and sends an unqualified `updateConfig` (`packages/chipmate-vscode/webview-ui/src/context/config.tsx:54`, `:243`).
- `ChipMateProvider.handleUpdateConfig()` resolves `getWorkspaceDirectory()` again at save time (`packages/chipmate-vscode/src/ChipMateProvider.ts:2982`). A draft loaded for project A can therefore be written to project B after detail activation changes.
- The backend derives the project target from request directory and has no target or revision precondition (`packages/opencode/src/config/config.ts:944`, `packages/opencode/src/chipmate/config/config.ts:67`). Concurrent editors can overwrite one another, and a newly created higher-priority config file can redirect a pending write.
- Hidden scope splitting currently writes `commit_message` and `indexing.enabled` to the project while most fields go global (`packages/chipmate-vscode/webview-ui/src/utils/config-scope.ts:3`). The UI does not make this target choice explicit.
- Project config writes also exist outside the save bar. In particular, provider disconnect can call `saveProject()` using the mutable provider workspace directory (`packages/chipmate-vscode/src/provider-actions.ts:252`). Disabling only `handleUpdateConfig()` is insufficient.
...
Evolve the ChipMate-owned `/config/overlay` API rather than adding project identity to generic `Config.update`. The backend does not know Agent Manager `projectId`; it enforces the routed directory and filesystem target while the extension enforces project identity.
...
- The backend resolver may choose an existing root or `.chipmate`/`.chipmate` config according to current precedence, but the expected target must remain within the trusted project checkout after canonical/symlink resolution. A symlink escape is read-only and must not be patched through the API.
...
1. In `packages/opencode/src/chipmate/config/overlay.ts`, replace path-only targets with revisioned target descriptors and add generic changed-path source resolution.
2. Add a ChipMate-owned compare-and-swap writer under `packages/opencode/src/chipmate/config/` that resolves, confines, locks, fingerprints, patches, validates, and atomically replaces one target. Keep shared `packages/opencode/src/config/config.ts` changes to minimal marked delegation/invalidation hooks.
3. Update `packages/opencode/src/chipmate/server/httpapi/groups/config-console.ts` and `handlers/config-console.ts` with the write precondition, result, and typed error contracts. Regenerate `packages/sdk/js/`.
4. Extract a config binding controller under `packages/chipmate-vscode/src/chipmate-provider/` and have `ChipMateProvider.ts` delegate reads/writes to it. The controller receives explicit project-route resolution; it never reads mutable active workspace state during a bound operation.
...
- `packages/opencode/test/chipmate/server/config-overlay.test.ts` for raw target snapshots, target changes, 409 revisions, source shadowing, locks, symlink confinement, and response outcomes;
- `packages/opencode/test/chipmate/project-config-update.test.ts` for target selection and atomic writes from repository roots/nested directories;
- a new ChipMate-owned backend unit test beside those files only if the compare-and-swap writer needs direct fault-injection coverage;
- `packages/chipmate-vscode/tests/unit/config-utils.test.ts`, replacing singleton `ConfigState` assumptions with binding/request-aware draft tests;
- `packages/chipmate-vscode/tests/unit/config-scope.test.ts`, deleted when `splitConfigByScope` is removed and replaced by tests proving one explicit scope per save;
- Agent Manager route/context tests for immutable project-root resolution, trust revocation, removal, and generation changes;
- ChipMateProvider protocol tests for A-to-B activation during reads and writes, out-of-order responses, cache partitioning, and auditing non-save-bar mutators.

## One `.chipmate/agent-manager.json` per project

Yes, every project has its own repository-local state file:

```text
project-a/.chipmate/agent-manager.json
project-b/.chipmate/agent-manager.json
project-c/.chipmate/agent-manager.json
```
...
- `WorktreeStateManager(root)` writing only `<root>/.chipmate/agent-manager.json`;
...
4. load only that repository's `.chipmate/agent-manager.json`;
...
- never deletes `.chipmate/agent-manager.json`, worktrees, branches, or sessions;
...
## One ChipMateProvider with a project/session route service

Keep one embedded `ChipMateProvider`. Multiple providers would duplicate event handling, terminal registrations, caches, and current-session state for one webview.
...
Project activation in `ChipMateProvider` must invalidate and refresh project-scoped config, providers, agents, commands, skills, MCP state, indexing, sandbox state, and Git status under an activation generation guard.
...
4. From `packages/chipmate-vscode/`, run typecheck, lint, focused/unit tests, knip, compile/package, and architecture guards.
...
- Each project reads and writes only its own `.chipmate/agent-manager.json`.
