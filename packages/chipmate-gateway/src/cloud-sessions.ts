...
const INGEST_BASE = process.env.CHIPMATE_SESSION_INGEST_URL ?? "https://ingest.chipmatesessions.ai"
...
export async function fetchCloudSessionForImport(token: string, sessionId: string): Promise<FetchResult> {
  ...
  if (!response.ok) {
    ...
    console.error("[ChipMate Gateway] cloud/session/import: export failed", {
