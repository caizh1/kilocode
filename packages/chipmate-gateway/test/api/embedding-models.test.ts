...
import { EMPTY_CHIPMATE_EMBEDDING_MODEL_CATALOG, fetchChipMateEmbeddingModelCatalog } from "../../src/api/embedding-models"
...
describe("fetchChipMateEmbeddingModelCatalog", () => {
  test("fetches catalog from ChipMate Gateway", async () => {
    const prev = global.fetch
    const fn = mock(() => Promise.resolve(response())) as unknown as typeof fetch
    global.fetch = fn

    try {
      const catalog = await fetchChipMateEmbeddingModelCatalog({ baseURL: "https://example.test" })
  ...
  test("bounds caller-controlled retry attempts", async () => {
    ...
    try {
      await fetchChipMateEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: Number.POSITIVE_INFINITY })
  ...
  test("reports a final failure without writing to the console", async () => {
    ...
    try {
      await expect(
        fetchChipMateEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: 1, onError: issue }),
      ).resolves.toEqual(EMPTY_CHIPMATE_EMBEDDING_MODEL_CATALOG)
      expect(issue).toHaveBeenCalledWith({
        code: "http",
        message: "Unable to load ChipMate embedding models (HTTP 500).",
        status: 500,
      })
  ...
  test("fallback catalog is empty so Cloud owns model metadata", () => {
    expect(EMPTY_CHIPMATE_EMBEDDING_MODEL_CATALOG).toEqual({
      defaultModel: "",
      models: [],
      aliases: {},
    })
