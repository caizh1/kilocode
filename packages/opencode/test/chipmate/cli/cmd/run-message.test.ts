...
import { ChipMateRun } from "../../../../src/chipmate/cli/cmd/run"
import { buildRunMessage } from "../../../../src/chipmate/cli/cmd/run-message"

describe("ChipMateRun", () => {
  test("prefers a configured command over an endpoint-backed built-in", async () => {
    ...
    expect(await ChipMateRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBeUndefined()
  })
  ...
  test("resolves an endpoint-backed built-in when no configured command matches", async () => {
    ...
    expect(await ChipMateRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBe("compact")
  })
...
describe("buildRunMessage", () => {
  ...
  test("passes args['--'] through verbatim without wrap-quote (#9622)", () => {
    // `chipmate run -- "- Who are you?"` - yargs+populate-- captures the leading-dash
