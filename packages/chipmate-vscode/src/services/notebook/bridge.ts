...
export class NotebookBridge {
  ...
  private revision = 0
  private backend: ChipMateClient | undefined
  ...
  constructor(
    ...
    this.unsubscribeState = connection.onStateChange((state) => {
      ...
      void this.recover(revision).catch((error: unknown) => {
        console.error("[ChipMate New] NotebookBridge: pending request recovery failed:", error)
      })
    })
  }
  ...
  dispose(): void {
    ...
    for (const context of this.contexts.values()) {
      void context
        ...
        .catch((error: unknown) => console.error("[ChipMate New] NotebookBridge: context disposal failed:", error))
    }
    this.contexts.clear()
    this.origins.clear()
    this.outcomes.clear()
  ...
  private event(event: SSEPayload, directory?: string): void {
    if (event.type === "chipmate.notebook.requested") {
      this.request(event as EventChipmateNotebookRequested, directory)
      return
    }
    if (event.type === "chipmate.notebook.cancelled") {
      this.cancel(event as EventChipmateNotebookCancelled, directory)
    }
  }

  private request(event: EventChipmateNotebookRequested, directory?: string): void {
  ...
  private start(request: NotebookRequest, origin: RequestOrigin): void {
    ...
    void this.run(request, origin, active).catch((error: unknown) => {
      console.error(`[ChipMate New] NotebookBridge: request ${request.id} failed:`, error)
    })
  }

  private cancel(event: EventChipmateNotebookCancelled, directory?: string): void {
  ...
  private async reply(requestID: string, directory: string, result: NotebookResult): Promise<boolean> {
    try {
      const response = await this.connection.getClient().chipmate.notebook.reply({ requestID, directory, result })
      if (!response.error) return true
      console.error(`[ChipMate New] NotebookBridge: reply ${requestID} failed:`, response.error)
      return false
    } catch (error) {
      console.error(`[ChipMate New] NotebookBridge: reply ${requestID} failed:`, error)
      return false
    }
  }
  ...
  private async reject(requestID: string, directory: string, error: NotebookFailure): Promise<boolean> {
    try {
      const response = await this.connection.getClient().chipmate.notebook.reject({ requestID, directory, error })
      if (!response.error) return true
      console.error(`[ChipMate New] NotebookBridge: rejection ${requestID} failed:`, response.error)
      return false
    } catch (cause) {
      console.error(`[ChipMate New] NotebookBridge: rejection ${requestID} failed:`, cause)
      return false
    }
  }
  ...
  private async recover(revision: number): Promise<void> {
    ...
    for (const directory of this.connection.getKnownDirectories()) {
      try {
        const response = await client.chipmate.notebook.list({ directory })
        if (this.disposed || revision !== this.revision) return
        if (response.error) {
          console.error(`[ChipMate New] NotebookBridge: could not list requests for ${directory}:`, response.error)
          continue
        }
        for (const request of response.data ?? []) {
          this.request({ id: request.id, type: "chipmate.notebook.requested", properties: request }, directory)
        }
      } catch (error) {
        console.error(`[ChipMate New] NotebookBridge: could not list requests for ${directory}:`, error)
      }
    }
  }
...
export function createNotebookBridge(connection: ChipMateConnectionService): NotebookBridge {
