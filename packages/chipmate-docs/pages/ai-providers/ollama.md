---
title: "Using Ollama with ChipMate | Run Local Models"
description: "Run local AI models with Ollama in ChipMate for offline, private coding. Setup guide for VS Code and the CLI."
sidebar_label: Ollama
---

# Using Ollama With ChipMate

ChipMate supports running models locally using Ollama. This provides privacy, offline access, and potentially lower costs, but requires more setup and a powerful computer.
...
**Ollama Cloud** appears in ChipMate Gateway and BYOK flows. It is a hosted provider path and does not connect to `http://localhost:11434`.
{% /callout %}
...
For the ChipMate agent the current recommendation is `qwen3-coder:30b`. `qwen3-coder:30b` sometimes fails to call tools correctly (it is much more likely to have this problem than the full `qwen3-coder:480b` model). As a mixture-of-experts model, this could be because it activated the wrong experts. Whenever this happens, try changing your prompt or use the Enhance Prompt button.

An alternative to `qwen3-coder:30b` is `devstral:24b`. For other features of ChipMate such as Enhance Prompt or Commit Message Generation smaller models may suffice.

## Setting up Ollama

To set up Ollama for use with ChipMate, follow the instructions below.
...
By default, API requests time out after 10 minutes. Local models can be slow, if you hit this timeout you can consider increasing it here: VS Code Extensions panel > ChipMate gear menu > Settings > API Request Timeout.
...
Use `ollama/<model_name>` for the selected model. If you see `ollama-cloud/...`, you are using the hosted ChipMate Gateway/BYOK provider instead of your local Ollama server.
...
If your Ollama model doesn't appear in the ChipMate model picker, register it as a custom model in your config file:
