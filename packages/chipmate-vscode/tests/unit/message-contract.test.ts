 ...
 * 2. Every WebviewMessage type handled in ChipMateProvider has a corresponding member in the WebviewMessage union
...
const CHIPMATE_PROVIDER_UTILS_FILE = path.join(ROOT, "src/chipmate-provider-utils.ts")
...
describe("ChipMateProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in ChipMateProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(CHIPMATE_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `ChipMateProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(CHIPMATE_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
