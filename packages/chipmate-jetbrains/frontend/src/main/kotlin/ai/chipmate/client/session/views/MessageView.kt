...
import ai.chipmate.client.session.model.Message
 ...
 * driven by [ai.chipmate.client.session.ui.SessionLayout] so that each
 ...
 * messages is owned by [ai.chipmate.client.session.ui.SessionLayout].
 */
class MessageView(
    val msg: Message,
...
) : ai.chipmate.client.session.ui.SessionLayoutPanel(
    ...
    private inner class PromptWrap(
        ...
        private fun queue(onDelete: () -> Unit) = Stack.horizontal(UiStyle.Gap.sm()).also { row ->
            row.isOpaque = false
            row.next(JBLabel(ChipMateBundle.message("session.queued")).apply {
                foreground = UIUtil.getContextHelpForeground()
            })
            row.next(toolbarButton(
                ToolbarButtonAction(
                    AllIcons.Actions.Close,
                    ChipMateBundle.message("session.queued.remove"),
                    onDelete,
                ),
            ))
        }
