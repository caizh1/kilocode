import {
  ...
  getDefaultHeaders,
  getChipMateUrlFromToken,
  resolveChipMateOpenRouterBaseUrl,
...
export namespace CloudCatalog {
  ...
  export class Service extends Context.Service<Service, Interface>()("@chipmate/CloudCatalog") {}
  ...
  export const layer = (options: Options = {}) => {
    ...
    const request = Effect.fn("CloudCatalog.request")(function* <A>(url: string, input: Input, schema: z.ZodType<A>) {
        ...
        try: () =>
          new Request(url, {
            headers: {
              ...
              ...buildChipMateHeaders(
                undefined,
                input.organizationID ? { chipmateOrganizationId: input.organizationID } : undefined,
              ),
              "X-CHIPMATE-FEATURE": "chipmate-cli",
          ...
          new CatalogError({
            kind: "schema",
            message: "ChipMate catalog URL is invalid",
          }),
      })
      const response = yield* Effect.tryPromise({
        try: () => fetcher(req),
          ...
          new CatalogError({
            kind: "network",
            message: "Unable to reach the ChipMate model catalog",
          }),
      })
      if (!response.ok) {
        ...
        return yield* Effect.fail(
          new CatalogError({
            ...
            message:
              kind === "auth"
                ? "ChipMate credentials or organization were rejected by the model catalog"
                : "The ChipMate model catalog is unavailable",
          }),
        )
      }
      ...
      const body = yield* Effect.tryPromise({
        try: () => readBoundedJson(response),
          ...
          new CatalogError({
            kind: "schema",
            message: "The ChipMate model catalog returned an invalid response",
          }),
      })
    ...
    const base = Effect.fn("CloudCatalog.base")(function* (input: Input) {
      const raw = env.CHIPMATE_API_URL?.trim()
      const fallback = raw || DEFAULT_CHIPMATE_API_URL
      const value = getChipMateUrlFromToken(fallback, Redacted.value(input.token))
      return yield* Effect.try({
        try: () => {
          const url = new URL(resolveChipMateOpenRouterBaseUrl({ baseURL: value }))
          ...
          new CatalogError({
            kind: "schema",
            message: "ChipMate catalog URL must be secure",
          }),
      })
    })
