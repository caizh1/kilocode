...
import { CodeIndexManager } from "@chipmate/chipmate-indexing/engine"
import { mkdir } from "node:fs/promises"
import path from "node:path"
import { createIndexingHost } from "../../src/chipmate/indexing-host"
import { IndexingWorker, indexingCommand } from "../../src/chipmate/indexing-worker-client"
import type { Request } from "../../src/chipmate/indexing-worker-protocol"
import { tmpdir } from "../fixture/fixture"

test("allows a packaged native indexing sidecar to override the compiled default", () => {
  const arm = String.raw`C:\extension\bin\chipmate-indexer-arm64.exe`

  expect(indexingCommand(arm, String.raw`C:\extension\bin\chipmate-arm64.exe`)).toEqual([arm])
  expect(indexingCommand("chipmate-indexer-arm64.exe", String.raw`C:\extension\bin\chipmate-arm64.exe`)).toEqual([arm])
})
...
test.serial("allows same-directory recreation while disposal is pending", async () => {
  await using tmp = await tmpdir()
  const before = process.env.CHIPMATE_INDEXING_LOCK_RETRY_MS
  ...
  try {
    ...
    restore("CHIPMATE_INDEXING_LOCK_RETRY_MS", before)
  }
}, 30_000)
...
test.serial("keeps the CLI process alive when the indexing process exceeds its RSS limit", async () => {
  ...
  const before = {
    soft: process.env.CHIPMATE_INDEXING_SOFT_RSS_BYTES,
    hard: process.env.CHIPMATE_INDEXING_HARD_RSS_BYTES,
    hardHold: process.env.CHIPMATE_INDEXING_HARD_HOLD_MS,
  }
  process.env.CHIPMATE_INDEXING_SOFT_RSS_BYTES = "1"
  process.env.CHIPMATE_INDEXING_HARD_RSS_BYTES = "1"
  process.env.CHIPMATE_INDEXING_HARD_HOLD_MS = "1"
  ...
  try {
    ...
    await engine.dispose()
    restore("CHIPMATE_INDEXING_SOFT_RSS_BYTES", before.soft)
    restore("CHIPMATE_INDEXING_HARD_RSS_BYTES", before.hard)
    restore("CHIPMATE_INDEXING_HARD_HOLD_MS", before.hardHold)
  }
})
...
test.serial("requests a checkpointed rollover after sustained critical RSS", async () => {
  await using tmp = await tmpdir()
  const keys = [
    "CHIPMATE_INDEXING_SOFT_RSS_BYTES",
    "CHIPMATE_INDEXING_CRITICAL_RSS_BYTES",
    "CHIPMATE_INDEXING_HARD_RSS_BYTES",
    "CHIPMATE_INDEXING_HARD_HOLD_MS",
    "CHIPMATE_INDEXING_RECOVERY_RSS_BYTES",
    "CHIPMATE_INDEXING_CRITICAL_HOLD_MS",
  ] as const
  const before = Object.fromEntries(keys.map((key) => [key, process.env[key]]))
  process.env.CHIPMATE_INDEXING_SOFT_RSS_BYTES = "1"
  process.env.CHIPMATE_INDEXING_CRITICAL_RSS_BYTES = "1"
  process.env.CHIPMATE_INDEXING_HARD_RSS_BYTES = String(1024 * 1024 * 1024)
  process.env.CHIPMATE_INDEXING_HARD_HOLD_MS = "1"
  process.env.CHIPMATE_INDEXING_RECOVERY_RSS_BYTES = "1"
  process.env.CHIPMATE_INDEXING_CRITICAL_HOLD_MS = "1"
  const failure = Promise.withResolvers<unknown>()
  const engine = IndexingWorker.create(tmp.path, tmp.path, {
    status() {},
    telemetry() {},
