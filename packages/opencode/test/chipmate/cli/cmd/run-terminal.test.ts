...
import { ChipMateRunTerminal } from "@/chipmate/cli/cmd/run-terminal"
...
test("routes direct interactive terminal requests through the session workspace", async () => {
  ...
  const terminal = ChipMateRunTerminal.create(sdk, () => "ses_terminal")
...
test("retries workspace lookup after a failed request", async () => {
  ...
  const sdk = createChipMateClient({ baseUrl: "http://test", fetch })
  const terminal = ChipMateRunTerminal.create(sdk, () => "ses_terminal")
