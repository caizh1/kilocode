...
class ViewSwitchingTest : SessionControllerTestBase() {
    ...
    private fun session(id: String) = ai.chipmate.rpc.dto.SessionDto(
        id = id,
        projectID = "prj",
        directory = "/test",
        title = "Title $id",
        version = "1",
        time = ai.chipmate.rpc.dto.SessionTimeDto(created = 1.0, updated = 2.0),
    )
    ...
    fun `test empty session overlay show includes logged in profile`() {
        ...
        appRpc.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = prof)
        val m = controller()
        val events = collect(m)
    ...
    fun `test selecting personal account emits switching overlay`() {
        ...
        appRpc.state.value = ChipMateAppStateDto(
            ...
            profile = ProfileDto(
                email = "user@example.com",
                currentOrgId = "org_1",
                organizations = listOf(ProfileOrganizationDto("org_1", "ChipMate", "OWNER")),
            ),
        )
        val m = controller()
        val events = collect(m)
