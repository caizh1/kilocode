...
{% browserFrame url="app.chipmate.ai/gastown/town/wasteland" caption="The Wanted Board — browse and filter open tasks through the Mayor" %}
...
<!-- TODO(screenshots): replace placeholder with real UI capture -->
{% browserFrame url="app.chipmate.ai/gastown/town/rig/bead" caption="A bead with a wasteland link — the upstream item badge connects local work to the Wanted Board" %}
...
| Evidence auto-submit fails | Check DoltHub OAuth in [ChipMate Integrations](https://app.chipmate.ai/integrations/dolthub), or check the API token in the Wasteland connection dialog if you used the advanced option. The Mayor will retry. See the [Wasteland overview](/docs/code-with-ai/gastown/wasteland). |
