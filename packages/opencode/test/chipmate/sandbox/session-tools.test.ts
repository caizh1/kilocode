...
import * as ToolNetwork from "@/chipmate/sandbox/network"
import { EmbeddedReviewThinRuntime } from "@/chipmate/embedded-review/thin-runtime"
import { MCP } from "@/mcp"
...
const sessions = Layer.mock(Session.Service)({
  get: () => Effect.succeed(session("/workspace/project/.chipmate/worktrees/a")),
})
...
const mcp = Layer.mock(MCP.Service)({
  ...
  clients: () => Effect.succeed({}), // chipmate_change - upstream's MCP resource tools probe the clients
})
...
function fixture() {
  return Effect.gen(function* () {
    ...
    const a = path.join(main, ".chipmate", "worktrees", "a")
    const b = path.join(main, ".chipmate", "worktrees", "b")
...
mac("confines model-originated file mutations to the active worktree", () =>
  Effect.gen(function* () {
    ...
    const chipmate = yield* call(write, { filePath: state, content: "state" }, "call-state").pipe(Effect.exit)
    ...
    expect(Exit.isSuccess(chipmate)).toBe(true)
