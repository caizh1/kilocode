...
import ai.chipmate.client.session.ui.attachment.AttachmentCard
import ai.chipmate.client.session.ui.attachment.AttachmentCardItem
...
import ai.chipmate.client.session.ui.prompt.PROMPT_ATTACHMENT_PASTE_HANDLER_KEY
import ai.chipmate.client.session.ui.prompt.PromptAttachmentPasteHandler
import ai.chipmate.client.session.ui.prompt.PromptAttachmentPasteProvider
...
import ai.chipmate.client.test.CopyProviderSink
...
class PromptPanelTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        workspaces = ChipMateWorkspaceService(scope, rpc)
    }
    ...
    fun `test empty enhance explanation resizes wrapped input`() {
        ...
        assertTrue(editor.preferredSize.height > min)
        assertEquals(ChipMateBundle.message("prompt.action.enhance.description"), editor.text)
    }
    ...
    fun `test prompt editor component undo redo shortcuts target prompt editor`() {
        ...
        invokeComponentAction("ChipMate Session Undo", editor)
        assertEquals("", editor.document.text)
        invokeComponentAction("ChipMate Session Redo", editor)
        assertEquals("hello", editor.document.text)
    }
    ...
    fun `test prompt local completion shortcut opens mention lookup`() {
        rpc.searchResult = ai.chipmate.rpc.dto.FileSearchResultDto(
            files = listOf(ai.chipmate.rpc.dto.WorkspaceFileDto("src/deploy.ts", "deploy.ts")),
        )
        val panel = PromptPanel(project = project, onSend = { _, _ -> }, onAbort = {}, onEnhance = { _, _ -> }, completion = completion())
        val field = panel.defaultFocusedComponent as EditorTextField
    ...
    fun `test attachment tooltip hides embedded binary content`() {
        ...
        assertTrue(tip.contains("Location: ${ChipMateBundle.message("prompt.attachment.embedded")}"))
    ...
    fun `test prompt paste provider invokes registered handler`() {
        ...
        val item = FileListTransferable(listOf(File.createTempFile("chipmate-paste", ".txt")))
    ...
    fun `test file list paste adds attachment`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("chipmate-paste", ".txt")
    ...
    fun `test frontend text file attachment stays file reference`() {
        ...
        val item = ai.chipmate.client.session.model.PromptAttachmentExtractor.files(listOf(file)).single()
    ...
    fun `test pasted frontend file sends reference payload`() {
        var sent: ai.chipmate.rpc.dto.PromptPartDto? = null
    ...
    fun `test file paste ignores companion image flavor`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("chipmate-paste", ".png")
    ...
    fun `test disabled media model allows file reference attachment`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("chipmate-paste", ".php")
    ...
    fun `test prompt button switches between send and stop state`() {
        ...
        assertEquals(KeymapUtil.createTooltipText("Send", "ChipMate.SendPrompt"), panel.buttonForTest().toolTipText)
    ...
    fun `test auto approve button toggles and updates tooltip`() {
        ...
        assertEquals(ChipMateBundle.message("prompt.action.autoApprove.enable"), button.accessibleContext.accessibleName)
        assertEquals(ChipMateBundle.message("prompt.action.autoApprove.disabled.tooltip"), button.toolTipText)
        val icon = button.icon
        ...
        assertNotSame(icon, button.icon)
        assertEquals(ChipMateBundle.message("prompt.action.autoApprove.disable"), button.accessibleContext.accessibleName)
        assertEquals(ChipMateBundle.message("prompt.action.autoApprove.enabled.tooltip"), button.toolTipText)
    ...
    private fun autoApproveButton(panel: PromptPanel): JButton {
        val enable = ChipMateBundle.message("prompt.action.autoApprove.enable")
        val disable = ChipMateBundle.message("prompt.action.autoApprove.disable")
    ...
    private fun attachmentRemoveButton(panel: PromptPanel, item: PromptAttachment): JButton {
        val name = ChipMateBundle.message("prompt.attachment.remove", item.name)
    ...
    private fun enhanceButton(panel: PromptPanel): JButton {
        val name = ChipMateBundle.message("prompt.action.enhance")
        return buttons(panel).first { it.accessibleContext.accessibleName == name }
    }
    ...
    private fun completion() = ChipMatePromptCompletionProvider(
    ...
    private fun invokeCompletionAction(editor: Editor) {
        val action = ActionUtil.getActions(editor.contentComponent).first { item ->
            item.templatePresentation.text == "ChipMate Prompt Completion"
        }
