...
import { preload } from "../../../../src/chipmate/cli/cmd/tui"
import { ChipMateTuiThreadDaemon } from "../../../../src/chipmate/cli/cmd/tui/thread"
...
describe("chipmate tui thread", () => {
  ...
  test(
    ...
    async () => {
      ...
      const proc = spawn(
        ...
        ],
        {
          ...
          env: sanitizedProcessEnv({
            ...
            CHIPMATE_DISABLE_TERMINAL_TITLE: "0",
            CHIPMATE_DEV_CWD: "",
            CHIPMATE_PURE: "1",
            CHIPMATE_NO_DAEMON: "1",
            TERM: "xterm-256color",
          }),
        },
      )
      const data = proc.onData((chunk) => {
        ...
        if (state.text.includes("ChipMate CLI")) ready.resolve()
      })
      ...
      try {
        await ready.promise
        expect(state.text).toContain("ChipMate CLI")
      } finally {
        clearTimeout(timer)
        data.dispose()
  ...
  test("uses chipmate-dev caller directory when running through package cwd", async () => {
    await using root = await tmpdir()
    const pkg = path.join(root.path, "packages", "opencode")
    await fs.mkdir(pkg, { recursive: true })

    const prev = process.env.CHIPMATE_DEV_CWD
    process.env.CHIPMATE_DEV_CWD = root.path
    try {
      ...
      if (prev === undefined) delete process.env.CHIPMATE_DEV_CWD
      else process.env.CHIPMATE_DEV_CWD = prev
    }
  })
  ...
  test("validates imported daemon session over HTTP after importing from cloud", async () => {
    ...
    using server = Bun.serve({
      ...
      fetch(request) {
        ...
        calls.push(route)
        if (route === "POST /chipmate/cloud/session/import") return Response.json({ id: local })
    ...
    const args = { port: 0, hostname: "127.0.0.1", mdns: false, "mdns-domain": "chipmate.local", cors: [] }
    const start: Parameters<typeof ChipMateTuiThreadDaemon.attach>[0]["start"] = async (input) => {
      opened.push(input.args.sessionID)
    }

    try {
      await ChipMateTuiThreadDaemon.attach({
      ...
      expect(calls).toEqual(["POST /chipmate/cloud/session/import", `GET /session/${local}`])
  ...
  test("imports cloud fork before validating daemon session", async () => {
    ...
    mock.module("@/chipmate/daemon/client", () => ({
      DaemonClient: {
    ...
    const mod = await import(`../../../../src/chipmate/cli/cmd/tui/thread?${key}`)

    const handled = await mod.ChipMateTuiThreadDaemon.attach({
