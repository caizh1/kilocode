...
class ChipMateSessionServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeSessionRpcApi
    private lateinit var service: ChipMateSessionService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeSessionRpcApi()
        service = ChipMateSessionService(project, scope, rpc)
    }
    ...
    fun `test events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        service = ChipMateSessionService(project, scope, rpc, log)
