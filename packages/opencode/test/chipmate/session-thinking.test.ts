...
import { ChipMateSessionThinking } from "../../src/chipmate/session/thinking"
...
describe("ChipMateSessionThinking", () => {
  test("moves a leading think block before the visible answer", () => {
    const out = ChipMateSessionThinking.events(
      [
  ...
  test("closes an unfinished think block at text end", () => {
    const out = ChipMateSessionThinking.events([start(), delta("<think>checking local context"), end()], model)
  ...
  test("leaves ordinary text and inline think examples unchanged", () => {
    ...
    expect(ChipMateSessionThinking.events(input, model)).toEqual(input)
  })
  ...
  test("does not run for other models, providers, or non-reasoning models", () => {
    ...
    for (const candidate of cases) {
      expect(ChipMateSessionThinking.events(input, candidate)).toBe(input)
    }
  })
