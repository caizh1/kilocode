...
import ai.chipmate.client.ui.RoundedContentPanel
...
internal class LoggedInProfileUi(
    ...
    companion object {
        private val LOG = ChipMateLog.create(LoggedInProfileUi::class.java)
    }
    ...
    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/chipmate-profile.svg", LoggedInProfileUi::class.java)).apply {
        name = "chipmate.profile.logo.loggedIn"
        accessibleContext.accessibleName = ChipMateBundle.message("settings.chipmate.displayName")
    }

    private val titleLabel = JBLabel(ChipMateBundle.message("profile.balance.title")).apply {
        foreground = UiStyle.Colors.weak()
    }
    ...
    private val refreshBtn = JButton(ChipMateBundle.message("profile.action.refresh"), AllIcons.Actions.Refresh)
        .also {
            it.isOpaque = false
    ...
    private val passTitleLabel = JBLabel(ChipMateBundle.message("profile.pass.title")).apply {
    ...
    private val passBonusLabel = JBLabel(ChipMateBundle.message("profile.pass.bonusLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewLabel = JBLabel(ChipMateBundle.message("profile.pass.renewsLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passMeter = PassMeter().apply {
        name = "chipmate.profile.passMeter"
    }
    ...
    private val passRenewRow = Stack.horizontal(UiStyle.Gap.md())
        ...
        .fill(UiStyle.Gap.md())
        .next(passRenewValue)
    private val passLink = ActionLink(ChipMateBundle.message("profile.pass.subscribe")) {
        pass()
    }.apply {
        name = "chipmate.profile.passSubscribe"
    }
    ...
    private val passPanel = Stack.vertical(UiStyle.Gap.md()).apply {
        name = "chipmate.profile.passPanel"
        next(passInfo)
    ...
    private val balanceCard = RoundedContentPanel(UiStyle.Gap.pad(), UiStyle.Gap.xl()).apply {
        name = "chipmate.profile.balanceCard"
        addToTop(titleLabel)
    ...
    val dashboardBtn = JButton(ChipMateBundle.message("profile.action.dashboard"))
        .also { it.addActionListener { dashboard() } }
    val topUpBtn = JButton(ChipMateBundle.message("profile.action.topUp"))
        .also { it.addActionListener { topUp() } }
    val logoutBtn = JButton(ChipMateBundle.message("profile.action.logout"))
    ...
    fun update(profile: ProfileDto) {
        ...
        val item = if (personal) profile.chipmatePass else null
    ...
    @RequiresEdt
    private fun syncPass(item: ProfileChipMatePassDto?, show: Boolean): Boolean {
        var changed = false
        ...
        if (item != null) {
            val usage = ChipMateBundle.message(
                "profile.pass.used",
    ...
    fun setRefreshing(refreshing: Boolean) {
        ...
        val text = if (refreshing) ChipMateBundle.message("profile.action.refreshing")
        else ChipMateBundle.message("profile.action.refresh")
    ...
    private fun applyOrganizations(profile: ProfileDto) {
        ...
        val keys: List<Pair<String?, String>> = (if (personal) listOf(null to ChipMateBundle.message("profile.personalAccount")) else emptyList()) +
