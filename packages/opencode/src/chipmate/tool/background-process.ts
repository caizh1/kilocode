...
import { enabled as sandboxed } from "@chipmate/sandbox"
...
export const Params = Schema.Struct({
  ...
  persistent: Schema.optional(Schema.Boolean).annotate({
    description: "Keep the process running and manageable after the session or ChipMate exits",
  }),
}).check(
  Schema.makeFilter(
    (params: {
  ...
  Effect.succeed({
    ...
    execute: (params, ctx) =>
      Effect.gen(function* () {
        ...
        const parent = params.inherit ? ChipMateSession.resolveParent(ctx.sessionID) : undefined
