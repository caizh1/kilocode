...
describe("loadIgnore", () => {
  ...
  test("loads root .chipmateignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".chipmateignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .chipmateignore is absent", async () => {
  ...
  test("loads nested .chipmateignore relative to its directory", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, "pkg", ".chipmateignore"), "secret.ts\n")
  ...
  test("allows descendants when a parent directory is re-included", async () => {
    await mkdir(path.join(root, "pkg"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".chipmateignore"), "!pkg/\n")
  ...
  test("keeps descendants ignored when only a child directory is re-included", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".chipmateignore"), "!pkg/sub/\n")
  ...
  test("applies .chipmateignore after .gitignore in the same directory", async () => {
    await writeFile(path.join(root, ".gitignore"), "*.ts\n")
    await writeFile(path.join(root, ".chipmateignore"), "!keep.ts\n")
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".chipmateignore")).toBe(true)
  })

  test("keeps ignore files ignored after negation rules", async () => {
    await writeFile(path.join(root, ".chipmateignore"), "!.gitignore\n")
  ...
  test("ignores ignore file names even when absent during loading", async () => {
    ...
    expect(ig.ignores(".gitignore")).toBe(true)
    expect(ig.ignores("pkg/.chipmateignore")).toBe(true)
  })
