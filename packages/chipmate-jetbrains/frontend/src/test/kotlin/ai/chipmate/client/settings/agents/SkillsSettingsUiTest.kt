...
class SkillsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test skill edit dialog shows content with fallback`() {
        edt {
            ...
            val readonly = SkillEditDialog(SkillDto("chipmate-config", "desc", "builtin", "<h1>ChipMate Config</h1>"), false)
            try {
                assertEquals("# Plan\nUse steps", content.content())
                assertEquals("desc", fallback.content())
                assertEquals("<h1>ChipMate Config</h1>", readonly.content())
    ...
    fun `test skill editor file type follows content syntax before location`() {
        assertEquals(
            ...
            skillFileType("builtin", "<h1>ChipMate CLI Configuration Reference</h1><p>All config lives in <code>chipmate.json</code>.</p>"),
        )
        assertEquals(
            skillFileType("SKILL.md"),
            skillFileType("builtin", "# ChipMate CLI Configuration Reference\n\nAll config lives in `chipmate.json`."),
        )
    ...
    private companion object {
        const val DIR = "/test"
        const val CUSTOM = "/home/test/.config/chipmate/skill/plan/SKILL.md"
        const val REMOTE = "/home/test/.cache/chipmate/skills/remote/SKILL.md"
    }
}
