...
import { AgentManagerEvent, type AgentManagerTask } from "@/chipmate/agent-manager/event"
import { AgentManager, HostError } from "@/chipmate/agent-manager/service"
import type { Result } from "@/chipmate/agent-manager/protocol"
...
const WireParams = Schema.Struct({
  ...
  action: Schema.optional(
    Schema.Literals(["list", "prompt", "stop", "move"]).annotate({
      description:
        "Use list first to discover IDs and assignments. Use move only after list, once per worktree. Never edit .chipmate/agent-manager.json for these operations.",
    }),
  ),
...
// Prefer the provider the user already uses for the invoking turn, then the ChipMate Gateway,
...
function rank(providerID: string, preferred: string | undefined): number {
  ...
  if (providerID === "chipmate") return 1
  return 2
}
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          ...
          const msg = ChipMateSessionMessageOrder.latest(ctx.messages).user
