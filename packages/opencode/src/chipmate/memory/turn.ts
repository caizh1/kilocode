...
import { MemoryTurn as TurnCore } from "@chipmate/chipmate-memory/effect/turn"
...
export namespace MemoryLifecycle {
  export const subscribe = Effect.fn("MemoryLifecycle.subscribe")(function* (input: {
    ...
    yield* input.bus.subscribeCallback(ChipMateSession.Event.TurnOpen, (evt) =>
      bridge.fork(
    ...
    yield* input.bus.subscribeCallback(ChipMateSession.Event.TurnClose, (evt) =>
      bridge.fork(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const enabled = yield* ChipMateSessionPrompt.memoryToolEnabled({ ctx })
          if (!enabled) return
