import type { ChipMateClient, SessionStatus } from "@chipmate/sdk/v2/client"
...
export class SessionAbort {
  ...
  async stop(client: ChipMateClient, sessionID: string, fallback: string) {
    ...
    if (failures.length > 0) {
      console.error("[ChipMate New] ChipMateProvider: Failed to abort session in one or more directories:", failures)
      return false
    }
...
export async function abortSession(input: { client: ChipMateClient; sessionID: string; dir: string }) {
