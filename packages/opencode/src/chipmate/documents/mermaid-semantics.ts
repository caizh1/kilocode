...
import type { QueryEvidenceResult } from "@chipmate/chipmate-indexing/engine"
...
async function load(issues: MermaidSemanticIssue[]): Promise<MermaidSemanticQuery | undefined> {
  try {
    const mod = await import("@/chipmate/indexing")
    return (query, options) => mod.ChipMateIndexing.queryEvidence(query, options)
  } catch (err) {
    push(issues, {
