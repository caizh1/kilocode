...
const skillRoot = path.join(root, ".chipmate/skills/source-backed-detail-design")
...
describe("source-backed detail design 分块控制器 Skill", () => {
  test("保留 Skill 身份并声明唯一控制器入口", async () => {
    ...
    expect(frontmatterValue(skill, "description")).toBe(
      "Generate or update source-backed detailed design documents with ChipMate native code/document evidence tools, Mermaid PNG artifacts, Word docx output, and render diagnostics.",
    )
