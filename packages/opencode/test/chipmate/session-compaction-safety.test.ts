// Unit tests for ChipMateSessionPrompt history-trim / media-strip helpers.
...
import { ChipMateSessionMessageOrder } from "../../src/chipmate/session/message-order"
...
describe("ChipMateSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(ChipMateSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(ChipMateSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(ChipMateSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = ChipMateSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    ChipMateSessionMessageOrder.annotate([active, queued, done])

    const state = ChipMateSessionMessageOrder.latest([active, done, queued])
...
describe("ChipMateSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = ChipMateSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(ChipMateSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = ChipMateSessionPrompt.trimBeforeLastSummary(msgs)
    const second = ChipMateSessionPrompt.trimBeforeLastSummary(first)
...
describe("ChipMateSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = ChipMateSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = ChipMateSessionPrompt.stripHistoricalMedia(input)
...
describe("ChipMateSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = ChipMateSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
