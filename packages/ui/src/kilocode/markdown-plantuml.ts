...
async function prepare(wrapper: HTMLElement, source: string, error: string, label: PlantUmlLabels) {
  ...
  const event = new CustomEvent("kilo:prepare-plantuml-repair", {
    bubbles: true,
    cancelable: true,
...
function wait(requestId: string, timeout: number) {
  ...
  const finish = (result: Result) => {
    ...
    clearTimeout(timer)
    window.removeEventListener("kilo:plantuml-rendered", receive)
    resolve(result)
  }
  window.addEventListener("kilo:plantuml-rendered", receive)
...
function request(source: string) {
  ...
  const event = new CustomEvent("kilo:render-plantuml", {
    bubbles: true,
    cancelable: true,
