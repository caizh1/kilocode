...
import ai.chipmate.client.settings.base.SettingsListRenderer
import ai.chipmate.client.settings.base.SettingsListActionCell
import ai.chipmate.client.settings.base.settingsListCellAt
import ai.chipmate.client.settings.base.settingsListCellBounds
import ai.chipmate.client.settings.base.settingsListSectionTitle
import ai.chipmate.client.settings.base.settingsListVisibleCells
import ai.chipmate.client.testing.FakeProviderRpcApi
...
class ProvidersSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test custom save error reports dropped provider`() {
        ...
        assertEquals(ChipMateBundle.message("settings.providers.customNotUsable"), customSaveError("my-openai", result))
    }
    ...
    fun `test custom dialog cancels model fetch and ignores late result`() {
        ...
        edt {
            assertEquals(ChipMateBundle.message("settings.providers.customFetchingModels"), field.text)
            assertEquals(ChipMateBundle.message("settings.providers.customCancelModels"), pick.text)
            ...
            pick.doClick()
            assertEquals("", field.text)
            assertEquals(ChipMateBundle.message("settings.providers.customSelectModels"), pick.text)
    ...
    fun `test custom dialog edit prefill locks id and leaves key blank`() {
        ...
        edt {
            ...
            assertEquals(ChipMateBundle.message("settings.providers.customSave"), dialog.okText())
            dispose(dialog)
        }
    }
    ...
    fun `test popular rows use vscode order including chipmate`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(
                    provider("openrouter", "OpenRouter", priority = 5),
                    provider("chipmate", "ChipMate", priority = 0),
        ...
        assertEquals(listOf("chipmate", "anthropic", "deepseek", "openai", "google", "openrouter", "vercel"), rows.map { it.key })
    ...
    fun `test connected chipmate gateway has no provider settings actions`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(provider("chipmate", "ChipMate Gateway")),
                connected = listOf("chipmate"),
            ),
            "",
        )

        assertEquals(listOf("chipmate"), rows.map { it.key })
        assertEquals("Connected providers", settingsListSectionTitle(rows, 0))
    ...
    fun `test provider oauth waiting countdown can be cancelled without refresh`() {
        val callback = CompletableDeferred<ai.chipmate.rpc.dto.ProviderActionResultDto>()
        val rpc = installProvider(
            ProviderSettingsDto(
                providers = listOf(provider("github-copilot", "GitHub Copilot")),
                auth = mapOf("github-copilot" to listOf(ProviderAuthMethodDto("oauth", "OAuth"))),
        ...
        callback.complete(ai.chipmate.rpc.dto.ProviderActionResultDto(providerState(provider("stale", "Stale"))))
    ...
    fun `test provider oauth auto response shows device auth panel`() {
        ...
        edt {
            ...
            assertEquals("https://auth.openai.com/device", fieldsByName(panel, "chipmate.provider.oauth.url").single().text)
            val qr = components(panel).filterIsInstance<JBLabel>().single { it.name == "chipmate.provider.oauth.qr" }
    ...
    fun `test provider action failure returns error state`() = runBlocking {
        ...
        rpc.disconnectError = IllegalStateException("ChipMate backend is not ready")
        val service = ChipMateProviderService(cs, rpc)

        val result = withContext(kotlinx.coroutines.Dispatchers.Default) {
            service.disconnect(ProviderDisconnectDto("/test", "openai"))
        }

        assertEquals("ChipMate backend is not ready", result.error)
    ...
    private fun installProvider(state: ProviderSettingsDto): FakeProviderRpcApi {
        ...
        ApplicationManager.getApplication().replaceService(
            ChipMateProviderService::class.java,
            ChipMateProviderService(cs, rpc),
            testRootDisposable,
        )
        return rpc
    }
