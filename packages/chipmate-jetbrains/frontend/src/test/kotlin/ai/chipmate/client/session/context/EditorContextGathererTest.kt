...
class EditorContextGathererTest : BasePlatformTestCase() {
    ...
    fun `test gather filters chipmateignore files from open tabs`() {
        ...
        myFixture.addFileToProject(".chipmateignore", "ignored/\n")
        val manager = FileEditorManager.getInstance(project)
    ...
    fun `test gather returns empty when setting is off`() {
        ChipMatePluginSettings.setAutoEditorContext(false)
