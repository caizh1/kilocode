...
class ChipMateWorkspaceService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: ChipMateWorkspaceRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = ChipMateLog.create(ChipMateWorkspaceService::class.java)
        private val INIT = ChipMateWorkspaceStateDto(ChipMateWorkspaceStatusDto.PENDING)
    }
    ...
    private suspend fun <T> call(block: suspend ChipMateWorkspaceRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(ChipMateWorkspaceRpcApi.getInstance()) }
    }

    private fun <T> stream(block: suspend ChipMateWorkspaceRpcApi.() -> Flow<T>): Flow<T> = flow {
        val api = rpc
        if (api != null) block(api).collect { emit(it) }
        else durable { block(ChipMateWorkspaceRpcApi.getInstance()).collect { emit(it) } }
    }
