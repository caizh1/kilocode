...
describe("RemoteWS", () => {
  ...
  test("ignores callbacks from a stale WebSocket generation", async () => {
    ...
    try {
      ...
      expect(conn.connected).toBe(true)
      // chipmate_change - K1 W1: with the immediate heartbeat on FIRST open,
  ...
  // chipmate_change - K1 W1: instance advertisement flows through the
  ...
  // chipmate-sessions.ts (see `setInstanceAdvertisement`) still fires one
  // immediate heartbeat when the flag is flipped, which is the practical
  // point at which a user runs `chipmate remote` and wants the cloud picker
  ...
  // is the practical "advertise on `chipmate remote` command" path — the
  ...
  test("setInstanceAdvertisement triggers an immediate heartbeat (out-of-band path)", async () => {
    await withFakeWebSocket(async (clock) => {
      ...
      // out-of-band path in chipmate-sessions.ts that calls
