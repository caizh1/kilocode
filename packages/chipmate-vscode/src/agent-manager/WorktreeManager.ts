 ...
 *
 * Ported from chipmate/src/core/chipmate/agent-manager/WorktreeManager.ts.
...
import { type GitOps, isChipMateOwnedSshCommand, nonInteractiveEnv } from "./GitOps"
...
const TEMP_PREFIX = ".chipmate-delete-"
...
import { CHIPMATE_DIR } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "chipmate-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, CHIPMATE_DIR, "worktrees")
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.chipmate-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .chipmate-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    return this.readMetadataFrom(worktreePath, CHIPMATE_DIR)
  }
  ...
  async resolveStartPoint(
    ...
    if (remote) {
      ...
      try {
        ...
        // is the fixed value ChipMate injects — never for an inherited one, which
        // could be attacker-controlled.
        const env = nonInteractiveEnv()
        await simpleGit(this.root, { unsafe: { allowUnsafeSshCommand: isChipMateOwnedSshCommand(env) } })
          .env(env)
