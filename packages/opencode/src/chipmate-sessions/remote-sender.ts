import { RemoteCommand } from "@/chipmate-sessions/remote-command"
import { RemoteExit } from "@/chipmate-sessions/remote-exit"
import { RemoteModelCatalog } from "@/chipmate-sessions/remote-model-catalog"
import { RemoteProtocol } from "@/chipmate-sessions/remote-protocol"
import { consumeRenameAdoption, markRenameAdopted } from "@/chipmate-sessions/rename-adoptions"
import type { RemoteWS } from "@/chipmate-sessions/remote-ws"
import { GlobalBus } from "@/bus/global"
import { RemoteAttachments } from "@/chipmate/remote-attachments"
...
type Provide = typeof import("@/chipmate/instance").provide
...
// chipmate_change start - create_session: strict v1 request with optional inheritance fields
...
// chipmate_change start - redact anything but the error class so messages/credentials
...
// chipmate_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | RemoteModelCatalog.ModelRef | undefined) {
  ...
  return {
    ...
    modelID: ModelV2.ID.make(model.startsWith("chipmate/") ? model.slice("chipmate/".length) : model),
  }
}
...
export namespace RemoteSender {
  export type Options = {
    ...
    session?: {
      ...
      // chipmate_change start - injectable create hook for create_session.
      ...
      // chipmate_change - injectable remove hook used to roll back an orphan
      ...
      // chipmate_change - injectable setTitle for system session.renamed handling
      readonly setTitle?: (input: { sessionID: SessionID; title: string }) => Promise<void>
      // chipmate_change end
    }
    // chipmate_change - K1 W1: in-process attach/detach/ownership/cancel
    // seams. All four are optional and default to a lazy import of
    // `ChipMateSessions` (production wires them in `enableRemote`, so the
  ...
  export function create(options: Options): Sender {
    ...
    // chipmate_change start - orphan rollback for create_session: when
    ...
    // chipmate_change end
    // chipmate_change - K1 W1: session create + in-process attach seams used by
    // create_session. Production wires `attachSession` to
    // `ChipMateSessions.attachRemoteSession` from inside `enableRemote` (see
    // chipmate-sessions.ts). Test fixtures inject stubs via the Options object.
    ...
    // chipmate_change end
    // chipmate_change start - injectable slash command discovery + execution
    ...
    async function replay(sessionId: string) {
      ...
      const queued = ChipMateSessionPromptQueue.snapshot(SessionID.make(sessionId))
      options.conn.send({
        type: "event",
        sessionId,
        ...(root ? { parentSessionId: root } : {}),
    ...
    function dispatch(msg: RemoteProtocol.Command) {
      // chipmate_change start - slash command discovery and execution
      ...
      if (msg.command === "exit_cli") {
        // chipmate_change - K1 W1: `exit_cli` now means "detach THIS remote
        // session and (if this is the last interactive session) close the
        // CLI." It is NOT "terminate the CLI." A headless `chipmate remote` host
        ...
        //      flush first. If zero remain + no callback (headless `chipmate
        ...
        void (async () => {
          try {
            ...
            //    (`chipmate remote`) never register a RemoteExit callback, so
      ...
      if (msg.command === "create_session") {
        // chipmate_change - K1 W1: in-process create_session. Optional wire
        // fields (agent/model/orgId) ride protocolVersion 1; orgId lands in
        // session metadata so the first chipmate_meta carries the claim. Handler
      ...
      // chipmate_change end
      // chipmate_change start - sessionless list_models for the pre-session instance picker
            ...
            try {
              ...
              await run({
                ...
                fn: async () => {
                  ...
                  // setTitle and the chipmate-sessions consumer is deferred, so a
