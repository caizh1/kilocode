...
import { assembleDesignDocument, buildQualityReport } from "@/chipmate/design-doc/assembler"
import { buildFullDesignWorkItems } from "@/chipmate/design-doc/content-contract"
import { inspectWordDocument } from "@/chipmate/documents/word"
import type { Artifact, DesignDocJob, EvidencePack, WorkItem } from "@/chipmate/design-doc/domain"
import { artifact, DesignDocStore } from "@/chipmate/design-doc/store"
...
describe("完整详细设计 Word 组装", () => {
  test("十四主题和 5D 矩阵生成同源 Markdown、原生目录 DOCX 与完整页面证据", async () => {
    ...
    const previous = process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]
    process.env["CHIPMATE_WORD_RENDER_ENDPOINT"] = `http://127.0.0.1:${server.port}`
    try {
      ...
      if (previous === undefined) delete process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]
      else process.env["CHIPMATE_WORD_RENDER_ENDPOINT"] = previous
