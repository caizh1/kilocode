...
import { DbPreflight } from "@opencode-ai/core/chipmate/db-preflight"
import { Database as ChipMateDatabase } from "@opencode-ai/core/database/database"
...
// reproducing the state a crashed chipmate process leaves behind
...
describe("DbPreflight", () => {
  ...
  test("accepts a writable database", async () => {
    ...
    const file = path.join(tmp.path, "chipmate.db")
    createWalDb(file)
    expect(() => DbPreflight.assertWritable(file)).not.toThrow()
  })

  test("names the offending file for a read-only sidecar outside the chipmate data dir", async () => {
    if (skip) return
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "chipmate.db")
  ...
  test("reports a missing directory as missing, not as read-only", async () => {
    ...
    const file = path.join(dir, "chipmate.db")
  ...
  test("pending WAL frames with a read-only sidecar fail with the actionable error, and repair recovers the data", async () => {
    ...
    expect(() => ChipMateDatabase.layerFromPath(file)).toThrow(`Database file is not writable: ${file}-wal`)
