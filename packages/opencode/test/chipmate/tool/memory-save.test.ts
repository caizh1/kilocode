...
import { MemorySaveTool } from "@/chipmate/tool/memory-save"
...
const watch = process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
...
beforeEach(() => {
  process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  spyOn(RemoteSender, "create").mockReturnValue({ handle() {}, dispose() {} })
})

afterEach(async () => {
  mock.restore()
  if (watch === undefined) delete process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
  if (watch !== undefined) process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER = watch
  await resetDatabase()
})
...
describe("chipmate_memory_save", () => {
  ...
  test("defaults mutating memory tool permission to ask", () => {
    const chipmate = ChipMateAgent.prepare({})

    expect(Permission.evaluate("chipmate_memory_recall", "typed", chipmate.defaultsPatch).action).toBe("ask")
    expect(Permission.evaluate("chipmate_memory_save", "remember", chipmate.defaultsPatch).action).toBe("ask")
  })

  test("remembers, corrects, and forgets explicit project memory", async () => {
    await using dir = await tmpdir({ git: true })
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      const saved = await execute(
        ...
        {
          action: "remember",
          key: "chipmate_cli_tui",
          text: "ChipMate CLI is a TUI.",
        },
        gate,
      )
      const corrected = await execute(
        dir.path,
        {
          action: "correct",
          key: "chipmate_cli_tui",
          text: "ChipMate CLI should be treated as a terminal UI.",
        },
        gate,
      )
      const shown = await ChipMateMemory.show({ ctx: memory })

      expect(saved.title).toBe("ChipMate memory saved: 1 op")
      expect(corrected.title).toBe("ChipMate memory correction saved: 1 op")
      expect(shown.sources.project).toContain("- chipmate_cli_tui :: ChipMate CLI is a TUI.")
      expect(shown.sources.corrections).toContain("- chipmate_cli_tui :: ChipMate CLI should be treated as a terminal UI.")

      const forgotten = await execute(dir.path, { action: "forget", query: "chipmate_cli_tui" }, gate)
      const next = await ChipMateMemory.show({ ctx: memory })

      expect(forgotten.title).toBe("ChipMate memory updated: 2 removed")
      expect(next.sources.project).not.toContain("chipmate_cli_tui")
      expect(next.sources.corrections).not.toContain("chipmate_cli_tui")
      expect(asks.map((req) => req.permission)).toEqual(["chipmate_memory_save", "chipmate_memory_save", "chipmate_memory_save"])
  ...
  test("does not save when project memory is disabled", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      expect(result.title).toBe("ChipMate memory: disabled")
  ...
  test("declines user-level memory but saves project conventions", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      expect(skipped.title).toBe("ChipMate memory skipped: out of scope")
      expect(skipped.output).toContain("user-level memory is not supported yet")
      expect(blocked.title).toBe("ChipMate memory unchanged")
  ...
  test("handles malformed save inputs without corrupting memory files", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      expect(missing.title).toBe("ChipMate memory remember: no text")
      expect(forget.title).toBe("ChipMate memory forget: no query")
      expect(secret.title).toBe("ChipMate memory: error")
      expect(secret.output).toContain("rejected secret-like content")
      expect(noisy.title).toBe("ChipMate memory saved: 1 op")
