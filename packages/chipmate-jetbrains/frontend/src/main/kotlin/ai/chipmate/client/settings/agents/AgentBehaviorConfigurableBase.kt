...
abstract class AgentBehaviorConfigurableBase<T : JComponent> : DraftReadyConfigurable<T>() {
    final override fun create(cs: CoroutineScope): T {
        ...
        cs.launch {
            val dir = service<ChipMateWorkspaceService>().resolveProjectDirectory(selected?.projectIdOrNull(), hint)
    ...
    private companion object {
        val LOG = ChipMateLog.create(AgentBehaviorConfigurableBase::class.java)
    }
}
