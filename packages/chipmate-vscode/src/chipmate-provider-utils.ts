import type { Session, Agent, Event, ProviderListResponse } from "@chipmate/sdk/v2/client"
...
import type { PartBatch, PartUpdate } from "./chipmate-provider/session-stream-scheduler"
import type { PartRemove } from "./shared/stream-messages"
import * as path from "path"

export { SessionStreamScheduler } from "./chipmate-provider/session-stream-scheduler"
...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[ChipMate New] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[ChipMate New] ${label} ended after server accepted it; ignoring transport error`, {
...
const DOCUMENT_AGENT_SCOPE_KEY = "chipmate.documentAgent.scope"
  ...
  const extra = await Promise.all(
    [...worktreeDirs].map((dir) =>
      list(dir).catch((err: unknown) => {
        console.error(`[ChipMate] Failed to list sessions for ${dir}:`, err)
        failed.add(dir)
        return [] as Session[]
      }),
    ),
