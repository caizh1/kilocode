...
/**
 * ChipMate Gateway Authentication Plugin
 *
 * Provides device authorization flow for ChipMate Gateway
 ...
 */
export const ChipMateAuthPlugin: Plugin = async (ctx) => {
  return {
    auth: {
      provider: "chipmate",
      async loader(getAuth, providerInfo) {
        ...
        if (auth.type === "api") {
          return {
            chipmateToken: auth.key,
          }
        }

        // For OAuth auth, access token contains the ChipMate token
      ...
      methods: [
        {
          type: "oauth",
          label: "ChipMate Gateway (Device Authorization)",
          async authorize() {
...
export default ChipMateAuthPlugin
