...
export const ServerProvider: ParentComponent = (props) => {
  ...
  onMount(() => {
    const unsubscribe = vscode.onMessage((message: ExtensionMessage) => {
      switch (message.type) {
        case "ready":
          console.log("[ChipMate New] Server ready:", message.serverInfo)
        ...
        case "connectionState":
          console.log("[ChipMate New] Connection state changed:", message.state)
        ...
        case "error":
          console.error("[ChipMate New] Server error:", message.message)
          setErrorMessage(message.message)
          setErrorDetails(message.message)
          break

        case "profileData":
          console.log("[ChipMate New] Profile data:", message.data ? "received" : "null")
          setProfileData(message.data)
          break

        case "deviceAuthStarted":
          console.log("[ChipMate New] Device auth started")
          setDeviceAuth({
            status: "pending",
            code: message.code,
        ...
        case "deviceAuthComplete":
          console.log("[ChipMate New] Device auth complete")
        ...
        case "deviceAuthFailed":
          console.log("[ChipMate New] Device auth failed:", message.error)
          setDeviceAuth({ status: "error", error: message.error })
          break

        case "deviceAuthCancelled":
          console.log("[ChipMate New] Device auth cancelled")
    ...
    console.log("[ChipMate New] Webview ready")
    const runtime = window as typeof window & {
      CHIPMATE_EXTENSION_VERSION?: string
      CHIPMATE_LOADING_MOTION_URI?: string
    }
    const reducedMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches
    vscode.postMessage({
      type: "webviewReady",
      ...(runtime.CHIPMATE_EXTENSION_VERSION ? { extensionVersion: runtime.CHIPMATE_EXTENSION_VERSION } : {}),
      ...(runtime.CHIPMATE_LOADING_MOTION_URI ? { motionBaseUri: runtime.CHIPMATE_LOADING_MOTION_URI } : {}),
   ...
   * outside the Profile page — e.g. the ChipMate Gateway card in the Providers
