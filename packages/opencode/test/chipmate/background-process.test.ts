...
`,
      ...
      const client = process.env["CHIPMATE_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["CHIPMATE_CLIENT"]
        else process.env["CHIPMATE_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["CHIPMATE_CLIENT"]
      const scans = process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"]
      process.env["CHIPMATE_CLIENT"] = "cli"
      process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"]
        else process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"]
      process.env["CHIPMATE_CLIENT"] = "cli"
      delete process.env["CHIPMATE_BACKGROUND_PROCESS_PORTS"]
