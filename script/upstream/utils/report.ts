 ...
 * Pass currentContent (our version of the file) to detect chipmate_change markers
...
export function getRecommendation(
  ...
  // Check if file should be skipped entirely (doesn't exist in ChipMate, shouldn't be added)
  if (shouldSkipFile(path, skipFiles)) {
    return {
      recommendation: "skip",
      reason: "File should be skipped (does not exist in ChipMate fork)",
    }
  }
  ...
  if (keepOurs.some((pattern) => path.includes(pattern) || path === pattern)) {
    return {
      ...
      reason: "File is ChipMate-specific and should not be overwritten",
    }
  }

  // ChipMate directories should always keep ours
  if (
    matches(
      path,
      defaultConfig.chipmateDirectories.map((dir) => `${dir}/**`),
    )
  ) {
    return {
      recommendation: "keep-ours",
      reason: "File is in a ChipMate-specific directory",
    }
  }
  ...
  if (shouldTakeTheirsTransform(path)) {
    // If our version has chipmate_change markers, flag for manual review
    if (currentContent?.includes("chipmate_change")) {
      return {
        recommendation: "manual",
        reason: "File has chipmate_change markers — auto-transform skipped, needs manual review",
      }
    }
    return {
      recommendation: "take-theirs-transform",
      reason: "Branding-only file: take upstream and apply ChipMate branding transforms",
    }
  }

  switch (type) {
    case "i18n":
      // i18n files that have chipmate_change markers need manual review
      if (currentContent?.includes("chipmate_change")) {
        return {
          recommendation: "manual",
          reason: "i18n file has chipmate_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "i18n-transform",
        reason: "i18n file: take upstream translations and apply ChipMate branding",
      }
    case "script":
      if (currentContent?.includes("chipmate_change")) {
        return {
          recommendation: "manual",
          reason: "Script file has chipmate_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
    ...
    case "extension":
      if (currentContent?.includes("chipmate_change")) {
        return {
          recommendation: "manual",
          reason: "Extension file has chipmate_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "extension-transform",
        reason: "Extension file: take upstream and apply ChipMate branding",
      }
    case "web":
      if (currentContent?.includes("chipmate_change")) {
        return {
          recommendation: "manual",
          reason: "Web/docs file has chipmate_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "web-transform",
        reason: "Web/docs file: take upstream and apply ChipMate branding",
      }
    case "markdown":
      return {
        recommendation: "keep-ours",
        reason: "Markdown files are typically ChipMate-specific documentation",
      }
    case "package":
      if (currentContent?.includes("chipmate_change")) {
        return {
          recommendation: "manual",
          reason: "package.json has chipmate_change markers — auto-transform skipped, needs manual review",
        }
      }
      return {
        recommendation: "package-transform",
        reason: "Package.json: take upstream, transform names, inject ChipMate deps, preserve version",
      }
    case "code":
      return {
        recommendation: "manual",
        reason: "Code files need manual review for chipmate_change markers",
      }
    case "config":
      return {
        recommendation: "manual",
        reason: "Config files may have ChipMate-specific settings",
      }
    default:
      return {
        recommendation: "manual",
...
export async function analyzeConflicts(
  ...
  for (const path of files) {
    const type = classifyFile(path)
    // Read current file content (our version) to detect chipmate_change markers
...
export function generateMarkdownReport(report: ConflictReport): string {
  ...
  for (const rec of order) {
    ...
    const titleMap: Record<ConflictFile["recommendation"], string> = {
      ...
      "i18n-transform": "i18n Transform (Auto-Apply ChipMate Branding)",
      "take-theirs-transform": "Take Upstream + ChipMate Branding (Auto)",
      ...
      "keep-ours": "Keep ChipMate Version (Ours)",
