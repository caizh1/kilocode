...
import { ChipMateConfigOverlay } from "@/chipmate/config/overlay"
import { ChipMateConfigWriter } from "@/chipmate/config/writer"
import { ChipMateConfigSources } from "@/chipmate/config/sources"
import { ChipMateModelState } from "@/chipmate/config/model-state"
import { ConfigRules } from "@/chipmate/server/routes/config-rules"
import { ChipMateKeybinds } from "@/chipmate/tui/keybinds"
import { ChipMateTuiConfig } from "@/chipmate/tui/config"
...
export const configConsoleHandlers = HttpApiBuilder.group(InstanceHttpApi, "config-console", (handlers) =>
  Effect.gen(function* () {
    ...
    const modelState = Effect.fn("ConfigConsoleHttpApi.modelState")(function* () {
      return yield* Effect.promise(() => ChipMateModelState.get())
    })
    ...
    const modelStateUpdate = Effect.fn("ConfigConsoleHttpApi.modelStateUpdate")(function* (ctx: {
      ...
      return yield* Effect.promise(() =>
        ChipMateModelState.update({ favorite: ctx.payload.favorite?.map((item) => ({ ...item })) }),
      )
    })

    const tuiConfigGet = Effect.fn("ConfigConsoleHttpApi.tuiConfigGet")(function* () {
      const instance = yield* InstanceState.context
      return yield* Effect.promise(() => ChipMateTuiConfig.get({ directory: instance.directory }))
    })

    const tuiKeybindList = Effect.fn("ConfigConsoleHttpApi.tuiKeybindList")(function* () {
      return { keybinds: ChipMateKeybinds.list() }
    })
