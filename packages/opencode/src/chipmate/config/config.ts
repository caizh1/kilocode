...
import { fetchOrganizationModes } from "@chipmate/chipmate-gateway"
...
export namespace ChipMateConfig {
  const log = Log.create({ service: "chipmate.config" })
  ...
  /** ChipMate-specific config file names (highest-to-lowest precedence within chipmate). */
  export const CHIPMATE_CONFIG_FILES = ["chipmate.jsonc", "chipmate.json"] as const

  /** All config file names in precedence order (chipmate + opencode). */
  ...
  /** Path patterns for resolving chipmate agent names from file paths. */
  export const AGENT_PATTERNS = ProductProfile.chipmate
    ? (["/.chipmate-v2/agent/", "/.chipmate-v2/agents/"] as const)
    : (["/.chipmate/agent/", "/.chipmate/agents/"] as const)

  /** Path patterns for resolving chipmate command names from file paths. */
  export const COMMAND_PATTERNS = ProductProfile.chipmate
    ? (["/.chipmate-v2/command/", "/.chipmate-v2/commands/"] as const)
    : (["/.chipmate/command/", "/.chipmate/commands/"] as const)

  /**
   * Choose the project config file that Config.update should patch.
   *
   * This mirrors the ChipMate project-config load chain: prefer existing config files
   * in ancestor config directories, then existing root config files, and create
   * `.chipmate/chipmate.jsonc` when no project config exists yet.
   */
  export const projectConfigFiles = Effect.fn("ChipMateConfig.projectConfigFiles")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
  }) {
    const dirs = yield* input.fs
      .up({ targets: [...CHIPMATE_DIR_SUFFIXES], start: input.directory, stop: input.worktree })
      .pipe(Effect.orDie)
  ...
  export const updateProjectConfig = Effect.fn("ChipMateConfig.updateProjectConfig")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
    config: Config.Info
  ...
  export const propagateUnset = Effect.fn("ChipMateConfig.propagateUnset")(function* (input: {
    fs: FSUtil.Interface
  ...
  /**
   * Load all ChipMate legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded chipmate custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped chipmate mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load chipmate modes", { error: err })
    }

    // Load ChipMate workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded chipmate workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load chipmate workflows", { error: err })
    }
    ...
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded chipmate rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("chipmate rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load chipmate rules", { error: err })
    }

    // Load ChipMate MCP servers (skip global VSCode extension paths unless running in an editor or Console daemon)
    const skipGlobal = process.env["CHIPMATE_PLATFORM"] !== "vscode" && process.env["CHIPMATE_FEATURE"] !== "daemon"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .chipmateignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded chipmate ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load chipmate ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the ChipMate Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const chipmate = auth["chipmate"]
      if (chipmate?.type === "oauth" && chipmate.access && chipmate.accountId) {
        const modes = await fetchOrganizationModes(chipmate.access, chipmate.accountId)
        if (modes.length > 0) {
  ...
  /** Client-neutral docs page describing where ChipMate reads configuration from. */
  export const CONFIG_DOCS_URL = "https://chipmate.ai/docs/getting-started/settings"

  /** Stable id for the synthetic "move your opencode config" notification (used for client-side dismissal). */
  export const OPENCODE_NOTIFICATION_ID = "chipmate.local.opencode-config-detected"

  /**
   * Detect leftover opencode config directories. ChipMate used to fall back to
  ...
  export function detectOpencodeConfig(input: {
    ...
    // Global opencode config dir (sibling of the chipmate global config dir, e.g. ~/.config/opencode).
  ...
  export function opencodeConfigNotification(input: { directory: string; worktree?: string; scanProject: boolean }) {
    ...
    return {
      ...
      message:
        `ChipMate no longer falls back to opencode configuration. ` +
        `Found opencode config at ${found[0]}${suffix}. ` +
        `Move it into a .chipmate directory (project) or ${Global.Path.config} (global).`,
