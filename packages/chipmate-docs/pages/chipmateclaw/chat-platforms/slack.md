---
title: "Slack"
description: "Using ChipMateClaw with Slack"
---

# Slack

This page covers everything you need to use ChipMateClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting ChipMateClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your ChipMateClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your ChipMateClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/chipmateclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to ChipMateClaw

1. In the [ChipMateClaw UI](https://app.chipmate.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the ChipMateClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with ChipMateClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.chipmate.ai/claw](https://app.chipmate.ai/claw) and confirm the pairing code and approve
...
By default, ChipMateClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making ChipMateClaw DM-Only (from you)

By default, ChipMateClaw will respond to DMs from any user in Slack.
...
## Adding ChipMateClaw to a Slack Channel

By default, ChipMateClaw will not respond in Slack channels, even if added. To have ChipMateClaw participate in a Slack channel:
...
Tell your ChipMateClaw agent (via DM):
