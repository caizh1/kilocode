...
describe("discoverScanFiles", () => {
  test("uses git for Code Graph exact extension discovery", async () => {
    ...
    const result = await discoverScanFiles({
      ...
      runGit: async (input) => {
        ...
        return ["main.c", "include/driver.H", "app.ts", ".chipmate/worktrees/feature/copy.c", "node_modules/pkg/native.c"]
      },
    })
  ...
  test("falls back to extension-scoped glob when git and rg fail", async () => {
    ...
    await mkdir(path.join(dir, ".chipmate", "worktrees", "feature"), { recursive: true })
    ...
    await Bun.write(path.join(dir, ".chipmate", "worktrees", "feature", "copy.c"), "int copy(void) { return 0; }\n")
