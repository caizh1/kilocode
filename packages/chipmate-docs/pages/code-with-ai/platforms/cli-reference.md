---
title: "CLI Command Reference"
description: "Complete reference for all ChipMate CLI commands and subcommands"
---

# CLI Command Reference

<!-- Auto-generated by script/generate-cli-docs.ts — do not edit manually -->

## chipmate acp

```
start ACP (Agent Client Protocol) server

Options:
  --help         Show help  [boolean]
  --version      Show version number  [boolean]
  --port         port to listen on  [number] [default: 0]
  --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
  --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
  --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
  --cors         additional domains to allow for CORS  [array] [default: []]
  --cwd          working directory  [string] [default: "."]
```

## chipmate mcp

```
manage MCP (Model Context Protocol) servers

Commands:
  chipmate mcp add [name]     add an MCP server
  chipmate mcp list           list MCP servers and their status  [aliases: ls]
  chipmate mcp auth [name]    authenticate with an OAuth-enabled MCP server
  chipmate mcp logout [name]  remove OAuth credentials for an MCP server
  chipmate mcp debug <name>   debug OAuth connection for an MCP server

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate mcp add

```
add an MCP server

Positionals:
  name  name of the MCP server  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --url      URL for a remote MCP server  [string]
  --env      environment variable for a local MCP server (KEY=VALUE)  [array]
  --header   HTTP header for a remote MCP server (KEY=VALUE)  [array]
```

### chipmate mcp list

```
list MCP servers and their status

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate mcp auth

```
authenticate with an OAuth-enabled MCP server

Commands:
  chipmate mcp auth list  list OAuth-capable MCP servers and their auth status  [aliases: ls]

Positionals:
  name  name of the MCP server  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate mcp auth list

```
list OAuth-capable MCP servers and their auth status

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate mcp logout

```
remove OAuth credentials for an MCP server

Positionals:
  name  name of the MCP server  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate mcp debug

```
debug OAuth connection for an MCP server

Positionals:
  name  name of the MCP server  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate [project]

```
start chipmate tui

Positionals:
  project  path to start chipmate in  [string]

Options:
      --help          Show help  [boolean]
      --version       Show version number  [boolean]
      --port          port to listen on  [number] [default: 0]
      --hostname      hostname to listen on  [string] [default: "127.0.0.1"]
      --mdns          enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
      --mdns-domain   custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
      --cors          additional domains to allow for CORS  [array] [default: []]
  -m, --model         model to use in the format of provider/model  [string]
  -c, --continue      continue the last session  [boolean]
  -s, --session       session id to continue  [string]
      --fork          fork the session when continuing (use with --continue or --session)  [boolean]
      --cloud-fork    fetch session from cloud and continue locally (use with --session)  [boolean]
      --prompt        prompt to use  [string]
      --agent         agent to use  [string]
      --auto          auto-approve permissions that are not explicitly denied (dangerous!)  [boolean] [default: false]
      --mini          start the minimal interactive interface  [boolean] [default: false]
      --no-replay     disable mini session history replay on resume and after resize  [boolean]
      --replay-limit  cap visible mini replay to the newest N messages  [number]
```

## chipmate attach

```
attach to a running chipmate server

Positionals:
  url  http://localhost:4096  [string]

Options:
      --help          Show help  [boolean]
      --version       Show version number  [boolean]
      --dir           directory to run in  [string]
  -c, --continue      continue the last session  [boolean]
  -s, --session       session id to continue  [string]
      --fork          fork the session when continuing (use with --continue or --session)  [boolean]
      --cloud-fork    fetch session from cloud and continue locally (use with --session)  [boolean]
  -p, --password      basic auth password (defaults to CHIPMATE_SERVER_PASSWORD)  [string]
  -u, --username      basic auth username (defaults to CHIPMATE_SERVER_USERNAME or 'chipmate')  [string]
      --mini          start the minimal interactive interface  [boolean] [default: false]
      --no-replay     disable mini session history replay on resume and after resize  [boolean]
      --replay-limit  cap visible mini replay to the newest N messages  [number]
```

## chipmate run

```
run chipmate with a message

Positionals:
  message  message to send  [string] [default: []]

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
      --command      the command to run, use message for args  [string]
  -c, --continue     continue the last session  [boolean]
  -s, --session      session id to continue  [string]
      --fork         fork the session before continuing (requires --continue or --session)  [boolean]
      --cloud-fork   fetch session from cloud and continue locally (use with --session)  [boolean]
      --share        share the session  [boolean]
  -m, --model        model to use in the format of provider/model  [string]
      --agent        agent to use  [string]
      --format       format: default (formatted) or json (raw JSON events)  [string] [choices: "default", "json"] [default: "default"]
  -f, --file         file(s) to attach to message  [array]
      --title        title for the session (uses truncated prompt if no value provided)  [string]
      --attach       attach to a running chipmate server (e.g., http://localhost:4096)  [string]
  -p, --password     basic auth password (defaults to CHIPMATE_SERVER_PASSWORD)  [string]
  -u, --username     basic auth username (defaults to CHIPMATE_SERVER_USERNAME or 'chipmate')  [string]
      --dir          directory to run in, path on remote server if attaching  [string]
      --port         port for the local server (defaults to random port if no value provided)  [number]
      --variant      model variant (provider-specific reasoning effort, e.g., high, max, minimal)  [string]
      --thinking     show thinking blocks  [boolean]
  -i, --interactive  run in direct interactive split-footer mode  [boolean] [default: false]
      --auto         auto-approve permissions that are not explicitly denied (dangerous!)  [boolean] [default: false]
```

## chipmate debug

```
debugging and troubleshooting tools

Commands:
  chipmate debug config        show resolved configuration
  chipmate debug lsp           LSP debugging utilities
  chipmate debug rg            ripgrep debugging utilities
  chipmate debug file          file system debugging utilities
  chipmate debug scrap         list all known projects
  chipmate debug skill         list all available skills
  chipmate debug snapshot      snapshot debugging utilities
  chipmate debug startup       print startup timing
  chipmate debug agent <name>  show agent configuration details
  chipmate debug v2            debug v2 catalog and built-in plugins
  chipmate debug info          show debug information
  chipmate debug paths         show global paths (data, config, cache, state)
  chipmate debug wait          wait indefinitely (for debugging)

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug config

```
show resolved configuration

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug lsp

```
LSP debugging utilities

Commands:
  chipmate debug lsp diagnostics <file>      get diagnostics for a file
  chipmate debug lsp symbols <query>         search workspace symbols
  chipmate debug lsp document-symbols <uri>  get symbols from a document

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug lsp diagnostics

```
get diagnostics for a file

Positionals:
  file  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug lsp symbols

```
search workspace symbols

Positionals:
  query  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug lsp document-symbols

```
get symbols from a document

Positionals:
  uri  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug rg

```
ripgrep debugging utilities

Commands:
  chipmate debug rg files             list files using ripgrep
  chipmate debug rg search <pattern>  search file contents using ripgrep

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug rg files

```
list files using ripgrep

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --query    Filter files by query  [string]
  --glob     Glob pattern to match files  [string]
  --limit    Limit number of results  [number]
```

### chipmate debug rg search

```
search file contents using ripgrep

Positionals:
  pattern  Search pattern  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --glob     File glob patterns  [array]
  --limit    Limit number of results  [number]
```

### chipmate debug file

```
file system debugging utilities

Commands:
  chipmate debug file read <path>     read file contents as JSON
  chipmate debug file list <path>     list files in a directory
  chipmate debug file search <query>  search files by query

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug file read

```
read file contents as JSON

Positionals:
  path  File path to read  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug file list

```
list files in a directory

Positionals:
  path  File path to list  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug file search

```
search files by query

Positionals:
  query  Search query  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug scrap

```
list all known projects

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug skill

```
list all available skills

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug snapshot

```
snapshot debugging utilities

Commands:
  chipmate debug snapshot track         track current snapshot state
  chipmate debug snapshot patch <hash>  show patch for a snapshot hash
  chipmate debug snapshot diff <hash>   show diff for a snapshot hash

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug snapshot track

```
track current snapshot state

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug snapshot patch

```
show patch for a snapshot hash

Positionals:
  hash  hash  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug snapshot diff

```
show diff for a snapshot hash

Positionals:
  hash  hash  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug startup

```
print startup timing

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug agent

```
show agent configuration details

Positionals:
  name  Agent name  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --tool     Tool id to execute  [string]
  --params   Tool params as JSON or a JS object literal  [string]
```

### chipmate debug v2

```
debug v2 catalog and built-in plugins

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug info

```
show debug information

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug paths

```
show global paths (data, config, cache, state)

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate debug wait

```
wait indefinitely (for debugging)

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate auth

```
manage AI providers and credentials

Commands:
  chipmate auth list               list providers and credentials  [aliases: ls]
  chipmate auth login [url]        log in to a provider
  chipmate auth logout [provider]  log out from a configured provider

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate auth list

```
list providers and credentials

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate auth login

```
log in to a provider

Positionals:
  url  chipmate auth provider  [string]

Options:
      --help      Show help  [boolean]
      --version   Show version number  [boolean]
  -p, --provider  provider id or name to log in to (skips provider selection)  [string]
  -m, --method    login method label (skips method selection)  [string]
```

### chipmate auth logout

```
log out from a configured provider

Positionals:
  provider  provider id or name to log out from  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate agent

```
manage agents

Commands:
  chipmate agent create  create a new agent
  chipmate agent list    list all available agents

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate agent create

```
create a new agent

Options:
      --help                  Show help  [boolean]
      --version               Show version number  [boolean]
      --path                  directory path to generate the agent file  [string]
      --description           what the agent should do  [string]
      --mode                  agent mode  [string] [choices: "all", "primary", "subagent"]
      --permissions, --tools  comma-separated list of permissions to allow (default: all). Available: "bash, read, edit, glob, grep, webfetch, task, todowrite, websearch, lsp, skill"  [string]
  -m, --model                 model to use in the format of provider/model  [string]
```

### chipmate agent list

```
list all available agents

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate upgrade

```
upgrade chipmate to the latest or a specific version

Positionals:
  target  version to upgrade to, for ex '0.1.48' or 'v0.1.48'  [string]

Options:
      --help     Show help  [boolean]
      --version  Show version number  [boolean]
  -m, --method   installation method to use  [string] [choices: "curl", "npm", "yarn", "pnpm", "bun", "brew", "choco", "scoop"]
```

## chipmate uninstall

```
uninstall chipmate and remove all related files

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
  -c, --keep-config  keep configuration files  [boolean] [default: false]
  -d, --keep-data    keep session data and snapshots  [boolean] [default: false]
      --dry-run      show what would be removed without removing  [boolean] [default: false]
  -f, --force        skip confirmation prompts  [boolean] [default: false]
```

## chipmate serve

```
starts a headless chipmate server

Options:
  --help         Show help  [boolean]
  --version      Show version number  [boolean]
  --port         port to listen on  [number] [default: 0]
  --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
  --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
  --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
  --cors         additional domains to allow for CORS  [array] [default: []]
```

## chipmate models

```
list all available models

Positionals:
  provider  provider ID to filter models by  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --verbose  use more verbose model output (includes metadata like costs)  [boolean]
  --refresh  refresh the models cache from models.dev  [boolean]
```

## chipmate roll-call

```
batch-test text models matching a filter for connectivity and latency

Positionals:
  filter  regex to filter models by provider/modelID (required)  [string]

Options:
  --help      Show help  [boolean]
  --version   Show version number  [boolean]
  --prompt    Prompt to send to each model  [string] [default: "Hello"]
  --timeout   Timeout for each model call in milliseconds  [number] [default: 25000]
  --parallel  Number of parallel model calls  [number] [default: 5]
  --verbose   Show verbose output  [boolean] [default: false]
  --quiet     Suppress progress and decoration  [boolean] [default: false]
  --output    Output format (table, json, or md)  [string] [choices: "table", "json", "md"] [default: "table"]
```

## chipmate profile

```
show ChipMate account profile

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --json     output profile as JSON  [boolean] [default: false]
```

## chipmate stats

```
show token usage and cost statistics

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --days     show stats for the last N days (default: all time)  [number]
  --tools    number of tools to show (default: all)  [number]
  --models   show model statistics (default: hidden). Pass a number to show top N, otherwise shows all
  --project  filter by project (default: all projects, empty string: current project)  [string]
```

## chipmate export

```
export session data as JSON

Positionals:
  sessionID  session id to export  [string]

Options:
  --help      Show help  [boolean]
  --version   Show version number  [boolean]
  --sanitize  redact sensitive transcript and file data  [boolean]
```

## chipmate import

```
import session data from JSON file or URL

Positionals:
  file  path to JSON file or share URL  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate github

```
manage GitHub agent

Commands:
  chipmate github install  install the GitHub agent
  chipmate github run      run the GitHub agent

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate github install

```
install the GitHub agent

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate github run

```
run the GitHub agent

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --event    GitHub mock event to run the agent for  [string]
  --token    GitHub personal access token (github_pat_********)  [string]
```

## chipmate pr

```
fetch and checkout a GitHub PR branch, then run chipmate

Positionals:
  number  PR number to checkout  [number]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate session

```
manage sessions

Commands:
  chipmate session list                list sessions
  chipmate session delete <sessionID>  delete a session

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate session list

```
list sessions

Options:
      --help       Show help  [boolean]
      --version    Show version number  [boolean]
  -n, --max-count  limit to N most recent sessions  [number]
      --format     output format  [string] [choices: "table", "json"] [default: "table"]
  -a, --all        list sessions from all projects  [boolean] [default: false]
  -s, --search     filter sessions by title  [string]
```

### chipmate session delete

```
delete a session

Positionals:
  sessionID  session ID to delete  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate remote

```
enable remote connection for real-time session relay

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate daemon

```
manage the local chipmate daemon

Commands:
  chipmate daemon          start the local chipmate daemon  [default]
  chipmate daemon start    start the local chipmate daemon
  chipmate daemon status   show local chipmate daemon status
  chipmate daemon stop     stop the local chipmate daemon
  chipmate daemon restart  restart the local chipmate daemon

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
      --port         port to listen on  [number] [default: 0]
      --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
      --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
      --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
      --cors         additional domains to allow for CORS  [array] [default: []]
      --json         print daemon details as JSON  [boolean]
  -f, --foreground   keep the command active until interrupted  [boolean]
```

### chipmate daemon start

```
start the local chipmate daemon

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
      --port         port to listen on  [number] [default: 0]
      --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
      --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
      --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
      --cors         additional domains to allow for CORS  [array] [default: []]
      --json         print daemon details as JSON  [boolean]
  -f, --foreground   keep the command active until interrupted  [boolean]
```

### chipmate daemon status

```
show local chipmate daemon status

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --json     print daemon details as JSON  [boolean]
```

### chipmate daemon stop

```
stop the local chipmate daemon

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --json     print daemon details as JSON  [boolean]
```

### chipmate daemon restart

```
restart the local chipmate daemon

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
      --port         port to listen on  [number] [default: 0]
      --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
      --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
      --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
      --cors         additional domains to allow for CORS  [array] [default: []]
      --json         print daemon details as JSON  [boolean]
  -f, --foreground   keep the command active until interrupted  [boolean]
```

## chipmate console

```
open or stop the local ChipMate Console (deprecated)

Commands:
  chipmate console       open the local ChipMate Console (deprecated)  [default]
  chipmate console stop  stop the local chipmate daemon

Options:
      --help         Show help  [boolean]
      --version      Show version number  [boolean]
      --port         port to listen on  [number] [default: 0]
      --hostname     hostname to listen on  [string] [default: "127.0.0.1"]
      --mdns         enable mDNS service discovery (defaults hostname to 0.0.0.0)  [boolean] [default: false]
      --mdns-domain  custom domain name for mDNS service (default: chipmate.local)  [string] [default: "chipmate.local"]
      --cors         additional domains to allow for CORS  [array] [default: []]
  -f, --foreground   keep the command active until interrupted  [boolean]
```

### chipmate console stop

```
stop the local chipmate daemon

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --json     print daemon details as JSON  [boolean]
```

## chipmate cloud

```
run Cloud Agent tasks

Commands:
  chipmate cloud start   start a Cloud Agent task
  chipmate cloud send    send a follow-up prompt to a Cloud Agent task
  chipmate cloud status  show Cloud Agent task status
  chipmate cloud result  show a Cloud Agent task result

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate cloud start

```
start a Cloud Agent task

Options:
  --help       Show help  [boolean]
  --version    Show version number  [boolean]
  --prompt     prompt for the Cloud Agent  [string] [required]
  --repo       repository shorthand or URL  [string]
  --repo-type  repository provider type  [string] [choices: "github", "gitlab", "git"]
  --branch     repository branch  [string]
  --model      Cloud Agent model  [string]
  --mode       Cloud Agent mode  [string]
  --org-id     ChipMate organization ID  [string]
  --stream     connect to the WebSocket stream and print events as JSONL  [boolean]
```

### chipmate cloud send

```
send a follow-up prompt to a Cloud Agent task

Options:
  --help        Show help  [boolean]
  --version     Show version number  [boolean]
  --session-id  Cloud Agent session ID  [string] [required]
  --prompt      follow-up prompt for the Cloud Agent  [string] [required]
```

### chipmate cloud status

```
show Cloud Agent task status

Options:
  --help        Show help  [boolean]
  --version     Show version number  [boolean]
  --session-id  Cloud Agent session ID  [string] [required]
  --message-id  Cloud Agent message ID  [string] [required]
```

### chipmate cloud result

```
show a Cloud Agent task result

Options:
  --help        Show help  [boolean]
  --version     Show version number  [boolean]
  --session-id  Cloud Agent session ID  [string] [required]
  --message-id  Cloud Agent message ID  [string] [required]
```

## chipmate db

```
database tools

Commands:
  chipmate db [query]     open an interactive sqlite3 shell or run a query  [default]
  chipmate db path        print the database path

Positionals:
  query  SQL query to execute  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --format   Output format  [string] [choices: "json", "tsv"] [default: "tsv"]
```

### chipmate db path

```
print the database path

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate config

```
configuration tools

Commands:
  chipmate config check  check configuration for warnings and errors

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

### chipmate config check

```
check configuration for warnings and errors

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```

## chipmate plugin

```
install plugin and update config

Positionals:
  module  npm module name  [string]

Options:
      --help     Show help  [boolean]
      --version  Show version number  [boolean]
  -g, --global   install in global config  [boolean] [default: false]
  -f, --force    replace existing plugin version  [boolean] [default: false]
```

## chipmate help

```
show full CLI reference

Positionals:
  command  command to show help for  [string]

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
  --all      show help for all commands  [boolean] [default: false]
  --format   output format  [string] [choices: "md", "text"] [default: "md"]
```

## chipmate completion

```
generate shell completion script

Options:
  --help     Show help  [boolean]
  --version  Show version number  [boolean]
```
