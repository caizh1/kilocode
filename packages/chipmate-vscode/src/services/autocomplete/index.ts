...
async function gateway(connection: ChipMateConnectionService): Promise<boolean> {
  const client = await connection.getClientAsync()
  const result = await client.chipmate.profile(undefined, { throwOnError: false })
...
export class Coordinator implements vscode.Disposable {
  ...
  constructor(context: vscode.ExtensionContext, connection: ChipMateConnectionService, options: Options = {}) {
