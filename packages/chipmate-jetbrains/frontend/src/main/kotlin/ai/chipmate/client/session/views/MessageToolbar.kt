...
import ai.chipmate.client.session.ui.selection.SessionCopyButton
...
internal class MessageToolbar(
    ...
    tooltip: String = ChipMateBundle.message("session.copy.hover"),
) : JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)) {
    constructor(text: () -> String?, revert: (() -> Unit)?) : this(
        text,
        revert?.let {
            listOf(ToolbarButtonAction(AllIcons.Actions.Rollback, ChipMateBundle.message("revert.message.rollback"), it))
        }.orEmpty(),
        ChipMateBundle.message("session.copy.prompt"),
    )
