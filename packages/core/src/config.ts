...
import { Flag } from "./flag/flag" // chipmate_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "chipmate.json", "chipmate.jsonc", "opencode.json", "opencode.jsonc"] // chipmate_change
    ...
    const discovered = locationIsGlobal || Flag.CHIPMATE_DISABLE_PROJECT_CONFIG // chipmate_change
      ? []
      : yield* fs
          .up({
            targets: [".chipmate", ...names.toReversed()], // chipmate_change
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".chipmate") // chipmate_change
        .toReversed()
    ...
    const directPaths = discovered.filter((item) => path.basename(item) !== ".chipmate").toReversed()
    // chipmate_change end
    ...
    // global config, project files, then ChipMate config-directory files. // chipmate_change
