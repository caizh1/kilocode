...
internal abstract class BaseSettingsUi<C : BaseContentPanel, D, P, R, W>(
    ...
    protected var appState: ChipMateAppStateDto = app.state.value
        private set
    ...
    @RequiresEdt
    private fun updateApp(state: ChipMateAppStateDto) {
        appState = state
        if (state.status != ChipMateAppStatusDto.READY) {
    ...
    private fun load() {
        ...
        if (appState.status != ChipMateAppStatusDto.READY || workspaceLoading || workspaceLoaded) return
    ...
    @RequiresEdt
    protected abstract fun draft(state: ChipMateAppStateDto): D
    ...
    @RequiresEdt
    protected open fun onSaveFailedAfterDispose(change: P) = ChipMateNotifications.error(failedText())
    ...
    @RequiresEdt
    protected open fun unavailable(state: ChipMateAppStateDto) = Unit
