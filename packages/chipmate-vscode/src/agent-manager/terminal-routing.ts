...
export interface TerminalRoutingDeps {
  ...
  /** Loopback URL + basic-auth password for the running `chipmate serve`. */
...
export class TerminalRouter {
  ...
  private buildWsUrl(ptyID: string, cwd: string): string {
    ...
    const token = Buffer.from(`chipmate:${config.password}`).toString("base64")
