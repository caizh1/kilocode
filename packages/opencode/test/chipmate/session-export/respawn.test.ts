...
import { getKillSwitchReason, resetEligibility } from "@/chipmate/session-export/eligibility"
...
describe("SessionExport worker respawn", () => {
  ...
  beforeEach(async () => {
    ...
    feature = process.env.CHIPMATE_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    await SessionExport.shutdown()
    resetEligibility()
    if (feature === undefined) delete process.env.CHIPMATE_FEATURE
    else process.env.CHIPMATE_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.CHIPMATE_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
