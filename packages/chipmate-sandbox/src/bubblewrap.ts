...
declare const CHIPMATE_BWRAP_SHA256: string | undefined
declare const CHIPMATE_SANDBOX_NETWORK_RELAY_PATH: string | undefined
declare const CHIPMATE_SANDBOX_SECCOMP_PATH: string | undefined
...
function relay() {
  if (typeof CHIPMATE_SANDBOX_NETWORK_RELAY_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./chipmate-sandbox-network-relay.ts", import.meta.url)), environment: {} }
  }
  const target = CHIPMATE_SANDBOX_NETWORK_RELAY_PATH.startsWith(".")
    ? fileURLToPath(new URL(CHIPMATE_SANDBOX_NETWORK_RELAY_PATH, import.meta.url))
    : path.resolve(path.dirname(process.execPath), CHIPMATE_SANDBOX_NETWORK_RELAY_PATH)
  return { path: target, environment: { BUN_BE_BUN: "1" } }
}

function seccomp() {
  if (typeof CHIPMATE_SANDBOX_SECCOMP_PATH !== "undefined") {
    return path.resolve(path.dirname(process.execPath), CHIPMATE_SANDBOX_SECCOMP_PATH)
  }
...
function digest() {
  return typeof CHIPMATE_BWRAP_SHA256 === "undefined" ? undefined : CHIPMATE_BWRAP_SHA256
}
...
function select(): Selection {
  const override = process.env.CHIPMATE_BWRAP_PATH
