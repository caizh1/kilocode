---
title: "Using ChipMate for Free"
description: "How to use ChipMate for free — Auto Free, finding free models, free autocomplete, and free background tasks"
---

# Using ChipMate for Free

ChipMate can be used completely free of charge. There are three places where ChipMate uses AI model inference, and each can be configured to use free models.

## Where ChipMate Uses Models
...
Each of these consumes credits by default. **To use ChipMate entirely for free, configure all three to use free models.**

## Free Agentic Usage

ChipMate provides free models for coding tasks through the ChipMate Gateway and partner providers.

### Auto Free

The easiest way to get started is [**Auto Free**](/docs/code-with-ai/agents/auto-model) (`chipmate-auto/free`). This is a ChipMate-provided model tier that automatically routes your requests to the best available free models — no configuration needed.
...
1. Run `chipmate` to open the CLI
...
ChipMate's cloud services — App Builder, Code Reviewer, and others — also support free models. Select any model labeled "(free)" in the model dropdown when configuring a cloud task.

{% callout type="tip" %}
Available free models change over time as ChipMate partners with different inference providers. Subscribe to our blog or join our [Discord](https://chipmate.ai/discord) for updates.
{% /callout %}

## Free Autocomplete

ChipMate's autocomplete feature provides AI-powered code completions as you type in IDE extensions.

By default, autocomplete routes through the ChipMate provider and uses credits. If you run out of credits without a free alternative configured, autocomplete stops working — but your main coding workflow is unaffected.

### How to Get It Free

Add your own Mistral AI API key via **BYOK (Bring Your Own Key)** on the ChipMate Gateway. Mistral offers a free tier for Codestral. When you configure a BYOK key, autocomplete requests use your key directly — at no cost on your ChipMate balance.
...
ChipMate uses a small model in the background for tasks like session titling. By default this is Auto Small, which consumes credits. If the small model is unavailable, ChipMate falls back to your primary model — which may also consume credits if it's a paid model.
...
**In the CLI:** Set the `small_model` parameter in `~/.config/chipmate/config.json`:
