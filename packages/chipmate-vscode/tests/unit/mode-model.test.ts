...
describe("modelPatch", () => {
  ...
  it("keeps current variant when next model supports it", () => {
    expect(modelPatch("chipmate", "anthropic/claude-sonnet-4-6", ["low", "high"], "high")).toEqual({
      model: "chipmate/anthropic/claude-sonnet-4-6",
    })
  })

  it("clears stale variant when next model does not support it", () => {
    expect(modelPatch("chipmate", "anthropic/claude-sonnet-4-6", ["low", "medium"], "high")).toEqual({
      model: "chipmate/anthropic/claude-sonnet-4-6",
      variant: null,
    })
  })
