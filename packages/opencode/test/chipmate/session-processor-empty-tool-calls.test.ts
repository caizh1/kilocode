        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === ChipMateSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(ChipMateSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === ChipMateSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(ChipMateSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          const selection = {
            ...
            modelID: ModelV2.ID.make("chipmate-auto/efficient"),
          }
          ...
          yield* test.reply(
            ...
            LLMEvent.stepFinish({
              ...
              usage: usage(),
              providerMetadata: {
                chipmate: { routedModelID: "openai/gpt-5.5-20260423", vercelID: "fra1::test" },
                gateway: {
