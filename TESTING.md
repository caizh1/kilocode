...
How to spin up the **local main-branch** ChipMate backend and test it with `curl` / `fetch`. Aimed at a running ChipMate CLI agent iterating on backend fixes without rebuilding the VS Code extension or TUI.
...
CHIPMATE_SERVER_PASSWORD="$PASS" bun dev serve --port 0 >/tmp/chipmate-serve.log 2>&1 &
echo $! >/tmp/chipmate-serve.pid
while ! grep -q "chipmate server listening" /tmp/chipmate-serve.log 2>/dev/null; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/chipmate-serve.log | grep -oE "[0-9]+$")
AUTH="Authorization: Basic $(printf 'chipmate:%s' "$PASS" | base64 | tr -d '\n')"
BASE="http://127.0.0.1:$PORT"

# Call any endpoint
curl -sS -H "$AUTH" -H "x-chipmate-directory: $PWD" "$BASE/global/health"

# Stop when done
kill "$(cat /tmp/chipmate-serve.pid)" 2>/dev/null || true
rm -f /tmp/chipmate-serve.pid /tmp/chipmate-serve.log
```

## 1. What this doc is for

Testing local backend fixes against a real running server, talking to it over HTTP the same way the VS Code extension, TUI, and `chipmate run --attach` do — but without any of those clients. Every request is a `curl` the agent can copy-paste.

For in-process tests (no socket, fastest loop) see `packages/opencode/test/chipmate/server/permission-allow-everything.test.ts` for the `Server.Default().app.request(...)` pattern. That's the right tool inside the `packages/opencode/` test suite; this doc is for out-of-process HTTP testing.

## 2. `chipmate serve` vs `bun dev serve` — important

| Command | What it runs |
|---|---|
| `chipmate serve` | The npm-installed production CLI on `$PATH`. **Not the code in this repo.** |
...
Do **not** use `createChipMateServer()` from `@chipmate/sdk/v2` to test local code: it spawns the PATH `chipmate` binary (`packages/sdk/js/src/v2/server.ts:38-136`), which is the wrong tool here.
...
```bash
CHIPMATE_SERVER_PASSWORD=$(openssl rand -hex 16) \
  bun dev serve --port 0 >/tmp/chipmate-serve.log 2>&1 &
echo $! >/tmp/chipmate-serve.pid
while ! grep -q "chipmate server listening" /tmp/chipmate-serve.log; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/chipmate-serve.log | grep -oE "[0-9]+$")
```
...
Omit `CHIPMATE_SERVER_PASSWORD` entirely. The server prints `Warning: CHIPMATE_SERVER_PASSWORD is not set; server is unsecured.` and the auth middleware is bypassed — fine for throwaway local testing, never for anything else.
...
| `--mdns-domain` | `chipmate.local` |  |
| `--cors` | `[]` | Extra allowed origins. |

## 4. The two mandatory request knobs

### Auth header (only if `CHIPMATE_SERVER_PASSWORD` was set)

```bash
AUTH="Authorization: Basic $(printf 'chipmate:%s' "$CHIPMATE_SERVER_PASSWORD" | base64 | tr -d '\n')"
```

The username is literally `chipmate` (same as the VS Code extension). Skip this whole block if you launched without a password.

### Directory header on every call

```bash
DIR_HEADER="x-chipmate-directory: $PWD"
```
...
|---|---|
| `CHIPMATE_SERVER_PASSWORD` | Enable Basic auth. Omit for auth-bypassed local testing. |
| `CHIPMATE_DB=":memory:"` | Skip on-disk SQLite — hermetic runs. |
| `CHIPMATE_DISABLE_DEFAULT_PLUGINS=true` | Don't auto-load bundled plugins. |
| `CHIPMATE_WORKSPACE_ID=<id>` | Single-workspace mode; disables control-plane routes. |
| `CHIPMATE_TELEMETRY_LEVEL=off` | Disable PostHog during tests. |
| `CHIPMATE_CONFIG_CONTENT='{…}'` | Inline JSON config without writing a file. |
...
const pass = process.env.CHIPMATE_SERVER_PASSWORD
const headers = pass ? { Authorization: "Basic " + Buffer.from("chipmate:" + pass).toString("base64") } : undefined
...
```bash
PORT="$PORT" CHIPMATE_SERVER_PASSWORD="$CHIPMATE_SERVER_PASSWORD" bun /tmp/probe.ts
rm /tmp/probe.ts
```

Reminder: this script **connects to** the server you launched in Section 3 — it does not start one. `createChipMateServer()` from the SDK would spawn the PATH `chipmate` binary (production CLI), which defeats the point of testing local code.

## 9. Pitfalls

- Running `chipmate serve` instead of `bun dev serve` runs the installed prod binary, not your edits.
- Missing `x-chipmate-directory` (or `?directory=`) returns `400` from `InstanceMiddleware`.
...
- `CHIPMATE_SERVER_PASSWORD` must be set **before** launch — changing it after doesn't rotate credentials.
- When sharing `/tmp/chipmate-serve.log` / `.pid` across terminals, unique-suffix the paths to avoid clobbering parallel runs.
