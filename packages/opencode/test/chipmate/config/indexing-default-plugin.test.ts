...
import { ChipMateDefaultPlugins } from "../../../src/chipmate/config/default-plugins"
import { INDEXING_PLUGIN } from "../../../src/chipmate/indexing-feature"
...
describe("chipmate default indexing plugin", () => {
  afterEach(async () => {
  ...
  test("injects indexing without registering an external plugin origin", () => {
    ...
    ChipMateDefaultPlugins.apply(config, { disabled: false })
  ...
  test("removes a persisted indexing marker from external plugin origins", () => {
    ...
    ChipMateDefaultPlugins.apply(config, { disabled: true })
  ...
  test("does not hard-enable indexing plugin when default plugins are disabled", async () => {
    const original = Flag.CHIPMATE_DISABLE_DEFAULT_PLUGINS
    ...
    try {
      ...
      Flag.CHIPMATE_DISABLE_DEFAULT_PLUGINS = original
    }
  })
})
