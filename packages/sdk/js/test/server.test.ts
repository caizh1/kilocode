// chipmate_change start - Tests for CHIPMATE_CONFIG_CONTENT merging
import { describe, test, expect, beforeEach, afterEach } from "bun:test"
import { buildConfigEnv } from "../src/server"

describe("buildConfigEnv", () => {
  const originalEnv = process.env.CHIPMATE_CONFIG_CONTENT

  beforeEach(() => {
    delete process.env.CHIPMATE_CONFIG_CONTENT
  })
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.CHIPMATE_CONFIG_CONTENT = originalEnv
    }
  })
  ...
  test("preserves existing CHIPMATE_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate ChipMate having injected modes via CHIPMATE_CONFIG_CONTENT
    process.env.CHIPMATE_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".chipmate/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".chipmate/rules/main.md")
  ...
  test("handles invalid JSON in existing CHIPMATE_CONFIG_CONTENT gracefully", () => {
    process.env.CHIPMATE_CONFIG_CONTENT = "invalid json {"
