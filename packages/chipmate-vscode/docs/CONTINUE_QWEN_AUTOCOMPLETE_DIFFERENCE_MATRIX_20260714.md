...
本文以旧迁移账本为历史基准，重新对齐当前 ChipMate 工作树与 Continue 当前固定源码。它取代旧账本作为“当前事实矩阵”，但不覆盖或删除旧文档。本文只描述 Qwen Coder inline completion；Chat、RAG、Agent、Next Edit 和 UI 不在范围内。

## 1. 审计口径

| 基线 | 固定值 | 说明 |
|---|---|---|
| 旧迁移账本 | `packages/chipmate-vscode/docs/CONTINUE_AUTOCOMPLETE_PARITY_LEDGER.md` | 历史计划和阶段记录，不再视为当前实现事实。 |
| 旧 Context 审计 | `packages/chipmate-vscode/docs/CONTINUE_AUTOCOMPLETE_CONTEXT_AUDIT.md` | 保留来源分析，重新判定 live capability。 |
| 旧 Prompt 审计 | `packages/chipmate-vscode/docs/CONTINUE_AUTOCOMPLETE_PROMPT_RENDERING_AUDIT.md` | 保留模板/token 公式，废弃阶段性 current-state 描述。 |
| ChipMate Git 基线 | `d567827207018deef65cf0af450cd9ecd7325835`，分支 `codex/v7.3.42-dev` | 本次比较使用该 HEAD 之上的当前未提交工作树，而不是只比较 HEAD。 |
| ChipMate 审计路径指纹 | `ab375d5931bd6a5bee7b4f5900ba8a6367c361c41660928db018f89d4c7fc9e2` | 由 Qwen provider、统一 selector/coordinator 和 CLI Qwen FIM 路由的文件内容清单计算；工作树继续变化后需重算。 |
...
| `ADAPTER` | ChipMate 的架构、安全或隐私适配；建议保留并测试。 |
| `EXTRA` | ChipMate 比 Continue 当前有效运行态更强或更严格。 |
| `GAP` | 非 streaming 的可执行差异，后续可直接补齐。 |
| `DECISION` | 确有差异，但必须先确认产品范围、默认值或隐私政策。 |
| `DORMANT` | 配置或代码存在，但不影响 live runtime；需接线、弃用或删去误导。 |
| `SHARED-RISK` | ChipMate 与 Continue 当前行为相同，但两者共有风险；不能误算 ChipMate parity gap。 |
| `EXCLUDED` | 本次按用户约束排除。 |
...
| 传输 | 扩展直接 fetch `/v1/completions`，扩展读取 endpoint/API key。 | 扩展经共享 `ChipMateConnectionService` 调 `/chipmate/qwen-fim`；CLI 解析 provider、endpoint、headers 和 auth，再请求上游 `/completions`。 | 旧账本 15、60-62、95 行已失效。 |
...
| Context | 多处描述为空 payload、仅 recently edited 或无 opened/import/root。 | ChipMate 已有 recently edited/opened/import definitions/root path 四个 qwen-owned collector；全部默认关闭。 | 旧阶段状态不能继续作为 current-state。 |
...
| 配置 | 旧 `chipmate.autocomplete.enabled/provider/qwen.model` 被视为激活入口。 | Live target 来自 `chipmate-code.new.autocomplete.provider/model`；`enableAutoTrigger` 只控制 automatic request gate，不再注销 provider。 | 旧 key 标为 dormant/compat；manual invoke 保留。 |
...
| ID | 证据 |
|---|---|
| `K1` | `packages/chipmate-vscode/src/services/autocomplete/index.ts:59-79,175-323,325-363`：统一 selector/coordinator、fallback 和 commands。 |
| `K2` | `packages/chipmate-vscode/src/services/qwen-autocomplete/config.ts:26-74`；`packages/chipmate-vscode/package.json:1029-1206`：live 选择配置、Qwen defaults 和 legacy keys。 |
| `K3` | `packages/chipmate-vscode/src/services/qwen-autocomplete/index.ts:37-118`：Qwen provider/collector registration 生命周期。 |
| `K4` | `packages/chipmate-vscode/src/services/qwen-autocomplete/ChipMateQwenInlineCompletionProvider.ts:153-461,463-575,651-688,856-948,955-1109,1182-1220`：gate、helper、context、prompt、cache、request、filter、render 和 stale check。 |
| `K5` | `packages/chipmate-vscode/src/services/qwen-autocomplete/QwenFimClient.ts:16-50`：扩展到 CLI SDK 的 Qwen FIM contract。 |
| `K6` | `packages/opencode/src/chipmate/server/httpapi/groups/chipmate-gateway.ts:151-162,307-319`；`packages/opencode/src/chipmate/server/httpapi/handlers/chipmate-gateway.ts:200-337`：CLI schema、provider/auth 解析和上游请求。 |
| `K7` | `packages/chipmate-vscode/src/services/qwen-autocomplete/qwenMultifileFimRenderer.ts:46-179`；`packages/chipmate-vscode/src/services/qwen-autocomplete/fimTemplates.ts:6-30`：Qwen single/multifile prompt、预算、suffix helper 和 stop list。 |
| `K8` | `packages/chipmate-vscode/src/services/qwen-autocomplete/snippets.ts:43-65,99-243,272-369`：payload、priority、token fit 和 opened-file formatting。 |
| `K9` | `packages/chipmate-vscode/src/services/qwen-autocomplete/recentlyEdited.ts:45-109`；`recentlyOpened.ts:43-145`；`importDefinitions.ts:68-155`；`rootPathContext.ts:60-149`：四个 live collector/adapters。 |
| `K10` | `packages/chipmate-vscode/src/services/qwen-autocomplete/prefilter.ts`；`language.ts`；`guard.ts`；`packages/chipmate-vscode/src/services/autocomplete/shims/FileIgnoreController.ts`：C/C++、Python、Shell、Gitea Workflow selector、语言元数据和安全/ignore gate。 |
| `K11` | `packages/chipmate-vscode/src/services/qwen-autocomplete/autocompleteLruCache.ts:23-115`：in-memory prefix LRU。 |
| `K12` | `packages/chipmate-vscode/src/services/qwen-autocomplete/range.ts:8-39`；`multiline.ts:21-79`；`postprocess.ts`；`streamFilters.ts`：非流式 filter/postprocess/render adapters。 |

## 5. 主差异矩阵

### 5.1 激活、编辑器入口和生命周期

| ID | 能力 | Continue `d0a3` 有效行为 | ChipMate 当前有效行为 | 状态 | 优先级 | 后续动作 | 证据 |
|---|---|---|---|---|---|---|---|
...
必须区分“option/type/collector 存在”和“当前 VS Code runtime 真的产生并选择数据”。Continue 的 recently edited/visited 声明为 enabled，但对应采集入口当前未接通；不能把它们误报成 ChipMate live gap。
...
| `T12` | Streaming generation/reuse | Continue 使用 stream、GeneratorReuse、fullStop 和 partial filters。 | ChipMate 固定 `stream:false`，完整响应后处理。 | `EXCLUDED` | — | 不实施；只保留非流式等价/防御性过滤。 | `U1`,`K4`,`K6` |
...
| `F09` | Cache key 与 multifile prefix | lookup 用 `helper.prunedPrefix`，put 用 rendered `outcome.prefix`；key 不含 file/model/provider/suffix/context。 | 同样 lookup raw helper prefix；有注入时 put rendered prefix；key 同样不 namespaced。 | `SHARED-RISK` | — | 不是 ChipMate parity gap。若后续加 namespaced semantic key，明确记录为安全 hardening，而非 Continue port。 | `U1`,`U10`,`K4`,`K11` |
| `F10` | Cache hit postprocess | cache 存储已 postprocessed completion；hit 直接进入 render，不再次 postprocess。 | 相同。 | `MATCH` | — | 保留。 | `U1`,`K4` |
| `F11` | Diagnostics | Continue outcome/logging 以 completion lifecycle 为中心。 | ChipMate 有 request phase、cache/context count、server phase、hashed path 和 stale diagnostics。默认 `logLevel=off`；debug 下 completion first-line preview 默认允许但会 redact，prompt preview 恒为空。 | `EXTRA` | — | 保留脱敏 diagnostics；不要扩大 code-bearing preview。 | `K4` |
...
| `S02` | Ignore policy | 支持 `disableInFiles`、global/workspace `.continueignore` 和 Continue config exclusion。 | `.chipmateignore` 优先；否则 `.gitignore`+`.env*`；workspace 外拒绝；错误 fail closed。 | `ADAPTER` | — | 保留 ChipMate policy；文档说明不读取 `.continueignore`。 | `U11`,`K10` |
| `S03` | `onlyMyCode` | 默认 true，但 d0 中它主要约束已被关闭的 IDE snippets；不能解释为所有 source 都 workspace-only。 | 所有 context read 都要求 workspace 内且通过 ChipMate ignore。 | `EXTRA` | — | 保留更强边界。 | `U5`,`K10` |
| `S04` | Legacy settings | Continue options 都进入 active config merge。 | `chipmate.autocomplete.enabled/provider/qwen.model`、`prefixChars`、`suffixChars` 不驱动当前 Qwen lifecycle。 | `DORMANT` | `P2` | 标 deprecated/compat 或删除 manifest 暴露；迁移代码确认后再动。 | `K2` |
...
- 🚫 不实施：Continue SQLite code-bearing cache persistence；保留 ChipMate memory-only adapter。
...
| 检查 | 结果 |
|---|---|
| `packages/chipmate-vscode`: `bun test tests/unit/qwen-autocomplete*.test.ts tests/unit/autocomplete-manager-lifecycle.test.ts tests/unit/autocomplete-runtime-isolation.test.ts tests/unit/autocomplete-selection.test.ts` | 234 pass，0 fail，859 assertions，20 files。 |
| `packages/opencode`: `bun test ./test/chipmate/server/qwen-fim-custom-provider.test.ts ./test/chipmate/server/chipmate-gateway-statuses.test.ts` | 20 pass，0 fail，58 assertions，2 files。 |
| `packages/chipmate-vscode`: `bun run typecheck` | 通过。 |
| `packages/opencode`: `bun run typecheck` | 通过。 |
| 根目录：`./script/generate.ts` | 通过；Qwen `stop` 已进入 OpenAPI 和 generated SDK。 |
| `bun run script/check-md-table-padding.ts packages/chipmate-vscode/docs/CONTINUE_QWEN_AUTOCOMPLETE_DIFFERENCE_MATRIX_20260714.md` | 通过，无 padded tables。 |
| `git diff --check -- packages/chipmate-vscode/docs/CONTINUE_QWEN_AUTOCOMPLETE_DIFFERENCE_MATRIX_20260714.md` | 通过。 |
...
3. 跑相应 `packages/chipmate-vscode` 定向单测；涉及 CLI route 时再跑 `packages/opencode/test/chipmate/server/qwen-fim-custom-provider.test.ts`。
4. 涉及 API schema 时重新生成 SDK，不手改 `packages/sdk/js/src/gen/**`。
5. 涉及 shared `packages/opencode/src` 时遵循 annotation guard；当前 Qwen route 位于 ChipMate-owned path。
