export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // chipmate_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // chipmate_change - encode as UTF-8 bytes
