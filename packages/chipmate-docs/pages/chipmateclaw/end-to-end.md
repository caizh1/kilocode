...
description: "Start-to-finish guide for configuring your ChipMateClaw instance"
---

# Setup walkthrough

This guide walks you through a full ChipMateClaw setup — from creating accounts to scheduling your first automated workflow. Plan for about 60 minutes.

## Planning your setup

For most users, a useful ChipMateClaw configuration involves:
...
We recommend creating **separate accounts** for your ChipMateClaw rather than connecting it to your personal accounts. This applies to Google, GitHub, and any other services you connect. A dedicated account improves isolation — your personal data stays separate, and you can control exactly what access the Claw has by sharing or delegating only what you want.

### Chat platform options

- **[ChipMate Chat](https://app.chipmate.ai)** — available in the ChipMate web and mobile apps, plus supported ChipMate editor and TUI surfaces; requires zero configuration
- **[Telegram](/docs/chipmateclaw/chat-platforms/telegram)** — easy to set up, private by default
- **[Discord](/docs/chipmateclaw/chat-platforms/discord)** — moderate setup
- **[Slack](/docs/chipmateclaw/chat-platforms/slack)** — most involved setup

{% callout type="warning" title="Chain-of-connection security" %}
If your Claw has access to sensitive data (like your email), be careful which chat platform you connect it to. On broadly-accessible platforms like Slack or Discord, anyone on the server could potentially message your Claw and access that data. If you're connecting sensitive integrations, use a private platform like ChipMate Chat or Telegram.
{% /callout %}
...
If you are doing a [1-1 configuration call with ChipMate](https://chipmate.ai/chipmateclaw/config-service), please complete these steps before the call.
...
1. **Create a Google Account for your Claw** — Go to [google.com](https://www.google.com/) and create a new Google/Gmail account dedicated to your ChipMateClaw. Something like `yourname.bot@gmail.com` works well.
...
2. **Set up Google Cloud** — Visit [console.cloud.google.com](https://console.cloud.google.com). Accept the terms of service and click "Start my free tier". You may need to add a credit card for identity verification.
   ...
   {% callout type="info" %}
   Nothing ChipMateClaw does costs any money with Google.
   {% /callout %}

3. **Install Docker** — ChipMateClaw configures Google by running a Docker container on your machine. Download Docker at [docker.com](https://www.docker.com/), then open it. You don't need to sign in or create a Docker account.
...
Your Claw needs a way to communicate with you. **[ChipMate Chat](https://app.chipmate.ai)** requires no setup — open the ChipMate web or mobile app, or use a supported ChipMate editor or TUI surface. For other platforms, follow the relevant guide:

- [Telegram](/docs/chipmateclaw/chat-platforms/telegram) — about 2 minutes
- [Discord](/docs/chipmateclaw/chat-platforms/discord) — about 10 minutes
- [Slack](/docs/chipmateclaw/chat-platforms/slack) — about 15 minutes; always use the manifest

{% callout type="tip" %}
If you're not sure which to pick, ChipMate Chat (no setup) or Telegram (2 minutes) are the easiest options.
{% /callout %}
...
1. In the ChipMateClaw dashboard, go to **Settings → Google Account** and copy the Docker command shown.
...
For full details, see the [Google setup guide](/docs/chipmateclaw/development-tools/google).
...
For full details, see the [GitHub setup guide](/docs/chipmateclaw/development-tools/github).
...
**Push notifications:** by default, your Claw wakes up on every incoming email. If you'd prefer a digest (e.g., once at 7am), disable push notifications in **Settings → Google Account** on the [dashboard](/docs/chipmateclaw/dashboard) — otherwise it processes each email as it arrives.
...
By default, ChipMateClaw asks for confirmation before every tool call. To let it act freely, go to the [ChipMateClaw dashboard](https://app.chipmate.ai/claw) and enable auto-approval in the **Default Permissions** section.
...
Or ask your Claw to build a custom skill from scratch — it has a built-in skill-builder skill for exactly this. You can explore popular skills and use case inspiration at the [ChipMateClaw Bytes library](https://chipmate.ai/chipmateclaw/bytes).

## Manage inference

**Model picker:** Efficient is a good starting point. Frontier is more capable but significantly more expensive.

You can also use your [ChipMate Pass](https://chipmate.ai/pricing/chipmate-pass) credits — find this under **Profile** in the dashboard.
