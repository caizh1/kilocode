...
test("project config permission keys are attributed to the local scope", async () => {
  ...
  await Bun.write(path.join(chipmate, "chipmate.json"), JSON.stringify({ permission: { bash: { "echo *": "allow" } } }))
...
test("a scalar project bash permission maps to the '*' pattern under the local scope", async () => {
  ...
  const chipmate = path.join(dir, ".chipmate")
  await fs.mkdir(chipmate, { recursive: true })
  await Bun.write(path.join(chipmate, "chipmate.json"), JSON.stringify({ permission: { bash: "allow" } }))
