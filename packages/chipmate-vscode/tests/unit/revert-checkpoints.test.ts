...
const PROVIDER_FILE = path.join(ROOT, "src/ChipMateProvider.ts")
...
describe("revert workspace restoration status", () => {
  it("renders explicit conversation-only outcomes", () => {
    ...
    expect(sdk).toMatch(
      /export type ChipmateSessionImportSessionData = \{[\s\S]*?workspace\?: "restored" \| "snapshots-disabled" \| "unavailable"/,
    )
