...
const RenderWordDocumentParameters = Schema.Struct({
  ...
  remoteEndpoint: Schema.optional(Schema.String).annotate({
    description:
      "Optional external renderer endpoint. If omitted, CHIPMATE_WORD_RENDER_ENDPOINT is used; if neither is set, the tool returns a warning artifact.",
  }),
  outputFile: Schema.optional(Schema.String).annotate({
...
function wordFailure(title: string, err: unknown, metadata: WordMeta = {}): Tool.ExecuteResult<WordMeta> {
  ...
  return {
    ...
    output: [
      `${title}: ${message}`,
      "This Word sidecar operation failed; ChipMate native QA, code understanding, and document_search are not replaced by this tool.",
    ].join("\n"),
  }
}
