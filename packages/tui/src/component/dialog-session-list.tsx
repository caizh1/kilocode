...
export function DialogSessionList() {
  ...
  const [global, setGlobal] = createSignal(false) // chipmate_change - show current worktree by default
  const deleteHint = useCommandShortcut("session.delete")
  const quickSwitch1 = useCommandShortcut("session.quick_switch.1")
  const quickSwitch9 = useCommandShortcut("session.quick_switch.9")

  // chipmate_change start - experimental sessions retain worktree labels and scope
  ...
  // chipmate_change - support local and global sessions
  ...
  const options = createMemo(() => {
    ...
    const all = global() // chipmate_change
    ...
    function buildOption(id: string, category: string) {
      ...
      return {
        ...
        description: all && "worktreeName" in x && x.worktreeName ? `(${x.worktreeName})` : undefined, // chipmate_change
  ...
  return (
    <DialogSelect
      title={global() ? "Sessions (all worktrees)" : "Sessions (current worktree)"} // chipmate_change
      options={options()}
      skipFilter={true}
      ...
      // chipmate_change start - preserve Ctrl+A worktree scope toggle with the upstream keymap engine
