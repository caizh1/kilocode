...
class ChipMateBackendModelStateManager(
    private val log: ChipMateLog,
) {
    companion object {
        private val DEFAULT_DIR = Path.of(System.getProperty("user.home"), ".local", "state", "chipmate")
    }
    ...
    suspend fun state(): ModelStateDto = mutex.withLock {
        ChipMateCliDataParser.parseModelState(read().orEmpty())
    }
    ...
    suspend fun favorite(update: ModelFavoriteUpdateDto): ModelStateDto = mutex.withLock {
        ...
        val state = ChipMateCliDataParser.parseModelState(raw.orEmpty())
        ...
        write(ChipMateCliDataParser.buildModelStateJson(raw, updated))
        updated
    }
    ...
    private fun resolve(): Path? {
        ...
        return try {
            http.newCall(request).execute().use { response ->
                ...
                val dir = ChipMateCliDataParser.parsePathState(raw)?.let(Path::of) ?: DEFAULT_DIR
