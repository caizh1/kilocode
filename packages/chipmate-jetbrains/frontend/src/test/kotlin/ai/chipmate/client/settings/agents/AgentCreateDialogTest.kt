...
class AgentCreateDialogTest : BasePlatformTestCase() {
    ...
    fun `test default form is empty primary project`() {
        ...
        assertEquals(AgentCreateDto("", "", ChipMateCliParser.MODE_PRIMARY, null, "project"), edt { d.result() })
    }
    ...
    fun `test reads form values into dto`() {
        ...
        assertEquals(AgentCreateDto("reviewer", "Review carefully", ChipMateCliParser.MODE_SUBAGENT, "Reviews code", "global"), result)
    }
    ...
    private fun title(field: String) = ChipMateBundle.message("settings.agentBehavior.agents.create.$field")
