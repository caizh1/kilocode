...
import { ChipMateConfigInjector } from "../../src/chipmate/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("ChipMateConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await ChipMateConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
    ...
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".chipmate", "rules", "main.md"), "# Rules")
        },
      })

      const result = await ChipMateConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".chipmateignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".chipmateignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = ChipMateConfigInjector.getEnvVars("{}")
    ...
    test("returns CHIPMATE_CONFIG_CONTENT for non-empty config", () => {
      const config = JSON.stringify({ agent: { test: {} } })
      const envVars = ChipMateConfigInjector.getEnvVars(config)

      expect(envVars).toEqual({
        CHIPMATE_CONFIG_CONTENT: config,
      })
    })
  })
})
