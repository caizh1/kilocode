...
class HistoryControllerTest : BasePlatformTestCase() {
    ...
    fun `test history panel sync updates running badges`() {
        ...
        assertEquals(ChipMateBundle.message("session.part.tool.running"), panel.badgeText(0))
    }
    ...
    fun `test history panel overlay shows specific badge`() {
        ...
        assertEquals(ChipMateBundle.message("history.badge.permission"), panel.badgeText(0))
    }
    ...
    fun `test history panel sync repaints activity kind change`() {
        ...
        assertEquals(ChipMateBundle.message("history.badge.question"), panel.badgeText(0))
    }
    ...
    fun `test panel shows back button with tabs`() {
        ...
        assertEquals(ChipMateBundle.message("history.back"), panel.backText())
        val backs = UIUtil.uiTraverser(panel.component).filter(HoverIcon::class.java)
            .filter { it.toolTipText == ChipMateBundle.message("history.back") }
            .toList()
    ...
    fun `test cloud history uses relative time and sections`() {
        ...
        assertEquals(ChipMateBundle.message("history.time.hours", 5), HistoryTime.relative(today, now.toEpochMilli()))
        ...
        assertEquals(ChipMateBundle.message("history.time.hours", 10), HistoryTime.relative(offset, now.toEpochMilli()))
    }
