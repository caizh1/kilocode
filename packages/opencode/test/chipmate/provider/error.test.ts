...
describe("Google Gemini authentication errors", () => {
  test("explains how to troubleshoot the rejected API key", () => {
    ...
    expect(result.data.message).toBe(
      "Google Gemini rejected this API key. Check its type and status in Google AI Studio. Replace a Standard key with a new auth key; if it is already an auth key, check its Gemini API access or create a replacement. Restricted Standard keys work only until September 2026. See https://chipmate.ai/docs/ai-providers/gemini.",
    )
