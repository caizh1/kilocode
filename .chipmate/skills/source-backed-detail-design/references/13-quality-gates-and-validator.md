...
本文件只提供源码驱动详细设计的人工 review 建议。它不是 ChipMate 文档合同，不是自动修复流水线，不是缺失图表渲染计划，也不是 ChipMate QA 的触发条件。

不要迁移或调用旧的 artifact validator helper。ChipMate 中的详细设计 skill 应依靠模型、ChipMate 原生证据工具、通用 Word/Mermaid/artifact tools，以及人工可读的 review notes。
...
- Word 封面是否有效：是否存在唯一且文本正确的 `Title`；Subtitle 是否根据本次范围与证据动态生成、非空、不等于“详细设计”、不重复标题且不是占位文字；默认作者是否为 `ChipMate source-backed-detail-design`，用户明确指定作者时是否正确覆盖；是否不存在模型自行生成的 ChipMate/ChipMate 作者变体；目录前是否存在简洁且有证据的范围、源码基线和证据状态说明，未知事实是否被省略或标记“待确认”。
...
- 双重就绪是否成立：是否计算 `expectedBaseSlots = 5D - evidencedStateMachineNaCount`，并以每次渲染返回的 `workflowProgress` 为唯一图形进度事实；`wordAllowed=false`、非空 `remainingDesignUnitIds` 或非空 `nextMissingSlots` 时是否保持 Word `not_started`，且不得用记忆中的批次数或当前 batch `complete:true` 覆盖全局结果。
