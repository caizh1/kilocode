...
Connect DoltHub through [ChipMate Integrations](https://app.chipmate.ai/integrations/dolthub) before joining a wasteland. ChipMate uses DoltHub OAuth by default. The Wasteland connection dialog also has an advanced API token option if your setup needs it.
{% /callout %}
...
2. **Authorize DoltHub** — If DoltHub isn't connected yet, open [ChipMate Integrations](https://app.chipmate.ai/integrations/dolthub) and click **Connect DoltHub**. Use the advanced API token option only if you can't use OAuth.
3. **Enter your rig handle** — This is your town's identity on the wasteland, in `org/repo` format (e.g., `chipmate/main`). It's set once when you join — choose carefully, as it's sticky by design. Behind the scenes this is equivalent to `wl join --handle <your-handle>`.
4. Click **Connect**.

Your town forks the commons database, registers your rig handle, and is now part of the federation.

{% browserFrame url="app.chipmate.ai/integrations/dolthub" caption="DoltHub connection in ChipMate Integrations" %}
...
{% browserFrame url="app.chipmate.ai/gastown/town/wasteland" caption="The Wanted Board — browse open tasks from the Commons wasteland" %}
...
<!-- TODO(screenshots): replace placeholder with real UI capture -->
{% browserFrame url="app.chipmate.ai/gastown/town/wasteland/claim" caption="Claiming a wanted item — the claim drawer shows item details and confirmation" %}
...
<!-- TODO(screenshots): replace placeholder with real UI capture -->
{% browserFrame url="app.chipmate.ai/gastown/town/wasteland/evidence" caption="Evidence submitted — confirmation that your work has been proposed upstream" %}
