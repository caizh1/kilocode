...
it.live("nvidia provider includes ChipMate billing origin header", () =>
    ...
    withNvidiaKey(
      Provider.Service.use((provider) =>
        Effect.gen(function* () {
          ...
          expect(headers["HTTP-Referer"]).toBe("https://chipmate.ai/")
          expect(headers["X-Title"]).toBe("ChipMate")
          expect(headers["X-BILLING-INVOKE-ORIGIN"]).toBe("ChipMate")
        }),
      ),
    ),
  ),
