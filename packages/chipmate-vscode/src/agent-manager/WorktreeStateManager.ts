 ...
 *
 * Persists to `.chipmate-v2/agent-manager.json`. Decouples worktrees from sessions
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, CHIPMATE_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    const migration = { refsFixed: 0 as const }
    try {
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .chipmate paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.chipmate([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.chipmate${trailingSep}`
        }) ?? wt.path
