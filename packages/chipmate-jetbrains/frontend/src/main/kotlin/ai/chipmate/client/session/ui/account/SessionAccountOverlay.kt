package ai.chipmate.client.session.ui.account
...
import ai.chipmate.client.settings.profile.formatBalance
...
internal class SessionAccountOverlay(
    ...
    private val profileBtn = HoverIcon().apply {
        ...
        toolTipText = ChipMateBundle.message("action.ChipMate.ShowProfile.description")
        accessibleContext.accessibleName = ChipMateBundle.message("action.ChipMate.ShowProfile.text")
    ...
    @RequiresEdt
    private fun updateLoggedIn(prof: ai.chipmate.rpc.dto.ProfileDto, switching: Boolean, target: String?): Boolean {
        var layout = false

        val orgs = prof.organizations
        val personal = prof.hasPersonalAccount
        val next = (if (personal) listOf(AccountChoice(null, ChipMateBundle.message("profile.personalAccount"))) else emptyList()) +
        ...
        val tip = if (switching) {
            ChipMateBundle.message("profile.switchingAccount")
        } else {
            ChipMateBundle.message("session.account.switcher")
        }
    ...
    @RequiresEdt
    private fun syncBalance(prof: ai.chipmate.rpc.dto.ProfileDto): Boolean {
        var layout = false
        ...
        if (next == null) {
            ...
            val tip = ChipMateBundle.message("session.account.balance", next)
