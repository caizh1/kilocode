...
import { Struct } from "effect" // chipmate_change
...
import { DiffFull } from "../chipmate/snapshot/diff-full"
import { ChipMateSnapshotTrack } from "../chipmate/snapshot/track"
import { ChipMateSnapshotSeed } from "../chipmate/snapshot/seed"
import { ChipMateSnapshotMaterialize } from "../chipmate/snapshot/materialize"
...
}).pipe(withStatics((s) => ({ zod: zod(s) }))) // chipmate_change
export type Patch = typeof Patch.Type

// chipmate_change - retain the legacy Zod facade while sharing the canonical schema
export const FileDiff = Info.pipe(withStatics((s) => ({ zod: zod(s) })))
export type FileDiff = typeof FileDiff.Type

// chipmate_change start - lightweight FileDiff without heavy content (patch/before/after) for session summaries
...
const prune = "7.days"
const retention = 7 * 24 * 60 * 60 * 1000 // chipmate_change
...
export const MAX_DIFF_SIZE = 256 * 1024 // chipmate_change
...
export interface Interface {
  ...
  // chipmate_change start - pass prompt context and managed initialization policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: ChipMateSnapshotTrack.SnapshotInitialization
  ...
  readonly diffFile: (from: string, to: string, file: string) => Effect.Effect<FileDiff | undefined> // chipmate_change - authoritative full-content detail
}
...
export const layer: Layer.Layer<Service, never, Requirements> =
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const flock = yield* EffectFlock.Service // chipmate_change
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          // chipmate_change start - restoration must fail if deletion fails
          const remove = (file: string) => fs.remove(file, { force: true }).pipe(Effect.orDie)
          // chipmate_change end
          // chipmate_change start - serialize snapshot repositories across CLI and extension processes
          ...
          const enabled = Effect.fnUntraced(function* () {
            ...
            if (Flag.CHIPMATE_CLIENT === "acp") return false // chipmate_change - ACP clients do not support snapshots
          ...
          const add = Effect.fnUntraced(function* (opts?: { env?: Record<string, string>; root?: boolean }) {
            ...
            // chipmate_change start - initial seeded writes stay protected by the source pin
            yield* stage(
          ...
          const materialize = Effect.fnUntraced(function* () {
            yield* locked(ChipMateSnapshotMaterialize.run({ gitdir: state.gitdir, git, fs }).pipe(Effect.orDie)).pipe(
          ...
          const cleanup = Effect.fnUntraced(function* () {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // chipmate_change start - retain snapshots for the same seven-day window as object pruning
                yield* ChipMateSnapshotMaterialize.prune({ gitdir: state.gitdir, git, fs }, Date.now() - retention)
                // chipmate_change end
          ...
          const track = Effect.fnUntraced(function* (opts?: Parameters<Interface["track"]>[0]) {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const seeded: { value?: ChipMateSnapshotSeed.Output } = {} // chipmate_change
                ...
                if (!existed) {
                  ...
                  // chipmate_change start - seed all eligible new snapshots from the worktree index
                  seeded.value = yield* ChipMateSnapshotSeed.seed({
                    dir: state.directory,
                ...
                // chipmate_change start - pin every snapshot before background materialization
                ...
                if (
                  seed &&
                  !(yield* ChipMateSnapshotMaterialize.localize({
                    gitdir: state.gitdir,
                    git,
                    fs,
                ...
                if (
                  seed &&
                  !(yield* ChipMateSnapshotMaterialize.localizeTrees(
                    { gitdir: state.gitdir, git, fs, staging: seed.staging },
                    hash,
                  ))
                )
                  return
                if (!(yield* ChipMateSnapshotMaterialize.pin({ gitdir: state.gitdir, git, fs }, hash))) return
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                if (result.code === 0) {
                  ...
                  return yield* Effect.die(new Error(`Failed to restore snapshot ${snapshot}`)) // chipmate_change
                }
                yield* Effect.logError("failed to restore snapshot", {
                  snapshot,
                  exitCode: result.code,
          ...
          const revert = Effect.fnUntraced(function* (patches: Patch[]) {
            return yield* locked(
              Effect.gen(function* () {
                // chipmate_change start - validate every checkpoint before mutating workspace files
                ...
                const single = Effect.fnUntraced(function* (op: (typeof ops)[number]) {
                  ...
                  // chipmate_change start - never report success for a file that Git could not restore
                  if (tree.code !== 0) {
          ...
          const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // chipmate_change start - use git patches to avoid blocking the event loop on large diffs
          ...
          yield* materialize() // chipmate_change - resume interrupted snapshot object materialization
          ...
          // chipmate_change start - authoritative full-content detail for editor diff tabs
          ...
          return { cleanup, track, patch, restore, revert, diff, diffFull, diffFile } // chipmate_change - diffFile
        }),
      )

      // chipmate_change start - service-local state and cache avoid leaking across Snapshot layer instances
      const trackState = ChipMateSnapshotTrack.makeStates()
      ...
      return Service.of({
        ...
        // chipmate_change start - isolate turn-facing snapshot work from poisoned locks
        track: Effect.fn("Snapshot.track")(function* (opts) {
          const ctx = yield* InstanceState.context
          const guard = trackState(ctx.worktree)
          return yield* ChipMateSnapshotTrack.protect({
            inner: ChipMateSnapshotTrack.wrap({
        ...
        diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
          // chipmate_change start - cache full diffs at the service boundary
          if (from === to) return []
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [FSUtil.node, AppProcess.node, Config.node, EffectFlock.node], // chipmate_change
})
