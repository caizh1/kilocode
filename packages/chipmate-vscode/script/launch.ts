#!/usr/bin/env bun
/**
 * Build the ChipMate VS Code extension and launch it in a development host.
 *
 * Usage:
...
const base = join(tmpdir(), `chipmate-vscode-dev-${hash}`)
...
function cleanEnv(input: NodeJS.ProcessEnv) {
  ...
  for (const key of ["XDG_DATA_HOME", "XDG_CACHE_HOME", "XDG_CONFIG_HOME", "XDG_STATE_HOME", "CHIPMATE_TEST_HOME"]) {
    const value = env[key]
...
async function launch() {
  ...
  if (mode === "dev") {
    ...
    args.push("--disable-extension=chipmate.chipmate-code")
  }
