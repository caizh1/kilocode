...
import ai.chipmate.client.testing.fire
...
import ai.chipmate.client.settings.base.settingsListCellBounds
import ai.chipmate.client.testing.FakeAgentBehaviorRpcApi
...
class AgentsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test changing default agent saves patch`() {
        ...
        assertEquals("ask", appRpc.configPatches.single().values[ChipMateCliParser.CONFIG_DEFAULT_AGENT])
    ...
    fun `test apply waits for backend ready before refetching`() {
        ...
        flushUntil { rows(panel).size == 6 }
        appRpc.afterConfig = {
            app._state.value = app._state.value.copy(status = ChipMateAppStatusDto.LOADING)
        ...
        assertTrue(edt { text(panel).contains(ChipMateBundle.message("settings.agentBehavior.saving")) })

        app._state.value = app._state.value.copy(status = ChipMateAppStatusDto.READY)
    ...
    fun `test importing an agent stages patch until apply`() {
        ...
        edt {
            ...
            assertEquals(ChipMateCliParser.MODE_SUBAGENT, patch.mode)
    ...
    fun `test importing invalid JSON shows settings error message`() {
        ...
        val message = ChipMateBundle.message("settings.agentBehavior.agents.import.invalidJson")
    ...
    fun `test apply failure reloads and keeps remaining staged changes`() {
        ...
        flushUntil { text(panel).contains(ChipMateBundle.message("settings.agentBehavior.agents.create.failed")) }
    ...
    private fun install() {
        ...
        agentRpc = FakeAgentBehaviorRpcApi().apply {
            agents = listOf(
                AgentDetailDto("ask", displayName = "Ask", description = "Ask questions", mode = ChipMateCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("code", displayName = "Code", description = "Code things", mode = ChipMateCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("generated", description = "Generated", mode = ChipMateCliParser.MODE_SUBAGENT, native = false, removable = false),
                AgentDetailDto("hidden", description = "Hidden custom", mode = ChipMateCliParser.MODE_PRIMARY, native = false, removable = true, hidden = true),
                AgentDetailDto("old", description = "Old custom", mode = ChipMateCliParser.MODE_PRIMARY, native = false, removable = true, deprecated = true),
                AgentDetailDto("worker", description = "Worker", mode = ChipMateCliParser.MODE_SUBAGENT, native = false, removable = true),
            )
        }
        val workspaceRpc = FakeWorkspaceRpcApi().apply { models = ModelsWorkspaceDto(providers()) }
        app = ChipMateAppService(cs, appRpc)
        val ready = ChipMateAppStateDto(
        ...
        ApplicationManager.getApplication().replaceService(ChipMateAppService::class.java, app, testRootDisposable)
        ApplicationManager.getApplication().replaceService(ChipMateAgentBehaviorService::class.java, ChipMateAgentBehaviorService(cs, agentRpc), testRootDisposable)
        ApplicationManager.getApplication().replaceService(ChipMateWorkspaceService::class.java, ChipMateWorkspaceService(cs, workspaceRpc), testRootDisposable)
    }

    private fun providers() = ProvidersDto(
        providers = listOf(ProviderDto("chipmate", "ChipMate", models = mapOf("gpt-5" to ModelDto("gpt-5", "GPT-5")))),
