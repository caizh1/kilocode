...
const entry = "file:///tmp/chipmate-cache/node_modules/@lancedb/lancedb/dist/index.js"
const add = mock(
  async (): Promise<{ directory: string; entrypoint: string | undefined }> => ({
    directory: "/tmp/chipmate-cache",
    entrypoint: entry,
  }),
)
...
const env = "CHIPMATE_LANCEDB_PATH"
const prev = process.env[env]

describe("LanceDBRuntime", () => {
  beforeEach(async () => {
    const { LanceDBRuntime } = await import("../../src/chipmate/lancedb")
    LanceDBRuntime.clear()
    add.mockClear()
    add.mockImplementation(async () => ({ directory: "/tmp/chipmate-cache", entrypoint: entry }))
  })
  ...
  test("fails fast when the installed package has no import entrypoint", async () => {
    const { LanceDBRuntime } = await import("../../src/chipmate/lancedb")
    add.mockImplementationOnce(async () => ({ directory: "/tmp/chipmate-cache", entrypoint: undefined }))
  ...
  test("retries after a failed install", async () => {
    ...
    add
      ...
      .mockImplementationOnce(async () => ({ directory: "/tmp/chipmate-cache", entrypoint: entry }))
