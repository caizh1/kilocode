...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "chipmate-memory-effect-"))
  return {
...
describe("MemoryCapture (fake ports)", () => {
  test("turn-close typed LLM saves environment memory", async () => {
    const t = await tmp()
    try {
      await ChipMateMemory.enable({ root: t.root })
      await ChipMateMemory.configure({ root: t.root, settings: { autoConsolidate: true } })
      ...
      const shown = await ChipMateMemory.show({ root: t.root })
  ...
  test("auto-consolidate off skips digest and typed model writes", async () => {
    ...
    try {
      await ChipMateMemory.enable({ root: t.root })
      await ChipMateMemory.configure({ root: t.root, settings: { autoConsolidate: false } })
...
describe("MemoryTimers signal ref-counting", () => {
  test("shares one controller per root and drops it once the last capture releases", () => {
    const root = "/chipmate-memory/ref-count-root"
