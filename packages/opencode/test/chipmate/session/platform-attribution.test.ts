  ...
  test("child sessions inherit the root platform override", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const attr = ChipMateSession.attribution(child.id)

        expect(ChipMateSession.getPlatformOverride(root.id)).toBe("agent-manager")
        expect(ChipMateSession.getPlatformOverride(child.id)).toBe("agent-manager")
  ...
  test("child sessions expose parent and root lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(ChipMateSession.resolveParent(root.id)).toBeUndefined()
        expect(ChipMateSession.resolveParent(child.id)).toBe(root.id)
        expect(ChipMateSession.resolveParent(leaf.id)).toBe(child.id)
        expect(ChipMateSession.resolveRoot(leaf.id)).toBe(root.id)
  ...
  test("turn close events include persisted parent lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(ChipMateSession.resolveParent(child.id)).toBeUndefined()
