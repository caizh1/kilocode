...
import { completeChipMateLogin, loadChipMateProfile, startChipMateLogin, type ProjectQuery } from "../../client"
  ...
  function start(input: ProjectQuery | undefined = server.query()) {
    ...
    void startChipMateLogin(input)
      .then((info) => {
        ...
        return completeChipMateLogin(input, ctl.signal).then(() => true)
      })
      .then((ok) => {
        if (!ok || attempt() !== rev) return
        setAuth({ status: "success" })
        void loadChipMateProfile(input)
    ...
    <section class="route-empty">
      <div class="profile-login-page">
        <header class="profile-login-header">
          <p class="eyebrow">ChipMate Login</p>
          <h1>Login to ChipMate</h1>
          <p>Authorize this ChipMate Console through the same device auth flow used by the editor clients.</p>
        </header>
        ...
        <Show when={!server.query() && !server.discoverable()}>
          <Card class="profile-login-card" variant="warning">
            ...
            <p>Start a local ChipMate server or pass a server URL with ?server=.</p>
