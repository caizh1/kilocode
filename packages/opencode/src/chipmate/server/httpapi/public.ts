...
export function matchLegacyChipMateOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/chipmate/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/chipmate/profile")?.schema?.properties
  const pass = profile?.chipmatePass?.properties
  ...
  if (profile?.chipmatePass) profile.chipmatePass = nullable(profile.chipmatePass)
  if (profile?.currentOrgId) profile.currentOrgId = nullable(profile.currentOrgId)

  const sessions = json("/chipmate/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/chipmate/claw/status")?.schema?.properties
  ...
  const credentials = json("/chipmate/claw/chat-credentials")
