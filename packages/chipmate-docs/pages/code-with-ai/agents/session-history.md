...
description: "Find, search, inspect, and resume local ChipMate sessions"
---

# Session history and search

ChipMate keeps local session metadata and chat history so you can resume earlier work. You can search session titles in the history UI or CLI, ask the agent to search chat content, or inspect the local SQLite database directly.

{% callout type="info" %}
Local sessions use SQLite, not PostgreSQL. Use `chipmate db` or `sqlite3`, not `psql`, to inspect the local database.
{% /callout %}
...
| [`chipmate session list --search`](#filter-session-titles) | Session titles | Current workspace, or every local workspace with `--all` |
| [Ask ChipMate to recall past chats](#ask-chipmate-to-search-past-chats) | Titles and high-signal chat content | Current workspace and related Git worktrees |
...
History and CLI list searches do **not** search message content. To find a string inside prompts or replies, ask ChipMate to search past chats or use a direct SQLite query.
...
ChipMate loads older messages while the search is active so the search covers the full local chat session, not only the messages initially visible.
...
1. Open the ChipMate tool window.
...
ChipMate adds the selected chat history as context for your message. This does not switch the TUI to the selected session. Use `/sessions`, `/resume`, or `/continue` when you want to load a different session instead.

{% /tab %}
{% /tabs %}

## Ask ChipMate to search past chats

The simplest way to search message content across past local sessions is to ask ChipMate directly:

```text
Search my local sessions for "database disk image is malformed" and summarize the matching conversations.
```

ChipMate's local recall search covers session titles, user and assistant text, file references, and failed tool errors in the current workspace and related Git worktrees. Every query term must occur somewhere in a matching session; exact phrases and user-authored matches rank higher.

The search includes archived and child sessions. It excludes reasoning, synthetic or ignored text, successful tool output, file contents, and other metadata. ChipMate can then read the full chat history for a selected result.
...
```bash
chipmate session list --search "database migration"
```

Search titles across every local workspace and return JSON:

```bash
chipmate session list --all --search "database migration" --format json
```

Limit the number of results:

```bash
chipmate session list --all --search "migration" --max-count 25
```
...
```bash
chipmate --session ses_123
chipmate run --session ses_123 "Summarize where we stopped"
chipmate export ses_123 > session.json
```
...
The actual path can differ when `CHIPMATE_DB`, `XDG_DATA_HOME`, a development channel, or an isolated development environment is active. Prefer `chipmate db path` over assuming a path.

## Query SQLite directly

{% callout type="warning" %}
The `chipmate db` query command accepts arbitrary SQL, including writes. Use read-only queries, and use `sqlite3 -readonly` for safer interactive inspection. Stop ChipMate before copying, replacing, or restoring the database because active transactions may also use `chipmate.db-wal` and `chipmate.db-shm`.
{% /callout %}
...
Replace the search string in the second `lower(...)` expression. This query searches ordinary user and assistant text. It intentionally does not reproduce ChipMate's recall ranking, workspace scope, file-reference matching, or tool-error matching.
...
```bash
sqlite3 -readonly "$(chipmate db path)"
```
...
Run the SQL examples above without the surrounding `chipmate db` command. Exit with `.quit`.

`psql` cannot open this file because `psql` speaks the PostgreSQL client protocol, while local ChipMate sessions are stored in an embedded SQLite database.
...
```bash
export CHIPMATE_SERVER_PASSWORD='replace-with-a-strong-password'
chipmate serve --port 4096
```

Search session titles in the current directory:

```bash
curl -u "chipmate:$CHIPMATE_SERVER_PASSWORD" \
...
const auth = Buffer.from(`chipmate:${process.env.CHIPMATE_SERVER_PASSWORD}`).toString("base64")
...
Local sessions live in the SQLite database on the machine where ChipMate runs. With VS Code Remote SSH, that means the remote host. Cloud sessions and shared-session copies are separate and are not available through local SQLite queries unless you import them locally.
