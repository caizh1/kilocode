...
/**
 * OkHttp implementation of [LegacyMigrationBackend] against `chipmate serve`.
 *
 ...
 * constraints getting in the way. Mirrors the pattern in ChipMateBackendChatManager.
 *
...
class LegacyMigrationHttpBackend(
    ...
    override fun importProject(project: JsonObject): String {
        ...
        val url = "$base/chipmate/session-import/project?directory=${encode(dir)}"
    ...
    override fun importSession(session: JsonObject): LegacyImportResult {
        ...
        val url = "$base/chipmate/session-import/session?directory=${encode(dir)}"
    ...
    override fun importMessage(message: JsonObject) {
        ...
        val url = "$base/chipmate/session-import/message?sessionID=${encode(sessionId)}"
    ...
    override fun importPart(part: JsonObject) {
        ...
        val url = "$base/chipmate/session-import/part?sessionID=${encode(sessionId)}"
