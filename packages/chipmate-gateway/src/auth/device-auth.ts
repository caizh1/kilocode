...
import { getChipMateProfile, getChipMateDefaultModel, promptOrganizationSelection } from "../api/profile.js"
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getChipMateProfile(token)
  ...
  const model = await getChipMateDefaultModel(token, organizationId)
