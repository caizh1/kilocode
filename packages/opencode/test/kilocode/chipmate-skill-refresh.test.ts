...
describe("ChipMate Skill refresh", () => {
      ...
      for (const item of [
        ...
        const child = Bun.spawn([process.execPath, "-e", code], {
          ...
          env: {
            ...
            KILO_STORAGE_ROOT: state,
            KILO_VSCODE_GLOBAL_STORAGE: storage,
            KILO_DISABLE_PROJECT_CONFIG: item.disabled,
          },
          stdout: "pipe",
          stderr: "pipe",
        })
  ...
  test("uses only fixed roots, validates identity, and prefers the project instance", async () => {
    ...
    const code = [
      ...
      "const effect = provideTmpdirInstance((dir) => Effect.gen(function* () {",
      '  yield* Effect.promise(() => Promise.all([write(path.join(dir, ".chipmate-v2", "skills", "shared", "SKILL.md"), "shared", "Project v1"), write(path.join(dir, ".agents", "skills", "agents-old", "SKILL.md"), "agents-old", "Ignored agents"), write(path.join(dir, ".kilo", "skills", "kilo-old", "SKILL.md"), "kilo-old", "Ignored kilo"), write(path.join(dir, ".kilocode", "skills", "kilocode-old", "SKILL.md"), "kilocode-old", "Ignored kilocode"), write(path.join(dir, ".chipmate-v2", "skills", "bad-name", "SKILL.md"), "Bad Name", "Invalid name"), write(path.join(dir, ".chipmate-v2", "skills", "wrong-folder", "SKILL.md"), "other", "Mismatch"), write(path.join(dir, ".chipmate-v2", "skills", "bad-metadata", "SKILL.md"), "bad-metadata", "Mismatch metadata", { id: "other" })]))',
