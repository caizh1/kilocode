...
export namespace ChipMateRoutedModel {
  const ns = "chipmate"
  ...
  export function readAuto(
    ...
    if (input.providerID !== ProviderV2.ID.chipmate) return undefined
    if (!input.modelID.startsWith("chipmate-auto/") && !input.modelID.startsWith("openrouter/") && !input.modelID.includes("fable")) return undefined
