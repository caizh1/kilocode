// chipmate_change - extracted state machine that separates presence-owned
...
export namespace AttachedState {
  ...
  // chipmate_change - collision-safe union key. The historical "|" join
  ...
  export function create(options: Options): Interface {
    ...
    // chipmate_change - K1 W1: tombstones for ids that have been remotely
    ...
    // chipmate_change - in-flight dedup. Concurrent announce(id) callers
    ...
    // chipmate_change - in-flight detach dedup, mirrors `inflight` for the
    ...
    // chipmate_change end
    let lastSentKey = ""
    // chipmate_change - lifecycle generation. Incremented on reset() so a
    ...
    return {
      setPresence(ids) {
        ...
        // chipmate_change - K1 W1: suppression tombstone. Any id in `next`
      ...
      async announce(id) {
        ...
        // chipmate_change - join a same-kind in-flight announce so concurrent
        ...
        // chipmate_change - K1 W1: if a detach is in flight for this id, we
        ...
        const owned = (async () => {
          ...
          try {
            ...
            // chipmate_change - K1 W1: if reset() ran while this heartbeat was
      ...
      // chipmate_change - K1 W1: session-detach semantics.
      async detach(id) {
        // chipmate_change - join a same-kind in-flight detach so concurrent
        ...
        // chipmate_change - K1 W1: if an announce is in flight for this id we
      ...
      reset() {
        ...
        pending.clear()
        inflight.clear()
        // chipmate_change - K1 W1: also drop the in-flight detach map and
        ...
        suppressed.clear()
        lastSentKey = ""
        // chipmate_change - bump the lifecycle generation so any in-flight
