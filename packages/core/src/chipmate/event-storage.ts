// chipmate_change start - released readers persist tool content in the pre-1.17.13 shapes
//
// ChipMate has shipped durable rows whose `content` uses `{type:"media"}` or `{type:"file",source}`.
