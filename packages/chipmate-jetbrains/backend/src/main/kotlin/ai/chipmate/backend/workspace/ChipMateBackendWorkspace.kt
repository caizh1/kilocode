 ...
 * [ChipMateBackendWorkspaceManager] for each directory. Receives a
 * pre-connected [DefaultApi] — no null checks needed.
 *
 * Session operations delegate to [ChipMateBackendSessionManager] with
 * this workspace's [directory], so the frontend only needs one
 * object per directory.
 */
class ChipMateBackendWorkspace(
    ...
    private val _state = MutableStateFlow<ChipMateWorkspaceState>(ChipMateWorkspaceState.Pending)
    val state: StateFlow<ChipMateWorkspaceState> = _state.asStateFlow()
    ...
    fun load() {
        synchronized(loadLock) {
            ...
            val progress = AtomicReference(ChipMateWorkspaceLoadProgress())
            _state.value = ChipMateWorkspaceState.Loading(progress.get())
            ...
            try {
                    ...
                    launch {
                        ...
                        if (result.value != null) {
                            ...
                            progress.updateAndGet { it.copy(providers = true) }
                                .also { _state.value = ChipMateWorkspaceState.Loading(it) }
                        } else {
                ...
                _state.value = ChipMateWorkspaceState.Ready(
                    providers = prov!!,
                    agents = ag!!,
                    commands = cmd!!,
                    skills = sk!!,
    ...
    fun stop() {
        ...
        _state.value = ChipMateWorkspaceState.Pending
    }
    ...
    private suspend fun fetchProviders(): FetchResult<ProviderData> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(ChipMateCliDataParser.parseProviders(fetch("/provider?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchCommands(): FetchResult<List<CommandInfo>> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(ChipMateCliDataParser.parseCommands(fetch("/command?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private fun setWorkspaceError(message: String, errors: List<LoadError>) {
        ...
        _state.value = ChipMateWorkspaceState.Error(message, errors)
    }
