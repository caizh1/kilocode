...
val generatedProps = layout.buildDirectory.dir("generated/chipmate-props")
val generatedCli = layout.buildDirectory.dir("generated/chipmate-cli-res")
val generatedChecksums = layout.buildDirectory.dir("generated/chipmate-cli-checksums")
val pinned = providers.gradleProperty("chipmate.cli.pinned").map { it.trim().toBoolean() }.orElse(true)
val repoCli = pinned.map { !it }
val bundled = providers.gradleProperty("chipmate.cli.bundled").map { it.trim().toBoolean() }.orElse(false)
...
val local = rootProject.layout.projectDirectory.file(".gradle/chipmate-cli-pin.properties")
...
if (repoCli.get() && bundled.get()) {
    error("chipmate.cli.bundled=true requires chipmate.cli.pinned=true; do not combine release CLI bundling with local repo CLI mode.")
}

val writeChipMateProperties by tasks.registering(WriteProperties::class) {
    description = "Write pinned ChipMate CLI properties"
    val out = generatedProps.map { it.file("chipmate.properties") }
...
val stageRepoCli by tasks.registering(StageRepoCliTask::class) {
    ...
    val bin = repoRootDir.dir("dist/@chipmate/cli-${platform()}/bin")
    this.bin.set(bin)
    archive.set(generatedCli.map { it.file("chipmate-cli.zip") })
    outputs.upToDateWhen { false }
}

val stageBundledCli by tasks.registering(StageBundledCliTask::class) {
    description = "Stage all pinned ChipMate CLI release assets into backend resources"
    cliVersion.set(pinnedCliVersion)
    token.set(
        providers.environmentVariable("GH_TOKEN")
            .orElse(providers.environmentVariable("GITHUB_TOKEN"))
...
val writeCliChecksums by tasks.registering(WriteCliChecksumsTask::class) {
    description = "Write pinned ChipMate CLI checksums"
    cliVersion.set(pinnedCliVersion)
    token.set(
        providers.environmentVariable("GH_TOKEN")
            .orElse(providers.environmentVariable("GITHUB_TOKEN"))
    )
    checksums.set(generatedChecksums.map { it.file("chipmate-cli-checksums.properties") })
}
...
openApiGenerate {
    ...
    packageName.set("ai.chipmate.jetbrains.api")
    apiPackage.set("ai.chipmate.jetbrains.api.client")
    modelPackage.set("ai.chipmate.jetbrains.api.model")
...
tasks.named("compileKotlin") {
    dependsOn(fixGeneratedApi, writeChipMateProperties)
...
tasks.named("processResources") {
    dependsOn(writeChipMateProperties)
    if (downloadsCli.get()) dependsOn(writeCliChecksums)
    if (repoCli.get()) dependsOn(stageRepoCli)
    if (bundled.get()) dependsOn(stageBundledCli)
}
