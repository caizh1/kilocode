 ...
 *
 * **EDT-only access** — no synchronization. [ai.chipmate.client.session.controller.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: ChipMateAppStateDto = ChipMateAppStateDto(ChipMateAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: ChipMateWorkspaceStateDto = ChipMateWorkspaceStateDto(ChipMateWorkspaceStatusDto.PENDING)
    ...
    @RequiresEdt
    fun isReady(): Boolean = app.status == ChipMateAppStatusDto.READY && workspace.status == ChipMateWorkspaceStatusDto.READY
...
private fun ai.chipmate.rpc.dto.PartTimeDto.durationMs(): Long? {
