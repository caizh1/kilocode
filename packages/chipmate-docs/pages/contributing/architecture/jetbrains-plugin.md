...
description: "Architecture of the ChipMate JetBrains split-mode plugin"
---

# JetBrains Plugin Architecture

The JetBrains plugin (`packages/chipmate-jetbrains/`) is a split-mode Swing client of [ChipMate CLI runtime](/docs/contributing/architecture/cli-runtime). Frontend module renders IDE UI. Backend module owns project-local logic and one bundled `chipmate serve` server. Shared module defines cross-process RPC contracts and serializable payloads.
...
flowchart LR
  ...
  subgraph backend ["JetBrains backend"]
    ...
    conn["ChipMateConnectionService"]
    workspaces["Directory workspace cache"]
    cli["Extracted chipmate serve --port 0"]
  end
...
|---|---|---|
| `ChipMateAppRpcApi` | Application | Connect, state flow, health, retry, restart, reinstall, model state, profile, login, telemetry |
| `ChipMateWorkspaceRpcApi` | Directory | Resolve real backend project directory, workspace state flow, reload, file lookup, open file |
| `ChipMateSessionRpcApi` | Session and directory | Create/list sessions, prompt, stream events, permission and question replies, config update |
| `ChipMateMigrationRpcApi` | Legacy migration | Detect, run, and observe migration state |
...
```text
<PathManager.getSystemPath()>/chipmate/bin/chipmate
<PathManager.getSystemPath()>/chipmate/bin/chipmate.exe   # Windows
```

It chooses platform resource by OS and CPU architecture, reuses extracted binary when resource size matches, and can force re-extraction during reinstall flow. This editor-owned child is separate from detached local daemon managed by `chipmate daemon`.

| Area | Behavior |
|---|---|
| Spawn | Runs extracted binary as `chipmate serve --port 0` |
| Port | CLI server prefers `4096`, then asks OS for free port; backend reads listening line from stdout |
| Authentication | Generates random 32-byte hex password and passes `CHIPMATE_SERVER_PASSWORD`; username defaults to `chipmate` |
...
This mirrors CLI `InstanceStore`: directory remains isolation key while one editor-owned `chipmate serve` process serves multiple workspace contexts.
...
| Check | Command from `packages/chipmate-jetbrains/` |
|---|---|
...
For plugin icon creation, modification, or review, use the `icon-jetbrains` skill at `.chipmate/skills/icon-jetbrains/SKILL.md` together with `packages/chipmate-jetbrains/AGENTS.md`. The skill covers IntelliJ New UI SVG roles, sizing, palette, dark variants, and icon validation guidance.
...
| Concern | Source path |
|---|---|
| Split modules | `packages/chipmate-jetbrains/settings.gradle.kts` and module XML descriptors |
| Contributor constraints | `packages/chipmate-jetbrains/AGENTS.md` |
| CLI lifecycle | `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/cli/ChipMateBackendCliManager.kt` |
| Connection recovery | `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/app/ChipMateBackendConnectionService.kt` |
| Workspace cache | `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/workspace/ChipMateBackendWorkspaceManager.kt` |
| Kotlin client generation | `packages/chipmate-jetbrains/backend/build.gradle.kts` |
| RPC contracts | `packages/chipmate-jetbrains/shared/src/main/kotlin/ai/chipmate/rpc/` |
