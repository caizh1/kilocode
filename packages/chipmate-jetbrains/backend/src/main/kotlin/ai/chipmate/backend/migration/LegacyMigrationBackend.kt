...
/**
 * Backend adapter for writing migrated data to the CLI (chipmate serve).
 *
...
interface LegacyMigrationBackend {
    ...
    /** POST /chipmate/session-import/project — returns the project ID */
    fun importProject(project: JsonObject): String

    /** POST /chipmate/session-import/session */
    fun importSession(session: JsonObject): LegacyImportResult

    /** POST /chipmate/session-import/message */
    fun importMessage(message: JsonObject)

    /** POST /chipmate/session-import/part */
