...
export interface MercuryEditProviderOptions {
  ...
  /** Provider id to send to the gateway (e.g. `"chipmate"` or `"inception"`). */
  providerId?: string
...
/**
 * Thin wrapper around the SDK's `client.chipmate.edit(...)` endpoint (non-streaming).
 * The gateway (`packages/chipmate-gateway/src/server/edit.ts`) handles auth, routing
...
export class MercuryEditProvider {
  ...
  async suggest(ctx: MercuryEditRequestContext): Promise<MercuryEditSuggestion | null> {
    ...
    nesLog(
      `-> /chipmate/edit provider=${provider} model=${model} region=[${ctx.editableRegionStartLine},${ctx.editableRegionEndLine}] diffs=${ctx.editDiffHistory.length} snippets=${ctx.recentlyViewedSnippets.length}`,
    )
    ...
    try {
      ...
      const { data, error, response } = await client.chipmate.edit(
        {
          provider,
          model,
