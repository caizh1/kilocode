...
const CLOUD_SESSION_FILE = path.join(ROOT, "src/chipmate-provider/handlers/cloud-session.ts")
...
describe("missing model send guards", () => {
  ...
  it("rejects missing models before importing a cloud session", () => {
    ...
    const request = cloud.indexOf("ctx.client.chipmate.cloud.session.import", start)
    expect(guard).toBeGreaterThan(start)
...
describe("ChipMateProvider pruneDeletedSession contract", () => {
...
describe("ChipMateConnectionService pruneSession contract", () => {
  const source = readFile(CONNECTION_SERVICE_FILE)

  it("drops the deleted session from attached and visible Maps", () => {
    // ChipMateProvider's pruneDeletedSession calls connectionService.pruneSession.
