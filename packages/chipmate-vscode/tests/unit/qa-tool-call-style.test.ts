...
const tool = await Bun.file(new URL("../../../chipmate-ui/src/components/basic-tool.tsx", import.meta.url)).text()
const base = await Bun.file(new URL("../../../chipmate-ui/src/components/basic-tool.css", import.meta.url)).text()
const en = await Bun.file(new URL("../../../chipmate-i18n/src/en.ts", import.meta.url)).text()
const zh = await Bun.file(new URL("../../../chipmate-i18n/src/zh.ts", import.meta.url)).text()
const zht = await Bun.file(new URL("../../../chipmate-i18n/src/zht.ts", import.meta.url)).text()
...
describe("QA 工具调用框", () => {
  ...
  it("强调线位于所有标准工具图标之前并保持既有文字坐标", () => {
    ...
    expect(css).toContain("font-size: var(--chipmate-font-size-11)")
