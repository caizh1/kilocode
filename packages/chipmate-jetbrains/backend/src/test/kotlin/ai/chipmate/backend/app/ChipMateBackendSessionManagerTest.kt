...
class ChipMateBackendSessionManagerTest {
    ...
    private fun setup(): ChipMateBackendAppService {
        return ChipMateBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }
    }

    private suspend fun ready(app: ChipMateBackendAppService) {
        app.connect()
        withTimeout(10_000) {
    ...
    fun `cloudSessions passes filters and cursor`() = runBlocking {
        ...
        app.sessions.cloudSessions("/repo path", "cur 1", 25, "git@github.com:ChipMate-Org/chipmate.git")

        val path = mock.lastCloudSessionsPath ?: error("missing cloud sessions request")
        assertTrue(path.startsWith("/chipmate/cloud-sessions?"))
        ...
        assertTrue(decoded.contains("gitUrl=git@github.com:ChipMate-Org/chipmate.git"), path)
    }
    ...
    fun `importCloudSession posts cloud id and returns local session`() = runBlocking {
        ...
        assertTrue(path.startsWith("/chipmate/cloud/session/import?"))
        assertTrue(URLDecoder.decode(path, "UTF-8").contains("directory=/repo path"), path)
