...
| `prefilter.ts` / `QWEN_DOCUMENT_SELECTOR` | Limit qwen provider to file-backed C/C++/headers. | Upstream `shouldPrefilter`, language config, and disabled-language options. | C | No | Partial | Hardcoded C/C++ selector and local non-code exclusions. Continue has broader language metadata; qwen-direct intentionally remains provider-scoped to C/C++/headers. | Company qwen C/C++ endpoint target. | Medium | Adapter acceptable, document tests | 5C |
...
| qwen provider-scope language support | qwen-direct targets the internal C/C++ qwen-coder workflow. | Continue has broader `AutocompleteLanguageInfo`; qwen only registers/supports C/C++ and headers. | Medium risk that non-C/C++ autocomplete parity remains incomplete. | Adapter acceptable, document tests | 5C |
