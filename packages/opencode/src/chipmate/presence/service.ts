...
import { EventServiceClient } from "@/chipmate/event-service/client"
import { CHIPMATE_EVENT_SERVICE_URL } from "@chipmate/chipmate-gateway"
...
const log = Log.create({ service: "chipmate-viewers" })

function inferPlatform(): Platform | undefined {
  const p = process.env.CHIPMATE_PLATFORM
...
function extract(auth: Auth.Info | undefined): { token: string | undefined; identity: string | undefined } {
  const envKey = process.env.CHIPMATE_API_KEY?.trim()
...
export namespace ChipMateViewers {
  export interface Interface {
    readonly update: (snapshot: ViewerSnapshot) => Effect.Effect<void>
    readonly invalidateAuth: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@chipmate/ChipMateViewers") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const auth = yield* Auth.Service
      const ChipMateSessions = (yield* Effect.promise(() => import("@/chipmate-sessions/chipmate-sessions"))).ChipMateSessions

      const platform = inferPlatform()
      const killSwitch = process.env.CHIPMATE_DISABLE_PRESENCE === "1"
      // Same endpoint the server envelope hands ChipMateClaw; CHIPMATE_EVENT_SERVICE_URL
      // is a presence-specific override on top of the gateway's EVENT_SERVICE_URL.
      const url = process.env.CHIPMATE_EVENT_SERVICE_URL || CHIPMATE_EVENT_SERVICE_URL
      ...
      function pushAttached() {
        ...
        if (!sameArr(union, s.prevAttached)) {
          ...
          ChipMateSessions.setAttachedSessions(union)
        }
      }
      ...
      const readAuth = auth.get("chipmate").pipe(Effect.orElseSucceed((): Auth.Info | undefined => undefined))

      const update = Effect.fn("ChipMateViewers.update")(function* (snapshot: ViewerSnapshot) {
      ...
      const invalidateAuth = Effect.fn("ChipMateViewers.invalidateAuth")(function* () {
        disconnectClient()
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          ...
          disconnectClient()
          s.viewers.clear()
          ChipMateSessions.setAttachedSessions([])
        }),
      )
