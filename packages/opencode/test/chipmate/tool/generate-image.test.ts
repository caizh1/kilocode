...
import {
  ...
  IMAGE_MODELS,
  DEFAULT_MODEL,
} from "../../../src/chipmate/tool/generate-image"
...
describe("generate-image provider resolver", () => {
  test("uses ChipMate cloud when ChipMate auth is present", () => {
    const result = resolveProvider({ type: "oauth", access: "chipmate-token", accountId: "org-123" }, undefined)
    expect(result).not.toBeNull()
    expect(result!.token).toBe("chipmate-token")
    expect(result!.organizationId).toBe("org-123")
    expect(result!.provider).toBe("chipmate")
    expect(result!.url).toContain("openrouter")
  })

  test("uses ChipMate cloud with API key auth", () => {
    const result = resolveProvider({ type: "api", key: "chipmate-api-key" }, undefined)
    expect(result!.token).toBe("chipmate-api-key")
    expect(result!.provider).toBe("chipmate")
  })

  test("falls back to OpenRouter with BYO key when no ChipMate auth", () => {
  ...
  test("prefers ChipMate auth over OpenRouter key", () => {
    const result = resolveProvider({ type: "oauth", access: "chipmate-token" }, "or-key")
    expect(result!.provider).toBe("chipmate")
    expect(result!.token).toBe("chipmate-token")
  })
})
