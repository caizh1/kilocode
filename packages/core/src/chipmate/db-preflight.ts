...
export function assertWritable(filename: string, trusted: string = Global.Path.data) {
  ...
  for (const file of [filename, `${filename}-wal`, `${filename}-shm`]) {
    ...
    if (owned) {
      ...
      if (writable(file)) {
        // visible trail: if files keep losing their write bit, something outside chipmate is doing it
        log.warn("repaired read-only database file", { file })
        continue
      }
    }
    throw new Error(
      `Database file is not writable: ${file}. Fix its permissions (chmod u+w "${file}") or point CHIPMATE_DB at a writable location.`,
      cause === undefined ? undefined : { cause },
    )
  }
  if (missing && !writable(dir)) {
    if (!exists(dir))
      throw new Error(`Database directory does not exist: ${dir}. Create it or point CHIPMATE_DB at an existing location.`)
    throw new Error(
      `Database directory is not writable: ${dir}. SQLite must create WAL files next to the database. Fix its permissions or point CHIPMATE_DB at a writable location.`,
    )
  }
}
