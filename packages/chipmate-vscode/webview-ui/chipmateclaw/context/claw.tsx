...
export function ClawProvider(props: { children: JSX.Element }) {
  ...
  const applyState = (s: ChipMateClawState) => {
    setPhase(s.phase)
    setLocale(s.locale)
    if (s.phase === "ready") {
      setStatus(s.status)
  ...
  const handleConversationMessage = (msg: ChipMateClawOutMessage, active: string | null): boolean => {
    switch (msg.type) {
      case "chipmateclaw.messages":
        if (msg.conversationId === active) {
          setMessages(msg.messages)
          setHasMoreMessages(msg.hasMore)
        }
        return true
      case "chipmateclaw.messageOptimistic":
        if (msg.conversationId === active) {
          setMessages((prev) => (prev.some((m) => m.id === msg.message.id) ? prev : [...prev, msg.message]))
        }
        return true
      case "chipmateclaw.messageReplaced":
        if (msg.conversationId === active) {
  ...
  const handler = (event: MessageEvent) => {
    const msg = event.data as ChipMateClawOutMessage
    if (msg?.type === "fontSizeChanged") {
      applyFontSize(msg.fontSize)
      return
    }
    if (!msg?.type?.startsWith("chipmateclaw.")) return
    const active = activeConversationId()
    if (handleConversationMessage(msg, active)) return

    switch (msg.type) {
      case "chipmateclaw.state":
        applyState(msg.state)
        return
      case "chipmateclaw.status":
        setStatus(msg.data)
        return
      case "chipmateclaw.locale":
        setLocale(msg.locale)
        return
      case "chipmateclaw.error":
        showToast({ title: msg.error, variant: "error", duration: 5000 })
        return
      case "chipmateclaw.conversations":
        setConversations(msg.conversations)
        setHasMoreConversations(msg.hasMore)
        return
      case "chipmateclaw.activeConversation":
        setActiveConversationId(msg.conversationId)
        setTypingMembers([])
        return
      case "chipmateclaw.botStatus":
        setBotStatus(msg.status)
        return
      case "chipmateclaw.conversationStatus":
  ...
  onMount(() => {
    vscode.postMessage({ type: "chipmateclaw.ready" })
  })
  ...
  const ctx: ClawCtx = {
    ...
    locale,
    error,
    retry: () => vscode.postMessage({ type: "chipmateclaw.ready" }),
    ...
    selectConversation: (conversationId) => vscode.postMessage({ type: "chipmateclaw.selectConversation", conversationId }),
    createConversation: (title) => vscode.postMessage({ type: "chipmateclaw.createConversation", title }),
    renameConversation: (conversationId, title) =>
      vscode.postMessage({ type: "chipmateclaw.renameConversation", conversationId, title }),
    leaveConversation: (conversationId) => vscode.postMessage({ type: "chipmateclaw.leaveConversation", conversationId }),
    loadMoreConversations: () => vscode.postMessage({ type: "chipmateclaw.loadMoreConversations" }),
    ...
    editMessage: (conversationId, messageId, content) =>
      vscode.postMessage({ type: "chipmateclaw.editMessage", conversationId, messageId, content }),
    deleteMessage: (conversationId, messageId) =>
      vscode.postMessage({ type: "chipmateclaw.deleteMessage", conversationId, messageId }),
    loadMoreMessages: (conversationId, before) =>
      vscode.postMessage({ type: "chipmateclaw.loadMoreMessages", conversationId, before }),

    addReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "chipmateclaw.addReaction", conversationId, messageId, emoji }),
    removeReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "chipmateclaw.removeReaction", conversationId, messageId, emoji }),

    executeAction: (conversationId, messageId, groupId, value) =>
      vscode.postMessage({ type: "chipmateclaw.executeAction", conversationId, messageId, groupId, value }),

    sendTyping: (conversationId) => vscode.postMessage({ type: "chipmateclaw.sendTyping", conversationId }),
    sendTypingStop: (conversationId) => vscode.postMessage({ type: "chipmateclaw.sendTypingStop", conversationId }),
    ...
    botStatus,
    conversationStatus,
    markConversationRead: (conversationId) => vscode.postMessage({ type: "chipmateclaw.markRead", conversationId }),

    openExternal: (url) => vscode.postMessage({ type: "chipmateclaw.openExternal", url }),
  }
