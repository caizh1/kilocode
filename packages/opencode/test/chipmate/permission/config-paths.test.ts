...
import { ConfigProtection } from "../../../src/chipmate/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = ChipMatePaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "chipmate.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting relative config path via patterns", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".chipmate/command/foo.md"],
    })
    expect(result).toBe(true)
  })
  ...
  test("returns false for edit targeting excluded subdir (plans)", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".chipmate/plans/plan.md"],
    })
    expect(result).toBe(false)
  })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["chipmate.json", "chipmate.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
  ...
  test("protects package lock files in project config directories", () => {
    for (const file of [".chipmate/package-lock.json", ".chipmate/package-lock.json"]) {
  ...
  test("protects a combined source and config lockfile edit", () => {
    expect(
      ConfigProtection.isRequest({
        permission: "edit",
        patterns: ["src/app/layout.tsx", ".chipmate/package-lock.json", ".chipmate/package-lock.json"],
        metadata: {
          filepath: "src/app/layout.tsx, .chipmate/package-lock.json, .chipmate/package-lock.json",
        },
      }),
    ).toBe(true)
  })
...
describe("ConfigProtection.isGlobalSkillRequest", () => {
  const roots = [Global.Path.config, ...ChipMatePaths.globalDirs()]
  ...
  test("rejects glob characters in the canonical rule", async () => {
    ...
    const skill = path.join(root, "chipmate", "skills", "unsafe-root")
