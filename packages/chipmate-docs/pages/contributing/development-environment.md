...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [ChipMate-Org/chipmate](https://github.com/ChipMate-Org/chipmate)** (extension at `packages/chipmate-vscode`, CLI at `packages/opencode`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
4. **Java 21** - Required only when running JetBrains plugin checks or repo-level checks that include `@chipmate/chipmate-jetbrains`
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [ChipMate GitHub repository](https://github.com/ChipMate-Org/chipmate)
     - Click the "Fork" button in the top-right corner to create your own copy.
   - **Clone Your Fork**:
     ```bash
     git clone https://github.com/[YOUR-USERNAME]/chipmate.git
     cd chipmate
     ```
...
- **`packages/opencode/`** - ChipMate CLI, agent runtime, local HTTP server, session management, and TUI
- **`packages/chipmate-vscode/`** - VS Code extension, webview UI, Agent Manager, and extension packaging
- **`packages/sdk/js/`** - Generated TypeScript SDK for the local server API
- **`packages/chipmate-docs/`** - Documentation site
- **`packages/chipmate-jetbrains/`** - JetBrains plugin
...
`bun dev` and `bun run dev` are equivalent. Both run the local source in `packages/opencode/`; they do not use a globally installed `chipmate` binary.

### Backend/API Validation

For backend and API validation, use the root [TESTING.md](https://github.com/ChipMate-Org/chipmate/blob/main/TESTING.md) guide. It covers starting the local backend with:
...
`extension:isolated` launches a separate VS Code process with user data and extensions under `.chipmate-dev/vscode/`, and points ChipMate at isolated XDG storage under `.chipmate-dev/{data,config,state,cache}`. Re-running it keeps the same isolated environment, so installed extensions, VS Code settings, ChipMate auth, sessions, config, state, and cache persist.

`extension:isolated:clean` deletes `.chipmate-dev/` before launch. Use it to simulate a fresh VS Code + ChipMate install without affecting your main VS Code profile or real ChipMate config.
...
When the Extension Development Host opens, check the ChipMate output channel and the Developer Tools console for startup or webview errors.
...
ChipMate uses several types of tests to ensure quality:
...
Use the root [TESTING.md](https://github.com/ChipMate-Org/chipmate/blob/main/TESTING.md) guide for backend/API checks that require `bun dev serve` and `curl`-based requests.
...
- When editing shared `packages/opencode/` files, keep changes small and mark ChipMate-only edits with `// chipmate_change` for a single line or `// chipmate_change start` / `// chipmate_change end` for a block. Do not add these markers inside `chipmate`-named paths.
...
- Check the Output panel in VSCode (View > Output) and select "ChipMate" from the dropdown
