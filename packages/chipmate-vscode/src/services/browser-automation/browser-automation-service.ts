...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "chipmate-playwright"

  constructor(private readonly connectionService: ChipMateConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("chipmate.v2.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[ChipMate New] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[ChipMate New] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[ChipMate New] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[ChipMate New] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private getClient(): ChipMateClient | null {
    try {
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[ChipMate New] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
  }
