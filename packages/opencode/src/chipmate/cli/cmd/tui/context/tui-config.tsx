 ...
 * changed from the ChipMate Console. Fetched config stays serializable and is resolved into a fresh
...
export namespace ChipMateTuiConfig {
  ...
  export function makeStore(initial: TuiConfig.Resolved) {
    ...
    const set: SetTuiConfig = (next) => {
      ...
      const config: TuiConfig.Resolved = {
        ...next,
        title_icon: next.title_icon ?? ChipMateTitleIcon.Default,
        attention: {
          ...
          sound_pack: next.attention?.sound_pack ?? "chipmate.default",
