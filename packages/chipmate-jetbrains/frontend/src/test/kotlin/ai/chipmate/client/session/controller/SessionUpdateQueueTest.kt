...
class SessionUpdateQueueTest : SessionControllerTestBase() {
    ...
    fun `test per token snapshot plus delta interleave does not double text or code`() {
        ...
        val text = (m.model.message("msg1")!!.parts["prt1"] as ai.chipmate.client.session.model.Text).content.toString()
    ...
    fun `test update hooks run on EDT around history and recovery`() {
        appRpc.state.value = ai.chipmate.rpc.dto.ChipMateAppStateDto(ai.chipmate.rpc.dto.ChipMateAppStatusDto.READY)
        projectRpc.state.value = workspaceReady()
        rpc.history.add(ai.chipmate.rpc.dto.MessageWithPartsDto(msg("msg1", "ses_test", "assistant"), emptyList()))
