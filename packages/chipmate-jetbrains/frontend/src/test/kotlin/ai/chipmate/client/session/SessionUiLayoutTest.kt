...
import ai.chipmate.client.session.ui.RevertProgress
import ai.chipmate.client.session.ui.SessionDropOverlay
import ai.chipmate.client.session.ui.SessionLayoutPanel
...
class SessionUiLayoutTest : SessionUiTestBase() {
    ...
    fun `test account overlay shows after recents complete`() {
        appRpc.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = ProfileDto(email = "user@example.com"))
        rpc.recent.add(session("ses_1"))
        ui = newUi(displayMs = 1_000)
