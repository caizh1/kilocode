...
import type { OrganizationMode } from "@chipmate/chipmate-gateway"

export namespace ModesMigrator {
  // ChipMate mode structure
  export interface ChipMateMode {
    slug: string
    name: string
  ...
  export interface ChipMateModesFile {
    customModes: ChipMateMode[]
  }
  ...
  export function convertPermissions(groups: ChipMateMode["groups"]): ConfigPermission.Info {
  ...
  export function convertMode(mode: ChipMateMode): ConfigAgentV1.Info {
  ...
  export async function readModesFile(filepath: string): Promise<ChipMateMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as ChipMateModesFile
  ...
  export async function migrate(options: {
    ...
    const allModes: ChipMateMode[] = []

    if (!options.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global modes)
      const vscodeGlobalPath = path.join(ChipMatePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(vscodeGlobalPath)))

      // 2. CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".chipmate", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .chipmatemodes
      const homeModesPath = path.join(os.homedir(), ".chipmatemodes")
    ...
    // 5. Project .chipmatemodes
    const projectModesPath = path.join(options.projectDir, ".chipmatemodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, ChipMateMode>()
