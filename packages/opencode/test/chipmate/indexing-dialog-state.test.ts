...
import {
  ...
  mergeIndexingConfig,
  type IndexingScope,
} from "../../src/chipmate/components/indexing-dialog-state"

describe("indexing dialog state", () => {
  test.serial("loads ChipMate models directly from the public catalog", async () => {
    ...
    global.fetch = (async (input) => {
      ...
      return new Response(
        JSON.stringify({
          defaultModel: "chipmate/default",
          models: [{ id: "chipmate/default", name: "Default", dimension: 1024, scoreThreshold: 0.35 }],
          aliases: {},
        }),
    ...
    try {
      const catalog = await loadChipMateEmbeddingModels()

      expect(catalog.models).toHaveLength(1)
      expect(catalog.defaultModel).toBe("chipmate/default")
  ...
  test("builds stable loading, empty, and loaded model options", () => {
    expect(chipmateModelOptions()).toEqual([{ value: "", title: "Loading supported models..." }])
    expect(chipmateModelOptions({ defaultModel: "", models: [], aliases: {} })).toEqual([
    ...
    expect(chipmateModelOptions(catalog)).toEqual([
      { value: "provider/default", title: "Default (1024d)" },
      { value: "provider/code", title: "Code (code, 1536d)" },
    ])
    expect(currentChipMateModel(catalog, "code")).toBe("provider/code")
    expect(currentChipMateModel(catalog, "missing")).toBe("provider/default")
  })
  ...
  test("isolates global auth config from project indexing values", () => {
    const project: IndexingConfig = { chipmate: { apiKey: "project-key", baseUrl: "https://project.test" } }
    const inherited: IndexingConfig = { enabled: true }
    const effective: Config = {
      provider: { chipmate: { options: { apiKey: "provider-key" } } },
      indexing: project,
    }
