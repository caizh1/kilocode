...
export namespace MemoryTool {
  ...
  export type AskInput = {
    permission: "chipmate_memory_recall" | "chipmate_memory_save"
    patterns: string[]
    always: string[]
  ...
  function disabled(count = false): Result {
    return {
      title: "ChipMate memory: disabled",
      output: "ChipMate memory is disabled for this project.",
  ...
  function block(input: string) {
    return ["```chipmate-memory-v1 targeted_context_not_instruction", input.replaceAll("```", "'''"), "```"].join("\n")
  }
  ...
  function catalog(memory: MemoryService.Interface, input: { root: string; query: string }) {
    ...
    return Effect.gen(function* () {
      ...
      const head = `# ChipMate Memory Catalog (${count} entr${count === 1 ? "y" : "ies"}${filter ? `, filter "${input.query}"` : ""})`
  ...
  function approvalRecall(input: Recall) {
    return input.ask({
      permission: "chipmate_memory_recall",
  ...
  function recallCatalog(input: Recall, live: Live, query: string) {
    return Effect.gen(function* () {
      ...
      return {
        title: `ChipMate memory catalog: ${result.count} entr${result.count === 1 ? "y" : "ies"}`,
        output: safe.output,
  ...
  function recallQuery(input: Recall, live: Live) {
    return Effect.succeed(
      {
        title: `ChipMate memory ${input.params.mode}: no query`,
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `ChipMate memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `ChipMate memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function approval(params: SaveParams, ask: Ask, input: { text?: string; query?: string }) {
    return ask({
      permission: "chipmate_memory_save",
  ...
  function noQuery(): Result {
    return {
      title: "ChipMate memory forget: no query",
  ...
  function noText(action: SaveParams["action"]): Result {
    return {
      title: `ChipMate memory ${action}: no text`,
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "ChipMate memory skipped: out of scope"
    if (input.action === "forget") return `ChipMate memory updated: ${input.removed} removed`
    if (input.added === 0) return "ChipMate memory unchanged"
    if (input.action === "correct")
      return `ChipMate memory correction saved: ${input.added} op${input.added === 1 ? "" : "s"}`
    return `ChipMate memory saved: ${input.added} op${input.added === 1 ? "" : "s"}`
  }
