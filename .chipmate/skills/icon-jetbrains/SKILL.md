...
Guidance for authoring SVG icons for the JetBrains plugin. This skill is the single source of truth for icon sizing, palette, dark variants, composition rules, and placement. Other docs (including `packages/chipmate-jetbrains/AGENTS.md`) defer here for SVG authoring details.
...
8. **File names use kebab-case and stay ASCII-only** (e.g. `arrow-down-to-line.svg`, `book-open-check.svg`, `chipmate@20x20.svg`), matching every existing icon in `packages/chipmate-jetbrains/frontend/src/main/resources/icons/`.

## Icon roles

Pick the canvas size from the role, not the other way around. All icons land in this repo's `packages/chipmate-jetbrains/frontend/src/main/resources/icons/` (with a `views/` subfolder for in-view icons — see [Where the SVGs live](#where-the-svgs-live)).
...
6. **Place and wire the file** → drop both files into `packages/chipmate-jetbrains/frontend/src/main/resources/icons/` (action icons, tool-window icons) or `icons/views/` (in-view chat/session icons), then reference the icon from the plugin's icon-holder class.
...
- `packages/chipmate-jetbrains/AGENTS.md` — repository-specific JetBrains plugin constraints; use it together with this skill for icon work.
- `packages/chipmate-jetbrains/frontend/src/main/resources/icons/` (with its `views/` subfolder) — where plugin icons live; this is the ground truth for placement.
