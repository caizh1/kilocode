...
import { CommandFiles } from "../../src/chipmate/command-files"
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "chipmate-command-files-"))
  roots.push(dir)
  return dir
}
...
describe("CommandFiles", () => {
  test("discovers editable command files and read-only builtins", async () => {
    ...
    const file = path.join(dir, ".chipmate", "command", "review.md")
    await mkdir(path.dirname(file), { recursive: true })
    await writeFile(file, "---\ndescription: Review code\n---\n\nReview $ARGUMENTS")

    const items = await CommandFiles.discover({
      directory: dir,
      directories: [path.join(dir, ".chipmate")],
      commands: [
        cmd({
          name: "review",
          source: "command",
  ...
  test("maps legacy workflows to editable commands", async () => {
    const dir = await temp()
    const file = path.join(dir, ".chipmate", "workflows", "ship.md")
    await mkdir(path.dirname(file), { recursive: true })
  ...
  test("prefers command file attribution over same-named legacy workflow", async () => {
    const dir = await temp()
    const workflow = path.join(dir, ".chipmate", "workflows", "ship.md")
    const file = path.join(dir, ".chipmate", "command", "ship.md")
  ...
  test("discovers symlinked command files", async () => {
    ...
    const link = path.join(dir, ".chipmate", "command", "review.md")
  ...
  test("remove only accepts known editable markdown files", async () => {
    const dir = await temp()
    const file = path.join(dir, ".chipmate", "command", "ok.md")
    await mkdir(path.dirname(file), { recursive: true })
