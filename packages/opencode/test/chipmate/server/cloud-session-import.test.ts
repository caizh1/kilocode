...
const [runtime, gateway, server, fixture] = await (async () => {
  const disabled = process.env.CHIPMATE_DISABLE_SESSION_INGEST
  const base = process.env.CHIPMATE_SESSION_INGEST_URL
  process.env.CHIPMATE_DISABLE_SESSION_INGEST = "1"
  process.env.CHIPMATE_SESSION_INGEST_URL = ingest.url.origin
  try {
    return await Promise.all([
      import("../../../src/effect/app-runtime"),
      import("../../../src/chipmate/server/httpapi/groups/chipmate-gateway"),
      import("../../../src/server/routes/instance/httpapi/server"),
      import("../../fixture/fixture"),
    ])
  } finally {
    if (disabled === undefined) delete process.env.CHIPMATE_DISABLE_SESSION_INGEST
    else process.env.CHIPMATE_DISABLE_SESSION_INGEST = disabled
    if (base === undefined) delete process.env.CHIPMATE_SESSION_INGEST_URL
    else process.env.CHIPMATE_SESSION_INGEST_URL = base
  }
})()
const { AppRuntime } = runtime
const { ChipMateGatewayPaths } = gateway
...
async function request(directory: string, body: unknown) {
  ...
  process.env.CHIPMATE_AUTH_CONTENT = JSON.stringify({ chipmate: { type: "api", key: token } })
  ...
  try {
    return await HttpApiApp.webHandler().handler(
      new Request(`http://localhost${ChipMateGatewayPaths.cloudSessionImport}`, {
        method: "POST",
        headers: { "content-type": "application/json", "x-chipmate-directory": directory },
...
async function routed<T>(id: string, run: () => Promise<T>) {
  const workspace = Flag.CHIPMATE_WORKSPACE_ID
  Flag.CHIPMATE_WORKSPACE_ID = id
  try {
    return await run()
  } finally {
    Flag.CHIPMATE_WORKSPACE_ID = workspace
  }
}
