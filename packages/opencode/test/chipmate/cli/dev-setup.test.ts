...
import { detectRepo, findRepoFrom } from "../../../src/chipmate/cli/dev-setup"
...
describe("findRepoFrom", () => {
  test("returns the repo root when started from a nested path", async () => {
    ...
    const nested = path.join(tmp.path, "packages", "opencode", "src", "chipmate", "cli")
  ...
  test("returns undefined when no sentinel is found up to filesystem root", async () => {
    ...
    // above tmp does not contain a chipmate repo at its own root.
    const found = await findRepoFrom(tmp.path)
...
describe("detectRepo", () => {
  test("honours CHIPMATE_DEV_REPO without touching the filesystem", async () => {
    // Pointing at a path that does not exist — the hint must be returned as-is
    // (this proves no walk happens when CHIPMATE_DEV_REPO is set).
    const hint = "/definitely/does/not/exist/chipmate-repo"
    const got = await withEnv("CHIPMATE_DEV_REPO", hint, () => detectRepo())
    expect(got).toBe(hint)
  })

  test("CHIPMATE_DEV_REPO wins over the filesystem walk", async () => {
    // Build a synthetic repo, point CHIPMATE_DEV_REPO at it, and run from an
    // unrelated cwd. Even though the walk from cwd (inside the real chipmate
    // checkout) would succeed, the hint must take priority.
    await using tmp = await tmpdir()
    await makeSynthRepo(tmp.path)

    const got = await withEnv("CHIPMATE_DEV_REPO", tmp.path, () => detectRepo())
