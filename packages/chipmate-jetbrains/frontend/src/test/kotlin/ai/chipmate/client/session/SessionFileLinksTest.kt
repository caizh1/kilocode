...
class SessionFileLinksTest : BasePlatformTestCase() {
    ...
    fun `test isFileHref separates file refs from urls`() {
        ...
        assertFalse(SessionFileLinks.isFileHref("https://chipmate.ai/docs"))
    ...
    fun `test managed worktree storage filter only rejects worktree subtree`() {
        ...
        assertFalse(isManagedWorktreeStorage(".chipmate/plans/x.md"))
        assertTrue(isManagedWorktreeStorage(".chipmate/worktrees"))
        assertTrue(isManagedWorktreeStorage(".chipmate/worktrees/foo/backend/src/Main.java"))
    }
