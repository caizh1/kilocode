...
const reg = process.env.REGISTRY ?? "ghcr.io/chipmate-org" // chipmate_change
...
const images = ["base", "bun-node", "jetbrains", "rust", "tauri-linux", "publish"] // chipmate_change

const setup = async () => {
  if (!push) return
  const list = await $`docker buildx ls`.text()
  // chipmate_change start
  if (list.includes("chipmate")) {
    await $`docker buildx use chipmate`
    return
  }
  await $`docker buildx create --name chipmate --use`
  // chipmate_change end
}
