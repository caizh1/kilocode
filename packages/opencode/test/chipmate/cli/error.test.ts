...
describe("MCP errors", () => {
  test("uses ChipMate-neutral capability messaging", () => {
...
describe("model not found errors", () => {
  test("indicates when no models are available", () => {
    ...
    expect(named).toContain("chipmate.json")
...
describe("remote config authentication errors", () => {
  test("uses the ChipMate login command", () => {
    const error = FormatError({
    ...
    expect(error).toContain("chipmate auth login https://example.com/config.json")
