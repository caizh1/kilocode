...
class AttachmentChip(
    ...
    init {
        ...
        toolTipText = tip
        accessibleContext?.accessibleName = ChipMateBundle.message("prompt.attachment.open", item.name)
    ...
    private fun label(): String {
        val start = startLine
        val end = endLine
        if (file && start != null && end != null) return ChipMateBundle.message("session.attachment.file.range", item.name, start, end)
        if (file) return item.name
        return ChipMateBundle.message("session.attachment.unknown", item.mime.ifBlank { "unknown" })
    }
...
open class AttachmentCard(
    ...
    private val action = remove?.let { callback ->
        CloseButton().apply {
            isVisible = false
            toolTipText = ChipMateBundle.message("prompt.attachment.remove", item.name)
...
private fun tooltip(item: AttachmentCardItem): String = XmlStringUtil.wrapInHtml(
    StringUtil.escapeXmlEntities(
        ChipMateBundle.message("prompt.attachment.tooltip", item.name, item.mime, location(item)),
    ).replace("\n", "<br>"),
)
...
private fun location(item: AttachmentCardItem): String {
    ...
    if (uri?.scheme == "data") return ChipMateBundle.message("prompt.attachment.embedded")
