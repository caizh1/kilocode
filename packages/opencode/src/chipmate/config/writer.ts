...
import { ChipMateConfigOverlay } from "./overlay"

export namespace ChipMateConfigWriter {
  export type Conflict = {
    ok: false
    code: "target-changed" | "revision-conflict" | "target-not-writable"
    message: string
    target: ChipMateConfigOverlay.Target
  }
  ...
  export async function write(input: {
    directory: string
    worktree?: string
    scope: ChipMateConfigOverlay.Scope
    ...
    const target = await ChipMateConfigOverlay.target(input)
    ...
    const patch = ChipMateConfigOverlay.patch({ scope: input.scope, set: input.set, unset: input.unset })
    if (Object.keys(patch).length === 0) return { ok: true, target, changed: false, sandboxChanged: false }
    await mkdir(path.dirname(target.path), { recursive: true })
    await input.beforeWrite?.()
    const checked = await ChipMateConfigOverlay.target(input)
    ...
    if (
      expected &&
      ChipMateConfigOverlay.revision(checked.path, checked.exists, checked.exists ? before : "") !== expected.revision
    ) {
      return {
        ok: false,
    ...
    return {
      ok: true,
      target: await ChipMateConfigOverlay.target(input),
