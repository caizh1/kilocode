...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "ChipMate", message: "focused" }]) // chipmate_change
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "ChipMate", message: "hello" }]) // chipmate_change
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "ChipMate", message: "hello", sound: false })).toEqual({
      // chipmate_change
      ok: true,
      notification: true,
      sound: false,
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "ChipMate", message: "hello again" }]) // chipmate_change
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("chipmate.default") // chipmate_change
  })
