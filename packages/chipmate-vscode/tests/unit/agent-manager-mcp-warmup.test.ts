...
describe("Agent Manager MCP warmup", () => {
  it("starts MCP status for the worktree directory before session creation", async () => {
    ...
    const result = await startSession(
      client as never,
      "/repo/.chipmate/worktrees/feature",
      async () => {
        calls.push(["session"])
        return "created"
      },
    ...
    expect(calls[0]).toEqual(["warm", { directory: "/repo/.chipmate/worktrees/feature" }, { throwOnError: true }])
  ...
  it("logs and contains MCP warmup failures", async () => {
    ...
    expect(result).toBe("created")
    expect(logs[0]).toEqual(["[MCPWarmup] Starting for /repo/.chipmate/worktrees/feature"])
    expect(logs[1]?.[0]).toBe("[MCPWarmup] Failed for /repo/.chipmate/worktrees/feature:")
