...
import { and, desc, eq, isNotNull, gt, or, sql } from "drizzle-orm" // chipmate_change
...
import * as StoredMessage from "../chipmate/session-message" // chipmate_change
import { SessionInput } from "./input"
import { WorkspaceV2 } from "../workspace"
import { SessionContextEpoch } from "./context-epoch"
import { MessageTable, PartTable, SessionInputTable, SessionMessageTable, SessionTable } from "./sql"
import type { DeepMutable } from "../schema"
import * as PromptCompat from "../chipmate/session/prompt-promoted" // chipmate_change - released replay key
...
function sessionRow(info: SessionV1.SessionInfo): typeof SessionTable.$inferInsert {
  return {
    ...
    share_url: info.share?.url ?? null, // chipmate_change - full session updates must clear removed shares
...
function run(db: DatabaseService, event: SessionEvent.Event) {
  return Effect.gen(function* () {
    const decodeRow = (row: typeof SessionMessageTable.$inferSelect) =>
      decodeMessage(StoredMessage.normalize({ ...row.data, id: row.id, type: row.type })) // chipmate_change
...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* events.project(PromptCompat.definition, (event) => PromptCompat.project(db, event, (next) => run(db, next))) // chipmate_change - replay released two-step promotions
    ...
    yield* events.project(SessionEvent.RevertEvent.Committed, (event) =>
      Effect.gen(function* () {
        const boundary = yield* db
          ...
          .get()
          .pipe(Effect.orDie)
        // chipmate_change - ChipMate keeps session_message.seq nullable for released-data compatibility
