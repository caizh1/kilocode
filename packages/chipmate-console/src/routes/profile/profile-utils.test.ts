...
test("parses device auth codes from instructions", () => {
  expect(parseDeviceCode("Open https://app.chipmate.ai/device-auth and enter code: ABCD-2345")).toBe("ABCD-2345")
  expect(parseDeviceCode("Use ABCD-2345 to continue")).toBe("ABCD-2345")
  expect(parseDeviceCode(undefined)).toBeUndefined()
})

test("detects ChipMate auth failures", () => {
  expect(authError(new Error("ChipMate profile: Unauthorized"))).toBe(true)
  expect(authError({ status: 401 })).toBe(true)
  expect(authError(new Error("ChipMate profile: temporary network failure"))).toBe(false)
})
...
test("keeps login returns internal", () => {
  ...
  expect(safeReturn("https://app.chipmate.ai/profile")).toBe("/profile")
  expect(safeReturn("//app.chipmate.ai/profile")).toBe("/profile")
})

test("builds local links while preserving server", () => {
  const params = new URLSearchParams({ server: "http://127.0.0.1:4097", ignored: "1" })
  expect(page(params, "/chipmate/login", { return: "/profile" })).toBe(
    "/chipmate/login?server=http%3A%2F%2F127.0.0.1%3A4097&return=%2Fprofile",
  )
})

test("builds account-aware cloud links", () => {
  const id = "9d4b144c-0a2b-477b-973d-24fa02bebf13"
  expect(usage(null)).toBe("https://app.chipmate.ai/usage")
  expect(usage(id)).toBe(`https://app.chipmate.ai/organizations/${id}/usage-details`)
  expect(credits(undefined)).toBe("https://app.chipmate.ai/profile")
  expect(credits(id)).toBe(`https://app.chipmate.ai/organizations/${id}`)
})
