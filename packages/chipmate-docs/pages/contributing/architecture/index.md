...
description: "Overview of the ChipMate platform architecture"
---

# Architecture Overview

This page maps ChipMate's repository-defined architecture. It introduces the local runtime, editor clients, cloud service boundaries, and hosted execution products before the subsystem pages add implementation detail.
...
References to ChipMate Console and `chipmate console` on the architecture pages describe a browser interface that is deprecated and will be removed in a future release.
{% /callout %}
...
| Repository | Contents |
|---|---|
| [ChipMate&#8209;Org/chipmate](https://github.com/ChipMate-Org/chipmate) | ChipMate CLI runtime, local daemon, ChipMate Console, VS Code extension, JetBrains plugin, JavaScript SDK, codebase indexing, ChipMate Gateway client, telemetry, docs, and shared UI packages |
| [ChipMate&#8209;Org/cloud](https://github.com/ChipMate-Org/cloud) | Web control plane, ChipMate Gateway routes, Cloud Agent session runtime, automation, generated-application preview and deployment services, ChipMateClaw, Gas Town, billing, and supporting Workers |

## Three architecture layers

| Layer | Responsibility | Typical boundaries |
|---|---|---|
| Local runtime and clients | Runs local coding sessions and connects editor surfaces to one local agent engine | ChipMate CLI runtime, `chipmate serve` server, local daemon, ChipMate Console, VS Code extension, JetBrains plugin |
| ChipMate Cloud shared services | Handles hosted identity, authorization, model routing, billing, orchestration, and shared product services | Web control plane, ChipMate Gateway, Workers, queues, Durable Objects, persistence |
| Hosted product runtimes and automation | Runs scoped cloud work for coding, app generation, assistants, security analysis, and multi-agent orchestration | Cloud Agent, Automation Services, App Builder, Security Agent, ChipMateClaw, Gas Town, Wasteland |

Local execution and hosted execution are separate boundaries. Editor clients use a local `chipmate serve` server. Hosted automation can launch Cloud Agent execution sessions when cloud coding work is required.

## Terms used throughout

| Term | Meaning |
|---|---|
| ChipMate | Umbrella product across local clients, ChipMate CLI runtime, and ChipMate Cloud services |
| ChipMate CLI runtime | Local agent engine in `packages/opencode/`; owns tools, sessions, config, persistence, and provider routing |
...
| Web control plane | Hosted ChipMate Cloud application layer for identity, organization authorization, billing, product configuration, and API orchestration |
| ChipMate Gateway | First-party hosted model-routing boundary |
...
flowchart LR
  subgraph clients ["Local clients"]
    tui["ChipMate CLI TUI"]
    run["chipmate run"]
    console["ChipMate Console"]
    editors["VS Code and JetBrains"]
  end

  subgraph local ["Local ChipMate CLI boundary"]
    daemon["Local daemon manager"]
    server["chipmate serve server"]
    runtime["ChipMate CLI runtime"]
    router["Provider router"]
  end

  subgraph cloud ["ChipMate Cloud shared services"]
    web["Web control plane"]
    workers["Automation Workers, queues, and Durable Objects"]
    gateway["ChipMate Gateway"]
    agent["Cloud Agent"]
  end
...
|---|---|---|---|
| Local coding | ChipMate CLI, ChipMate Console, VS Code, or JetBrains | ChipMate CLI runtime on developer machine | Editor clients talk to local `chipmate serve` server. Local runtime owns coding session and sends model requests directly or through ChipMate Gateway. |
| Hosted work | Webhook, source-control event, command, schedule, or hosted product | ChipMate Cloud services; Cloud Agent when coding is required | Cloud services coordinate work. Only flows that need repository changes launch Cloud Agent execution session. |
...
flowchart LR
  ...
  chat["ChipMate Chat, events, and notifications"]
  claw["ChipMateClaw"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| ChipMateClaw | Coordinates owner-scoped hosted assistant runtimes | [Cloud Platform](/docs/contributing/architecture/cloud-platform#chipmateclaw) | [ChipMateClaw ingress](/docs/contributing/architecture/cloud-security#chipmateclaw-ingress) |
...
These local surfaces live in [`ChipMate-Org/chipmate`](https://github.com/ChipMate-Org/chipmate). Package paths below are relative to that repository root.

| Surface | Package in `ChipMate-Org/chipmate` | Runtime model |
|---|---|---|
| ChipMate CLI TUI | `packages/opencode/` | Interactive local client with daemon attach and worker-backed fallback paths |
| `chipmate run` | `packages/opencode/` | Headless prompt execution through explicit attach, daemon attach, or embedded fallback |
| `chipmate serve` | `packages/opencode/` | Local HTTP + SSE server for local clients |
| ChipMate Console (deprecated) | `packages/chipmate-console/`{% linebreak /%}`packages/opencode/` | Deprecated browser UI served at `/console` by a started or reused local daemon |
| VS Code extension | `packages/chipmate-vscode/` | Extension host starts one shared editor-owned `chipmate serve` server and routes webviews through HTTP + global SSE; SDK directory selects local runtime instance |
| JetBrains plugin | `packages/chipmate-jetbrains/` | Split-mode Swing plugin; backend module starts one editor-owned `chipmate serve` server and caches workspace clients by directory |

## Cloud service families

Hosted service families live in [`ChipMate-Org/cloud`](https://github.com/ChipMate-Org/cloud). Paths below are relative to that repository root unless another repository is named.

| Boundary | Primary source paths | Role |
|---|---|---|
| ChipMate Cloud | `apps/web/`{% linebreak /%}`services/` | Hosted platform repository for identity, billing, routing, product configuration, automation, and scoped execution services |
| Web control plane | `apps/web/` | Hosted application layer for authorization, configuration, and API orchestration |
| ChipMate Gateway | `apps/web/src/app/api/gateway/`{% linebreak /%}`apps/web/src/lib/ai-gateway/`{% linebreak /%}Local integration: `ChipMate-Org/chipmate/packages/chipmate-gateway/` | First-party model-routing boundary and local client integration |
...
| Adjacent hosted boundaries | `services/app-builder/`{% linebreak /%}`services/chipmateclaw/`{% linebreak /%}`services/gastown/`{% linebreak /%}`services/wasteland/`{% linebreak /%}Supporting services | App Builder, ChipMateClaw, Gas Town, Wasteland, chat, notifications, and supporting services |

## Supporting packages

These supporting packages also live in [`ChipMate-Org/chipmate`](https://github.com/ChipMate-Org/chipmate). Package paths below are relative to that repository root.

| Package in `ChipMate-Org/chipmate` | Role |
|---|---|
| `packages/chipmate-indexing/` | Per-directory asynchronous codebase indexing engine behind ChipMate CLI bridge |
| `packages/sdk/js/` | Generated JavaScript client and handwritten wrapper for local server APIs |
| `packages/chipmate-gateway/` | Local ChipMate Gateway client integration used by ChipMate CLI runtime |
| `packages/chipmate-console/` | Deprecated browser UI served by local daemon at `/console` |

## Architecture pages

| Page | What it covers |
|---|---|
| [CLI Runtime](/docs/contributing/architecture/cli-runtime) | Local execution modes, daemon, server authentication, routing, persistence, snapshots, SDK, config, SSE, ChipMate Console, and indexing |
| [VS Code Extension](/docs/contributing/architecture/vscode-extension) | Shared local `chipmate serve` ownership, webview bridge, Agent Manager, PTYs, recovery, bundled resources, and build outputs |
| [JetBrains Plugin](/docs/contributing/architecture/jetbrains-plugin) | Split-mode modules, RPC, bundled local `chipmate serve` lifecycle, Kotlin SDK, recovery, and remote-development constraints |
...
- [ChipMateClaw Overview](/docs/chipmateclaw/overview) - customer-facing ChipMateClaw docs
