...
class SessionSidePanelManager(
    ...
    private val resolve: (String) -> Workspace = { dir -> service<ChipMateWorkspaceService>().workspace(dir) },
    private val status: () -> Map<String, SessionActivityKind> = { project.service<ChipMateSessionService>().activity() },
    ...
    private fun schedule(ui: SessionUi) {
        cancel(ui)
        val delay = Registry.intValue("chipmate.session.inactive.disposeTimeoutMs").coerceAtLeast(0)
