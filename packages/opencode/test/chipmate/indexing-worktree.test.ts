...
async function wait(state: ChipMateIndexing.Status["state"]) {
  for (const _ of Array.from({ length: 100 })) {
    const status = await ChipMateIndexing.current()
...
describe("indexing worktrees", () => {
  test("does not pass a baseline for the primary checkout root", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        await ChipMateIndexing.search("primary checkout")
  ...
  test("does not pass a baseline for a directory inside the primary checkout", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        await wait("Complete")
        await ChipMateIndexing.search("primary checkout directory")
      },
    })
  ...
  test("shares the primary checkout index with a linked worktree", async () => {
    ...
    const worktree = path.join(tmp.path, ".chipmate", "worktrees", "feature")
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        await wait("Complete")
        await ChipMateIndexing.search("worktree")
        expect(await ChipMateIndexing.available()).toBe(true)
      },
    })
  ...
  test("maps a directory inside a linked worktree into the primary checkout", async () => {
    ...
    process.env["CHIPMATE_CONFIG_DIR"] = tmp.path
    await Bun.$`git -C ${tmp.path} add opencode.json && git -C ${tmp.path} commit -m config`.quiet()
    const worktree = path.join(tmp.path, ".chipmate", "worktrees", "nested")
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        await wait("Complete")
        await ChipMateIndexing.search("linked worktree directory")
      },
    })
  ...
  test("does not classify an ordinary directory from its pathname", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["CHIPMATE_CONFIG_DIR"] = tmp.path
    const directory = path.join(tmp.path, ".chipmate", "worktrees", "feature")
    await mkdir(directory, { recursive: true })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        await wait("Complete")
        await ChipMateIndexing.search("ordinary directory")
        expect(await ChipMateIndexing.available()).toBe(true)
      },
    })
