...
describe("document tool routing boundary", () => {
  test("keeps generated document artifacts out of ordinary QA routing", async () => {
    const artifacts = await readSource("src/chipmate/tool/document-artifacts.ts")
    const word = await readSource("src/chipmate/tool/word-documents.ts")
    const mermaid = await readSource("src/chipmate/tool/mermaid-documents.ts")
  ...
  test("keeps sidecar document tool failures as bounded tool results", async () => {
    ...
    expect(artifacts).toContain("ChipMate native QA, code understanding, and document_search are not replaced")

    expect(word).toContain("function wordFailure")
    expect(word).toContain("This Word sidecar operation failed")
    expect(word).toContain("ChipMate native QA, code understanding, and document_search are not replaced")

    expect(mermaid).toContain("function mermaidFailure")
    expect(mermaid).toContain("This Mermaid sidecar operation failed")
    expect(mermaid).toContain("ChipMate native QA, code understanding, and document_search are not replaced")
  })
})
