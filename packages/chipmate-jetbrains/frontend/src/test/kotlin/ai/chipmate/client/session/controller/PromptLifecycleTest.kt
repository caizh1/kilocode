...
class PromptLifecycleTest : SessionControllerTestBase() {
    ...
    override fun setUp() {
        super.setUp()
        edt { ChipMatePluginSettings.unsetAutoApprove() }
    }
    ...
    fun `test prompt records aggregate mention telemetry`() {
        appRpc.state.value = ai.chipmate.rpc.dto.ChipMateAppStateDto(ai.chipmate.rpc.dto.ChipMateAppStatusDto.READY, config = ai.chipmate.rpc.dto.ConfigDto(model = "chipmate/gpt-5"))
    ...
    fun `test delete queued message delegates to RPC`() {
        ...
        assertEquals(listOf(ai.chipmate.client.testing.FakeSessionRpcApi.MessageDeleteCall("ses_test", "/test", "u2")), rpc.messageDeletes)
    ...
    fun `test PermissionAsked moves state to AwaitingPermission`() {
        ...
        assertSession(
            ...
            [code] [chipmate/gpt-5] [awaiting-permission]
            """,
            m,
        )
    }
    ...
    fun `test PermissionReplied resumes Busy state`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [busy] [considering next steps]
            """,
            m,
        )
    }
    ...
    fun `test QuestionAsked moves state to AwaitingQuestion`() {
        ...
        assertSession(
            ...
            [code] [chipmate/gpt-5] [awaiting-question]
            """,
            m,
        )
    }
    ...
    fun `test recovery surfaces a pending skill shell as a card under auto approve`() {
        ...
        projectRpc.state.value = workspaceReady()
        rpc.pendingPermissionList.add(permission("perm_skill").copy(metadata = mapOf("skillShell" to "true")))
        edt { ChipMatePluginSettings.setAutoApprove(true) }
    ...
    fun `test auto approve persists in properties`() {
        ...
        assertFalse(ChipMatePluginSettings.getAutoApprove())
        edt { m.setAutoApprove(true) }

        assertTrue(ChipMatePluginSettings.getAutoApprove())
    ...
    fun `test child non-permission events do not change root state`() {
        ...
        emit(ChatEventDto.SessionStatusChanged("ses_child", ai.chipmate.rpc.dto.SessionStatusDto("busy")), flush = false)
        ...
        val stateEvents = modelEvents.filterIsInstance<ai.chipmate.client.session.model.SessionModelEvent.StateChanged>()
    ...
    private fun planWorkspace() = workspaceReady(
        ...
        defaults = mapOf("plan" to "chipmate/gpt-5", "code" to "chipmate/gpt-5"),
    )
}
