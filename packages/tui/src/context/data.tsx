...
import { createStore, produce, reconcile } from "solid-js/store" // chipmate_change
import { createSimpleContext } from "./helper"
import { useSDK } from "./sdk"
import { useEvent } from "./event"
import { createSignal, onCleanup, onMount } from "solid-js"
import { hydrate } from "../chipmate/hydration" // chipmate_change
...
// chipmate_change start - "global" is an event-routing sentinel, not a filesystem location.
...
// chipmate_change start - suppress only refreshes canceled by normal TUI disposal
...
export const { use: useData, provider: DataProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const syncing = new Map<string, Promise<void>>() // chipmate_change
    ...
    function handleEvent(event: V2Event) {
      switch (event.type) {
        ...
        case "session.next.compaction.ended": {
          // chipmate_change start - legacy v1 compaction events do not carry a projectable message identity.
    ...
    onMount(() => {
      const unsub = events.subscribe((event, metadata) => {
        handleEvent({
          ...
          location: eventLocation({ directory: metadata.directory, workspace: metadata.workspace }), // chipmate_change
        } as V2Event)
      })
      onCleanup(unsub)
    })
    ...
    const result = {
      session: {
        ...
        message: {
          ...
          async refresh(sessionID: string) {
            // chipmate_change start - reconcile the snapshot without losing live updates received while it was loading
    ...
    // chipmate_change start - classify each rejection when it occurs so later disposal cannot hide earlier failures
    let disposed = false
    onCleanup(() => {
      disposed = true
    })
