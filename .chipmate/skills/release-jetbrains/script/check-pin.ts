...
if (values.help) {
  console.log(`
Usage: bun .chipmate/skills/release-jetbrains/script/check-pin.ts

Checks the CLI pin that a JetBrains release would lock. Prepare tags origin/main,
so this reads packages/chipmate-jetbrains/package.json from origin/main and compares
it with the latest published ChipMate CLI release plus the local worktree pin.
...
const pinMain = JSON.parse(await $`git show origin/main:packages/chipmate-jetbrains/package.json`.text()).version as string
const pinLocal = (await Bun.file("packages/chipmate-jetbrains/package.json").json()).version as string
const propsMain = await $`git show origin/main:packages/chipmate-jetbrains/gradle.properties`.text()
const propsLocal = await Bun.file("packages/chipmate-jetbrains/gradle.properties").text()
...
async function previous() {
  ...
  if (!tag) return null
  const res = await $`git show ${tag}:packages/chipmate-jetbrains/package.json`.nothrow().text()
