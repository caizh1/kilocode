...
// equivalents in @chipmate/sdk. All API types (Session, Event, Agent,
// McpStatus, Config, etc.) should be imported from "@chipmate/sdk/v2/client".

import type { IndexingStatus as SdkIndexingStatus } from "@chipmate/sdk/v2/client"
...
export interface ChipMateOrganization {
  id: string
  name: string
  role: string
}
...
export interface ChipMateProfile {
  email: string
  name?: string
  organizations?: ChipMateOrganization[]
...
// Cloud session from the ChipMate cloud API (cli_sessions_v2)
...
// Full cloud session data for preview (from /chipmate/cloud/session/:id)
