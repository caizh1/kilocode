...
import { CHIPMATE_ERROR_CODES, isChipMateError, parseChipMateErrorCode } from "../../src/chipmate/chipmate-errors"
...
describe("parseChipMateErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parseChipMateErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-ChipMate error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parseChipMateErrorCode(error)).toBeUndefined()
  })
...
describe("isChipMateError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(isChipMateError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without ChipMate code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(isChipMateError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with ChipMate errors", () => {
