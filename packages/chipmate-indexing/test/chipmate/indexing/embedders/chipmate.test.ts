...
import { ChipMateEmbedder, CHIPMATE_INDEXING_FEATURE } from "../../../../src/indexing/embedders/chipmate"

describe("ChipMateEmbedder", () => {
  beforeEach(() => {
    mockEmbeddingsCreate.mockReset()
    setOpenAIConstructorHook(undefined)
  })

  test("uses ChipMate Gateway headers and configured embedding model", async () => {
    const seen: unknown[] = []
    ...
    const embedder = new ChipMateEmbedder({
    ...
    expect(seen[0]).toEqual({
      ...
      apiKey: "chipmate-token",
      defaultHeaders: {
        "X-CHIPMATE-FEATURE": CHIPMATE_INDEXING_FEATURE,
        "X-CHIPMATE-ORGANIZATIONID": "org_123",
      },
    })
