...
{% browserFrame url="app.chipmate.ai/gastown/town/settings/wasteland" caption="Wasteland settings tab in your Gas Town dashboard" %}
...
Your **rig handle** is your town's identity on the wasteland. It's an `org/repo`-style identifier (e.g., `chipmate/main`, `acme/backend`) that other participants see when you claim items and submit evidence.
...
{% browserFrame url="app.chipmate.ai/gastown/town/settings/wasteland" caption="DoltHub credentials dialog with OAuth default and advanced API token option" %}
...
### Default: OAuth through ChipMate Integrations

1. Go to [ChipMate Integrations → DoltHub](https://app.chipmate.ai/integrations/dolthub)
2. Click **Connect DoltHub**
3. Approve ChipMate on DoltHub
4. Return to ChipMate and confirm DoltHub shows **Connected**

When you join a wasteland, choose **Use your connected DoltHub account**. Confirm your DoltHub username so ChipMate can label commits and contribution branches correctly.
...
Your DoltHub credentials need read and write access. ChipMate uses them to:
...
ChipMate uses DoltHub OAuth for the normal Gas Town Wasteland flow. Use the advanced API token option only for setups that cannot use OAuth.
{% /callout %}
...
4. Approve ChipMate again on DoltHub
...
- DoltHub shows **Connected** in [ChipMate Integrations](https://app.chipmate.ai/integrations/dolthub)
