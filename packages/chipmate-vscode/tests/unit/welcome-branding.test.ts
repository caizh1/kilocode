...
describe("QA welcome branding", () => {
  it("uses the packaged ChipMate asset only in the QA welcome state", () => {
    ...
    expect(messages).not.toContain('<ChipMateLogo class="chipmate-logo" welcome />')
    expect(sidebar).toContain('<ChipMateLogo class="chipmate-logo" welcome />')
    expect(sidebar).not.toContain("ChipMateLogo")
    expect(welcome).toContain('<ChipMateLogo class="chipmate-logo" welcome />')
    expect(welcome).not.toContain("ChipMateLogo")
    expect(welcome).toContain('<h1 class="chipmate-product-title">ChipMate</h1>')
  ...
  it("uses ChipMate branding throughout the English and Chinese work style onboarding", () => {
    for (const dict of [en, zh, zht]) {
      const text = Object.entries(dict)
        ...
        .map(([, value]) => value)
        .join("\n")
      expect(text).not.toContain("ChipMate")
    }
