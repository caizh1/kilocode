...
import { PlanFile } from "../../src/chipmate/plan-file"
  ...
  test("plan_exit recovers generated plan path when omitted", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const file = path.join(Instance.worktree, ".chipmate", "plans", `${session.time.created}-xy.md`)
        await Bun.write(file, "Do implementation step 1")
        ...
        expect(result.metadata.plan.replaceAll(path.sep, "/")).toBe(`.chipmate/plans/${session.time.created}-xy.md`)
      },
    })
  })
