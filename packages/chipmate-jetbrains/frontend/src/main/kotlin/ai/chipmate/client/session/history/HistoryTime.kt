...
internal object HistoryTime {
    ...
    fun title(section: HistorySection): String = when (section) {
        HistorySection.TODAY -> ChipMateBundle.message("history.group.today")
        HistorySection.YESTERDAY -> ChipMateBundle.message("history.group.yesterday")
        HistorySection.WEEK -> ChipMateBundle.message("history.group.week")
        HistorySection.MONTH -> ChipMateBundle.message("history.group.month")
        HistorySection.OLDER -> ChipMateBundle.message("history.group.older")
    }
    ...
    fun relative(item: HistoryItem, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return ChipMateBundle.message("history.time.moments")
        if (diff < HOUR) return ChipMateBundle.message("history.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return ChipMateBundle.message("history.time.hours", (diff / HOUR).coerceAtLeast(1))
        if (diff < 7 * DAY) return ChipMateBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        ...
        if (months < 1) return ChipMateBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        if (months < 12) return ChipMateBundle.message("history.time.months", months)
        return ChipMateBundle.message("history.time.years", months / 12)
    }
