...
description: "How to configure and use MCP servers in the ChipMate CLI"
---

# Using MCP in the CLI

The ChipMate CLI supports both local and remote MCP servers. Once added, MCP tools are automatically available to the LLM alongside built-in tools.
...
|---|---|---|
| **Global** | `~/.config/chipmate/chipmate.json` | `chipmate.jsonc`, `config.json` |
| **Project** | `./chipmate.json` or `./.chipmate/chipmate.json` | `chipmate.jsonc` |
...
Connected servers can also add usage instructions to the model context and expose resources, including parameterized resource templates. See [Server instructions and resources](/docs/automate/mcp/using-in-chipmate-code#server-instructions-and-resources).
...
Browse community-contributed MCP server configurations and agent skills in the [ChipMate Marketplace](https://github.com/ChipMate-Org/chipmate-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
