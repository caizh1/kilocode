...
import { ChipMateSessionRevert } from "@/chipmate/session/revert"
    ...
    provideTmpdirInstance(
      (root) =>
        Effect.gen(function* () {
          ...
          yield* ChipMateSessionRevert.restore(item.snapshot, item.after, item.patch.files).pipe(provideInstance(dir))
