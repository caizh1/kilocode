...
import { billable, creditLabel, format, passLine, resetLabel, scope } from "../../src/chipmate/plugins/sidebar-footer"
...
const message = {
  ...
  parentID: "msg_0",
  modelID: "chipmate-auto/balanced",
  providerID: "chipmate",
  mode: "build",
  agent: "build",
...
describe("ChipMate sidebar footer", () => {
  test("formats money", () => {
  ...
  test("shows pass period usage and reset date", () => {
    expect(passLine(chipmatePass)).toBe("$73 / $199")
    expect(resetLabel(chipmatePass.nextBillingAt)).toBe("Jul 1")
    expect(resetLabel(null)).toBeUndefined()
    expect(resetLabel("not-a-date")).toBeUndefined()
  })

  test("refreshes only after completed billed ChipMate turns", () => {
