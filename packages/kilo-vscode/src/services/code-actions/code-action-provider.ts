...
export class KiloCodeActionProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    add.command = { command: "kilo-code.new.addToContext", title: "Add to ChipMate" }
    actions.push(add)
    ...
    if (hasDiagnostics) {
      ...
      fix.command = { command: "kilo-code.new.fixCode", title: "Fix with ChipMate" }
    ...
    if (!hasDiagnostics) {
      ...
      explain.command = { command: "kilo-code.new.explainCode", title: "Explain with ChipMate" }
      actions.push(explain)

      const improve = new vscode.CodeAction("Improve with ChipMate", vscode.CodeActionKind.RefactorRewrite)
      improve.command = { command: "kilo-code.new.improveCode", title: "Improve with ChipMate" }
