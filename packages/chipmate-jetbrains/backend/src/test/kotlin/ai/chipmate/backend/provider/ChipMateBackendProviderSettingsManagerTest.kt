...
class ChipMateBackendProviderSettingsManagerTest {
    ...
    @Test
    fun `disconnecting chipmate gateway returns error without logout`() = runBlocking {
        mock.providers = """{
            "all":[{"id":"chipmate","name":"ChipMate Gateway","source":"custom","models":{}}],
            "default":{},
            "connected":["chipmate"],
            "failed":[]
        }""".trimIndent()
        ...
        mock.resetCounts()
        val result = manager.disconnect(ProviderDisconnectDto("/test", "chipmate"))

        assertEquals("ChipMate Gateway cannot be disconnected from provider settings.", result.error)
        assertFalse(result.profileCleared)
        assertNull(mock.lastAuthDeletePath)
        assertEquals(0, mock.requestCount("/auth/chipmate"))
        assertEquals(0, mock.requestCount("/global/dispose"))
    }
    ...
    fun `state waits through dispose triggered reload`() = runBlocking {
        ...
        val app = app()
        val manager = ChipMateBackendProviderSettingsManager(app)
        ...
        try {
            ...
            withTimeout(5_000) {
                app.appState.first { it is ChipMateAppState.Loading }
            }
    ...
    private suspend fun manager(): ChipMateBackendProviderSettingsManager {
        return ChipMateBackendProviderSettingsManager(app())
    }
