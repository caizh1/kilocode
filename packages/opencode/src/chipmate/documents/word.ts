...
import { assertValidWordDocumentBytes, prepareWordDocumentBytes } from "@/chipmate/documents/word-validation"
...
export async function renderWordDocument(input: RenderWordDocumentInput): Promise<RenderedWordDocument> {
  ...
  const endpoint = input.remoteEndpoint?.trim() || process.env["CHIPMATE_WORD_RENDER_ENDPOINT"]?.trim()
  ...
  if (!endpoint) {
    ...
    if (!local) {
      diagnostics.push({
        ...
        severity: "warning",
        message:
          "Word render remote endpoint is not configured and no local soffice executable was found; set remoteEndpoint, CHIPMATE_WORD_RENDER_ENDPOINT, CHIPMATE_WORD_RENDER_SOFFICE, or put soffice on PATH to render PDF/page PNG artifacts.",
      })
...
function coreXml(spec: CreateWordDocumentSpec): string {
  return xml(
    `<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${escapeXml(spec.title)}</dc:title><dc:creator>${escapeXml(spec.author ?? "ChipMate")}</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${new Date().toISOString()}</dcterms:created></cp:coreProperties>`,
  )
}

function appXml(): string {
  return xml(
    '<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>ChipMate</Application></Properties>',
  )
}
...
async function tryLocalWordRenderer(
  ...
  const soffice = await findExecutable(process.env["CHIPMATE_WORD_RENDER_SOFFICE"], "soffice")
  if (!soffice) return undefined

  const diagnostics: WordRenderDiagnostic[] = []
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-word-render-"))
  try {
    ...
    const pdftoppm = await findExecutable(process.env["CHIPMATE_WORD_RENDER_PDFTOPPM"], "pdftoppm")
    ...
    if (!pdftoppm) {
      diagnostics.push({
        ...
        severity: "warning",
        message:
          "Local PDF renderer succeeded, but pdftoppm was not found; set CHIPMATE_WORD_RENDER_PDFTOPPM or put pdftoppm on PATH to render page PNG artifacts.",
      })
    } else {
...
async function localCjkFontQa(pdf: string, sourceCjkCount: number, timeoutMs: number) {
  ...
  const text = await findExecutable(process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"], "pdftotext")
  const sibling = text
    ? path.join(path.dirname(text), process.platform === "win32" ? "pdffonts.exe" : "pdffonts")
    : undefined
  const executable = await findExecutable(process.env["CHIPMATE_WORD_RENDER_PDFFONTS"] ?? sibling, "pdffonts")
  if (!executable) {
...
async function localWordTextQa(source: string, pdf: string, timeoutMs: number): Promise<WordRenderTextQa> {
  const executable = await findExecutable(process.env["CHIPMATE_WORD_RENDER_PDFTOTEXT"], "pdftotext")
...
async function pdfPageCount(bytes: Uint8Array, timeoutMs: number): Promise<number | undefined> {
  const executable = await findExecutable(process.env["CHIPMATE_WORD_RENDER_PDFINFO"], "pdfinfo")
  if (executable) {
    const dir = await fs.mkdtemp(path.join(Instance.directory, ".chipmate-pdf-info-"))
...
function materializeFieldsInDocument(
  ...
  if (placeholders.length) {
    ...
    const headings = parseTopLevelBlocks(xml)
      ...
      .map((block, index) => ({ ...block, bookmark: `_ChipMateToc${index + 1}`, bookmarkID: 10_000 + index }))
    xml = splice(
      xml,
      placeholder.start,
      placeholder.end,
