...
This review records the current source-level evidence for the ChipMate non-QA feature migration into Kilo. It is intentionally limited to static inspection and does not claim test, package, or installed VS Code validation.
...
- Agent Terminal
- Kilo native QA, Document RAG, autocomplete, terminal/session manager boundaries

## Static Evidence

### Kilo native QA tools remain registered

Evidence:

- `packages/opencode/src/kilocode/tool/registry.ts` still tells the agent to use `codebase_analysis`, `semantic_search`, and `document_search` for code/document understanding.
- `packages/opencode/test/kilocode/tool-registry-indexing.test.ts` contains regression coverage expecting `codebase_analysis`, `semantic_search`, and `document_search` when indexing is ready.
...
- No source-level evidence that ChipMate QA planner, question routing, or CodeGraph/RAG management replaced Kilo native QA.
...
- Artifact root is `.kilo/artifacts`.
- VS Code artifact commands read `kilo.documents.artifacts.root` for browse/export diagnostics, while current tool-generated artifacts still default to `.kilo/artifacts`.
- VS Code artifact commands respect `kilo.documents.tools.enabled`; disabling this setting blocks artifact browsing/export commands without touching native QA tools.
- Root `.gitignore` ignores `.kilo/artifacts/` and `.kilocode/artifacts/`, so generated document artifacts should not pollute tracked source by default.
...
- The current render runtime is driven by explicit tool parameters or `KILO_WORD_RENDER_ENDPOINT`; VS Code `wordRender` settings are not used to replace Kilo's native session configuration chain.
...
- Word sidecar tool operation failures are wrapped as ordinary tool results with `failed` metadata and readable output, after the permission prompt has completed. This keeps explicit Word failures separate from Kilo native QA routing.
...
- Word generation/edit/render tools do not replace Kilo native `document_search`.
...
### Source-backed detailed design is a Kilo-native skill

Evidence:

- `.kilo/skills/source-backed-detail-design/SKILL.md` allows Kilo native `codebase_analysis`, `semantic_search`, and `document_search`.
...
- `kilo.agentTerminal.enabled` default is `false`.
- `registerAgentTerminal` only adds `kilo.agentTerminal.open` and a terminal profile provider.
...
- Helper text says it does not replace Kilo native shell/tool loop.
- Existing Agent Manager and terminal action registration remain in `extension.ts`.

Static conclusion:

- Agent Terminal does not replace Kilo terminal/session manager in source structure.
...
- `package.json` still exposes `kilo.autocomplete.provider` with `qwen-direct`.
- New document and Agent Terminal settings do not change autocomplete provider defaults.

Static conclusion:

- No source-level evidence that the migration replaced Kilo autocomplete/Qwen direct.
...
- The static dependency scan found no newly bundled LibreOffice, Chromium, Mermaid CLI, or Poppler renderer dependency for the migrated Word/Mermaid tools. The existing internal-offline VSIX build script may still bundle Poppler `pdftotext` for Kilo's pre-existing document extraction path; runtime packaging review must distinguish that from newly introduced Word/Mermaid renderer dependencies.
