...
process.env.CHIPMATE_SESSION_RETRY_LIMIT = "2"
...
afterEach(() => {
  delete process.env.CHIPMATE_SESSION_RETRY_LIMIT
})
...
describe("session processor retry limit", () => {
  ...
  it.effect("only positive integers enable the limit", () =>
    Effect.promise(async () => {
      ...
      delete process.env.CHIPMATE_SESSION_RETRY_LIMIT
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.CHIPMATE_SESSION_RETRY_LIMIT = "0"
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.CHIPMATE_SESSION_RETRY_LIMIT = "-1"
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.CHIPMATE_SESSION_RETRY_LIMIT = "abc"
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.CHIPMATE_SESSION_RETRY_LIMIT = "2"
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBe(2)
    }),
  )
  ...
  it.effect("reads env at access time (dynamic getter)", () =>
    Effect.promise(async () => {
      ...
      delete process.env.CHIPMATE_SESSION_RETRY_LIMIT
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBeUndefined()
      process.env.CHIPMATE_SESSION_RETRY_LIMIT = "5"
      expect(Flag.CHIPMATE_SESSION_RETRY_LIMIT).toBe(5)
      delete process.env.CHIPMATE_SESSION_RETRY_LIMIT
    }),
  )
  ...
  it.effect("limits only summary or large-payload retries by default", () =>
    Effect.sync(() => {
      ...
      expect(ChipMateSessionProcessor.retryOpts({ ...base, bytes: 127_999 }).limit).toBeUndefined()
      expect(ChipMateSessionProcessor.retryOpts({ ...base, bytes: 128_000 }).limit).toBe(2)
      expect(ChipMateSessionProcessor.retryOpts({ ...base, bytes: 1, summary: true }).limit).toBe(2)
    }),
  )
  ...
  it.effect("converts only large retryable 5xx failures into compaction", () =>
    Effect.sync(() => {
      ...
      expect(ChipMateSessionProcessor.compact({ error, bytes: 128_000, auto: true })).toBe(true)
      expect(ChipMateSessionProcessor.compact({ error, bytes: 127_999, auto: true })).toBe(false)
      expect(ChipMateSessionProcessor.compact({ error, bytes: 128_000, auto: false })).toBe(false)
    }),
  )
})
