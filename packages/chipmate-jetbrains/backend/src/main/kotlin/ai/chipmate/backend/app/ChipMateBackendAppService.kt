...
import ai.chipmate.backend.cli.CliServer
import ai.chipmate.backend.cli.ChipMateBackendCliManager
import ai.chipmate.backend.cli.ChipMateCliDataParser
...
import ai.chipmate.backend.workspace.ChipMateBackendWorkspaceManager
import ai.chipmate.jetbrains.api.client.DefaultApi
import ai.chipmate.jetbrains.api.infrastructure.ClientError
import ai.chipmate.jetbrains.api.infrastructure.ClientException
import ai.chipmate.jetbrains.api.infrastructure.ServerError
import ai.chipmate.jetbrains.api.infrastructure.ServerException
import ai.chipmate.jetbrains.api.model.ConfigWarnings200ResponseInner
...
import ai.chipmate.jetbrains.api.model.ProviderOauthAuthorizeRequest
import ai.chipmate.jetbrains.api.model.ProviderOauthCallbackRequest
...
class ChipMateBackendAppService private constructor(
  ...
  private val loadTimeoutMs: Long,
    ...
    constructor(cs: CoroutineScope) : this(
        cs,
        ChipMateBackendCliManager(),
      ChipMateLog.create(ChipMateBackendAppService::class.java),
        APP_LOAD_TIMEOUT_MS,
    )
    ...
    companion object {
        ...
        ) = ChipMateBackendAppService(cs, server, log, loadTimeoutMs)
    }

    private val mutex = Mutex()
    private val connection = ChipMateConnectionService(cs, server, onReconnect = {
        cs.launch { reconnect() }
    ...
    private val _appState = MutableStateFlow<ChipMateAppState>(ChipMateAppState.Disconnected)
    val appState: StateFlow<ChipMateAppState> = _appState.asStateFlow()
    ...
    val sessions = ChipMateBackendSessionManager(cs, log)
    val chat = ChipMateBackendChatManager(cs, log)
    val models = ChipMateBackendModelStateManager(log)
    val workspaces = ChipMateBackendWorkspaceManager(cs, sessions, log)
    @Volatile var profile: ChipMateProfile200Response? = null
        private set

    @Volatile var config: ConfigDto? = null
        private set

    @Volatile var notifications: List<ChipMateNotifications200ResponseInner> = emptyList()
        private set
    ...
    suspend fun connect() {
        mutex.withLock {
            ...
            if (current is ChipMateAppState.Ready || current is ChipMateAppState.Downloading || current is ChipMateAppState.Connecting || current is ChipMateAppState.Loading || current is ChipMateAppState.MigrationRequired) return
            ensureWatcher()
            connection.connect()
        }
    }
    ...
    suspend fun retry() {
        mutex.withLock {
            when (val current = _appState.value) {
                ChipMateAppState.Disconnected -> {
                    ensureWatcher()
                    connection.connect()
                }
                is ChipMateAppState.Downloading,
                ChipMateAppState.Connecting,
                is ChipMateAppState.Loading -> Unit
                is ChipMateAppState.MigrationRequired -> {
                    log.info("retry: rerunning migration detection")
                    load()
                }
                is ChipMateAppState.Ready -> {
                    if (current.data.warnings.isEmpty()) return
                    log.info("retry: refreshing config warnings")
                    refreshConfigState()
                    val next = _appState.value
                    val warns = (next as? ChipMateAppState.Ready)?.data?.warnings
                    if (next is ChipMateAppState.Ready && warns.isNullOrEmpty()) return
                    restartConnection("warnings remained after refresh")
                }
                is ChipMateAppState.Error -> {
    ...
    fun requireReady() {
        when (_appState.value) {
            is ChipMateAppState.Ready -> return
            is ChipMateAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            else -> throw IllegalStateException("ChipMate backend is not ready")
        }
    }
    ...
    suspend fun awaitReady(timeoutMs: Long = READY_TIMEOUT_MS) {
        when (_appState.value) {
            ...
            is ChipMateAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            is ChipMateAppState.Downloading,
            is ChipMateAppState.Loading,
            ChipMateAppState.Connecting -> {
                val state = withTimeoutOrNull(timeoutMs) {
                    appState.first { it !is ChipMateAppState.Downloading && it !is ChipMateAppState.Loading && it !is ChipMateAppState.Connecting }
                }
                when (state) {
                    is ChipMateAppState.Ready -> return
                    is ChipMateAppState.MigrationRequired -> throw IllegalStateException("Migration required")
    ...
    suspend fun updateConfig(patch: ConfigPatchDto): ChipMateAppState {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val current = _appState.value as? ChipMateAppState.Ready ?: throw IllegalStateException("ChipMate backend is not ready")
        val connected = connection.state.value as? ConnectionState.Connected
        val body = ChipMateCliDataParser.buildConfigPatch(patch)
        ...
        if (cfg == null) {
            ...
            return (_appState.value as? ChipMateAppState.Ready) ?: current
        }
        val warns = fetchWarnings()
        val state = _appState.value
        if (state is ChipMateAppState.Ready && state.data === current.data && connection.state.value == connected) {
            config = cfg
    ...
    internal suspend fun resumeAfterMigration() {
        mutex.withLock {
            if (_appState.value !is ChipMateAppState.MigrationRequired) return
    ...
    private suspend fun reconnect() {
        mutex.withLock {
            val current = _appState.value
            if (current is ChipMateAppState.Ready || current is ChipMateAppState.Downloading || current is ChipMateAppState.Loading || current is ChipMateAppState.MigrationRequired) {
    ...
    private fun ensureWatcher() {
        ...
        watcher = cs.launch {
            connection.state.collect { next ->
                when (next) {
                    ConnectionState.Disconnected -> _appState.value = ChipMateAppState.Disconnected
                    is ConnectionState.Downloading -> _appState.value = ChipMateAppState.Downloading(next.percent, next.version, next.platform)
                    ConnectionState.Connecting -> _appState.value = ChipMateAppState.Connecting
     ...
     *
     * Progress is tracked via [LoadProgress] and emitted as [ChipMateAppState.Loading].
     * On success, transitions to [ChipMateAppState.Ready].
     * On failure of required data, transitions to [ChipMateAppState.Error].
     */
    private fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                _appState.value = ChipMateAppState.Loading(progress.get())
                ...
                if (migration != null) {
                    ...
                    warnings = emptyList()
                    _appState.value = ChipMateAppState.MigrationRequired(migration)
                ...
                var prof: ChipMateProfile200Response? = null
                var notifs: List<ChipMateNotifications200ResponseInner> = emptyList()
                    ...
                    withTimeout(loadTimeoutMs) {
                        ...
                        launch {
                            ...
                            progress.updateAndGet { it.copy(profile = status) }
                                .also { _appState.value = ChipMateAppState.Loading(it) }
                        }
                        launch {
    ...
    private fun captureLoad(event: String, start: Long, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<ChipMateBackendTelemetry>().capture(
                    http,
                    port,
                    event,
    ...
    private fun setTelemetry(enabled: Boolean) {
        ...
        cs.launch {
            runCatching {
                service<ChipMateBackendTelemetry>().setEnabled(http, port, enabled)
    ...
    private fun captureBackend(event: String, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<ChipMateBackendTelemetry>().capture(http, port, event, props)
    ...
    private suspend fun detectMigration(): LegacyMigrationDetection? = withContext(Dispatchers.IO) {
        try {
            ...
            else ChipMateBackendLegacyMigrationStoreService.status(log)
            ...
            val source = ChipMateBackendLegacyMigrationStoreService.resolveSource(log, includeFile = migrationForceRequested)
            val store = source.store
            val detection = ChipMateBackendMigrationManager(http, connection.port).detect(store)
     ...
     * as failures.
     */
    private suspend fun fetchProfile(): FetchResult<ChipMateProfile200Response?> {
        val client = connection.appLoadApi
            ?: return FetchResult.ok(null)
        return try {
            val response = client.chipmateProfile()
    ...
    private suspend fun fetchConfig(): FetchResult<ConfigDto> {
        ...
        return try {
            ...
            FetchResult.ok(ChipMateCliDataParser.parseConfig(body))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchNotifications(): FetchResult<List<ChipMateNotifications200ResponseInner>> {
        val client = connection.appLoadApi
            ?: return FetchResult.fail("notifications", detail = "Not connected")
        return try {
            FetchResult.ok(client.chipmateNotifications())
        } catch (e: Exception) {
    ...
    private suspend fun refreshConfigState() {
        val current = _appState.value as? ChipMateAppState.Ready ?: return
        val connection = connection.state.value as? ConnectionState.Connected ?: return
        val cfg = fetchConfig().value ?: return
        val warns = fetchWarnings()
        val state = _appState.value
        if (state !is ChipMateAppState.Ready || state.data !== current.data) return
    ...
    private fun setAppReady(data: AppData) {
        ...
        _appState.value = ChipMateAppState.Ready(data, rev.incrementAndGet())
    }

    private fun setAppError(message: String, errors: List<LoadError>) {
        val state = ChipMateAppState.Error(message, errors)
        warnAppError(state)
        _appState.value = state
    }

    private fun warnAppError(state: ChipMateAppState.Error) {
    ...
    private suspend fun awaitLoadResult(prev: ChipMateAppState): ChipMateAppState {
        val next = appState.first { it !== prev }
        if (next !is ChipMateAppState.Loading) return next
        return appState.first { it !is ChipMateAppState.Loading }
    }
     ...
     *   Re-fetches config and updates [ChipMateAppState.Ready] data in-place.
     *
                ...
                connection.events.collect { event ->
                    when (event.type) {
                        ...
                        "global.disposed" -> {
                            ...
                            val current = _appState.value
                            if (current is ChipMateAppState.Ready) load()
                        }
    ...
    private fun reset() {
        ...
        notifications = emptyList()
        warnings = emptyList()
        _appState.value = ChipMateAppState.Disconnected
    }
     ...
     * Updates the current [ChipMateAppState.Ready] profile in-place if the app is ready.
     */
    suspend fun refreshProfile(): ChipMateProfile200Response? {
        val result = fetchProfile()
        val fresh = result.value
        val current = _appState.value
        if (current is ChipMateAppState.Ready) {
    ...
    /**
     * Start the ChipMate device auth login flow.
    ...
    suspend fun startLogin(directory: String?): DeviceAuthDto {
        ...
        val response = client.providerOauthAuthorize(providerID = "chipmate", directory = directory, providerOauthAuthorizeRequest = body)
    ...
    /**
     * Complete the ChipMate device auth login flow.
     ...
     */
    suspend fun completeLogin(directory: String?): ChipMateProfile200Response? {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        client.providerOauthCallback(providerID = "chipmate", directory = directory, providerOauthCallbackRequest = ProviderOauthCallbackRequest(method = 0.0))
        return refreshProfile()
    }

    /**
     * Log out from ChipMate Gateway.
     * Removes credentials and clears the profile from app state.
     */
    suspend fun logout(): Boolean {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        val result = client.authRemove(providerID = "chipmate")
        val current = _appState.value
        if (current is ChipMateAppState.Ready) {
            profile = null
     ...
     */
    suspend fun setOrganization(organizationId: String?): ChipMateProfile200Response? {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val body = JsonObject(
            mapOf("organizationId" to (organizationId?.let { JsonPrimitive(it) } ?: JsonNull)),
        ).toString()
        val request = Request.Builder()
            .url("http://127.0.0.1:$port/chipmate/organization")
            .header("Accept", "application/json")
