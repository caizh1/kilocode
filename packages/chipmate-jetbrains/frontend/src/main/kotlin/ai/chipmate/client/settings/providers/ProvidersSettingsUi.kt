...
import ai.chipmate.client.app.ChipMateProviderService
import ai.chipmate.client.plugin.ChipMateBundle
import ai.chipmate.client.settings.base.BaseContentPanel
import ai.chipmate.client.settings.base.SettingsPanel
...
import ai.chipmate.client.ui.picker.PickerListRenderer
import ai.chipmate.client.ui.picker.PickerPopup
import ai.chipmate.log.ChipMateLog
import ai.chipmate.rpc.dto.CustomModelDto
...
import ai.chipmate.rpc.dto.ProviderAuthMethodDto
import ai.chipmate.rpc.dto.ProviderAuthOptionDto
...
internal fun customSaveError(id: String, result: ProviderActionResultDto): String? {
    ...
    if (result.state.providers.none { it.id == id }) return ChipMateBundle.message("settings.providers.customNotUsable")
    return null
}
...
internal class ProvidersSettingsUi(
    ...
    companion object {
        val LOG = ChipMateLog.create(ProvidersSettingsUi::class.java)
    }

    private val add = SettingsToolbarAction(
        ChipMateBundle.message("settings.providers.addCustom"),
        ChipMateBundle.message("settings.providers.addCustom.description"),
        AllIcons.General.Add,
        { !busy },
    ) { custom() }
    private val refresh = SettingsToolbarAction(
        ChipMateBundle.message("settings.providers.refresh"),
        ChipMateBundle.message("settings.providers.refresh.description"),
        AllIcons.Actions.Refresh,
        { !busy },
    ) { reload() }
    private val view = ProvidersContent(::connect, ::oauth, ::disconnect, ::enable, ::edit)
    private val search = SearchTextField(false).apply {
        textEditor.emptyText.text = ChipMateBundle.message("settings.providers.search")
    }
    ...
    fun reload() {
        ...
        if (!launch("reload") { id ->
            val next = service<ChipMateProviderService>().state(directory)
    ...
    private fun syncLoading() {
        checkEdt()
        showProgress(ChipMateBundle.message("settings.providers.loading"))
    }
    ...
    private fun connect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("connect provider=${provider.id}") { id ->
            val result = service<ChipMateProviderService>().connect(ProviderConnectDto(directory, provider.id, key, metadata))
    ...
    private fun oauth(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("authorize provider=${provider.id}") { id ->
            val ready = service<ChipMateProviderService>().authorize(ProviderOAuthAuthorizeDto(directory, provider.id, method))
                    ...
                    if (ready.method == "auto" && url != null) {
                        showOAuthDevice(
                            ...
                            DeviceOAuthInfo(
                                ...
                                expiresIn = (ChipMateProviderService.OAUTH_RPC_TIMEOUT_MS / 1000).toInt(),
            ...
            val result = service<ChipMateProviderService>().callback(ProviderOAuthCallbackDto(directory, provider.id, method, code))
            apply(id, result.state, result.error)
        }) return
        showProgress(
            ChipMateBundle.message("settings.providers.oauth.starting", provider.name),
            ChipMateBundle.message("settings.providers.oauth.cancel"),
        ) { cancelOAuth(request) }
    }
    ...
    private fun disconnect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("disconnect provider=${provider.id}") { id ->
            val result = service<ChipMateProviderService>().disconnect(ProviderDisconnectDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun enable(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("enable provider=${provider.id}") { id ->
            val result = service<ChipMateProviderService>().enable(ProviderEnableDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun openCustomDialog(existing: CustomProviderEdit?) {
        ...
        val dialog = CustomProviderDialog(
            cs,
            directory,
            { service<ChipMateProviderService>().fetchCustomModels(it) },
            { service<ChipMateProviderService>().saveCustom(it) },
            existing,
        )
        if (!dialog.showAndGet()) return
    ...
    private fun syncOAuthWaiting(id: Int) {
        checkEdt()
        if (!active(id)) return
        val expiry = System.currentTimeMillis() + ChipMateProviderService.OAUTH_RPC_TIMEOUT_MS
        fun text(): String {
            val ms = (expiry - System.currentTimeMillis()).coerceAtLeast(0)
            val remain = ((ms + 999) / 1000).toInt()
            val min = remain / 60
            val sec = remain % 60
            return ChipMateBundle.message("settings.providers.oauth.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
        }
        stopTimer()
        showProgress(text(), ChipMateBundle.message("settings.providers.oauth.cancel")) { cancelOAuth(id) }
        timer = Timer(1000) {
            if (!active(id)) {
                stopTimer()
                return@Timer
...
internal class ProvidersContent(
    ...
    private val view = SettingsListView(ChipMateBundle.message("settings.providers.noMatches"), SettingsListConfig.Preferred) { key, id ->
        activate(key, id)
    }
    private var state = ProviderSettingsDto()
    private var busy = false
...
private class ApiKeyDialog(title: String, method: ProviderAuthMethodDto?) : DialogWrapper(true) {
    ...
    override fun createCenterPanel(): JComponent {
        ...
        panel.next(JBLabel(ChipMateBundle.message("settings.providers.apiKey")))
        panel.next(key)
    ...
    override fun doValidate(): ValidationInfo? {
        if (key().isBlank()) return ValidationInfo(ChipMateBundle.message("settings.providers.apiKeyRequired"), key)
        return null
    }
...
internal class CustomProviderDialog(
    ...
    private val pick = JButton(ChipMateBundle.message("settings.providers.customSelectModels"))
    ...
    init {
        title = if (existing != null) {
            ChipMateBundle.message("settings.providers.customEditTitle")
        } else {
            ChipMateBundle.message("settings.providers.customTitle")
        }
        setOKButtonText(
            if (existing != null) ChipMateBundle.message("settings.providers.customSave")
            else ChipMateBundle.message("settings.providers.customAdd"),
        )
        init()
        initValidation()
    ...
    override fun createCenterPanel(): JComponent {
        val panel = Stack.vertical(UiStyle.Gap.sm())
        listOf(
            ChipMateBundle.message("settings.providers.customId") to id,
            ChipMateBundle.message("settings.providers.customName") to name,
            ChipMateBundle.message("settings.providers.customUrl") to url,
            ChipMateBundle.message("settings.providers.apiKey") to key,
            ChipMateBundle.message("settings.providers.customEnv") to env,
        ).forEach { (label, field) ->
            panel.next(JBLabel(label))
            panel.next(field)
        }
        panel.next(JBLabel(ChipMateBundle.message("settings.providers.customModels")))
    ...
    override fun doValidate(): ValidationInfo? {
        if (id.text.isBlank()) return ValidationInfo(ChipMateBundle.message("settings.providers.customIdRequired"), id)
        if (url.text.isBlank()) return ValidationInfo(ChipMateBundle.message("settings.providers.customUrlRequired"), url)
        actionError?.let { return ValidationInfo(it) }
        if (!fetching && modelIds().isEmpty()) return ValidationInfo(ChipMateBundle.message("settings.providers.customModelsRequired"), models)
        return null
    }
    ...
    private fun selectModels() {
        ...
        job = cs.launch {
            ...
            withContext(edt) {
                ...
                if (ids.isEmpty()) {
                    fail(ChipMateBundle.message("settings.providers.customModelsEmpty"))
                    return@withContext
                }
                showModelPopup(ids)
            }
    ...
    private fun startFetch() {
        ...
        models.text = ChipMateBundle.message("settings.providers.customFetchingModels")
        syncActions()
    }
    ...
    private fun fetchValidationError(): String? {
        if (url.text.isBlank()) return ChipMateBundle.message("settings.providers.customUrlRequired")
        if (!url.text.trim().let { it.startsWith("http://") || it.startsWith("https://") }) return ChipMateBundle.message("settings.providers.customUrlInvalid")
        return null
    }
    ...
    private fun showModelPopup(ids: List<String>) {
        ...
        val select = ActionLink(ChipMateBundle.message("settings.providers.customModelsSelectAll"))
        val clear = ActionLink(ChipMateBundle.message("settings.providers.customModelsUnselectAll"))
    ...
    private fun syncActions() {
        ...
        pick.text = if (fetching) {
            ChipMateBundle.message("settings.providers.customCancelModels")
        } else {
            ChipMateBundle.message("settings.providers.customSelectModels")
        }
    }
