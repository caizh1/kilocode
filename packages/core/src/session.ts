...
import { and, asc, desc, eq, gt, isNotNull, like, lt, or, type SQL } from "drizzle-orm" // chipmate_change
...
import { normalize } from "./chipmate/session-message" // chipmate_change
import { Snapshot } from "./snapshot"
import { SessionRevert } from "./session/revert"
import { Revert } from "@opencode-ai/schema/revert"
import { FSUtil } from "./fs-util"
import { SessionDurable, type SessionDurableEvent } from "@opencode-ai/schema/durable-event-manifest" // chipmate_change
...
export interface Interface {
  ...
  readonly events: (input: {
    sessionID: SessionSchema.ID
    after?: number
  }) => Stream.Stream<SessionDurableEvent, NotFoundError> // chipmate_change - released durable event compatibility
  readonly history: (input: {
    sessionID: SessionSchema.ID
    after?: number
    limit: number
  }) => Effect.Effect<{ events: ReadonlyArray<SessionDurableEvent>; hasMore: boolean }, NotFoundError> // chipmate_change
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const isDurableSessionEvent = Schema.is(SessionDurable.schema) // chipmate_change - include released storage keys
    const decode = (row: typeof SessionMessageTable.$inferSelect) =>
      decodeMessage(normalize({ ...row.data, id: row.id, type: row.type })).pipe(
        // chipmate_change - normalize released tool content on paginated reads
        Effect.mapError(
          () =>
    ...
    const result = Service.of({
      ...
      messages: Effect.fn("V2Session.messages")(function* (input) {
          ...
          ? yield* db
              ...
              .where(
                and(
                  ...
                  isNotNull(SessionMessageTable.seq), // chipmate_change
                ),
              )
              .get()
              .pipe(Effect.orDie)
        ...
        const where = boundary
          ...
          : and(eq(SessionMessageTable.session_id, input.sessionID), isNotNull(SessionMessageTable.seq)) // chipmate_change
        const query = db
          .select()
          .from(SessionMessageTable)
          .where(where)
      ...
      events: (input) =>
        ...
        ).pipe(Stream.filter((event): event is SessionDurableEvent => isDurableSessionEvent(event))), // chipmate_change
