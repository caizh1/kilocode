...
import { cycle, next, sequence, variants } from "../../../kilo-ui/src/components/spinner-sequence"

const root = path.resolve(import.meta.dir, "../../..")
const component = fs.readFileSync(path.join(root, "kilo-ui/src/components/dynamic-spinner.tsx"), "utf8")
const styles = fs.readFileSync(path.join(root, "kilo-ui/src/components/dynamic-spinner.css"), "utf8")
const assets = path.join(root, "kilo-vscode/assets/loading-motion")
const indicator = fs.readFileSync(
  path.join(root, "kilo-vscode/webview-ui/src/components/shared/WorkingIndicator.tsx"),
  "utf8",
)
const layers = {
...
describe("dynamic spinner component contract", () => {
  ...
  it("uses compositor motion for Orbital, Signal, and Prism", () => {
    ...
    expect(styles).toContain("kilo-signal-fold")
    expect(styles).toContain("kilo-prism-a")
    expect(styles).not.toContain("kilo-orbital-outer")
