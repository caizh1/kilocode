...
export function migrate(info: typeof ConfigV1.Info.Type) {
  return {
    $schema: info.$schema,
    shell: info.shell,
    model: info.model ?? undefined, // chipmate_change - v1 null delete sentinel is not valid in v2
    default_agent: info.default_agent ?? undefined, // chipmate_change
...
function permissions(info?: ConfigPermissionV1.Info, tools?: Readonly<Record<string, boolean>>) {
  ...
  for (const [action, rule] of Object.entries(info ?? {})) {
    ...
    // chipmate_change - per-resource effect may also be null (delete sentinel); skip those entries
    rules.push(
      ...Object.entries(rule)
...
// chipmate_change - v1 fields are nullable (delete sentinel); the v2 format has no such concept, so null collapses to undefined
...
function providers(info?: Readonly<Record<string, ConfigProviderV1.Info | null>>) {
  ...
  // chipmate_change - provider entries may be null (delete sentinel); migration has nothing to convert for those
...
function migrateProvider(info: ConfigProviderV1.Info) {
  ...
  return {
    ...
    // chipmate_change - model entries may be null (delete sentinel); migration has nothing to convert for those
    models:
      info.models &&
      Object.fromEntries(
...
function migrateModel(info: typeof ConfigProviderV1.Model.Type, packageName?: string) {
  ...
  return {
    ...
    // chipmate_change - variant entries may be null (delete sentinel); migration has nothing to convert for those
    variants:
      info.variants &&
