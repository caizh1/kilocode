...
import { ChipMateHeadless } from "../../src/chipmate/permission/headless"
import { ChipMateSessionPrompt } from "../../src/chipmate/session/prompt"
import { ChipMateReadObject } from "../../src/chipmate/tool/read-object"
...
it.live("recognizes Windows named-pipe paths before filesystem inspection", () =>
  Effect.sync(() => {
    expect(ChipMateReadObject.namedPipe("\\\\.\\pipe\\secret")).toBe(true)
    expect(ChipMateReadObject.namedPipe("\\\\server\\pipe\\secret")).toBe(true)
    expect(ChipMateReadObject.namedPipe("C:\\project\\secret.txt")).toBe(false)
  }),
)
...
it.live(
  "blocks @file content denied by .chipmateignore",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "CHIPMATE_12133_MENTION_SENTINEL"
        yield* Effect.promise(() =>
          Promise.all([
            Bun.write(path.join(dir, "my_file.txt"), sentinel),
            Bun.write(path.join(dir, ".chipmateignore"), "my_file.txt\n"),
          ]),
        )
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "CHIPMATE_12133_ASK_SENTINEL"
        const denied = "CHIPMATE_12133_REPLACEMENT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "CHIPMATE_12133_APPROVED_SENTINEL"
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir, llm }) {
        const sentinel = "CHIPMATE_12133_ABORT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const sentinel = "CHIPMATE_12133_COMMAND_ABORT_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const allowed = "CHIPMATE_12133_ALLOWED_BINARY_SENTINEL"
        const denied = "CHIPMATE_12133_REPLACEMENT_BINARY_SENTINEL"
...
it.live(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const allowed = "CHIPMATE_12133_ALLOWED_FILE_SENTINEL"
        const denied = "CHIPMATE_12133_DENIED_INSTRUCTION_SENTINEL"
        ...
        yield* Effect.promise(() =>
          Promise.all([
            ...
            Bun.write(path.join(dir, ".chipmateignore"), "nested/AGENTS.md\n"),
          ]),
        )
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const nested = "CHIPMATE_12133_DIRECTORY_SENTINEL"
        const direct = "CHIPMATE_12133_BINARY_SENTINEL"
        const fs = yield* FSUtil.Service
...
symlinkIt(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const text = "CHIPMATE_12133_SYMLINK_TEXT_SENTINEL"
        const binary = "CHIPMATE_12133_SYMLINK_BINARY_SENTINEL"
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const outside = path.join(os.tmpdir(), `chipmate-12133-${crypto.randomUUID()}.txt`)
        const sentinel = "CHIPMATE_12133_REFERENCE_SYMLINK_SENTINEL"
        const fs = yield* FSUtil.Service
...
symlinkIt(
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        const outside = path.join(os.tmpdir(), `chipmate-12133-missing-${crypto.randomUUID()}`)
        const fs = yield* FSUtil.Service
  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir }) {
        ...
        const outside = path.join(os.tmpdir(), `chipmate-12133-directory-${crypto.randomUUID()}`)
        const fs = yield* FSUtil.Service
        yield* fs.ensureDir(folder)
        yield* fs.ensureDir(outside)
        yield* Effect.addFinalizer(() => Effect.promise(() => rm(outside, { recursive: true, force: true })))
...
it.live("headless run: subagent permission asks fail instead of waiting forever", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      expect(yield* permission.list()).toEqual([])
      expect(yield* ChipMateHeadless.denies(child.id)).toBe(true)
      expect(yield* ChipMateHeadless.denies(root.id)).toBe(false)

      ChipMateHeadless.clear(root.id)
    }),
    { git: true },
  ),
)
...
it.live("interactive run: subagent permission asks still queue for a human reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      const fiber = yield* ChipMateSessionPrompt.askPermission({
        permission,
        agents: { get: () => Effect.succeed(agent) },
        sessions,
        agent,
