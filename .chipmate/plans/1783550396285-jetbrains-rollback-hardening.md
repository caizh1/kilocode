...
All work is inside `packages/chipmate-jetbrains/`. No shared `packages/opencode/` files change,
so no `chipmate_change` markers are needed.
...
- Test harnesses already exist: frontend `SessionControllerTestBase` + `FakeSessionRpcApi`
  ...
  `ChipMateBackendChatManagerTest`. Banner tests instantiate `RevertBanner(model, redo, redoAll)`
...
- `frontend/src/main/resources/messages/ChipMateBundle.properties` (+ translated `ChipMateBundle_*.properties`)
- `backend/.../app/ChipMateBackendChatManager.kt` (`revert`/`unrevert`), `backend/.../cli/ChipMateCliDataParser.kt` (`buildRevertJson`)
- `backend/.../app/ChipMateBackendSessionManager.kt` (`revertDto` duplication)
- Tests: `frontend/.../session/controller/TurnLifecycleTest.kt` (or new `RevertFlowTest.kt`),
  ...
  `backend/.../testing/MockCliServer.kt`, `backend/.../app/ChipMateBackendChatManagerTest.kt`,
  `backend/.../cli/ChipMateCliDataParserTest.kt`
...
1. Add bundle key in `ChipMateBundle.properties` (place near the other `revert.banner.*` keys):
   - `revert.banner.filesNotRestored=Snapshots are off — only the conversation was reverted; your files were not changed.`
   - Add the same key to every translated `ChipMateBundle_*.properties` file. If translations are
...
2. In `RevertBanner.kt`:
   ...
   - In `update()`, after computing `revert`, set
     ...
     `ChipMateBundle.message("revert.banner.filesNotRestored")`.
...
4. In `ChipMateBackendSessionManager.kt`, collapse the two identical `revertDto(...)` overloads
   ...
   a pure refactor covered by existing `ChipMateBackendSessionManagerTest` parsing assertions.
...
9. `MockCliServer.kt` — add routing + capture fields (the current generic `/session/ses_*`
   ...
   - Cases (place beside the `summarize`/`prompt_async` cases):
     ...
     - `bare.matches(Regex("/session/ses_[^/]+/unrevert")) && method == "POST"` → store path+body,
       `respond(output, 200, sessionCreate)`.
10. `ChipMateBackendChatManagerTest.kt` — mirror the existing `compact posts summarize request` test:
...
11. `ChipMateCliDataParserTest.kt` — add `buildRevertJson` unit tests:
...
12. Remove the unused bundle key `revert.disabled.agentBusy` from `ChipMateBundle.properties` and all
...
- Targeted while iterating: run the touched test classes (`TurnLifecycleTest`/`RevertFlowTest`,
  `SessionMessageListPanelTest`, `SessionModelTest`, `ChipMateBackendChatManagerTest`,
  `ChipMateCliDataParserTest`).
...
Add `.changeset/<slug>.md` (`"@chipmate/chipmate-jetbrains": patch`) describing the user-facing
