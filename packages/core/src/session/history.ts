import { and, asc, desc, eq, gt, gte, isNotNull, ne, or } from "drizzle-orm" // chipmate_change
...
// chipmate_change - normalize released storage shapes only at the assistant tool-state boundary
...
export const entriesForRunner = Effect.fn("SessionHistory.entriesForRunner")(function* (
  ...
  return yield* Effect.forEach(rows, (row) => {
    const seq = row.seq
    if (seq === null) return Effect.die("Sequenced session history returned a legacy row") // chipmate_change
