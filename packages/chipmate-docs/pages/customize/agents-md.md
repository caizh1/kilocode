...
The ChipMate **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.chipmate/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask ChipMate to do it for you)
{% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including ChipMate, Cursor, and Windsurf.
...
Both `AGENTS.md` and `AGENT.md` are **write-protected files** in ChipMate. This means:
...
## How AGENTS.md Works in ChipMate

### Loading Behavior

When you start a task in ChipMate:

1. ChipMate checks for `AGENTS.md` or `AGENT.md` at the project root
...
| **[Instructions](/docs/customize/custom-rules)** | Project | `instructions` key in project `chipmate.jsonc` | 2 |
| **AGENTS.md** | Project | `AGENTS.md` at project root | 3 |
| **[Instructions](/docs/customize/custom-rules)** | Global | `instructions` key in global `chipmate.jsonc` | 4 |
| **[Skills](/docs/customize/skills)** | Both | `.chipmate/skills/`, config `skills` key | Loaded on demand |
...
```bash
export CHIPMATE_DISABLE_EXTERNAL_SKILLS=true
```
...
- **[Custom Rules](/docs/customize/custom-rules)** - ChipMate-specific rules with more control
