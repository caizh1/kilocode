...
description: "Architecture of the ChipMate VS Code extension and Agent Manager"
---

# VS Code Extension Architecture

The VS Code extension (`packages/chipmate-vscode/`) is a client of [ChipMate CLI runtime](/docs/contributing/architecture/cli-runtime). It bundles platform CLI binary, starts one shared editor-owned `chipmate serve` server on demand, and drives that server through generated SDK HTTP calls plus global SSE.
...
Activation creates one `ChipMateConnectionService`. It owns one `ServerManager`, one active SDK client, and one SSE adapter. `ServerManager` owns child process lifecycle. This editor-owned child is separate from detached local daemon managed by `chipmate daemon`.

```mermaid
flowchart LR
  subgraph host ["VS Code extension host"]
    consumers["Sidebar, tabs, panels, services"]
    service["ChipMateConnectionService"]
    manager["ServerManager"]
    sdk["Generated SDK client"]
    sse["SdkSSEAdapter"]
  end

  server["bin/chipmate serve --port 0"]
...
| Binary | Uses extension `bin/chipmate`, or `bin/chipmate.exe` on Windows |
| Port | Starts `chipmate serve --port 0`; CLI server prefers `4096`, then asks OS for free port |
| Authentication | Generates random 32-byte hex password per spawn and passes it as `CHIPMATE_SERVER_PASSWORD`; username defaults to `chipmate` |
...
| Panels | Settings, profile and marketplace surfaces, sub-agent viewers, Agent Manager, ChipMateClaw |
| Diff | Diff Viewer, Diff Virtual, and diff source catalog |
| Editor assistance | Autocomplete and commit-message generation |
| Integrations | Browser automation MCP registration and ChipMateClaw bootstrap |
...
webview vscode.postMessage()
  -> ChipMateProvider host handler
  -> generated SDK HTTP request
  -> CLI runtime
  -> /global/event SSE
  -> SdkSSEAdapter
  -> ChipMateConnectionService subscribers
  -> ChipMateProvider directory/session filtering and stream coalescing
  -> webview postMessage()
```
...
| Backend | Shared `chipmate serve` process | Same shared process |
...
Agent Manager persists state in `.chipmate/agent-manager.json` and worktrees under `.chipmate/worktrees/`. Startup migration moves Agent Manager-owned data from legacy `.chipmate/` paths when target items do not already exist and repairs git worktree refs.
...
| CLI PTY WebSocket tab | Agent Manager and `chipmate serve` server | Server-created PTY session streamed over loopback WebSocket |

Agent Manager PTY WebSocket URL uses `auth_token=<base64 chipmate:password>` query mode because browser WebSocket API cannot attach Basic header. Webview CSP permits loopback HTTP and WebSocket origins for active server port. CLI also exposes scope-bound short-lived PTY ticket API as alternate browser WebSocket auth mode.

## Config split

| Config owner | Examples |
|---|---|
| VS Code settings | `chipmate-code.new.*` extension UI, proxy, autocomplete, and integration settings |
| CLI config | Global and project `chipmate.jsonc`, `chipmate.json`, compatible OpenCode files, provider auth, tools, permissions, modes |
...
| Resource | Behavior |
|---|---|
| CLI executable | Platform binary under extension `bin/`; Windows uses `chipmate.exe` |
| CLI Tree-sitter WASM | Copied under `bin/tree-sitter`; backend spawn sets `CHIPMATE_TREE_SITTER_WASM_DIR` |
...
| Empty-window indexing | Sets `CHIPMATE_DISABLE_CODEBASE_INDEXING=vscode-no-workspace` so CLI reports indexing disabled |

Speech-to-text captures audio locally, then sends completed recording through shared editor-owned `chipmate serve` server to authenticated ChipMate Gateway transcription path. It is batch transcription, not direct provider streaming.
...
| ChipMateClaw webview | `webview-ui/chipmateclaw/index.tsx` | `dist/chipmateclaw.js` |
...
Extension host bundle targets Node/CommonJS. Browser webviews and shared worker use esbuild browser bundles. Run `bun run typecheck`, `bun run lint`, and targeted unit tests from `packages/chipmate-vscode/` after changing this area.
...
| Concern | Source path |
|---|---|
| Activation | `packages/chipmate-vscode/src/extension.ts` |
| Editor-owned server child process | `packages/chipmate-vscode/src/services/cli-backend/server-manager.ts` |
| Shared SDK and SSE ownership | `packages/chipmate-vscode/src/services/cli-backend/connection-service.ts` |
| SSE reconnect adapter | `packages/chipmate-vscode/src/services/cli-backend/sdk-sse-adapter.ts` |
| Agent Manager | `packages/chipmate-vscode/src/agent-manager/` |
| Build entries | `packages/chipmate-vscode/esbuild.js` |
