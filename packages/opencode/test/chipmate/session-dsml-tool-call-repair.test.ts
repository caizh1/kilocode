...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(invalid[0].state.input).toMatchObject({ tool: "chipmate_dsml_orphan_retry" })
      ...
      expect(JSON.stringify(inputs[1]?.messages)).toContain("chipmate_dsml_orphan_retry")
