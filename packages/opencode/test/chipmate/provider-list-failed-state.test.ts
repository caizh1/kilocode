...
it.live("failedProviders returns empty array when no fetch has occurred", () =>
  ModelCache.Service.use((cache) =>
    Effect.gen(function* () {
      expect(yield* cache.failedProviders()).not.toContain("chipmate")
    }),
  ...
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) =>
      Effect.gen(function* () {
        ...
        expect(yield* cache.getFailure("chipmate")).toBeUndefined()
        expect(yield* cache.failedProviders()).not.toContain("chipmate")
      }),
    ).pipe(Effect.provide(layer()))
  }),
...
it.live("gateway rejection remains recoverable through the Effect error channel", () =>
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) =>
      Effect.gen(function* () {
        const models = yield* cache.fetch("chipmate").pipe(Effect.catch(() => Effect.succeed({})))
