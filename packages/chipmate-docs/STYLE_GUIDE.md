...
description: "Guidelines for writing ChipMate documentation"
---

# Documentation Style Guide

This guide covers writing, formatting, and structuring documentation for the ChipMate docs site.

## Voice and Tone

ChipMate documentation should be:
...
- Use present tense
- Be specific: "Run `chipmate run` to execute a task" not "You can run chipmate run"
...
```markdown
{% callout type="tip" %}
**The easiest way to configure ChipMate is to ask the agent.** Just tell the agent what you want.
{% /callout %}
```
...
- Use `chipmate run` for CLI examples, not hypothetical commands

````markdown
```bash
chipmate run "create a utils.py file with a function that adds two numbers"
```
````
...
- `chipmate.jsonc` for configuration files
...
- Use generated screenshots from `packages/chipmate-docs/public/img/screenshot-tests/` when available
...
|---|---|
| `chipmate serve` | The prod CLI on `$PATH`. |
```
...
| Term | Use for |
|---|---|
| ChipMate | The product name |
| chipmate CLI | The command-line interface |
| VS Code extension | The VS Code extension |
| JetBrains plugin | The JetBrains IDE plugin |
| `chipmate serve` | The local HTTP server |
| `chipmate run` | The headless execution command |
