...
describe("ChipMate post-step compaction safety", () => {
...
describe("ChipMate request estimation", () => {
  test("skips output estimation when no output cap can use it", () => {
    const mdl = model({ context: 200_000, output: 32_000 })

    expect(ChipMateLLM.needsEstimate({ model: mdl, configured: undefined })).toBe(false)
    expect(ChipMateLLM.needsEstimate({ model: mdl, configured: 0 })).toBe(false)
    expect(ChipMateLLM.needsEstimate({ model: model({ context: 0, output: 32_000 }), configured: 32_000 })).toBe(false)
    expect(ChipMateLLM.needsEstimate({ model: mdl, configured: 32_000 })).toBe(true)
  })
  ...
  test("does not reduce output for encoded media payload size", () => {
    ...
    const usage = ChipMateSessionOverflow.measure({ messages, tools: {} })

    expect(usage.raw).toBeGreaterThan(usage.normalized)
    expect(ChipMateLLM.capOutputTokens({ model: mdl, messages, tools: {}, configured: 32_000, usage })).toBe(32_000)
  })
  ...
  test("still reduces output for oversized text", () => {
    ...
    const cap = ChipMateLLM.capOutputTokens({ model: mdl, messages, tools: {}, configured: 32_000 })
  ...
  test("prefers provider-reported context over the client estimate for images", () => {
    ...
    expect(ChipMateLLM.capOutputTokens({ model: mdl, messages, tools: {}, configured: 32_000 })).toBe(32_000)

    // With the provider-reported context size, output is capped to fit real usage.
    expect(ChipMateLLM.capOutputTokens({ model: mdl, messages, tools: {}, configured: 32_000, reported: 280_000 })).toBe(
      17_952,
    )
  })
  ...
  test("uses the media-normalized floor when reported usage is smaller", () => {
    ...
    const withoutReported = ChipMateLLM.capOutputTokens({ model: mdl, messages, tools: {}, configured: 32_000 })
    const withStaleReported = ChipMateLLM.capOutputTokens({
      model: mdl,
      messages,
      tools: {},
      configured: 32_000,
...
describe("ChipMate preflight compaction", () => {
