/**
 * Runtime contract tests for chipmate-vscode's dependencies on @chipmate/chipmate-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports chipmate-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/chipmate-ui/ where bun picks up the
...
const CHIPMATE_UI_DIR = path.join(MONOREPO_ROOT, "packages/chipmate-ui")
...
const CHIPMATE_MESSAGE_PART_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-ui/src/components/message-part.tsx")
const CHIPMATE_MESSAGE_HIGHLIGHT_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-ui/src/components/message-highlight.ts")
const CHIPMATE_BASIC_TOOL_CSS_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-ui/src/components/basic-tool.css")
const CHIPMATE_MESSAGE_PART_CSS_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-ui/src/components/message-part.css")
const SHELL_ROLLING_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-ui/src/components/shell-rolling-results.tsx")
const ASSISTANT_MESSAGE_FILE = path.join(
  MONOREPO_ROOT,
  "packages/chipmate-vscode/webview-ui/src/components/chat/AssistantMessage.tsx",
)
const TASK_HEADER_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-vscode/webview-ui/src/components/chat/TaskHeader.tsx")
const CONTEXT_TAB_FILE = path.join(
  MONOREPO_ROOT,
  "packages/chipmate-vscode/webview-ui/src/components/settings/ContextTab.tsx",
)
const PROMPT_INPUT_FILE = path.join(
  MONOREPO_ROOT,
  "packages/chipmate-vscode/webview-ui/src/components/chat/PromptInput.tsx",
)
const TRANSCRIPT_PARTS_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-vscode/webview-ui/src/utils/transcript-parts.ts")
const CHAT_LAYOUT_FILE = path.join(MONOREPO_ROOT, "packages/chipmate-vscode/webview-ui/src/styles/chat-layout.css")
...
/**
 * Tool names that chipmate-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by chipmate-vscode are registered in ToolRegistry", () => {
...
describe("DataProvider contract (runtime)", () => {
  ...
  it("DataProvider accepts onOpenFile prop and exports OpenFileFn (source)", () => {
    // onOpenFile and OpenFileFn are `chipmate_change` additions — TypeScript types
  ...
  it("DataProvider accepts onOpenDiff prop and exports OpenDiffFn (source)", () => {
    // onOpenDiff and OpenDiffFn are `chipmate_change` additions — TypeScript types
    // erased at runtime, so we verify via source analysis
    const src = fs.readFileSync(DATA_CONTEXT_FILE, "utf-8")
...
describe("Assistant Markdown streaming contract (source)", () => {
  const src = fs.readFileSync(CHIPMATE_MESSAGE_PART_FILE, "utf-8")
  const block =
    src.match(
...
describe("Expanded tool motion and typography (source)", () => {
  ...
  it("uses the assistant markdown line-height ratio for reasoning output", () => {
    const css = fs.readFileSync(CHIPMATE_MESSAGE_PART_CSS_FILE, "utf-8")
    const block = css.match(
      /html\[data-theme="chipmate-vscode"\] \[data-component="reasoning-part"\][\s\S]*?(?=@keyframes reasoning-pulse)/,
    )?.[0]
...
describe("HighlightedText @mention regex fallback and click handler (source)", () => {
  const src = fs.readFileSync(CHIPMATE_MESSAGE_PART_FILE, "utf-8")
  const helper = fs.readFileSync(CHIPMATE_MESSAGE_HIGHLIGHT_FILE, "utf-8")
...
describe("AssistantMessage visible row contract (source)", () => {
  ...
  it("uses the native recall tool without a separate memory badge", () => {
    const tools = fs.readFileSync(CHIPMATE_MESSAGE_PART_FILE, "utf-8")
...
describe("Native tool summary contract (source)", () => {
  const tools = fs.readFileSync(CHIPMATE_MESSAGE_PART_FILE, "utf-8")
  const css = fs.readFileSync(CHIPMATE_BASIC_TOOL_CSS_FILE, "utf-8")
...
describe("Collapsed deferred tool details contract (source)", () => {
  ...
  const message = fs.readFileSync(CHIPMATE_MESSAGE_PART_FILE, "utf-8")
