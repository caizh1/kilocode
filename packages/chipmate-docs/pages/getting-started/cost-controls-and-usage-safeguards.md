...
How much you spend with ChipMate is shaped by several factors working together:
...
This page covers the controls currently available in ChipMate. For a direct overview of Auto Model tiers and token optimization tips, see [Cost Efficiency & Model Selection](/docs/getting-started/rate-limits-and-costs).
...
When the agent enters a repeated failure cycle — attempting the same action multiple times without making progress — ChipMate pauses and asks for permission before continuing. This is controlled by the `doom_loop` permission, which defaults to `ask`.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission.doom_loop` key in `chipmate.jsonc` (CLI).
...
Every action ChipMate takes — reading files, editing code, running shell commands, launching sub-agents — is governed by the permission system. Each tool can be set to `allow`, `ask`, or `deny`. When set to `ask`, ChipMate pauses before executing and you can approve or reject that specific action.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission` section in `chipmate.jsonc` (CLI). See [Auto-Approving Actions](/docs/getting-started/settings/auto-approving-actions) for the full list of available permissions.
...
A shield button in the prompt controls lets you toggle auto-approve on and off at runtime without opening Settings. When enabled, pending permission prompts are approved automatically. The state stays synced across the sidebar and open ChipMate tabs.
...
**Where to configure:** Organization spending limits are managed in the organization dashboard at [app.chipmate.ai](https://app.chipmate.ai). Individual credit top-up is at Settings → Adding Credits.

### Free model rate limits

Requests to free models (`chipmate-auto/free` and other free-tier models) are rate-limited to **200 requests per hour**. If you exceed this, requests return HTTP 429 and you must wait before continuing.
...
ChipMate automatically skips a set of directories including `node_modules`, `dist`, `build`, `.git`, `__pycache__`, `.cache`, and `vendor`. You can add additional paths using **permission deny rules** in `chipmate.jsonc` or using a `.chipmateignore` file at your workspace root.
...
**Where to configure:** `chipmate.jsonc` (VS Code / CLI). See [.chipmateignore](/docs/customize/context/chipmateignore) for full details.

### Compact long conversations

When a conversation grows long, use `/compact` in the chat (also searchable as `smol` or `condense`) to summarize the history and free up context space. ChipMate replaces older conversation turns with an anchored summary that captures your goal, constraints, progress, and next steps.

Auto-compaction is **enabled by default** — ChipMate automatically compacts when approaching the context window limit so you do not need to intervene manually.

**Where to configure:** Toggle auto-compaction in **Settings → Context** (VS Code) or set `compaction.auto` in `chipmate.jsonc`. Configure the trigger threshold with `compaction.threshold_percent` (e.g. `80` to compact at 80% of the model's context window).
...
**Where to configure:** Model settings in the ChipMate UI, or the `limit.output` key in custom model configuration.
...
ChipMate automatically applies prompt caching on supported providers. Repeated context — your system prompt, stable file contents, and tool definitions — is reused from cache at a discounted rate. No configuration is required to benefit from this.
...
ChipMate has auto-models that can help you control costs; more information is available in [Auto Model](/docs/code-with-ai/agents/auto-model).

### Practical examples by task type

| Task type | Suggested approach |
|---|---|
| Quick questions, syntax lookups, simple formatting | `chipmate-auto/efficient` or a lightweight model |
| Routine edits, test generation, straightforward refactors | `chipmate-auto/efficient` or a mid-tier model |
| Complex debugging, tracing unexpected behavior | `chipmate-auto/frontier` or a strong reasoning model; Debug mode |
| Architecture planning, design decisions | `chipmate-auto/frontier`; Architect mode |
| Repository-wide analysis or search | A model with a large context window (256K+); Architect mode |
| Code review and summarization | `chipmate-auto/efficient` or a cost-effective model |
| Automated background tasks (CI, scripting) | `chipmate-auto/efficient` or `chipmate-auto/free` |

### Manually selecting a model

Use the **model selector dropdown** in the ChipMate chat interface to switch models for the current session. In the CLI, pass the `--model` flag to `chipmate run` or use the model picker in the TUI (`Ctrl+X m` or `/models`).
...
- **VS Code:** Settings → Models → Model per Mode, or edit `chipmate.jsonc` directly.
- **CLI:** Set `agent.<name>.model` in `chipmate.jsonc`.

```jsonc
{
  "agent": {
    "code": {
      "model": "chipmate-auto/efficient"
    },
    "architect": {
      "model": "chipmate-auto/frontier"
    }
  }
}
```
...
**Where to access:** [app.chipmate.ai](https://app.chipmate.ai) → Usage tab.
...
- Use `chipmate-auto/efficient` as the default model
- Switch to `chipmate-auto/free` for low-stakes questions and exploration
...
- Assign `chipmate-auto/efficient` to Code and Debug agents for everyday work
- Assign `chipmate-auto/frontier` to Architect (or Plan) agent for planning tasks
- Set `chipmate-auto/efficient` as the compaction model for all agents
- If on an Enterprise plan, use Providers & Models to block high-cost models that are not needed for your team's typical work

```jsonc
{
  "agent": {
    "code": { "model": "chipmate-auto/efficient" },
    "debug": { "model": "chipmate-auto/efficient" },
    "architect": { "model": "chipmate-auto/frontier" },
...
The snippet below is a ready-to-copy `chipmate.jsonc` that turns on every available cost-control knob at its most restrictive setting. Drop it into your project root (or your global `~/.config/chipmate/chipmate.jsonc`) and adjust individual values upward as you get comfortable with how each one behaves.

{% callout type="tip" %}
This configuration uses only `chipmate-auto/efficient`.
{% /callout %}
...
```jsonc
{
  ...
  // ── Model selection ──────────────────────────────────────────────────────
  // Route all requests through the two lowest-cost ChipMate Auto tiers.
  // chipmate-auto/efficient: lowest-cost paid tier (classifies each request by
  //   difficulty and routes to the cheapest benchmark-proven model).
  "model": "chipmate-auto/efficient",
  "subagent_model": "chipmate-auto/efficient", // default model for Task-tool subagents
  ...
  "agent": {
    ...
    "compaction": { "model": "chipmate-auto/free" },
    "title":      { "model": "chipmate-auto/free" },
    "summary":    { "model": "chipmate-auto/free" }
  },
  ...
  // "ask" means ChipMate pauses and requires your approval before executing.
...
- **Check your usage dashboard** at [app.chipmate.ai/usage](https://app.chipmate.ai/usage) for a breakdown by day, model, and project
...
For further reading: [4 Levers to Take Control of Your AI Spend](https://blog.chipmate.ai/p/4-spend-levers)
