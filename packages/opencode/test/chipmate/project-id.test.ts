...
import { getChipMateProjectId } from "../../src/chipmate/project-id"
...
describe("project-id", () => {
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/ChipMate-Org/handbook.git`.cwd(dir).quiet()
        },
      })

      const id = await provideTestInstance({
        directory: tmp.path,
        fn: () => getChipMateProjectId(),
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:ChipMate-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/ChipMate-Org/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/ChipMate-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/ChipMate-Org/${longName}.git`.cwd(dir).quiet()
        },
      })
  ...
  describe("config file priority", () => {
    test("uses project.id from .chipmate/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          ...
          await Bun.write(
            path.join(dir, ".chipmate", "config.json"),
            JSON.stringify({
              project: {
    ...
    test("falls back to .chipmate/config.json when .chipmate/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".chipmate"), { recursive: true })
    ...
    test("uses .chipmate/config.json and ignores .opencode/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".chipmate"), { recursive: true })
          await Bun.write(path.join(dir, ".chipmate", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/ChipMate-Org/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".chipmate"), { recursive: true })
          await Bun.write(path.join(dir, ".chipmate", "config.json"), "{ invalid json")
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/ChipMate-Org/handbook.git`.cwd(dir).quiet()
        },
      })
