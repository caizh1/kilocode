 ...
 *
 * Editor clients (VS Code extension, JetBrains plugin) run `chipmate serve` as a child
 ...
 * PID via `CHIPMATE_PARENT_PID`; we poll that PID and re-parenting so the server shuts
 * itself down when the client is gone.
 *
 * No-op unless `CHIPMATE_PARENT_PID` is set to a valid PID, so a manually launched
 * `chipmate serve` (whose parent shell exiting may be intentional) is never affected.
 *
 * Returns a function that stops the watchdog.
 */
export function startParentWatchdog(onOrphan: () => void, intervalMs = 1000): () => void {
  const configured = Number(process.env["CHIPMATE_PARENT_PID"])
