...
import * as ChipMateSkill from "../../src/chipmate/skill-remove"
import { BUILTIN_SKILLS } from "../../src/chipmate/skills/builtin"
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const skill = yield* Skill.Service
      const item = yield* skill.get("chipmate-config")
      expect(item).toBeDefined()
      expect(item!.name).toBe("chipmate-config")
      expect(item!.location).toBe(Skill.BUILTIN_LOCATION)
      expect(item!.content).toContain("chipmate")
    }),
  { git: true },
)
...
it.instance(
  "chipmate-config is protected from removal",
  () =>
    Effect.gen(function* () {
      const skill = yield* Skill.Service
      const item = yield* skill.get("chipmate-config")
      expect(item).toBeDefined()
      expect(ChipMateSkill.builtin(item!.location)).toBe(true)
    }),
  { git: true },
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const dir = path.join(instance.directory, ".chipmate", "skill", "chipmate-config")
      yield* Effect.promise(() =>
        Bun.write(
          path.join(dir, "SKILL.md"),
          `---
name: chipmate-config
description: User override of chipmate-config.
---

# Custom chipmate-config
...
`,
      ...
      const item = yield* skill.get("chipmate-config")
      expect(item).toBeDefined()
      expect(item!.description).toBe("User override of chipmate-config.")
      expect(item!.location).not.toBe(Skill.BUILTIN_LOCATION)
      expect(item!.location).toContain(path.join("skill", "chipmate-config", "SKILL.md"))
    }),
  { git: true },
)
