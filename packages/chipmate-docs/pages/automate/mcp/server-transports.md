...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between ChipMate and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (ChipMate) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as ChipMate
...
- Single-client scenarios (one ChipMate instance per server)
...
1. The client (ChipMate) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from ChipMate
...
STDIO servers run locally on the same machine as ChipMate, which has several important implications:
...
- **Execution**: Starts and stops with ChipMate (child process lifecycle)
...
- Start when needed by ChipMate
- Not require network configuration
- Need to be installed alongside ChipMate or via a package manager
...
## Configuring Transports in ChipMate

For detailed information on configuring STDIO and SSE transports in ChipMate, including example configurations, see the [Understanding Transport Types](using-in-chipmate-code#understanding-transport-types) section in the Using MCP in ChipMate guide.
