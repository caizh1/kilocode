...
import { getNetworkIPs, serverUrls } from "../../../../src/chipmate/cli/server-urls"
...
describe("console daemon startup", () => {
  test("detects every explicit network option form", () => {
    ...
    expect(explicitNetworkOptions(["chipmate", "console", "--", "--port=4321"])).toStrictEqual([])
  })
  ...
  test("rejects legacy fixed-password daemon state", () => {
    expect(Daemon.matches({ ...state(), password: "chipmate" }, opts(), [])).toBe(false)
  })
