import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@chipmate/plugin/tui"
import { createMemo, createSignal, onCleanup, onMount, Show } from "solid-js"
import { Global } from "@opencode-ai/core/global"
import * as Log from "@opencode-ai/core/util/log"
import type { ChipMatePassState } from "@chipmate/chipmate-gateway"
import type { Message } from "@chipmate/sdk/v2"
import { onBalanceRefresh } from "../balance-refresh"
import { REDACTED_BALANCE } from "../pii"

const id = "internal:chipmate-sidebar-footer"
...
export function passLine(pass: ChipMatePassState) {
...
// A billed turn: a completed ChipMate assistant message with a non-zero cost.
export function billable(info: Message) {
  if (info.role !== "assistant") return false
  return info.providerID === "chipmate" && info.time.completed !== undefined && info.cost > 0
}
...
function View(props: { api: TuiPluginApi }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) =>
        (item.id !== "opencode" && item.id !== "chipmate") ||
  ...
  const refresh = () => {
    ...
    void props.api.client.chipmate
      ...
      .then((res) => {
        ...
        const next: State = {
          ...
          pass: res.data.chipmatePass ?? null,
        }
        setState(next)
              ...
              {(pass) => (
                <box gap={0}>
                  <box flexDirection="row" justifyContent="space-between" gap={1}>
                    <text fg={theme().textMuted}>{" └ ChipMate Pass"}</text>
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>ChipMate includes free models so you can start immediately.</text>
