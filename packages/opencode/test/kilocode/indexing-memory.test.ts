...
import { indexingBudget, pressureFor } from "../../src/kilocode/indexing-memory"
...
test("honors explicit indexing memory limits", () => {
  expect(
    indexingBudget(
      {
        KILO_INDEXING_SOFT_RSS_BYTES: String(2 * gib),
        KILO_INDEXING_CRITICAL_RSS_BYTES: String(2.5 * gib),
        KILO_INDEXING_HARD_RSS_BYTES: String(3 * gib),
        KILO_INDEXING_RECOVERY_RSS_BYTES: String(1.5 * gib),
      },
      64 * gib,
    ),
