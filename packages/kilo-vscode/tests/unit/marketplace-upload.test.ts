...
import { buildMarketplaceBuiltinSkillUploadPayload, buildMarketplaceSkillUploadPayload } from "../../src/services/marketplace/upload"
...
describe("marketplace skill upload scanner", () => {
  ...
  it("requires SKILL.md at the skill root", async () => {
    await withTempSkill(async (dir) => {
      ...
      await expect(buildMarketplaceSkillUploadPayload(dir)).rejects.toThrow("ENOENT")
    })
  })
