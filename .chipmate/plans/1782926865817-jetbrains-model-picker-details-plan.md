...
## Affected Areas
- `packages/chipmate-jetbrains/shared/src/main/kotlin/ai/chipmate/rpc/dto/ProviderDto.kt`
- `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/workspace/ChipMateWorkspaceState.kt`
- `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/cli/ChipMateCliDataParser.kt`
- `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/rpc/ChipMateWorkspaceDtoMapper.kt`
- `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/session/model/SessionModel.kt`
- `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/session/SessionUi.kt`
- `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/settings/models/ModelsSettingsUi.kt`
- `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/session/ui/model/ModelPicker*.kt`
- `packages/chipmate-jetbrains/frontend/src/main/resources/messages/ChipMateBundle*.properties`
- Existing tests under `packages/chipmate-jetbrains/frontend/src/test/...` and `backend/src/test/...`
...
2. Parse and map the extra metadata in the JetBrains backend.
   - Extend `ModelInfo` in `ChipMateWorkspaceState.kt` with matching nullable metadata fields.
   - Update `ChipMateCliDataParser.parseModel` and `parseModelDto` to read the fields currently used by VS Code: `inputPrice`, `outputPrice`, `contextLength`, `releaseDate`, `latest`, `cost`, `capabilities.input`, `options.description`, `autoRouting.models`, and `terminalBench`.
   ...
   - Update `ChipMateWorkspaceDtoMapper.model` to copy all new fields to `ModelDto`.
...
5. Rework popup layout and size behavior.
   ...
   - Persist state under a key such as `chipmate.model.picker.expanded` using `PropertiesComponent.getInstance().getBoolean(key, false)` and `setValue(key, value.toString())`.
...
7. Add localization strings.
   - Add base `ChipMateBundle.properties` entries for maximize/minimize tooltips, detail labels, pricing labels, context, capabilities, terminal-bench, description, auto-routing choices, and unavailable values.
...
## Validation
- From `packages/chipmate-jetbrains/`: run `./gradlew typecheck` or `bun run typecheck`.
- From `packages/chipmate-jetbrains/`: run targeted tests for backend parser/serialization and frontend model picker tests, or `./gradlew test` if targeted Gradle filters are not straightforward.
