...
it.instance("shares and unshares sessions through ChipMate public URLs", () => {
  const urls: string[] = []
  ...
  return Effect.gen(function* () {
    ...
    expect(result.url).toBe("https://app.chipmate.ai/s/public-1")
    expect((yield* session.get(info.id)).share?.url).toBe("https://app.chipmate.ai/s/public-1")
    ...
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("chipmate").pipe(Effect.ignore)
        request.mockRestore()
      }),
    ),
  )
