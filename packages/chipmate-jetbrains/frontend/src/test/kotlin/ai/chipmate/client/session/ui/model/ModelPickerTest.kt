...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "ChipMate Auto", "chipmate", "ChipMate", 0.0),
        ...
        assertEquals("chipmate/auto", rows[1].key)
    ...
    fun `test chipmate provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "chipmate", "ChipMate"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("ChipMate", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test small rows are included only when requested`() {
        val items = listOf(
            item("auto-small", "Small", "chipmate", "ChipMate"),
            item("auto", "Auto", "chipmate", "ChipMate"),
        )
        ...
        assertEquals(listOf("chipmate/auto"), default.mapNotNull { it.key })
        assertEquals(listOf("chipmate/auto", "chipmate/auto-small"), small.mapNotNull { it.key }.sorted())
    }
    ...
    fun `test non-chipmate selected model uses provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-chipmate selected model strips duplicate provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-chipmate selected model strips vscode provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test chipmate selected model remains unprefixed`() {
        val picker = ModelPicker()

        picker.setItems(listOf(item("auto", "ChipMate Auto", "chipmate", "ChipMate")))
    ...
    fun `test details panel displays preview metadata and updates favorite star`() {
        ...
        val item = ModelPicker.Item(
            id = "auto",
            display = "ChipMate Auto",
            provider = "chipmate",
            providerName = "ChipMate",
            ...
            options = ModelOptionsDto("Fast routed model https://chipmate.ai"),
        ...
        assertTrue(panel.html().any { it.contains("href=\"https://chipmate.ai\"") })
    ...
    fun `test details panel updates retained description component`() {
        ...
        val item = ModelPicker.Item(
            ...
            options = ModelOptionsDto("First https://chipmate.ai"),
        )

        panel.update(item)
        val count = panel.treeSize()
        val html = panel.htmlPanes().single()
        panel.update(item.copy(id = "second", display = "Second", options = ModelOptionsDto("Second https://chipmate.ai/docs")))
    ...
    fun `test selected paid model with training flag indicates data collection`() {
        ...
        picker.setItems(listOf(item("paid", "Paid", "chipmate", "ChipMate", training = true)))
    ...
    fun `test selected free model without training flag does not indicate data collection`() {
        ...
        picker.setItems(listOf(item("free", "Free", "chipmate", "ChipMate", free = true)))
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "chipmate", "ChipMate"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "chipmate", "ChipMate"), "ChipMate", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "chipmate", "ChipMate"), "ChipMate", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("chipmate/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "chipmate", "ChipMate", free = true), "ChipMate", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows data collection warning for paid training model`() {
        val row = ModelPickerRow(item("paid", "Paid", "chipmate", "ChipMate", training = true), "ChipMate", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows BYOK instead of free when both are available`() {
        val row = ModelPickerRow(
            ModelPicker.Item("claude", "Claude", "chipmate", "ChipMate", free = true, byok = true),
            "ChipMate",
            false,
        )
        val model = CollectionListModel(listOf(row))
