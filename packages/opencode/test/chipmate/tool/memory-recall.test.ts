...
import { MemoryRecallTool } from "@/chipmate/tool/memory-recall"
...
describe("chipmate_memory_recall description", () => {
...
describe("chipmate_memory_recall", () => {
  ...
  test("shows typed memory hits separately from session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await ChipMateMemory.enable({ ctx: memory })
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "vscode_tests",
            text: "Run VS Code unit tests from packages/chipmate-vscode with bun run test:unit.",
          },
          {
            action: "add",
            file: "project.md",
      ...
      expect(typed.title).toContain("ChipMate memory typed")
      ...
      expect(digest.title).toContain("ChipMate memory digest")
  ...
  test("catalog mode lists all stored keys with optional filter", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
        ...
        ops: [
          {
            ...
            file: "project.md",
            section: "Facts",
            key: "chipmate_was_originally_a_fork",
            text: "chipmate was originally a fork of roo and has a chipmate-legacy repo",
          },
          {
            action: "add",
            file: "environment.md",
      ...
      expect(all.output).toContain("chipmate_was_originally_a_fork")
      expect(all.output).toContain("vscode_tests")
      expect(filtered.output).toContain("chipmate_was_originally_a_fork")
  ...
  test("search mode renders typed and digest memory without catalog mode", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      expect(result.title).toContain("ChipMate memory search")
  ...
  test("handles large unmatched recall queries without returning stored memory", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".chipmate"), async () => {
      ...
      expect(result.title).toBe("ChipMate memory search: no results")
