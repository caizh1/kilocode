...
import {
  CHIPMATE_EXA_URL,
  MAX_CHIPMATE_EXA_RESULTS,
  type ChipMateExaParams,
  callChipMateExa,
} from "../../../src/chipmate/tool/websearch-chipmate-exa"
...
const runCall = async (
  params: ChipMateExaParams,
  respond: (status: number) => Response,
  recorded?: Recorded,
  chipmateToken = "chipmate-test-token",
) =>
  Effect.runPromiseExit(
    Effect.gen(function* () {
      const http = fakeHttp(respond, recorded)
      return yield* callChipMateExa(http, params, chipmateToken)
    }),
  )

describe("callChipMateExa request shape", () => {
  test("posts to CHIPMATE_EXA_URL with bearer token and highlights-only contents", async () => {
    ...
    expect(recorded.authorization).toBe("Bearer chipmate-test-token")
    const parsed = JSON.parse(recorded.body!)
    expect(parsed.query).toBe("drone")
    expect(parsed.type).toBe("auto")
    expect(parsed.numResults).toBe(MAX_CHIPMATE_EXA_RESULTS)
  ...
  test("clamps numResults at MAX_CHIPMATE_EXA_RESULTS", async () => {
    const recorded: Recorded = {}
    await runCall({ query: "x", numResults: 25 }, () => okJson({ results: [] }), recorded)
    expect(JSON.parse(recorded.body!).numResults).toBe(MAX_CHIPMATE_EXA_RESULTS)
  })
  ...
  test("CHIPMATE_EXA_URL is built from CHIPMATE_API_BASE", () => {
    expect(CHIPMATE_EXA_URL).toMatch(/\/api\/exa\/search$/)
  })
})

describe("callChipMateExa response formatting", () => {
...
describe("callChipMateExa error handling", () => {
  test("dies with auth-required message on 401", async () => {
    ...
    expect(String((exit as Extract<typeof exit, { _tag: "Failure" }>).cause)).toContain("chipmate auth login")
  })
