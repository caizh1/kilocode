...
class ChipMateIgnoreCacheTest : BasePlatformTestCase() {
    fun `test matcher caches until ignore file changes`() {
        val file = myFixture.addFileToProject(".chipmateignore", "ignored/\n").virtualFile
        val root = file.parent
        val cache = project.service<ChipMateIgnoreCache>()
    ...
    fun `test null root allows everything`() {
        assertSame(ChipMateIgnore.EMPTY, project.service<ChipMateIgnoreCache>().matcher(null))
    }
}
