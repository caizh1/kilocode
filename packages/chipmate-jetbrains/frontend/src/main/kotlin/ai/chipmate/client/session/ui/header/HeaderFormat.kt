...
import ai.chipmate.client.session.model.ContextUsage
...
internal fun contextTip(value: ContextUsage?): String? {
    val ctx = value ?: return null
    val pct = ctx.percentage
    if (pct != null) return ChipMateBundle.message("session.header.context.tooltip.percent", num(ctx.tokens), pct)
    if (ctx.tokens > 0) return ChipMateBundle.message("session.header.context.tooltip.tokens", num(ctx.tokens))
    return null
}

internal fun todo(done: Int, total: Int): String? {
    if (total <= 0) return null
    if (done >= total) return ChipMateBundle.message("session.header.todos.done", total)
    return ChipMateBundle.message("session.header.todos.progress", done, total)
}
