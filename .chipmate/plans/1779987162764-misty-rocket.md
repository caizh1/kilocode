...
The JetBrains plugin currently imports `com.intellij.util.SVGLoader` in `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/ui/SvgIconColorizer.kt` and calls `SVGLoader.load(ByteArrayInputStream(patch()), scale.toFloat())` from `SvgIcon.image(Graphics)`.
...
1. Add ChipMate-owned SVG rendering code under the frontend UI package, for example:
   - `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/ui/SvgLoader.kt`
...
3. If jsvg does not expose intrinsic size cleanly from public API, adjust the API to avoid duplicating SVG parsing:
   - Add `width` and `height` parameters to the ChipMate loader.
...
4. Add the jsvg dependency explicitly to the plugin frontend module instead of relying on the IntelliJ-bundled library:
   - In `packages/chipmate-jetbrains/gradle/libs.versions.toml`, add `jsvg = "2.1.0"` and `jsvg = { module = "com.github.weisj:jsvg", version.ref = "jsvg" }`.
   - In `packages/chipmate-jetbrains/frontend/build.gradle.kts`, add `implementation(libs.jsvg)`.
...
5. Update `SvgIconColorizer.kt`:
   ...
   - Replace `SVGLoader.load(ByteArrayInputStream(patch()), scale.toFloat())` with the ChipMate-owned loader call.
   - Keep the existing color patching, cache key, scale calculation, and size parsing intact.

6. Add a focused frontend unit test:
   - New test file: `frontend/src/test/kotlin/ai/chipmate/client/ui/SvgIconColorizerTest.kt` or `SvgLoaderTest.kt`.
...
7. Run verification from `packages/chipmate-jetbrains/`:
   - `./gradlew typecheck`
...
- This change is in `packages/chipmate-jetbrains/`, a ChipMate-owned package, so `chipmate_change` markers are not needed.
