...
import { plain, session, supports, tui } from "../../src/chipmate/cli/logo"

describe("chipmate logo", () => {
  ...
  test("allows an override", () => {
    expect(supports({ CHIPMATE_UNICODE_LOGO: "1", SSH_TTY: "/dev/pts/0" }, "linux")).toBe(true)
    expect(supports({ CHIPMATE_UNICODE_LOGO: "0" }, "linux")).toBe(false)
  })

  test("uses modern and fallback logo variants", () => {
    expect(tui({ CHIPMATE_UNICODE_LOGO: "1" }, "linux").join("\n")).toContain("🬺🬏")
