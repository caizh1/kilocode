...
import type { NotebookRequest } from "@chipmate/sdk/v2/client"
...
function harness(context: NotebookBridgeContext, dirs = ["/repo"]) {
  ...
  const request = (value: NotebookRequest = read, directory = "/repo") =>
    handlers.event?.(
      { id: `event-${value.id}`, type: "chipmate.notebook.requested", properties: value } as SSEPayload,
      directory,
    )
...
describe("ChipMateConnectionService notebook directories", () => {
  it("tracks the workspace root, current request directory, and provider directories", async () => {
    ...
    const service = new ChipMateConnectionService({} as vscode.ExtensionContext)
