...
class ChipMateAgentBehaviorRpcApiImplTest {
    ...
    fun `skills and remove skill call CLI endpoints`() = runBlocking {
        val dir = Files.createTempDirectory("chipmate-skill-test")
        ...
        assertEquals(1, mock.requestCount("/chipmate/skill/remove"))
    ...
    fun `command files and remove command call CLI endpoints`() = runBlocking {
        val dir = Files.createTempDirectory("chipmate-command-test")
        ...
        assertEquals(1, mock.requestCount("/chipmate/command/remove"))
    ...
    fun `save commands validates known and new project command paths`() = runBlocking {
        val project = Files.createTempDirectory("chipmate-command-project")
        val known = Files.createDirectories(project.resolve(".chipmate/command")).resolve("known.md")
        val added = project.resolve(".chipmate/commands/new.md")
        val other = Files.createTempFile("chipmate-command-other", ".md")
        ...
        assertEquals(1, mock.requestCount("/chipmate/command/files"))
    ...
    fun `save commands validates new global command paths`() = runBlocking {
        val project = Files.createTempDirectory("chipmate-command-project")
        val config = Files.createTempDirectory("chipmate-command-config")
    ...
    fun `url cached skills are read only`() = runBlocking {
        val cache = Path.of(System.getProperty("user.home"), ".cache", "chipmate", "skills", "remote")
    ...
    fun `custom skills under non cache paths remain editable`() = runBlocking {
        val dir = Files.createTempDirectory("chipmate-skill-test")
        val file = Files.createDirectories(dir.resolve("cache/chipmate/skills/custom")).resolve("SKILL.md")
    ...
    private suspend fun rpc(): ChipMateAgentBehaviorRpcApiImpl = ChipMateAgentBehaviorRpcApiImpl(app())

    private suspend fun app(): ChipMateBackendAppService {
        val app = ChipMateBackendAppService.create(scope, FakeCliServer(mock), TestLog())
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is ChipMateAppState.Ready }
        }
        return app
    }
}
