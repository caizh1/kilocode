...
description: "Configure automatic approval settings for ChipMate operations"
---

# Auto-Approving Actions

{% callout type="danger" %}
**Security Warning:** Auto-approve settings bypass confirmation prompts, giving ChipMate direct access to your system. This can result in data loss, file corruption, or worse. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
{% /callout %}

Auto-approve settings speed up your workflow by eliminating repetitive confirmation prompts, but they significantly increase security risks. The VS Code extension, JetBrains plugin, and CLI share the same permission model; choose the tab that matches how you configure ChipMate. In the JetBrains plugin, the same rules are configured under **Settings → Tools → ChipMate → Auto-Approve**.

{% callout type="note" %}
**Editing project config while a session is running:** ChipMate caches project-level `chipmate.jsonc` / `chipmate.json` (in `.chipmate/`) when it first loads a workspace, and does not re-read it on every prompt. If you add, change, or remove a project permission rule while the backend is already running, reload the VS Code window (or start a fresh CLI session) for the change to take effect. Until then, ChipMate keeps using the previously loaded rules — so an auto-approved call may still cite a project rule you just edited. Global config (`~/.config/chipmate/`) is reloaded automatically.
{% /callout %}
...
The UI reads and writes to the same `chipmate.jsonc` config files used by the CLI, so changes made in either place are reflected in both.
...
| `"ask"` | ChipMate pauses and asks for approval before running the tool |
...
When a tool is set to `"ask"`, ChipMate pauses and displays a permission prompt with two options:
...
For full details and examples, see [MCP Tool Permissions](/docs/automate/mcp/using-in-chipmate-code#auto-approve-tools).
...
The CLI uses a granular, per-tool permission system configured in `chipmate.jsonc`. Instead of broad categories like "read" or "write," each tool has its own permission level with glob-pattern rules for fine-grained control.
...
Permissions are configured under the `permission` key in `chipmate.jsonc`. The following tool-specific permission levels are available:
...
If you define agents in Markdown files, the `permission` frontmatter uses the same `allow` / `ask` / `deny` values and glob patterns as `chipmate.jsonc`:
...
When a tool is set to `"ask"`, ChipMate pauses and displays a permission prompt. You have three options:
