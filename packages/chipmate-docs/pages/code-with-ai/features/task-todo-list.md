...
{% image src="/docs/img/screenshot-tests/chipmate-vscode/visual-regression/composite-webview/todo-write-docs-overview-chromium-linux.png" alt="Task Todo List overview showing interactive checklist in ChipMate" width="420" /%}
...
- Ask ChipMate to "use the [`todowrite` tool](/docs/automate/tools)"
- Say "create a todo list"

**The bottom line**: ChipMate decides what goes in the list, but you can provide feedback during approval dialogs.
...
ChipMate updates the list when:
...
**Now**: ChipMate creates structured checklists that update automatically as work progresses. You see exactly where you are and what's coming up.
...
{% image src="/docs/img/screenshot-tests/chipmate-vscode/visual-regression/chat/task-header-with-todos-chromium-linux.png" alt="Task header summary showing todo list progress" width="420" /%}
...
- Edit descriptions when ChipMate asks for approval
- Stage changes using the "Edit" button

**3. Environment Details**
Background "REMINDERS" table that keeps ChipMate informed about current progress
...
When ChipMate proposes a todo list update, you can edit the list before approving. Use the "Edit" button in the tool block to update item text, add or remove steps, or adjust status. Once approved, ChipMate continues with the updated list.

## Common questions

**"Can I create my own todo lists?"**
Yes, just ask ChipMate to use the `todowrite` tool. ChipMate stays in control of the content and workflow.

**"What about simple tasks?"**
ChipMate typically skips todo lists for simple tasks. The overhead isn't worth it.

**"Why can't I directly edit the list?"**
Design choice. ChipMate maintains authority over task management to ensure consistent progress tracking. You provide input, ChipMate executes.

---

## Settings

You can disable todo lists in Settings -> Advanced -> **Enable todo list tool**. When disabled, ChipMate won't create or update todo lists, and the REMINDERS table won't appear in Environment Details.
