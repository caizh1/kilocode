...
import { ChipMateConfig } from "../../../src/chipmate/config/config"
...
afterEach(async () => {
  delete process.env.CHIPMATE_MD_TEST
  await clear()
...
describe("markdown substitutions", () => {
  test("applies file and env substitutions to parsed markdown body", async () => {
    process.env.CHIPMATE_MD_TEST = "env content"
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, "body.md"), "file content")
        await Filesystem.write(
          path.join(dir, "SKILL.md"),
          ["---", "name: test", "description: Test", "---", "{file:body.md}", "{env:CHIPMATE_MD_TEST}"].join("\n"),
        )
      },
    })
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await Bun.file(path.join(globalTmp.path, "chipmate.jsonc")).json()
...
describe("project MCP trust boundaries", () => {
  ...
  test("drops env-backed project MCP headers without dropping static siblings", async () => {
    ...
    await using tmp = await tmpdir({ git: true })
    const prev = Global.Path.config
    const secret = process.env.CHIPMATE_PROJECT_MCP_SECRET
    ;(Global.Path as { config: string }).config = globalTmp.path
    process.env.CHIPMATE_PROJECT_MCP_SECRET = "process-secret"
    await clear()
    await disposeAllInstances()
    ...
    try {
      await writeConfig(tmp.path, {
        mcp: {
          unsafe: {
            type: "remote",
            url: "https://project.example.com/unsafe",
            headers: { Authorization: "Bearer {env:CHIPMATE_PROJECT_MCP_SECRET}" },
          },
          sibling: {
            type: "remote",
      ...
      if (secret === undefined) delete process.env.CHIPMATE_PROJECT_MCP_SECRET
      else process.env.CHIPMATE_PROJECT_MCP_SECRET = secret
      ;(Global.Path as { config: string }).config = prev
      await clear()
      await disposeAllInstances()
    }
  ...
  test("does not carry global credentials through remote-local-remote project layers", async () => {
    ...
    try {
      ...
      await writeConfig(path.join(tmp.path, ".chipmate"), {
...
describe("chipmate web search config", () => {
...
describe("chipmate indexing config", () => {
    ...
    try {
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await load()
          const input = ChipMateIndexing.input(config.indexing, global.indexing)
  ...
  test("project indexing enabled overrides global enablement", async () => {
    const input = ChipMateIndexing.input({ enabled: false }, { enabled: true })
    expect(input.enabled).toBe(false)
    expect(ChipMateIndexing.input(undefined, { enabled: true }).enabled).toBe(true)
    expect(ChipMateIndexing.input({ enabled: undefined }, { enabled: true }).enabled).toBe(true)
    expect(ChipMateIndexing.input({ enabled: true }, { enabled: false }).enabled).toBe(true)
  })

  test("resolves nested indexing fields independently across project and global config", () => {
    const input = ChipMateIndexing.input(
      {
  ...
  test("uses public-off and internal-on defaults only when enablement is unset", () => {
    ...
    try {
      ...
      expect(ChipMateIndexing.input().enabled).toBe(false)

      process.env.CHIPMATE_INTERNAL_OFFLINE = "1"
      expect(ChipMateIndexing.input()).toMatchObject({
        enabled: true,
        embedderProvider: "openai-compatible",
      ...
      expect(ChipMateIndexing.input(undefined, { enabled: false }).enabled).toBe(false)
      expect(ChipMateIndexing.input({ enabled: false }, { enabled: true }).enabled).toBe(false)
      expect(ChipMateIndexing.input({ enabled: true }, { enabled: false }).enabled).toBe(true)
    } finally {
  ...
  test("creates missing project config as .chipmate/chipmate.jsonc", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    expect(await Bun.file(path.join(tmp.path, ".chipmate", "chipmate.jsonc")).exists()).toBe(true)
    expect(await Bun.file(path.join(tmp.path, ".chipmate", "chipmate.json")).exists()).toBe(false)
  })

  test("accepts delete sentinels for indexing model overrides", () => {
    const patch = decode({ indexing: { model: null, dimension: null } })
    const merged = ChipMateConfig.mergeConfig(
      {
        indexing: {
          provider: "openai",
    ...
    const input = ChipMateIndexing.input(patch.indexing)
...
describe("chipmate sandbox config", () => {
...
describe("custom provider model config", () => {
  test("persists and removes reasoning across a global config reload", async () => {
    await using globalTmp = await tmpdir()
    const file = path.join(globalTmp.path, "chipmate.json")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
...
describe("unset propagation across layered config files", () => {
  ...
  test("removes subagent_model from every global config file when unset", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "chipmate.json")
    const jsonc = path.join(globalTmp.path, "chipmate.jsonc")
    ...
    try {
      await writeConfig(globalTmp.path, { subagent_model: "chipmate/openai/gpt-5" }, "chipmate.json")
      await Filesystem.write(jsonc, jsoncText)

      await saveGlobal(decode({ subagent_model: null }))

      // The key must be gone from the lower-precedence chipmate.json as well, or
  ...
  test("removes nested sentinels from jsonc siblings while preserving comments", async () => {
    ...
    try {
      await Filesystem.write(path.join(globalTmp.path, "chipmate.jsonc"), '{ "username": "marius" }\n')
      await Filesystem.write(
        opencode,
        [
          "{",
  ...
  test("does not rewrite sibling files on set or when the key is absent", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "chipmate.json")
    const jsonText = JSON.stringify({ subagent_model: "chipmate/old-model", username: "marius" }, null, 2)
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      ...
      await saveGlobal(decode({ subagent_model: "chipmate/new-model" }))
  ...
  test("removes subagent_model from every project config file when unset", async () => {
    await using tmp = await tmpdir({ git: true })
    await Filesystem.write(
      path.join(tmp.path, ".chipmate", "chipmate.json"),
      JSON.stringify({ subagent_model: "chipmate/openai/gpt-5" }),
    )
    await Filesystem.write(path.join(tmp.path, ".chipmate", "chipmate.jsonc"), '{\n  "username": "keep"\n}\n')
    ...
    const json = JSON.parse(await Bun.file(path.join(tmp.path, ".chipmate", "chipmate.json")).text())
    expect(json).not.toHaveProperty("subagent_model")
    // The primary target only gained nothing; the delete was a no-op there.
    const jsonc = JSON.parse(await Bun.file(path.join(tmp.path, ".chipmate", "chipmate.jsonc")).text())
...
describe("agent config", () => {
  ...
  test("removes an agent variant override without removing its model", () => {
    ...
    expect(patch.agent?.explore?.variant).toBeNull()
    expect(merged.agent?.explore).toEqual({ model: "chipmate/anthropic/claude-sonnet-4-6" })
  })

  test("removes agent model and variant overrides from global JSONC config", async () => {
    await using globalTmp = await tmpdir()
    const file = path.join(globalTmp.path, "chipmate.jsonc")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
...
describe("project config directory precedence", () => {
  test("loads .chipmate and ignores .opencode", async () => {
    await using tmp = await tmpdir()
    const entries = [
      {
        root: ".opencode",
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(config.username).toBe("chipmate")
        expect(config.model).toBe("test/chipmate")
        expect(config.small_model).toBeUndefined()

        expect(config.command?.shared).toMatchObject({
          description: "chipmate command",
          template: "chipmate command template",
        })
        ...
        expect(config.agent?.shared).toMatchObject({
          description: "chipmate agent",
          prompt: "chipmate agent prompt",
        })
        ...
        expect(plugins).toContain("chipmate.ts")
        expect(plugins).toContain("chipmate.ts")
...
describe("linked worktree config", () => {
  test("uses primary config directories as local fallbacks", async () => {
    ...
    await Bun.write(path.join(primary.path, "chipmate.json"), JSON.stringify({ model: "test/primary" }))
    await $`git add chipmate.json`.cwd(primary.path).quiet()
    ...
    try {
      await Bun.write(path.join(worktree, "chipmate.json"), JSON.stringify({ model: "test/worktree" }))
      await Bun.write(
        path.join(primary.path, ".chipmate", "chipmate.jsonc"),
        JSON.stringify({ username: "primary-dir", indexing: { enabled: true } }),
      )
      await Bun.write(path.join(worktree, ".chipmate", "chipmate.jsonc"), JSON.stringify({ username: "worktree-dir" }))
  ...
  test("uses nested primary config directories as local fallbacks", async () => {
    ...
    try {
      ...
      await Bun.write(
        path.join(primary.path, "packages", ".opencode", "chipmate.jsonc"),
        JSON.stringify({ snapshot: true, autoupdate: false, share: "auto", default_agent: "opencode-only" }),
      )
      await Bun.write(
        path.join(primary.path, "packages", ".chipmate", "chipmate.jsonc"),
        JSON.stringify({ snapshot: false, autoupdate: "notify", share: "disabled" }),
      )
      await Bun.write(path.join(directory, ".chipmate", "chipmate.jsonc"), JSON.stringify({ share: "manual" }))
  ...
  test("keeps CHIPMATE_CONFIG_DIR above the primary fallback", async () => {
    await using primary = await tmpdir({ git: true })
    ...
    await Bun.write(path.join(primary.path, ".chipmate", "chipmate.jsonc"), JSON.stringify({ username: "primary-dir" }))
    await Bun.write(path.join(explicit.path, "chipmate.jsonc"), JSON.stringify({ username: "explicit-dir" }))
    const previous = process.env["CHIPMATE_CONFIG_DIR"]
    process.env["CHIPMATE_CONFIG_DIR"] = explicit.path

    try {
      const config = await provideTestInstance({ directory: worktree, fn: load })
      expect(config.username).toBe("explicit-dir")
    } finally {
      if (previous === undefined) delete process.env["CHIPMATE_CONFIG_DIR"]
      else process.env["CHIPMATE_CONFIG_DIR"] = previous
      await $`git worktree remove --force ${worktree}`.cwd(primary.path).quiet().nothrow()
    }
  })
})
...
describe("opencode config migration notice", () => {
  ...
  test("detects a project .opencode directory", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "chipmate"), () => {
      const found = ChipMateConfig.detectOpencodeConfig({ directory: tmp.path, scanProject: true })
  ...
  test("skips the project scan when disabled", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "chipmate"), () => {
      const found = ChipMateConfig.detectOpencodeConfig({ directory: tmp.path, scanProject: false })
  ...
  test("builds a dismissible notification when opencode config exists", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "chipmate"), () => {
      const notice = ChipMateConfig.opencodeConfigNotification({ directory: tmp.path, scanProject: true })
      expect(notice?.id).toBe(ChipMateConfig.OPENCODE_NOTIFICATION_ID)
      expect(notice?.message).toContain(path.join(tmp.path, ".opencode"))
      expect(notice?.action?.actionURL).toBe(ChipMateConfig.CONFIG_DOCS_URL)
...
describe("bash permission migration", () => {
  ...
  "permission": "${action}"
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "chipmate.jsonc"), input)
        },
      })
      ...
      try {
        await ChipMateConfig.migrateBashPermission()

        const file = path.join(tmp.path, "chipmate.jsonc")
    ...
    test(`preserves string-form ${action} permission in json`, async () => {
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "chipmate.json"), input)
        },
      })
      ...
      try {
        ...
        const file = path.join(tmp.path, "chipmate.json")
        const text = await Filesystem.readText(file)
        const parsed = ConfigParse.schema(Config.Info, ConfigParse.jsonc(text, file), file)
        expect(text).toBe(input)
        expect(parsed.permission?.["*"]).toBe(action)
