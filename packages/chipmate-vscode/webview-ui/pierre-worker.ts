// ChipMate-specific replacement for the shared `@opencode-ai/ui/pierre/worker`
...
import { LINE_DIFF_TYPE } from "@chipmate/chipmate-ui/pierre"
import { WorkerPoolManager } from "@pierre/diffs/worker"
import { ensureChipMateDiffTheme, CHIPMATE_DIFF_THEME } from "@opencode-ai/ui/pierre/chipmate-diff-theme"

// Register the "ChipMate" theme before any pool initializes. resolveThemes([theme])
// runs on the main thread during initialize() and throws "resolveTheme: No valid
// loader for ChipMate" if the theme name was never registered. Registering here makes
...
function uri(): string | undefined {
  ...
  return (window as { CHIPMATE_SHIKI_WORKER_URI?: string }).CHIPMATE_SHIKI_WORKER_URI
}

export function workerFactory(): Worker {
  const url = uri()
  if (!url) throw new Error("CHIPMATE_SHIKI_WORKER_URI is not set")
...
function createPool() {
  const pool = new WorkerPoolManager(
    ...
    { theme: CHIPMATE_DIFF_THEME, lineDiffType: LINE_DIFF_TYPE, preferredHighlighter: ENGINE },
  )
  void pool.initialize().catch((err) => console.warn("[ChipMate New] Failed to initialize Pierre worker pool", err))
  return pool
}
