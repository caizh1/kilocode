...
describe("Settings UI alignment", () => {
  it("keeps all settings pages in the locked grouped navigation", () => {
    ...
    expect(settings).not.toContain("@chipmate/chipmate-ui/icon")
  })
  ...
  it("keeps close, project config, global config and tab actions wired", () => {
    ...
    expect(editor).toContain("returnState.requestClose()")
    expect(editor).toContain("panel.dispose()")
    expect(editor).toContain("existing.viewColumn ?? vscode.ViewColumn.Active")
    expect(editor).toContain("vscode.ViewColumn.Active")
    expect(editor).not.toContain("existing.reveal(vscode.ViewColumn.One)")
    expect(editor).toContain("/^chipmate\\.v2\\.(\\w+)Panel$/")
    expect(editor).not.toContain("/^chipmate-code\\.new\\.")
  ...
  it("renders semantic server states with Codicons in a stable control row", () => {
    expect(server).not.toContain("@chipmate/chipmate-ui/icon")
