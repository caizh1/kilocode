...
const chipmate = { id: "chipmate" } as Provider.Info

// Calculated cost for the `model()` + `baseUsage` pair: 1M input * $3 + 100k output * $15 = 3 + 1.5
const fallback = 3 + 1.5

describe("ChipMateSession.providerCost — Anthropic Messages / OpenAI Responses", () => {
...
describe("ChipMateSession.providerCost — Vercel AI Gateway", () => {
    ...
    const result = SessionNs.getUsage({
      ...
      metadata: {
        gateway: {
          ...
          // which ChipMate doesn't pass on to end users — must be ignored.
          cost: "0",
...
describe("ChipMateSession.providerCost — fallback", () => {
