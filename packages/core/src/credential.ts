...
import { asc, desc, eq } from "drizzle-orm" // chipmate_change
...
import { parse as parseChipMateAccounts } from "./chipmate/credential-migration"
import { isBusy } from "./chipmate/sqlite-error"
...
// chipmate_change start - legacy JSON credential stores that predate the integration credential table
...
// chipmate_change start - preserve ChipMate's account JSON stores and reconcile auth.json on every startup
export const legacyImportLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const chipmateName = "credential.chipmate-account-json-v3"
    if (!(yield* db.select().from(DataMigrationTable).where(eq(DataMigrationTable.name, chipmateName)).get())) {
      ...
      const values = parseChipMateAccounts(raw).toSorted(
          ...
          Effect.gen(function* () {
            ...
            for (const [index, item] of values.entries()) {
              ...
              yield* tx.insert(CredentialTable).values({
                id: ID.make(`cred_chipmate_${Buffer.from(item.id).toString("base64url")}`),
            ...
            yield* tx.insert(DataMigrationTable).values({ name: chipmateName, time_completed: Date.now() }).run()
          }),
        )
      }
    }
      ...
      Effect.gen(function* () {
        for (const item of values) {
          ...
          const current = yield* tx
            ...
            .orderBy(desc(CredentialTable.time_created)) // chipmate_change - reconcile the active imported account
            .get()
          if (current) {
...
// chipmate_change - retained for ChipMate migration and compatibility test layers
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // chipmate_change start - process-local workspace credentials override host storage without being persisted
    const content = process.env.CHIPMATE_AUTH_CONTENT
    const injected = yield* content === undefined
      ...
      : Effect.try({
          ...
          Effect.catch((cause) =>
            Effect.logWarning("invalid CHIPMATE_AUTH_CONTENT; using no process-local credentials", { cause }).pipe(
      ...
      lock.withPermit(
        Effect.gen(function* () {
          ...
          const row = yield* db
            ...
            .orderBy(desc(CredentialTable.time_created)) // chipmate_change - persist the active imported account
            .get()
            .pipe(Effect.orDie)
    ...
    return Service.of({
      all: Effect.fn("Credential.all")(function* () {
        if (isolated) return [...local.values()] // chipmate_change
        return (yield* db
          .select()
          .from(CredentialTable)
      ...
      get: Effect.fn("Credential.get")(function* (id) {
        if (isolated) return find(id) // chipmate_change - injected workspace credentials are process-local
      ...
      create: Effect.fn("Credential.create")(function* (input) {
        ...
        // chipmate_change start - credential changes in isolated workspaces are process-local
        if (isolated) {
        ...
        yield* db
          ...
          .pipe(Effect.orDie)
        yield* writeLegacy(credential.integrationID) // chipmate_change
        return credential
      }),
      update: Effect.fn("Credential.update")(function* (id, updates) {
        if (!updates.label && !updates.value) return
        // chipmate_change start - isolated updates never reach the host database
        if (isolated) {
        ...
        yield* db
          ...
          .run()
          .pipe(Effect.orDie)
        if (row?.integration_id) yield* writeLegacy(row.integration_id) // chipmate_change
      }),
      remove: Effect.fn("Credential.remove")(function* (id) {
        // chipmate_change start - isolated removals remain process-local
        if (isolated) {
          const credential = find(id)
...
export const node = makeGlobalNode({
  service: Service,
  layer: layer.pipe(Layer.provideMerge(legacyImportLayer)), // chipmate_change
  deps: [Database.node, FSUtil.node, Global.node], // chipmate_change
})
