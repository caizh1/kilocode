...
internal class McpEditDialog(
    ...
    private val command = JBTextField(cfg.command?.firstOrNull().orEmpty()).apply {
        ...
        emptyText.text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.command.placeholder")
    }
    private val args = JBTextArea(cfg.command.orEmpty().drop(1).joinToString("\n")).apply {
        ...
        emptyText.text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.args.placeholder")
    }
    private val url = JBTextField(cfg.url.orEmpty()).apply {
        columns = FIELD_COLUMNS
        emptyText.text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.url.placeholder")
    }
    private val env = linkedMapOf<String, String>().apply { putAll(cfg.environment.orEmpty()) }
    private val key = JBTextField().apply {
        columns = ENV_COLUMNS
        emptyText.text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.key")
    }
    private val value = JBTextField().apply {
        columns = ENV_COLUMNS
        emptyText.text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.value")
    }
    private val list = EnvList(env) { syncEnv() }
    private var center: JComponent? = null

    init {
        title = ChipMateBundle.message("settings.agentBehavior.mcp.edit.title", id)
        init()
    }
    ...
    override fun createCenterPanel(): JComponent {
        ...
        SettingsRows().apply {
            row(SettingsRow(
                ChipMateBundle.message("settings.agentBehavior.mcp.edit.transport"),
                transport(),
                JBLabel(id),
            ))
            panel.next(this)
        ...
        if (type == REMOTE) {
            SettingsRows().apply {
                row(SettingsStackedRow(
                    ChipMateBundle.message("settings.agentBehavior.mcp.edit.url"),
                    null,
                    url,
                ))
                panel.next(this)
            ...
            SettingsRows().apply {
                row(SettingsStackedRow(
                    ChipMateBundle.message("settings.agentBehavior.mcp.edit.command"),
                    null,
                    command,
                ))
                row(SettingsStackedRow(
                    ChipMateBundle.message("settings.agentBehavior.mcp.edit.args"),
                    ChipMateBundle.message("settings.agentBehavior.mcp.edit.args.help"),
                    scroll(args),
                ))
                panel.next(this)
            }
            panel.section(
                ChipMateBundle.message("settings.agentBehavior.mcp.edit.env"),
                ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.help"),
            ).row(envEditor())
        }
    ...
    override fun getDimensionServiceKey(): String = "ChipMate.McpEditDialog"

    private fun envEditor(): JComponent = Stack.vertical(UiStyle.Gap.sm())
        .next(Stack.horizontal(UiStyle.Gap.sm())
            .next(key)
            .next(value)
            .next(JButton(ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.add")).apply {
    ...
    private fun transport(): String {
        if (type == REMOTE) return ChipMateBundle.message("settings.agentBehavior.mcp.edit.transport.remote")
        return ChipMateBundle.message("settings.agentBehavior.mcp.edit.transport.local")
    }
    ...
    private class EnvRenderer : JPanel(BorderLayout()), ListCellRenderer<EnvItem> {
        ...
        override fun getListCellRendererComponent(
            ...
            val text = ChipMateBundle.message("settings.agentBehavior.mcp.edit.env.remove", value.name)
