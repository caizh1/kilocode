...
description: Use when releasing the ChipMate JetBrains plugin -- resolve a version ("next rc" or explicit), run the prepare workflow, edit and commit a filtered human-readable changelog on the release PR, then watch publish to completion.
---

# JetBrains Release

Use this skill when releasing the ChipMate JetBrains plugin.
...
- `gh` must be authenticated for `ChipMate-Org/chipmate` with permission to dispatch workflows, read PRs, and write contents. Merge permission is only required if the user asks the skill to merge the release PR automatically.
...
- Reference `packages/chipmate-jetbrains/RELEASING.md` for manual recovery rules.
...
```bash
bun .chipmate/skills/release-jetbrains/script/resolve-version.ts --spec "next rc"
```
...
Before dispatching prepare, verify the JetBrains plugin is pinned to the intended ChipMate Core release. The plugin downloads the CLI version from `packages/chipmate-jetbrains/package.json`, not from the JetBrains plugin version. Prepare tags `origin/main`, so the authoritative pin is the value on `origin/main`, not a local edit.
...
| `latestCli` | Latest stable `v*` ChipMate CLI GitHub release. |
| `prevJetbrainsCli` | CLI pin used by the latest `jetbrains/v*` release tag, for reviewing the jump. |
| `pinnedMain` / `pinnedLocal` | Whether `chipmate.cli.pinned=true`; `false` means repo CLI dev mode. |
...
| `repo-mode-on-main` | Stop. `chipmate.cli.pinned=false` is dev-only and must be reset to `true` on `main` before release. |
| `repo-mode-local` | Stop or reset local `chipmate.cli.pinned=true`; release checks should run from a releasable local state. |
...
# or
bun .chipmate/skills/release-jetbrains/script/set-pin.ts --version 7.4.1
```

Then test from `packages/chipmate-jetbrains/`:
...
# or
bun .chipmate/skills/release-jetbrains/script/set-pin.ts --version 7.4.1 --pr
```
...
```bash
bun .chipmate/skills/release-jetbrains/script/dispatch-prepare.ts --kind rc --version 7.0.1-rc.7
```
...
```bash
bun .chipmate/skills/release-jetbrains/script/dispatch-prepare.ts --kind rc --version 7.0.1-rc.7 --run-id <run-id>
```
...
```bash
git log --oneline <from-tag>..<tag> -- packages/opencode packages/chipmate-jetbrains
```
...
```text
packages/chipmate-jetbrains/build/release/<version>-changelog.md
```
...
- #1234 feat(jetbrains): ... https://github.com/ChipMate-Org/chipmate/pull/1234
- #1235 fix(cli): ... https://github.com/ChipMate-Org/chipmate/pull/1235
-->
```
...
```bash
bun .chipmate/skills/release-jetbrains/script/update-changelog.ts --version 7.0.1-rc.7 --file /path/to/clean-section.md
```

The script updates `packages/chipmate-jetbrains/CHANGELOG.md` on `jetbrains/release/v<version>` through the GitHub contents API and commits with:
...
```bash
gh api "repos/ChipMate-Org/chipmate/contents/packages/chipmate-jetbrains/CHANGELOG.md?ref=jetbrains/release/v<version>"
```
...
```bash
bun .chipmate/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7
```

Only merge automatically when the user explicitly asks for it and `gh` has merge permission:

```bash
bun .chipmate/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7 --merge
```

Pass a generous Bash timeout, such as `1800000` ms. If the shell times out, re-attach with:

```bash
bun .chipmate/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version 7.0.1-rc.7 --run-id <run-id>
```
...
```bash
bun .chipmate/skills/release-jetbrains/script/watch-publish.ts --pr <number> --version <version>
```
