...
import { isMemoryOperation, type MemoryOperation } from "@chipmate/chipmate-memory/commands"
...
export type ChipMateProviderMemoryInput = {
  client(): ChipMateClient | undefined
...
function memory(client: ChipMateClient | undefined): MemoryApi | undefined {
...
function request(input: Record<string, unknown>): { value: ChipMateProviderMemoryMessage } | { error: string } {
...
export class ChipMateProviderMemory {
  private readonly cached = new Map<string, unknown>()
  private tail = Promise.resolve()

  constructor(private readonly input: ChipMateProviderMemoryInput) {}
  ...
  async handle(message: Record<string, unknown>): Promise<boolean> {
    if (message.type === "requestMemory") {
      this.fetch(typeof message.sessionID === "string" ? message.sessionID : undefined).catch((err: unknown) =>
        console.error("[ChipMate New] fetchAndSendMemory failed:", err),
      )
      return true
    }
  ...
  private async load(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to fetch memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  // Memory inspection assembles the source document and status picker from one consistent backend snapshot.
  ...
  private async doShow(sessionID: string | undefined, mode: "status" | "show"): Promise<void> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed to show memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  run(message: ChipMateProviderMemoryMessage): Promise<boolean> {
  ...
  private async execute(message: ChipMateProviderMemoryMessage): Promise<boolean> {
    const client = this.input.client()
    if (!client) {
      this.input.post({
        type: "memoryOperationResult",
    ...
    try {
      ...
      const refreshed =
        message.operation === "status"
          ...
          : await retry(() => api.status({ directory }, { throwOnError: true })).catch((err: unknown) => {
              console.warn("[ChipMate New] Memory changed but refresh failed:", err)
              return undefined
            })
      ...
      console.error("[ChipMate New] ChipMateProvider: Failed memory operation:", err)
      this.input.post({
        type: "memoryOperationResult",
        operation: message.operation,
        sessionID: message.sessionID,
  ...
  private async action(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
  ...
  private async remember(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
  ...
  private async correct(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
    const text = message.text?.trim()
  ...
  private async forget(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
  ...
  private async purge(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
  ...
  private async auto(api: MemoryApi, directory: string, message: ChipMateProviderMemoryMessage) {
