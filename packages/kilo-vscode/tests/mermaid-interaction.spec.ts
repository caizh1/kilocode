...
test.describe("QA Mermaid", () => {
  ...
  test("emits SVG and PNG save events", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      window.addEventListener("kilo:save-image", (event) => {
  ...
  test("keeps controls inside the diagram at supported widths and themes", async ({ page }) => {
    ...
    for (const globals of [
      "colorScheme:light;theme:kilo-vscode;vscodeTheme:light-modern",
      "colorScheme:dark;theme:kilo-vscode;vscodeTheme:dark-modern",
      "colorScheme:dark;theme:kilo-vscode;vscodeTheme:hc-black",
    ]) {
