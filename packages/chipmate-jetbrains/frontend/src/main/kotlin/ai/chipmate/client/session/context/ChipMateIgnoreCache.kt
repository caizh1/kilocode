...
/**
 * Caches the compiled [ChipMateIgnore] per workspace-root directory so editor-context
 * gathering does not re-read and re-compile the ignore files on every prompt.
 *
 * The compiled matcher is reused until a `.chipmateignore` or `.gitignore` change
...
internal class ChipMateIgnoreCache : Disposable {
    private val cache = ConcurrentHashMap<String, ChipMateIgnore>()
    ...
    fun matcher(root: VirtualFile?): ChipMateIgnore {
        if (root == null) return ChipMateIgnore.EMPTY
        return cache.getOrPut(root.url) { ChipMateIgnore.load(root) }
    }
    ...
    private fun relevant(event: VFileEvent): Boolean {
        ...
        return name == ChipMateIgnore.CHIPMATE || name == ChipMateIgnore.GIT
    }
}
