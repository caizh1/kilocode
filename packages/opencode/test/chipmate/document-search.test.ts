...
import { DocumentSearchTool } from "../../src/chipmate/tool/document-search"
...
function stub(state: ChipMateIndexing.Status["state"] = "Complete") {
  ...
  const ready = spyOn(ChipMateIndexing, "documentReady").mockReturnValue(true)
...
describe("tool.document_search", () => {
  ...
  test("returns a bounded fallback without asking permission when indexing is not ready", async () => {
    ...
    await provideTestInstance({
      directory: tmp.path,
      fn: async () => {
        const ready = spyOn(ChipMateIndexing, "documentReady").mockReturnValue(false)
        const search = spyOn(ChipMateIndexing, "searchDocuments").mockResolvedValue([])
