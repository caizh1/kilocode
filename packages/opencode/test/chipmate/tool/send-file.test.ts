...
import { SendFileTool } from "@/chipmate/tool/send-file"
...
const status = spyOn(ChipMateSessions, "remoteStatus")
...
describe("send_file tool", () => {
  test("is only available while remote is connected", () => {
    ...
    expect(ChipMateToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: false })
    expect(ChipMateToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: true })
    expect(ChipMateToolRegistry.available(tool, agentInfo)).toBe(true)
  })
  ...
  test("sniffs correct MIME for a file with wrong extension", async () => {
    const dir = await tmpdir()
    try {
      // Actually just test a real PNG — ChipMateReadObject opens by path,
  ...
  // chipmate_change start — send_file now authorizes missing files with external_directory + read
  ...
  // chipmate_change start — send_file now authorizes directories before returning a
  ...
  test("included in extra() list", () => {
    const tool = { id: "send_file" } as Tool.Def
    const extra = ChipMateToolRegistry.extra(
      {
        codebase: tool,
        recall: tool,
        managerModels: tool,
...
// chipmate_change start — fsService mock now includes stat + realPath for the
// missing/directory authorization sequence that runs before ChipMateReadObject.file().
