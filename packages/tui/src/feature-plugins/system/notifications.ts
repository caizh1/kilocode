import type { Event } from "@chipmate/sdk/v2"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@chipmate/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  // chipmate_change start - only completed turns need completion attention
  ...
  api.event.on("session.error", (event) => {
    ...
    if (event.properties.error?.name === "MessageAbortedError") return // chipmate_change - manual stops do not need attention
