...
import { ChipMatePaths } from "./paths"

export namespace McpMigrator {
  const log = Log.create({ service: "chipmate.mcp-migrator" })

  // Remote transport types used by the ChipMate extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: ChipMateMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // ChipMate MCP server structure
  export interface ChipMateMcpServer {
    command?: string
    args?: string[]
  ...
  export interface ChipMateMcpSettings {
    mcpServers: Record<string, ChipMateMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<ChipMateMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    try {
      const content = await fs.readFile(filepath, "utf-8")
      return JSON.parse(content) as ChipMateMcpSettings
    } catch (err) {
  ...
  export function convertServer(name: string, server: ChipMateMcpServer): ConfigMCP.Info | null {
    if (isRemote(server)) {
      if (!server.url) {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: ChipMateMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      const vscodeSettingsPath = path.join(ChipMatePaths.vscodeGlobalStorage(), "settings", "mcp_settings.json")
    ...
    // Check .chipmate/mcp.json and .chipmate/mcp.json for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    // .chipmate is loaded first (lower precedence), .chipmate second (higher precedence)
    if (options?.projectDir) {
      for (const dir of [".chipmate"]) {
    ...
    const serversByName = new Map<string, ChipMateMcpServer>()
  ...
  /**
   * Load ChipMate MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(
    ...
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded chipmate MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped chipmate MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("chipmate MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load chipmate MCP servers", { error: err })
      return {}
    }
  }
}
