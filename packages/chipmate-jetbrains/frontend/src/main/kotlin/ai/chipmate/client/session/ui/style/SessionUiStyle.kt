package ai.chipmate.client.session.ui.style
...
object SessionUiStyle {
    ...
    object View {
        ...
        object Prompt {
            ...
            fun bgColor(style: SessionEditorStyle): Color = JBColor.namedColor(
                "ChipMate.Session.Prompt.Background",
        ...
        /** Markdown colors that mirror ChipMate's VS Code webview tokens. */
        object Markdown {
            fun string(): Color = JBColor.namedColor(
                "ChipMate.Session.Markdown.String",
                JBColor(0xA31515, 0xCE9178),
            )
        }

        object Todo {
            fun checkBg(): Color = JBColor.namedColor("ChipMate.Session.Todo.Checkbox.Background", Color.WHITE)

            fun checkFg(): Color = JBColor.namedColor("ChipMate.Session.Todo.Checkbox.Foreground", Color(0x1F, 0x23, 0x28))
    ...
    object Timeline {
        val READ: Color = JBColor.namedColor("ChipMate.Session.Timeline.Read", Color(0x37, 0x94, 0xff))
        val WRITE: Color = JBColor.namedColor("ChipMate.Session.Timeline.Write", Color(0x00, 0x7f, 0xd4))
        val TOOL: Color = JBColor.namedColor("ChipMate.Session.Timeline.Tool", Color(0x00, 0x7a, 0xcc))
        val SUCCESS: Color = JBColor.namedColor("Label.successForeground", UIUtil.getLabelSuccessForeground())
        val ERROR: Color = JBColor.namedColor("ChipMate.Session.Timeline.Error", UIUtil.getErrorForeground())
        val TEXT: Color = JBColor.namedColor("ChipMate.Session.Timeline.Text", UIUtil.getContextHelpForeground())
        val STEP: Color = JBColor.namedColor("ChipMate.Session.Timeline.Step", JBColor.border())
    }
}
