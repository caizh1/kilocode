import { fetchChipMateEmbeddingModelCatalog, resolveChipMateGatewayBaseUrl } from "@chipmate/chipmate-gateway"
import type { Config, IndexingConfig, ChipMateEmbeddingModelCatalog } from "@chipmate/sdk/v2"
...
export async function loadChipMateEmbeddingModels(onError?: (message: string) => void) {
  const endpoint = new URL("embedding-models", resolveChipMateGatewayBaseUrl()).toString()
  log.info("loading ChipMate embedding model catalog", { endpoint })
  const catalog = await fetchChipMateEmbeddingModelCatalog({
    onError: (issue) => {
      log.warn("failed to load ChipMate embedding model catalog", {
        code: issue.code,
        status: issue.status,
  ...
  log.info("loaded ChipMate embedding model catalog", {
...
export function chipmateModelOptions(catalog?: ChipMateEmbeddingModelCatalog) {
...
export function currentChipMateModel(catalog: ChipMateEmbeddingModelCatalog | undefined, model?: string | null) {
