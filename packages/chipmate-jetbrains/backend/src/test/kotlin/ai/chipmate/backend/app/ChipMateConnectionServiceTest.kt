...
import ai.chipmate.backend.app.ConnectionState
import ai.chipmate.backend.app.ChipMateConnectionService
...
class ChipMateConnectionServiceTest {
    ...
    fun `connect transitions to Connected`() = runBlocking {
        ...
        val svc = ChipMateConnectionService(
          scope,
          fake,
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = ChipMateConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `connect reports downloading then connecting before ready`() = runBlocking {
        ...
        val svc = ChipMateConnectionService(scope, server, {}, log)
    ...
    fun `SSE concurrent callbacks preserve callback order`() = runBlocking {
        ...
        val svc = ChipMateConnectionService(scope, fake, {}, blocked)
        val field = ChipMateConnectionService::class.java.getDeclaredField("listener")
        ...
        val sourceField = ChipMateConnectionService::class.java.getDeclaredField("source")
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = ChipMateConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to ChipMateCliDataParserTest
    ...
    fun `dispose prevents reconnect on late SSE callbacks`() = runBlocking {
        val reconnects = AtomicInteger(0)
        val svc = ChipMateConnectionService(scope, fake, { reconnects.incrementAndGet() }, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    private class BlockingLog : ChipMateLog {
