...
import ai.chipmate.client.diff.DiffLineNumbers
...
class EditToolView(
     ...
     * known, so [ai.chipmate.client.session.views.MessageView] rebinds it once the opener is wired.
     */
    @RequiresEdt
    ...
    private fun sync(): Boolean {
        ...
        val titleText = if (count > 1) ChipMateBundle.message("session.part.tool.patch") else title(item)
    ...
    private fun syncFilesTag(count: Int): Boolean {
        ...
        if (show) changed = setText(filesTag, ChipMateBundle.message("session.part.tool.edit.files", count)) || changed
        return changed
    }
...
private fun diffTitle(tool: Tool): String =
    ...
    if (editFiles(tool).size > 1) ChipMateBundle.message("session.part.tool.patch") else tail(editPath(tool))
