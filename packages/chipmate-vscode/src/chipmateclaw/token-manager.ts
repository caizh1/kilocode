/**
 * ChipMateChat access-token cache.
 *
 * Mirrors the web client pattern (see `apps/web/src/app/(app)/claw/chipmate-chat/token.ts`
 * in the cloud monorepo). The token is minted by the ChipMate gateway
 * (`chipmate.claw.chatCredentials`) and kept in memory with a 5-minute freshness
 * buffer. The gateway operation is a historical name — it used to return
 * Stream Chat credentials and was repurposed when ChipMate migrated to its own
 * chipmate-chat service.
 *
...
export class TokenManager {
  ...
  constructor(private readonly getClient: () => ChipMateClient | null) {}
  ...
  private async fetch(): Promise<ChatToken> {
    ...
    const res = await client.chipmate.claw.chatCredentials()
    ...
    if (!data.chipmateChatUrl) missing.push("chipmateChatUrl")
    ...
    return {
      ...
      chipmateChatUrl: data.chipmateChatUrl!,
