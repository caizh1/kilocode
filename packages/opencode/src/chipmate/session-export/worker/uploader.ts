...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-chipmate-export-api-version": "1",
    "x-chipmate-export-schema-version": "1",
    "x-chipmate-export-agent-version": args.agentVersion,
    "x-chipmate-export-surface": args.surface,
    "x-chipmate-export-root-session-id": first.rootSessionId,
    "x-chipmate-export-session-id": first.sessionId,
    "x-chipmate-export-batch-id": args.batchId,
    "x-chipmate-export-seq-start": String(Math.min(...seqs)),
    "x-chipmate-export-seq-end": String(Math.max(...seqs)),
    "x-chipmate-export-event-count": String(args.rows.length),
    "x-chipmate-export-payload-sha256": await sha256Hex(args.body),
    "x-chipmate-export-client-sent-at": new Date().toISOString(),
    "x-chipmate-export-content-encoding": "identity",
  })
  const token = process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-chipmate-anon-id", anon)
  return out
}
