...
import { ChipMateGatewayApi, ChipMateGatewayPaths } from "../../../src/chipmate/server/httpapi/groups/chipmate-gateway"
import { chipmateGatewayHandlers } from "../../../src/chipmate/server/httpapi/handlers/chipmate-gateway"
...
const TestHttpApi = HttpApi.make("opencode-instance").addHttpApi(ChipMateGatewayApi)
...
const layer = HttpRouter.serve(
  HttpApiBuilder.layer(TestHttpApi).pipe(
    Layer.provide(chipmateGatewayHandlers),
...
describe("ChipMate gateway HttpApi statuses", () => {
  it.live("proxies configured Qwen FIM through the provider auth store", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(ChipMateGatewayPaths.qwenFim, {
        providerID: "qwen",
  ...
  it.live("reports locally stored API authentication without a Gateway request", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(ChipMateGatewayPaths.authStatus)
  ...
  it.live("preserves cloud session list rate limits", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(ChipMateGatewayPaths.cloudSessions)
  ...
  it.live("preserves missing cloud session previews", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(ChipMateGatewayPaths.cloudSession.replace(":id", "missing"))
  ...
  it.live("preserves cloud session preview server failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(ChipMateGatewayPaths.cloudSession.replace(":id", "failed"))
  ...
  it.live("preserves cloud session import authentication failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(ChipMateGatewayPaths.cloudSessionImport, { sessionId: "unauthorized" })
  ...
  it.live("maps cloud session import transport failures to internal errors", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(ChipMateGatewayPaths.cloudSessionImport, { sessionId: "failed" })
  ...
  it.live("preserves ChipMateClaw worker failures", () =>
    Effect.gen(function* () {
      yield* stub(() => new Response("worker failed", { status: 500 }))

      const response = yield* HttpClient.get(ChipMateGatewayPaths.clawStatus)

      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "ChipMateClaw request failed: 500 worker failed" })
    }),
  )

  it.live("normalizes numeric ChipMateClaw timestamps", () =>
    Effect.gen(function* () {
  ...
  it.live("maps ChipMateClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      yield* stub(() => Promise.reject(new TypeError("network error")))
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach ChipMateClaw" })
    }),
  )
})
