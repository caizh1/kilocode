...
import ai.chipmate.client.session.controller.SessionControllerEvent.AccountOverlaySnapshot
...
class SessionAccountOverlayTest : SessionControllerTestBase() {
    ...
    private fun snap(profile: ProfileDto?) =
        AccountOverlaySnapshot(status = ChipMateAppStatusDto.READY, profile = profile)
    ...
    fun `test transient null profile keeps logged in balance badge`() {
        ...
        show(AccountOverlaySnapshot(status = ChipMateAppStatusDto.READY, profile = null, transient = true))
        edt {
            assertTrue(panel.loggedInVisible())
            assertTrue(panel.balanceVisible())
