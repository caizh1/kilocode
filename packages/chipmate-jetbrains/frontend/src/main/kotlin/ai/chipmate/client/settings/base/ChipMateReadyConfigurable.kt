...
abstract class ChipMateReadyConfigurableBase : SearchableConfigurable, Configurable.NoMargin {
    ...
    override fun createComponent(): JComponent {
        ...
        updateProject(root)
        cs.launch { service<ChipMateAppService>().connect() }
        cs.launch {
            service<ChipMateAppService>().state.collect { state ->
    ...
    @RequiresEdt
    private fun update(state: ChipMateAppStateDto) {
        checkEdt()
        if (state.status != ChipMateAppStatusDto.READY || ready != null) return
        val cs = scope ?: return
    ...
    private fun unavailable(): JComponent {
        val title = JBLabel(ChipMateBundle.message("settings.cli.unavailable.title"))
        title.font = JBFont.h3().asBold()
        val message = JBLabel(ChipMateBundle.message("settings.cli.unavailable.message"))
...
abstract class ChipMateReadyConfigurable : ChipMateReadyConfigurableBase(), Configurable.NoScroll
