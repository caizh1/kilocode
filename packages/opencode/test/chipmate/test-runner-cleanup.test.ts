...
function env(marker: string) {
  const vars: NodeJS.ProcessEnv = { ...process.env, CHIPMATE_TEST_RUNNER_PID_FILE: marker }
  delete vars.CHIPMATE_TEST_PROFILE
  delete vars.CHIPMATE_TEST_SHARD
  delete vars.CHIPMATE_TEST_CLI_PATH
  return vars
}

function spawn(name: string, marker: string, args: string[] = []) {
  return Bun.spawn(
    ["bun", "run", "script/test-runner.ts", "--concurrency", "1", "--retries", "-1", ...args, `chipmate/${name}`],
    {
      cwd: root,
      env: env(marker),
      stdout: "pipe",
...
describe("test runner cleanup", () => {
  test(
    ...
    async () => {
      ...
      const state = { pid: 0 }
      const src = [
        "const marker = process.env.CHIPMATE_TEST_RUNNER_PID_FILE",
        'if (!marker) throw new Error("CHIPMATE_TEST_RUNNER_PID_FILE is required")',
