...
import { ChipMateConfigSources } from "./sources"
import { ProductProfile } from "../product-profile"

export namespace ChipMateConfigOverlay {
  const log = Log.create({ service: "chipmate.config.overlay" })
  ...
  export const Result = z.object({
    ...
    sources: z.array(ChipMateConfigSources.Source),
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: ChipMateConfigSources.Source[]
  }

  const files = ChipMateConfig.ACTIVE_CONFIG_FILES
  ...
  const fieldPaths = [
    ...
    ["indexing", "chipmate", "apiKey"],
    ["indexing", "chipmate", "baseUrl"],
    ["indexing", "chipmate", "organizationId"],
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    // chipmate_change - project config is untrusted; confine {file:} reads to the project root
    ...
    return configs.reduce((result, cfg) => ChipMateConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? ProductProfile.project(input.directory, "chipmate.jsonc")
  }
  ...
  export async function resolve(input: Input): Promise<Result> {
    // chipmate_change start - project agents untrusted, {file:} confined to the project root; global agents trusted
    const root = input.worktree && input.worktree !== "/" ? input.worktree : input.directory
  ...
  function globalDirs() {
    ...
    return [Global.Path.config, path.join(Global.Path.home, ".chipmate")]
  }

  // chipmate_change start - root confines untrusted agent {file:} reads
  async function withAgents(input: Config.Info, dirs: string[], trusted: boolean, root?: string): Promise<Config.Info> {
    ...
    const next = ChipMateConfig.mergeConfig(ChipMateConfig.mergeConfig(input, { agent }), { agent: mode })
  ...
  async function load(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    // chipmate_change start - a single unsafe/invalid project config file must not break the settings overlay;
  ...
  async function loadUnsafe(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    ...
    // chipmate_change start - remove variable-bearing MCP headers before resolving other project file references
