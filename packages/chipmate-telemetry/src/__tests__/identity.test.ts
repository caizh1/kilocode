...
beforeEach(() => {
  profileCalls = 0
  dir = mkdtempSync(path.join(tmpdir(), "chipmate-telemetry-identity-"))
  Identity.reset()
...
describe("Identity.updateFromChipMateAuth profile cache", () => {
  test("fetches profile and writes cache keyed by token hash", async () => {
    await Identity.updateFromChipMateAuth("token-a")
  ...
  test("refetches when the token changes", async () => {
    await Identity.updateFromChipMateAuth("token-a")
    Identity.reset()
    await Identity.updateFromChipMateAuth("token-b")
  ...
  test("clears identity when token is null", async () => {
    await Identity.updateFromChipMateAuth("token-a")
    Identity.reset()
    await Identity.updateFromChipMateAuth(null)
