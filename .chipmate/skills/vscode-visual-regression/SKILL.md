...
description: Write Storybook stories and visual regression tests for the ChipMate VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/chipmate-vscode/`.
...
Stories live in `packages/chipmate-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
Snapshots live at `packages/chipmate-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/chipmate-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/chipmate-vscode/webview-ui/**`
- `packages/chipmate-vscode/.storybook/**`
- `packages/chipmate-vscode/tests/visual-regression*`
...
chipmate-ui components are imported via deep subpaths:

```tsx
import { Part } from "@chipmate/chipmate-ui/message-part"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@chipmate/sdk/v2"
...
```
globals=colorScheme:dark;theme:chipmate-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyChipMateTheme()` from chipmate-ui. Stories do NOT need to handle theming — it happens automatically.
