...
import * as Job from "@/chipmate/source-backed-design/job"
...
describe("source-backed design 分块任务控制器", () => {
  ...
  test("单目标模块可从范围、正文和四类适用图自动组装为严格有效的最终 Word", async () => {
    ...
    const previousMermaid = process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT
    const previousWord = process.env.CHIPMATE_WORD_RENDER_ENDPOINT
    process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT = mermaid.origin
    process.env.CHIPMATE_WORD_RENDER_ENDPOINT = word.origin
    try {
      await provideTestInstance({
        directory: temp.path,
        fn: async () => {
      ...
      if (previousMermaid === undefined) delete process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT
      else process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT = previousMermaid
      if (previousWord === undefined) delete process.env.CHIPMATE_WORD_RENDER_ENDPOINT
      else process.env.CHIPMATE_WORD_RENDER_ENDPOINT = previousWord
