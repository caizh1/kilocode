...
- VS Code markdown styling is split across `packages/ui/src/components/markdown.css`, `packages/chipmate-ui/src/components/markdown.css`, `packages/chipmate-ui/src/styles/vscode-bridge.css`, and message-part overrides.
...
1. Expand JetBrains markdown style tokens.
   ...
   - Compute defaults in `MdCommon.defaults(style)` from IntelliJ/editor theme sources and centralized ChipMate semantic colors where no platform key matches.
   - Use `JBColor.namedColor("ChipMate.Markdown.*", fallback)` for ChipMate-specific markdown palette fallbacks, so themes can override them and runtime code avoids scattered hardcoded colors.
...
6. Add focused tests.
   ...
   - Add a changeset: `@chipmate/chipmate-jetbrains` patch with user-facing wording such as `Improve markdown readability in JetBrains chat transcripts.`

## Verification

- Run targeted JetBrains markdown tests first from `packages/chipmate-jetbrains/`: `./gradlew frontend:test --tests '*MdView*'` if the Gradle module supports it; otherwise run the closest supported targeted Gradle test command.
- Run `bun run typecheck` from `packages/chipmate-jetbrains/`.
- If targeted Gradle filtering is unreliable, run `./gradlew test` from `packages/chipmate-jetbrains/`.

## Constraints

- Do not introduce JCEF, Compose, or Kotlin UI DSL.
- Keep changes inside `packages/chipmate-jetbrains/` and `.changeset/` unless a shared ChipMate UI source of truth is explicitly required.
- No `chipmate_change` markers are needed for JetBrains or ChipMate UI paths.
