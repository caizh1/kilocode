#!/usr/bin/env bun
/**
 * Keep ChipMate's version of specific files during merge
 *
 * This script handles files that should always keep ChipMate's version
...
export function shouldKeepOurs(filePath: string, patterns: string[]): boolean {
  const dirs = defaultConfig.chipmateDirectories.map((dir) => `${dir}/**`)
  return matches(filePath, [...patterns, ...dirs]) || patterns.some((pattern) => filePath.includes(pattern))
}

/**
 * Keep ChipMate's version of conflicted files
 */
...
/**
 * Reset specific files to ChipMate's version (even if not conflicted)
 */
export async function resetToOurs(files: string[], options: KeepOursOptions = {}): Promise<KeepOursResult[]> {
  ...
  for (const file of files) {
    if (options.dryRun) {
      ...
      try {
        // Get the file from HEAD (ChipMate's version)
...
if (import.meta.main) {
  ...
  console.log()
  success(`Kept ChipMate's version for ${kept.length} files`)
