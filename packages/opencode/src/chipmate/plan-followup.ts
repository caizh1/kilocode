...
export const PlanFollowupRuntime = {
  ...
  handover(input: LLM.StreamInput, signal: AbortSignal) {
    return llm().runPromise((svc) => ChipMateLLM.text(svc.stream(input)).pipe(Effect.orDie), { signal })
  },
      ...
      questions: [
        {
          ...
          custom: Flag.CHIPMATE_CLIENT === "cli" || Flag.CHIPMATE_CLIENT === "jetbrains",
          options: [
            {
