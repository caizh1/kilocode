...
describe("agent manager fork session", () => {
  ...
  it("uses the selected worktree directory for the handoff", async () => {
    ...
    const client = { session: { fork, promptAsync } }
    const state = {
      getWorktree: () => ({ path: "/repo/.chipmate/worktrees/feature" }),
    ...
    expect(fork).toHaveBeenCalledWith(
      { sessionID: "source", directory: "/repo/.chipmate/worktrees/feature" },
      { throwOnError: true },
    )
    expect(promptAsync).toHaveBeenCalledWith(expect.objectContaining({ directory: "/repo/.chipmate/worktrees/feature" }), {
      throwOnError: true,
    })
  })
