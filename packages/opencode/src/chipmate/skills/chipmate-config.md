# ChipMate CLI Configuration Reference

All config lives in `chipmate.json` (or `chipmate.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/chipmate/chipmate.json`), env `CHIPMATE_CONFIG`, project `./chipmate.json`, `.chipmate/chipmate.json`, `CHIPMATE_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where ChipMate looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.chipmate/`, `.chipmate/`, and `~/.config/chipmate/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.chipmate/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.chipmate/`, legacy `.chipmate/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
2. `~/.chipmate/`
3. `~/.chipmate/`
4. The `CHIPMATE_CONFIG_DIR` directory (if the env var is set)
5. project `.chipmate/` and `.chipmate/` directories from the current working directory up to the worktree root
...
## Agents (`.chipmate/agent/*.md`)

Also loaded from legacy `.chipmate/` directories and plural `agents/` variants.
...
Markdown files in `.chipmate/workflows/` or `.chipmate/workflows/` (project-level) and `~/.chipmate/workflows/` or `~/.chipmate/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.

## Agent Manager Setup And Run Scripts

For the full product guidance, use the canonical [Agent Manager reference](https://chipmate.ai/docs/automate/agent-manager) and [Agent Manager Workflows guide](https://chipmate.ai/docs/automate/agent-manager-workflows). Prefer these links instead of guessing documentation paths.

Agent Manager setup/run scripts are project files in the main repository's `.chipmate/` directory. They are not `chipmate.json` settings and should not be configured inside generated `.chipmate/worktrees/<name>/` checkouts.

Agent Manager worktrees usually live under `.chipmate/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.chipmate/setup-script`, `.chipmate/setup-script.sh` |
| Windows | `.chipmate/setup-script.ps1`, `.chipmate/setup-script.cmd`, `.chipmate/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.chipmate/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
Tool permissions: `read`, `edit`, `glob`, `grep`, `list`, `bash`, `task`, `webfetch`, `websearch`, `semantic_search`, `chipmate_memory_save`, `chipmate_memory_recall`, `lsp`, `skill`, `external_directory`, `todowrite`, `todoread`, `question`, `doom_loop`.
...
For example, this configuration will hide all models from the built-in ChipMate Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.chipmate.ai/config.json",
  "disabled_providers": ["chipmate", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `chipmate`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.chipmate/` and legacy `.chipmate/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (chipmate, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/chipmate/themes/` or `.chipmate/themes/`.
...
### Config files (chipmate.json)

| Scope | Path |
|---|---|
| Project | `./chipmate.json`, `./chipmate.jsonc`, `./opencode.json` (legacy), `./opencode.jsonc` (legacy) |
| Global | `~/.config/chipmate/chipmate.json`, `~/.config/chipmate/chipmate.jsonc`, `~/.config/chipmate/opencode.json` (legacy), `~/.config/chipmate/opencode.jsonc` (legacy), `~/.config/chipmate/config.json` (legacy) |
| Managed | Linux: `/etc/chipmate/`, macOS: `/Library/Application Support/chipmate/`, Windows: `%ProgramData%\chipmate\` — loads `chipmate.json`, `chipmate.jsonc`, `opencode.json`, `opencode.jsonc` (enterprise, highest priority) |

Each config directory (`.chipmate/` and legacy `.chipmate/`) can also contain `chipmate.json`, `chipmate.jsonc`, `opencode.json`, or `opencode.jsonc`.

### Config directories

Two directory names are scanned: `.chipmate` (canonical) and `.chipmate` (legacy fallback). Both are checked at each level, and `.chipmate` wins when both define the same entry. `.opencode` directories are not loaded.

- **Project**: walks up from CWD to the git worktree root, checking both directories at each level
- **Home**: `~/.chipmate/` and `~/.chipmate/`
- **XDG global**: `~/.config/chipmate/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/chipmate/command/*.md` (global), `~/.chipmate/command/*.md` (legacy home), and `.chipmate/commands/*.md` (project) all load commands.
...
|---|---|
| `CHIPMATE_CONFIG` | Path to an additional config file (loaded after global) |
| `CHIPMATE_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `CHIPMATE_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `CHIPMATE_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
