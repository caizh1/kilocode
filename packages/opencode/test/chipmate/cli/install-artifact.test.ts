...
const wrapper = path.join(root, "bin", "chipmate")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.CHIPMATE_BIN_PATH")
    expect(text).toContain('const base = "@chipmate/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@chipmate", "cli")
      const native = path.join(tmp, "node_modules", "@chipmate", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(
        ...
        JSON.stringify({
          optionalDependencies: {
            [`@chipmate/cli-${process.platform}-${process.arch}`]: "1.0.0",
          },
        }),
      )
      await Bun.write(
        path.join(native, "package.json"),
        JSON.stringify({ name: `@chipmate/cli-${process.platform}-${process.arch}` }),
      )
      const binary = "#!/bin/sh\n# binary\nexit 0\n"
      await Bun.write(path.join(bin, "chipmate"), binary)
      await Bun.write(path.join(bin, "chipmate-sandbox-mutation-worker.js"), "worker")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".chipmate")).text()).toBe(binary)
      expect(await Bun.file(path.join(pkg, "bin", "chipmate-sandbox-mutation-worker.js")).text()).toBe("worker")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "chipmate"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "chipmate-install-artifact-repro",
            version: "1.0.0",
            bin: {
      ...
      const commands = ["chipmate", "chipmate"]
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".chipmate")
