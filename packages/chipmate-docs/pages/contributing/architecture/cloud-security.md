---
title: "ChipMate Cloud Security Architecture"
description: "Security architecture overview for ChipMate Cloud"
---

# ChipMate Cloud Security Architecture

This page gives contributors and customer security reviewers a high-level view of ChipMate Cloud security architecture. It covers logical topology, trust boundaries, data flows, persistence, execution isolation, external integrations, and shared responsibility.

{% callout type="info" title="Static source scope" %}
This overview is based on deployable code and configuration in open-source `ChipMate-Org/cloud` repository. Static source does not prove live production enablement, rollout percentages, exact regions, retention enforcement, backup policy, WAF rules, credential rotation, or vendor settings. Validate those against live production inventory before making contractual, production, or compliance claims.
{% /callout %}
...
ChipMate Cloud combines web control plane with Cloudflare-hosted services and scoped execution environments.
...
- ChipMateClaw assistant instances run in owner-scoped provider-backed runtimes with instance-scoped storage and encrypted configuration delivery.
...
flowchart LR
  ...
  subgraph execution["Scoped execution"]
    ...
    claw["Owner-scoped ChipMateClaw runtime"]
...
| ChipMateClaw execution | Owner-scoped provider-backed runtime with persistent storage |
...
| Control plane to ChipMateClaw runtime | Owner routing, config, proxy traffic, and machine lifecycle | JWT auth, one-time code redemption, derived gateway tokens, machine API keys, Durable Object owner scope, and encrypted config delivery |
...
| ChipMate Cloud to third parties | Repository operations, model requests, billing, notifications, and telemetry | Provider credentials, opt-in where applicable, scoped tokens, and feature-specific routing |
...
ChipMate Cloud uses several authorization contexts:
...
Application records commonly scope to user or organization. Cloud Agent durable state scopes to session while sandbox allocation remains policy-selected. ChipMateClaw runtime scopes to owner or instance rather than global assistant process.
...
| AI and session content | Prompts, responses, conversation history, attachments, and session events | Inference, Cloud Agent sessions, ChipMateClaw, and enabled experiments |
...
| Runtime storage | Owner-scoped ChipMateClaw workspace and config persistence | Separate execution boundary tied to assigned runtime provider |
...
flowchart TB
  ...
  subgraph coordination ["ChipMateClaw coordination"]
    ...
    platform["ChipMateClaw platform delivery"]
    controllerDelivery["ChipMateClaw controller delivery"]
  end
...
ChipMateClaw separates lifecycle coordination from runtime process. Fly is provider path and legacy fallback, docker-local supports development, and Northflank support exists in provider model. Active rollout must be checked in live environment. See [ChipMateClaw](/docs/contributing/architecture/cloud-platform#chipmateclaw) for canonical runtime topology.
...
| Execution isolation | Cloud Agent workspaces, preview sandbox, deployment builder sandbox, town container, and owner-scoped ChipMateClaw runtimes |
| Secret handling | Protected config storage, encrypted delivery for supported runtime secrets, fail-closed ChipMateClaw bootstrap, and sensitive-log prohibitions |
...
ChipMate Cloud provides platform controls for auth, scoped authorization, internal-service separation, durable coordination, execution isolation, and protected handling of supported secrets.
