...
const paths = [/\bDatabase\.getPath\s*\(/, /\bDatabase\.Path\b/, /chipmate\.db(?:-wal|-shm)?/]
...
//    so CHIPMATE_DB was not set to :memory:.
// 2. The test process inherited CHIPMATE_DISABLE_CHANNEL_DB=true from the VS Code
//    extension. Database.getPath() therefore returned the real shared database
//    at ~/.local/share/chipmate/chipmate.db.
// 3. The upstream resetDatabase() helper deleted chipmate.db, chipmate.db-wal, and
//    chipmate.db-shm. The test then recreated the database and inserted its
//    identifiable "parent" and "child" sessions.
// 4. Conversations stored only in the deleted database were lost. Git
//    worktrees, branches, and .chipmate/agent-manager.json survived because they are
//    stored separately.
//
...
describe("test database safety", () => {
  ...
  test("preserves the resolved database when the in-memory preload is missing", async () => {
    ...
    // user database. This simulates a disk-backed CHIPMATE_DB and verifies that the
