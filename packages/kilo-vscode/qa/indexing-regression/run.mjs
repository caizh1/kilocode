...
const output = resolve(
  typeof argv.output === "string"
    ? argv.output
    : join(repository, "packages/kilo-vscode/qa/artifacts/indexing-regression", timestamp()),
)
await mkdir(output, { recursive: true })

const executorDefinitions = {
  "progress-core": {
    cwd: join(repository, "packages/kilo-indexing"),
    command: [
      "bun",
      "test",
      "test/kilocode/indexing/state-manager.test.ts",
      "test/kilocode/indexing/orchestrator.test.ts",
  ...
  "manager-lifecycle": {
    ...
    command: [
      "bun",
      "test",
      "test/kilocode/indexing/manager.test.ts",
  ...
  "lancedb-native": {
    ...
    command: [
      "bun",
      "test",
      "test/kilocode/indexing/vector-store/lancedb-vector-store.test.ts",
      "test/kilocode/indexing/vector-store/safe-lancedb-vector-store.test.ts",
      "test/kilocode/indexing/vector-store/lancedb-vector-store.native.test.ts",
      "--timeout",
      "60000",
    ],
  },
  "vscode-status-sync": {
    cwd: extension,
    command: ["bun", "test", "tests/unit/kilo-provider-indexing-refresh.test.ts", "--timeout", "30000"],
  },
}
