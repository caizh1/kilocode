...
import { ChipMateCompactionPayloadRecovery } from "../../src/chipmate/session/compaction-payload-recovery"
...
describe("ChipMateCompactionPayloadRecovery", () => {
  test("detects ChipMate gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(ChipMateCompactionPayloadRecovery.matches(error)).toBe(true)
  })
    ...
    async () => {
      ...
      stub.push((input) => {
        ...
        return Stream.fail(
          new APICallError({
            message: "Request Entity Too Large",
            url: "https://api.chipmate.ai/api/openrouter/responses",
            requestBodyValues: {},
            statusCode: 413,
