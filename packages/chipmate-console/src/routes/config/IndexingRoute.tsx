...
import type { IndexingConfig } from "@chipmate/sdk/v2/client"
...
export function IndexingRoute() {
  ...
  const chipmateModels = createMemo<SelectOption<string>[]>(() => {
    const models = catalog()?.models ?? []
    if (models.length === 0) return [{ value: "", label: "No ChipMate embedding models available", disabled: true }]
    return models.map((model) => ({
      value: model.id,
      label: `${model.name} (${model.note ? `${model.note}, ` : ""}${model.dimension}d)`,
    }))
  })
  const chipmateModel = createMemo(() => {
    const data = catalog()
    if (!data) return ""
        ...
        <section class="builder-form agent-builder-stack">
          ...
          <Card class="ui-card agent-builder-card" padding={0}>
            ...
            <div class="ui-form agent-builder-form">
              <FieldCard
                label="Provider"
                description="Automatic uses ChipMate when signed in, otherwise the provider runtime default."
                actions={
                  <SourceBadge
              ...
              <FieldCard
                ...
                description={
                  provider() === "chipmate"
                    ? "Select a ChipMate-hosted embedding model."
              ...
              >
                ...
                >
                  <CustomSelect
                    class="indexing-select"
                    label="ChipMate embedding model"
                    value={chipmateModel()}
              ...
              >
                <input
                  ...
                  disabled={Boolean(ctx.saving()) || provider() === "chipmate"}
                  onInput={(event) => number("dimension", event.currentTarget.value)}
                />
              </FieldCard>

              <Show when={provider() === "chipmate"}>
                <div class="indexing-note">
                  ChipMate embeddings use the account currently signed in to this ChipMate server. Model dimensions are supplied
                  by the catalog.
                </div>
              </Show>
                ...
                fallback={
                  ...
                  >
                    <input
                      ...
                      placeholder="Default ChipMate state directory"
                      disabled={Boolean(ctx.saving())}
                      onInput={(event) =>
