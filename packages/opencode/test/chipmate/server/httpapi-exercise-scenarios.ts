...
const skill = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".chipmate/skill/httpapi-remove/SKILL.md"),
    "---\nname: httpapi-remove\ndescription: HTTP API removal fixture.\n---\n# HTTP API remove\n",
  )
  await Bun.write(path.join(dir, ".chipmate/skill/httpapi-remove/KEEP.txt"), "synthetic sentinel\n")
}

const agent = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".chipmate/agent/httpapi-remove.md"),
...
const command = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".chipmate/command/httpapi-remove.md"),
...
function enable(ctx: ScenarioContext) {
  const dir = directory(ctx)
  return Effect.promise(() => ChipMateMemory.enable({ ctx: { directory: dir, worktree: dir } }))
}
...
export const chipmateScenarios: Scenario[] = [
  ...
  http.protected.get("/chipmate/profile", "chipmate.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/chipmate/auth-status", "chipmate.authStatus").json(200, (body) => {
    object(body)
    check(body.authenticated === false, "ChipMate auth status should report signed out")
    check(body.type === undefined, "ChipMate auth status should not expose a credential type while signed out")
  }),
  http.protected.get("/chipmate/modes", "chipmate.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/chipmate/fim", "chipmate.fim")
    .at((ctx) => ({ path: "/chipmate/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/chipmate/edit", "chipmate.edit")
    .at((ctx) => ({ path: "/chipmate/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/chipmate/audio/transcriptions", "chipmate.audio.transcriptions")
    .at((ctx) => ({
      path: "/chipmate/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/chipmate/notifications", "chipmate.notifications").json(200, array),
  http.protected.get("/chipmate/models/images", "chipmate.models.images").probe({ path: "/path" }).status(401),
  http.protected.get("/chipmate/models/transcriptions", "chipmate.models.transcriptions").probe({ path: "/path" }).status(401),
  http.protected
    .post("/chipmate/organization", "chipmate.organization.set")
    .at((ctx) => ({ path: "/chipmate/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/chipmate/claw/status", "chipmate.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/chipmate/claw/chat-credentials", "chipmate.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/chipmate/cloud-sessions", "chipmate.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/chipmate/cloud/session/{id}", "chipmate.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/chipmate/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/chipmate/cloud/session/import", "chipmate.cloud.session.import")
    .at((ctx) => ({ path: "/chipmate/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .get("/session/{sessionID}/model-usage", "chipmate.sessionModelUsage")
  ...
  http.protected
    .post("/chipmate/heap/snapshot", "chipmate.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .get("/chipmate/agent/requirements", "chipmate.agentRequirements")
    .at((ctx) => ({ path: "/chipmate/agent/requirements?agent=httpapi-agent", headers: ctx.headers() }))
    .json(200, (body, ctx) => {
      object(body)
  ...
  http.protected
    .get("/chipmate/command/files", "chipmate.commandFiles")
    ...
    .json(200, (body, ctx) => {
      ...
      check(
        item.location === path.join(directory(ctx), ".chipmate/command/httpapi-remove.md"),
  ...
  http.protected
    .post("/chipmate/command/remove", "chipmate.removeCommand")
    .inProject({ git: true, init: command })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/chipmate/command/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".chipmate/command/httpapi-remove.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "command removal should return true")
        const location = path.join(directory(ctx), ".chipmate/command/httpapi-remove.md")
        check(
  ...
  http.protected
    .post("/chipmate/skill/remove", "chipmate.removeSkill")
    .inProject({ git: true, init: skill })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/chipmate/skill/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".chipmate/skill/httpapi-remove/SKILL.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "skill removal should return true")
        const location = path.join(directory(ctx), ".chipmate/skill/httpapi-remove/SKILL.md")
        const sentinel = path.join(directory(ctx), ".chipmate/skill/httpapi-remove/KEEP.txt")
  ...
  http.protected
    .post("/chipmate/skill/refresh", "chipmate.refreshSkills")
    .inProject({ git: true })
    .mutating()
    .at((ctx) => ({ path: "/chipmate/skill/refresh", headers: ctx.headers(), body: { scope: "project" } }))
    .json(200, (body) => check(body === true, "skill refresh should return true")),
  http.protected
    .post("/chipmate/agent/remove", "chipmate.removeAgent")
    .inProject({ git: true, init: agent })
    .mutating()
    .at((ctx) => ({ path: "/chipmate/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "agent removal should return true")
        const location = path.join(directory(ctx), ".chipmate/agent/httpapi-remove.md")
        check(!(yield* Effect.promise(() => Bun.file(location).exists())), "removed agent should not remain on disk")
      }),
    ),
  http.protected
    .post("/chipmate/agent/remove", "chipmate.removeAgent")
    .at((ctx) => ({ path: "/chipmate/agent/remove", headers: ctx.headers(), body: { name: "httpapi-missing" } }))
    .status(400),
  http.protected
    .post("/chipmate/session-import/project", "chipmate.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/chipmate/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/chipmate/session-import/session", "chipmate.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/chipmate/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/chipmate/session-import/message", "chipmate.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/chipmate/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/chipmate/session-import/part", "chipmate.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
    ...
    .at((ctx) => ({
      path: "/chipmate/session-import/part",
      headers: ctx.headers(),
      body: {
