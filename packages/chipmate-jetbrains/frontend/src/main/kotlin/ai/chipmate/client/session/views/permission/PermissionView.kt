package ai.chipmate.client.session.views.permission
...
import ai.chipmate.client.session.ui.SessionView
import ai.chipmate.client.session.views.base.BaseQuestionView
...
import ai.chipmate.client.ui.iconButton
import ai.chipmate.client.ui.editor.BashCommandHighlighter
...
import ai.chipmate.client.ui.md.MdCodeBlockBorder
import ai.chipmate.client.ui.md.MdCodeBlockFactory
import ai.chipmate.client.ui.md.MdCodeBlockOptions
import ai.chipmate.client.ui.md.MdCommon
import ai.chipmate.client.ui.md.MdView
import ai.chipmate.client.ui.md.MdViewFactory
...
/**
 * Transcript-style permission view — rendered inside [ai.chipmate.client.session.ui.SessionMessageListPanel]
 * at the end of the transcript when the session is in
 * [ai.chipmate.client.session.model.SessionState.AwaitingPermission].
 *
...
class PermissionView(
    ...
    init {
        ...
        card.setHeaderIcon(AllIcons.General.Warning, ChipMateBundle.message("session.permission.title"))
        card.setContent(body)
        body.next(desc).next(codeSlot).next(diffRow).next(rules).next(state)
        card.setActions(
            listOf(
                BaseQuestionView.Action(ID_DENY, ChipMateBundle.message("session.permission.reject"), primary = false) { reject() },
                BaseQuestionView.Action(ID_RUN, ChipMateBundle.message("session.permission.allow.once"), primary = true) { allow() },
            ),
        )
        addToCenter(card)
    }
    ...
    fun show(permission: Permission) {
        ...
        card.setHeader(
            if (skillShell && !skill.isNullOrBlank())
                ...
                ChipMateBundle.message("session.permission.skillShell.title", escapeControl(skill))
            else ChipMateBundle.message("session.permission.title"),
        )
    ...
    private fun syncState(permission: Permission) {
        val msg = when (permission.state) {
            PermissionRequestState.ERROR ->
                permission.message ?: ChipMateBundle.message("session.permission.error")
            PermissionRequestState.RESPONDING ->
                ChipMateBundle.message("session.permission.responding")
            else -> null
        }
    ...
    // (packages/opencode/src/chipmate/skills/display.ts); keep the ranges in sync.
    ...
    private fun toolLabel(tool: String): String = when (tool) {
        "read" -> ChipMateBundle.message("session.permission.tool.read")
        "edit" -> ChipMateBundle.message("session.permission.tool.edit")
        "write" -> ChipMateBundle.message("session.permission.tool.write")
        "patch" -> ChipMateBundle.message("session.permission.tool.patch")
        "multiedit" -> ChipMateBundle.message("session.permission.tool.multiedit")
        "glob" -> ChipMateBundle.message("session.permission.tool.glob")
        "grep" -> ChipMateBundle.message("session.permission.tool.grep")
        "list" -> ChipMateBundle.message("session.permission.tool.list")
        "bash" -> ChipMateBundle.message("session.permission.tool.bash")
        "external_directory" -> ChipMateBundle.message("session.permission.tool.external_directory")
        "webfetch" -> ChipMateBundle.message("session.permission.tool.webfetch")
        "websearch" -> ChipMateBundle.message("session.permission.tool.websearch")
        "codesearch" -> ChipMateBundle.message("session.permission.tool.codesearch")
        "todoread" -> ChipMateBundle.message("session.permission.tool.todoread")
        "todowrite" -> ChipMateBundle.message("session.permission.tool.todowrite")
        "task" -> ChipMateBundle.message("session.permission.tool.task")
        "skill" -> ChipMateBundle.message("session.permission.tool.skill")
        "lsp" -> ChipMateBundle.message("session.permission.tool.lsp")
        else -> tool
    }
    ...
    private fun syncPrimaryText() {
        ...
        card.setActionText(
            ID_RUN,
            ChipMateBundle.message(key),
        )
        card.setActionText(
            ID_DENY,
            ChipMateBundle.message("session.permission.reject"),
        )
        syncButtons(responding)
    }
    ...
    // Test helpers
    internal fun runButtonForTest() = buttons(card).first { it.text == ChipMateBundle.message("session.permission.allow") || it.text == ChipMateBundle.message("session.permission.allow.once") }
    internal fun denyButtonForTest() = buttons(card).first { it.text == ChipMateBundle.message("session.permission.reject") }
...
internal class PermissionRulesView(
    ...
    private val title = JBLabel(ChipMateBundle.message("session.permission.rules.title"))
    ...
    private fun syncExpanded() {
        ...
        if (!ChipMatePluginSettings.getPermissionRulesExpanded()) return
        inset.add(body())
    }
    ...
    fun toggle() {
        ...
        ChipMatePluginSettings.setPermissionRulesExpanded(isExpanded())
        syncArrow()
        revalidate()
        repaint()
    }
    ...
    private class RuleRow(
        ...
        fun update(value: PermissionRuleDecision) {
            ...
            hint.text = ChipMateBundle.message(when (value) {
        ...
        private fun defaultLabel(): String = when (default) {
            PermissionRuleDecision.APPROVED -> ChipMateBundle.message("session.permission.allow")
            PermissionRuleDecision.DENIED -> ChipMateBundle.message("session.permission.reject")
            PermissionRuleDecision.PENDING -> ChipMateBundle.message("session.permission.ask")
        }
    ...
    private class RuleToggleButton(
        ...
        fun update(value: Boolean) {
            ...
            val text = ChipMateBundle.message(key)
            toolTipText = text
