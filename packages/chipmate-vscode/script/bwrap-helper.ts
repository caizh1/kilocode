...
function source() {
  ...
  if (writable) {
    throw new Error(`Refusing writable Bubblewrap executable at ${target}; set CHIPMATE_BWRAP_PATH to trust it explicitly`)
  }
  return target
}
...
export async function ensureBwrapForTarget(target: string, root?: string): Promise<string | undefined> {
  ...
  const { buildBubblewrap } = await import("../../opencode/script/chipmate/bubblewrap")
