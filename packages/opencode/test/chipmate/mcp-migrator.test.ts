...
import { McpMigrator } from "../../src/chipmate/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.ChipMateMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .chipmate/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".chipmate")
          await Bun.write(
    ...
    test("reads from legacy .chipmate/mcp.json when .chipmate/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".chipmate")
          await Bun.write(
    ...
    // Regression: malformed .chipmate/mcp.json must not prevent .chipmate/mcp.json from loading
    test("loads .chipmate/mcp.json even when .chipmate/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".chipmate", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".chipmate", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".chipmate/mcp.json overrides .chipmate/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".chipmate", "mcp.json"),
    ...
    test("does not read project-level .chipmate/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".chipmate")
          await Bun.write(
