 ...
 *
 * Reads legacy ChipMate v5.x data from VS Code SecretStorage and the extension's
...
async function migrateProvider(
  ...
  // For providers that support an organization ID (e.g. ChipMate Gateway), migrate using OAuth
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("chipmate.v2.autocomplete")
