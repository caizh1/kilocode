...
describe("saveCustomProvider", () => {
  it("uses a normal final disposal request after deferred writes", async () => {
    ...
    expect(calls).toEqual([{ headers: { "x-chipmate-memory-operation": "custom-provider:abc" } }])
  })
...
describe("fetchProviderData", () => {
  ...
  it("uses local ChipMate auth status instead of profile availability", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "chipmate", name: "ChipMate Gateway", source: "custom", env: [], models: {} }],
            connected: ["chipmate"],
