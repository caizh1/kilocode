...
class WorkspacePathScopingTest {
    ...
    private val base: Path = Path.of(System.getProperty("user.home")).resolve("chipmate-scope-test").normalize()
    ...
    @Test
    fun `workspace scope keeps normal project and chipmate plan files`() {
        assertEquals("backend/src/Main.java", relativeWithinWorkspace(base, at("backend", "src", "Main.java")))
        assertEquals(".chipmate/plans/x.md", relativeWithinWorkspace(base, at(".chipmate", "plans", "x.md")))
    }

    @Test
    fun `workspace scope rejects managed worktree storage from main checkout`() {
        assertNull(relativeWithinWorkspace(base, at(".chipmate", "worktrees")))
        assertNull(relativeWithinWorkspace(base, at(".chipmate", "worktrees", "foo", "backend", "src", "Main.java")))
    }

    @Test
    fun `workspace scope allows files inside the active worktree`() {
        val root = at(".chipmate", "worktrees", "foo")
    ...
    fun `workspace scope rejects sibling worktrees from active worktree`() {
        val root = at(".chipmate", "worktrees", "foo")
        val sibling = at(".chipmate", "worktrees", "bar", "backend", "src", "Main.java")
    ...
    fun `workspace scope rejects nested managed worktree storage`() {
        ...
        assertNull(relativeWithinWorkspace(root, root.resolve(".chipmate/worktrees/bar/backend/src/Main.java")))
    }
    ...
    fun `git changes returns capped large diff without blocking`() = runBlocking {
        ...
        try {
            ...
            val diff = assertNotNull(ChipMateWorkspaceRpcApiImpl().gitChanges(dir.toString()))
    ...
    fun `git changes returns null outside git repositories`() = runBlocking {
        val dir = Files.createTempDirectory("chipmate-non-git")
        try {
            assertNull(ChipMateWorkspaceRpcApiImpl().gitChanges(dir.toString()))
        } finally {
            delete(dir)
        }
    }
    ...
    private fun repo(): Path? {
        ...
        val dir = Files.createTempDirectory("chipmate-git")
        git(dir, "init")
