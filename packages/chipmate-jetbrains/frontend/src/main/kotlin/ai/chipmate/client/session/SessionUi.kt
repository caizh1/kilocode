...
import ai.chipmate.client.diff.ChipMateDiffEditorKind
import ai.chipmate.client.diff.ChipMateInlineDiffStore
import ai.chipmate.client.diff.diffParams
import ai.chipmate.client.diff.ensureDiffEditorKind
import ai.chipmate.client.migration.ChipMateMigrationService
import ai.chipmate.client.migration.MigrationUiController
import ai.chipmate.client.migration.MigrationUiState
import ai.chipmate.client.migration.ui.MigrationOverlayPanel
...
import ai.chipmate.client.session.scroll.SessionScroll
import ai.chipmate.client.session.ui.ConnectionPanel
import ai.chipmate.client.session.ui.empty.EmptySessionPanel
import ai.chipmate.client.session.ui.LoadingPanel
import ai.chipmate.client.session.ui.ReasoningPicker
import ai.chipmate.client.session.ui.RevertBanner
import ai.chipmate.client.session.ui.mode.ModePicker
import ai.chipmate.client.session.ui.model.ModelPicker
import ai.chipmate.client.session.ui.prompt.ChipMatePromptCompletionProvider
import ai.chipmate.client.session.ui.prompt.MentionAction
import ai.chipmate.client.session.ui.prompt.PromptPanel
import ai.chipmate.client.session.ui.prompt.SlashAction
import ai.chipmate.client.session.ui.prompt.mentionParts as promptMentionParts
import ai.chipmate.client.session.ui.account.SessionAccountOverlay
import ai.chipmate.client.session.ui.popup.HeaderPopupController
import ai.chipmate.client.session.ui.SessionDropOverlay
import ai.chipmate.client.session.ui.SessionRootPanel
import ai.chipmate.client.session.ui.SessionMessageListPanel
import ai.chipmate.client.session.ui.attachment.AttachmentEditorKind
import ai.chipmate.client.session.ui.attachment.attachmentParams
import ai.chipmate.client.session.ui.attachment.ensureAttachmentEditorKind
import ai.chipmate.client.session.ui.attachment.isEmbeddedAttachment
import ai.chipmate.client.session.ui.header.SessionHeaderPanel
import ai.chipmate.client.session.ui.selection.SessionContextMenu
import ai.chipmate.client.session.ui.selection.SessionHoverCopyOverlay
...
import ai.chipmate.client.session.controller.EVENT_FLUSH_MS
import ai.chipmate.client.session.controller.SessionController
import ai.chipmate.client.session.controller.SessionControllerEvent
import ai.chipmate.client.session.context.EditorContextGatherer
import ai.chipmate.client.session.ui.style.SessionUiStyle
import ai.chipmate.client.session.views.LoginRequiredView
import ai.chipmate.client.session.views.permission.PermissionView
import ai.chipmate.client.session.views.question.QuestionView
...
import ai.chipmate.client.vfs.ChipMateVfsManager
...
import ai.chipmate.rpc.dto.SessionRevertDto
...
class SessionUi(
    project: Project,
    workspace: Workspace,
    sessions: ChipMateSessionService,
    ...
    private val migration: MigrationUiController = service<ChipMateMigrationService>(),
    private val timers: UiTimerSource = UiTimers,
) : JPanel(BorderLayout()), Disposable, SessionEditorStyleTarget, UiDataProvider {

    companion object {
        private val LOG = ChipMateLog.create(SessionUi::class.java)
    ...
    private val flushMs =
        Registry.intValue("chipmate.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        comp = this,
        flushMs = flushMs,
        condense = Registry.`is`("chipmate.session.condense", true),
        displayMs = displayMs,
    ...
    private lateinit var completion: ChipMatePromptCompletionProvider
    ...
    private fun buildUi() {
        ...
        completion = ChipMatePromptCompletionProvider(
            workspace = workspace,
            service = workspaces,
    ...
    private fun syncPromptRevert() {
        ...
        val text = msg.parts.values.filterIsInstance<ai.chipmate.client.session.model.Text>().firstOrNull()?.content?.toString() ?: return
        prompt.setText(text)
    ...
    private fun slashActions(): List<SlashAction> {
        val fns: Map<SlashAction.Spec, () -> Unit> = mapOf(
            ...
            SlashAction.SETTINGS to { openChipMateSettings() },
            SlashAction.HELP to { BrowserUtil.browse("https://chipmate.ai/docs") },
        )
    ...
    private fun bind(spec: SlashAction.Spec, action: () -> Unit) = SlashAction(
        spec.name,
        ChipMateBundle.message(spec.descriptionKey),
        spec.hints,
        {
    ...
    private fun openInlineDiff(files: List<DiffFileDto>, title: String, key: String) {
        ...
        cs.launch {
            ...
            val label = branch?.let { ChipMateBundle.message("diff.editor.inline.title.named", title, it) } ?: title
            LOG.info("open inline diff session=${controller.id ?: "pending"} dir=${ChatLogSummary.dir(dir)} files=${files.size}")
            withContext(Dispatchers.Main) {
                ensureDiffEditorKind()
                project.service<ChipMateInlineDiffStore>().put(key, files)
                project.service<ChipMateVfsManager>().open(
                    ChipMateDiffEditorKind.ID,
    ...
    private fun openBranchDiff(branch: String?) {
        ...
        ensureDiffEditorKind()
        val dir = workspace.directory
        val title = branch?.let { ChipMateBundle.message("diff.editor.branch.title.named", it) }
            ?: ChipMateBundle.message("diff.editor.branch.title")
        project.service<ChipMateVfsManager>().open(
            ChipMateDiffEditorKind.ID,
    ...
    private fun openAttachment(messageId: String, item: FileAttachment) {
        ...
        if (isEmbeddedAttachment(url)) {
            ...
            LOG.info("kind=attachment-open route=chipmate-vfs session=$id message=$messageId part=${item.id} name=${attachmentName(item)}")
    ...
    private fun openChipMateSettings() {
        ShowSettingsUtil.getInstance().showSettingsDialog(
            project,
            Predicate { cfg: Configurable ->
                cfg is ConfigurableWithId && cfg.getId() == ChipMateSettingsConfigurable.ID
            },
            { _: Configurable -> },
        )
    }
...
private fun ai.chipmate.client.session.model.QuestionItem.planFollowup() =
