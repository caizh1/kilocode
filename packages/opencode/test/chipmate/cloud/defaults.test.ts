...
import { CloudAuth } from "@/chipmate/cloud/auth"
import { CloudCatalog } from "@/chipmate/cloud/catalog"
import { CloudDefaults } from "@/chipmate/cloud/defaults"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "@/chipmate/cloud/response-json"
...
const authLayer = (info: Auth.Info) =>
  Layer.mock(Auth.Service)({
    get: (id) => Effect.succeed(id === "chipmate" ? info : undefined),
  })
      ...
      Bun.serve({
        ...
        fetch(request) {
          ...
          requests.push({
            ...
            feature: request.headers.get("x-chipmate-feature"),
            organization: request.headers.get("x-chipmate-organizationid"),
            path: url.pathname,
          })
...
it.instance(
    ...
    withCatalog(["anthropic/explicit", "anthropic/default"], "anthropic/default", (url, requests) =>
      Effect.gen(function* () {
        ...
        const auth = Layer.mock(Auth.Service)({
          get: (id) => (id === "chipmate" ? Ref.get(current) : Effect.succeed(undefined)),
          set: (id, info) => (id === "chipmate" ? Ref.set(current, info) : Effect.void),
        })

        const resolved = yield* Effect.gen(function* () {
          const result = yield* CloudDefaults.resolve({
            mode: "debug",
            model: "chipmate/anthropic/explicit",
            orgID: explicitID,
            env: {
              CHIPMATE_API_KEY: "ignored-env-token",
              CHIPMATE_ORG_ID: envID,
            },
          })
          const service = yield* Auth.Service
          expect(yield* service.get("chipmate")).toEqual(stored)
          return result
        }).pipe(
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { debug: { providerID: "chipmate", modelID: "anthropic/saved" } },
                }),
              ),
              CloudCatalog.layer({ env: { CHIPMATE_API_URL: url.origin } }),
            ),
          ),
        )
    ...
    ),
  {
    config: {
      default_agent: "plan",
      model: "chipmate/anthropic/repository",
      agent: { plan: { model: "chipmate/anthropic/mode" } },
    },
  },
)
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { code: { providerID: "chipmate", modelID: "anthropic/stale" } },
                  recent: [{ providerID: "chipmate", modelID: "anthropic/recent" }],
                }),
              ),
              CloudCatalog.layer({ env: { CHIPMATE_API_URL: url.origin } }),
            ),
      ...
      CloudDefaults.resolve().pipe(
        ...
        Effect.provide(
          Layer.mergeAll(
            ...
            stateLayer(
              state({
                model: { code: { providerID: "chipmate", modelID: "anthropic/saved" } },
              }),
            ),
            CloudCatalog.layer({ env: { CHIPMATE_API_URL: url.origin } }),
          ),
...
it.instance(
    ...
    ),
  {
    config: {
      ...
      agent: {
        code: { model: "chipmate/anthropic/code" },
        custom: { mode: "primary", model: "chipmate/anthropic/custom" },
      },
    },
  },
)
...
it.instance("rejects invalid persisted organization state and insecure catalog origins", () =>
  Effect.gen(function* () {
    ...
    const catalog = yield* CloudDefaults.resolve({ env: { CHIPMATE_API_URL: "http://example.com" } }).pipe(
      Effect.provide(
        Layer.mergeAll(
          authLayer(new Auth.Api({ type: "api", key: "stored-api-token" })),
          stateLayer(state()),
          CloudCatalog.layer({
            env: { CHIPMATE_API_URL: "http://example.com" },
    ...
    withCatalog(
      ...
      (url, requests) =>
        Effect.gen(function* () {
          ...
          const resolved = yield* CloudDefaults.resolve({
            env: { CHIPMATE_API_KEY: "ignored-env-token" },
          }).pipe(
            Effect.provide(
              Layer.mergeAll(
                ...
                stateLayer(
                  state({
                    model: { plan: { providerID: "chipmate", modelID: "anthropic/saved" } },
                    recent: [{ providerID: "chipmate", modelID: "anthropic/recent" }],
                  }),
                ),
                CloudCatalog.layer({ env: { CHIPMATE_API_URL: url.origin } }),
