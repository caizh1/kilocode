...
const jb = "packages/chipmate-jetbrains"
const props = `${jb}/gradle.properties`
const pkg = `${jb}/package.json`
const zip = `${jb}/backend/build/generated/chipmate-cli-res/chipmate-cli.zip`
const hint = `${jb}/.gradle/chipmate-cli-pin.properties`
...
if (values.help || !cmd) {
  console.log(`
Usage: bun .chipmate/skills/jetbrains-cli-pin/script/cli-pin.ts <command> [--no-verify]

Commands:
  pin      Pin the JetBrains plugin to the latest released CLI. Cleans artifacts,
           sets chipmate.cli.pinned=true, bumps package.json to the latest release,
           then verifies with a cold gradle build (needs network + Java 21).
  unpin    Use the local repo CLI. Cleans artifacts, sets chipmate.cli.pinned=false,
...
async function pinned() {
  ...
  const line = text.split(/\r?\n/).find((l) => l.startsWith("chipmate.cli.pinned="))
  return (line?.split("=", 2)[1]?.trim().toLowerCase() ?? "true") === "true"
}

async function setPinned(value: boolean) {
  const text = await Bun.file(props).text()
  if (!/^chipmate\.cli\.pinned=.*$/m.test(text)) throw new Error(`chipmate.cli.pinned not found in ${props}`)
  await Bun.write(props, text.replace(/^chipmate\.cli\.pinned=.*$/m, `chipmate.cli.pinned=${value}`))
}
...
async function report() {
  ...
  console.log(`\nState: chipmate.cli.pinned=${await pinned()}, package.json version=${version}`)
}
...
if (cmd === "pin") {
  ...
  await $`bun .chipmate/skills/release-jetbrains/script/set-pin.ts --latest`
  if (!values["no-verify"]) {
