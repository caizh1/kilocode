...
| Rule source discovery | Auto-discovers from `.chipmate/rules/`, `.chipmaterules`, `.roorules`, `.clinerules`, mode-specific dirs, `AGENTS.md` | Only shows explicitly configured paths in `config.instructions` |
...
- **Auto-discovery**: Consider adding a section showing auto-discovered rule sources (`.chipmate/rules/`, `AGENTS.md`) that the CLI loads regardless of explicit `config.instructions` entries
