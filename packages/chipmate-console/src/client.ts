import { createChipMateClient, type Config as EffectiveConfig } from "@chipmate/sdk/v2/client"
import type {
  ...
  GlobalEvent,
  ChipMateEmbeddingModelCatalog,
  ChipmateProfileResponse,
  LspStatusResponse,
  McpStatusResponse,
  Pty as PtyInfo,
  PermissionRequest,
  Project as ChipMateProject,
  QuestionRequest,
  ProviderAuthAuthorization,
  ProviderAuthResponse,
  ProviderListResponse,
  Session as ChipMateSession,
  SessionStatus,
  ToolIdsResponse,
  ToolListResponse,
...
export type ChipMateProfileData = ChipmateProfileResponse

export type ProjectItem = ChipMateProject
...
const key = "chipmate.config.server"
...
function server(input: string) {
  ...
  const user = decode(url.username) || "chipmate"
  const pass = decode(url.password) || "chipmate"
  url.username = ""
  url.password = ""
  url.search = ""
  url.hash = ""
...
function owner(items: ProjectItem[], session: Pick<ChipMateSession, "projectID" | "directory">) {
...
export async function loadEmbeddingModels(input: Query): Promise<ChipMateEmbeddingModelCatalog> {
  return demand("ChipMate embedding models", await client(input).indexing.models())
}

export async function loadChipMateProfile(input: ProjectQuery): Promise<ChipMateProfileData> {
  const sdk = client(input)
  const result = await sdk.chipmate.profile(directory(input))
  return demand("ChipMate profile", result)
}

export async function setChipMateOrganization(input: ProjectQuery, organizationId: string | null) {
  const sdk = client(input)
  const result = await sdk.chipmate.organization.set({ ...directory(input), organizationId })
  demand("Switch ChipMate account", result)
  await sdk.global.dispose()
}

export async function logoutChipMate(input: ProjectQuery) {
  const sdk = client(input)
  const result = await sdk.auth.remove({ providerID: "chipmate" })
  demand("Log out of ChipMate", result)
  await sdk.global.dispose()
}

export async function startChipMateLogin(input: ProjectQuery): Promise<ProviderAuthAuthorization> {
  const sdk = client(input)
  const result = await sdk.provider.oauth.authorize({ ...directory(input), providerID: "chipmate", method: 0 })
  return demand("Start ChipMate login", result)
}

export async function completeChipMateLogin(input: ProjectQuery, signal?: AbortSignal) {
  const sdk = client(input)
  const result = await sdk.provider.oauth.callback(
    { ...directory(input), providerID: "chipmate", method: 0 },
    signal ? { signal } : undefined,
  )
  demand("Complete ChipMate login", result)
  await sdk.global.dispose()
}
...
export async function loadProjectTerminals(input: ProjectQuery, dir: string): Promise<ProjectTerminalItem[]> {
  ...
  const sessions = new Map<string, ChipMateSession>()
...
export async function createProjectPty(input: Query, dir: string, title = "ChipMate session"): Promise<ProjectPtyInfo> {
  const sdk = client({ url: input.url, dir })
  const result = await sdk.pty.create({ directory: dir, command: "chipmate", cwd: dir, title })
