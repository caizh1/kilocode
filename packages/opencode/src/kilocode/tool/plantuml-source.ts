...
function attachment(filename: string | undefined, ctx: Tool.Context): Loaded {
  const current = KiloSessionMessageOrder.latest(ctx.messages).userMessage
...
function workspace(input: string, ctx: Tool.Context): Effect.Effect<Loaded> {
  return Effect.gen(function* () {
    ...
    const inspected = yield* settle(KiloReadObject.file(requested))
    ...
    const read = yield* settle(
      KiloReadObject.use(inspected.value, (file) =>
        Effect.tryPromise({
