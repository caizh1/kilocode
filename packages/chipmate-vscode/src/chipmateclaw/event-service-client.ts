 ...
 *
 * Minimal inline port of `@chipmate/event-service` (cloud monorepo). Connects
 * to the chipmate events Cloudflare Worker using a two-step ticket flow:
...
import type { ChipMateChatEventMap, ChipMateChatEventName } from "./types"

const WS_SUBPROTOCOL = "chipmate.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends ChipMateChatEventName>(event: N, handler: (ctx: string, payload: ChipMateChatEventMap[N]) => void): () => void {
    ...
    // chipmate-chat worker edge before broadcast.
    const wrapped: EventHandler = (ctx, payload) => handler(ctx, payload as ChipMateChatEventMap[N])
    set.add(wrapped)
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[ChipMate New] event-service server error", m)
    }
  }
