 ...
 *
 * Owns the ChipMate Chat HTTP client + event-service WebSocket connection
...
import { ChipMateChatApiError, ChipMateChatClient } from "./chipmate-chat-client"
...
import type {
  ...
  ChipMateClawInMessage,
  ChipMateClawOutMessage,
  ChipMateClawState,
  Message,
  MessageCreatedEvent,
  MessageDeletedEvent,
...
export class ChipMateClawProvider implements vscode.Disposable {
  static readonly viewType = "chipmate.v2.ChipMateClawPanel"
  ...
  private chat: ChipMateChatClient | null = null
  ...
  openPanel(): void {
    ...
    const panel = vscode.window.createWebviewPanel(ChipMateClawProvider.viewType, "ChipMateClaw", vscode.ViewColumn.One, {
      enableScripts: true,
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.uri, "assets", "icons", "chipmate-light.svg"),
      dark: vscode.Uri.joinPath(this.uri, "assets", "icons", "chipmate-dark.svg"),
    }
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      ...
      motionBaseUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "assets", "loading-motion")),
      workerUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "shiki-worker.js")),
      title: "ChipMateClaw",
    })

    const msgSub = panel.webview.onDidReceiveMessage((msg: ChipMateClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "chipmateclaw.locale", locale })
    })
  ...
  private post(msg: ChipMateClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: ChipMateClawInMessage): Promise<void> {
    switch (msg.type) {
      case "chipmateclaw.ready":
        await this.init()
        return
      case "chipmateclaw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("chipmate.v2").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "chipmateclaw.state", state: { phase: "loading", locale: this.locale } })
      ...
      this.post({ type: "chipmateclaw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
   ...
   */
  private async bootstrap(client: ChipMateClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "chipmateclaw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async resolveStatus(client: ChipMateClient, gen: number): Promise<boolean> {
    const statusRes = await client.chipmate.claw.status().catch(() => null)
    if (this.stale(gen)) return false
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateClaw chat token fetch failed:", message)
      // Token fetch typically fails when the instance hasn't been upgraded
      // to support chipmate-chat — surface that as the upgrade prompt.
      this.post({ type: "chipmateclaw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "chipmateclaw.error", error: "Authentication expired" })
      },
    })
    this.events = events

    const chat = new ChipMateChatClient({
      baseUrl: envelope.chipmateChatUrl,
      getToken: () => tokens.get(),
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "chipmateclaw.error", error: "Authentication expired" })
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[ChipMate New] ChipMateClaw event-service connect failed:", message)
      this.post({
        type: "chipmateclaw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[ChipMate New] ChipMateClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[ChipMate New] ChipMateClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[ChipMate New] ChipMateClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<ChipMateClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[ChipMate New] ChipMateClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[ChipMate New] ChipMateClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/chipmateclaw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/chipmateclaw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/chipmateclaw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "chipmateclaw.activeConversation", conversationId: null })
          this.post({
            type: "chipmateclaw.messages",
            conversationId: e.conversationId,
            messages: [],
            hasMore: false,
            replace: true,
          })
          this.post({ type: "chipmateclaw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "chipmateclaw.botStatus", status: this.botStatus })
      }),
    )
      ...
      events.on("message.created", (ctx, e: MessageCreatedEvent) => {
        ...
        if (e.clientId) {
          ...
          if (idx !== -1) {
            ...
            this.post({
              type: "chipmateclaw.messageReplaced",
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "chipmateclaw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "chipmateclaw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "chipmateclaw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "chipmateclaw.activeConversation", conversationId: null })
        this.post({ type: "chipmateclaw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    // chipmate-chat validates clientId as a ULID (Crockford Base32); generate
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "chipmateclaw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[ChipMate New] ChipMateClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "chipmateclaw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "chipmateclaw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[ChipMate New] ChipMateClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[ChipMate New] ChipMateClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "chipmateclaw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "chipmateclaw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private broadcastConversations(opts: { replace: boolean }): void {
    this.post({
      type: "chipmateclaw.conversations",
      conversations: this.conversations,
  ...
  private formatError(err: unknown, fallback: string): string {
    if (err instanceof ChipMateChatApiError) {
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      ...
      this.chat.requestBotStatus(this.sandboxId).catch((err) => {
        console.debug("[ChipMate New] ChipMateClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const res = await client.chipmate.claw.status()
      if (res?.data) {
        this.status = res.data as ClawStatus
        this.post({ type: "chipmateclaw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[ChipMate New] ChipMateClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
