...
import { Storage } from "@/chipmate/session-export/worker/storage"
import { Uploader, backoffFor } from "@/chipmate/session-export/worker/uploader"
import { Config } from "@/chipmate/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN
    else process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.CHIPMATE_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-chipmate-export-api-version")).toBe("1")
    expect(headers.get("x-chipmate-export-schema-version")).toBe("1")
    expect(headers.get("x-chipmate-export-agent-version")).toBe("v0")
    expect(headers.get("x-chipmate-export-root-session-id")).toBe("s1")
    expect(headers.get("x-chipmate-export-session-id")).toBe("s1")
    expect(headers.get("x-chipmate-export-seq-start")).toBe("0")
    expect(headers.get("x-chipmate-export-seq-end")).toBe("0")
    expect(headers.get("x-chipmate-export-event-count")).toBe("1")
    expect(headers.get("x-chipmate-export-content-encoding")).toBe("identity")
    expect(headers.get("x-chipmate-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-chipmate-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-chipmate-export-api-version",
      "x-chipmate-export-schema-version",
      "x-chipmate-export-agent-version",
      "x-chipmate-export-surface",
      "x-chipmate-export-root-session-id",
      "x-chipmate-export-session-id",
      "x-chipmate-export-batch-id",
      "x-chipmate-export-seq-start",
      "x-chipmate-export-seq-end",
      "x-chipmate-export-event-count",
      "x-chipmate-export-payload-sha256",
      "x-chipmate-export-client-sent-at",
      "x-chipmate-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-chipmate-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-chipmate-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-chipmate-export-surface")).toBe("vscode-extension")
