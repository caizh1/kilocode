...
import { matchFollowup, recordFollowup } from "../../src/chipmate-provider/followup-session"
...
describe("followup-session", () => {
  ...
  it("matches pending follow-ups by normalized directory before expiry", () => {
    const pending = {
      dir: "c:/repo/.chipmate/worktrees/feature",
      time: 1,
    }

    expect(matchFollowup({ pending, dir: "C:\\repo\\.chipmate\\worktrees\\feature\\", now: 2 })).toBe(true)
    expect(matchFollowup({ pending, dir: "c:/repo/.chipmate/worktrees/other", now: 2 })).toBe(false)
    expect(matchFollowup({ pending, dir: "c:/repo/.chipmate/worktrees/feature", now: 30_002 })).toBe(false)
  })
