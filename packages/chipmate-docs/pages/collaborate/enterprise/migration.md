---
title: "Migration"
description: "Migrate your team to ChipMate Enterprise"
---

# Migration

Switch to **ChipMate Teams** or **ChipMate Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to ChipMate
...
**ChipMate Teams** and **ChipMate Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**ChipMate Teams** and **ChipMate Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**ChipMate Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **ChipMate Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with ChipMate Teams or ChipMate Enterprise
...
3. **Calculate current costs** (if visible) vs. ChipMate pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: ChipMate Setup**

1. **Create organization** at [app.chipmate.ai](https://app.chipmate.ai)
2. **Subscribe to Teams ($15/user/month)** or **Enterprise ([Contact Sales](https://chipmate.ai/contact-sales))**
3. **Configure team settings** and usage policies
4. **Purchase initial AI credits** based on usage estimates

**Step 3: Team Migration**

1. **Invite team members** to ChipMate
2. **Install ChipMate extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on ChipMate features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in ChipMate dashboard
...
| Cursor Feature | ChipMate Equivalent |
|---|---|
...
2. **Install ChipMate** extension for team members
3. **Start with simple tasks** in ChipMate
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use ChipMate** for new projects
...
ChipMate uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.chipmate/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.chipmate/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in ChipMate (no conversion needed)
- **Global rules**: Recreate in `~/.chipmate/rules/*.md` directory

ChipMate also supports mode-specific rules (`.chipmate/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- ChipMate extension installation and setup
...
Ready to make the switch? Contact our migration team at migrations@chipmate.ai to plan your transition to transparent AI coding.
