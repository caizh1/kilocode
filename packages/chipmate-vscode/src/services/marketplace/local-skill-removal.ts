...
export class LocalSkillRemoval {
  ...
  async remove(
    ...
    try {
      ...
      progress("removing")
      await client.chipmate.removeSkill(
...
async function list(client: Awaited<ReturnType<ChipMateConnectionService["getClientAsync"]>>, directory: string) {
...
function contains(root: string, location: string) {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === "ENOENT") return false
    console.warn("[ChipMate New] Failed to resolve local Skill scope:", root, location, err)
    return false
  }
}
