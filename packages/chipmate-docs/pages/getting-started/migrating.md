...
description: "Guide for migrating to ChipMate from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to ChipMate. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

ChipMate supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why ChipMate's Rules System?

ChipMate simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in ChipMate

| Cursor | ChipMate | Key Difference |
|---|---|---|
| `.cursor/rules/*.mdc` with YAML frontmatter | `.chipmate/rules/*.md` plain Markdown | No YAML metadata required |
...
| Global rules in UI settings | `~/.chipmate/rules/*.md` files | Global rules stored as files in home folder |
...
**2. Create ChipMate directory:**

```bash
mkdir -p .chipmate/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".chipmate/rules/${basename%.mdc}.md"
done
...
- Save to `~/.chipmate/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .chipmate/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. ChipMate handles this through **mode-specific directories** instead.
...
**ChipMate approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .chipmate/rules-code
# Save TypeScript-specific rules here
```

**ChipMate approach (Option 2 - Custom mode):**

```yaml
# .chipmatemodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.chipmate/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. ChipMate uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# ChipMate: .chipmate/rules/backend-server-api-rules.md
```
...
| Windsurf | ChipMate | Key Difference |
|---|---|---|
| `.windsurf/rules/*.md` | `.chipmate/rules/*.md` | Same Markdown format |
| GUI configuration for activation modes | File location determines scope | Scope controlled by directory structure |
| "Always On" mode (GUI) | Place in `.chipmate/rules/` | Rules stored as files, not GUI settings |
...
| Global rules in `~/.codeium/windsurf/memories/global_rules.md` | `~/.chipmate/rules/*.md` | Global rules in home folder, multiple files |
...
```bash
cp .windsurf/rules/*.md .chipmate/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.chipmate/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .chipmate/rules/legacy-rules.md
```
...
# .chipmate/rules/api-conventions.md
# .chipmate/rules/testing-standards.md
# .chipmate/rules/code-style.md
```

### Converting Windsurf's Activation Modes

Windsurf configures activation through the GUI. In ChipMate, file organization replaces GUI configuration:

| Windsurf GUI Mode | ChipMate Equivalent |
|---|---|
| **Always On** | Place in `.chipmate/rules/` (default) |
...
Then place the rule in `.chipmate/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in ChipMate automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - ChipMate loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). ChipMate also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in ChipMate and require user approval to modify.

## Understanding Mode-Specific Rules

This is ChipMate's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.chipmate/rules/              # Apply to ALL modes
.chipmate/rules-code/         # Only in Code mode
.chipmate/rules-debug/        # Only in Debug mode
.chipmate/rules-ask/          # Only in Ask mode
.chipmate/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .chipmate/rules-test

# 2. Save rule as plain Markdown
cat > .chipmate/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in ChipMate panel
- [ ] **Test rule application:** Ask ChipMate to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.chipmate/rules/` location
- [ ] **Commit to version control:** `git add .chipmate/`
...
```bash
ls -la .chipmate/rules/      # Project rules
ls -la ~/.chipmate/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.chipmate/rules/`
...
- **After migrating:** Organize files accordingly in ChipMate

### Nested Rules Flattened

Cursor's nested directories don't map to ChipMate. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .chipmate/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in ChipMate settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

ChipMate supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .chipmate/rules-review
mkdir -p .chipmate/rules-docs
```

**Note:** `.chipmatemodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
...
- [Join our Discord](https://chipmate.ai/discord) for migration support
