...
describe("instruction markdown substitutions", () => {
  it.live("preserves trusted relative instructions when project config is disabled", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const prior = {
          flag: process.env.CHIPMATE_DISABLE_PROJECT_CONFIG,
          secret: process.env.CHIPMATE_INSTRUCTION_GLOBAL_PATTERN_SECRET,
        }
        process.env.CHIPMATE_DISABLE_PROJECT_CONFIG = "1"
        process.env.CHIPMATE_INSTRUCTION_GLOBAL_PATTERN_SECRET = "environment secret"
        return prior
      }),
      () =>
        Effect.gen(function* () {
          ...
          yield* write(
            ...
            "{env:CHIPMATE_INSTRUCTION_GLOBAL_PATTERN_SECRET}",
          )
      ...
      (prior) =>
        Effect.sync(() => {
          if (prior.flag === undefined) delete process.env.CHIPMATE_DISABLE_PROJECT_CONFIG
          else process.env.CHIPMATE_DISABLE_PROJECT_CONFIG = prior.flag
          if (prior.secret === undefined) delete process.env.CHIPMATE_INSTRUCTION_GLOBAL_PATTERN_SECRET
          else process.env.CHIPMATE_INSTRUCTION_GLOBAL_PATTERN_SECRET = prior.secret
        }),
    ),
  )
  ...
  it.live("does not trust project markdown selected by a trusted relative instruction", () =>
    provideTmpdirInstance((dir) => {
      ...
      return Effect.gen(function* () {
        const name = "CHIPMATE_INSTRUCTION_RELATIVE_SECRET"
  ...
  it.live("does not trust a global-path instruction selected by project config", () =>
    Effect.gen(function* () {
      ...
      const name = "CHIPMATE_INSTRUCTION_SELECTED_SECRET"
      process.env[name] = "environment secret"
      yield* write(path.join(project, "README.md"), "project")
      ...
      const config = TestConfig.layer({
        get: () =>
          Effect.succeed({
            instructions: [item],
            instruction_origins: {
              [item]: { trusted: false, source: path.join(project, "chipmate.json"), root: project },
            },
          }),
      })
  ...
  it.live("trusts a global-path instruction declared by trusted config", () =>
    Effect.gen(function* () {
      ...
      const item = path.join(home, "private.md")
      const secret = path.join(dir, "secret.txt")
      const name = "CHIPMATE_INSTRUCTION_TRUSTED_SECRET"
      process.env[name] = "environment secret"
      yield* write(path.join(project, "README.md"), "project")
      yield* write(secret, "file secret")
      yield* write(item, [`{file:${secret}}`, `{env:${name}}`].join("\n"))
  ...
  it.live("omits nearby project instructions with environment substitutions", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const name = "CHIPMATE_INSTRUCTION_PROJECT_SECRET"
        process.env[name] = "environment secret"
  ...
  it.live("preserves substitutions in trusted global instructions", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const name = "CHIPMATE_INSTRUCTION_GLOBAL_SECRET"
        process.env[name] = "environment secret"
        const home = path.join(dir, "global")
