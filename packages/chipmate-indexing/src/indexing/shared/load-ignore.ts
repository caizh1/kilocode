...
const product = process.env.CHIPMATE_PRODUCT_PROFILE === "chipmate-v2"
const ignoreFile = product ? ".chipmate-v2ignore" : ".chipmateignore"
...
function compile(sorted: Entry[]): IgnoreMatcher {
...
export async function loadIgnore(root: string): Promise<IgnoreMatcher> {
  return compile(await collect(root))
}

export async function loadIgnoreWithFingerprint(root: string): Promise<{ ignore: IgnoreMatcher; fingerprint: string }> {
  const entries = await collect(root)
  const parts = [
    `builtin:${FileIgnore.PATTERNS.join("\n")}`,
    ...entries.map((entry) => `${entry.dir}/${entry.name}:${entry.txt ?? ""}`),
  ]
  const fingerprint = createHash("sha256").update(parts.join("\0")).digest("hex")
  return { ignore: compile(entries), fingerprint }
}
