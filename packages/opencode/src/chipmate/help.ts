...
async function getHelpText(name: string, cmd: Cmd): Promise<string> {
  const inst = yargs([])
    .scriptName(name ? `chipmate ${name}` : "chipmate")
    .wrap(null)
  if (cmd.builder) {
...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`chipmate ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`chipmate ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `chipmate ${section.name}` : "chipmate"}`)
    parts.push("")
    if (section.hidden) {
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `chipmate ${section.name}` : "chipmate"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- chipmate ${sub.name} [internal] ---` : `--- chipmate ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "chipmate" + raw.slice(2) : "chipmate " + raw
