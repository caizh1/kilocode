...
describe("config resilience", () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Filesystem.write(
          path.join(project, "chipmate.json"),
  ...
  test("skips project markdown that references environment or out-of-project files", async () => {
    const name = "CHIPMATE_CONFIG_MARKDOWN_PROJECT_SECRET"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Filesystem.write(path.join(project, ".chipmate", "agent", "unsafe.md"), prompt)
          await Filesystem.write(path.join(project, ".chipmate", "command", "unsafe.md"), prompt)
  ...
  test("skips project markdown symlinks that escape the project root", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const project = path.join(dir, "project")
        const item = path.join(project, ".chipmate", "agent", "unsafe.md")
  ...
  test.serial(
    ...
    async () => {
      const name = "CHIPMATE_EXTERNAL_MARKDOWN_SECRET"
      const env = process.env[name]
      const config = Flag.CHIPMATE_CONFIG
      process.env[name] = "environment secret"
      await using tmp = await tmpdir({
        init: async (dir) => {
          const project = path.join(dir, "project")
          ...
          await fs.symlink(agents, path.join(project, ".chipmate", "agents"), type)
          await fs.symlink(commands, path.join(project, ".chipmate", "commands"), type)
          await Filesystem.write(
            global,
            JSON.stringify({
              permission: {
      ...
      Flag.CHIPMATE_CONFIG = tmp.extra.global
  ...
  test("skips invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".chipmate", "agent", "skip.md"),
          `---
mode: "banana"
---
Broken agent prompt`,
        ...
        await Filesystem.write(
          path.join(dir, ".chipmate", "agent", "keep.md"),
          `---
model: test/model
---
Valid agent prompt`,
...
Broken agent prompt`,
  ...
  test("skips invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".chipmate", "command", "skip.md"),
          `---
subtask: "banana"
---
Broken command template`,
        ...
        await Filesystem.write(
          path.join(dir, ".chipmate", "command", "keep.md"),
          `---
...
Broken command template`,
  ...
  test("collects warnings for invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".chipmate", "agent", "broken.md"),
          `---
mode: "banana"
---
Broken agent`,
  ...
  test("collects warnings for invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".chipmate", "command", "broken.md"),
          `---
subtask: "banana"
---
Broken command`,
  ...
  test("collects warnings for invalid JSON in .chipmate directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".chipmate", "chipmate.json"), "{ not valid json !!!")
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(warns.some((w) => w.path.includes("chipmate.json") && w.message.includes("not valid JSON"))).toBe(true)
      },
    })
  })

  test("collects warnings for invalid schema in .chipmate directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".chipmate", "chipmate.json"), JSON.stringify({ unknownField: true }))
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(cfg).toBeDefined()
        expect(warns.some((w) => w.path.includes("chipmate.json") && w.message.includes("invalid"))).toBe(true)
      },
    })
  })
