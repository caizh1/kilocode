...
import { AllowEverythingPermission } from "../../../src/chipmate/permission/allow-everything"
...
const original = {
  password: Flag.CHIPMATE_SERVER_PASSWORD,
  username: Flag.CHIPMATE_SERVER_USERNAME,
  envPassword: process.env.CHIPMATE_SERVER_PASSWORD,
  envUsername: process.env.CHIPMATE_SERVER_USERNAME,
}

afterEach(() => {
  Flag.CHIPMATE_SERVER_PASSWORD = original.password
  Flag.CHIPMATE_SERVER_USERNAME = original.username
...
const auth = () => `Basic ${Buffer.from("chipmate:secret").toString("base64")}`
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const blocked = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-chipmate-directory": tmp.path },
        ...
        const enable = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-chipmate-directory": tmp.path, authorization: auth() },
          body: JSON.stringify({ enable: true }),
        })
