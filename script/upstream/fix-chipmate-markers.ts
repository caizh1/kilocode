#!/usr/bin/env bun
/**
 * Rebuild chipmate_change markers for one file by comparing it with the last
 * merged upstream version.
 *
 * Usage:
 *   bun run script/upstream/fix-chipmate-markers.ts packages/opencode/src/file.ts
 *   bun run script/upstream/fix-chipmate-markers.ts packages/opencode/src/file.ts --dry-run
 */
...
function usage() {
  console.log(`Usage: bun run script/upstream/fix-chipmate-markers.ts <repo-relative-file> [--dry-run]

Rebuilds chipmate_change markers by:
  ...
  4. Removing existing chipmate_change markers and adding fresh markers around remaining changed lines.
...
async function main() {
  ...
  header("Fix chipmate_change markers")
  ...
  if (base === null && annotates(file)) warn(`${file} does not exist upstream; marked as a new ChipMate file`)
  ...
  if (next === current) {
    success(`${file} already has normalized chipmate_change markers`)
    return
  }
