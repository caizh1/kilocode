...
// Each ChipMateViewers.layer construction reads these env vars (post-refactor), so
// setting them here controls presence wiring per test.
process.env.CHIPMATE_EVENT_SERVICE_URL = "wss://test-presence"
...
mock.module("@/chipmate/event-service/client", () => ({
...
describe("ChipMateViewers.Service presence contexts", () => {
  ...
  test("kill switch blocks the presence socket but attached union still reaches ChipMateSessions", async () => {
    attachedCalls.length = 0
    current = new FakeClient()
    const prev = process.env.CHIPMATE_DISABLE_PRESENCE
    process.env.CHIPMATE_DISABLE_PRESENCE = "1"
    try {
      await run((v) =>
        v.update({ viewer: { id: uid, active: true }, attached: ["ses_a"], visible: ["ses_a"] }),
      )
      ...
      if (prev === undefined) delete process.env.CHIPMATE_DISABLE_PRESENCE
      else process.env.CHIPMATE_DISABLE_PRESENCE = prev
    }
  })
})
...
describe("ChipMateViewers.Service viewer lifecycle", () => {
  ...
  test("account switch disconnects the old client before connecting the new one", async () => {
    ...
    await run(
      ...
      ChipMateViewers.layer.pipe(Layer.provide(mutableAuthLayer)),
    )
