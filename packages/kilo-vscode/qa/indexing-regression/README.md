...
`--package-bin` 必须指向与当前机器操作系统和架构匹配的扩展 `bin` 目录。执行器会启动包内真实 `kilo-indexer`，通过正式 IPC 驱动 CodeGraph、Code RAG、Document RAG、检索、配置更新和 HTTP 404 故障路径。

Windows PowerShell 示例：

```powershell
cd C:\path\to\kilocode\packages\kilo-vscode
...
```bash
cd /path/to/kilocode/packages/kilo-vscode
