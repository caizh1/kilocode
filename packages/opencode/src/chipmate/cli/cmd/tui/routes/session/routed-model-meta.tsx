...
import type { AssistantMessage, Part, Provider, StepFinishPart } from "@chipmate/sdk/v2"
...
export namespace RoutedModelMeta {
  ...
  export function label(list: Providers, model: StepFinishPart["model"]) {
    ...
    const id = ChipMateRoutedModel.display(model.modelID)
    ...
    return ChipMateRoutedModel.displayName(text)
  }

  function routed(model: StepFinishPart["model"], message: Message) {
    if (!model) return undefined
    if (message.providerID !== "chipmate") return undefined
    if (!message.modelID.startsWith("chipmate-auto/")) return undefined
