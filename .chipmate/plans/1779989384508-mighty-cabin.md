...
5. Verification.
   - Run the targeted JetBrains frontend session scroll tests from `packages/chipmate-jetbrains/` using Gradle test filtering for `SessionScrollTest` if supported.
   - Run `./gradlew typecheck` from `packages/chipmate-jetbrains/` if the targeted tests pass or if Gradle filtering is unavailable.
...
- `packages/chipmate-jetbrains/frontend/src/test/kotlin/ai/chipmate/client/session/SessionScrollTest.kt`
