...
describe("sandbox launch preparation", () => {
  ...
  test("blocks configured credential reads on macOS and Linux backends", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-sandbox-read-"))
    ...
    try {
      ...
      const linux = generateBubblewrap(profile, launch, "/opt/chipmate/bwrap")
  ...
  test("keeps the host network namespace in Linux allow mode", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-policy-"))
    ...
    try {
      const result = generateBubblewrap(profile, { ...launch, cwd: root }, "/opt/chipmate/bwrap")
      const writable = result.args.indexOf("--bind")
      const protectedPath = result.args.indexOf("--ro-bind", writable + 1)
      expect(result.command).toBe("/opt/chipmate/bwrap")
  ...
  test("isolates the Linux network namespace in deny mode", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-network-"))
  ...
  test("re-binds unreadable directories read-only instead of failing Linux setup", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-unreadable-"))
    const git = path.join(root, ".git")
  ...
  test("rejects an unreadable writable root", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-root-"))
    ...
    try {
      ...
      expect(() => generateBubblewrap(profile, { ...launch, cwd: root }, "/opt/chipmate/bwrap")).toThrow(
  ...
  test("parses escaped mount points from Linux mountinfo", () => {
    const content = [
      ...
      String.raw`37 36 0:33 / /tmp/chipmate\040root rw - tmpfs tmpfs rw`,
      String.raw`38 37 0:34 / /tmp/chipmate\040root/nested\011mount rw - tmpfs tmpfs rw`,
      String.raw`39 36 0:35 / /tmp/back\134slash rw - tmpfs tmpfs rw`,
      "",
    ].join("\n")

    expect(parseMountinfo(content)).toEqual(["/", "/tmp/chipmate root", "/tmp/chipmate root/nested\tmount", "/tmp/back\\slash"])
  })

  test("allows a mounted writable root but rejects nested mount points", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-mount-"))
    ...
    try {
      expect(() => generateBubblewrap(profile, launch, "/opt/chipmate/bwrap", [root])).not.toThrow()
      expect(() => generateBubblewrap(profile, launch, "/opt/chipmate/bwrap", [root, nested])).toThrow(
  ...
  test("rejects a Bubblewrap executable inside a writable root", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-bubblewrap-helper-"))
  ...
  test("prepares proxy mode when platform support is available", async () => {
    ...
    expect(Result.isSuccess(result)).toBe(true)
    if (Result.isSuccess(result)) expect(result.success.environment?.HTTPS_PROXY).toContain("http://chipmate:")
  })
