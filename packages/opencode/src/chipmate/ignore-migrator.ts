...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "chipmate.ignore-migrator" })

  const IGNORE_FILE = ProductProfile.chipmate ? ".chipmate-v2ignore" : ".chipmateignore"
  const GLOBAL_IGNORE = ProductProfile.chipmate
    ? path.join(ProductProfile.config()!, IGNORE_FILE)
    : path.join(os.homedir(), ".chipmate", IGNORE_FILE)
  ...
  /**
   * Parse .chipmateignore content into patterns.
  ...
  /**
   * Load patterns from a .chipmateignore file
   */
  ...
  /**
   * Migrate .chipmateignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .chipmateignore (lower priority)
    ...
    // 2. Load project .chipmateignore (higher priority - added last)
  ...
  /**
   * Load .chipmateignore and return permission config.
