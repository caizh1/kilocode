...
## CHIPMATE_CHANGE_MARKERS.md

Review carefully, file by file, whether we accidentally removed any `chipmate_change` marker.

Get the full list of files changed in the reviewed PR. Check each changed file. For each file, compare both our `main` branch and the upstream-merged version in the reviewed PR. Determine whether any marker removal, marker move, or ChipMate-specific change makes sense, and comment on that.
...
Check whether the merge now mentions OpenCode somewhere user-facing instead of ChipMate, or links to OpenCode web properties.
...
Check whether any merged files now use `chipmate_change` markers without any actual difference to upstream.
...
For each `chipmate_change` in this PR, trace the full chain: where the value or behavior is introduced, where it needs to flow through, and where it is ultimately consumed. Verify every link in the chain still exists after the merge.
...
Check whether this PR introduces or re-introduces fallback logic for `opencode` config files, or accidentally breaks code that now correctly expects only `.chipmate`-based configuration.

ChipMate removed fallback support for `opencode` config directories. Look for:
...
- Changes that break `.chipmate`-only config lookup by removing or reordering it in a multi-path search.

When in doubt, add a finding. A human should verify config path changes manually.

## TESTS.md

Check whether this PR removed any ChipMate-specific tests.

ChipMate-specific tests may live in paths containing `chipmate` or `chipmate`, or may include ChipMate-specific assertions, fixtures, or `chipmate_change` markers.
