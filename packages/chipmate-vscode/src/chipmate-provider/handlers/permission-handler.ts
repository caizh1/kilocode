/**
 * Permission handlers — extracted from ChipMateProvider.
 *
...
import type { ChipMateClient, PermissionRequest } from "@chipmate/sdk/v2/client"
...
export async function handlePermissionResponse(
  ...
  if (!target) {
    console.error("[ChipMate New] ChipMateProvider: No sessionID for permission response")
    ctx.postMessage({ type: "permissionError", permissionID: permissionId })
    return
  }
  ...
  if (approvedAlways.length > 0 || deniedAlways.length > 0) {
    const saveResult = await ctx.client.permission
      ...
      .catch((error: unknown) => {
        ...
        console.error("[ChipMate New] ChipMateProvider: Failed to save always-rules:", error)
        ctx.postMessage({ type: "permissionError", permissionID: permissionId })
  ...
  const replyResult = await ctx.client.permission
    ...
    .catch((error: unknown) => {
      if (isNotFoundError(error)) return "stale" as const
      console.error("[ChipMate New] ChipMateProvider: Failed to respond to permission:", error)
      ctx.postMessage({ type: "permissionError", permissionID: permissionId })
      return "error" as const
    })
...
export async function fetchAndSendPendingPermissions(ctx: PermissionContext): Promise<void> {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      if (error) {
        console.error(`[ChipMate New] ChipMateProvider: Failed to fetch pending permissions for ${dir}:`, error)
        continue
      }
      valid.add(dir)
      if (!data) continue
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to fetch pending permissions:", error)
  }
}
