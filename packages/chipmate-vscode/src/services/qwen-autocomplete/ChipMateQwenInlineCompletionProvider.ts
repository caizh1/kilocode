...
export class ChipMateQwenInlineCompletionProvider implements vscode.InlineCompletionItemProvider, vscode.Disposable {
  ...
  private injectable(cfg: QwenAutocompleteConfig, state: SnippetState): InjectionState {
    ...
    const priority = [...state.edited, ...state.opened, ...state.imports, ...state.root]
  ...
  private injectItem(
    ...
    // qwen/ChipMate adapter deviation from Continue selection parity: Continue
  ...
  private logDone(req: Pending, started: number, prefix: number, suffix: number, text: string): void {
    this.log(
      `[ChipMate New] qwen-autocomplete requestId=${req.id} pathHash=${pathHash(req.path)} line=${req.line + 1} character=${
        req.character + 1
