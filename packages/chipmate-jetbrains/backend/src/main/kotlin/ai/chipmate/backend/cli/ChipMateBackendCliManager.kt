...
import ai.chipmate.backend.dev.ChipMateDevMode
...
/**
 * Manages the ChipMate CLI binary lifecycle.
 *
 * Downloads the pinned CLI into IntelliJ's system directory,
 * spawns `chipmate serve --port 0`, and exposes the result as [State].
 *
 * Concurrency is handled by the owning [ChipMateBackendAppService] — all public
 * methods except [exited] are called under its mutex. [exited] is called from
 * [ChipMateConnectionService]'s IO dispatcher and is thread-safe via the stale-ref
 * guard and volatile [process] field.
 */
class ChipMateBackendCliManager(
    private val log: ChipMateLog = ChipMateLog.create(ChipMateBackendCliManager::class.java),
    ...
    @Volatile
    private var job: ChipMateProcessJob? = null
    ...
    private suspend fun resolveCli(onProgress: (CliDownload) -> Unit): File {
        ...
        forceExtract = false
        val version = ChipMateProps.cliVersion()
        val platform = ChipMateCliPlatform.current()
        if (ChipMateRepoCli.available()) {
            if (force) log.info("Force re-extracting bundled CLI $version")
            log.info("ChipMate CLI mode: BUNDLED — using CLI $version ($platform) shipped in the plugin; no download needed")
            val cli = ChipMateRepoCli.extract(force)
            onProgress(CliDownload(100, version, platform))
            return cli
        }
        if (force) log.info("Force re-downloading CLI $version")
        log.info("ChipMate CLI mode: DOWNLOAD — resolving CLI $version ($platform) from the GitHub release")
        return ChipMateCliDownloader(log = log).resolve(version, force, onProgress)
    }
    ...
    internal fun buildEnv(pwd: String, base: Map<String, String> = EnvironmentUtil.getEnvironmentMap()): Map<String, String> =
        buildChipMateCliEnv(pwd, base, log)
    ...
    private suspend fun spawn(cli: File, start: Long): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            log.info("CLI env: CHIPMATE_CLIENT=jetbrains CHIPMATE_PLATFORM=jetbrains CHIPMATE_APP_NAME=chipmate-code")
            val proc = try {
                builder.start()
            } catch (e: Exception) {
            ...
            // can never be orphaned. Null on other platforms / when unavailable — see ChipMateProcessJob.
            val jobHandle = ChipMateProcessJob.assign(proc.pid(), log)
            ...
            }, "chipmate-cli-stderr").apply { isDaemon = true; start() }
            this@ChipMateBackendCliManager.stderr = err
    ...
    private fun clearJob(): ChipMateProcessJob? = synchronized(lock) {
        val current = job
        job = null
        current
    }
 ...
 */
private fun confirmKilled(proc: Process, kids: List<ProcessHandle>, log: ChipMateLog, timeoutSeconds: Long) {
    val parentExited = proc.waitFor(timeoutSeconds, TimeUnit.SECONDS)
 ...
 * order deadlocked IDE shutdown; the order is locked by ChipMateBackendCliShutdownTest. Never waits, so
...
internal fun closeStreams(proc: Process, log: ChipMateLog) {
...
internal fun startupDiagnostics(cli: File, env: Map<String, String>, log: ChipMateLog): String {
    val home = System.getProperty("user.home").orEmpty()
    val profile = EnvironmentUtil.getValue("USERPROFILE").orEmpty()
    val data = env["XDG_DATA_HOME"] ?: home.takeIf { it.isNotBlank() }?.let { File(it, ".local/share/chipmate").absolutePath }.orEmpty()
    ...
    if (data.isNotBlank() && remote(data)) {
        lines += "warning: ChipMate CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk."
    }
    val text = lines.joinToString("\n")
    log.info("CLI startup diagnostics:\n$text")
    if (data.isNotBlank() && remote(data)) {
        log.warn("ChipMate CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk.")
    }
    return text
}
...
internal suspend fun awaitReady(
    ...
    onTimeout: () -> Unit,
    diagnostics: () -> String,
    log: ChipMateLog = ChipMateLog.create(ChipMateBackendCliManager::class.java),
    ...
    }, "chipmate-cli-stdout").apply { isDaemon = true; start() }
    onThread(thread)
...
// Must be called from a background thread — devStorageEnv() performs blocking I/O (mkdirs).
internal fun buildChipMateCliEnv(
    pwd: String,
    base: Map<String, String> = EnvironmentUtil.getEnvironmentMap(),
    log: ChipMateLog = ChipMateLog.create(ChipMateBackendCliManager::class.java),
): Map<String, String> = buildMap {
    putAll(base)
    put("CHIPMATE_SERVER_PASSWORD", pwd)
    ...
    put("CHIPMATE_PARENT_PID", ProcessHandle.current().pid().toString())
    put("CHIPMATE_CLIENT", "jetbrains")
    put("CHIPMATE_ENABLE_QUESTION_TOOL", "true")
    put("CHIPMATE_PLATFORM", "jetbrains")
    put("CHIPMATE_APP_NAME", "chipmate-code")
    put("CHIPMATE_TELEMETRY_LEVEL", if (ChipMateDevMode.enabled()) "off" else "all")
    if (!ChipMateClaudeCompatSettings.get()) put("CHIPMATE_DISABLE_CLAUDE_CODE", "true")
    put("CHIPMATE_FEATURE", "jetbrains-plugin")
    putIfAbsent("CHIPMATE_CONFIG_CONTENT", DEFAULT_CONFIG)
    ideEnv(log).forEach { entry -> put(entry.key, entry.value) }
    devStorageEnv(log)?.forEach { entry -> put(entry.key, entry.value) }
}

private fun ideEnv(log: ChipMateLog): Map<String, String> = buildMap {
    runCatching {
        val info = ApplicationInfo.getInstance()
        val name = info.fullApplicationName
        val build = info.build.asString()
        put("CHIPMATE_EDITOR_NAME", name)
        put("CHIPMATE_EDITOR_NAME", "$name $build")
    }.onFailure { log.info("Could not read ApplicationInfo: ${it.message}") }

    runCatching {
        val version = ChipMatePlugin.version()
        if (version != null) put("CHIPMATE_APP_VERSION", version)
    }.onFailure { log.info("Could not read plugin version: ${it.message}") }

    runCatching {
        put("CHIPMATE_MACHINE_ID", machineId())
    }.onFailure { log.info("Could not read machine ID: ${it.message}") }
}

private fun machineId(): String {
    val file = File(PathManager.getSystemPath(), "chipmate/machine-id")
...
private fun devStorageEnv(log: ChipMateLog): Map<String, String>? {
    val enabled = System.getProperty("chipmate.dev.storage.isolated", "false").toBoolean()
    if (!enabled) return null
    val root = System.getProperty("chipmate.dev.worktree.root") ?: run {
        log.warn("chipmate.dev.storage.isolated=true but chipmate.dev.worktree.root is not set; skipping dev storage isolation")
        return null
    }
    val dev = File(root, ".chipmate-dev")
