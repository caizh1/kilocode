...
import ai.chipmate.backend.workspace.ChipMateBackendWorkspace
...
class ChipMateBackendWorkspaceTest {
    ...
    private fun setup(): ChipMateBackendAppService =
        ChipMateBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }

    private suspend fun connect(app: ChipMateBackendAppService) {
        app.connect()
    ...
    private suspend fun ready(app: ChipMateBackendAppService): ChipMateBackendWorkspace {
        connect(app)
        return app.workspaces.get("/test/project")
    }

    private suspend fun loaded(ws: ChipMateBackendWorkspace) {
        withTimeout(15_000) {
            ws.state.first { it is ChipMateWorkspaceState.Ready }
        }
    }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        val state = ws.state.value as ChipMateWorkspaceState.Ready
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<ChipMateWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<ChipMateWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is ChipMateWorkspaceState.Error }
        }

        val err = ws.state.value as ChipMateWorkspaceState.Error
    ...
    // ------ Data mapping ------
    // Detailed provider/command/path parsing correctness is covered in ChipMateCliDataParserTest.
    ...
    fun `concurrent get for same directory returns same instance`() = runBlocking {
        val port = mock.start()
        val http = ChipMateBackendHttpClients.api(mock.password)
        val api = DefaultApi(basePath = "http://127.0.0.1:$port", client = http)
        val events = MutableSharedFlow<SseEvent>()
        val sessions = ChipMateBackendSessionManager(scope, log)
        val manager = ChipMateBackendWorkspaceManager(scope, sessions, log)
        ...
        try {
            ...
            manager.stop()
            ChipMateBackendHttpClients.shutdown(http)
        }
    }
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        val state = withTimeout(15_000) {
            ws.state.first {
                it is ChipMateWorkspaceState.Ready &&
                    it.providers.providers.firstOrNull()?.id == "openai"
            }
        } as ChipMateWorkspaceState.Ready
    ...
    fun `SSE global disposed triggers full app reload with new data`() = runBlocking {
        ...
        val prev = (app.appState.value as ChipMateAppState.Ready).rev
        val before = mock.requestCount("/global/config")
        val reload = async(start = CoroutineStart.UNDISPATCHED) {
            app.appState.first { it is ChipMateAppState.Ready && it.rev > prev }
        }
