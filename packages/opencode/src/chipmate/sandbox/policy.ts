...
import { backendSupport, run as runSandbox, unrestricted, type Profile } from "@chipmate/sandbox"
import { Bus } from "@/bus"
...
const snapshot = Effect.fn("SandboxPolicy.snapshot")(function* (sessionID: SessionID) {
  ...
  return yield* locked(
    ...
    Effect.gen(function* () {
      ...
      // A session's create-time chipmate.sandbox toggle takes precedence over the config default, so a
