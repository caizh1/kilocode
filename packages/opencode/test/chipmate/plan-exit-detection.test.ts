...
import { PlanFollowup } from "../../src/chipmate/plan-followup"
...
describe("plan_exit detection", () => {
        ...
        tools: [
          {
            tool: "plan_exit",
            input: {},
            output: "Plan is ready at .chipmate/plans/plan.md. Ending planning turn.",
          },
        ],
      })
  ...
  test("ChipMateSessionPrompt resolves plan follow-up through the supplied question service", () =>
    withInstance(async () => {
      const seeded = await seed({
        text: "Here is the plan",
        tools: [
      ...
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const pending = ChipMateSessionPrompt.askPlanFollowup({
            sessionID: seeded.sessionID,
            messages: seeded.messages,
            abort: AbortSignal.any([]),
            question,
  ...
  test("ChipMateSessionPrompt cleans listener-local plan follow-up when aborted outside instance context", () => {
  ...
  test("JetBrains client enables plan follow-up with custom answer", () =>
    withInstance(async () => {
      ...
      try {
        process.env.CHIPMATE_CLIENT = "jetbrains"
        const seeded = await seed({
          text: "Here is the plan",
          tools: [
            {
  ...
  test("plan reminder reuses custom plan_exit path when refining", () =>
    withInstance(async () => {
      ...
      await ChipMateSessionPrompt.insertPlanReminders({
