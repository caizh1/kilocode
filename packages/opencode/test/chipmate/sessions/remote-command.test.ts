...
import { RemoteExit } from "../../../src/chipmate-sessions/remote-exit"
...
describe("RemoteCommand", () => {
  ...
  test("validates structured command requests without duplicating session identity", () => {
    const valid = {
      ...
      model: { providerID: "chipmate", modelID: "anthropic/claude-sonnet-4" },
      variant: "high",
    }
  ...
  test("advertises only canonical exit while the embedded worker callback is registered", () => {
    ...
    // chipmate_change - K1 W1: canExitSession is always true, independent of
  ...
  test("catalog omits exit until the embedded worker TUI is ready", async () => {
    ...
    // chipmate_change - K1 W1: canExitSession is true even when the synthetic
    // `/exit` entry is absent (e.g. a headless `chipmate remote` host has no
