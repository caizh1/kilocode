/**
 * ChipMate Gateway Commands for TUI
 *
 * Provides /profile and /teams commands that are only visible when connected to ChipMate Gateway.
 */
...
import { DialogChipMateTeamSelect } from "./components/dialog-chipmate-team-select.js"
import { DialogChipMateProfile } from "./components/dialog-chipmate-profile.js"
...
export function registerChipMateCommands(useSDK: () => UseSDK) {
  ...
  // Only show ChipMate commands when connected to ChipMate Gateway
  const isChipMateConnected = createMemo(() => {
    return sync.data.provider_next.connected.includes("chipmate")
  })
  const indexing = createMemo(() => indexingEnabled(sync.data.config))

  useBindings(() => ({
    commands: [
      // /chipmateclaw command
      {
        name: "chipmate.claw",
        title: "ChipMateClaw",
        desc: "Open ChipMateClaw chat & dashboard",
        category: "ChipMate",
        slashName: "chipmateclaw",
        slashAliases: ["claw"],
        enabled: isChipMateConnected(),
        hidden: !isChipMateConnected(),
        run: async () => {
          // Fetch profile (for org context) and instance status in parallel
          const [profileRes, res] = await Promise.all([
            sdk.client.chipmate.profile().catch(() => null),
            sdk.client.chipmate.claw.status().catch(() => null),
          ])
          ...
          const creds = await sdk.client.chipmate.claw.chatCredentials().catch(() => null)
          ...
          route.navigate({ type: "chipmateclaw" })
          dialog.clear()
        },
      },
      ...
      // /profile command
      {
        name: "chipmate.profile",
        title: "Profile",
        desc: "View your ChipMate Gateway profile",
        category: "ChipMate",
        slashName: "profile",
        ...
        run: async () => {
          try {
            ...
            const response = await sdk.client.chipmate.profile()

            if (response.error || !response.data) {
              dialog.replace(() => (
                <DialogAlert
                  title="Error"
                  message="Failed to fetch profile. Please ensure you're authenticated with ChipMate Gateway."
                />
              ))
              return
            }
            ...
            dialog.replace(() => <DialogChipMateProfile profile={profile} balance={balance} currentOrgId={currentOrgId} />)
          } catch (error) {
      ...
      // /teams command
      {
        name: "chipmate.teams",
        title: "Teams",
        desc: "Switch between ChipMate Gateway teams",
        category: "ChipMate",
        slashName: "teams",
          ...
          try {
            ...
            if (response.error || !response.data) {
              dialog.replace(() => (
                <DialogAlert
                  title="Error"
                  message="Failed to fetch teams. Please ensure you're authenticated with ChipMate Gateway."
                />
              ))
              return
            }
            ...
            if (!profile.organizations || profile.organizations.length === 0) {
              dialog.replace(() => (
                <DialogAlert
                  ...
                  message="You're not a member of any teams.\nVisit https://app.chipmate.ai to create or join a team."
                />
              ))
              return
            }
              ...
              <DialogChipMateTeamSelect
                ...
                onSelect={async (orgId) => {
                  try {
                    ...
                    const result = await sdk.client.chipmate.organization.set({
