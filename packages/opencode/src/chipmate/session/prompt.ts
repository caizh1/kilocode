...
import { PlanFollowup } from "@/chipmate/plan-followup"
import { PlanFile } from "@/chipmate/plan-file"
...
import { environmentDetails } from "@/chipmate/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util/filesystem"
import NATIVE_PLAN_PROMPT from "@/chipmate/session/native-plan-prompt.txt"
...
import { consumeAutoTitle, markAutoTitle } from "@/chipmate-sessions/rename-adoptions"

export namespace ChipMateSessionPrompt {
  ...
  export function system(input: {
    ...
    if (ChipMateSystemPrompt.appends(input.agent))
  ...
  export const abortIntakes = Effect.fn("ChipMateSessionPrompt.abortIntakes")(function* (sessionID: SessionID) {
    const entries = [...(intakes.get(sessionID) ?? [])]
  ...
  function supportsPlanFollowup() {
    return ["cli", "vscode", "jetbrains"].includes(Flag.CHIPMATE_CLIENT)
  }
  ...
  export const cancelTree = Effect.fn("ChipMateSessionPrompt.cancelTree")(function* (input: {
    sessionID: SessionID
    ...
    yield* Effect.forEach(
      ...
      (sessionID) =>
        Effect.gen(function* () {
          yield* ChipMateSessionPromptQueue.cancel(sessionID)
  ...
  export const recoverProviderFinishError = Effect.fn("ChipMateSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
  ...
  export const askPermission = Effect.fn("ChipMateSessionPrompt.askPermission")(function* (input: {
    ...
    // chipmate_change start - tag every rule with its true origin before merging, so the winning
  ...
  export function memoryToolEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return ChipMateToolRegistry.memoryToolsEnabled({ ctx: input.ctx })
  }
  ...
  export const memoryInject = Effect.fn("ChipMateSessionPrompt.memoryInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
    record: boolean
    ...
    const verbose =
      input.cache.verbose ??
      (enabled
        ? yield* Effect.tryPromise(() => ChipMateMemory.status({ ctx: input.ctx })).pipe(
    ...
    const built =
      cached?.enabled === enabled
        ? cached
        : yield* ChipMateSystemPrompt.memoryBlocks({
            ctx: input.ctx,
            sessionID: input.sessionID,
            record: input.record,
            enabled,
   ...
   * OneDrive ReparsePoint directories in some Node versions (chipmate#9755).
   */
  ...
  export async function insertPlanReminders(input: {
    ...
    if (mode(input.agent.name) === "plan") add(ChipMateSystemPrompt.brand(NATIVE_PLAN_PROMPT))
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, ChipMateSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): ChipMateSession.CloseReason {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || ChipMateSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
