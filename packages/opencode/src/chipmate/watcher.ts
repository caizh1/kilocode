...
const log = Log.create({ service: "chipmate-watcher" })

export namespace ChipMateWatcher {
  export interface Interface {
    readonly init: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@chipmate/Watcher") {}
  ...
  export function eager(client = Flag.CHIPMATE_CLIENT) {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("ChipMateWatcher.state")(function* (ctx) {
      ...
      return Service.of({
        init: Effect.fn("ChipMateWatcher.init")(function* () {
  ...
  export const defaultLayer = Layer.unwrap(
    Effect.gen(function* () {
      if (!eager() || (yield* Flag.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER.pipe(Effect.orElseSucceed(() => false))))
