...
export interface Options {
  ...
  extendEnv?: boolean // chipmate_change - allow a complete sanitized environment
  stdin?: Stdio
  stdout?: Stdio
  stderr?: Stdio
  shell?: Shell
...
export function spawn(cmd: string[], opts: Options = {}): Child {
  ...
  const proc = launch(cmd[0], cmd.slice(1), {
    cwd: opts.cwd,
    shell: opts.shell,
    env: opts.env === null ? {} : opts.env ? (opts.extendEnv === false ? opts.env : { ...process.env, ...opts.env }) : undefined, // chipmate_change
...
export async function run(cmd: string[], opts: RunOptions = {}): Promise<Result> {
  const proc = spawn(cmd, {
    cwd: opts.cwd,
    env: opts.env,
    extendEnv: opts.extendEnv, // chipmate_change
    stdin: opts.stdin,
    shell: opts.shell,
    abort: opts.abort,
    kill: opts.kill,
