...
import * as SandboxActivation from "@/chipmate/sandbox/activation"
...
const it = testEffect(
  Layer.mergeAll(
    Session.layer.pipe(
      ...
      Layer.provide(AppNodeBuilder.build(SessionV2.node, [[SessionExecution.node, SessionExecution.noopLayer]])), // chipmate_change
    ),
...
describe("sandbox session cleanup", () => {
  ...
  it.live("creates honor the chipmate.sandbox metadata over the config default", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      ...
      const session = yield* provideInstance(dir)(
        sessions.create({ title: "sandbox-explicit", metadata: { "chipmate.sandbox": { enabled: true, version: 0 } } }),
      )
