      ...
      <Match when={props.message.role === "user" && props.message}>
        {(userMessage) => (
          <UserMessageDisplay
            ...
            queued={props.queued} // chipmate_change
          />
        )}
      </Match>
            ...
            {(file) => {
              ...
              return (
                <div
                  ...
                  data-queued={props.queued ? "" : undefined} // chipmate_change
      ...
      <Show when={text()}>
        <>
          <div data-slot="user-message-body">
            <div data-slot="user-message-text" dir="auto" data-queued={props.queued ? "" : undefined}>{/* chipmate_change */}
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  const isLastTextPart = createMemo(() => {
    const last = (data.store.part?.[props.message.id] ?? [])
      .filter((item): item is TextPart => item?.type === "text" && !!item.text?.trim() && !item.synthetic) // chipmate_change
      .at(-1)
    return last?.id === part().id
  })
  const showCopy = createMemo(() => {
    if (part().synthetic) return false // chipmate_change
  ...
  return (
    <Show when={text() && showSyntheticPart() /* chipmate_change */}>
