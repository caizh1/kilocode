...
/**
 * Branch coverage for [ChipMateBackendLegacyMigrationStoreService.resolveSource]: file-backed,
 * raw v5, and none. Uses the injectable overload so no real home directory is touched.
 */
class LegacyMigrationSourceTest {

    private fun env(dir: File) = mapOf("CHIPMATE_CONFIG_DIR" to dir.absolutePath)

    private fun emptySources(): LegacyV5Sources {
        val home = Files.createTempDirectory("chipmate-v5-empty-home").toFile()
        val cfg = Files.createTempDirectory("chipmate-v5-empty-config").toFile()
        return LegacyV5Sources(home, cfg)
    }

    private fun rawSources(): LegacyV5Sources {
        val home = Files.createTempDirectory("chipmate-v5-raw-home").toFile()
        val cfg = Files.createTempDirectory("chipmate-v5-raw-config").toFile()
        val settings = home.resolve(".chipmate/globalStorage/settings")
        settings.mkdirs()
    ...
    fun `file present with includeFile returns file backed and skips raw import`() {
        ...
        val source = ChipMateBackendLegacyMigrationStoreService.resolveSource(TestLog(), true, env(dir), emptySources())
    ...
    fun `file absent and raw present returns v5 raw with data`() {
        val dir = Files.createTempDirectory("chipmate-migration-config").toFile()
        val source = ChipMateBackendLegacyMigrationStoreService.resolveSource(TestLog(), false, env(dir), rawSources())
    ...
    fun `both absent returns none without data`() {
        val dir = Files.createTempDirectory("chipmate-migration-config").toFile()
        val source = ChipMateBackendLegacyMigrationStoreService.resolveSource(TestLog(), false, env(dir), emptySources())
    ...
    fun `raw present but empty import returns none`() {
        val dir = Files.createTempDirectory("chipmate-migration-config").toFile()
        val home = Files.createTempDirectory("chipmate-v5-emptyraw-home").toFile()
        val cfg = Files.createTempDirectory("chipmate-v5-emptyraw-config").toFile()
        // A present-but-empty tasks directory makes anyPresent() true while the import stays empty.
        home.resolve(".chipmate/globalStorage/tasks").mkdirs()
        val source = ChipMateBackendLegacyMigrationStoreService.resolveSource(TestLog(), false, env(dir), LegacyV5Sources(home, cfg))
        assertIs<LegacyMigrationSource.None>(source)
    }
}
