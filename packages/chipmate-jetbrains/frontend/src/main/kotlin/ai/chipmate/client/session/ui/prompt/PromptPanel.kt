...
import ai.chipmate.client.actions.SendPromptAction
import ai.chipmate.client.actions.StopSessionAction
...
import ai.chipmate.client.session.model.PromptAttachment
import ai.chipmate.client.session.model.PromptAttachmentExtractor
...
class PromptPanel(
    ...
    private val completion: ChipMatePromptCompletionProvider? = null,
    ...
    companion object {
        private val LOG = ChipMateLog.create(PromptPanel::class.java)
    ...
    private val reset = HoverIcon().apply {
        ...
        toolTipText = ChipMateBundle.message("model.picker.reset")
        accessibleContext.accessibleName = ChipMateBundle.message("model.picker.reset")
        isVisible = false
    ...
    private val enhance = HoverIcon().apply {
        icon = WAND_ICON
        toolTipText = ChipMateBundle.message("prompt.action.enhance")
        accessibleContext.accessibleName = ChipMateBundle.message("prompt.action.enhance")
    ...
    private fun key(kind: ChipMatePromptCompletionProvider.HighlightKind): TextAttributesKey = when (kind) {
        ChipMatePromptCompletionProvider.HighlightKind.MENTION -> MENTION_KEY
        ChipMatePromptCompletionProvider.HighlightKind.COMMAND -> COMMAND_KEY
        ChipMatePromptCompletionProvider.HighlightKind.INVALID -> INVALID_KEY
    }
    ...
    private fun enhance() {
        ...
        if (source.isBlank()) {
            editor.text = ChipMateBundle.message("prompt.action.enhance.description")
            syncEditorHeight()
            focus()
            return
        }
    ...
    private fun completeEnhancement(id: Long, source: String, result: Result<String>) {
        ...
        result.onSuccess {
            ...
            ChipMateNotifications.error(
                project,
                ChipMateBundle.message("prompt.action.enhance.failed"),
                ChipMateBundle.message("prompt.action.enhance.failed.description"),
            )
        }
    }
    ...
    private fun syncEnhance() {
        ...
        enhance.toolTipText = if (enhancing) {
            ChipMateBundle.message("prompt.action.enhance.loading")
        } else {
            ChipMateBundle.message("prompt.action.enhance")
        }
    }
        ...
        cs.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    notify(ChipMateBundle.message("prompt.attachment.send.failed", e.message ?: e.javaClass.simpleName))
                }
            }
        }
    }
    ...
    private fun installCompletionShortcut(ed: com.intellij.openapi.editor.Editor) {
        ...
        val action = completionAction ?: object : DumbAwareAction(ChipMateBundle.message("prompt.completion.action")) {
    ...
    private fun addAttachment(item: PromptAttachment) {
        if (!attachment && !item.reference && PromptAttachmentExtractor.image(item.mime)) {
            ...
            notify(ChipMateBundle.message("prompt.attachment.unsupported.model"))
            return
        }
    ...
    private fun promptDto(text: String, files: List<PromptPartDto>) = ai.chipmate.rpc.dto.PromptDto(
        parts = buildList {
            ...
            try {
                ...
                if (items.isEmpty()) {
                    if (list.isNotEmpty()) {
                        ApplicationManager.getApplication().invokeLater {
                            if (project.isDisposed) return@invokeLater
                            notify(ChipMateBundle.message("prompt.attachment.drop.empty"))
                        }
                    }
    ...
    private fun notify(text: String) {
        com.intellij.notification.Notification("ChipMate", text, com.intellij.notification.NotificationType.WARNING).notify(project)
    }
    ...
    private fun syncAutoApprove() {
        ...
        auto.toolTipText = if (autoApprove) {
            ChipMateBundle.message("prompt.action.autoApprove.enabled.tooltip")
        } else {
            ChipMateBundle.message("prompt.action.autoApprove.disabled.tooltip")
        }
        auto.accessibleContext.accessibleName = if (autoApprove) {
            ChipMateBundle.message("prompt.action.autoApprove.disable")
        } else {
            ChipMateBundle.message("prompt.action.autoApprove.enable")
        }
        auto.repaint()
    }
    ...
    private fun tooltip(): String {
        ...
        val text = if (stop) {
            ChipMateBundle.message("prompt.button.stop")
        } else {
            ChipMateBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(ChipMateBundle.message("prompt.button.send.tooltip.stop", shortcut))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return ChipMateBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return ChipMateBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return ChipMateBundle.message("prompt.placeholder.with.newline", line)
        return ChipMateBundle.message("prompt.placeholder")
    }
