...
export namespace WorkflowsMigrator {
  ...
  const CHIPMATE_WORKFLOWS_DIRS = [".chipmate/workflows"]
  const globalWorkflowsDirs = () => [path.join(home(), ".chipmate", "workflows")]

  export interface ChipMateWorkflow {
    name: string
    path: string
    content: string
  ...
  async function loadWorkflowsFromDir(
    ...
    root?: string,
    warnings: string[] = [],
  ): Promise<ChipMateWorkflow[]> {
    if (!(await Filesystem.isDir(dir))) return []
    const files = await findWorkflowFiles(dir)
    const workflows: ChipMateWorkflow[] = []
    for (const file of files) {
      const options = {
        trusted: source === "global",
        fileScope: source === "project" && root ? { root, source: file } : undefined,
      }
      const content = await ChipMateMarkdown.read(file, options)
        .then((text) => ChipMateMarkdown.substitute(text, file, options))
        .catch((err) => {
  ...
  export function convertToCommand(workflow: ChipMateWorkflow): ConfigCommandV1.Info {
    return {
  ...
  export async function migrate(options: {
    ...
    const workflowsByName = new Map<string, ChipMateWorkflow>()
