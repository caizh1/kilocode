...
import { IngestQueue } from "../../../src/chipmate-sessions/ingest-queue"
...
describe("share ingest queue", () => {
  ...
  test("chipmate_meta uses stable key and coalesces", async () => {
    const sent: unknown[] = []
    const sched = scheduler(() => clock.now)
    ...
    await q.sync("s7", [
      { type: "chipmate_meta", data: { platform: "cli", gitUrl: "https://github.com/old/repo.git", gitBranch: "main" } },
    ])
    clock.now = 100
    await q.sync("s7", [
      {
    ...
    expect(sent.length).toBe(1)
    expect((sent[0] as any).data.length).toBe(1)
    expect((sent[0] as any).data[0].type).toBe("chipmate_meta")
