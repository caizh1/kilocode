...
const MARKER_START = "# >>> chipmatedev launcher >>>"
const MARKER_END = "# <<< chipmatedev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `chipmatedev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}ChipMate CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}chipmatedev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `chipmate dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "chipmatedev")
  const bat = path.join(repo, "bin", "chipmatedev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias chipmatedev ${q(sh)}`
  if (shell === "powershell") return `function chipmatedev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias chipmatedev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.chipmatedev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.CHIPMATE_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/chipmate, so
  ...
  throw new Error(
    "cannot locate chipmate source checkout; set CHIPMATE_DEV_REPO=/path/to/chipmate or run ./bin/chipmatedev dev-setup from the repo",
  )
}
