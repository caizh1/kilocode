...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in ChipMate. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.

## Prerequisites

- A [ChipMate account](https://app.chipmate.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in ChipMate

1. Log into the [ChipMate platform](https://app.chipmate.ai).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.chipmate.ai/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, ChipMate's autocomplete will automatically use your Codestral key through the ChipMate Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the ChipMate extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("ChipMate Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your ChipMate balance.
...
```
Your Editor → ChipMate Gateway (with your key) → Mistral
```

- The ChipMate Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — ChipMate does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to ChipMate's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in ChipMate settings (it is on by default). Also verify you are signed into ChipMate in the extension.
...
- **Seeing charges on your ChipMate balance?** If you haven't configured BYOK, autocomplete defaults to using your ChipMate credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
