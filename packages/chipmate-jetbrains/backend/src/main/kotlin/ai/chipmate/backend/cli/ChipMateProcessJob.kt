 ...
 */
internal class ChipMateProcessJob private constructor(
    ...
    companion object {
        ...
        private val lib: Kernel32Job? by lazy {
            runCatching { Native.load("kernel32", Kernel32Job::class.java) }
                .getOrElse { ChipMateLog.create(ChipMateProcessJob::class.java).warn("Native.load(kernel32) failed", it); null }
        }
         ...
         */
        fun assign(pid: Long, log: ChipMateLog): ChipMateProcessJob? {
        ...
        private fun setup(pid: Long, log: ChipMateLog): ChipMateProcessJob? {
        ...
        private fun create(k: Kernel32Job, pid: Long, log: ChipMateLog): ChipMateProcessJob? {
            ...
            try {
                ...
                return ChipMateProcessJob(k, job, pid, log)
            } finally {
                if (!owned) closeHandle(k, job, "job (rollback)", pid, log)
            }
        }

        private fun closeHandle(k: Kernel32Job, handle: Pointer, label: String, pid: Long, log: ChipMateLog) {
