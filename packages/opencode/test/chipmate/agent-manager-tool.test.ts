...
import { AgentManagerTool, Params } from "../../src/chipmate/tool/agent-manager"
import { AgentManagerEvent, type AgentManagerStart } from "../../src/chipmate/agent-manager/event"
import { AgentManager } from "../../src/chipmate/agent-manager/service"
...
const providers = {
  test: {
    ...
    models: {
      ...
      // "Shared" is also offered by the chipmate provider, to exercise provider resolution.
  ...
  chipmate: {
    ...
    models: {
      "chipmate/shared": { id: "chipmate/shared", providerID: "chipmate", name: "Shared", variants: { low: {} } },
      "chipmate/only": { id: "chipmate/only", providerID: "chipmate", name: "Gateway Only", variants: { low: {} } },
    },
  } as unknown as Provider.Info,
  zeta: {
    id: "zeta",
...
// Default provider is `test`, so resolution should prefer test, then chipmate, then others.
...
describe("agent_manager tool", () => {
  ...
  test("lists the compact overview with a separate read-only permission pattern", async () => {
    ...
    expect(JSON.parse(result.output)).toEqual({
      instructions:
        "This overview is the source of truth. Use sections[].id as sectionID and sessions[].id/session.id as sessionID for action=move. Do not edit .chipmate/agent-manager.json.",
      sections: [],
      ungrouped: [
        {
          id: "wt-1",
  ...
  test("inherits the latest invoking model and variant when omitted", async () => {
    const task = await publish(runtime, { prompt: "Fix" }, [
      message("msg_current", "chipmate", "chipmate/shared", "low", 2),
      message("msg_old", "test", "reasoning/model", "high", 1),
    ])

    expect(String(task?.model?.providerID)).toBe("chipmate")
    expect(String(task?.model?.modelID)).toBe("chipmate/shared")
  ...
  test("explicit model and variant override the invoking selection", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "test/reasoning/model", variant: "high" }, [
      message("msg_current", "chipmate", "chipmate/shared", "low"),
    ])
  ...
  test("does not inherit a variant when only the model is overridden", async () => {
    ...
    expect(String(task?.model?.providerID)).toBe("chipmate")
    expect(String(task?.model?.modelID)).toBe("chipmate/only")
    expect(task?.variant).toBeUndefined()
  })
  ...
  test("prefers the invoking provider for an explicit model override", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Shared", variant: "low" }, [
      message("msg_current", "chipmate", "chipmate/only", "low"),
    ])
    expect(String(task?.model?.providerID)).toBe("chipmate")
    expect(String(task?.model?.modelID)).toBe("chipmate/shared")
  })
  ...
  test("falls back to the chipmate gateway when the preferred provider lacks the model", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Gateway Only" })
    // Default provider `test` does not offer it; chipmate is preferred over zeta.
    expect(String(task?.model?.providerID)).toBe("chipmate")
  })

  test("narrows to a provider that supports the requested variant", async () => {
    const rt = makeRuntime("chipmate")
    // chipmate is preferred, but only `test`'s Shared has the `high` variant.
