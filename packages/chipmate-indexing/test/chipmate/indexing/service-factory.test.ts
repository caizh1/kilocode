...
describe("CodeIndexServiceFactory", () => {
  ...
  test("suffixes custom LanceDB and Qdrant storage for ChipMate v2", () => {
    const previous = process.env.CHIPMATE_PRODUCT_PROFILE
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CHIPMATE_PRODUCT_PROFILE
      else process.env.CHIPMATE_PRODUCT_PROFILE = previous
    }
  })
  ...
  test("creates ChipMate embedder with Cloud-provided model", async () => {
    const factory = createFactory({
    ...
    expect(embedder.embedderInfo).toEqual({ name: "chipmate" })
    expect(mockEmbeddingsCreate).toHaveBeenCalledWith(
      {
        input: ["hello"],
