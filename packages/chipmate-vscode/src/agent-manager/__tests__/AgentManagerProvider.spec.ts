...
describe("AgentManagerProvider worktree creation", () => {
  it("registers the first worktree session with session provider", async () => {
    ...
    const created = {
      worktree: { id: "wt-1" },
      result: { path: "/repo/.chipmate/worktrees/wt-1", branch: "feature/wt-1", parentBranch: "main" },
    }
  ...
  it("does not arm automatic naming for a custom branch", async () => {
    ...
    manager.createWorktreeOnDisk.mockResolvedValue({
      worktree: { id: "wt-1" },
      result: { path: "/repo/.chipmate/worktrees/custom", branch: "my-custom-branch", parentBranch: "main" },
    })
  ...
  it("waits for state initialization before creating a worktree", async () => {
    ...
    manager.createWorktreeOnDisk.mockResolvedValue({
      worktree: { id: "wt-2" },
      result: { path: "/repo/.chipmate/worktrees/wt-2", branch: "feature/wt-2", parentBranch: "main" },
    })
  ...
  it("resolves new sends to the selected worktree directory", async () => {
    const manager = createHarness()
    const state = {
      getWorktree: vi.fn().mockReturnValue({ id: "wt-1", path: "/repo/.chipmate/worktrees/wt-1" }),
    }
    manager.getStateManager.mockReturnValue(state)
    ...
    expect(result).toEqual({
      ...
      agentManagerContext: "wt-1",
      draftID: "draft-1",
      contextDirectory: "/repo/.chipmate/worktrees/wt-1",
    })
