...
import { IngestQueue } from "@/chipmate-sessions/ingest-queue"
import { IngestDrain } from "@/chipmate-sessions/ingest-drain"
import { clearInFlightCache, withInFlightCache } from "@/chipmate-sessions/inflight-cache"
import type * as SDK from "@chipmate/sdk/v2"
import z from "zod"
import { Context, Effect, Layer, Schema } from "effect"
import { CHIPMATE_API_BASE } from "@chipmate/chipmate-gateway"
import { Config } from "@/config/config"
import { InstanceState } from "@/effect/instance-state"
import { Instance } from "@/chipmate/instance"
import { Vcs } from "@/project/vcs"
import simpleGit from "simple-git"
import { RemoteWS } from "@/chipmate-sessions/remote-ws"
import { RemoteSender } from "@/chipmate-sessions/remote-sender"
import { RemoteProtocol } from "@/chipmate-sessions/remote-protocol"
import { buildInstanceAdvertisement } from "@/chipmate-sessions/instance-advertisement"
import { AttachedState } from "@/chipmate-sessions/attached-state"
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "@/chipmate-sessions/rename-adoptions"
...
import { cumulativeSessionDiff } from "@/chipmate/session-portability/cumulative-diff"
...
async function provide<R>(input: { directory: string; fn: () => R }): Promise<R> {
  const { provide } = await import("@/chipmate/instance")
  return provide(input)
}

export namespace ChipMateSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "chipmate-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@chipmate/ChipMateSessions") {}

  const log = Log.create({ service: "chipmate-sessions" })
  ...
  const tokenValidKeyTemplate = "chipmate-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "chipmate-sessions:token"
  const orgKey = "chipmate-sessions:org"
  const clientKey = "chipmate-sessions:client"
  const gitUrlKeyPrefix = "chipmate-sessions:git-url:"
  ...
  function agentNotificationTimeoutMs(): number {
    const value = process.env["CHIPMATE_AGENT_NOTIFICATION_TIMEOUT_MS"]
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${CHIPMATE_API_BASE}/api/user`, {
        headers: {
  ...
  async function chipmateToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("chipmate"))
      ...
      const key = process.env["CHIPMATE_API_KEY"]?.trim()
      if (key) return key
      return undefined
    })
  }
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await chipmateToken()
  ...
  const shareDisabled = process.env["CHIPMATE_DISABLE_SHARE"] === "true" || process.env["CHIPMATE_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["CHIPMATE_DISABLE_SESSION_INGEST"] === "true" || process.env["CHIPMATE_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["CHIPMATE_DEBUG_SESSION_INGEST"] === "true" || process.env["CHIPMATE_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["CHIPMATE_REMOTE"] === "1"
  let remote: { conn: RemoteWS.Connection; sender: RemoteSender.Sender } | undefined
  let enabling: Promise<void> | undefined
  let remoteSeq = 0
  // chipmate_change - K1 W1: module-level instance advertisement flag.
  // `enableRemote` can be triggered either by the explicit `chipmate remote` command
  // or by bootstrap auto-enable (`CHIPMATE_REMOTE=1` / `remote_control` config); it
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const reportSessionTitle = Effect.fn("ChipMateSessions.reportSessionTitle")(function* (
        sessionID: string,
        title: string,
        opts: { generated: boolean },
      ) {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("ChipMateSessions.state")(function* (ctx) {
          ...
          watch(Session.Event.Updated, async (evt) => {
            ...
            try {
              await ingest.sync(sessionID, [
                { type: "chipmate_meta", data: await meta(sessionID, session) },
      ...
      const init = Effect.fn("ChipMateSessions.init")(function* () {
        yield* InstanceState.get(state)
      })

      const sendAgentNotification = Effect.fn("ChipMateSessions.sendAgentNotification")(function* (
        sessionID: string,
        input: { id: string; message: string },
      ) {
        if (ingestDisabled) {
  ...
  // chipmate_change - DEF-1: default advertisement for every successful
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await chipmateToken()
      if (!token) {
        throw new Error("Unable to enable remote: no ChipMate credentials found. Run `chipmate auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired ChipMate credentials. Run `chipmate auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify ChipMate credentials.")

      const url = (process.env["CHIPMATE_SESSION_INGEST_URL"] ?? "https://ingest.chipmatesessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")

      const [{ RemoteWS }, { RemoteSender }] = await Promise.all([
        import("@/chipmate-sessions/remote-ws"),
        import("@/chipmate-sessions/remote-sender"),
      ])
      ...
      // chipmate_change - K1 W1: capture module-level advertisement so each
              ...
              [...ids].map((id) =>
                svc.get(SessionID.make(id)).pipe(
                  Effect.map((session) => ({
                    ...
                    gitUrl,
                    gitBranch,
                    // chipmate_change - K1 W1: per-session platform, mirrors
                    // meta()'s resolution order so the live value always agrees
                    // with the session's stored created_on_platform.
                    platform: ChipMateSession.resolvePlatform(id) || process.env["CHIPMATE_PLATFORM"] || "cli",
                  })),
                  Effect.orElseSucceed(() => undefined),
                ),
              ),
      ...
      const conn = RemoteWS.connect({
        ...
        onOpen: () => {
          ...
          // chipmate_change - K1 W1: on reconnect, a headless `chipmate remote` host
      ...
      const sender = RemoteSender.create({
        ...
        log,
        // chipmate_change - K1 W1: in-process attach/detach/ownership seams
        // back to ChipMateSessions. The sender does NOT spawn a process per
        // session — concurrent remote sessions share this CLI process with
        // per-directory InstanceRef isolation.
        attachSession: (id) => ChipMateSessions.attachRemoteSession(id),
        detachSession: (id) => ChipMateSessions.detachRemoteSession(id),
        hasSession: (id) => ChipMateSessions.hasRemoteSession(id),
        ownedCount: () => ChipMateSessions.ownedRemoteSessionCount(),
        cancelPrompt: async (id) => {
          // chipmate_change - K1 W1: dynamic import breaks the module-load cycle
          // (@/session/prompt reads ChipMateSessionPrompt at eval; a static edge here
  ...
  // chipmate_change - K1 W1: instance advertisement setter.
  ...
  // explicit `chipmate remote` command — `enableRemote` itself is coalescing), we
  ...
  // (once a process runs `chipmate remote`, it keeps advertising for its whole
  ...
  // chipmate_change - K1 W1: session-detach semantics. The exit_cli handler
  ...
  // chipmate_change - K1 W1: ownership probe used by the exit_cli handler
  ...
  // chipmate_change - K1 W1: count of "owned" sessions (presence ∪ pending).
  ...
  // zero remaining + no callback (chipmate remote) => keep host alive. Sessions
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (CHIPMATE_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (CHIPMATE_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no ChipMate credentials found. Run `chipmate auth login`.")
    }
    ...
    const url = `https://app.chipmate.ai/s/${result.public_id}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (CHIPMATE_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no ChipMate credentials found. Run `chipmate auth login`.")
    }
  ...
  async function meta(sessionId?: string, info?: Session.Info | null) {
    const override = sessionId ? ChipMateSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["CHIPMATE_PLATFORM"] || "cli"
  ...
  async function getOrgId(sessionId?: string, info?: Session.Info | null): Promise<Uuid | undefined> {
    ...
    const env = process.env["CHIPMATE_ORG_ID"]
