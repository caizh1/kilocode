...
function getConfig() {
  return vscode.workspace.getConfiguration("chipmate.v2")
}
...
export function watchWorkStyleConfig(post: (message: unknown) => void, next?: vscode.Disposable) {
  ...
  const watcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (keys.some((key) => event.affectsConfiguration(`chipmate.v2.${key}`))) post(getWorkStylePayload())
  })
...
async function hasAnySession(connection: ChipMateConnectionService, directory: string): Promise<boolean> {
...
async function initializeWorkStyle(connection: ChipMateConnectionService, directory: string): Promise<void> {
...
export async function handleWorkStyleMessage(input: {
  ...
  if (input.message.type === "requestWorkStyle") {
    const initialized = await initializeWorkStyle(input.connection, input.directory)
      .then(() => true)
      .catch((err: unknown) => {
        console.error("[ChipMate New] Failed to initialize work style:", err)
        return false
      })
  ...
  if (!input.message.style) {
    console.error("[ChipMate New] Missing style in setWorkStyle message")
    return true
  }
