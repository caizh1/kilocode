---
title: "ChipMate for VS Code: Free Open-Source AI Coding Extension"
description: "Using ChipMate in Visual Studio Code"
---

# ChipMate for VS Code

The ChipMate VS Code extension uses the shared ChipMate agent runtime while integrating chat, agents, and automation into the editor.

## Installation

1. Open VS Code
2. Go to Extensions (`Ctrl+Shift+X` / `Cmd+Shift+X`)
3. Search for "ChipMate"
4. Click the dropdown arrow next to **Install** and select **Install Pre-Release Version**

The extension includes its own embedded runtime. No separate ChipMate CLI installation is required.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `chipmate.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.chipmate/agents/*.md`) replacing the modes system
...
Settings apply across extension surfaces, including the sidebar and Agent Manager. The standalone CLI uses the same `~/.config/chipmate/chipmate.jsonc` (global) and `./chipmate.jsonc` (project) files when used directly.

## Interface Language

The extension UI follows VS Code's display language by default. Override it with the `chipmate-code.new.language` setting (for example `en`, `de`, `ja`, or `fa`). Right-to-left languages such as Arabic and Persian switch the layout direction automatically.

## Proxy and Certificate Troubleshooting

ChipMate for VS Code starts its embedded runtime from the extension and applies the relevant VS Code network settings to that runtime. On managed networks, configure proxy and certificate trust in VS Code settings rather than in a separate CLI install.
...
- Leave `http.proxySupport` enabled unless you intentionally want VS Code and ChipMate to ignore proxy settings.
...
- If the operating system trust store is not enough, set `chipmate-code.new.extraCaCerts` to the absolute path of a PEM file that contains the additional certificate authority certificates.
...
```json
{
  ...
  "chipmate-code.new.extraCaCerts": "/absolute/path/to/corporate-ca.pem"
}
```
