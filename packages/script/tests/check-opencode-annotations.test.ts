...
function repo() {
  const root = mkdtempSync(path.join(os.tmpdir(), "chipmate-annotations-"))
  ...
  exec(root, ["add", "."])
  exec(root, ["-c", "user.name=ChipMate", "-c", "user.email=chipmate@example.com", "commit", "-m", "init"])
...
describe("CLI worktree mode", () => {
  ...
  test("worktree mode checks local edits on upstream merge branches", () => {
    ...
    try {
      ...
      writeFileSync(path.join(root, "packages/opencode/src/shared.ts"), "export const value = 2\n")
      exec(root, ["add", "."])
      exec(root, ["-c", "user.name=ChipMate", "-c", "user.email=chipmate@example.com", "commit", "-m", "upstream"])
...
describe("hasMarker", () => {
  const cases: Array<[string, boolean]> = [
    // JS-style inline
    ["// chipmate_change", true],
    ["  // chipmate_change", true],
    ["const x = 1 // chipmate_change", true],
    ["// chipmate_change start", true],
    ["// chipmate_change end", true],
    ["// chipmate_change - new file", true],
    ["//   chipmate_change", true],
    ["// chipmate_change  ", true],
    ...
    ["{/* chipmate_change end */}", true],
    ["{/* chipmate_change - new file */}", true],
    ["{/* chipmate_change - ChipMateNews added */}", true],
    ["{/*   chipmate_change */}", true],
    ["{/* chipmate_change  */}", true],
    ...
    ["# chipmate_change", true],
    ["  # chipmate_change", true],
    ["name: test # chipmate_change", true],
    ['name = "zed" # chipmate_change', true],
    ['export FOO="bar" # chipmate_change', true],
    ["# chipmate_change start", true],
    ["# chipmate_change end", true],
    ...
    ["// chipmate_changes", false],
    ["// chipmate_changelog", false],
    ["/* chipmate_change_log */", false],
    ["{/* chipmate_changes */}", false],
    ["// chipmate_changeable", false],
    ["", false],
    ["  ", false],
  ]
...
describe("isExempt", () => {
  const cases: Array<[string, boolean]> = [
    // exempt — "chipmate" in path
    ["packages/opencode/src/chipmate/foo.ts", true],
    ["packages/opencode/test/chipmate/bar.test.ts", true],
    ["packages/opencode/src/some/chipmate/deep/path.ts", true],
    ["packages/opencode/src/chipmate/deep/nested/file.tsx", true],
    ["packages/opencode/src/chipmate-sessions/session.ts", true],
    ["packages/chipmate-ui/src/components/icon.tsx", true],
    ["packages/chipmate-vscode/src/extension.ts", true],
    ...
    // exempt — "chipmate" in filename
    ["packages/opencode/src/foo/chipmate.ts", true],
    ["packages/opencode/src/bar/chipmate.test.ts", true],
    ["packages/opencode/src/file.chipmate.ts", true],
    // exempt — case-insensitive
    ["packages/opencode/src/ChipMate/foo.ts", true],
    ["packages/opencode/src/CHIPMATE/bar.ts", true],
    // NOT exempt
    ...
    // chipmate_change is not the same as chipmate
...
describe("isChecked", () => {
  const cases: Array<[string, boolean]> = [
    ...
    ["packages/chipmate-ui/src/components/icon.tsx", false],
    ["packages/chipmate-vscode/src/extension.ts", false],
...
describe("coveredLines", () => {
  ...
  test("whole-file JS annotation", () => {
    const covered = coveredLines("// chipmate_change - new file\nexport const x = 1\nexport const y = 2")
  ...
  test("bare /* */ block markers", () => {
    const text = ["/* chipmate_change start */", "const b = 2", "/* chipmate_change end */"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("YAML block markers", () => {
    const text = ["# chipmate_change start", "name: test", "# chipmate_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("TOML block markers", () => {
    const text = ["# chipmate_change start", 'id = "opencode"', "# chipmate_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("shell block markers", () => {
    const text = ["# chipmate_change start", "set -euo pipefail", "# chipmate_change end"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("inline JS marker covers only that line", () => {
    const text = ["const a = 1", "const b = 2 // chipmate_change", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JSX marker covers only that line", () => {
    const text = ["const a = 1", "{/* chipmate_change */}", "const c = 3"].join("\n")
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })

  test("inline JS marker with code on same line", () => {
    const text = "const url = Flag.CHIPMATE_MODELS_URL || 'https://models.dev' // chipmate_change\n"
    const covered = coveredLines(text)
  ...
  test("JSX block marker with descriptive suffix", () => {
    const text = [
      "{/* chipmate_change start - ChipMate-specific error display */}",
      "<ErrorDisplay />",
      "{/* chipmate_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("marker line with extra text after marker is still covered", () => {
    const text = [
      "const a = 1",
      "// chipmate_change start - this is chipmate specific",
      "const b = 2",
      "// chipmate_change end",
    ].join("\n")
    const covered = coveredLines(text)
...
describe("checkLine (main loop simulation)", () => {
  ...
  test("covered line reports no violation", () => {
    const text = ["// chipmate_change start", "const chipmate = 1", "// chipmate_change end"].join("\n")
  ...
  test("marker lines are skipped even if uncovered", () => {
    ...
    const text = ["{/* chipmate_change */}", "{/* chipmate_change start */}"].join("\n")
  ...
  test("real-world TSX permission.tsx inline pattern", () => {
    const text = [
      ...
      "{/* chipmate_change */}",
      "<AnotherChipMateComponent />",
    ].join("\n")
    expect(check(text, [2, 4])).toEqual(["line 2: <PermissionDeniedCard />", "line 4: <AnotherChipMateComponent />"])
    expect(check(text, [1, 3])).toEqual([])
  })

  test("JS-style session/index.tsx pattern (from existing codebase)", () => {
    const text = ["const foo = 1", "<Toast />", "{/* chipmate_change */}", "<Footer />", "</box>"].join("\n")
  ...
  test("whole-file annotated file — no violations even for unmarked lines", () => {
    const text = [
      "// chipmate_change - new file",
      "export const chipmateFeature = true",
      "export const alsoChipMate = 123",
...
// and a flag indicating whether the diff removes any chipmate_change marker
// (i.e. the change is reverting ChipMate modifications back to upstream).
...
describe("parseDiff (revert detection)", () => {
  ...
  test("revert: hunk removes chipmate_change marker block and adds upstream original", () => {
    ...
    const diff = [
      ...
      "+++ b/test.ts",
      "@@ -16,3 +16 @@ describe(...)",
      "-  // chipmate_change start - TODO: skip flaky test",
      "-  test.skip('foo', async () => {",
      "-    // chipmate_change end",
      "+  test('foo', async () => {",
    ].join("\n")
  ...
  test("revert: inline marker removed, upstream original added", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -5 +5 @@",
      "-const url = Flag.X || 'fallback' // chipmate_change",
      "+const url = Flag.X",
    ].join("\n")
    const out = parseDiff(diff)
  ...
  test("file-level revert: marker removed in one hunk covers other hunks", () => {
    // Mirrors the prompt.test.ts case from PR #9908: chipmate_change marker
    // is removed in hunk A, while a separate hunk B replaces references that
    // depended on the removed ChipMate construct.
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -218 +217,0 @@",
      "-const unixSkip = it.live.skip // chipmate_change - skip flaky tests",
      "@@ -1589 +1583 @@ unixSkip(",
      "-unixSkip(",
      "+unix(",
    ].join("\n")
  ...
  test("multiple chipmate_change start/end markers removed across hunks", () => {
    const diff = [
      "diff --git a/test.ts b/test.ts",
      "@@ -1432,2 +1431 @@",
      "-// chipmate_change start - flaky on Linux CI",
      "-unixSkip(",
      "+unix(",
      "@@ -1469 +1466,0 @@",
      "-// chipmate_change end",
  ...
  test("YAML/shell marker removal also triggers revert", () => {
    const diff = [
      ...
      "@@ -10 +10 @@",
      "-      - uses: actions/checkout@v6 # chipmate_change",
...
describe("MARKER_PREFIX regex edge cases", () => {
  test("handles { followed immediately by /*", () => {
    expect(hasMarker("{/* chipmate_change */}")).toBe(true)
  })
  ...
  test("does not match {/* without chipmate_change", () => {
  ...
  test("does not match chipmate_changes (word boundary)", () => {
    expect(hasMarker("// chipmate_changes")).toBe(false)
    expect(hasMarker("// chipmate_changelog")).toBe(false)
    expect(hasMarker("{/* chipmate_changes */}")).toBe(false)
    expect(hasMarker("// chipmate_changeable")).toBe(false)
  })
})
...
describe("isExempt — Windows backslash paths", () => {
  test("Windows paths with backslashes", () => {
    expect(isExempt("packages\\opencode\\src\\chipmate\\foo.ts")).toBe(true)
    expect(isExempt("packages\\opencode\\test\\chipmate\\bar.test.ts")).toBe(true)
...
describe("coveredLines — additional patterns", () => {
  test("block with descriptive suffix is still recognized", () => {
    const text = [
      "{/* chipmate_change start - ChipMate-specific indexing display */}",
      "<IndexingStatus />",
      "{/* chipmate_change end */}",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("empty file content", () => {
    const covered = coveredLines("// chipmate_change start\n  \n// chipmate_change end")
    expect(covered).toEqual(new Set([1, 2, 3]))
  })

  test("multiple separate JS inline markers", () => {
    const text = [
      "const a = 1 // chipmate_change",
      "const b = 2",
      "const c = 3 // chipmate_change",
      "const d = 4",
    ].join("\n")
    const covered = coveredLines(text)
  ...
  test("consecutive block markers (no content)", () => {
    const text = ["// chipmate_change start", "// chipmate_change end"].join("\n")
    const covered = coveredLines(text)
  ...
  test("trailing empty line after block end is not covered", () => {
    const text = "// chipmate_change start\nconst a = 1\n// chipmate_change end\n\n"
    const covered = coveredLines(text)
...
describe("checkLine — additional patterns", () => {
  ...
  test("real-world TUI routes — line between marker and code should be uncovered", () => {
    ...
    const text = ["{/* chipmate_change start */}", "", "<ChipMateIndexing />", "", "{/* chipmate_change end */}"].join("\n")
  ...
  test("end marker on same line as content is covered", () => {
    const text = "const a = 1\n{/* chipmate_change end */} // block already closed, still covered\n"
    const covered = coveredLines(text)
    expect(covered).toEqual(new Set([2]))
  })
