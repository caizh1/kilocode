...
import type { AgentSendRequest, AgentStartRequest, MessageResult } from "@/chipmate/cloud/contracts"
import { CloudCommands } from "@/chipmate/cloud/commands"
import { CloudError } from "@/chipmate/cloud/errors"
import { Git } from "@/git"
...
it.instance(
  "assembles the default start request from ChipMate state and the current repository",
  () =>
    Effect.acquireUseRelease(
      ...
      (server) =>
        Effect.gen(function* () {
          ...
          yield* run(test.directory, "remote", "add", "origin", "git@github.com:ChipMate-Org/chipmate.git")
          ...
          const response = {
            ...
            chipmateSessionId: "ses_command_test",
            messageId: MESSAGE,
          ...
          const result = yield* CloudCommands.start(
            ...
            {
              env: { CHIPMATE_API_URL: server.url.origin },
              make: (options) => {
          ...
          expect(requests[0]).toEqual({
            ...
            repository: { type: "github", repo: "ChipMate-Org/chipmate" },
            options: {
              createdOnPlatform: "chipmate-cli",
              chipmateOrganizationId: ORG,
            },
          })
    ...
    ),
  {
    ...
    config: {
      default_agent: "plan",
      agent: { plan: { model: "chipmate/anthropic/command-model" } },
    },
  },
)
    ...
    Effect.acquireUseRelease(
      ...
      (server) =>
        Effect.gen(function* () {
          ...
          const response = {
            cloudAgentSessionId: SESSION,
            chipmateSessionId: "ses_stream_test",
            messageId: MESSAGE,
            delivery: "queued" as const,
...
it.instance("sends follow-ups, prints status without assistant content, and applies the result exit code", () =>
  Effect.gen(function* () {
    ...
    const deps = {
      env: { CHIPMATE_ORG_ID: "not-relevant-to-existing-sessions" },
      make: () => ({
        async start() {
