 ...
 *
 * Static analysis — reads ChipMateProvider.ts, AgentManagerProvider.ts,
 ...
 *
 * Protects against accidental removal during ChipMate development.
 */
...
const CHIPMATEPROVIDER_FILE = path.join(ROOT, "src/ChipMateProvider.ts")
...
describe("ChipMateProvider editor-panel visible registration contract", () => {
  const source = readFile(CHIPMATEPROVIDER_FILE)
...
describe("ChipMateProvider disableViewedRegistration contract", () => {
  const chipmateProvider = readFile(CHIPMATEPROVIDER_FILE)
  const vscodeHost = readFile(VSCODE_HOST_FILE)

  it("registerPresence skips viewed registration when the option is set", () => {
    const match = chipmateProvider.match(/private registerPresence\(\): void \{([\s\S]*?)\n {2}\}/)
    expect(match).not.toBeNull()
    const body = match![1]
  ...
  it("focusSession and trackOpenSessions report through registerPresence", () => {
    ...
    const focus = chipmateProvider.match(/private focusSession\(id\?: string\): void \{([\s\S]*?)\n {2}\}/)
    expect(focus).not.toBeNull()
    expect(focus![1]).toContain("this.registerPresence()")
    const track = chipmateProvider.match(/private trackOpenSessions\(ids: string\[\]\): void \{([\s\S]*?)\n {2}\}/)
  ...
  it("the editor-panel view-state callback honors the same option", () => {
    const match = chipmateProvider.match(
      /this\.viewStateDisposable = this\.visibleTaskStreams\.bindPanel\(panel, \(\) => \{([\s\S]*?)\n {4}\}\)/,
    )
    expect(match).not.toBeNull()
  ...
  it("embedded Agent Manager providers disable generic viewed registration", () => {
    // Each Agent Manager panel hosts a full ChipMateProvider; the "agent-manager"
...
describe("ChipMateConnectionService connection snapshot contract", () => {
