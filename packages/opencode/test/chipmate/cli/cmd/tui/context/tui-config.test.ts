...
import { ChipMateTuiConfig } from "@/chipmate/cli/cmd/tui/context/tui-config"
import { ChipMateTerminalTitle } from "@/chipmate/cli/cmd/tui/terminal-title"
...
function resolve(input: TuiConfig.Info): TuiConfig.Resolved {
  ...
  return {
    ...
    attention: {
      ...
      sound_pack: input.attention?.sound_pack ?? "chipmate.default",
...
describe("ChipMateTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = ChipMateTuiConfig.makeStore(resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "chipmate" })))
    ...
    createRoot((d) => {
      ...
      createEffect(() =>
        titles.push(
          ChipMateTerminalTitle.format({ base: "ChipMate CLI", indicator: "working", icon: store.config.title_icon }),
        ),
      )
    })
    ...
    expect(themes).toEqual(["chipmate"])
    expect(icons).toEqual(["none"])
    expect(titles).toEqual(["ChipMate CLI"])
    ...
    expect(themes).toEqual(["chipmate", "nord"])
    expect(icons).toEqual(["none", "emojis"])
    expect(titles).toEqual(["ChipMate CLI", "💭 ChipMate CLI"])

    dispose()
  })

  test("set() restores the default title icon when the setting is removed", () => {
    const store = ChipMateTuiConfig.makeStore(resolve(cfg({ title_icon: "unicode" })))
