package ai.chipmate.client.migration.ui

import ai.chipmate.client.migration.MigrationItemUiProgress
import ai.chipmate.client.migration.MigrationSelectionBuilder
import ai.chipmate.client.migration.MigrationSettingsUiSelections
import ai.chipmate.client.migration.MigrationUiPhase
import ai.chipmate.client.migration.MigrationUiSelections
import ai.chipmate.client.migration.MigrationUiState
import ai.chipmate.client.migration.groupStatus
...
import ai.chipmate.rpc.dto.MigrationItemCategoryDto
...
class MigrationWizardPanel : JPanel(BorderLayout()) {
    ...
    private val settingsRow = MigrationItemRow(ChipMateBundle.message("migration.row.settings"), MigrationItemCategoryDto.settings)
    private val providerRow = MigrationItemRow(ChipMateBundle.message("migration.row.providers"), MigrationItemCategoryDto.provider)
    private val mcpRow = MigrationItemRow(ChipMateBundle.message("migration.row.mcp"), MigrationItemCategoryDto.mcpServer)
    private val modesRow = MigrationItemRow(ChipMateBundle.message("migration.row.modes"), MigrationItemCategoryDto.customMode)
    private val sessionsRow = MigrationItemRow(ChipMateBundle.message("migration.row.sessions"), MigrationItemCategoryDto.session)
    private val modelRow = MigrationItemRow(ChipMateBundle.message("migration.row.model"), MigrationItemCategoryDto.defaultModel)

    private val question = BaseQuestionView()
    private val keepBox = JBCheckBox(ChipMateBundle.message("migration.keep_legacy_settings"), true)

    private val emptyLabel = JBLabel(ChipMateBundle.message("migration.empty")).apply {
    ...
    init {
        ...
        question.setHeader(
            ChipMateBundle.message("migration.migrate.title"),
            ChipMateBundle.message("migration.migrate.subtitle"),
        )
        question.setContent(buildContent())
        question.setActions(
            listOf(
                BaseQuestionView.Action(ACTION_SKIP, ChipMateBundle.message("migration.button.skip"), primary = false) {
                    onSkip?.invoke()
                },
                BaseQuestionView.Action(ACTION_LATER, ChipMateBundle.message("migration.button.later"), primary = false) {
                    onLater?.invoke()
                },
                BaseQuestionView.Action(ACTION_MIGRATE, ChipMateBundle.message("migration.button.migrate"), primary = true) {
                    onStart?.invoke(currentSelections())
                },
                BaseQuestionView.Action(ACTION_DONE, ChipMateBundle.message("migration.button.done"), primary = true) {
                    onDone?.invoke()
                },
                BaseQuestionView.Action(ACTION_CONTINUE, ChipMateBundle.message("migration.button.continue"), primary = true) {
    ...
    private fun updateButtons(phase: MigrationUiPhase, running: Boolean) {
        ...
        question.setActionText(
            ACTION_MIGRATE,
            if (running) ChipMateBundle.message("migration.button.migrating") else ChipMateBundle.message("migration.button.migrate"),
        )
