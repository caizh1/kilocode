...
import { Location } from "../location" // chipmate_change
import { SessionSchema } from "./schema"
import { SessionContextEpochTable, SessionTable } from "./sql"

type DatabaseService = Database.Interface["db"]

class LocationMismatch extends Error {} // chipmate_change - a concurrent Session move must abort the epoch
...
export function initialize(
  ...
  location: Location.Ref, // chipmate_change
...
const prepareOnce = Effect.fnUntraced(function* (
  ...
  if (!stored) {
    ...
    const baselineSeq = yield* insert(db, sessionID, generation, location) // chipmate_change
...
const insert = Effect.fnUntraced(function* (
  ...
  // chipmate_change start - upstream only checks the Location before loading system context, so a move
