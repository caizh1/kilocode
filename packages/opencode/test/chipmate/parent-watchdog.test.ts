...
import { startParentWatchdog } from "../../src/chipmate/parent-watchdog"

describe("startParentWatchdog", () => {
  afterEach(() => {
    delete process.env["CHIPMATE_PARENT_PID"]
  })

  test("is a no-op when CHIPMATE_PARENT_PID is unset", () => {
    delete process.env["CHIPMATE_PARENT_PID"]
    let called = false
  ...
  test("is a no-op for an invalid CHIPMATE_PARENT_PID", () => {
    process.env["CHIPMATE_PARENT_PID"] = "0"
    let called = false
    const stop = startParentWatchdog(() => {
      called = true
    })
  ...
  test("fires onOrphan once the watched parent process is gone", async () => {
    ...
    child.kill("SIGKILL")
    await child.exited
    process.env["CHIPMATE_PARENT_PID"] = String(pid)
