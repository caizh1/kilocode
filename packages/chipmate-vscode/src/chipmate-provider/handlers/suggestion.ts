/**
 * Suggestion handlers — extracted from ChipMateProvider.
 *
 * Manages suggestion accept and dismiss flows plus recovery after SSE reconnects.
 * No vscode dependency.
 */

import type { ChipMateClient, SuggestionRequest } from "@chipmate/sdk/v2/client"
...
export async function handleSuggestionAccept(
  ...
  try {
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to accept suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function handleSuggestionDismiss(
  ...
  try {
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to dismiss suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function fetchAndSendPendingSuggestions(ctx: SuggestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[ChipMate New] ChipMateProvider: Failed to fetch pending suggestions:", error)
  }
}
