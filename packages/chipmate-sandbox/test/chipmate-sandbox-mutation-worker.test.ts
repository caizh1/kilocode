...
async function worker(request: Request) {
  const entry = fileURLToPath(new URL("../src/chipmate-sandbox-mutation-worker.ts", import.meta.url))
...
describe("filesystem mutation worker", () => {
  test("executes ordered mutation batches", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "chipmate-mutation-worker-"))
    roots.push(root)
