 ...
 */
@Rpc
interface ChipMateAppRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): ChipMateAppRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<ChipMateAppRpcApi>())
        }
    }
    ...
    suspend fun state(): Flow<ChipMateAppStateDto>
    ...
    suspend fun updateConfig(patch: ConfigPatchDto): ChipMateAppStateDto

    /** Refresh the user profile and return the latest data, or null if not logged in. */
    suspend fun refreshProfile(): ProfileDto?

    /**
     * Start the device auth login flow for ChipMate Gateway.
