...
Security Agent syncs dependency vulnerability alerts from selected GitHub repositories and stores them as Security Findings in ChipMate. It uses AI to triage each finding and runs sandbox analysis when project-specific evidence is needed. It also helps you dismiss or remediate findings, send notifications, and review recorded activity.
...
1. The [ChipMate GitHub App](/docs/automate/integrations#connecting-github) installed with the `vulnerability_alerts` permission.
2. [Dependabot alerts](https://docs.github.com/en/code-security/dependabot/dependabot-alerts) enabled on target repositories.
3. ChipMate credits for AI model usage.
...
3. Choose a repository scope: all repositories available to the ChipMate GitHub App or selected repositories.
...
Organization owners and billing managers can change Security Agent settings, turn the agent on or off, dismiss findings, clear orphaned findings, and access organization audit reports. ChipMate platform admins can also access these reports for audited support and operations.
...
```mermaid
flowchart LR
  alert["Dependabot alert"] --> sync["Sync into ChipMate"]
...
1. **Sync** pulls Dependabot alerts from repositories in scope and stores them as Security Findings in ChipMate.
...
Security Agent treats Dependabot alerts as source data and ChipMate Security Findings as the working record. A finding can stay open even after a remediation PR exists. The finding closes only when GitHub reports it fixed or someone dismisses it.
...
When enabled, auto-dismiss automatically dismisses findings that sandbox analysis determines are not exploitable and recommends dismissing. It can also dismiss triage-only findings that recommend dismissal and meet the configured confidence threshold. Security Agent attempts to write each auto-dismissed finding back to GitHub with a `[ChipMate auto-dismiss]` prefix. See [Configure auto-dismiss](#configure-auto-dismiss) for threshold behavior.
...
| Triage model | ChipMate Efficient | Used for initial triage and exploitability recommendations. |
| Analysis model | ChipMate Efficient | Used for sandbox analysis and result extraction. |
| Remediation model | ChipMate Efficient | Used by Cloud Agent for remediation PR work. |
...
- **All repositories** includes every repository currently available to the ChipMate GitHub App.
...
ChipMate Efficient is the default for all three stages. You can change each model independently. The model recorded in finding details is the model used when that analysis or remediation attempt ran. AI triage, sandbox analysis, and remediation consume ChipMate credits.
...
Without Include existing findings, previously synced findings are not added to the automatic-analysis backlog. Turning on Include existing findings queues eligible open findings that already exist in ChipMate. Re-enabling Auto-analysis while Include existing findings remains on also checks that backlog. This can use additional credits.
...
Security Agent records the dismissal in ChipMate and attempts to sync it to GitHub with a `[ChipMate auto-dismiss]` prefix. A GitHub write-back failure does not reopen the local finding. See [Dismiss findings](#dismiss-findings) for manual dismissal behavior.
...
| Setting | Default | Notes |
|---|---|---|
| New-finding Notifications | Off | Sends an email when ChipMate first inserts an eligible open finding. |
...
A New-finding Notification is eligible only when ChipMate first inserts an open finding whose severity meets the configured minimum. Severity thresholds are cumulative: for example, High and above includes high and critical findings. Later updates, severity changes, and reopening do not make an existing finding new again. Lowering the severity threshold also does not replay earlier findings.

Existing Dependabot alerts discovered during the first Security Agent sync count as new because that sync is the first time ChipMate inserts them. Enabling New-finding Notifications later does not replay those historical insertions.
...
| Kind | When eligible |
|---|---|
| New-finding Notification | ChipMate first inserts an eligible open finding. |
...
Delivery is asynchronous. Before sending an email, ChipMate rechecks the current finding state, Security Agent settings, severity thresholds, SLA state, and recipient authorization. ChipMate cancels unsent notification work if the finding is fixed, dismissed, superseded, deleted, or no longer meets the configured threshold, or if the recipient is no longer authorized.

Email subjects are:

| Kind | Subject |
|---|---|
| New finding | ChipMate Security Agent: New finding |
| SLA warning | ChipMate Security Agent: SLA warning |
| SLA breach | ChipMate Security Agent: SLA breached |
...
The audit report is based on activity recorded by ChipMate. It does not prove that every historical event is present, show repository scan coverage, or provide aggregate SLA compliance.

### Report period and filters

By default, the report covers the last 90 calendar days, ending today. A reporting period cannot exceed 90 inclusive calendar days. ChipMate rejects future or reversed ranges.
...
Each timeline event shows when ChipMate recorded or applied the activity, who or what performed it, and the evidence needed to understand the outcome. Superseded and deleted findings retain their recorded identity and lifecycle context.

### Reportable activity

The audit report includes material Security Finding activity recorded during the selected period:

- a finding imported into ChipMate;
- a severity change;
...
If a report query fails, times out, or exceeds its budget, ChipMate returns no report content rather than partial results. Choose a shorter reporting period and generate the report again.

### Report access

Personal audit reports are available to the owning user.

Organization audit reports are available to organization owners, billing managers, and ChipMate platform admins. Under current route permissions, organization members who are not owners or billing managers can use other Security Agent surfaces but cannot access organization audit reports.
