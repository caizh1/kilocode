 ...
 * `GIT_SSH_COMMAND` originated from ChipMate (safe) or was inherited from the
 * parent process (untrusted).
 */
export const CHIPMATE_NON_INTERACTIVE_SSH_COMMAND = "ssh -o BatchMode=yes"
...
export function nonInteractiveEnv(): NodeJS.ProcessEnv {
  ...
  if (!process.env.GIT_SSH_COMMAND) {
    env.GIT_SSH_COMMAND = CHIPMATE_NON_INTERACTIVE_SSH_COMMAND
  }
  return env
}

/**
 * True when `env.GIT_SSH_COMMAND` is the fixed value ChipMate sets, rather than
 ...
 */
export function isChipMateOwnedSshCommand(env: NodeJS.ProcessEnv): boolean {
  return env.GIT_SSH_COMMAND === CHIPMATE_NON_INTERACTIVE_SSH_COMMAND
}
...
export class GitOps {
  ...
  async buildWorktreePatch(sourcePath: string, baseBranch: string, selectedFiles?: string[]): Promise<string> {
    const tmp = await fs.mkdtemp(nodePath.join(os.tmpdir(), "chipmate-apply-"))
