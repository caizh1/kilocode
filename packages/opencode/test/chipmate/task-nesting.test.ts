...
import * as SandboxPolicy from "../../src/chipmate/sandbox/policy"
...
describe("ChipMate task nesting", () => {
  ...
  test("preserves inherited restrictions while refreshing prompt tool toggles", () => {
    const permission = ChipMateSessionPrompt.mergeToolPermissions({
      existing: [
      ...
      () =>
        Effect.gen(function* () {
          ...
          const effective = Permission.merge(
            ...
            ChipMateSessionPrompt.guardPermissions({ agent: validator, session: child }),
          )
