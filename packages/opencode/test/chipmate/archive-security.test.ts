...
test("extracts ZIP paths with PowerShell metacharacters literally", async () => {
  ...
  const cmd = "Compress-Archive -LiteralPath $env:CHIPMATE_TEST_SOURCE -DestinationPath $env:CHIPMATE_TEST_ARCHIVE -Force"

  await fs.writeFile(source, "safe")
  await Process.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", cmd], {
    env: {
      CHIPMATE_TEST_SOURCE: source,
      CHIPMATE_TEST_ARCHIVE: archive,
    },
  })
