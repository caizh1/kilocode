...
export function registerHeapSnapshot(context: vscode.ExtensionContext, connectionService: ChipMateConnectionService): void {
  context.subscriptions.push(
    vscode.commands.registerCommand("chipmate.v2.takeHeapSnapshot", async () => {
      try {
...
async function snapshot(connectionService: ChipMateConnectionService) {
  ...
  const res = await fetch(`${cfg.baseUrl}/chipmate/heap/snapshot`, {
    method: "POST",
    headers: {
