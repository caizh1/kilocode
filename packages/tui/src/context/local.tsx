...
import { useProject } from "./project" // chipmate_change
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    const sync = useSync()
    const sdk = useSDK()
    const project = useProject() // chipmate_change
    ...
    function createAgent() {
      ...
      return {
        ...
        current() {
          // chipmate_change start - fall back to first agent when current is removed (e.g. org switch)
        ...
        move(direction: 1 | -1) {
          batch(() => {
            ...
            if (!value) return // chipmate_change - guard against empty agent list during org switch
    ...
    function createModel() {
      const [modelStore, setModelStore] = createStore<{
        ready: boolean
        // chipmate_change start - persisted picks plus process-local overrides
        model: Record<
          string,
          | {
              providerID: string
      ...
      const state = {
        pending: false,
        writer: Promise.resolve() as Promise<unknown>, // chipmate_change - serialize writes
      }

      // chipmate_change start - keep configured-agent selections process-local
      ...
      function save() {
        ...
        state.pending = false
        // chipmate_change start - serialize writes so a slow first write cannot overwrite a later one
        const data = {
      ...
      readJson<unknown>(filePath)
        .then((x) => {
          ...
          if (typeof value.model === "object" && value.model !== null)
            setModelStore("model", value.model as Record<string, { providerID: string; modelID: string } | undefined>) // chipmate_change
        })
        .catch(() => {})
        .finally(() => {
      ...
      const currentModel = createMemo(() => {
        ...
        if (!a) return fallbackModel() // chipmate_change - guard against empty agent list
        // chipmate_change start - configured models beat stale persisted picks
        return (
          getFirstValidModel(
      ...
      return {
        ...
        // chipmate_change start - expose persisted per-agent pick separately from overrides
        saved(name: string) {
          return modelStore.model[name]
        },
        // chipmate_change end
        // chipmate_change start - resolve once all queued writes (atomic write+rename) have settled.
        ...
        cycle(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, val, !a.model) // chipmate_change
          save() // chipmate_change
        },
        cycleFavorite(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, next, !a.model) // chipmate_change
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            ...
            const a = agent.current()
            if (!a) return
            apply(a.name, model, !a.model) // chipmate_change
            if (options?.recent) {
    ...
    createEffect(() => {
      // chipmate_change start - configured models resolve directly without persistence
