...
description: "Learn how to use the ChipMate chat interface effectively"
---

# Chatting with ChipMate

{% callout type="tip" %}
**Bottom line:** ChipMate is an AI coding assistant. You chat with it in plain English, and it writes, edits, and explains code for you.
{% /callout %}
...
Click the ChipMate icon ({% chipmateCodeIcon /%}) in VS Code's Primary Side Bar to open the sidebar chat. You can also pop it out into an editor tab for a larger workspace.

{% /tab %}
{% tab label="CLI" %}

Open your terminal and run `chipmate` to launch the interactive terminal interface (TUI). You'll see a prompt where you can start typing requests immediately. The TUI is fully keyboard-driven — no mouse required.

{% /tab %}
{% /tabs %}

## How to Talk to ChipMate

**The key insight:** Just type what you want in normal English. No special commands needed.

{% image src="/docs/img/typing-your-requests/typing-your-requests.png" alt="Example of typing a request in ChipMate" width="800" caption="Example of typing a request in ChipMate" /%}
...
ChipMate builds the export from the complete local session history, not only the messages currently loaded in the chat view.
...
| Assuming ChipMate knows context | Use `@` to reference specific files |
...
**Why it matters:** ChipMate works best when you communicate like you're talking to a smart teammate who needs clear direction.

## Suggested Responses

When ChipMate needs more information to complete a task, it asks a follow-up question and often provides suggested answers to make responding faster.

**How it works:**

1. **Question Appears** - ChipMate asks a question using the `question` tool
...
- **Clarity** - Suggestions often clarify the type of information ChipMate needs
- **Flexibility** - Edit suggestions to provide precise, customized answers when needed

This feature streamlines the interaction when ChipMate requires clarification, allowing you to guide the task effectively with minimal effort.
...
{% callout type="tip" %}
**Move ChipMate to the Secondary Side Bar** for a better layout. Right-click on the ChipMate icon in the Activity Bar and select **Move To → Secondary Side Bar**. This lets you see the Explorer, Search, Source Control, etc. alongside ChipMate.

{% image src="/docs/img/move-to-secondary.png" alt="Move to Secondary Side Bar" width="600" caption="Move ChipMate to the Secondary Side Bar for better workspace organization" /%}
{% /callout %}
...
Ready to start coding? Start a session in ChipMate and describe what you want to build!
