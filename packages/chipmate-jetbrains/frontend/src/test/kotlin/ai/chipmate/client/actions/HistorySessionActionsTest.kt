...
import ai.chipmate.client.session.history.CloudHistoryItem
import ai.chipmate.client.session.history.HistoryController
import ai.chipmate.client.session.history.HistoryDataKeys
import ai.chipmate.client.session.history.HistorySelection
import ai.chipmate.client.session.history.HistorySource
...
class HistorySessionActionsTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val workspaces = ChipMateWorkspaceService(coroutines.scope, FakeWorkspaceRpcApi().also {
            it.state.value = ChipMateWorkspaceStateDto(status = ChipMateWorkspaceStatusDto.READY)
        })
    ...
    fun `test rename action passes untitled fallback to input`() {
        ...
        assertEquals(listOf(ChipMateBundle.message("history.untitled")), prompts)
        assertTrue(rpc.renames.isEmpty())
    }
    ...
    fun `test frontend descriptor registers history actions`() {
        val xml = javaClass.classLoader.getResourceAsStream("chipmate.jetbrains.frontend.xml")
            ?.bufferedReader()
            ?.use { it.readText() }
            ?: error("missing frontend descriptor")

        assertTrue(xml.contains("id=\"ChipMate.Session.Open\""))
        assertTrue(xml.contains("id=\"ChipMate.Session.Rename\""))
        assertTrue(xml.contains("id=\"ChipMate.Session.Delete\""))
        assertTrue(xml.contains("id=\"ChipMate.History.ContextMenu\""))
        assertTrue(xml.contains("id=\"ChipMate.Session.ContextMenu\""))
        assertTrue(xml.contains("ref=\"ChipMate.Session.Open\""))
        assertTrue(xml.contains("ref=\"ChipMate.Session.Rename\""))
        assertTrue(xml.contains("ref=\"ChipMate.Session.Delete\""))
