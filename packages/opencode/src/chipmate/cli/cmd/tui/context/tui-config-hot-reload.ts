 ...
 * (`sdk.tui.config.get`) and reconcile it into the store via `ChipMateTuiConfig.useSet`. The
...
import { ChipMateTuiConfig } from "./tui-config"
...
export function useTuiConfigHotReload() {
  const set = ChipMateTuiConfig.useSet()
  const sdk = useSDK()
