  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      const fileChanges: Array<{
        ...
        deletions: number
        bom: boolean
        encoding: string // chipmate_change - preserved per-file encoding
      }> = []
        ...
        switch (hunk.type) {
          case "add": {
            ...
            fileChanges.push({
              ...
              deletions,
              bom: next.bom,
              encoding: "utf-8", // chipmate_change - new files default to utf-8
            })
          ...
          case "update": {
            ...
            // chipmate_change start - encoding-aware read so non-UTF-8 files decode without
            ...
            let bom = source.bom
            let encoding = read.encoding // chipmate_change - overwritten by deriveNewContentsFromChunks below
          ...
          case "delete": {
            // chipmate_change start - encoding-aware read so non-UTF-8 files decode without corruption
            ...
            fileChanges.push({
              ...
              deletions,
              bom: source.bom,
              encoding: deleteRead.encoding, // chipmate_change
            })
      ...
      for (const change of fileChanges) {
        ...
        switch (change.type) {
          case "add":
            ...
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // chipmate_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
            updates.push({ file: change.filePath, event: "add" })
            break

          case "update":
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // chipmate_change - encoding-aware write replaces afs.writeWithDirs
          ...
          case "move":
            if (change.movePath) {
              // Create parent directories (recursive: true is safe on existing/root dirs)
              yield* EncodedIO.write(afs, change.movePath, Bom.join(change.newContent, change.bom), change.encoding) // chipmate_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
      ...
      // chipmate_change start - append ChipMate config validation warnings
      ...
      return {
        ...
        metadata: {
          diff: totalDiff,
          files,
          diagnostics: filterDiagnostics(diagnostics, changedPaths), // chipmate_change
        },
        output,
      }
    })
