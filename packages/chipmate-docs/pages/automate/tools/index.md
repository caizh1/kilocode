...
description: Learn how ChipMate's tools automate your development workflow
---

# Tool Use Overview

ChipMate implements a sophisticated tool system that allows AI models to interact with your development environment in a controlled and secure manner. This document explains how tools work, when they're called, and how they're managed.
...
| **Browser Group** | Web browser automation | `chipmate-playwright_*` (via built-in Playwright MCP) | Browser testing and interaction |
...
These tools help ChipMate understand your code and project:
...
These tools help ChipMate make changes to your code:
...
For multiple replacements in one file, ChipMate uses repeated `edit` calls or a patch-style edit when the model supports it.

### Execute Tools

These tools help ChipMate run commands:

- `bash` - Runs shell commands with configurable timeout and working directory

### Web Tools

These tools help ChipMate access web content:
...
`websearch` is available automatically with the ChipMate provider. For models from other providers it is off by default; enable it for all providers by setting `web_search` in `chipmate.jsonc`:
...
In the VS Code extension, the same option lives under **Settings → Web Tools → Web Search → Enable for All Providers**. The `CHIPMATE_ENABLE_EXA` and `CHIPMATE_ENABLE_PARALLEL` environment flags also enable it.

#### Web Search Providers

`websearch` routes through the Exa or Parallel search providers. When the Exa provider is used and you are signed into ChipMate, requests go through the ChipMate proxy automatically — no separate Exa API key is required. Setting `EXA_API_KEY` uses your own Exa key instead. Exa searches return at most 10 results.

Set the `CHIPMATE_WEBSEARCH_PROVIDER` environment variable to force a provider:

| Value | Behavior |
|---|---|
| `exa` | Use Exa — through the ChipMate proxy when signed in, through `EXA_API_KEY` when set |
| `parallel` | Use Parallel |
| `chipmate-exa` | Always route Exa searches through the ChipMate proxy (requires ChipMate sign-in) |

### Browser Tools

The VS Code extension has a built-in browser automation tool powered by [Playwright MCP](https://www.npmjs.com/package/@playwright/mcp). Enable it in Settings → Browser Automation. When enabled, it registers an MCP server named `chipmate-playwright` and exposes tools such as:

- `chipmate-playwright_browser_navigate` - Navigate to a URL
- `chipmate-playwright_browser_click` - Click an element
- `chipmate-playwright_browser_type` - Type text into an element
- `chipmate-playwright_browser_screenshot` - Capture a screenshot
- `chipmate-playwright_browser_snapshot` - Capture an accessibility snapshot
