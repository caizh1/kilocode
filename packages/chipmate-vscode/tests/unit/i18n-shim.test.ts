...
describe("extension host i18n", () => {
  it("returns translated string for known key", () => {
    const result = t("chipmate:autocomplete.statusBar.enabled")
    expect(typeof result).toBe("string")
    expect(result.length).toBeGreaterThan(0)
    expect(result).not.toBe("chipmate:autocomplete.statusBar.enabled")
  })
  ...
  it("interpolates a single variable", () => {
    const result = t("chipmate:autocomplete.statusBar.tooltip.noUsableProvider", {
      providers: "OpenAI, Anthropic",
      command: "command:chipmate.v2.settingsButtonClicked",
    })
  ...
  it("interpolates multiple variables", () => {
    const result = t("chipmate:autocomplete.statusBar.tooltip.completionSummary", {
      count: "5",
      startTime: "10:00",
      endTime: "11:00",
      cost: "$0.05",
  ...
  it("leaves unreferenced vars intact in template", () => {
    const key = "chipmate:autocomplete.statusBar.tooltip.noUsableProvider"
  ...
  it("prefers ChipMate new language setting over VS Code language", () => {
    const vscode = {
      env: { language: "en" },
      workspace: {
        getConfiguration: (section: string) => ({
          get: () => (section === "chipmate.v2" ? "de" : undefined),
        }),
      },
  ...
  it("uses VS Code language when ChipMate language setting is automatic", () => {
    const vscode = {
      env: { language: "nl" },
      workspace: {
        getConfiguration: () => ({
  ...
  it("translates status bar tooltip copy for German", () => {
    const text = translate("de", "chipmate:autocomplete.statusBar.tooltip.completionSummary", {
      count: 1,
      startTime: "12:25:24",
      endTime: "12:25:26",
      cost: "$0.00",
