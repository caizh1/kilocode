package ai.chipmate.client.session.views.base
 ...
 * to [ai.chipmate.client.session.views.MessageView] without an extra component wrapper.
 *
...
abstract class PartView : JPanel(), Disposable, SessionEditorStyleTarget {
     ...
     * Called when [ai.chipmate.client.session.model.SessionModelEvent.ContentUpdated] fires.
     */
    abstract fun update(content: Content)

    /**
     * Append a streaming delta to the existing content.
     * Only meaningful for text-bearing renderers ([ai.chipmate.client.session.views.TextView],
     * [ai.chipmate.client.session.views.ReasoningView]); others ignore deltas by default.
     */
