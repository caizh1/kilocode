...
import { IgnoreMigrator } from "../../src/chipmate/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .chipmateignore", async () => {
    ...
    test("loads project .chipmateignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .chipmateignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".chipmateignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .chipmateignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .chipmateignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".chipmateignore"), "secrets/")
        },
