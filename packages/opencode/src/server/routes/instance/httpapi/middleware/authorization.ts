...
// chipmate_change start - require auth for high-risk permission toggles even when global auth is optional
...
function validateCredential<A, E, R>(
  ...
  force = ServerAuth.required(config), // chipmate_change - allow endpoint-specific required auth
) {
  return Effect.gen(function* () {
    if (!force) return yield* effect // chipmate_change
...
// chipmate_change start - fail closed for high-risk unauthenticated endpoints
...
export const authorizationLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Authorization.of((effect) =>
      Effect.gen(function* () {
        const request = yield* HttpServerRequest.HttpServerRequest
        const url = new URL(request.url, "http://localhost") // chipmate_change - inspect endpoint-specific auth policy
        if (!guarded(url, config)) return yield* effect // chipmate_change
        return yield* credentialFromRequest(request).pipe(
          Effect.flatMap((credential) => validateCredential(effect, credential, config, true)), // chipmate_change
        )
      }),
    )
  }),
