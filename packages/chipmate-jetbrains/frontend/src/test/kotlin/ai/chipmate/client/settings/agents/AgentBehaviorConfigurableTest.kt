...
class AgentBehaviorConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test id matches xml registration`() {
        ...
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior", cfg.id)
    }

    fun `test child ids match xml registration`() {
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior.agents", AgentsConfigurable.ID)
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior.mcp", McpConfigurable.ID)
        assertEquals("ai.chipmate.jetbrains.settings.agentBehavior.skills", SkillsConfigurable.ID)
