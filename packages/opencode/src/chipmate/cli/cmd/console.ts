...
import type { Daemon } from "@/chipmate/daemon/daemon"
import { cmd } from "@/cli/cmd/cmd"
import { explicitNetworkOptions, withNetworkOptions } from "@/cli/network"
import { serverUrls } from "@/chipmate/cli/server-urls"
import { hasDisplay } from "@/chipmate/cli/cmd/tui/util/display"
import { StopCommand } from "@/chipmate/cli/cmd/daemon"
...
const OpenCommand = cmd({
  command: "$0",
  describe: "open the local ChipMate Console (deprecated)",
  builder: (yargs) =>
  ...
  handler: async (args) => {
    console.warn("ChipMate Console is deprecated and will be removed in a future release.")
    const { Daemon } = await import("@/chipmate/daemon/daemon")
    const { warnedNetworkOptions } = await import("@/chipmate/cli/port-warning")
    const run = async (signal?: AbortSignal) => {
      ...
      if (!state) throw new Error("ChipMate daemon did not provide connection state")
      if (signal?.aborted) return state
      if (daemon.restarted) console.warn("Restarted the ChipMate daemon to apply the requested network options")
      ...
      if (hasDisplay()) {
        ...
        console.warn("No display detected; open the ChipMate Console URL manually")
      }
      console.log("ChipMate Console:")
    ...
    await Daemon.foreground(async (signal) => {
      ...
      if (!signal.aborted) console.log("Press Ctrl+C to stop the ChipMate daemon.")
      return state
    })
  },
})

export const ChipMateConsoleCommand = cmd({
  command: "console",
  describe: "open or stop the local ChipMate Console (deprecated)",
