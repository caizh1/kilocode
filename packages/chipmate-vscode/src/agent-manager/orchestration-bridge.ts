...
import { sameDirectory } from "../chipmate-provider-utils"
...
export class AgentManagerOrchestrationBridge {
  ...
  private event(event: SSEPayload, directory?: string): void {
    ...
    if (event.type === "chipmate.agent_manager.requested") {
      this.request((event as unknown as { properties: Request }).properties, directory)
      return
    }
    if (event.type === "chipmate.agent_manager.cancelled") {
  ...
  private async reply(requestID: string, directory: string, result: Result): Promise<boolean> {
    try {
      const response = await this.connection.getClient().chipmate.agentManager.reply({ requestID, directory, result })
  ...
  private async reject(requestID: string, directory: string, error: Failure): Promise<boolean> {
    try {
      const response = await this.connection.getClient().chipmate.agentManager.reject({ requestID, directory, error })
      if (!response.error) return true
  ...
  private async recover(revision: number): Promise<void> {
    ...
    await Promise.all(
      this.connection.getKnownDirectories().map(async (directory) => {
        const response = await client.chipmate.agentManager.list({ directory }).catch((error: unknown) => {
