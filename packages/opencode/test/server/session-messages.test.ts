...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
    ...
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
