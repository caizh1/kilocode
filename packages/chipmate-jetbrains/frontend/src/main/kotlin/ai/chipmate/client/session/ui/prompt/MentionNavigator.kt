...
internal class MentionNavigator(
    ...
    private val provider: ChipMatePromptCompletionProvider,
    ...
    fun install() {
        ...
        object : DumbAwareAction(ChipMateBundle.message("prompt.mention.goto")) {
    ...
    private fun syncTooltip(hit: ChipMatePromptCompletionProvider.MentionHit?) {
        val text = hit?.takeIf { !it.resolved }?.let { ChipMateBundle.message("prompt.mention.unresolved", it.value) }
        if (editor.contentComponent.toolTipText != text) editor.contentComponent.toolTipText = text
    }

    private fun syncLink(hit: ChipMatePromptCompletionProvider.MentionHit?, mouse: MouseEvent) {
