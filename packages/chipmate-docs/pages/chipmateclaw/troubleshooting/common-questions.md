...
description: "Answers to common ChipMateClaw troubleshooting questions"
---
...
2. Click **OpenClaw Doctor** on your [dashboard](/docs/chipmateclaw/dashboard)
...
1. Check that the channel token is configured in [Settings](/docs/chipmateclaw/dashboard#channels)
...
You can directly access the files in `/root/.openclaw/` on the [ChipMateClaw Dashboard](https://app.chipmate.ai/claw) using the file browser of the edit files dialog. This can be a useful way to examine or update the config files (especially `openclaw.json`) if you run into an issue. There may also be backups in the form of `openclaw.bak` files that you can manually restore from if needed.
...
3. If the issue persists, join the [ChipMate Discord](https://chipmate.ai/discord) and share details in the ChipMateClaw channel
