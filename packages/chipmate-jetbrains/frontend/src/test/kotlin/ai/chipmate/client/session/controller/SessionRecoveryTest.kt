 ...
 * reconnect. JetBrains now does the same in [ai.chipmate.client.session.controller.SessionController.recoverPending].
 */
class SessionRecoveryTest : SessionControllerTestBase() {
    ...
    // ------ Status seeding from ChipMateSessionService.statuses ------
    ...
    fun `test retry status is seeded with message attempt and next`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [retry] [Rate limited]
            """,
            m, show = true,
        )
    ...
    fun `test offline status is seeded with message and requestId`() {
        ...
        assertSession(
            """
            [code] [chipmate/gpt-5] [offline] [No network]
            """,
            m, show = true,
        )
