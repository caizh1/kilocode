...
async function apply(connection: ChipMateConnectionService, directory: string, style: WorkStyle) {
  const settings = vscode.workspace.getConfiguration("chipmate.v2")
...
export async function handleWorkStyleApplyMessage(input: {
  ...
  if (input.message.style !== "human-in-the-loop" && input.message.style !== "autonomous") {
    console.error("[ChipMate New] Invalid style in applyWorkStyle message")
