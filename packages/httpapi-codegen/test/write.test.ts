...
import { join } from "node:path" // chipmate_change - use native separators in cross-platform tests
...
describe("HttpApiCodegen.write", () => {
  ...
  it.effect("removes only stale files owned by the previous manifest", () => {
    ...
    return write(
      ...
      Effect.tap(() => Effect.sync(() => expect(removed).toEqual([join("/generated", "old.ts")]))), // chipmate_change
    )
  })
