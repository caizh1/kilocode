...
usage() {
  ...
  printf '       %s <chipmate-log> <session-id>\n' "$0" >&2
}
...
resolve() {
  case "$target" in
    client|frontend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/chipmate.jetbrains/chipmate-frontend/chipmate-dev.log" \
    ...
    backend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/chipmate.jetbrains/chipmate-backend/chipmate-dev.log" \
...

{
  sub(/\r$/, "")
  cls = logger($0)
  if ((mode == "client" || mode == "frontend") && cls !~ /ChipMateSessionService$/) next
  if (mode == "backend" && cls !~ /ChipMateBackendChatManager$/) next
