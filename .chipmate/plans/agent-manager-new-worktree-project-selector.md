...
With `chipmate-code.new.experimental.multiProject` enabled, the New Worktree dialog has no
...
```
┌─ New Worktree ──────────────────────────────┐
│  [ New ] [ Import ]       [ folder  chipmate ▾ ] │  ← inline, multiProject only
...
- **In the dialog title** (`New Worktree in [chipmate ▾]`). Reads nicely but requires
  widening `Dialog`'s `title` prop to accept JSX, which touches chipmate-ui for one caller.
...
(persisted per project as `activeTarget` in each repo's `.chipmate/agent-manager.json`, plus
...
Checks to run before declaring done, from `packages/chipmate-vscode/`:
...
`chipmate-code.new.experimental.multiProject` enabled and two repositories registered:
...
```
           ┌────────────────────────────────────────────┐
           │ 📁 chipmate      ~/Documents/git/chipmate ✓│  ← .am-project-option-active
...
All of it goes into `webview-ui/agent-manager/agent-manager.css`. No changes to chipmate-ui.
...
- No new CSS variables or tokens. Only the ones listed above, all already in use in this
  file.
- No edits to `packages/chipmate-ui/` or `packages/ui/`.
...
- With `props.projects` empty, the rendered dialog markup must be identical to before the
  change. Verify by toggling `chipmate-code.new.experimental.multiProject` off.
