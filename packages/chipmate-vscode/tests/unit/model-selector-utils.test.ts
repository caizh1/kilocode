...
describe("providerSortKey", () => {
  it("returns 0 for chipmate gateway", () => {
    expect(providerSortKey(CHIPMATE_GATEWAY_ID)).toBe(0)
  })
  ...
  it("sorts providers correctly when used with sort", () => {
    const ids = ["google", "anthropic", "chipmate", "openai", "deepseek"]
    const sorted = ids.slice().sort((a, b) => providerSortKey(a) - providerSortKey(b))
    expect(sorted).toEqual(["chipmate", "anthropic", "deepseek", "openai", "google"])
  })
})
...
describe("stripSubProviderPrefix", () => {
  ...
  it("does not strip 'ChipMate: ' prefix", () => {
    expect(stripSubProviderPrefix("ChipMate: Auto")).toBe("ChipMate: Auto")
    expect(stripSubProviderPrefix("chipmate: Auto")).toBe("chipmate: Auto")
  })
})
...
describe("sanitizeName", () => {
  ...
  it("brands ChipMate Auto Free while preserving other bare trailing Free names", () => {
    expect(sanitizeName("ChipMate Auto Free")).toBe("ChipMate Auto Free")
...
describe("isAuto", () => {
  it("matches only ChipMate Auto model ids", () => {
    expect(isAuto({ providerID: CHIPMATE_GATEWAY_ID, id: "chipmate-auto/efficient" })).toBe(true)
    expect(isAuto({ providerID: CHIPMATE_GATEWAY_ID, id: "auto-small" })).toBe(true)
    expect(isAuto({ providerID: "anthropic", id: "chipmate-auto/efficient" })).toBe(false)
    expect(isAuto({ providerID: CHIPMATE_GATEWAY_ID, id: "anthropic/claude-sonnet" })).toBe(false)
  })
})
...
describe("autoChoices", () => {
  it("uses backend Auto Efficient routes and resolves names when available", () => {
    expect(
      autoChoices(
        {
          providerID: CHIPMATE_GATEWAY_ID,
  ...
  it("ignores missing routes and non-efficient Auto models", () => {
    expect(autoChoices({ providerID: CHIPMATE_GATEWAY_ID, id: "chipmate-auto/efficient" })).toEqual([])
    expect(
      autoChoices({
        providerID: CHIPMATE_GATEWAY_ID,
        id: "chipmate-auto/frontier",
...
describe("buildTriggerLabel", () => {
  it("returns resolved model name for non-chipmate provider unchanged", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", undefined, null, false, "", true, labels)).toBe("GPT-4o")
  })

  it("strips sub-provider prefix from resolved name for chipmate gateway models", () => {
    expect(
      buildTriggerLabel("Anthropic: Claude Sonnet", CHIPMATE_GATEWAY_ID, undefined, null, false, "", true, labels),
    ).toBe("Claude Sonnet")
  })

  it("does not strip prefix for non-chipmate provider even if name contains ': '", () => {
  ...
  it("returns providerName / resolvedName for non-chipmate provider with providerName", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", "OpenAI", null, false, "", true, labels)).toBe("OpenAI / GPT-4o")
  })

  it("returns modelID for chipmate gateway raw selection", () => {
    const raw = { providerID: "chipmate", modelID: "chipmate-auto/frontier" }
    expect(buildTriggerLabel(undefined, undefined, undefined, raw, false, "", true, labels)).toBe("chipmate-auto/frontier")
  })

  it("brands the raw chipmate-auto/free selection when the catalog is unavailable", () => {
    const raw = { providerID: "chipmate", modelID: "chipmate-auto/free" }
    expect(buildTriggerLabel(undefined, undefined, undefined, raw, false, "", true, labels)).toBe(
      "ChipMate Auto Free",
    )
  })

  it("returns providerID / modelID for non-chipmate raw selection", () => {
