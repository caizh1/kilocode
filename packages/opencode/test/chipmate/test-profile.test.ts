...
import { TestProfile } from "../../script/chipmate/test-profile"
...
describe("test profiles", () => {
  test("darwin profile contains valid test files", () => {
    ...
    expect(result.files).toContain("chipmate/cli/install-artifact.test.ts")
    expect(result.files).toContain("chipmate/cli/tui/thread.test.ts")
    expect(result.files).toContain("chipmate/sandbox/macos-confinement.test.ts")
    expect(result.files).toContain("chipmate/core-watcher.test.ts")
    expect(result.files).toContain("chipmate/background-process.test.ts")
    expect(result.files).toContain("filesystem/filesystem.test.ts")
    expect(result.files).toContain("chipmate/interactive-terminal.test.ts")
    const sandbox = all.filter((file) => file.startsWith("chipmate/sandbox/"))
    expect(result.files.filter((file) => file.startsWith("chipmate/sandbox/"))).toEqual(sandbox)
    ...
    expect(result.files).not.toContain("chipmate/sessions/remote-ws.test.ts")
    ...
    expect(result.files).not.toContain("chipmate/daemon.test.ts")
    ...
    expect(result.files).not.toContain("chipmate/server/config-overlay.test.ts")
