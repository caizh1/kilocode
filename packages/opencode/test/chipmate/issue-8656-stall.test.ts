...
// Regression coverage for https://github.com/ChipMate-Org/chipmate/issues/8656
//
...
// `config` hook (see ../fixture/stall-plugin.ts), so the SDK, ChipMate's fetch
...
function session(dir: string) {
  ...
  const headers = { "Content-Type": "application/json", "x-chipmate-directory": dir }
