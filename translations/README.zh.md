...
ChipMate 是一个 AI 编码代理，可以在你工作的任何地方使用：[VS Code](https://chipmate.ai/landing/vs-code)、[JetBrains](https://chipmate.ai/features/jetbrains-native) 和 [CLI](https://chipmate.ai/cli)。它是开源的，并采用开放定价。你可以从 500 多个模型中选择，在任务中途切换模型，并按模型提供商的价格付费，没有加价。开始使用无需 API 密钥。
...
直接安装 [ChipMate 扩展](vscode:extension/chipmate.chipmate-code)，或从 [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=chipmate.ChipMate-Code) 获取。创建账户后，你可以按提供商价格访问 500 多个模型，包括 GPT-5.5、Claude Opus 4.7、Claude Sonnet 4.6 和 Gemini 3.1 Pro Preview。
...
从 JetBrains Marketplace 安装 [ChipMate 插件](https://plugins.jetbrains.com/plugin/28350-chipmate-code)，或在任意 JetBrains IDE 的 `Settings → Plugins` 中搜索 "ChipMate"。
...
无需本地机器，在 Web 上通过 [app.chipmate.ai/cloud](https://app.chipmate.ai/cloud) 运行 ChipMate。
...
在 [app.chipmate.ai/code-reviews](https://app.chipmate.ai/code-reviews) 为你的 Pull Request 设置自动 AI 代码审查。
...
在 [app.chipmate.ai/claw](https://app.chipmate.ai/claw) 启动你的常驻 AI 代理。
...
从 [Releases 页面](https://github.com/ChipMate-Org/chipmate/releases) 下载最新二进制文件。

| 平台 | 资源 |
|---|---|
| Windows（大多数 PC） | `chipmate-windows-x64.zip` |
...
说明：`x64-baseline` 是面向不支持 AVX 的旧 CPU 的兼容构建。`musl` 是面向 Alpine 或无 glibc 的极简 Docker 镜像的静态链接构建。`chipmate-vscode-*.vsix` 是 VS Code 扩展包，不是 CLI。`Source code` 压缩包用于从源码构建。

</details>

### Agents

ChipMate 内置了可按任务切换的专用 Agents。你也可以构建自己的自定义 Agents。
...
了解更多关于 [agents 和自定义 agents](https://chipmate.ai/docs/code-with-ai/agents/using-agents) 的信息。
...
使用 `--auto` 运行 `chipmate run`，可在 CI/CD 流水线中实现无提示的完全自主操作：
...
关于配置和其他内容，请查看[文档](https://chipmate.ai/docs)。

### 贡献

欢迎开发者、写作者以及所有人参与贡献。请先阅读 [Contributing Guide](/CONTRIBUTING.md)，了解环境设置、编码标准以及如何创建 Pull Request。VS Code 扩展和 CLI 的发布流程请参阅 [RELEASING.md](../RELEASING.md)，JetBrains 插件请参阅 [packages/chipmate-jetbrains/RELEASING.md](../packages/chipmate-jetbrains/RELEASING.md)。
...
<details>
<summary>ChipMate CLI 从哪里来？</summary>

ChipMate CLI 是 [OpenCode](https://github.com/anomalyco/opencode) 的一个 fork，并增强为可在 ChipMate agentic engineering 平台中使用。

</details>

---

**加入社区** [Discord](https://chipmate.ai/discord) | [X](https://x.com/chipmate) | [Reddit](https://www.reddit.com/r/chipmate/)
