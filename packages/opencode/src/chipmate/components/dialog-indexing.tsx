...
import type { Config } from "@chipmate/sdk/v2"
import * as Log from "@opencode-ai/core/util/log"
import { hasChipMateIndexingAuth, resolveChipMateIndexingAuth, shouldDefaultIndexingToChipMate } from "../indexing-auth"
import {
  createIndexingDialogState,
  currentChipMateModel,
  indexingInheritance,
  indexingPatch,
  indexingScopeConfig,
  inheritedDescription,
  chipmateModelOptions,
  loadChipMateEmbeddingModels,
  mergeIndexingConfig,
  type IndexingScope,
...
function hasChipMateAuth(sync: ReturnType<typeof useSync>, scope: IndexingScope, indexing: IndexingConfig): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "chipmate")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  return hasChipMateIndexingAuth({ config, provider })
}
...
function defaultIndexing(
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "chipmate")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  const auth = resolveChipMateIndexingAuth({ config, provider })
  if (!shouldDefaultIndexingToChipMate({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "chipmate", model: null, dimension: null, dimensionMode: "auto" }
}
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "chipmate" || hasChipMateAuth(sync, props.scope, indexing) || indexing.provider === "chipmate")
    .map(([value, title]) => ({
      value,
      title,
...
function ChipMateModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const [catalog] = createResource(() => loadChipMateEmbeddingModels(setError))
  ...
  createEffect(() => {
    ...
    seen.state = state
    log.info("ChipMate embedding model resource changed", {
      state: catalog.state,
      models: cfg?.models.length ?? 0,
      current: currentChipMateModel(cfg, indexing.model),
      defaultModel: cfg?.defaultModel || undefined,
      scope: props.scope,
    })
  })
  const options = createMemo(() => chipmateModelOptions(catalog()))
  const current = createMemo(() => currentChipMateModel(catalog(), indexing.model))

  return (
    <DialogSelect
      title="ChipMate Embedding Model"
      options={options()}
      current={current()}
      renderFilter={(catalog()?.models.length ?? 0) > 0}
      onSelect={async (option) => {
        if (!option.value || !catalog()?.models.some((model) => model.id === option.value)) return
        log.info("selected ChipMate embedding model", { model: option.value, scope: props.scope })
...
function ScopeSelect(props: DialogIndexingProps & { scope: IndexingScope }) {
  ...
  const options: DialogSelectOption<IndexingScope>[] = [
    ...
    { value: "project", title: "Project", description: "Stored in this repo's .chipmate config" },
  ]
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    const result: DialogSelectOption<string>[] = [
      ...
      },
      {
        ...
        category: "Embedding",
        description: mark(
          indexing.provider === "chipmate" ? (indexing.model ?? "ChipMate catalog") : (indexing.model ?? "default"),
          [["model"]],
        ),
      },
      {
        ...
        category: "Embedding",
        description:
          indexing.provider === "chipmate"
            ? "provided by ChipMate"
            : mark(indexing.dimension ? String(indexing.dimension) : "auto", [["dimension"]]),
        disabled: indexing.provider === "chipmate",
      },
      {
        value: "vectorStore",
        title: "Vector Store",
      ...
      onSelect={async (option) => {
        ...
        switch (option.value) {
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "chipmate") {
              dialog.replace(() => (
                <ChipMateModelSelect useSDK={props.useSDK} scope={scope()} indexing={indexing} raw={raw} />
              ))
              break
            }
          ...
          case "dimension": {
            if (indexing.provider === "chipmate") break
