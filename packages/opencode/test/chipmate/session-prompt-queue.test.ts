...
import { ChipMateSessionCompaction } from "@/chipmate/session/compaction"
...
const dbfile = path.join(os.tmpdir(), `chipmate-prompt-queue-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      ChipMateSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => ChipMateSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const result = await Effect.runPromise(
      ChipMateSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          ChipMateSessionPromptQueue.retarget(sessionID, injected)
          return {
            active: ChipMateSessionPromptQueue.active(sessionID),
            ids: ChipMateSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id),
          }
        }),
        ...
        const result = await Effect.runPromise(
          ChipMateSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              ...
              return { compact, ids: ChipMateSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      ChipMateSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: ChipMateSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: ChipMateSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      ChipMateSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: ChipMateSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(ChipMateSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      ChipMateSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: ChipMateSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(ChipMateSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
  ...
  test("closes a queued-handoff turn as superseded, not interrupted", async () => {
    ...
    try {
      ...
      await provideTestInstance({
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            const closed: ChipMateSession.CloseReason[] = []
            const unsubscribe = Bus.subscribe(ChipMateSession.Event.TurnClose, (event) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() => (ChipMateSessionPromptQueue.hasFollowup(session.id) ? (true as const) : undefined)),
  ...
  test("cancel on a session with no active tail is a no-op and does not leak state", async () => {
    ...
    await Effect.runPromise(ChipMateSessionPromptQueue.cancel(sessionID))

    expect(ChipMateSessionPromptQueue._hasInternalState(sessionID)).toBe(false)
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() =>
                  ChipMateSessionPromptQueue.snapshot(session.id).length >= 2 ? (true as const) : undefined,
                ),
            ...
            const ids = await Effect.runPromise(
              ChipMateSessionPromptQueue.enqueue(
                session.id,
                MessageID.make("msg_probe"),
                Effect.succeed(ChipMateSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(ChipMateSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
      ...
      await provideTestInstance({
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() => (ChipMateSessionPromptQueue.hasFollowup(session.id) ? true : undefined)),
                "Timed out waiting for queued prompt",
              ),
            )
            expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(session.id, queuedID))).toBe(true)
            expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(session.id, activeID))).toBe(false)
  ...
  test("drop returns false for the actively running prompt", async () => {
    ...
    await ready.promise
    expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(sessionID, id))).toBe(false)
    done.resolve()
    await first
  })
  ...
  test("drop returns false for an unknown message", async () => {
    ...
    expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(sessionID, MessageID.make("msg_missing")))).toBe(false)
  })
  ...
  test("drop cancels a queued prompt and is idempotent", async () => {
    ...
    expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(sessionID, secondID))).toBe(true)
    expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(sessionID, secondID))).toBe(false)
    done.resolve()
  ...
  describe("session.queue.changed", () => {
    test("snapshot() returns an empty list for an unknown session", () => {
      expect(ChipMateSessionPromptQueue.snapshot(SessionID.make("session_unknown"))).toEqual([])
    })
    ...
    test("enqueueing on an idle session does not transiently publish a non-empty snapshot", async () => {
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const off = Bus.subscribe(ChipMateSession.Event.QueueChanged, (event) => {
          ...
          try {
            ...
            expect(ChipMateSessionPromptQueue.snapshot(sessionID)).toEqual([])
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(ChipMateSessionPromptQueue.snapshot(sessionID)).toEqual([m2, m3])
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(await Effect.runPromise(ChipMateSessionPromptQueue.drop(sessionID, m2))).toBe(true)
            expect(ChipMateSessionPromptQueue.snapshot(sessionID)).toEqual([m3])
            expect(ChipMateSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
            await Effect.runPromise(
              pollWithTimeout(
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(ChipMateSessionPromptQueue.snapshot(sessionID)).toEqual([m2])
            await Bun.sleep(10)
