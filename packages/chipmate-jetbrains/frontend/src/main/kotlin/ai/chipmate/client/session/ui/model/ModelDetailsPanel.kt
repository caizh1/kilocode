package ai.chipmate.client.session.ui.model
...
internal class ModelDetailsPanel(
    ...
    private val empty = JBLabel(ChipMateBundle.message("model.picker.details.empty")).apply {
    ...
    private val data = badge(ChipMateBundle.message("model.picker.dataCollected"))
    private val latest = badge(ChipMateBundle.message("model.picker.details.latest"))
    private val badges = Stack.horizontal(UiStyle.Gap.xs()).next(free).next(byok).next(data).next(latest)
    private val props = RowsSection(ChipMateBundle.message("model.picker.details.properties"))
    private val bench = RowsSection(ChipMateBundle.message("model.picker.details.terminalBench"))
    private val caps = TagsSection(ChipMateBundle.message("model.picker.details.capabilities"))
    private val desc = MarkdownSection(ChipMateBundle.message("model.picker.details.description"))
    private val routeText = JBLabel().apply {
        foreground = UIUtil.getLabelForeground()
        setAllowAutoWrapping(true)
    }
    private val route = Stack.vertical(UiStyle.Gap.xs())
        .next(heading(ChipMateBundle.message("model.picker.details.autoRouting")))
        .next(routeText)
    private val ids = RowsSection(ChipMateBundle.message("model.picker.details.ids"))
    ...
    fun update(value: ModelPicker.Item?) {
        ...
        syncRouting(value)
        ids.update(listOf(
            ChipMateBundle.message("model.picker.details.providerId") to value.provider,
            ChipMateBundle.message("model.picker.details.modelId") to value.id,
        ))
        refresh()
    }
    ...
    private fun syncStar(value: ModelPicker.Item) {
        ...
        star.toolTipText = if (selected) {
            ChipMateBundle.message("model.picker.favorite.remove")
        } else {
            ChipMateBundle.message("model.picker.favorite.add")
        }
    }
    ...
    private fun properties(item: ModelPicker.Item): List<Pair<String, String>> {
        ...
        return buildList {
            item.releaseDate?.let { add(ChipMateBundle.message("model.picker.details.released") to date(it)) }
            if (!item.free) {
                item.cost?.takeIf { it.input > 0.0 || it.output > 0.0 }?.let { cost ->
                    add(ChipMateBundle.message("model.picker.details.input") to price(cost.input))
                    add(ChipMateBundle.message("model.picker.details.output") to price(cost.output))
                    add(ChipMateBundle.message("model.picker.details.cached") to cached(cost.input, cost.cache?.read))
                    add(ChipMateBundle.message("model.picker.details.average") to price(average(cost.input, cost.output, cost.cache?.read)))
                } ?: run {
                    item.inputPrice?.takeIf { it > 0.0 }?.let { add(ChipMateBundle.message("model.picker.details.input") to price(it)) }
                    item.outputPrice?.takeIf { it > 0.0 }?.let { add(ChipMateBundle.message("model.picker.details.output") to price(it)) }
                }
            }
            ctx?.let { add(ChipMateBundle.message("model.picker.details.context") to context(it)) }
        }
    }
    ...
    private fun bench(item: ModelPicker.Item): List<Pair<String, String>> {
        ...
        return listOf(
            ChipMateBundle.message("model.picker.details.completion") to percent(bench.overallScore),
            ChipMateBundle.message("model.picker.details.costAttempt") to attempt(bench.avgAttemptCostUsd),
        )
    }
    ...
    private fun capabilities(item: ModelPicker.Item): List<String> {
        ...
        return buildList {
            if (cap?.reasoning == true || item.reasoning) add(ChipMateBundle.message("model.picker.details.reasoning"))
            val input = cap?.input
            if (input?.text == true) add(ChipMateBundle.message("model.picker.details.modality.text"))
            if (input?.image == true) add(ChipMateBundle.message("model.picker.details.modality.image"))
            if (input?.audio == true) add(ChipMateBundle.message("model.picker.details.modality.audio"))
            if (input?.video == true) add(ChipMateBundle.message("model.picker.details.modality.video"))
            if (input?.pdf == true) add(ChipMateBundle.message("model.picker.details.modality.pdf"))
            if (item.attachment) add(ChipMateBundle.message("model.picker.details.attachments"))
        }
    }
...
private fun price(value: Double): String {
    if (value == 0.0) return ChipMateBundle.message("model.picker.free")
...
private fun cached(input: Double, read: Double?): String {
    ...
    return ChipMateBundle.message("model.picker.details.notSupported")
}
