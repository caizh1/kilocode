...
import { generateHelp, generateCommandTable } from "../../src/chipmate/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // chipmate_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/chipmate/help-command"
import { ProfileCommand } from "../../src/chipmate/cli/cmd/profile"
import { DaemonCommand } from "../../src/chipmate/cli/cmd/daemon"
import { ChipMateConsoleCommand } from "../../src/chipmate/cli/cmd/console"
import { CloudCommand } from "../../src/chipmate/cli/cmd/cloud"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running chipmate server",
  handler() {},
}
...
const commands = [
  ...
  ProfileCommand,
  DaemonCommand,
  ChipMateConsoleCommand,
  CloudCommand,
  HelpCommand,
  CompletionStub,
] as any[]

describe("chipmate help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent", "profile"]) {
      expect(output).toContain(`## chipmate ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("chipmate auth login")
    expect(output).toContain("chipmate auth logout")
    expect(output).toContain("chipmate debug config")
  })
})

describe("chipmate help --all (text)", () => {
...
describe("chipmate help <command>", () => {
  test("chipmate help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("chipmate auth login")
    expect(output).toContain("chipmate auth logout")
    expect(output).toContain("chipmate auth list")
  })

  test("chipmate help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## chipmate run")
    expect(output).not.toContain("## chipmate debug")
  })

  test("documents console stop and foreground mode", async () => {
    const output = await generateHelp({ command: "console", format: "md", commands })
    expect(output).toContain("chipmate console stop")
  ...
  test("documents daemon foreground mode", async () => {
    ...
    expect(output).toContain("chipmate daemon start")
    expect(output).toContain("--foreground")
    expect(output).toContain("-f")
  })
})

describe("chipmate cloud help", () => {
  async function parser() {
    const cli = yargs([])
      .scriptName("chipmate cloud")
      .exitProcess(false)
      .help()
      .fail((msg, err) => {
  ...
  test("requires a subcommand and exposes only the public Cloud Agent operations", async () => {
    ...
    const names = [...help.matchAll(/^\s*chipmate cloud ([a-z][a-z-]*)\b/gm)].map((match) => match[1])
...
describe("edge cases", () => {
  ...
  test("chipmate help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`chipmate ${name}`)
    }
  })

  test("default command appears as chipmate [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`chipmate [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`chipmate generate`")
  })

  test("contains chipmate completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`chipmate completion`")
  })

  test("contains chipmate help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`chipmate help")
  })
})

describe("ChipMate CLI customizations are wired into index.ts", () => {
  const file = (rel: string) => Bun.file(path.resolve(import.meta.dir, rel)).text()
  const INDEX = "../../src/index.ts"
  const SETUP = "../../src/chipmate/cli/setup.ts"
  const BARREL = "../../src/chipmate/commands.ts"

  test("CLI is branded `chipmate`, not `opencode`", async () => {
    const index = await file(INDEX)
    expect(index).toContain('.scriptName("chipmate")')
    expect(index).not.toContain('.scriptName("opencode")')
  })

  test("index.ts invokes the ChipMateCli integration points", async () => {
    // These thin call-sites are the only wiring between upstream index.ts and the ChipMate
    // customizations in setup.ts. If a future upstream merge drops them, every ChipMate command
    // and the telemetry/lifecycle hooks silently disappear, exactly the regression this guards.
    const index = await file(INDEX)
    expect(index).toContain("ChipMateCli.register(")
    expect(index).toContain("ChipMateCli.bootstrap(")
    expect(index).toContain("ChipMateCli.shutdown(")
  })

  test("registers the local ChipMate Console instead of the upstream account console", async () => {
    const index = await file(INDEX)
    const setup = await file(SETUP)
    const barrel = await file(BARREL)
    expect(setup).toContain("ChipMateConsoleCommand")
