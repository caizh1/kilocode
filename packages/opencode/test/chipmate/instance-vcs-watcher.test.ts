...
const disableFilewatcher = process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
beforeAll(() => {
  delete process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER
})
afterAll(() => {
  if (disableFilewatcher !== undefined) process.env.CHIPMATE_EXPERIMENTAL_DISABLE_FILEWATCHER = disableFilewatcher
})
...
describe("ChipMateWatcher.eager", () => {
  test("skips eager location watchers for VS Code", () => {
    expect(ChipMateWatcher.eager("vscode")).toBe(false)
  })

  test("skips eager location watchers for JetBrains", () => {
    expect(ChipMateWatcher.eager("jetbrains")).toBe(false)
  })

  test("keeps eager location watchers for the standalone CLI", () => {
    expect(ChipMateWatcher.eager("cli")).toBe(true)
    expect(ChipMateWatcher.eager(undefined)).toBe(true)
  })
})
