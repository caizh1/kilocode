...
const OUTPUT = path.join(ROOT, "packages/chipmate-docs/source-links.md")

const check = process.argv.includes("--check")

const DIRS = [
  path.join(ROOT, "packages/chipmate-vscode/src"),
  path.join(ROOT, "packages/chipmate-vscode/webview-ui"),
...
const EXCLUDE_PATTERNS = [
  ...
  /^https?:\/\/chipmate\.internal/,
  /^https?:\/\/dev\.chipmate\.ai/,
  ...
  /^https?:\/\/api\.chipmate\.ai\/api\//,
  /^https?:\/\/supermassive-black-hole\.chipmateapps\.io\/v1\/session-export\//, // chipmate_change
  /^https?:\/\/ingest\.chipmatesessions\.ai/,
  /^https?:\/\/api\.openai\.com/,
  /^https?:\/\/api\.github\.com/,
  /^https?:\/\/api\.githubcopilot\.com/,
  /^https?:\/\/[^/]+\.openai\.azure\.com\/openai/, // chipmate_change
...
const SKIP_FILES = ["check-forbidden-strings.ts"] // chipmate_change
...
function shouldSkipFile(filepath: string): boolean {
  if (filepath === "packages/opencode/src/cli/cmd/account.ts") return true // chipmate_change - command is not registered in ChipMate
  ...
  if (parts.includes("i18n") && path.basename(filepath) !== "en.ts") return true // chipmate_change
...
if (check) {
  const committed = await Bun.file(OUTPUT)
    ...
    .catch(() => "")
  if (committed === output) {
    console.log("packages/chipmate-docs/source-links.md is up to date.")
    process.exit(0)
  }
  console.error(
    [
      "ERROR: packages/chipmate-docs/source-links.md is out of date.",
      "",
...
console.log(`Wrote ${sorted.length} unique URLs to packages/chipmate-docs/source-links.md`)
