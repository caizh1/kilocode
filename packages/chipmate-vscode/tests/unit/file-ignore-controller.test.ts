...
async function createTempWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-ignore-"))
  tempDirs.push(dir)
  return dir
}
...
describe("FileIgnoreController", () => {
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross =
        "c:/Users/User/AppData/Roaming/Code/User/globalStorage/chipmate.chipmate-code/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/chipmate.chipmate-code/settings/mcp_settings.json"
