...
internal object EditorContextGatherer {
    private val LOG = ChipMateLog.create(EditorContextGatherer::class.java)
    ...
    fun gather(project: Project, root: String): Result {
        if (!ChipMatePluginSettings.getAutoEditorContext()) {
        ...
        val ignore = project.service<ChipMateIgnoreCache>().matcher(rootDir(listOfNotNull(activeFile) + openFiles, base))
    ...
    private fun selection(editor: Editor, root: Path, ignore: ChipMateIgnore): PromptPartDto? {
    ...
    private fun local(file: VirtualFile, root: Path): Path? {
        if (file.fileSystem.protocol == ChipMateVirtualFileSystem.PROTOCOL) return null
