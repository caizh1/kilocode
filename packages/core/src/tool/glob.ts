...
const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const fs = yield* FSUtil.Service // chipmate_change
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              ...
              return yield* ripgrep
                .glob({
                  cwd: target.path, // chipmate_change
...
export const node = makeLocationNode({
  name: "tool/glob",
  layer,
  deps: [ToolRegistry.node, Ripgrep.node, Location.node, PermissionV2.node, FSUtil.node, Reference.node], // chipmate_change - search targets and references
})
