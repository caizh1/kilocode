...
describe("util.error", () => {
  ...
  // chipmate_change start - preserve ChipMate-branded CLI guidance
  test("uses ChipMate commands and capability messaging", () => {
    const model = cliErrorMessage({
      _tag: "ProviderModelNotFoundError",
      providerID: "anthropic",
      modelID: "claude-sonnet-4",
    })
    expect(model).toContain("chipmate models")
    expect(model).toContain("chipmate.json")
