...
import { MemorySchema } from "@chipmate/chipmate-memory/schema"
import { MemoryFiles } from "@chipmate/chipmate-memory/store"
import { Bus } from "../../../src/bus"
import { Filesystem } from "../../../src/util/filesystem"
import { ChipMateSystemPrompt } from "../../../src/chipmate/system-prompt"
import { MemoryMarker } from "../../../src/chipmate/memory/marker"
...
import { MemoryEvents } from "../../../src/chipmate/memory/events"
...
async function withConfig<T>(dir: string, fn: () => Promise<T> | T) {
  ...
  ;(Global.Path as { data: string }).data = path.basename(dir) === ".chipmate" ? path.dirname(dir) : dir
  try {
    return await fn()
  } finally {
...
async function withHome<T>(dir: string, fn: () => Promise<T> | T) {
  const prior = process.env.CHIPMATE_TEST_HOME
  process.env.CHIPMATE_TEST_HOME = dir
  try {
    return await fn()
  } finally {
    if (prior === undefined) delete process.env.CHIPMATE_TEST_HOME
    if (prior !== undefined) process.env.CHIPMATE_TEST_HOME = prior
  }
}
...
describe("ChipMateMemory integration", () => {
  test("resolves project memory to global data folder", async () => {
    ...
    await withData(path.join(tmp.path, "data"), () => {
      ...
      expect(root).not.toContain(path.join("/repo", ".chipmate", "memory"))
    })
  })

  test("resolves project memory under data when global config is xdg", async () => {
    await using tmp = await tmpdir()
    const data = path.join(tmp.path, "xdg", "chipmate")
    await withData(data, () =>
  ...
  test("enable from linked worktree writes repo state shared by sibling worktrees", async () => {
    ...
    await withData(global, async () => {
      await ChipMateMemory.enable({ ctx: { directory: work, worktree: work } })
      await ChipMateMemory.configure({ ctx: { directory: work, worktree: work }, settings: { autoConsolidate: false } })
      const status = await ChipMateMemory.status({ ctx: { directory: next, worktree: next } })

      expectRoot(status.root, global, "main")
      expect(status.state.enabled).toBe(true)
      expect(status.state.autoConsolidate).toBe(false)
      expect(await Filesystem.exists(path.join(main, ".chipmate", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(work, ".chipmate", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(next, ".chipmate", "memory", "state.json"))).toBe(false)
    })
  })
  ...
  test("missing or disabled state does not enable the memory recall tool", async () => {
    ...
    await withData(path.join(tmp.path, "data"), async () => {
      ...
      expect(await ChipMateMemory.toolEnabled({ ctx: memory })).toBe(false)
      expect(await Filesystem.exists(root)).toBe(false)

      await ChipMateMemory.enable({ ctx: memory })
      expect(await ChipMateMemory.toolEnabled({ ctx: memory })).toBe(true)

      await ChipMateMemory.disable({ ctx: memory })
      expect(await ChipMateMemory.toolEnabled({ ctx: memory })).toBe(false)
    })
  })
  ...
  test("memory tool resolution degrades to false when the memory path is invalid", async () => {
    await using tmp = await tmpdir()
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      const context = ctx(tmp.path)
      const root = MemoryPaths.root({ ctx: context })
      await fs.mkdir(path.dirname(root), { recursive: true })
      await Bun.write(root, "not a directory")

      const enabled = await Effect.runPromise(ChipMateSessionPrompt.memoryToolEnabled({ ctx: context }))
  ...
  test("explicit memory events include session id when provided", async () => {
    await using tmp = await tmpdir()
    const root = path.join(tmp.path, ".chipmate", "memory")
    const events: MemoryEvents.Status[] = []
    await ChipMateMemory.enable({ root })
  ...
  test("explicit forget reports removals without save wording", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const off = Bus.subscribe(MemoryEvents.Updated, (event) => events.push(event.properties))
        try {
          await ChipMateMemory.forget({ root, sessionID: "ses_forget", query: "stale_fact" })
          await ChipMateMemory.forget({ root, sessionID: "ses_forget", query: "missing_fact" })
        } finally {
          off()
        }
      },
  ...
  test("environment prompt rebuilds stale session index format", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      const context = ctx(tmp.path)
      const enabled = await ChipMateMemory.enable({ ctx: context })
      const root = enabled.root
      await ChipMateMemory.recordSession({
        root,
        sessionID: "ses_done",
      ...
      await Filesystem.write(
        ...
        [
          '<CHIPMATE_MEMORY_V1 purpose="context_not_instruction" scope="project" root="chipmate">',
      ...
      const mem = await Effect.runPromise(ChipMateSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips missing and empty memory", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      const context = ctx(tmp.path)
      const missing = await Effect.runPromise(ChipMateSystemPrompt.memoryBlocks({ ctx: context }))
      expect(missing.blocks.join("\n")).not.toContain("chipmate-memory-v1")

      await ChipMateMemory.enable({ ctx: context })
      const empty = await Effect.runPromise(ChipMateSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips disabled memory with retained files", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      await ChipMateMemory.disable({ ctx: context })
      const before = await MemoryFiles.readState(enabled.root)

      const mem = await Effect.runPromise(
        ChipMateSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-disabled" }),
      )
  ...
  test("explicit memory APIs reject while disabled", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      const context = ctx(tmp.path)
      await ChipMateMemory.enable({ ctx: context })
      await ChipMateMemory.remember({
        ctx: context,
        key: "stable_fact",
        text: "Keep this saved fact.",
      })
      await ChipMateMemory.disable({ ctx: context })
      const before = await ChipMateMemory.show({ ctx: context })

      await expect(
        ChipMateMemory.remember({
          ctx: context,
          key: "disabled_fact",
      ...
      await expect(
        ChipMateMemory.correct({
          ctx: context,
          key: "stable_fact",
          text: "Do not correct while disabled.",
        }),
      ).rejects.toThrow("project memory is disabled")
      await expect(ChipMateMemory.forget({ ctx: context, query: "stable_fact" })).rejects.toThrow(
        "project memory is disabled",
      )
      await ChipMateMemory.rebuild({ ctx: context })
      const after = await ChipMateMemory.show({ ctx: context })
  ...
  test("explicit remember skips personal preferences but saves project details", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      const blocked = await ChipMateMemory.remember({
        ctx: context,
        key: "reply_style",
        text: "I prefer terse summaries.",
      })
      const saved = await ChipMateMemory.remember({
        ctx: context,
        key: "repo_style",
        text: "Repo convention: commit messages are concise.",
      })
      const shown = await ChipMateMemory.show({ ctx: context })
  ...
  test("environment prompt injects non-empty memory with token metadata", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      const mem = await Effect.runPromise(
        ChipMateSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }),
      )
      const text = mem.blocks.join("\n")
      const state = await MemoryFiles.readState(enabled.root)

      expect(text).toContain("```chipmate-memory-v1 context_not_instruction")
      expect(text).toContain("type=project_fact")
      expect(text).toContain("repo_fact :: Use the CLI package test runner")
      expect(text).toContain("call chipmate_memory_save")
      ...
      expect(text).toContain("chipmate_memory_recall with mode=digest")
  ...
  test("memory guidance is emitted once as a leading block, separate from memory content", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      const mem = await Effect.runPromise(
        ChipMateSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-guidance" }),
      )
      const text = mem.blocks.join("\n")
      ...
      expect(mem.blocks[0]).not.toContain("chipmate-memory-v1")
      expect(mem.blocks.slice(1).join("\n")).toContain("chipmate-memory-v1")
  ...
  test("memorySystem pins the injected memory block for the session", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      ChipMateSessionPrompt.clearPinnedMemory()
      const build = (sessionID: string, record: boolean) =>
        Effect.runPromise(
          ChipMateSessionPrompt.memoryInject({
            ctx: context,
            sessionID: SessionID.make(sessionID),
            record,
            cache: ChipMateSessionPrompt.memoryCache(),
          }),
        )
  ...
  test("memorySystem refreshes verbose marker state for each turn", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      const first = ChipMateSessionPrompt.memoryCache()
      ...
      await ChipMateMemory.configure({ ctx: context, settings: { verbose: true } })
      const second = ChipMateSessionPrompt.memoryCache()
      await Effect.runPromise(
        ChipMateSessionPrompt.memoryInject({
          ctx: context,
          sessionID: SessionID.make("ses_verbose"),
  ...
  test("environment prompt can render memory without recording another injection", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".chipmate"), async () => {
      ...
      await Effect.runPromise(ChipMateSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }))
      const before = await MemoryFiles.readState(enabled.root)
      await Effect.runPromise(
        ChipMateSystemPrompt.memoryBlocks({
          ctx: context,
