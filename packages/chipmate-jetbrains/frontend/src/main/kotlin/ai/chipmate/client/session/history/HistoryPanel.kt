package ai.chipmate.client.session.history
...
class HistoryPanel(
    ...
    private val repoOnly = JBCheckBox(ChipMateBundle.message("history.cloud.repo.only"), true).apply {
        isVisible = false
    ...
    private val localInfo = TabInfo(localPanel)
        .setText(ChipMateBundle.message("history.tab.local"))
        .setForeSideComponent(back())
    private val cloudInfo = TabInfo(cloudPanel)
        .setText(ChipMateBundle.message("history.tab.cloud"))
        .setForeSideComponent(back())
    ...
    private fun search(model: HistoryModel<out HistoryItem>) = SearchTextField(false).apply {
        textEditor.emptyText.text = ChipMateBundle.message("history.search.placeholder")
    ...
    private fun back(): JComponent {
        val label = ChipMateBundle.message("history.back")
    ...
    private fun localList() = HistoryList(controller.local).apply {
        ...
        cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)
        emptyText.text = ChipMateBundle.message("history.empty")
    ...
    private fun <T : HistoryItem> syncList(list: JBList<T>, model: HistoryModel<T>) {
        ...
        list.emptyText.text = when {
            model.loading -> ChipMateBundle.message("history.loading")
            model.error != null -> model.error.orEmpty()
            else -> ChipMateBundle.message("history.empty")
        }
    }
    ...
    private fun installContextMenu(list: JBList<out HistoryItem>) {
        val group = ActionManager.getInstance().getAction("ChipMate.History.ContextMenu")
    ...
    private fun confirm(item: LocalHistoryItem) {
        ...
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            ChipMateBundle.message("history.delete.confirm.message", title(item)),
            ChipMateBundle.message("history.delete.confirm.title"),
    ...
    internal fun confirmDelete(items: List<LocalHistoryItem>) {
        ...
        val msg = if (active.size == 1)
            ChipMateBundle.message("history.delete.confirm.message", title(active[0]))
        else
            ChipMateBundle.message("history.delete.confirm.message.multiple", active.size)
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            msg,
            ChipMateBundle.message("history.delete.confirm.title"),
    ...
    private class LoadMoreButton : JButton(ChipMateBundle.message("history.cloud.load.more")) {
