...
import { CloudRepository } from "../../../src/chipmate/cloud/repository"
...
describe("CloudRepository", () => {
  it.live("resolves and validates an explicit repository branch outside Git", () =>
    Effect.gen(function* () {
      ...
      const error = yield* CloudRepository.resolve({ cwd, repo: "chipmate-org/chipmate", branch: "feature.lock" }).pipe(
        Effect.flip,
      )
      ...
      const type = yield* CloudRepository.resolve({
        cwd,
        repo: "https://github.com/chipmate-org/chipmate.git",
        type: "gitlab",
      }).pipe(Effect.flip)
  ...
  it.live("rejects GitHub repositories that become dot segments after trimming .git", () =>
    Effect.gen(function* () {
      ...
      for (const repo of ["https://github.com/chipmate-org/...git", "git@github.com:chipmate-org/...git"]) {
  ...
  it.live("normalizes an inferred GitHub SCP remote without adding the current branch", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "origin", "git@github.com:chipmate-org/ssh-repo.git")
      yield* run(cwd, "checkout", "-b", "feature/not-in-output")

      const result = yield* CloudRepository.resolve({ cwd })

      expect(result).toEqual({ type: "github", repo: "chipmate-org/ssh-repo" })
    }),
  )
  ...
  it.live("rejects inferred local remotes that resemble GitHub shorthand", () =>
    Effect.gen(function* () {
      const cwd = yield* tmpdirScoped({ git: true })
      yield* run(cwd, "remote", "add", "origin", "chipmate-org/local-repo")
  ...
  it.live("prefers the tracking remote, then origin, then a sole remote fetch URL", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "origin", "https://github.com/chipmate-org/origin.git")
      yield* run(cwd, "remote", "add", "tracked", "https://github.com/chipmate-org/tracked.git")
      yield* run(cwd, "remote", "set-url", "--add", "--push", "tracked", "https://github.com/chipmate-org/push.git")
      yield* run(cwd, "config", `branch.${branch}.remote`, "tracked")

      expect(yield* CloudRepository.resolve({ cwd })).toEqual({ type: "github", repo: "chipmate-org/tracked" })

      yield* run(cwd, "config", `branch.${branch}.remote`, "missing")
      expect(yield* CloudRepository.resolve({ cwd })).toEqual({ type: "github", repo: "chipmate-org/origin" })
  ...
  it.live("returns typed errors when no remote can be selected", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "alpha", "https://github.com/chipmate-org/alpha.git")
      yield* run(cwd, "remote", "add", "beta", "https://github.com/chipmate-org/beta.git")
