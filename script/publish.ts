...
// chipmate_change start - keep JetBrains CLI pin reviewable outside CLI release commits
const jetbrainsPkg = fileURLToPath(new URL("../packages/chipmate-jetbrains/package.json", import.meta.url))
const jetbrainsPin = await Bun.file(jetbrainsPkg).text()
// chipmate_change end

// chipmate_change start - consume changesets on the publish runner so changelog
...
{
  await $`bun install`
  const paths = ["packages/chipmate-vscode/CHANGELOG.md", "packages/opencode/CHANGELOG.md"]
...
for (const file of pkgjsons) {
  // chipmate_change start - create a follow-up PR for JetBrains CLI pin bumps
...
if (Script.release) {
  // chipmate_change start - commit, tag, and push with rebase + retry to handle
  ...
  // chipmate_change start - publish channel-aware GitHub release notes
  const { publishNotes } = await import("./chipmate/release-notes")
  await publishNotes({
...
// chipmate_change - ChipMate does not ship the upstream preview CLI package
...
// chipmate_change - ChipMate does not publish the upstream-owned @opencode-ai/ui package

// chipmate_change start
console.log("\n=== vscode ===\n")
await import(`../packages/chipmate-vscode/script/publish.ts`)
// chipmate_change end

// chipmate_change start - ChipMate does not ship the opencode desktop app
// if (Script.release) {
...
// chipmate_change start - non-blocking JetBrains CLI pin bump PR after stable CLI release
...
async function createJetbrainsPinPr() {
  ...
  const result =
    await $`bun .chipmate/skills/release-jetbrains/script/set-pin.ts --version ${Script.version} --pr`.nothrow()
