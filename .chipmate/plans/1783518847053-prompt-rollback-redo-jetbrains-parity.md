...
- CLI already exposes everything needed; no CLI release or SDK regen required. Pinned CLI version is `7.4.1` (`packages/chipmate-jetbrains/package.json`).
...
- JetBrains split-mode packages differ from AGENTS.md's stated `ai.chipmate.jetbrains`; actual source roots are:
  - shared → `packages/chipmate-jetbrains/shared/src/main/kotlin/ai/chipmate/rpc/`
  - backend → `packages/chipmate-jetbrains/backend/src/main/kotlin/ai/chipmate/backend/`
  - frontend → `packages/chipmate-jetbrains/frontend/src/main/kotlin/ai/chipmate/client/`
- All target files confirmed to exist: `SessionDto.kt`, `ChipMateSessionRpcApi.kt`, `ChipMateBackendChatManager.kt`, `ChipMateSessionRpcApiImpl.kt`, `ChipMateSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt`, `SessionView.kt`, `MessageView.kt`, `MessageToolbar.kt`, `PromptPanel.kt`, `PromptEditorTextField.kt`, plus reuse targets `DiffStatBadge.kt`, `HoverIcon.kt`, `PermissionDiffView.kt`.
...
File: `packages/chipmate-vscode/webview-ui/src/i18n/en.ts:138`
...
- In the 19 non-English locale files under `packages/chipmate-vscode/webview-ui/src/i18n/` (ar, br, bs, da, de, es, fr, it, ja, ko, nl, no, pl, ru, th, tr, uk, zh, zht), overwrite `revert.banner.hint` with the identical new English string; leave a note that these need re-translation.
...
- `shared/.../rpc/ChipMateSessionRpcApi.kt`: add
...
- `backend/.../app/ChipMateBackendChatManager.kt`: add `revert(id, dir, messageID, partID?)` → `POST /session/$id/revert?directory=…` with `{messageID, partID?}` body, and `unrevert(id, dir)` → `POST /session/$id/unrevert?directory=…`. Mirror the existing `compact()` OkHttp pattern; add JSON body builders to `ChipMateCliDataParser`.
- `backend/.../rpc/ChipMateSessionRpcApiImpl.kt`: implement both, resolve worktree dir via `sessions.getDirectory(id, directory)`, wrap in `ready { }` like `compact`/`abort`.

### 3. Frontend service

- `frontend/.../app/ChipMateSessionService.kt`: add `suspend revert(...)` / `unrevert(...)` wrappers over the RPC, using `durable {}` per split-mode rules.
...
- New `session/ui/RevertBanner.kt` Swing view, built once and mutated via `update()` per retained-Swing rules. Listens to `RevertChanged` + `DiffUpdated`. Contents:
  ...
  - per-file diff rows reusing `ai.chipmate.client.ui.DiffStatBadge` fed from `model.diff` (`DiffFileDto.additions/deletions`),
...
### 7. i18n (`frontend/src/main/resources/messages/ChipMateBundle*.properties`)

- Add to base `ChipMateBundle.properties`: `revert.banner.count.one`, `revert.banner.count.other`, `revert.banner.redo`, `revert.banner.redo.all`, `revert.banner.hint` (new wording), `revert.disabled.agentBusy`.
- Other locale bundles fall back to base until translated; do not add stale translations.

### 8. Tests

- Backend: extend `ChipMateSessionRpcApiImplTest` / a `ChipMateBackendChatManager` test against `MockCliServer` asserting the exact HTTP path + JSON body for `revert`/`unrevert`, and that a session carrying `revert` deserializes into `SessionRevertDto`.
...
| Shared | `SessionDto.kt` (+`SessionRevertDto`), `ChipMateSessionRpcApi.kt` |
| Backend | `ChipMateBackendChatManager.kt`, `ChipMateCliDataParser`, `ChipMateSessionRpcApiImpl.kt` |
| Frontend service/controller/model | `ChipMateSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt` |
| Frontend views | new `RevertBanner.kt` (reusing `DiffStatBadge`), `MessageView.kt`, `MessageToolbar.kt`, `SessionView.kt`, `PromptPanel.kt` |
| i18n | `ChipMateBundle.properties` |
| Tests | backend RPC/chat-manager, frontend controller + view tests, `FakeSessionRpcApi` |

## Validation

- VS Code string change: `bun run typecheck` from repo root (or from `packages/chipmate-vscode/`).
- JetBrains: `./gradlew typecheck` and `./gradlew test` from `packages/chipmate-jetbrains/` (Java 21).
- Manual: `./gradlew runIde` (or split mode) to exercise rollback → redo → redo-all and reverted-transcript hiding.

## Risks / Notes

- Confirm no `chipmate_change` markers are needed for the VS Code i18n edits (ChipMate-owned webview path — none required).
