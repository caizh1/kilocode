...
/**
 * Fake [ChipMateAppRpcApi] for testing.
 *
 * Push state changes via [state]. Health check returns [health].
 *
 * Every `suspend` method asserts it is NOT called on the EDT.
 */
class FakeAppRpcApi : ChipMateAppRpcApi {

    val state = MutableStateFlow(ChipMateAppStateDto(ChipMateAppStatusDto.DISCONNECTED))
    ...
    var fakeDeviceAuth = DeviceAuthDto(code = "TEST-1234", verificationUrl = "https://auth.chipmate.ai/device")
