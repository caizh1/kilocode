 ...
 * bot's rolling pull request, and writes them into packages/chipmate-docs/LEARNINGS.md
...
const LEARNINGS_FILE = "packages/chipmate-docs/LEARNINGS.md"
 ...
 * `chipmate run` prints the assistant message twice; the last copy wins.
...
export function validateDelta(delta, { existing, candidateSources, deletedInWindow }) {
  ...
  for (const a of add) {
    ...
    if (String(a.rule).match(/#\d{2,}|https?:\/\/|@[A-Za-z0-9-]|packages\/chipmate-docs|\.md\b/)) {
...
async function extract() {
  ...
  const { api, repo, searchIssues, appendOutput, appendSummary, backoffMsForAttempt, runChipMate, sleepSync } =
  ...
  for (const line of rawCommits) {
    ...
    try {
      ...
      docDiff = git(["show", "--format=", sha, "--", "packages/chipmate-docs"])
      // Cap diff sizes.
