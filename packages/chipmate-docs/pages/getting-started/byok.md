...
description: "Use your own API keys with ChipMate Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the ChipMate Gateway, while retaining ChipMate platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, ChipMate Gateway supports most new models in minutes
...
ChipMate Gateway supports BYOK keys for these providers.
...
These providers offer coding-focused subscriptions or dedicated endpoints. Bring the API key issued by your plan to use its included models through the ChipMate Gateway:
...
1. Log into the ChipMate platform and select the account or organization you want to add the BYOK key to.
...
- When you use the **ChipMate Gateway** provider, ChipMate checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using ChipMate's keys.
...
- BYOK works with the ChipMate Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
- ChipMate Gateway models that can use one of your enabled personal or organization BYOK providers display a `BYOK` badge in the model picker. The badge does not apply to models selected through other providers.
