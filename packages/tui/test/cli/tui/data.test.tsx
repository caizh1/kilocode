...
import type { Event, GlobalEvent } from "@chipmate/sdk/v2"
import { createSignal, onMount, Show, type ParentProps } from "solid-js" // chipmate_change
import { ProjectProvider, useProject } from "../../../src/context/project" // chipmate_change
...
// chipmate_change start - initialize ChipMate's project filter before mounting V2 event consumers
...
test("refreshes effective catalog data after catalog updates", async () => {
  ...
  const app = await testRender(() => (
    <TestTuiContexts>
      <SDKProvider url="http://test" directory={directory} events={events.source} fetch={calls.fetch}>
        <ProjectProvider>
          {/* chipmate_change start - initialize ChipMate's project filter before consuming catalog events */}
          <Ready>
            <DataProvider>
              <box />
            </DataProvider>
...
// chipmate_change start - retain promoted prompt compatibility used by ChipMate sync
...
// chipmate_change start - preserve ChipMate's V2 hydration-race coverage after the DataProvider extraction
