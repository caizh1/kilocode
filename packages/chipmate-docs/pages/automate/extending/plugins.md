---
title: "Plugins"
description: "Extend the ChipMate CLI with custom hooks, tools, auth providers, and more"
platform: new
---

# Plugins

Plugins extend ChipMate by hooking into events, adding custom tools, registering auth or model providers, and customizing runtime behavior. They are TypeScript or JavaScript modules loaded at startup, and work in both the ChipMate CLI and the VS Code extension.
...
- Global: `~/.config/chipmate/plugin/`
- Project: `.chipmate/plugin/` or legacy `.chipmate/plugin/`
...
### From the `chipmate plugin` command

Install an npm plugin and patch your config in one step:

```bash
# Install into the current project's config
chipmate plugin my-plugin

# Install into your global config
chipmate plugin my-plugin --global

# Replace an existing entry
chipmate plugin my-plugin --force
```

The command resolves the package, reads its `package.json` for plugin entrypoints, and writes the entry into the appropriate config file (`.chipmate/opencode.jsonc` / `.chipmate/tui.jsonc` for local installs, or `~/.config/chipmate/opencode.jsonc` / `~/.config/chipmate/tui.jsonc` for `--global`) while preserving JSONC comments.
...
- **Install scripts are disabled** for npm plugins. ChipMate installs packages with lifecycle scripts such as `install` and `postinstall` blocked.
- **Local plugins** are loaded directly from the plugin directory. If your plugin imports external packages, add a `package.json` to your config directory (see [Dependencies](#dependencies)) — ChipMate runs `bun install` on startup so imports resolve.

### Load order

Plugins from all sources run on every session. They load in this order:

1. Internal built-ins (ChipMate Gateway auth, Codex auth, Copilot auth, Cloudflare, etc.)
2. Global config plugin array (`~/.config/chipmate/chipmate.json`)
3. Global plugin directory (`~/.config/chipmate/plugin/`)
4. Project config plugin array (`chipmate.json` / `opencode.json`)
5. Project plugin directory (`.chipmate/plugin/` and friends)
...
Set the `CHIPMATE_PURE=1` environment variable to skip all external plugins — only built-in plugins will load. Useful for reproducible CI runs or debugging.
...
| `client` | A ChipMate SDK client (`@chipmate/sdk`) for calling the local server. |
| `$` | [Bun's shell API](https://bun.com/docs/runtime/shell). |
| `serverUrl` | URL of the local ChipMate server. |
...
Workspace adaptors let plugins add custom workspace targets to ChipMate's workspace creation flow. This API is experimental and may change.
...
const WorkspacePlugin: Plugin = async ({ experimental_workspace }) => {
  experimental_workspace.register("folder", {
    ...
    configure(config) {
      return { ...config, directory: `/tmp/chipmate-${Date.now()}` }
    },
    async create(config) {
...
Published npm plugins should declare separate package entrypoints for each runtime they support. ChipMate detects install targets from `package.json`:
...
```bash
bun add -d @chipmate/plugin
```

```ts
import type { Plugin } from "@chipmate/plugin"
import { tool } from "@chipmate/plugin/tool"
```

ChipMate automatically creates a `package.json` in config directories that contain a `plugin/` folder and installs `@chipmate/plugin` so types resolve out of the box.
...
ChipMate runs `bun install` at startup so your plugins can import the packages:

```ts
// .chipmate/plugin/escape-bash.ts
...
| `shell.env` | Inject environment variables into every shell command ChipMate runs. |
...
ChipMate fills provider/model IDs from the returned catalog and uses the returned models in the picker and provider router.
...
The `event` hook fires for every event on ChipMate's internal bus. Common event types include:
...
For tools that don't need the full plugin context, drop them in a `tool/` or `tools/` folder inside any config directory — for example `.chipmate/tool/database.ts` or `~/.config/chipmate/tool/database.ts`. The filename becomes the tool name, and each file exports a `tool()` definition directly. The layout is identical to the [OpenCode custom tools guide](https://opencode.ai/docs/custom-tools); use `.chipmate/`, or legacy `.chipmate/`, instead of `.opencode/`.
...
Prefer `client.app.log()` over `console.log` so entries land in ChipMate's log pipeline:
...
By default, ChipMate sends a synthetic "continue" turn after compaction so the agent resumes the interrupted task. Use `experimental.compaction.autocontinue` to disable that turn for specific sessions or providers:
...
Plugins can also target the ChipMate TUI itself — registering slash commands, routes, slots, dialogs, and keybinds. TUI plugins are SolidJS modules exported from `"./tui"` in your plugin package, or theme-only packages declared with `oc-themes`.

TUI plugins live in a separate module namespace (`@chipmate/plugin/tui`) and have their own API surface (`TuiPluginApi`). Because the TUI API is larger and still evolving, this guide doesn't cover it exhaustively — use the types in `@chipmate/plugin/tui` as the reference, and look at the built-in TUI plugins under `packages/opencode/src/cli/cmd/tui/feature-plugins/` for working examples.
...
- **Plugin failed to load** — check the CLI logs with `chipmate --print-logs --log-level DEBUG`. Load failures are also surfaced as session errors in the TUI and VS Code extension.
...
- **Plugin loads in dev but not in CI** — verify `CHIPMATE_PURE` is not set, and that npm-installed plugins are cached under `packages/` in the current CLI XDG cache directory (`~/.cache/opencode/packages/` by default, or `$XDG_CACHE_HOME/opencode/packages/` when `XDG_CACHE_HOME` is set). Run with `--log-level DEBUG` to see install output.
- **Reset the plugin cache** — delete the plugin package folder under the CLI's `packages/` cache directory (or the `node_modules` cache under your config directory) and restart ChipMate.

---

## Reference

- Types: [`@chipmate/plugin`](https://github.com/ChipMate-Org/chipmate/tree/main/packages/plugin) — `Plugin`, `Hooks`, `PluginInput`, `ToolDefinition`, `AuthHook`, `ProviderHook`.
- Example plugin: [`packages/plugin/src/example.ts`](https://github.com/ChipMate-Org/chipmate/blob/main/packages/plugin/src/example.ts)
- CLI command: [`chipmate plugin`](/docs/code-with-ai/platforms/cli-reference#chipmate-plugin)
