...
import { fetchProfile } from "@chipmate/chipmate-gateway"
...
export namespace Identity {
  ...
  export async function getMachineId(): Promise<string | undefined> {
    ...
    const override = process.env.CHIPMATE_MACHINE_ID
    if (override) {
      machineId = override
      return machineId
    }
  ...
  async function write(cache: Cache): Promise<void> {
    ...
    try {
      ...
      await rm(tmp, { force: true }).catch((rmErr) => {
        if (process.env.CHIPMATE_PRINT_LOGS) console.warn("telemetry profile cache temp cleanup failed", rmErr)
      })
      if (process.env.CHIPMATE_PRINT_LOGS) console.warn("telemetry profile cache write failed", err)
    }
  }
  ...
  export async function updateFromChipMateAuth(token: string | null, accountId?: string): Promise<void> {
    ...
    if (cached && cached.token === tokenHash) {
      ...
      if (Date.now() - cached.fetchedAt > CACHE_TTL) {
        refresh(token, tokenHash).catch((err) => {
          if (process.env.CHIPMATE_PRINT_LOGS) console.warn("telemetry profile refresh failed", err)
        })
      }
      return
    }
