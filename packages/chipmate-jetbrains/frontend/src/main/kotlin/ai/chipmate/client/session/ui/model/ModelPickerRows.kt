...
internal fun modelPickerRows(
    ...
    emptyText: String = ChipMateBundle.message("settings.models.notSet"),
    ...
    if (q.isBlank()) {
        ...
        if (fav.isNotEmpty()) {
            val section = ChipMateBundle.message("model.picker.favorites")
            out += fav.map { ModelPickerRow(it, section, favorite = true) }
        }
    }
    if (recommended.isNotEmpty()) {
        val section = ChipMateBundle.message("model.picker.recommended")
