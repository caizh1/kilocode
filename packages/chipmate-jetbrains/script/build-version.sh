...
if grep -Eq '^[[:space:]]*chipmate\.cli\.pinned[[:space:]]*=[[:space:]]*false[[:space:]]*$' "$plugin/gradle.properties"; then
  echo "chipmate.cli.pinned=false is a dev-only mode and cannot be released. Set chipmate.cli.pinned=true before building a version." >&2
  exit 1
fi
...
./gradlew buildPlugin -Pproduction=true -Pchipmate.version="$version" -Pchipmate.channel=eap

if [[ "$sign" == "1" ]]; then
  ./gradlew signPlugin -Pproduction=true -Pchipmate.version="$version" -Pchipmate.channel=eap
fi
...
if [[ "$skip_verification" == "1" ]]; then
  ...
  if [[ "$sign" == "1" ]]; then
    ./gradlew verifyPluginSignature -Pproduction=true -Pchipmate.version="$version" -Pchipmate.channel=eap
  fi
  ./gradlew verifyPlugin -Pproduction=true -Pchipmate.version="$version" -Pchipmate.channel=eap
fi
