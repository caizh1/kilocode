...
description: "How to provide context to ChipMate using mentions"
---

# Context Mentions

Providing the right context helps ChipMate understand your project and perform tasks accurately. All platforms support `@`-mentions for referencing files, and the agent can also discover context on its own using built-in tools like `read`, `grep`, and `glob`.
...
Selected code and editor diagnostics (errors/warnings) are not included automatically. However, you can send these to ChipMate through VS Code's Code Actions: select code or hover over an error, then use the lightbulb menu to find context-dependent actions like "Explain with ChipMate" or "Fix with ChipMate."
...
- **Reference a past chat** — Type `@`, choose **Past chats**, then search for and select a session. ChipMate adds that chat history as context when you send the message without switching sessions.
...
- **Use `chipmate run -f`** — When using the non-interactive `chipmate run` command, pass `-f path/to/file.ts` to explicitly include a file's contents in the context.
...
| **Use `chipmate run -f`** | Pass key files with `-f` when using `chipmate run` for immediate context |
| **Trust the agent's tools** | The agent can search, read, and explore your codebase — let it do the discovery work |
