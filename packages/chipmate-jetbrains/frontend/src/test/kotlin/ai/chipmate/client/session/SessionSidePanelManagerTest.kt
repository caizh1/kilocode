...
import ai.chipmate.client.session.history.HistoryController
import ai.chipmate.client.session.history.HistoryDataKeys
import ai.chipmate.client.session.history.HistoryPanel
...
import ai.chipmate.client.testing.TestUiTimers
...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        sessions = ChipMateSessionService(project, coroutines.scope, rpc)
        app = ChipMateAppService(coroutines.scope, FakeAppRpcApi().also {
            it.state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY)
        })
        workspaces = ChipMateWorkspaceService(coroutines.scope, FakeWorkspaceRpcApi().also {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.chipmate.client.session.ui.style.SessionEditorStyle.create(family = "Courier New", size = 24)
    ...
    fun `test history overlays update before hidden timeout`() {
        ...
        assertEquals(ChipMateBundle.message("history.badge.question"), history.badgeText(0))
    }
    ...
    private fun setInactiveDisposeTimeout(ms: Int) {
        val key = "chipmate.session.inactive.disposeTimeoutMs"
    ...
    private fun JPanel.controller(): ai.chipmate.client.session.controller.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.chipmate.client.session.controller.SessionController
    }
