...
import { buildChipMateHeaders, getFeatureHeader, getEditorNameHeader } from "@chipmate/chipmate-gateway"
import { HEADER_FEATURE, ENV_FEATURE, ENV_EDITOR_NAME, ENV_VERSION, DEFAULT_EDITOR_NAME } from "@chipmate/chipmate-gateway"
...
describe("getEditorNameHeader", () => {
  ...
  it("returns default editor name without version when CHIPMATE_VERSION is not set", () => {
    delete process.env[ENV_VERSION]
    expect(getEditorNameHeader()).toBe(DEFAULT_EDITOR_NAME)
  })

  it("appends version when CHIPMATE_VERSION is set", () => {
    process.env[ENV_VERSION] = "1.2.3"
    expect(getEditorNameHeader()).toBe(`${DEFAULT_EDITOR_NAME} 1.2.3`)
  })
})

describe("buildChipMateHeaders", () => {
  const original = process.env[ENV_FEATURE]
  ...
  it("includes feature header when env var is set", () => {
    ...
    const headers = buildChipMateHeaders()
  ...
  it("always includes editor name header", () => {
    delete process.env[ENV_FEATURE]
    const headers = buildChipMateHeaders()
    expect(headers["X-CHIPMATE-EDITORNAME"]).toBe(getEditorNameHeader())
  })
