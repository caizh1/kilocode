...
export function indexingBudget(
  ...
  const hard = limit(env.CHIPMATE_INDEXING_HARD_RSS_BYTES, adaptive)
  const soft = Math.min(hard, limit(env.CHIPMATE_INDEXING_SOFT_RSS_BYTES, Math.floor(hard * 0.75)))
  const critical = Math.max(soft, Math.min(hard, limit(env.CHIPMATE_INDEXING_CRITICAL_RSS_BYTES, Math.floor(hard * 0.875))))
  const recovery = Math.min(soft, limit(env.CHIPMATE_INDEXING_RECOVERY_RSS_BYTES, Math.floor(soft * 0.85)))
