...
import { ChipMateBootstrap } from "../../src/chipmate/bootstrap"
import { ChipMateWatcher } from "../../src/chipmate/watcher"
...
describe("chipmate tool registry indexing", () => {
  ...
  it.live("omits indexing tools without waiting for slow indexing startup", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const avail = spyOn(ChipMateIndexing, "available").mockImplementation(() => new Promise<boolean>(() => {}))
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const ready = spyOn(ChipMateIndexing, "ready").mockImplementation(() => {
            throw err
          })
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const ready = spyOn(ChipMateIndexing, "ready").mockImplementation(() => Promise.reject(err) as unknown as boolean)
          const warn = spyOn(logger, "warn").mockImplementation(() => {})
  ...
  it.live("registers indexing tools when indexing is ready", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ready = spyOn(ChipMateIndexing, "ready").mockReturnValue(true)
          const analysisReady = spyOn(ChipMateIndexing, "analysisReady").mockReturnValue(true)
          const documentReady = spyOn(ChipMateIndexing, "documentReady").mockReturnValue(true)
  ...
  it.live("omits semantic_search hint from glob and grep descriptions when indexing is not ready", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ready = spyOn(ChipMateIndexing, "ready").mockReturnValue(false)
  ...
  it.live("omits interactive_terminal from subagent definitions", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        ...
        process.env["CHIPMATE_CLIENT"] = "cli"
        return prev
      }),
      () =>
        provideTmpdirInstance(
      ...
      (prev) =>
        Effect.sync(() => {
          ...
          if (prev !== undefined) process.env["CHIPMATE_CLIENT"] = prev
        }),
    ),
  )
  ...
  test("enables semantic search from indexing configuration before the index is ready", () => {
    ...
    expect(ChipMateToolRegistry.indexing({}, { indexing: { enabled: true } })).toBe(true)
  })

  it.live("omits memory tools when project memory is disabled but keeps chipmate_local_recall", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const agent = yield* Agent.Service
          ...
          expect(ids).not.toContain("chipmate_memory_recall")
          expect(ids).not.toContain("chipmate_memory_save")
          // chipmate_local_recall is a transcript-recall tool gated by `recall: "ask"` in agent
          // permissions; it must NOT be coupled to project-memory enablement.
          expect(ids).toContain("chipmate_local_recall")
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const probe = spyOn(ChipMateMemory, "toolEnabled")

          try {
            const a = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })
            const b = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })
            const c = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })

            expect([a, b, c]).toEqual([false, false, false])
            // Cache hit: only the first call should reach ChipMateMemory.toolEnabled.
  ...
  it.live("memoryToolsEnabled reflects enable/disable immediately after invalidate", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const root = (yield* Effect.promise(() => ChipMateMemory.prepare({ ctx }))).toString()

          const first = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })
          expect(first).toBe(false)

          yield* Effect.promise(() => ChipMateMemory.enable({ ctx }))

          // The bootstrap MemoryEvents subscriber invalidates on mutation; call it directly here.
          ChipMateToolRegistry.invalidateMemoryEnabled(root)
          const afterEnable = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })
          expect(afterEnable).toBe(true)

          yield* Effect.promise(() => ChipMateMemory.disable({ ctx }))

          ChipMateToolRegistry.invalidateMemoryEnabled(root)
          const afterDisable = yield* ChipMateToolRegistry.memoryToolsEnabled({ ctx })
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          expect(ids).toContain("chipmate_memory_recall")
          expect(ids).toContain("chipmate_memory_save")
          expect(ids).toContain("chipmate_local_recall")
        }),
      { git: true },
    ),
  ...
  test("conditionally includes ChipMate registry extras", () => {
    const prev = process.env["CHIPMATE_CLIENT"]
    ...
    const tools = {
      ...
      memory: def("chipmate_memory_recall"),
      save: def("chipmate_memory_save"),
    ...
    try {
      process.env["CHIPMATE_CLIENT"] = "cli"
      expect(ChipMateToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "codebase_analysis",
        "semantic_search",
        "document_search",
        "declare_artifact",
      ...
      expect(ChipMateToolRegistry.extra(tools, { experimental: { codebase_search: true } }).map((tool) => tool.id)).toEqual(
        [
          "codebase_search",
          "codebase_analysis",
          "semantic_search",
      ...
      expect(
        ChipMateToolRegistry.extra(tools, { experimental: { codebase_search: true, image_generation: true } }).map(
          (tool) => tool.id,
        ),
      ).toEqual([
        "codebase_search",
      ...
      process.env["CHIPMATE_CLIENT"] = "vscode"
      expect(ChipMateToolRegistry.extra(tools, { experimental: { codebase_search: true } }).map((tool) => tool.id)).toEqual(
        [
          "codebase_search",
          "codebase_analysis",
      ...
      expect(
        ChipMateToolRegistry.extra(tools, {
      ...
      expect(ChipMateToolRegistry.extra({ ...tools, semantic: undefined }, {}).map((tool) => tool.id)).toEqual([
        "codebase_analysis",
        "document_search",
        "declare_artifact",
        "list_artifacts",
      ...
      process.env["CHIPMATE_CLIENT"] = "desktop"
      expect(ChipMateToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "codebase_analysis",
        "semantic_search",
        "document_search",
      ...
      process.env["CHIPMATE_CLIENT"] = "run"
      expect(ChipMateToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "codebase_analysis",
        "semantic_search",
        "document_search",
      ...
      process.env["CHIPMATE_CLIENT"] = "acp"
      expect(ChipMateToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "codebase_analysis",
        "semantic_search",
        "document_search",
      ...
      expect(
        ChipMateToolRegistry.extra({ ...tools, sourceBackedDesignJob: def("source_backed_design_job") }, {}).map(
          (tool) => tool.id,
        ),
      ).toContain("source_backed_design_job")
      expect(
        ChipMateToolRegistry.extra({ ...tools, excel: [def("create_excel_workbook")] }, {}).map((tool) => tool.id),
  ...
  test("preloads internal retrieval tools without consulting runtime readiness", async () => {
    ...
    const result = await Effect.runPromise(
      ChipMateToolRegistry.build(defs, deps, {
        indexing: async () => {
          calls.push("indexing")
  ...
  test("keeps public exact-search descriptions free of CodeGraph routing", () => {
    ...
    const result = ChipMateToolRegistry.describe(
      tools,
      {
        analysis: def("codebase_analysis"),
        semantic: def("semantic_search"),
  ...
  test("resolves internal retrieval visibility from fresh effective config on each model step", async () => {
    ...
    const publicTools = await Effect.runPromise(ChipMateToolRegistry.resolve(tools, source, false))
    expect(publicTools).toBe(tools)
    expect(state.reads).toBe(0)

    const enabled = await Effect.runPromise(ChipMateToolRegistry.resolve(tools, source, true))
    ...
    const disabled = await Effect.runPromise(ChipMateToolRegistry.resolve(tools, source, true))
    ...
    const missing = await Effect.runPromise(ChipMateToolRegistry.resolve(tools, source, true))
  ...
  test("logs indexing bootstrap failures without blocking session bootstrap", async () => {
    const platform = process.env["CHIPMATE_PLATFORM"]
    process.env["CHIPMATE_PLATFORM"] = "cli"
    const logger = Log.create({ service: "chipmate-bootstrap" })
    const err = new Error("indexing init failed")
    const calls: string[] = []
    const sessions = Layer.succeed(
      ChipMateSessions.Service,
      ChipMateSessions.Service.of({
    ...
    const watcher = Layer.succeed(ChipMateWatcher.Service, ChipMateWatcher.Service.of({ init: () => Effect.void }))
    const indexing = spyOn(ChipMateIndexing, "init").mockRejectedValue(err)
    const warn = spyOn(logger, "warn").mockImplementation(() => {})

    try {
      await Effect.runPromise(
        ChipMateBootstrap.Service.use((svc) => svc.init()).pipe(
          Effect.provide(
            ChipMateBootstrap.layer.pipe(Layer.provide([sessions, bus, memory, session, summary, provider, watcher])),
          ),
          Effect.scoped,
        ),
      )
      ...
      if (platform === undefined) delete process.env["CHIPMATE_PLATFORM"]
      else process.env["CHIPMATE_PLATFORM"] = platform
