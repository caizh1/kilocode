...
} from "../../src/chipmate-provider/agent-requirements"
import { AgentRequirementsController } from "../../src/chipmate-provider/agent-requirements-controller"
...
describe("agent requirement helpers", () => {
  ...
  it("scopes requests to active workspace, session, and worktree directories", () => {
    const sessions = new Map([["session-1", "/repo/.chipmate/worktrees/one"]])
    const base = {
      workspaceDirectory: root,
      workspaceDirectories: ["/workspace"],
      projectDirectory: root,
      sessionDirectories: sessions,
      worktreeDirectories: () => ["/repo/.chipmate/worktrees/two"],
    }

    expect(requirementDirectory({ ...base, requested: "/repo" })).toBe(root)
    expect(requirementDirectory({ ...base, requested: "/repo/.chipmate/worktrees/one", sessionID: "session-1" })).toBe(
      "/repo/.chipmate/worktrees/one",
    )
    expect(requirementDirectory({ ...base, requested: "/repo/.chipmate/worktrees/two" })).toBe("/repo/.chipmate/worktrees/two")
