...
All work is under `packages/chipmate-jetbrains/frontend/`. No backend/RPC/DTO changes. Files are
ChipMate-owned (no `chipmate_change` markers).
...
- **Location**: New package `ai/chipmate/client/ui/picker/` with `PickerPopup<T>` and
...
9. **Strings**: add a "Deselect All" key to `ChipMateBundle.properties` (e.g.
...
- `./gradlew typecheck`
- `./gradlew test` (iterate with `--tests ai.chipmate.client.session.ui.model.ModelPickerTest`
  and `--tests ai.chipmate.client.settings.providers.ProvidersSettingsUiTest`).
