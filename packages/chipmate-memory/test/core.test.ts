...
import { ChipMateMemory } from "../src/effect/index"

async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "chipmate-memory-"))
  return {
    dir,
...
describe("memory core package", () => {
  ...
  test("prepare removes legacy decisions once from owned memory roots", async () => {
    await use(async (t) => {
      ...
      await ChipMateMemory.status({ root: t.root })
      ...
      await ChipMateMemory.status({ root: other })
  ...
  test("prepare ignores legacy decisions cleanup failures", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        const status = await ChipMateMemory.status({ root: t.root })
  ...
  test("prepare ignores corrupt manifests during legacy cleanup", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        await writeFile(paths.manifest, '{"kind":"chipmate-memory","version":1}\n')
  ...
  test("uses unicode-safe project and memory identifiers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "設定_é",
        text: "日本語の設定は packages/chipmate-vscode に保存します。",
      })
      const shown = await Memory.show({ root: t.root })

      expect(id.display).toBe("proyecto_ñ_日本")
      expect(id.folder).toContain("proyecto_ñ_日本-")
      expect(shown.sources.project).toContain("- 設定_é :: 日本語の設定は packages/chipmate-vscode に保存します。")
  ...
  test("targeted recall dedupes lower-value session hits when typed memory answers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "memory_tests",
        text: "Run memory tests from packages/opencode with bun test ./test/chipmate/memory.",
      })
    ...
    await use(async (t) => {
      ...
      await Memory.apply({
        ...
        ops: [
          ...
          { action: "add", key: "設定", text: "日本語の設定は packages/chipmate-vscode に保存します。" },
        ],
      })
    ...
    await use(async (t) => {
      ...
      await MemoryFiles.writeSource(
        ...
        [
          ...
          "- vscode_unit_tests :: Run VS Code memory unit tests from packages/chipmate-vscode with bun test tests/unit/memory-command.test.ts.",
          ...Array.from(
            { length: 40 },
            (_, idx) =>
  ...
  test("index suppresses bulk session digests covered by typed memory", async () => {
    await use(async (t) => {
      ...
      // ...but a compact pointer keeps its id targetable via chipmate_memory_recall mode=digest.
