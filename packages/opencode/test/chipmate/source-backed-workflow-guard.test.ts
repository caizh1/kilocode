...
import { ChipMateSessionPrompt } from "@/chipmate/session/prompt"
...
describe("source-backed workflow guard", () => {
  ...
  test("blocks shell only in the loaded skill turn and uninterrupted continuation turns", () => {
    ...
    expect(Guard.mutation("s1", first, "/workspace", "/workspace/.chipmate/artifacts/manual/file.md")).toContain(
      "declare_artifact",
    )
    Guard.declare("s1", first, ".chipmate/artifacts/task-1")
    expect(Guard.mutation("s1", first, "/workspace", "/workspace/.chipmate/artifacts/task-1/file.md")).toBeUndefined()
    expect(Guard.mutation("s1", first, "/workspace", "/workspace/.chipmate/artifacts/other/file.md")).toContain(
      ".chipmate/artifacts/task-1",
    )

    const continued = [...first, user("u2", "继续"), user("u3", "continue")]
    expect(Guard.shell("s1", continued)).toBe(true)
    expect(Guard.mutation("s1", continued, "/workspace", "/workspace/.chipmate/artifacts/task-1/next.md")).toBeUndefined()
  })
  ...
  test("only raises the agent step limit for an exactly active source-backed workflow", () => {
    ...
    const first = [user("u1", "生成完整详细设计")]
    expect(ChipMateSessionPrompt.sourceBackedStepLimit(session, first as never, 4)).toBe(4)
    expect(ChipMateSessionPrompt.sourceBackedStepLimit(session, first as never)).toBe(Infinity)

    Guard.activate(session, "source-backed-detail-design", first)
    expect(ChipMateSessionPrompt.sourceBackedStepLimit(session, first as never, 4)).toBe(32)
    expect(ChipMateSessionPrompt.sourceBackedStepLimit(session, first as never, 48)).toBe(48)

    const ordinary = [...first, user("u2", "解释这个函数")]
    expect(ChipMateSessionPrompt.sourceBackedStepLimit(session, ordinary as never, 4)).toBe(4)
  })
  ...
  test("recovers the loaded workflow and declared root without another skill call after a process restart", () => {
    ...
    const continued = [...first, loaded("a1"), declared("a2", ".chipmate/artifacts/task-1"), user("u2", "继续")]
    expect(Guard.shell("s1", continued)).toBe(true)
    expect(Guard.mutation("s1", continued, "/workspace", "/workspace/.chipmate/artifacts/task-1/next.md")).toBeUndefined()
    expect(Guard.mutation("s1", continued, "/workspace", "/workspace/.chipmate/artifacts/task-2/next.md")).toContain(
      ".chipmate/artifacts/task-1",
    )
  })
  ...
  test("keeps the restored root when a compacted tool window retains only the continuation user", () => {
    const history = [
      user("u1", "生成完整详细设计"),
      loaded("a1"),
      declared("a2", ".chipmate/artifacts/task-1"),
      user("u2", "继续"),
    ]
    Guard.activate("s1", "source-backed-detail-design", history)
    const compacted = [user("u2", "继续")]
    expect(Guard.shell("s1", compacted)).toBe(true)
    expect(Guard.mutation("s1", compacted, "/workspace", "/workspace/.chipmate/artifacts/task-1/next.md")).toBeUndefined()
    expect(Guard.mutation("s1", compacted, "/workspace", "/workspace/.chipmate/artifacts/task-2/next.md")).toContain(
      ".chipmate/artifacts/task-1",
    )
    expect(Guard.artifact("s1", compacted, "/workspace", "/workspace/.chipmate/artifacts/task-1/review.md")).toBeUndefined()
    expect(Guard.artifact("s1", compacted, "/workspace", "/workspace/src/module.c")).toBeUndefined()
    expect(Guard.artifact("s1", compacted, "/workspace", "/workspace/.chipmate/artifacts/task-2/review.md")).toContain(
      ".chipmate/artifacts/task-1",
    )
  })
  ...
  test("ignores automatic compaction user records when synchronizing and evaluating an active continuation", () => {
    ...
    expect(Guard.root("s1", [user("u3", "继续"), compacted("c2")])).toBe(".chipmate/artifacts/task-1")
    expect(
      Guard.mutation(
        "s1",
        [user("u3", "继续"), compacted("c2")],
        "/workspace",
        "/workspace/.chipmate/artifacts/task-2/file.md",
      ),
    ).toContain(".chipmate/artifacts/task-1")
  })
  ...
  test("keeps the synchronized workflow when compacted tool context retains only stale non-continuation history", () => {
    ...
    expect(Guard.root("s1", [first])).toBe(".chipmate/artifacts/task-1")
    expect(Guard.shell("s1", [first])).toBe(true)
  })
  ...
  test("does not recover from a continuation word without prior source-backed skill evidence", () => {
    const messages = [user("u1", "普通问题"), declared("a1", ".chipmate/artifacts/task-1"), user("u2", "继续")]
  ...
  test("blocks diagram authoring until all separate prose topics are ready", async () => {
    ...
    const root = path.join(workspace, ".chipmate/artifacts/task-1")
    ...
    const history = [...first, loaded("a1"), declared("a2", ".chipmate/artifacts/task-1"), user("u2", "继续")]
    Guard.sync("s1", history)
