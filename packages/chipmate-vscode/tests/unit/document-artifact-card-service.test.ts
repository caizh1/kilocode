...
describe("artifactManifestToCard", () => {
  ...
  test("registers and executes diagnostics export inside the artifact root", async () => {
    const value = state()
    value.workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-document-artifacts-"))
    install(value)
  ...
  test("ignores root-level diagnostics JSON files when exporting diagnostics again", async () => {
    const value = state()
    value.workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "chipmate-document-artifacts-repeat-"))
    install(value)
