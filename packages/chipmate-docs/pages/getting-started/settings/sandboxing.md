...
The sandbox adds an operating-system boundary around agent tools. It limits where tools can write and, by default, blocks outbound network access from model-originated commands. This boundary applies even when a tool passes ChipMate's permission checks.
...
Sandboxing is not available on Windows. If an enabled macOS or Linux sandbox policy cannot be enforced, ChipMate reports the reason and refuses to run the affected tool. It does not silently fall back to unrestricted execution.
{% /callout %}
...
1. Open ChipMate Settings using the gear icon ({% codicon name="gear" /%}).
2. Select **Sandboxing**.
3. Turn on **Sandbox**.
...
You can also configure the default in the global `chipmate.jsonc` file:
...
|---|---|---|
| Permissions | Whether ChipMate allows, asks about, or denies a matching tool invocation | Prompting for sensitive file reads, blocking specific commands or tools, reviewing consequential actions, and limiting MCP tool or subagent invocation |
...
- ChipMate's runtime directories listed below
- Paths listed in `sandbox.writable_paths`

| Writable ChipMate path | Purpose |
|---|---|
| `$XDG_DATA_HOME/chipmate` (normally `~/.local/share/chipmate`) | Session data, logs, and ChipMate's managed repository cache under `repos/` |
| `$XDG_CACHE_HOME/chipmate` (normally `~/.cache/chipmate`) | Cached data and downloaded binaries |
| `$XDG_STATE_HOME/chipmate` (normally `~/.local/state/chipmate`) | Runtime state |
| `$TMPDIR/chipmate` | Temporary files; on macOS this is commonly under `/var/folders/.../T/chipmate` |
...
- ChipMate's stored sandbox policy and preference files are read-only.
- ChipMate's global config root is read-only so a sandboxed command cannot widen network or write authority for a later session.
...
Shell commands and their child processes inherit the same restrictions. ChipMate's file tools perform mutations through a sandboxed worker. Writable file handles are unavailable, so a tool that requires an open read-write handle may fail even for an allowed path.

Direct filesystem access inside trusted integrations is confined only when the integration uses ChipMate's sandbox-aware filesystem service. Interactive terminals, notebook execution, and starting or restarting a background process are unavailable while sandboxing is active because those processes do not yet run inside the same boundary.
...
When network restriction is on, ChipMate blocks:
...
- Local and remote MCP tool calls and typed resource reads, plus custom or plugin tools that ChipMate cannot prove will remain offline
...
On every platform, ChipMate reports an error and refuses to run the affected tool if the configured confinement or destination proxy cannot be established. It does not fall back to unrestricted execution.
