...
import { capture } from "../../src/chipmate/instance" // chipmate_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // chipmate_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // chipmate_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // chipmate_change start - reload disposers retain legacy instance context
  ...
  // chipmate_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // ChipMateBootstrap (and anything it forkDetaches, e.g. ChipMateIndexing.init) can read
  // the current directory. This regression test pins the ChipMate contract.
  it.live("provides InstanceRef during bootstrap for ChipMate bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
