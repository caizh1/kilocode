...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "chipmate.v2.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("ChipMate Diff Panel")
  }
  ...
  /**
   * Entry point for the `chipmate.v2.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("chipmate.v2").get<string>("language"),
