/**
 * Legacy migration handlers — extracted from ChipMateProvider.
 *
 * Manages the migration wizard for users upgrading from ChipMate v5.x.
...
async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[ChipMate New] ChipMateProvider: 🔄 Legacy data detected, showing migration wizard")
...
async function startLegacyMigration(
  ...
  try {
    ...
    console.error("[ChipMate New] ChipMateProvider: ❌ Migration failed", error)
    ctx.postMessage({
      type: "migrationComplete",
      source: "legacy",
      operationId,
