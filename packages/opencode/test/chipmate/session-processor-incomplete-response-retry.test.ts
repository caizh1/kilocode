...
function empty(vercelID?: string) {
  ...
  const providerMetadata = vercelID ? { chipmate: { vercelID } } : undefined
  return [
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          expect(error.data.message).toBe(ChipMateSessionProcessor.INCOMPLETE_RESPONSE_MESSAGE)
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          yield* ctx.test.reply(
            LLMEvent.stepStart({ index: 0 }),
            LLMEvent.toolCall({ id: "call", name: "web_search", input: { query: "ChipMate" }, providerExecuted: true }),
            LLMEvent.toolResult({
              id: "call",
              name: "web_search",
