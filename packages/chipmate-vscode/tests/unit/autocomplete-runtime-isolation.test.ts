...
const builtin = { providerID: "chipmate", modelID: "inception/mercury-next-edit" }
...
describe("autocomplete coordinator", () => {
  it("keeps every autocomplete runtime stopped while official ChipMate owns completion", async () => {
    config(builtin)
    const ctx = context()
  ...
  it("discards an in-flight provider lookup when official ChipMate is installed", async () => {
    const cfg = config()
    const ctx = context()
    const runtime = manager()
    let enabled = true
