...
function rewrite(request: Request, directory?: string) {
  ...
  const value = pick(request.headers.get("x-chipmate-directory"), directory)
  ...
  const next = new Request(url.href, request) // chipmate_change
  next.headers.delete("x-chipmate-directory")
  return next
}

export function createChipMateClient(config?: Config & { directory?: string }) {
  if (!config?.fetch) {
