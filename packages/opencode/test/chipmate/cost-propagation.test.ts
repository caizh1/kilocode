// Verifies ChipMateCostPropagation.propagate() serializes concurrent writes to
...
import { ChipMateCostPropagation } from "../../src/chipmate/session/cost-propagation"
...
describe("ChipMateCostPropagation.propagate", () => {
  it.live("sums deltas correctly under parallel execution", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        yield* Effect.all(
          deltas.map((d) => ChipMateCostPropagation.propagate(sessions, chat.id, assistant.id, d)),
  ...
  it.live("is a no-op when amount is non-positive", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const sessions = yield* Session.Service
        const { chat, assistant } = yield* seed()
        yield* ChipMateCostPropagation.propagate(sessions, chat.id, assistant.id, 0)
        yield* ChipMateCostPropagation.propagate(sessions, chat.id, assistant.id, -1.5)
        const parent = yield* MessageV2.get({ sessionID: chat.id, messageID: assistant.id })
        if (parent.info.role !== "assistant") return
