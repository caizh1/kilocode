package ai.chipmate.backend.provider

import ai.chipmate.backend.app.ChipMateBackendAppService
import ai.chipmate.backend.app.LoadError
import ai.chipmate.backend.cli.ChipMateBackendHttpClients
import ai.chipmate.backend.cli.ChipMateCliDataParser
import ai.chipmate.backend.rpc.ChipMateWorkspaceDtoMapper
...
internal class ChipMateBackendProviderSettingsManager(
    private val app: ChipMateBackendAppService,
) {
    companion object {
        private val LOG = ChipMateLog.create(ChipMateBackendProviderSettingsManager::class.java)
        private val JSON = "application/json".toMediaType()
        private val FETCH by lazy { ChipMateBackendHttpClients.modelFetch() }
    ...
    suspend fun state(directory: String): ProviderSettingsDto {
        ...
        val providers = load("providers", errors) {
            ChipMateCliDataParser.parseProviderSettingsProviders(get("/provider?directory=${enc(directory)}"))
        }
        val auth = load("provider_auth", errors) {
            ChipMateCliDataParser.parseProviderAuth(get("/provider/auth?directory=${enc(directory)}"))
        } ?: emptyMap()
    ...
    suspend fun connect(input: ProviderConnectDto): ProviderActionResultDto {
        val body = ChipMateCliDataParser.buildProviderAuthJson(input.key, input.metadata)
    ...
    suspend fun authorize(input: ProviderOAuthAuthorizeDto): ProviderOAuthReadyDto {
        val body = ChipMateCliDataParser.buildProviderOAuthJson(input.method, input.inputs)
        val raw = post("/provider/${enc(input.providerId)}/oauth/authorize?directory=${enc(input.directory)}", body, OAUTH_CALL_TIMEOUT_SECONDS)
        val parsed = ChipMateCliDataParser.parseOAuthReady(raw)
    ...
    suspend fun callback(input: ProviderOAuthCallbackDto): ProviderActionResultDto {
        val body = ChipMateCliDataParser.buildProviderOAuthJson(input.method, code = input.code)
    ...
    suspend fun disconnect(input: ProviderDisconnectDto): ProviderActionResultDto {
        ...
        if (input.providerId == "chipmate") {
            return ProviderActionResultDto(current, error = "ChipMate Gateway cannot be disconnected from provider settings.")
        }
        ...
        if (cfg?.npm == "@ai-sdk/openai-compatible") {
            patch(input.directory, cfg.scope, ChipMateCliDataParser.buildCustomProviderDeletePatch(input.providerId))
        ...
        if (provider?.source == "config") {
            ...
            patch(input.directory, scope, ChipMateCliDataParser.buildDisabledProviderPatch(ids))
            dispose()
            return ProviderActionResultDto(state(input.directory))
        }
        deleteAuth(input.providerId)
    ...
    suspend fun enable(input: ProviderEnableDto): ProviderActionResultDto {
        ...
        scopes.distinct().forEach { scope ->
            patch(input.directory, scope, ChipMateCliDataParser.buildDisabledProviderPatch(disabledFor(current, scope).filter { it != input.providerId }))
        }
        dispose()
        return ProviderActionResultDto(state(input.directory))
    }
    ...
    suspend fun saveCustom(input: CustomProviderSaveDto): ProviderActionResultDto {
        ...
        patch(ChipMateCliDataParser.buildCustomProviderPatch(input))
        if (input.envVar.isNullOrBlank()) {
            val key = input.apiKey?.takeIf { it.isNotBlank() }
            if (key != null) put("/auth/${enc(input.id)}", ChipMateCliDataParser.buildProviderAuthJson(key, emptyMap()))
        } else {
            deleteAuth(input.id)
        }
        dispose()
    ...
    suspend fun fetch(input: CustomModelFetchDto): CustomModelFetchResultDto {
        ...
        return try {
            ...
            CustomModelFetchResultDto(ChipMateCliDataParser.parseModelIds(raw))
        } catch (e: Exception) {
    ...
    private suspend fun <T> load(resource: String, errors: MutableList<LoadErrorDto>, block: suspend () -> T): T? {
        ...
        return try {
            ...
            errors.add(ChipMateWorkspaceDtoMapper.error(LoadError(resource = resource, detail = e.message)))
            null
        }
    }
    ...
    private fun parsed(raw: String): ParsedConfig {
        val result = ChipMateCliDataParser.parseProviderConfig(raw)
    ...
    private suspend fun request(request: Request, timeoutSeconds: Long = CALL_TIMEOUT_SECONDS): String {
        ...
        val http = app.http?.let { ChipMateBackendHttpClients.bounded(it, timeoutSeconds) }
            ?: throw IllegalStateException("ChipMate HTTP client is unavailable")
