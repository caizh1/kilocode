 ...
 * its own `chipmate run` call. A chunk that fails, is only partially classified,
 ...
 *
 * Env: TRIAGE_MODEL (provider/model), CHIPMATE_API_KEY + CHIPMATE_ORG_ID (gateway auth, set by
 * the workflow; the chipmate provider reads them natively). Reads the prompt from triage-prompt.md next to this script.
...
import { appendSummary, backoffMsForAttempt, deadline, remainingMs, runChipMate, sleepSync } from "./lib.mjs"
...
function triageChunk(chunk, index, budgetDeadline) {
  ...
  for (let attempt = 1; attempt <= ATTEMPTS; attempt++) {
    ...
    // Headless `chipmate run` auto-rejects every permission ask; without --auto the
    ...
    // permission.bash map via CHIPMATE_CONFIG_CONTENT should replace --auto once the
    // required shell patterns are stable (see PR #12605 review thread).
    const result = runChipMate({
