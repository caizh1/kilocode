...
afterAll(async () => {
  ...
  for (const file of ["chipmate.jsonc", "chipmate.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["chipmate-permission-8353 test"],
          metadata: { rules: ["chipmate-permission-8353 *", "chipmate-permission-8353 test"] },
          always: ["chipmate-permission-8353 *", "chipmate-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["chipmate-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "chipmate-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "chipmate-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionV1.ID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["chipmate-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
