...
async function resolveArchiveOrExecutable(
  ...
  const cacheDir = process.env.CHIPMATE_RIPGREP_CACHE_DIR?.trim() || join(homedir(), ".cache", "chipmate", "ripgrep")
...
async function fetchWithRetry(url: string, attempts: number): Promise<Response> {
  ...
  throw new Error(
    `Failed to download ripgrep from ${url}. For offline builds, preseed RIPGREP_${envPrefix}_EXE, RIPGREP_${envPrefix}_DIR, RIPGREP_${envPrefix}_ARCHIVE, or CHIPMATE_RIPGREP_CACHE_DIR: ${msg}`,
    { cause: last },
  )
}
