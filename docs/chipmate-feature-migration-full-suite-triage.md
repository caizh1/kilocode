...
```bash
cd /Users/archer/Work/chipmate/packages/opencode && bun run test
```
...
|---|---|---|
| Artifact manager | `PASS chipmate/document-artifacts.test.ts` | Supports migrated artifact tool contract. |
| Mermaid document tools | `PASS chipmate/mermaid-documents.test.ts` | Supports migrated Mermaid validate/render/save/insert contract. |
| Source-backed detail-design skill fixture | `PASS chipmate/source-backed-detail-design-skill.test.ts` | Supports skill contract, not installed runtime quality. |
| Word document tools | `PASS chipmate/word-documents.test.ts` | Supports Word create/edit/template/diff/render warning contracts. |
| ChipMate session LLM fixture under `chipmate/` | `PASS chipmate/session/llm.test.ts` | Supports migrated ChipMate-specific LLM fixture coverage, but does not clear global `session/llm.test.ts` failures. |

Failed C1 areas:

| Failed area | Observed failure shape | Migration relation | M11 status |
|---|---|---|---|
| `chipmate/global-config-refresh.test.ts` | Timeout after `60000ms`. | Not directly caused by Word/Mermaid/artifact sidecars in the captured log. | Keep as full-suite blocker until separately rerun or accepted as known baseline. |
...
| `chipmate/session-prompt-queue.test.ts` | Multiple prompt queue tests timed out after `60000ms`. | ChipMate session queue behavior; not enough evidence to clear. | Keep as full-suite blocker. |
...
```bash
cd /Users/archer/Work/chipmate/packages/chipmate-vscode && bun run test:unit
```
...
| Qwen autocomplete diagnostics and context injection | `qwen-autocomplete-diagnostics`, `recently-edited`, `import-definitions`, `snippets`, cache integration, provider rendering, root path, multiline classifier. | Existing autocomplete pipeline surface. It matters for ChipMate no-regression, but is not caused by Word/Mermaid/document sidecars in the captured evidence. | Keep K7 open until full suite rerun or S16 installed autocomplete smoke passes. |
| Backend/session/worktree/provider mocks | `ChipMateProvider.handleLoadMessages`, pending session refresh, `ChipMateConnectionService backend crash`, `WorktreeManager.resolveStartPoint`, `nativeTitle`. | Existing ChipMate provider/session/worktree behavior. | Keep as broad unit-suite known issue. |
| Code action provider | `ChipMateActionProvider` Add/Explain/Improve/Fix action expectations. | Existing VS Code command/action behavior. | Keep as broad unit-suite known issue. |
...
| Claim ChipMate native QA no-regression | Not allowed yet. S1-S3 are still blocked by provider auth. |
| Claim ChipMate autocomplete no-regression | Not allowed yet. C3 has autocomplete failures and S16 has not run. |
