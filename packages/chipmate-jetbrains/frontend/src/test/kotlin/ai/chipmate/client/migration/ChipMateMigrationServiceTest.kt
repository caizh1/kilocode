...
import ai.chipmate.client.testing.FakeMigrationRpcApi
...
class ChipMateMigrationServiceTest : BasePlatformTestCase() {

    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeMigrationRpcApi
    private lateinit var service: ChipMateMigrationService
    private lateinit var app: MutableStateFlow<ChipMateAppStateDto>
    ...
    override fun setUp() {
        ...
        app = MutableStateFlow(ChipMateAppStateDto(ChipMateAppStatusDto.DISCONNECTED))
        autocomplete.clear()
        telemetry.clear()
        service = ChipMateMigrationService(scope, rpc, app, { autocomplete.add(it) }) { event, props -> telemetry.add(event to props) }
    }
    ...
    fun `test migration required app state shows needed without polling`() {
        app.value = ChipMateAppStateDto(ChipMateAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
    ...
    fun `test ready app state hides migration`() {
        app.value = ChipMateAppStateDto(ChipMateAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
        app.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY)
        settle()
    ...
    fun `test duplicate migration required does not reset running migration`() {
        ...
        app.value = ChipMateAppStateDto(ChipMateAppStatusDto.MIGRATION_REQUIRED, migration = detection)
        settle()
    ...
    fun `test complete event without items finalizes pending session progress`() = runBlocking {
        app.value = ChipMateAppStateDto(ChipMateAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection().copy(
