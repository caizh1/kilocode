# AGENTS.md — ChipMate JetBrains Plugin

## Package Overview

- **Split-mode plugin** with three Gradle modules: `shared/`, `frontend/`, `backend/`. The module descriptors are `chipmate.jetbrains.shared.xml`, `chipmate.jetbrains.frontend.xml`, `chipmate.jetbrains.backend.xml` — these must stay in sync with `plugin.xml`'s `<content>` block.
...
- Kotlin source goes under `{module}/src/main/kotlin/ai/chipmate/jetbrains/`. Package name is `ai.chipmate.jetbrains` (matches `group` in root `build.gradle.kts`).
...
- `plugin.xml` `<content>` entries ↔ module XML descriptors (`chipmate.jetbrains.{shared,frontend,backend}.xml`)
- Service classes ↔ `<applicationService>`/`<projectService>` entries in the corresponding module XML
- `packages/chipmate-jetbrains/package.json` version ↔ GitHub CLI release tag consumed by the backend downloader
- `packages/chipmate-jetbrains/gradle.properties` `chipmate.cli.pinned` ↔ Gradle and release-script gates
- `.chipmate/skills/release-jetbrains/script/check-pin.ts` / `set-pin.ts` ↔ release skill and CLI pin documentation
...
- Non-light services that need XML registration go in `chipmate.jetbrains.backend.xml` under `<extensions defaultExtensionNs="com.intellij"><applicationService>` (or `<projectService>`).
...
- By default, the plugin does not bundle CLI binaries. At connect time the backend downloads the GitHub Release asset for the version pinned in `packages/chipmate-jetbrains/package.json`; `backend` resources include `chipmate.properties` with `cli.version` and `cli.pinned` for split-mode RPC and runtime use.
- Bundled release builds pass `-Pchipmate.cli.bundled=true` while keeping `chipmate.cli.pinned=true`. This build-only flag stages all pinned CLI release assets into `chipmate-cli.zip`; runtime detects that resource and extracts only the current platform instead of downloading. Do not add a `cli.bundled` key to `chipmate.properties` or repurpose `chipmate.cli.pinned=false` for public bundled releases.
- For release questions, use the `release-jetbrains` skill and reference `.chipmate/skills/release-jetbrains/SKILL.md`; it verifies the CLI pin before creating immutable `jetbrains/v*` tags.
...
For a one-shot pin/unpin/regen that also cleans every leftover CLI binary and build artifact in the current worktree, use the `jetbrains-cli-pin` skill (`.chipmate/skills/jetbrains-cli-pin/SKILL.md`): `bun .chipmate/skills/jetbrains-cli-pin/script/cli-pin.ts <pin|unpin|regen|clean>`.

**Pin mode** (`chipmate.cli.pinned` in `packages/chipmate-jetbrains/gradle.properties`) controls release CLI vs local repo CLI.

| Ask | Do |
|---|---|
| Unpin / use local repo CLI | Set `chipmate.cli.pinned=false`, then run `./gradlew :backend:buildRepoCli` from `packages/chipmate-jetbrains/`. `:backend:stageRepoCli` bundles `packages/opencode/dist/@chipmate/cli-<os>-<arch>/bin/`; runtime extracts it instead of downloading. |
| Re-pin / use release CLI | Set `chipmate.cli.pinned=true`. This is the default and the only releasable state. |

`chipmate.cli.pinned=false` is dev-only: OpenAPI generation runs from local `packages/opencode/` source and the local binary is bundled. Production Gradle builds, `script/build-version.sh`, and the release scripts hard-fail on `false`, so restore `true` before releasing.

**Pinned CLI version** (`packages/chipmate-jetbrains/package.json` `version`) controls which GitHub CLI release the plugin downloads and generates the client from. The JetBrains release locks the value already merged to `origin/main`.

| Ask | Do |
|---|---|
| Check whether the CLI pin is current | `bun .chipmate/skills/release-jetbrains/script/check-pin.ts` |
| Bump the pin to `<version>` / latest and test locally | `bun .chipmate/skills/release-jetbrains/script/set-pin.ts --version <x.y.z>` or `bun .chipmate/skills/release-jetbrains/script/set-pin.ts --latest`, then run `./gradlew typecheck && ./gradlew test` from `packages/chipmate-jetbrains/`. |
| Land a tested pin bump for release | `bun .chipmate/skills/release-jetbrains/script/set-pin.ts --version <x.y.z> --pr` or `bun .chipmate/skills/release-jetbrains/script/set-pin.ts --latest --pr`; merge the PR to `main`, then re-run `check-pin.ts` before dispatching prepare. |
...
For the full release process (resolve version, pin verification, prepare, changelog, publish), load the `release-jetbrains` skill: `.chipmate/skills/release-jetbrains/SKILL.md`.

### Server Protocol

- The plugin spawns `chipmate serve --port 0` (OS assigns random port) and reads stdout for `listening on http://...:(\d+)` to discover the port.
- A random 32-byte hex password is passed via `CHIPMATE_SERVER_PASSWORD` env var for Basic Auth.
- Fixed env vars set on every spawn: `CHIPMATE_CLIENT=jetbrains`, `CHIPMATE_PLATFORM=jetbrains`, `CHIPMATE_APP_NAME=chipmate-code`, `CHIPMATE_ENABLE_QUESTION_TOOL=true`, `CHIPMATE_DISABLE_CLAUDE_CODE=true`, `CHIPMATE_FEATURE=jetbrains-plugin`.
- Unless already provided by the base environment, the backend sets `CHIPMATE_CONFIG_CONTENT` to make `edit` and `bash` permissions ask by default for JetBrains-launched CLI processes.
- This is the same protocol used by the VS Code extension (`packages/chipmate-vscode/src/services/cli-backend/server-manager.ts`).

### Dev Storage Isolation

- In development (`runIdeSplitMode`, `runIdeBackend`, `runIdeFrontend`, or `runIde`), the Gradle property `chipmate.dev.storage.isolated=true` makes the backend set `XDG_DATA_HOME`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` to `<worktree>/.chipmate-dev/{data,config,state,cache}` before spawning the CLI. The worktree root comes from the `chipmate.dev.worktree.root` JVM system property (auto-set by Gradle from the project directory).
- The checked-in `Run IDE (Backend)`, `Run IDE (Frontend)`, and `Run IDE (Split Mode)` run configurations enable isolation by default (`-Pchipmate.dev.storage.isolated=true`). Developers can disable it by passing `-Pchipmate.dev.storage.isolated=false`.
- Use standard `XDG_*_HOME` env vars for this isolation. Do not introduce custom `CHIPMATE_DATA_DIR`, `CHIPMATE_GLOBAL_CONFIG_DIR`, `CHIPMATE_STATE_DIR`, or `CHIPMATE_CACHE_DIR` env vars — the CLI core already respects `XDG_*_HOME` via `xdg-basedir`.
- The `.chipmate-dev/` directory is gitignored and created automatically on first run.
- The implementation lives in `ChipMateBackendCliManager.buildEnv()` / `devStorageEnv()`. Tests: `ChipMateBackendCliManagerEnvTest`.

### Debugging Session Event Logs

- Use `script/dev/part-update.sh client <session-id>` from `packages/chipmate-jetbrains/` to print frontend `message.part.delta` text by part id.
- Use `script/dev/part-update.sh backend <session-id>` from `packages/chipmate-jetbrains/` for backend sandbox events.
- Append with `>> file.txt` when you need to keep the output.
- For full chat payload previews in JetBrains dev runs, pass `-Pchipmate.dev.log.chat.content=<mode>` where `<mode>` is `off` (default, no content), `preview` (cleaned/truncated content), or `full` (cleaned full content).
- `-Pchipmate.dev.log.chat.preview.max=<n>` controls preview length, clamped from 1 to 2000.

## Build and Verification

- **Marketplace version build**: Use `script/build-version.sh <version>` from `packages/chipmate-jetbrains/` to clean, build, sign, and verify the JetBrains Marketplace plugin ZIP. Pass `--skip-verification` only when explicitly needed.
- **Test version build**: If the user asks for a JetBrains test build, still require a version and use `script/build-version.sh <version> --skip-signing --skip-verification` from `packages/chipmate-jetbrains/` so no signing secrets are needed. Add `--skip-clean` only when the user wants a faster incremental test build.
- **Typecheck**: `bun run typecheck` or `./gradlew typecheck` from `packages/chipmate-jetbrains/` — compiles all Kotlin sources including the generated API client. A cold pinned build downloads the pinned CLI release via `generateOpenApiSpec` and needs network access; Gradle-cached incremental runs skip the download. Repo CLI mode (`-Pchipmate.cli.pinned=false`) generates the spec from local source and bundles the staged local CLI binary.
- **Build local repo CLI for JetBrains dev**: `./gradlew :backend:buildRepoCli` from `packages/chipmate-jetbrains/` builds `packages/opencode/dist/@chipmate/cli-<os>-<arch>/bin/`. `stageRepoCli` intentionally does not depend on this task; missing binaries fail with instructions instead of silently starting a slow CLI build.
- **Full build**: `bun run build` from `packages/chipmate-jetbrains/` (runs Gradle `buildPlugin`).
- **Gradle only**: `./gradlew buildPlugin` from `packages/chipmate-jetbrains/`.
...
- **Via Turbo**: `bun turbo build --filter=@chipmate/chipmate-jetbrains` from repo root.
...
- JetBrains runtime behavior normally depends on the downloaded CLI release pinned by `packages/chipmate-jetbrains/package.json`; local `packages/opencode/` changes are used only with `chipmate.cli.pinned=false` repo CLI mode.
- If there are relevant server/API changes outside `packages/chipmate-jetbrains/`, warn the user that JetBrains may need a newly published/pinned CLI release and regenerated SDK artifacts.

## UI Guidelines

### Technology Choices

**Do not use Kotlin UI DSL v2 (`com.intellij.ui.dsl.builder`) in this plugin.** Use standard Swing with IntelliJ Platform components for all UI layout. The JetBrains modular template and some older sections of the codebase reference the DSL, but ChipMate should not introduce it.

**Do not use Kotlin Compose or `intellij.platform.compose` in this plugin.** The JetBrains modular template uses Compose for its demo tool window, but ChipMate should use standard Swing with IntelliJ Platform components only. Keep all plugin UI in the existing Swing-based stack.
...
**`UiStyle`** (`frontend/src/main/kotlin/ai/chipmate/client/ui/UiStyle.kt`) is the single source of truth for reusable UI constants in this plugin.
...
**`SessionUiStyle`** (`frontend/src/main/kotlin/ai/chipmate/client/session/ui/style/SessionUiStyle.kt`) owns static tokens specific to the chat/session UI:
...
| One-dimensional multi-component rows/columns | `ai.chipmate.client.ui.layout.Stack` — see section below |
| Fluent platform panels | `JBPanel.withBorder(...)`, `.andTransparent()`, `.andOpaque()`, `.withBackground(...)` |
| Single-component alignment wrapper | `ai.chipmate.client.ui.layout.Align` — see section below |

### Stack — One-Dimensional Multi-Component Layout

Use `Stack` (`ai.chipmate.client.ui.layout.Stack`) when multiple Swing components should be laid out as one vertical column or one horizontal row without visual chrome. It is a transparent, no-border, no-color `JPanel(null)` that lays out visible children in insertion order.
...
- Register declaratively in module XML via `com.intellij.toolWindow` extension point (already done in `chipmate.jetbrains.frontend.xml`).
...
- Declare in module XML: `<notificationGroup id="ChipMate" displayType="BALLOON"/>`.
- Show: `Notification("ChipMate", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound: `displayType="TOOL_WINDOW" toolWindowId="ChipMate"`.
...
Settings UI has reusable primitives in `frontend/src/main/kotlin/ai/chipmate/client/settings/base/`. Check these before adding new settings components or custom Swing assemblies.
...
- Keep settings UI on the EDT and continue using existing platform Swing components, `Stack`, `Align`, `UiStyle`, and localized `ChipMateBundle` strings according to the UI guidance above.
...
`frontend/src/main/kotlin/ai/chipmate/client/session/`.
...
- After history load, `recoverPending()` seeds state in this priority order: (1) pending permission, (2) pending question, (3) current session status (`busy`/`retry`/`offline` from `ChipMateSessionService.statuses`), (4) `Idle`.
