...
export namespace MemoryDebug {
  ...
  export function enabled() {
    return process.env.CHIPMATE_MEMORY_DEBUG === "1"
  }

  export function directory() {
    return process.env.CHIPMATE_MEMORY_DEBUG_DIR || path.join(Global.Path.log, "memory-debug")
  }
  ...
  export function operation(headers: Headers | Record<string, string | undefined>) {
    const value =
      headers instanceof Headers ? headers.get("x-chipmate-memory-operation") : headers["x-chipmate-memory-operation"]
  ...
  export function start() {
    ...
    started = true
    void event({ name: "cli.started", data: { pid: process.pid, runId: process.env.CHIPMATE_MEMORY_DEBUG_RUN_ID } })
    void sample()
  ...
  export function event(input: Event) {
    ...
    const item = {
      ...
      source: "cli",
      runId: process.env.CHIPMATE_MEMORY_DEBUG_RUN_ID,
      pid: process.pid,
      event: input.name,
  ...
  export function snapshot(reason = "manual") {
    const file = path.join(
      directory(),
      `heap-${process.env.CHIPMATE_MEMORY_DEBUG_RUN_ID ?? process.pid}-${reason}-${new Date().toISOString().replace(/[:.]/g, "")}.heapsnapshot`,
    )
    return fs
  ...
  async function append(item: Record<string, unknown>) {
    ...
    const file = path.join(dir, `memory-debug-cli-${process.env.CHIPMATE_MEMORY_DEBUG_RUN_ID ?? process.pid}.jsonl`)
    await rotate(file)
