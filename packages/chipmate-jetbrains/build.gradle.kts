...
group = "ai.chipmate.jetbrains"

fun port(value: String): Int {
    val text = value.trim()
    require(text.isNotEmpty()) {
        "chipmate.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port"
    }
    val n = text.toIntOrNull()
        ?: error("chipmate.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port")
...
val pinned = providers.gradleProperty("chipmate.cli.pinned").map { it.trim().toBoolean() }.orElse(true).get()
val override = providers.gradleProperty("chipmate.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val prop = providers.gradleProperty("chipmate.jetbrains.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val tag = gitTag()?.removePrefix("jetbrains/v")
val ver = override?.let(::checked) ?: prop?.let(::checked) ?: if (release) checked(
    tag ?: error("Missing JetBrains plugin version. Publish builds must set chipmate.jetbrains.version or run from a jetbrains/v<version> tag."),
) else checked(tag ?: "0.0.0-dev")

if (release && !pinned) error(
    "chipmate.cli.pinned=false is a dev-only mode and cannot be released. Set chipmate.cli.pinned=true before a production/publish build."
)

val channel = providers.gradleProperty("chipmate.channel").map { it.trim() }.orElse("default")
val splitPort = providers.gradleProperty("chipmate.splitModeServerPort").map(::port).orElse(0)
val isolated = providers.gradleProperty("chipmate.dev.storage.isolated").map { it.toBoolean() }.orElse(false)
val worktreeRoot = providers.gradleProperty("chipmate.dev.worktree.root").orElse(
...
changelog {
    ...
    repositoryUrl = "https://github.com/ChipMate-Org/chipmate"
    groups = listOf("Added", "Changed", "Fixed", "Removed", "Security")
    combinePreReleases = false
}

val notes = providers.gradleProperty("chipmate.changeNotes").orElse(
    provider {
...
tasks.withType<RunIdeTask> {
    val level = providers.gradleProperty("chipmate.dev.log.level").orNull ?: "DEBUG"
    val content = providers.gradleProperty("chipmate.dev.log.chat.content").orNull ?: "off"
    val preview = providers.gradleProperty("chipmate.dev.log.chat.preview.max").orNull ?: "160"
    systemProperty("chipmate.dev.log.level", level)
    systemProperty("chipmate.dev.log.chat.content", content)
    systemProperty("chipmate.dev.log.chat.preview.max", preview)
    systemProperty("chipmate.dev.storage.isolated", isolated.get().toString())
    systemProperty("chipmate.dev.worktree.root", worktreeRoot.get())
}
