  ...
  return Effect.gen(function* () {
    yield* SessionEvent.All.match(event, {
      ...
      "session.next.compaction.ended": (event) => {
        if (event.data.messageID === undefined || event.data.reason === undefined) return Effect.void // chipmate_change
        return adapter.appendMessage(
          SessionMessage.Compaction.make({
            ...
            recent: event.data.recent ?? "", // chipmate_change - current v1 writes include recent; released rows omit it
            time: { created: event.data.timestamp },
          }),
        )
      },
