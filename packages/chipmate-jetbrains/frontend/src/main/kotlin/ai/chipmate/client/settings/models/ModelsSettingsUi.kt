...
internal class ModelsSettingsUi(
...
) : BaseSettingsUi<ModelsSettingsContent, ModelsDraft, ConfigPatchDto, ChipMateAppStateDto, ModelsWorkspaceDto>(
    cs,
    ModelsDraft(),
    app,
    workspaces,
    ...
    companion object {
        private val LOG = ChipMateLog.create(ModelsSettingsUi::class.java)
    }
    ...
    override fun base(result: ChipMateAppStateDto): ModelsDraft = modelsDraft(result.config, agents)

    override fun draft(state: ChipMateAppStateDto): ModelsDraft = modelsDraft(state.config, agents)

    override fun saved(base: ModelsDraft, draft: ModelsDraft): Boolean = savedMatches(base, draft)

    override fun pendingText(): String = ChipMateBundle.message("settings.models.save.pending")

    override fun failedText(): String = ChipMateBundle.message("settings.models.save.failed")
    ...
    override fun unavailable(state: ChipMateAppStateDto) {
    ...
    override fun syncContent() {
        ...
        val state = modelsStatus(
            ready = appState.status == ChipMateAppStatusDto.READY && hasProjectDirectory,
            loading = workspaceLoading || (appState.status == ChipMateAppStatusDto.READY && !workspaceLoaded && hasProjectDirectory),
            providers = providers,
            items = allItems.size,
            errors = errors,
            saving = saving,
        ...
        val bannerVisible = modelsLoginBannerVisible(
            ready = appState.status == ChipMateAppStatusDto.READY,
            authenticated = appState.profile != null,
        )
        syncModelBanner(state, bannerVisible)
        val err = saveError
        if (saving || state == ModelsStatus.SAVING) {
            showProgress(ChipMateBundle.message("settings.models.save.pending"))
        } else if (err != null) {
            showError(err)
        } else if (state == ModelsStatus.UNAVAILABLE || state == ModelsStatus.LOADING) {
            showProgress(ChipMateBundle.message("settings.models.loading"))
        } else {
            clearProgress()
        }
        var layout = false
        ...
        syncLoginBanner(login) {
            ...
            when (state) {
                ModelsStatus.LOAD_FAILED -> top.showBanner(
                    ChipMateBundle.message("settings.models.load.failed"),
                    emptyList(),
                    SettingsBannerKind.ERROR,
                )
                ModelsStatus.NO_PROVIDERS -> top.showBanner(ChipMateBundle.message("settings.models.noProviders"), emptyList())
                ModelsStatus.MODES_FAILED -> top.showBanner(ChipMateBundle.message("settings.models.modes.failed"), emptyList())
                else -> top.hideBanner()
            }
        }
    }
...
internal class ModelsSettingsContent(
    ...
    val variantRow = SettingsRow(
        ChipMateBundle.message("settings.models.subagentVariant.title"),
        ChipMateBundle.message("settings.models.subagentVariant.description"),
    ...
    init {
        ...
        rows.row(SettingsRow(
            ChipMateBundle.message("settings.models.defaultModel.title"),
            ChipMateBundle.message("settings.models.defaultModel.description"),
            defaults,
        ))
        rows.row(SettingsRow(
            ChipMateBundle.message("settings.models.smallModel.title"),
            ChipMateBundle.message("settings.models.smallModel.description"),
            small,
        ))
        rows.row(SettingsRow(
            ChipMateBundle.message("settings.models.subagentModel.title"),
            ChipMateBundle.message("settings.models.subagentModel.description"),
            subagent,
        ))
        rows.row(variantRow)
        modes = section(
            ChipMateBundle.message("settings.models.modeModels.title"),
            ChipMateBundle.message("settings.models.modeModels.description"),
        )
    }
}
