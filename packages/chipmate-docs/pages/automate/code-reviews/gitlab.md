...
ChipMate's Code Reviews integrate with GitLab to automatically review merge requests with AI. When an MR is opened, updated, or reopened, the Review Agent analyzes the changes and posts feedback directly on the merge request — as summary notes and inline discussion comments.

Both **GitLab.com** and **self-hosted GitLab instances** are supported.

## Prerequisites

- A ChipMate account at [app.chipmate.ai](https://app.chipmate.ai)
- A GitLab account with **Maintainer** role (or higher) on the projects you want to review
- ChipMate credits for AI model usage

> **Why Maintainer role?** ChipMate creates a bot account (Project Access Token) on each project so that review comments appear from a bot, not your personal account. This requires Maintainer access.
...
1. Go to **Code Reviews**:
   - **Personal**: [app.chipmate.ai/code-reviews](https://app.chipmate.ai/code-reviews)
...
When you select repositories, ChipMate **automatically creates webhooks** on each project.
...
1. A 👀 reaction appears on the MR — this means ChipMate is reviewing
...
Review comments are posted by a **ChipMate Review Bot** — not by your personal GitLab account. This bot is created automatically as a Project Access Token on each project.

- Created automatically the first time a project is reviewed
- Valid for 365 days and rotated automatically before expiry
- If you manually revoke the bot token in GitLab, ChipMate creates a new one on the next review
- Requires **Maintainer role** on the project

## Webhooks

ChipMate manages webhooks automatically:
...
- **URL**: `https://app.chipmate.ai/api/webhooks/gitlab`
...
- Ensure you have sufficient ChipMate credits
