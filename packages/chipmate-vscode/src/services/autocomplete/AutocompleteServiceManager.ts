...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: ChipMateConnectionService) {
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    ...
    const msg =
      status === 402
        ? t("chipmate:autocomplete.creditsExhausted.message")
        : t("chipmate:autocomplete.authError.message")

    if (status === 402) {
      vscode.window.showWarningMessage(msg, t("chipmate:autocomplete.creditsExhausted.addCredits")).then((choice) => {
        if (choice === t("chipmate:autocomplete.creditsExhausted.addCredits")) {
          vscode.env.openExternal(vscode.Uri.parse("https://app.chipmate.ai/credits"))
        }
      })
    } else {
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("chipmate:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("chipmate:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("chipmate:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
