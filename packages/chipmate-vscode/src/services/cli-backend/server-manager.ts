...
export function resolveCliPath(
  ...
  const arm = paths.join(bin, "chipmate-arm64.exe")
  if (isWindowsArm(env, platform) && exists(arm)) return arm
  return paths.join(bin, platform === "win32" ? "chipmate.exe" : "chipmate")
}

export function cliRuntimeEnv(file: string): Record<string, string> {
  const paths = /^[a-z]:[\\/]/i.test(file) || file.includes("\\") ? path.win32 : path
  if (paths.basename(file).toLowerCase() !== "chipmate-arm64.exe") return {}
  return { CHIPMATE_INDEXING_PROCESS_PATH: paths.join(paths.dirname(file), "chipmate-indexer-arm64.exe") }
}
...
export function emptyWorkspaceEnv(
  ...
  return { CHIPMATE_VSCODE_EMPTY_WORKSPACE_DIR: path.resolve(cwd) }
}
...
export function buildBundledToolEnv(root: string, base: NodeJS.ProcessEnv = process.env): Record<string, string> {
  ...
  return {
    ...
    CHIPMATE_VSCODE_BUNDLED_BIN: bin,
    ...(base.CHIPMATE_RIPGREP_PATH ? {} : { CHIPMATE_RIPGREP_PATH: rg }),
  }
}
...
export function resolveManagedServerEnv(env: NodeJS.ProcessEnv, storage: string): NodeJS.ProcessEnv {
  const blocked = new Set([
    ...
    "CHIPMATE_DEV_CWD",
    "CHIPMATE_DEV_REPO",
    "CHIPMATE_MEMORY_DEBUG_DIR",
    "CHIPMATE_INDEXING_PROCESS_PATH",
    "CHIPMATE_PLUGIN_META_FILE",
    "CHIPMATE_TUI_CONFIG",
    "CHIPMATE_ZED_DB",
  ])
  const clean = Object.fromEntries(
    Object.entries(env).filter(([key]) => !blocked.has(key) && !key.startsWith("CHIPMATE_TEST_")),
  )
  return {
    ...clean,
    CHIPMATE_DISABLE_CHANNEL_DB: "true",
    CHIPMATE_PRODUCT_PROFILE: PRODUCT,
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    ...
    console.log("[ChipMate New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[ChipMate New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[ChipMate New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[ChipMate New] ServerManager: 🚀 Starting new server instance...")
    const start = this.startServer()
      .then((instance) => {
        ...
        console.log("[ChipMate New] ServerManager: ✅ Server started successfully:", { port: instance.port })
        return instance
      })
      .finally(() => {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[ChipMate New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[ChipMate New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    const cliArch = path.basename(cliPath).toLowerCase() === "chipmate-arm64.exe" ? "arm64" : process.arch
    console.log("[ChipMate New] ServerManager: 🔎 CLI SHA-256:", cliHash)
    console.log("[ChipMate New] ServerManager: 🧭 Runtime architecture:", {
    ...
    console.log("[ChipMate New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[ChipMate New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[ChipMate New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("chipmate.v2")
      const render = renderEnv()
      console.log("[ChipMate New] ServerManager: 🖼️ Mermaid render endpoint:", mermaidEndpoint(render))
      console.log("[ChipMate New] ServerManager: 🖼️ PlantUML render endpoint:", plantumlEndpoint(render))
      ...
      const bwrapEnv = process.env.CHIPMATE_BWRAP_PATH ? {} : resolveLocalBwrapEnv(this.context.extensionPath, localCli)
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          CHIPMATE_MEMORY_DEBUG: "1",
          CHIPMATE_MEMORY_DEBUG_DIR: MemoryDebug.directory(),
          CHIPMATE_MEMORY_DEBUG_RUN_ID: runId,
          ...
          CHIPMATE_CLIENT: "vscode",
          CHIPMATE_ENABLE_QUESTION_TOOL: "true",
          CHIPMATE_FEATURE: "vscode-extension",
          ...internal,
          ...indexingControl,
          ...empty,
          CHIPMATE_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          CHIPMATE_APP_NAME: "chipmate",
          CHIPMATE_EDITOR_NAME: vscode.env.appName,
          CHIPMATE_PLATFORM: "vscode",
          CHIPMATE_MACHINE_ID: vscode.env.machineId,
          CHIPMATE_APP_VERSION: this.context.extension.packageJSON.version,
          CHIPMATE_VSCODE_VERSION: vscode.version,
          CHIPMATE_VERSION: this.context.extension.packageJSON.version,
          ...(!claudeCompat && { CHIPMATE_DISABLE_CLAUDE_CODE: "true" }),
      ...
      console.log("[ChipMate New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      const finish = (code: number | null, signal: NodeJS.Signals | null) => {
        ...
        console.log("[ChipMate New] ServerManager: 🛑 Process exited:", { code, signal })
        this.lastExitInfo = {
          code,
          signal,
          stderr: [...stderrLines],
      ...
      const timer = setTimeout(() => {
        if (resolved) return
        console.error(`[ChipMate New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[ChipMate New] ServerManager: 📥 CLI Server stdout:", output)
        ...
        if (port !== null && !resolved) {
          resolved = true
          clearTimeout(timer)
          console.log("[ChipMate New] ServerManager: 🎯 Port detected:", port)
          void MemoryDebug.append({
            event: "cli.ready",
            runId,
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[ChipMate New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
      ...
      serverProcess.on("error", (error) => {
        console.error("[ChipMate New] ServerManager: ❌ Process error:", error)
  ...
  private getCliPath(): string {
    ...
    console.log("[ChipMate New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  private static killProcess(proc: ChildProcess, signal: NodeJS.Signals = "SIGTERM"): void {
    ...
    if (process.platform === "win32") {
      ...
      killer.on("error", (err) => {
        console.warn("[ChipMate New] ServerManager: taskkill failed:", { pid: proc.pid, error: err.message })
      })
      return
    }
    try {
      ...
      console.debug("[ChipMate New] ServerManager: process group already exited:", {
        pid: proc.pid,
  ...
  dispose(): void {
    ...
    console.log("[ChipMate New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ServerManager.killProcess(proc, "SIGTERM")
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[ChipMate New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
...
export function mermaidEndpoint(
  ...
  inherited = process.env.CHIPMATE_MERMAID_RENDER_ENDPOINT,
): RenderEndpoint {
  const injected = render.CHIPMATE_MERMAID_RENDER_ENDPOINT
...
export function plantumlEndpoint(
  render: Record<string, string> = renderEnv(),
  inherited = process.env.CHIPMATE_PLANTUML_RENDER_ENDPOINT,
): RenderEndpoint {
  const injected = render.CHIPMATE_PLANTUML_RENDER_ENDPOINT
  if (injected) return { state: "injected", endpoint: redactEndpoint(injected) }
  if (inherited) return { state: "inherited", endpoint: redactEndpoint(inherited) }
  return { state: "absent" }
}
...
function indexingControlEnv(internal: Record<string, string>): Record<string, string> {
  ...
  return {
    ...
    !process.env.CHIPMATE_INTERNAL_INDEXING_OPENAI_COMPATIBLE_BASE_URL
      ? { CHIPMATE_INTERNAL_INDEXING_OPENAI_COMPATIBLE_BASE_URL: openAICompatibleBaseUrl }
      : {}),
    ...(process.platform === "linux" &&
    Object.keys(internal).length > 0 &&
    !process.env.CHIPMATE_CODEGRAPH_WORKER_CONCURRENCY
