...
- [x] 本文件是 ChipMate Skill Market Web、Render Service 市场后端和 Kilo Marketplace 对齐工作的唯一主计划账本。
- [x] 实施目标仓库固定为 `/Users/archer/Work/kilocode`。
- [x] Render Service 目标目录固定为 `server/chipmate-word-render`。
- [x] Kilo 插件目标包固定为 `packages/kilo-vscode`。
...
- [x] Web 与 Kilo Marketplace 共享同一套身份、目录、版本、收藏、安装、发布、校验和分析数据。
- [x] 完全对齐指能力、数据、状态和结果对齐，不要求浏览器和 VS Code 视觉完全相同。
- [x] Web 使用 React + Vite 和 iOS Liquid Glass 空间感技能画廊。
- [x] Kilo 继续使用 SolidJS、kilo-ui、Codicons 和 VS Code 主题。
...
- [x] 旧版 Kilo 插件连接新服务端时，浏览、下载和安装 latest Skill 无回归。
- [x] 新版 Kilo 插件连接旧服务端时自动进入 legacy 模式，不显示失效按钮。
- [x] 同一用户在 Web 收藏 Skill 后，Kilo 能通过事件或缓存失效机制显示相同状态。
- [x] Kilo 安装、更新或卸载 Skill 后，Web 显示一致的安装状态和 revision。
- [x] Web 与 Kilo 对相同 Skill 上传输入返回相同 ValidationReport。
...
- [x] 首要用户是内网团队用户。
- [x] Web v1 覆盖发现与创作闭环。
- [x] Web 与 Kilo Marketplace 并存，共享能力和数据。
...
- [x] AI 修复只有在用户点击后才把必要内容发送给当前 Kilo Provider。
- [x] 自动发布门禁包含格式校验和安全静态扫描。
- [x] Web 详情包含净化后的 SKILL.md 和安全文件浏览。
- [x] Web 支持一键唤起 Kilo 安装，并提供归档下载兜底。
- [x] Web 提供只读服务状态页。
...
| 能力 | Web Market | Kilo Marketplace | 对齐要求 |
|---|---|---|---|
...
- [x] 从同一 OpenAPI 生成 Kilo Marketplace TypeScript 客户端。
- [x] 生成文件禁止手改。
...
- [x] Web session 与 Kilo Bearer key 必须映射到相同 MarketUser.id。
...
- [x] kilo.marketplace.baseUrl 和 kilo.marketplace.skillsOnly 不改名。
...
- [x] Kilo 使用当前 Provider 生成 AI 补丁。
- [x] 用户点击使用 AI 修复后才允许发送必要内容。
- [x] Web 语义校验失败时提供在 Kilo 中使用 AI 修复深链。
- [x] 深链只携带短期 publicationRunId，不携带 Skill 内容或 API key。
- [x] Kilo 验证任务所有权后获取必要片段。
- [x] AI 补丁逐文件显示并要求作者确认。
- [x] 服务端接收补丁后重新执行全部权威校验。
- [x] 验证通过后自动发布。
- [x] Kilo 不可用时，Web 仍提供完整手工修复说明。
...
- [x] Kilo 只接受与当前 marketplace base URL 同源的 intent。
- [x] Kilo 消费 intent 后取得 Skill ID、revision、SHA-256 和下载地址。
- [x] 用户确认项目或全局安装范围。
- [x] Kilo 未安装时，Web 提供归档下载和手动步骤。
...
## 14. Kilo Marketplace UI

- [x] 保留现有 SolidJS 和 MarketplacePanelProvider。
- [x] 使用 kilo-ui 组件，不在 Kilo Webview 引入 React。
...
- [x] 确认 Kilo Marketplace 当前 API、安装、上传和 URI Handler 契约。
...
- 用户在三套真实视觉稿中选择方向 1；Web 以空间画廊 Liquid Glass 为视觉目标，Kilo webview 继续遵循 VS Code 原生主题边界。
- 冻结现有 13 个 legacy API 入口、Kilo Marketplace 当前读取/上传/安装契约和扩展身份/配置名。
- 将 Git 状态、真实 DOCX、Mermaid、VSIX 和 Skill 归档固定为后续 Gate 的可复现证据源。

Commands Run:

- `find docs .kilo ...`：确认仓库已有 docs 计划账本惯例。
- `git status --short -- docs/chipmate-skill-market-kilo-alignment-plan.md`：创建前文件不存在。
- `bun run script/check-md-table-padding.ts docs/chipmate-skill-market-kilo-alignment-plan.md`：PASS，未发现 padded tables。
- `node -e '<plan structure check>'`：PASS，状态字段、G0、G9、legacy 契约、测试计划和 Decision Log 均存在。
- `git diff --no-index --check /dev/null docs/chipmate-skill-market-kilo-alignment-plan.md`：PASS，无空白错误。
...
- `packages/kilo-vscode/src/services/marketplace/generated/**`
...
- TypeBox schema 单一来源生成 OpenAPI 3.1、Web client 和 Kilo client；生成文件拆分为 API types 与 fetch client。
...
- `bun run typecheck`：PASS，Kilo extension 和 webview 编译通过。
- `bun test tests/unit/marketplace-generated-contract.test.ts`：PASS，1 pass。
- `cmp -s <web generated> <kilo generated>`：PASS，两端生成物字节一致。
...
- Workspace：7 pass；Kilo generated client：1 pass；TypeScript/ESLint/Vite/generate:check：PASS。
...
- `bun test tests/unit/marketplace-installer.test.ts`：PASS，实际 Kilo installer 9 pass。

Test Results:

- migration/WAL/FTS/revision/import/export/state/publication/events/immutable release：PASS。
- 实际 Kilo `MarketplaceInstaller` 从 Worker 生成目录下载并安装 latest：PASS。
- market DB 启动失败不阻塞 health/render：PASS。

Runtime Evidence:

- Environment：macOS、Node 24.18.0、真实 SQLite/Worker/文件系统/tar/Fastify/Kilo installer。
...
- TypeBox schema、生成的 Web/Kilo clients、G4 screenshots、`design-qa.md` 和 `g4/review.md`。
...
- `bun run typecheck && bun test tests/unit/marketplace-generated-contract.test.ts && bun run check-kilocode-change`
...
- [x] 实现 Web session。
- [x] 实现 Kilo Bearer principal。
- [x] 对齐稳定 MarketUser.id。
- [x] 实现收藏双端同步。
- [x] 实现 installation API。
- [x] 实现安装 intent。
- [x] 实现 Kilo URI Handler。
...
- [x] 实现版本发布和幂等。
- [x] 实现 Web 上传和发布进度。
- [x] 实现 Kilo 发布和详细 ValidationReport。
- [x] 实现 Kilo AI 修复。
- [x] 实现 Web 到 Kilo 的 AI 修复深链。
...
- [x] 验证 Linux VSIX 包含 kilo、ripgrep、Tree-sitter、LanceDB 与扩展运行 bundles，并在 x86_64 Linux VM 真实执行 `kilo --version` 与 `rg --version`。
...
- [x] 确认 Web `token-resolver-disabled` 与 Kilo“未验证”来自同一服务端 resolver 配置缺失。
- [x] 确认手动注入三项 `NEW_API_*` 后的 429 来自 New API 上游，而非 Kilo API Key 读取失败。
...
### G10：Kilo Skill Hub 本地导入与跨平台 Skill 兼容
...
- [x] G10.1 将 `@chipmate/skill-spec` 提升为 Kilo 扩展和 ChipMate Server 共享规范包。
...
- [x] G10.5 通过共享规范、Kilo 扩展、服务端、发布往返和真实 CLI Skill 发现门禁。
...
- `@chipmate/skill-spec` 成为 Render Service 与 Kilo 扩展的同一 workspace 依赖；纯文件校验、归档校验、Node 来源发现、YAML AST 修复和确定性 TAR.GZ 均由同一包导出。
...
- Kilo 不模拟 Claude hooks/model/context、Codex MCP 自动安装或 OpenCode 专属运行语义；这些字段只保真保存并显示兼容性提示。
...
- `packages/kilo-vscode/src/services/marketplace/local-import.ts`
- `packages/kilo-vscode/src/services/marketplace/local-import-registry.ts`
...
- `packages/kilo-vscode/src/services/marketplace/skills.ts`
- `packages/kilo-vscode/src/types/messages.ts`
- `packages/kilo-vscode/tests/unit/marketplace-local-import.test.ts`
- `packages/kilo-vscode/tests/unit/marketplace-registry.test.ts`
...
- `bun test ./tests/unit/marketplace-*.test.ts`，目录 `packages/kilo-vscode`
  - Result: `PASS`
...
- `packages/kilo-vscode/webview-ui/src/components/marketplace/LocalSkillImportDialog.tsx`
...
- `packages/kilo-vscode/webview-ui/src/components/marketplace/AlignedSkillMarket.tsx`
- `packages/kilo-vscode/webview-ui/src/components/marketplace/marketplace.css`
- `packages/kilo-vscode/webview-ui/src/stories/marketplace.stories.tsx`
- `packages/kilo-vscode/webview-ui/src/types/marketplace.ts`
- `packages/kilo-i18n/src/en.ts`、`packages/kilo-i18n/src/zh.ts`

Design Summary:

- 标题区新增本地导入主入口；六阶段弹窗覆盖来源、候选、兼容/安全、元数据修复、scope/冲突和结果，长操作提供阶段、取消与立即错误反馈。
- 使用 kilo-ui、Codicons、VS Code token、`currentColor` 和正常 flex/grid 流；Glass 内容采用通透分层、柔和高光、材质边界、内外阴影与 reduced-motion。
...
- 首版不提供浏览器端本地安装；Web 无权写入 Kilo Skill 目录。
...
- `packages/kilo-vscode/src/services/marketplace/upload.ts`
- `packages/kilo-vscode/src/services/marketplace/archive.ts`
...
- 旧服务端不声明发布能力时，新扩展隐藏远程发布，但本地导入与 Kilo 使用不受影响。
...
- 用当前源码启动真实 `kilo serve`，在同一服务进程运行期间写入项目 `.kilo/skills/g10-runtime-skill`，仅调用 instance dispose 失效机制后重新读取 `app.skills`。
...
- `bun run typecheck`、`bun run lint`、`bun run compile`、`bun run knip`、`bun run check-kilocode-change`
  - Result: `PASS`
...
- Kilo 本地导入即时加载、共享规范、扩展事务、发布往返、服务端兼容、Word/Mermaid 压测和 Liquid Glass 视觉 Review 均通过。
...
- CLI Skill Service 与 Kilo-owned HTTP handler：安全删除、两级缓存失效、回滚和内置保护。
...
- CLI 和扩展共同要求安装目录名与 Skill ID 匹配；工作区根、项目根、用户目录、Kilo 配置/数据根、文件系统根以及包含其他已发现 Skill 的父目录均不可删除。
...
- SHA-256：Skill Service `dee9aeb649bd`、Kilo handler `a72e8d546c6c`、KiloProvider `7fb9178c85db`、session context `515a2d626a4d`、MarketplacePanelProvider `088a747f723b`、local registry `c6472901d8ed`。

Commands Run:

- `packages/opencode: bun test ./test/kilocode/skill-removal.test.ts ./test/session/system.test.ts`
  - Result: `PASS`，5 tests、18 assertions；覆盖即时缓存刷新、任意路径/物化内置拒绝、工作区根保护和同名跨 scope 保留。
- `packages/opencode: bun run typecheck`
  - Result: `PASS`。
- `packages/kilo-vscode: targeted Marketplace/removal/message tests`
  - Result: `PASS`，21 tests、109 assertions；使用真实临时目录验证 token、旧 CLI fallback、目录删除、目录名约束、registry 清理和历史记录对账。
- `packages/kilo-vscode: bun run typecheck && bun run lint && bun run compile && bun run knip && bun run check-kilocode-change`
...
- `bun run script/check-opencode-annotations.ts`
  - Result: `BASELINE_FAIL`；失败仅来自当前分支已有的 `packages/opencode/script/build.ts`、provider 和 control/global handler 等提交内容，本次 `skill/index.ts` 新增内容均位于 `kilocode_change` block/inline marker，Kilo-owned handler/test 路径免标记。
...
- `packages/kilo-vscode: marketplace-installer + local removal tests`
...
- [x] bun run check-kilocode-change。
...
| 2026-07-12 | Web 与 Kilo 能力和数据完全对齐，视觉不强求一致 | 浏览器与 VS Code 的平台约束不同 | 共用契约，分别实现 React 和 SolidJS UI |
| 2026-07-12 | Web 使用 React + Vite | 独立浏览器产品需要成熟生态和丰富交互 | 不复用 Kilo SolidJS UI |
...
| 2026-07-12 | AI 修复由 Kilo 当前 Provider 执行 | Render Service 不保存模型配置或凭据 | Web 使用深链进入 Kilo 修复 |
...
| 2026-07-12 | TypeBox schema 作为 aligned-v1 单一契约来源 | Fastify、React 和 SolidJS 需要共享类型和状态，但不能共享 UI | 同一生成脚本产出 OpenAPI、Web client 和 Kilo client；生成文件禁止手改 |
...
| 2026-07-12 | G5 身份采用服务端解析、会话哈希与同一稳定假名用户 | Web 原始 key 不得持久化，Kilo 必须使用真实 auth store 的 Bearer key，双端状态必须由服务端权威映射 | Cookie 写操作使用 Origin/Host + CSRF；intent 哈希落盘、五分钟、一次性；globalStorage 仅保存假名化安装元数据 |
| 2026-07-12 | G6 使用服务端单一 PublicationRun 与权威 Skill 校验器 | Web 与 Kilo 必须对相同归档给出相同结果，自动修复和 AI 修复不得绕开安全复验 | 确定性修复只作用上传快照；AI 仅由当前 Kilo Provider 在双重确认后生成；revision 和归档不可变 |
| 2026-07-12 | G7 以 capabilities 驱动 Kilo Marketplace 信息架构 | internal skillsOnly、公共完整市场和旧服务端必须在同一扩展中安全共存 | aligned-v1 提供首页/详情/版本/收藏/安装/发布/分析/诊断；legacy 隐藏失效动作；Agent/MCP 保留 |
