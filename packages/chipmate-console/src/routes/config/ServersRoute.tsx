...
export function ServersRoute() {
  ...
  function add() {
    const server: Server = {
      id: `${url()}-${Date.now()}`,
      name: name().trim() || "ChipMate Server",
      url: url().trim(),
      dir: "Server default",
      status: "pending",
      apply: true,
  ...
  return (
    ...
    >
      <ConfigToolbar
        title="Server Sync"
        description="Register global ChipMate server endpoints and choose which ones receive configuration updates."
      ...
      <Card class="server-table-card">
        ...
        <div class="server-table" role="table" aria-label="Connected ChipMate servers">
