...
export class AgentManagerProvider implements Disposable {
  ...
  constructor(
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "chipmate.agent_manager.start",
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[ChipMate New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from ChipMateProvider when the sidebar sends "continueInWorktree".
   */
