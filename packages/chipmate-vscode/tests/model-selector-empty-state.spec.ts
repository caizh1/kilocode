...
test("legacy ChipMate Auto Free selection keeps its id but uses ChipMate branding", async ({ page }) => {
  ...
  await expect(page.getByText("ChipMate Auto Free", { exact: true })).toHaveCount(0)
})
