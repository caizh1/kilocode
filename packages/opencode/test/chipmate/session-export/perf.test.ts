...
describe("session export performance budget", () => {
  ...
  const enabled = process.env.CHIPMATE_SESSION_EXPORT_PERF === "1" && process.env.CI !== "true"
  ...
  test.skipIf(!enabled)("eligible beforeRequest p99 stays under 1 ms for 256 KB inputs", () => {
    ...
    for (let i = 0; i < 200; i++) {
      const start = performance.now()
      cap.beforeRequest({
        input: { model: { api: { npm: "@chipmate/chipmate-gateway" }, isFree: true }, org: { type: "personal" as const } },
