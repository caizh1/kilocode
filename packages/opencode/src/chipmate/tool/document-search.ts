...
import type { DocumentSearchResult } from "@chipmate/chipmate-indexing/engine"
  ...
  Effect.succeed({
    ...
    execute: (
      ...
      Effect.gen(function* () {
        ...
        if (!ChipMateIndexing.documentReady()) return unavailable(maxPackChars, undefined, ctx.agent)
        const status = yield* Effect.promise(() => ChipMateIndexing.current())
        ...
        const results = yield* Effect.promise(() =>
          ChipMateIndexing.searchDocuments(params.query, {
