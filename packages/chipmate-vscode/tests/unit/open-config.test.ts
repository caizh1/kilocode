...
import { globalFiles, localFiles } from "../../src/chipmate-provider/config-file"
import { openConfig } from "../../src/chipmate-provider/open-config"
...
const labels = {
  ...
  sourceHomeChipMate: "Home ChipMate",
  sourceHomeChipMate: "Home ChipMate",
  sourceHomeOpencode: "Home Opencode",
  sourceProjectChipMate: "Project ChipMate",
  sourceProjectChipMate: "Project ChipMate",
...
describe("config file discovery", () => {
  it("exposes only the ChipMate v2 global config target", async () => {
    ...
    expect(list[0]).toMatchObject({
      file: path.join(storage, "config", "chipmate.jsonc"),
      source: "sourceXdg",
      recommended: true,
    })
  })
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    ...
    await file(path.join(root, ".chipmate-v2", "chipmate.jsonc"))
    ...
    expect(list[0]?.file).toBe(path.join(root, ".chipmate-v2", "chipmate.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".chipmate-v2", "chipmate.jsonc")
    await file(cfg)
