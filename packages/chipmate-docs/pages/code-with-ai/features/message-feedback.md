...
description: "Rate assistant responses with thumbs up/down to help ChipMate improve"
---

# Message Feedback

You can give a thumbs up or thumbs down to any response ChipMate gives you. It's a quick way to tell us when something worked well, or when it didn't.
...
When ChipMate finishes a response, you'll see thumbs-up and thumbs-down buttons next to the **Copy response** button. Click one to send your rating. Click the same button again to take it back, or click the other one to change your mind.
...
Your feedback is the most direct way to tell us what's working and what isn't. We use it to tune the prompts behind ChipMate's behavior, pick better default models, and find and fix problems faster than we could on our own.
...
- For responses that came through ChipMate Gateway, the request ID so we can match the rating back to the session.

No further information is included in the rating feedback. For more information on telemetry, see [PRIVACY.md](https://github.com/ChipMate-Org/chipmate/blob/main/PRIVACY.md).
...
Set `CHIPMATE_TELEMETRY_LEVEL=off` in your environment before starting ChipMate:

```bash
CHIPMATE_TELEMETRY_LEVEL=off chipmate
```
