/**
 * Provider action handlers extracted from ChipMateProvider to stay under max-lines.
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose(opts).catch((error: unknown) => {
        console.warn(`[ChipMate New] ChipMateProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[ChipMate New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
