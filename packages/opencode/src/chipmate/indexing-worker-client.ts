...
import type { IndexingStatus } from "@chipmate/chipmate-indexing/status"
...
declare global {
  const CHIPMATE_INDEXING_PROCESS_PATH: string
}
  ...
  const worker = (directory: string, root: string, hooks: Hooks, options: Options = {}): Driver => {
    ...
    const task = Process.spawn(indexingCommand(), {
      ...
      env: {
        ...userEnv(process.env),
        ...selfEnv(),
        CHIPMATE_INDEXING_SOFT_RSS_BYTES: String(budget.soft),
        CHIPMATE_INDEXING_CRITICAL_RSS_BYTES: String(budget.critical),
        CHIPMATE_INDEXING_HARD_RSS_BYTES: String(budget.hard),
        CHIPMATE_INDEXING_RECOVERY_RSS_BYTES: String(budget.recovery),
        CHIPMATE_INDEXING_FORCED_LOW: options.forcedLow ? "1" : "0",
      },
      extendEnv: false,
    })
    ...
    return {
      init(config, baselineDirectory) {
        const request: Request = {
          ...
          id: id++,
          method: "init",
          input: { directory, root, config, baselineDirectory, lancedbPath: process.env.CHIPMATE_LANCEDB_PATH },
        }
...
export function indexingCommand(
  configured = process.env.CHIPMATE_INDEXING_PROCESS_PATH?.trim(),
  exec = process.execPath,
): string[] {
  const bundled = typeof CHIPMATE_INDEXING_PROCESS_PATH === "undefined" ? undefined : CHIPMATE_INDEXING_PROCESS_PATH
