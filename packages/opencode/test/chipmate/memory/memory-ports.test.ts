...
import { MemoryModel, MemorySession } from "../../../src/chipmate/memory/ports"
...
describe("memory ports", () => {
  test("session port extracts the latest turn, recall markers, and all assistant steps", async () => {
    ...
    const diffs = [
      {
        file: "packages/opencode/src/chipmate/memory/ports.ts",
        additions: 4,
        deletions: 1,
    ...
    expect(view?.recent).toContain("Tool chipmate_memory_recall completed")
