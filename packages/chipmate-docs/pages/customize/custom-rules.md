---
title: "Custom Rules"
description: "Define custom rules for ChipMate behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the ChipMate AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
ChipMate supports two types of custom rules:
...
Project rules are configured via the `instructions` key in your project's `chipmate.jsonc` file. You can edit this file directly or use the **Settings** webview to manage the `instructions` configuration. Each entry points to a file path or glob pattern:

```jsonc
// chipmate.jsonc
{
  "instructions": [".chipmate/rules/formatting.md", ".chipmate/rules/*.md"],
}
```

You can also place rule files in the **`.chipmate/`** directory structure:
...
Global rules are configured via the `instructions` key in your global `chipmate.jsonc` config file (typically at `~/.config/chipmate/chipmate.jsonc`).

{% callout type="note" title="Migration" %}
The extension is backward compatible with `.chipmate/rules/` directories. Existing rules will continue to work, but migrating to `chipmate.jsonc` is recommended.
{% /callout %}
...
Project rules are configured via the `instructions` key in your project's `chipmate.jsonc` file. Each entry points to a file path or glob pattern:
...
{% callout type="note" title="Migration" %}
The CLI is backward compatible with `.chipmate/rules/` directories. Existing rules will continue to work, but migrating to `chipmate.jsonc` is recommended.
{% /callout %}
...
Rules are managed by editing the `instructions` array in your `chipmate.jsonc` config file. You can also use the **Settings** webview in VS Code to edit the configuration.

- **Add a rule**: Add a file path or glob pattern to the `instructions` array
- **Remove a rule**: Remove the entry from the array
- **Disable a rule temporarily**: Comment out the line in `chipmate.jsonc` (JSONC supports `//` comments)

```jsonc
// chipmate.jsonc
{
  "instructions": [
    ".chipmate/rules/formatting.md",
    // ".chipmate/rules/experimental.md"  -- temporarily disabled
    ".chipmate/rules/naming_conventions.md",
  ],
}
```
...
Rules are managed by editing the `instructions` array in your `chipmate.jsonc` config file directly.
...
Rules are loaded in the order they appear in the `instructions` array in `chipmate.jsonc`:

1. **Global instructions** from the global `chipmate.jsonc` config
2. **Project instructions** from the project's `chipmate.jsonc`
...
If `.chipmate/rules/` directories exist in your project, their contents are automatically included for backward compatibility. To fully migrate, move your rule files and reference them in `chipmate.jsonc`.
{% /callout %}
...
1. Create a `chipmate.jsonc` file in your project root (if it doesn't exist)
2. Create a `.chipmate/rules/` directory (or any directory you prefer)
3. Write your rule as a Markdown file in that directory
4. Add the file path or a glob pattern to the `instructions` array in `chipmate.jsonc`

```jsonc
// chipmate.jsonc
{
  "instructions": [".chipmate/rules/my-new-rule.md"],
}
```

Rules are applied on the next interaction. You can also edit `chipmate.jsonc` through the **Settings** webview in VS Code.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="ChipMate ignores request to read sensitive file" width="600" /%}
