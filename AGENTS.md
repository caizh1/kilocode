...
- After packaging the Windows baseline VSIX, create or update `latest.json` in the repo root for the internal update check manifest. It must use `publisher: "chipmate"`, `name: "chipmate"`, `version: "<version>"`, `vsix: "chipmate-<version>-win32-x64-baseline.vsix"`, and `sha256` from `shasum -a 256 chipmate-<version>-win32-x64-baseline.vsix`.
- Create a `.tar.gz` transfer archive only for the Windows baseline VSIX and `latest.json`. From the repo root, run `tar -czf chipmate-<version>-win32-x64-baseline.vsix.tar.gz chipmate-<version>-win32-x64-baseline.vsix latest.json`; the archive must contain only those two files. Do not compress the macOS VSIX. Verify with `tar -tzf chipmate-<version>-win32-x64-baseline.vsix.tar.gz`.
