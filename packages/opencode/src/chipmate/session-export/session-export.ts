...
declare global {
  const CHIPMATE_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof CHIPMATE_SESSION_EXPORT_WORKER_PATH !== "undefined") return CHIPMATE_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.CHIPMATE_FEATURE?.trim() || "unknown"
}
