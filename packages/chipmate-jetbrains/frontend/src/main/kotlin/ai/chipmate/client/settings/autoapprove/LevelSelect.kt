...
internal fun levelLabel(level: String): String = when (level) {
    "allow" -> ChipMateBundle.message("settings.autoApprove.level.allow")
    "ask" -> ChipMateBundle.message("settings.autoApprove.level.ask")
    "deny" -> ChipMateBundle.message("settings.autoApprove.level.deny")
    else -> level
}
...
internal class LevelSelect(
    ...
    init {
        renderer = SimpleListCellRenderer.create("") { item ->
            when (item) {
                is Item.Default -> ChipMateBundle.message("settings.autoApprove.default", levelLabel(item.resolved))
