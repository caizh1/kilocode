...
import ai.chipmate.client.session.ui.prompt.SendPromptContext
...
class SendPromptActionTest : BasePlatformTestCase() {
    fun `test action invokes prompt context`() {
        ...
        assertEquals("Send the current ChipMate prompt", action.templatePresentation.description)
    }
    ...
    fun `test frontend descriptor registers send prompt shortcuts`() {
        ...
        assertTrue(xml.contains("id=\"ChipMate.SendPrompt\""))
