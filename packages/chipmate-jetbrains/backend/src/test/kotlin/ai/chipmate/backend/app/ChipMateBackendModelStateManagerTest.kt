...
class ChipMateBackendModelStateManagerTest {
    private val mock = MockCliServer()
    private val log = TestLog()
    private val dir = createTempDirectory("chipmate-model-state-test")
    private val http = ChipMateBackendHttpClients.api(mock.password)
    ...
    fun `state loads favorites from cli model json`() = runBlocking {
        val port = start()
        dir.resolve("model.json").writeText("""{"favorite":[{"providerID":"chipmate","modelID":"auto"}],"recent":[{"providerID":"anthropic","modelID":"claude"}],"model":{"code":{"providerID":"openai","modelID":"gpt"}},"variant":{"openai/gpt":"high"}}""")
        val mgr = ChipMateBackendModelStateManager(log)
        mgr.start(http, port)

        val state = mgr.state()

        assertEquals(1, state.favorite.size)
        assertEquals("chipmate", state.favorite[0].providerID)
    ...
    fun `favorite update writes parser built model json`() = runBlocking {
        ...
        dir.resolve("model.json").writeText(
            """{"model":{"code":{"providerID":"chipmate","modelID":"auto"}},"recent":[{"providerID":"openai","modelID":"gpt"}],"variant":{"chipmate/auto":"fast"},"favorite":[]}""",
        )
        val mgr = ChipMateBackendModelStateManager(log)
        mgr.start(http, port)
    ...
    fun `selection update writes model json`() = runBlocking {
        ...
        val state = mgr.selection(ModelSelectionUpdateDto("code", "chipmate", "auto"))
        val raw = dir.resolve("model.json").readText()
    ...
    fun `clear selection removes agent model`() = runBlocking {
        val port = start()
        dir.resolve("model.json").writeText("""{"model":{"code":{"providerID":"chipmate","modelID":"auto"},"plan":{"providerID":"openai","modelID":"gpt"}}}""")
        val mgr = ChipMateBackendModelStateManager(log)
        mgr.start(http, port)
    ...
    fun `variant update writes model json`() = runBlocking {
        ...
        val state = mgr.variant(ModelVariantUpdateDto("chipmate/auto", "medium"))
