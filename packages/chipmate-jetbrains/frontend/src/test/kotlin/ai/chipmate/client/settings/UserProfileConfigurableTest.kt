...
import ai.chipmate.client.settings.profile.ProfileUi
import ai.chipmate.client.settings.profile.formatResetDate
import ai.chipmate.client.settings.profile.formatShortBalance
import ai.chipmate.client.testing.FakeAppRpcApi
import ai.chipmate.client.testing.TestCoroutines
...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = ChipMateAppService(coroutines.scope, rpc)
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY)
        edt {
            panel = ProfileUi(
                profile = null,
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with ChipMate" }.doClick()
        }
        flush()
        ...
        assertEquals(listOf("https://auth.chipmate.ai/device"), urls)
    }
    ...
    fun `test logout updates profile UI`() {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, ChipMateAppStatusDto.READY) }
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with ChipMate" })
        }
    }
    ...
    fun `test organization switch updates balance UI`() {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = personal)
        edt { panel.update(personal, ChipMateAppStatusDto.READY) }
    ...
    fun `test logged in profile uses compact stack and copyable email`() {
        ...
        edt {
            ...
            layout(panel)
            val logo = labelsByName(panel, "chipmate.profile.logo.loggedIn").single()
    ...
    fun `test personal profile shows chipmate pass usage`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
            chipmatePass = ProfileChipMatePassDto(
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("ChipMate Pass"))
            ...
            assertTrue("pass meter should be visible", panelsByName(panel, "chipmate.profile.passPanel").single().isVisible)
            assertFalse(t, t.contains("Get ChipMate Pass"))
        }
    }

    fun `test chipmate pass short amounts use half up rounding`() {
        assertEquals("$3", formatShortBalance(2.5))
        assertEquals("$2", formatShortBalance(2.49))
    }

    fun `test chipmate pass hides empty bonus and invalid renewal rows`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val invalid = profile.copy(
            chipmatePass = profile.chipmatePass?.copy(
                currentPeriodBonusCreditsUsd = 0.0,
                nextBillingAt = "not-a-date",
            ),
        )
        edt { panel.update(invalid, ChipMateAppStatusDto.READY) }
        ...
        val none = invalid.copy(chipmatePass = invalid.chipmatePass?.copy(nextBillingAt = null))
        edt { panel.update(none, ChipMateAppStatusDto.READY) }
    ...
    fun `test chipmate pass meter clamps base zero and avoids noop repaint`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val meter = edt { componentsByName(panel, "chipmate.profile.passMeter").single() }
        ...
        edt { panel.update(full, ChipMateAppStatusDto.READY) }
    ...
    fun `test personal profile shows chipmate pass without balance`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            chipmatePass = ProfileChipMatePassDto(
        ...
        val updated = profile.copy(
            chipmatePass = profile.chipmatePass?.copy(currentPeriodUsageUsd = 88.0),
        )
        ...
        edt {
            ...
            assertTrue("pass panel should be visible", panelsByName(panel, "chipmate.profile.passPanel").single().isVisible)
    ...
    fun `test personal profile without chipmate pass shows subscribe link`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(10.0),
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Get ChipMate Pass to add credits and earn bonuses"))
            buttons(panel).first { it.text == "Dashboard" }.doClick()
            buttons(panel).first { it.text == "Top up" }.doClick()
            buttons(panel).first { it.text == "Get ChipMate Pass to add credits and earn bonuses" }.doClick()
        }

        assertEquals(listOf("https://app.chipmate.ai/profile", "https://app.chipmate.ai/credits", "https://chipmate.ai/pricing/chipmate-pass"), urls)
    }

    fun `test chipmate pass renewal date uses utc`() {
    ...
    fun `test org profile hides chipmate pass`() {
        val orgs = listOf(ProfileOrganizationDto(id = "org_1", name = "Acme", role = "ADMIN"))
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
        ...
        edt {
            val t = text(panel)
            assertFalse(t, t.contains("ChipMate Pass"))
            assertFalse(t, t.contains("Get ChipMate Pass"))
        }
    }
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with ChipMate" }
            panel.update(null, ChipMateAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with ChipMate" }
            assertSame(btn, btn2)
        }
    }

    fun `test logged out profile shows chipmate icon above login content`() {
        edt {
            panel.update(null, ChipMateAppStatusDto.READY)
            panel.setSize(800, 600)
            layout(panel)

            val logo = labelsByName(panel, "chipmate.profile.logo.loggedOut").single()
    ...
    fun `test account update retains name label`() {
        ...
        edt {
            panel.update(alice, ChipMateAppStatusDto.READY)
            val lbl = labels(panel).first { it.text == "Alice" }
            panel.update(bob, ChipMateAppStatusDto.READY)
    ...
    fun `test organization switch keeps account visible during transient null profile`() {
        ...
        val transientState = ChipMateAppStateDto(
            status = ChipMateAppStatusDto.READY,
            profile = null,
        ...
        edt {
            panel.update(personal, ChipMateAppStatusDto.READY)
    ...
    fun `test login shows device auth card before completion`() {
        ...
        flushUntil { text(panel).contains("Sign in to ChipMate") }

        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sign in to ChipMate"))
            assertTrue(t, t.contains("Step 1:"))
            assertTrue(t, t.contains("Open this URL"))
            assertTrue(t, t.contains("https://auth.chipmate.ai/device"))
        ...
        edt {
            val qr = labelsByName(panel, "chipmate.login.qr").firstOrNull()
            assertNotNull(qr)
    ...
    fun `test cancel login invalidates stale completion`() {
        ...
        edt { buttons(panel).first { it.text == "Login with ChipMate" }.doClick() }
        flushUntil { text(panel).contains("Sign in to ChipMate") }
    ...
    fun `test auth card retains qr label across sync`() {
        ...
        val qrBefore = edt { labelsByName(panel, "chipmate.login.qr").firstOrNull() }
        assertNotNull(qrBefore)

        // Force another sync call while still pending
        edt { panel.update(null, ChipMateAppStatusDto.READY) }
        flush()

        val qrAfter = edt { labelsByName(panel, "chipmate.login.qr").firstOrNull() }
    ...
    fun `test url field selects all on click`() {
        ...
        edt {
            val field = fieldsByName(panel, "chipmate.login.url").firstOrNull()
    ...
    fun `test balance card has card background`() {
        ...
        edt {
            val card = panelsByName(panel, "chipmate.profile.balanceCard").firstOrNull()
    ...
    fun `test code panel has card background`() {
        ...
        edt {
            val codePanel = panelsByName(panel, "chipmate.login.codePanel").firstOrNull()
    ...
    fun `test combo model not rebuilt on balance change with same org list`() {
        ...
        edt { panel.update(updated, ChipMateAppStatusDto.READY) }
    ...
    fun `test combo model updated in place when org list changes`() {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = profile1)
        edt { panel.update(profile1, ChipMateAppStatusDto.READY) }
        ...
        edt { panel.update(profile2, ChipMateAppStatusDto.READY) }
    ...
    fun `test profile update does not trigger organization rpc`() {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, ChipMateAppStatusDto.READY) }
        edt { panel.update(profile.copy(currentOrgId = "org_1"), ChipMateAppStatusDto.READY) }
        flush()
    ...
    fun `test connecting while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, ChipMateAppStatusDto.CONNECTING) }
    ...
    fun `test loading while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, ChipMateAppStatusDto.LOADING) }
    ...
    fun `test loading with null profile while logged in does not crash`() {
        ...
        edt { panel.update(ChipMateAppStateDto(ChipMateAppStatusDto.LOADING)) }
        ...
        edt { panel.update(switched, ChipMateAppStatusDto.READY) }
    ...
    fun `test connecting while logged in with org selected keeps combo selection`() {
        ...
        app._state.value = ChipMateAppStateDto(ChipMateAppStatusDto.READY, profile = profile)
        edt {
            panel.update(profile, ChipMateAppStatusDto.READY)
            combos(panel).single().selectedIndex = 1
        }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with ChipMate" }
    ...
    fun `test device auth without code hides code panel and step2 label`() {
        ...
        rpc.fakeDeviceAuth = DeviceAuthDto(code = null, verificationUrl = "https://auth.chipmate.ai/device")
