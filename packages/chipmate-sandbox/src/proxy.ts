...
export const CurrentProxy = Context.Reference<ProxyRuntime | undefined>("@chipmate/sandbox/CurrentProxy", {
...
export const CurrentProxyFactory = Context.Reference<ProxyFactory>("@chipmate/sandbox/CurrentProxyFactory", {
...
export async function startProxy(
  ...
  server.on("connect", (request, client, head) => {
    ...
    if (!authenticate(request.headers["proxy-authorization"], token)) {
      client.end('HTTP/1.1 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic realm="chipmate"\r\n\r\n')
      return
    }
    try {
  ...
  server.on("request", async (request, response) => {
    if (!authenticate(request.headers["proxy-authorization"], token)) {
      response.writeHead(407, { "Proxy-Authenticate": 'Basic realm="chipmate"' })
      response.end()
      return
    }
    try {
  ...
  const dir = platform === "linux" ? await mkdtemp(path.join(os.tmpdir(), "chipmate-sandbox-proxy-")) : undefined
  ...
  const url = `http://chipmate:${encodeURIComponent(token)}@127.0.0.1:${port}`
  return {
    url,
    token,
    allowedHosts,
